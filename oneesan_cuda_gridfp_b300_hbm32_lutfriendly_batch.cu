...
int main(int argc,char**argv){
    ...
    auto lut_score_host=[&](const WindowPlan& wp){
        uint32_t f=0;for(int q:wp.fixed_pos)f|=1u<<q;int sc=0;
        if constexpr(LOW_LUT_K>0){uint32_t lm=(1u<<LOW_LUT_K)-1u;if((f&lm)==lm)sc+=100;}
        if constexpr(HIGH_LUT_K>0){uint32_t hm=((1u<<HIGH_LUT_K)-1u)<<(W-HIGH_LUT_K);uint32_t wm=(W==32)?0xffffffffu:((1u<<W)-1u);if((f&hm)==hm && (f&(wm^hm))==0)sc+=100;}
        sc += (wp.p_hi-wp.p_lo+1);return sc;
    };
    int threads=256,maxgroups=0;auto prep0=std::chrono::steady_clock::now();std::vector<PreparedWindow> schedule;
    for(int hi=W-1;hi>=1;){WindowPlan wp;bool found=false;int bestScore=-1;for(int lo=std::max(1,hi-max_window+1);lo<=hi;++lo){auto t=plan_window(W,hi,lo,target);if(t.max_bytes&&t.max_bytes<=target){int sc=lut_score_host(t);if(sc>bestScore){wp=std::move(t);bestScore=sc;found=true;}}}if(!found){std::cerr<<"cannot fit window hi="<<hi<<"\n";return 4;}int k=wp.fixed_pos.size();int nj=1<<k;maxgroups=std::max(maxgroups,nj);PreparedWindow pw;pw.wp=wp;pw.groups.reserve(nj);for(int g=0;g<nj;++g)pw.groups.push_back(prepare_group(W,pw.wp,g,mc,bc,ng));std::sort(pw.groups.begin(),pw.groups.end(),[](auto const&a,auto const&b){return a.work>b.work;});std::cerr<<"chosen window p="<<wp.p_hi<<".."<<wp.p_lo<<" groups="<<nj<<" lut_score="<<bestScore<<"\n";schedule.push_back(std::move(pw));hi=wp.p_lo-1;}
