...
struct MappedCounts {
    ...
    void open_file(const std::string& fn,size_t count){
        ...
        if(q==MAP_FAILED){perror("anonymous mmap");std::exit(2);}
        p=(Count*)q;
        madvise(p,bytes,MADV_HUGEPAGE);
    }
    void close_file(){ if(p){munmap(p,bytes);p=nullptr;} }
...
#pragma omp parallel for schedule(static)
    for(long long i=0;i<(long long)iv.size();++i){ const auto& x=iv[(size_t)i]; std::memcpy(local.data()+x.local,global+x.global,size_t(x.len)*sizeof(Count)); }
}
static void scatter(Count* global,const std::vector<Interval>& iv,const std::vector<Count>& local){
#pragma omp parallel for schedule(static)
    for(long long i=0;i<(long long)iv.size();++i){ const auto& x=iv[(size_t)i]; std::memcpy(global+x.global,local.data()+x.local,size_t(x.len)*sizeof(Count)); }
}
...
int main(int argc,char**argv){
    ...
    std::cout<<"backend=gridfp-host32-omp n="<<n<<" residue="<<ans<<" modulus="<<mod<<" main_states="<<mainN<<" blocked_states="<<blockN<<" external_bytes="<<size_t(mainN+blockN)*sizeof(Count)<<" max_group_main="<<maxGM<<" max_group_blocked="<<maxGD<<" max_vram_bytes="<<size_t(2*maxGM+maxGD)*sizeof(Count)<<" max_intervals="<<maxIntervals<<" windows="<<total_windows<<" max_groups="<<max_groups<<" max_window_len="<<max_window_len<<" target_mib="<<target_mib<<" max_window_cfg="<<max_window<<" transfer_gib="<<transferred/(1ull<<30)<<" gpu_ms="<<ms<<"\n";
