...
// state: 0=empty, 1=writer owns slot, 2=ready.
__device__ static inline void hash_insert(
    ...
    uint32_t* occupied, uint32_t* occupied_n, uint32_t mask) {
    ...
    for (uint32_t probe = 0; probe <= mask; ++probe, slot = (slot + 1) & mask) {
        ...
        if (s == 0) {
            ...
            if (st.compare_exchange_strong(expected, 1u, cuda::memory_order_acq_rel)) {
                table_keys[slot] = key;
                table_vals[slot] = value;
                uint32_t oi = atomicAdd(occupied_n, 1u);
                occupied[oi] = slot;
                st.store(2u, cuda::memory_order_release);
                return;
            }
            s = expected;
        }
        if (s == 1) {
            do { s = st.load(cuda::memory_order_acquire); } while (s == 1);
        }
        if (s == 2 && table_keys[slot] == key) {
...
__global__ void expand_hash_kernel(
    ...
    if (transition_one(keys[i], m, 0, out))
        hash_insert(out, vals[i], mod, table_keys, table_vals, table_state, occupied, occupied_n, mask);
...
int main(int argc, char** argv) {
    ...
    for (int ei=0; ei<(int)edges.size(); ++ei) {
        ...
        size_t cap = next_pow2(std::max<size_t>(1024, in_n * 4));
        ...
        ck(cudaMemset(table_state, 0, allocated_cap*sizeof(unsigned int)), "clear state");
        ...
        expand_hash_kernel<<<blocks,threads>>>(
            ...
            table_keys, table_vals, table_state, occupied, occupied_n, static_cast<uint32_t>(cap-1));
