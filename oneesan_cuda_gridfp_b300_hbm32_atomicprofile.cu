...
__device__ unsigned long long D_ATOM_TOTAL=0,D_ATOM_UNIQUE=0;
__device__ __forceinline__ void atomic_add_mod(Count*p,Count v){if(!v)return;unsigned mask=__activemask();unsigned grp=__match_any_sync(mask,(unsigned long long)(uintptr_t)p);int lane=threadIdx.x&31;atomicAdd(&D_ATOM_TOTAL,1ULL);if(lane==(__ffs(grp)-1))atomicAdd(&D_ATOM_UNIQUE,1ULL);Count mod=D_MOD;Count old=atomicCAS(p,0u,0u);for(;;){Count neu=(old>=mod-v)?old-(mod-v):old+v;Count seen=atomicCAS(p,old,neu);if(seen==old)return;old=seen;}}
...
int main(int argc,char**argv){int n=argc>1?std::atoi(argv[1]):16;unsigned long long raw_mod=argc>2?std::strtoull(argv[2],nullptr,10):4294967291ULL;if(raw_mod<2||raw_mod>0xffffffffULL){std::cerr<<"HBM32 modulus must be in [2, 4294967295], got "<<raw_mod<<"\n";return 1;}Count mod=(Count)raw_mod;int target_mib=argc>3?std::atoi(argv[3]):16384;int max_window=argc>4?std::atoi(argv[4]):14;int requested=argc>5?std::atoi(argv[5]):0;int W=n+1;if(n<2||W>MAXW){std::cerr<<"n=2..27\n";return 1;}if(W!=TARGET_W){std::cerr<<"specialized for width "<<TARGET_W<<" (n="<<(TARGET_W-1)<<")\n";return 1;}build_full_dp();int visible=0;ck(cudaGetDeviceCount(&visible),"count");int ng=requested<=0?visible:std::min(requested,visible);if(ng<1||ng>MAXGPU){std::cerr<<"need 1..8 GPUs\n";return 2;}
    ...
    unsigned long long zatom=0;for(int d=0;d<ng;++d){cudaSetDevice(d);ck(cudaMemcpyToSymbol(D_ATOM_TOTAL,&zatom,sizeof(zatom)),"zero atom total");ck(cudaMemcpyToSymbol(D_ATOM_UNIQUE,&zatom,sizeof(zatom)),"zero atom unique");}
    ...
    unsigned long long atomT=0,atomU=0;for(int d=0;d<ng;++d){unsigned long long a=0,b=0;cudaSetDevice(d);ck(cudaMemcpyFromSymbol(&a,D_ATOM_TOTAL,sizeof(a)),"atom total");ck(cudaMemcpyFromSymbol(&b,D_ATOM_UNIQUE,sizeof(b)),"atom unique");atomT+=a;atomU+=b;}std::cerr<<"atomic_profile total="<<atomT<<" unique="<<atomU<<" ratio="<<(atomT?double(atomU)/atomT:0)<<"\n";
