...
SRC="${SRC:-}"
if [[ -z "$SRC" ]]; then
  if (( N >= 27 )); then SRC=oneesan_cuda_gridfp_b300_hbm32_forced2window_opt_batch.cu; else SRC=oneesan_cuda_gridfp_b300_hbm32_fullmate_dropN_batch.cu; fi
fi
OUT="${OUT:-oneesan_cuda_gridfp_b300_hbm32_batch_n${N}}"
LOW_LUT_K="${LOW_LUT_K:-}"
