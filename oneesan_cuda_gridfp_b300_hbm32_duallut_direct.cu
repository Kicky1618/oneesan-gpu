...
__constant__ uint32_t* D_HIGH_DIRECT_MAIN;
__constant__ uint32_t* D_HIGH_DIRECT_BLOCK;
...
struct HighTablesHost{std::vector<HighEntry> entries;std::vector<uint32_t> offsets,direct;};
...
static uint32_t high_direct_key(uint32_t code){uint32_t key=0;for(int p=HIGH_LUT_K-1;p>=0;--p){uint32_t v=(code>>(2*p))&3u;key=key*3u+(v==uint32_t(R)?1u:(v==uint32_t(L)?2u:0u));}return key;}
static HighTablesHost build_high_tables(int width){
    HighTablesHost t;if constexpr(HIGH_LUT_K==0)return t;
    constexpr uint32_t NM=1u<<HIGH_LUT_K;int lowlen=width-HIGH_LUT_K;uint32_t n3=1;for(int i=0;i<HIGH_LUT_K;++i)n3*=3u;
    t.offsets.resize(NM+1);t.direct.assign(n3,0xffffffffu);
    for(uint32_t mask=0;mask<NM;++mask){size_t a=t.entries.size();t.offsets[mask]=(uint32_t)a;uint64_t base=0;high_enum_rec(HIGH_LUT_K-1,1,mask,0,lowlen,base,t.entries);for(size_t i=a;i<t.entries.size();++i)t.direct[high_direct_key(t.entries[i].code)]=t.entries[i].base;}t.offsets[NM]=(uint32_t)t.entries.size();
    std::cerr<<"high_lut width="<<width<<" K="<<HIGH_LUT_K<<" entries="<<t.entries.size()<<" entries_mib="<<double(t.entries.size()*sizeof(HighEntry))/(1<<20)<<" direct_mib="<<double(t.direct.size()*sizeof(uint32_t))/(1<<20)<<"\n";return t;
}
...
__device__ __forceinline__ Code rank_high_lut(MateID m,uint32_t occ){
    ...
    uint32_t key=0;
#pragma unroll
    for(int p=HIGH_LUT_K-1;p>=0;--p){uint32_t v=(code>>(2*p))&3u;key=key*3u+(v==uint32_t(R)?1u:(v==uint32_t(L)?2u:0u));}
    uint32_t* direct = WIDTH==TARGET_W ? D_HIGH_DIRECT_MAIN : D_HIGH_DIRECT_BLOCK;
    Code r=direct[key];
    int h=high_final_height<WIDTH>(code);
    constexpr MateID LM=(MateID(1)<<(2*LOW))-1;
    return r+rank_free_suffix<WIDTH>(m&LM,LOW,h);
}
template<int WIDTH>
...
int main(int argc,char**argv){int n=argc>1?std::atoi(argv[1]):16;unsigned long long raw_mod=argc>2?std::strtoull(argv[2],nullptr,10):4294967291ULL;if(raw_mod<2||raw_mod>0xffffffffULL){std::cerr<<"HBM32 modulus must be in [2, 4294967295], got "<<raw_mod<<"\n";return 1;}Count mod=(Count)raw_mod;int target_mib=argc>3?std::atoi(argv[3]):16384;int max_window=argc>4?std::atoi(argv[4]):14;int requested=argc>5?std::atoi(argv[5]):0;int W=n+1;if(n<2||W>MAXW){std::cerr<<"n=2..27\n";return 1;}if(W!=TARGET_W){std::cerr<<"specialized for width "<<TARGET_W<<" (n="<<(TARGET_W-1)<<")\n";return 1;}build_full_dp();int visible=0;ck(cudaGetDeviceCount(&visible),"count");int ng=requested<=0?visible:std::min(requested,visible);if(ng<1||ng>MAXGPU){std::cerr<<"need 1..8 GPUs\n";return 2;}
    ...
    HighTablesHost highM=build_high_tables(W),highB=build_high_tables(W-1);HighEntry* highEM[MAXGPU]{},*highEB[MAXGPU]{};uint32_t* highOM[MAXGPU]{},*highOB[MAXGPU]{},*highDM[MAXGPU]{},*highDB[MAXGPU]{};
    if constexpr(HIGH_LUT_K>0){for(int d=0;d<ng;++d){cudaSetDevice(d);ck(cudaMalloc(&highEM[d],highM.entries.size()*sizeof(HighEntry)),"high main entries");ck(cudaMemcpy(highEM[d],highM.entries.data(),highM.entries.size()*sizeof(HighEntry),cudaMemcpyHostToDevice),"high main entries copy");ck(cudaMalloc(&highEB[d],highB.entries.size()*sizeof(HighEntry)),"high block entries");ck(cudaMemcpy(highEB[d],highB.entries.data(),highB.entries.size()*sizeof(HighEntry),cudaMemcpyHostToDevice),"high block entries copy");ck(cudaMalloc(&highOM[d],highM.offsets.size()*sizeof(uint32_t)),"high main offsets");ck(cudaMemcpy(highOM[d],highM.offsets.data(),highM.offsets.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"high main offsets copy");ck(cudaMalloc(&highOB[d],highB.offsets.size()*sizeof(uint32_t)),"high block offsets");ck(cudaMemcpy(highOB[d],highB.offsets.data(),highB.offsets.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"high block offsets copy");ck(cudaMemcpyToSymbol(D_HIGH_ENTRIES_MAIN,&highEM[d],sizeof(highEM[d])),"high main entries ptr");ck(cudaMemcpyToSymbol(D_HIGH_ENTRIES_BLOCK,&highEB[d],sizeof(highEB[d])),"high block entries ptr");ck(cudaMemcpyToSymbol(D_HIGH_OFFSETS_MAIN,&highOM[d],sizeof(highOM[d])),"high main offsets ptr");ck(cudaMemcpyToSymbol(D_HIGH_OFFSETS_BLOCK,&highOB[d],sizeof(highOB[d])),"high block offsets ptr");ck(cudaMalloc(&highDM[d],highM.direct.size()*sizeof(uint32_t)),"high main direct");ck(cudaMemcpy(highDM[d],highM.direct.data(),highM.direct.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"high main direct copy");ck(cudaMalloc(&highDB[d],highB.direct.size()*sizeof(uint32_t)),"high block direct");ck(cudaMemcpy(highDB[d],highB.direct.data(),highB.direct.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"high block direct copy");ck(cudaMemcpyToSymbol(D_HIGH_DIRECT_MAIN,&highDM[d],sizeof(highDM[d])),"high main direct ptr");ck(cudaMemcpyToSymbol(D_HIGH_DIRECT_BLOCK,&highDB[d],sizeof(highDB[d])),"high block direct ptr");}}
