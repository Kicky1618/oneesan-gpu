...
static uint64_t host_mix64(uint64_t x) {
    x ^= x >> 30; x *= 0xbf58476d1ce4e5b9ULL;
    x ^= x >> 27; x *= 0x94d049bb133111ebULL;
    return x ^ (x >> 31);
}
static std::pair<uint64_t,uint64_t> host_fingerprint(const Key128* dkeys, size_t n) {
    std::vector<Key128> h(n);
    ck(cudaMemcpy(h.data(), dkeys, n*sizeof(Key128), cudaMemcpyDeviceToHost), "copy fingerprint keys");
    uint64_t x=0, s=0;
    for (auto k: h) {
        uint64_t q=host_mix64(k.lo ^ host_mix64(k.hi + 0x9e3779b97f4a7c15ULL));
        x ^= q; s += q;
    }
    return {x,s};
}
...
int main(int argc, char** argv) {
    ...
    for (int ei=0; ei<(int)edges.size(); ++ei) {
        ...
        if ((ei+1)%w==0 || ei+1==(int)edges.size()) {
            auto fp = host_fingerprint(cur_keys, in_n);
            std::cerr << "edge " << ei+1 << "/" << edges.size() << " frontier=" << after.size() << " states=" << rn
                      << " fp=" << std::hex << fp.first << ":" << fp.second << std::dec << "\n";
        }
    }
