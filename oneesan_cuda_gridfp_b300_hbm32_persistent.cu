...
__global__ void gather_main_kernel(Count*out,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){MateID m=unrank_group(i,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);out[i]=global_load_main(rank_full_dev(m,D_MAIN_W));}}
__global__ void gather_block_kernel(Count*out,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){MateID m=unrank_group(i,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);out[i]=global_load_block(rank_full_dev(m,D_BLOCK_W));}}
__global__ void scatter_main_kernel(const Count*in,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){MateID m=unrank_group(i,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);global_store_main(rank_full_dev(m,D_MAIN_W),in[i]);}}
__global__ void scatter_block_kernel(const Count*in,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){MateID m=unrank_group(i,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);global_store_block(rank_full_dev(m,D_BLOCK_W),in[i]);}}
...
__global__ void blocked_group_kernel(const Count*in,Code n,Count*out_main,int p){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID sm=unrank_group(i,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);MateID t=minsert(sm,p,N);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(out_main+j,c);}}
__global__ void main_group_kernel(const Count*in,Code n,Count*out_main,Count*out_block,int p){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=unrank_group(i,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,p);switch(w){case NN:{MateID t=msetpair(m,p,LR);atomic_add_mod(out_main+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}case NR:case NL:{if(p==1){MateID t=msetpair(m,p,w==NR?RN:LN);atomic_add_mod(out_main+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);}else{MateID t=mshrink(m,p);atomic_add_mod(out_block+rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}case RN:{MateID t=msetpair(m,p,NR);atomic_add_mod(out_main+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}case LN:{MateID t=msetpair(m,p,NL);atomic_add_mod(out_main+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}case LL:{MateID t=msetpair(m,p,NN);int q=p-1,s=1;while(s){--q;auto v=mget(t,q);if(v==L)++s;else if(v==R)--s;}t=mset(t,q,L);if(p==1)atomic_add_mod(out_main+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}case RR:{MateID t=msetpair(m,p,NN);int q=p,s=1;while(s){++q;auto v=mget(t,q);if(v==L)--s;else if(v==R)++s;}t=mset(t,q,R);if(p==1)atomic_add_mod(out_main+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}case RL:{MateID t=msetpair(m,p,NN);if(p==1)atomic_add_mod(out_main+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}default:break;}}}


__global__ void group_persistent_kernel(Count* A,Count* B,Count* D,Code mn,Code dn,int p_hi,int p_lo){
    namespace cg=cooperative_groups;
    cg::grid_group grid=cg::this_grid();
    Code tid=Code(blockIdx.x)*blockDim.x+threadIdx.x;
    Code stride=Code(gridDim.x)*blockDim.x;
    // Gather the transition-closed group directly from distributed authoritative HBM.
    for(Code i=tid;i<mn;i+=stride){MateID m=unrank_group(i,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);A[i]=global_load_main(rank_full_dev(m,D_MAIN_W));}
    for(Code i=tid;i<dn;i+=stride){MateID m=unrank_group(i,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);D[i]=global_load_block(rank_full_dev(m,D_BLOCK_W));}
    grid.sync();
    Count*cur=A;Count*nxt=B;
    for(int p=p_hi;p>=p_lo;--p){
        // Identity/exclusion path. Must finish before atomic contributions arrive.
        for(Code i=tid;i<mn;i+=stride)nxt[i]=cur[i];
        grid.sync();
        // Consume old deferred states.
        for(Code i=tid;i<dn;i+=stride){Count c=D[i];if(c){MateID sm=unrank_group(i,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);MateID t=minsert(sm,p,N);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(nxt+j,c);}}
        grid.sync();
        for(Code i=tid;i<dn;i+=stride)D[i]=0;
        grid.sync();
        // Inclusion transitions from main states.
        for(Code i=tid;i<mn;i+=stride){
            Count c=cur[i];if(!c)continue;
            MateID m=unrank_group(i,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,p);
            switch(w){
            case NN:{MateID t=msetpair(m,p,LR);atomic_add_mod(nxt+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}
            case NR:case NL:{if(p==1){MateID t=msetpair(m,p,w==NR?RN:LN);atomic_add_mod(nxt+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);}else{MateID t=mshrink(m,p);atomic_add_mod(D+rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}
            case RN:{MateID t=msetpair(m,p,NR);atomic_add_mod(nxt+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}
            case LN:{MateID t=msetpair(m,p,NL);atomic_add_mod(nxt+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}
            case LL:{MateID t=msetpair(m,p,NN);int q=p-1,dep=1;while(dep){--q;auto v=mget(t,q);if(v==L)++dep;else if(v==R)--dep;}t=mset(t,q,L);if(p==1)atomic_add_mod(nxt+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(D+rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}
            ...
            case RL:{MateID t=msetpair(m,p,NN);if(p==1)atomic_add_mod(nxt+rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(D+rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}
            default:break;
            }
        }
        grid.sync();
        Count*tmp=cur;cur=nxt;nxt=tmp;
    }
    // Write the completed group back into the global-rank HBM shards.
    for(Code i=tid;i<mn;i+=stride){MateID m=unrank_group(i,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);global_store_main(rank_full_dev(m,D_MAIN_W),cur[i]);}
    for(Code i=tid;i<dn;i+=stride){MateID m=unrank_group(i,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);global_store_block(rank_full_dev(m,D_BLOCK_W),D[i]);}
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,int g,int threads){
    ...
    auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);if(!ms.size&&!ds.size)return;c.ensure(ms.size,ds.size);
    ...
    int sm=0;ck(cudaDeviceGetAttribute(&sm,cudaDevAttrMultiProcessorCount,c.dev),"SM count");
    int per_sm=0;ck(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&per_sm,group_persistent_kernel,threads,0),"occupancy");
    int blocks=std::max(1,sm*per_sm);
    int phi=wp.p_hi, plo=wp.p_lo;
    void* args[]={&c.dA,&c.dB,&c.dD,&ms.size,&ds.size,&phi,&plo};
    ck(cudaLaunchCooperativeKernel((void*)group_persistent_kernel,blocks,threads,args,0,nullptr),"cooperative group kernel");
    ck(cudaDeviceSynchronize(),"group sync");c.groups++;c.active+=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
}
