...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    int cutoff=wp.p_lo-1;size_t countBytes=size_t(2*ms.size+2*ds.size)*sizeof(Count);size_t prefixBytes=size_t(ms.size)*sizeof(uint32_t);bool usePrefix=(wp.p_hi==W-1)&&!pg.use_mi&&cutoff>=0&&(W-cutoff)<=16&&(countBytes+prefixBytes<=target);
