...
int main(int argc, char** argv) {
    ...
    for (int ei=0; ei<(int)edges.size(); ++ei) {
        ...
        // At most two transitions are emitted per input state. New unique states are
        // written directly into these dense output arrays by the hash insertion.
        size_t max_out = 2 * in_n;
        thrust::device_vector<Key128> next_keys(max_out);
        thrust::device_vector<Count> next_vals(max_out * NRES);
        ...
        expand_hash_kernel<<<blocks,threads>>>(
            ...
            table_keys, table_index, table_state,
            thrust::raw_pointer_cast(next_keys.data()), thrust::raw_pointer_cast(next_vals.data()), dense_n,
        ...
        next_keys.resize(rn);
        next_vals.resize(size_t(rn) * NRES);
