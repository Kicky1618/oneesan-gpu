...
static Vec finish_column(Vec const&cur,int r,bool source){
...
#pragma omp for schedule(static)
  for(long long k=0;k<(long long)cur.size();++k){
   State base=unpack(cur[(size_t)k]);bool ok=true;
   for(int y=0;y<r;++y){bool src=source&&y==0;if((src&&base.deg[y]!=1)||(!src&&base.deg[y]!=0&&base.deg[y]!=2)){ok=false;break;}}
   if(!ok)continue;
   if(source){auto q=base.comp[0];if(!q||base.sp>=r)continue;for(int j=base.sp;j>0;--j)base.stack[j]=base.stack[j-1];base.stack[0]=q;++base.sp;}
   for(int sym=0;sym<3;++sym){State s=base;auto q=s.comp[bottom];
    ...
    std::array<std::uint8_t,MAXV> vanished{};int nv=0;for(int y=0;y<r;++y)vanished[nv++]=s.comp[y];vanished[nv++]=s.comp[bottom];
    ...
    for(int ii=0;ii<nv&&ok;++ii){auto qq=vanished[ii];if(!qq)continue;bool alive=stack_has(out,qq);if(!alive)for(int y=0;y<r;++y)if(out.comp[y]==qq){alive=true;break;}if(!alive&&!(out.status[qq]&2))ok=false;}
    if(!ok){ok=true;continue;}for(int qq=1;qq<out.ns;++qq)if((out.status[qq]&2)&&!closed_consistent(out,qq)){ok=false;break;}if(!ok){ok=true;continue;}append(v,out);
   }
  }
 }
 Vec out;size_t z=0;for(auto const&v:loc)z+=v.size();out.reserve(z);for(auto&v:loc)out.insert(out.end(),std::make_move_iterator(v.begin()),std::make_move_iterator(v.end()));norm(out);return out;
...
static Vec column_step(Vec const&boundary,int r,bool source){
 ...
 for(int y=0;y<r-1;++y)x=physical_batch(x,y,y+1,source&&y==0?1:2,2);
 x=physical_batch(x,r-1,2*r,2,2);
 return finish_column(x,r,source);
}
