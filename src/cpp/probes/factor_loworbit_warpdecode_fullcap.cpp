...
int main(int argc, char** argv) {
    ...
    const U64 saturated_rows = U64(W - full_cap + 1);
    ...
    for (std::uint32_t mask = 0; mask < nm; ++mask) {
        ...
        for (int h = 0; h <= high + 1; ++h) {
            seg[size_t(h)] = high_cum[
                std::size_t(mask) * (high + 2) + h][size_t(full_cap)]
                * low_cum[size_t(h)][size_t(full_cap)];
            total += seg[size_t(h)];
        }
        if (!total) continue;

        const U64 repeats = U64(low) * saturated_rows;
        baseline_searches += U128(total) * repeats;
        ...
        for (U64 wid : crossing) {
            ...
            searches += std::min<U64>(32, total - start);
        }
        warp_searches += searches * repeats;
        total_warps += U128(warps) * repeats;
        crossing_warps += U128(crossing.size()) * repeats;
    }

    if (W == 28 && low == 14) {
        if (baseline_searches != U128(28353256135470ULL)
            || warp_searches != U128(1772261285130ULL)
            || total_warps != U128(886040073030ULL)
            || crossing_warps != U128(5660760ULL)) {
            std::cerr << "n=27 LOW orbit full-cap warp-decode regression\n";
            return 2;
        }
    }

    std::cout << std::fixed << std::setprecision(12)
              << "loworbit-warpdecode-fullcap W=" << W << " low=" << low
              << " high=" << high << " saturated_rows=" << saturated_rows << '\n'
