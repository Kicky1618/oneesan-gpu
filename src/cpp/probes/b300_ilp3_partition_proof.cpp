...
int main(){
    ...
    for(std::uint64_t n=1;n<=20000;++n){
        for(std::uint64_t grid: {1ull,2ull,3ull,7ull,31ull,32ull,33ull,127ull,256ull,1024ull,4096ull}){
            ...
            for(std::uint64_t tid=0;tid<grid;++tid){
                for(std::uint64_t base=tid;base<n;base+=3*grid){
    ...
    for(std::uint64_t round=0;round<30;++round){
        for(std::uint64_t tid:tids){
            ...
            const std::uint64_t base=tid+(round/3)*(3*G);
            const std::uint64_t reconstructed=base+(round%3)*G;
            if(reconstructed!=i)return 4;
            ++indices;
        }
    }
    cases+=30*tids.size();
    std::cout<<"b300-ilp3-partition-proof OK cases="<<cases<<" visited_indices="<<indices
             <<" pattern=base_tid_plus_lane_grid lanes=3 stride=3grid duplicate=0 missing=0 exact=1\n";
    return 0;
}
