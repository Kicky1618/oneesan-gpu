...
static void putbits(Packed& p,int&bit,std::uint32_t v,int n){
    int q=bit>>6,o=bit&63; p.w[q]|=std::uint64_t(v)<<o;
    if(o+n>64)p.w[q+1]|=std::uint64_t(v)>>(64-o); bit+=n;
}
static std::uint32_t getbits(Packed const&p,int&bit,int n){
    int q=bit>>6,o=bit&63; std::uint64_t z=p.w[q]>>o;
    if(o+n>64)z|=p.w[q+1]<<(64-o); bit+=n;
    return std::uint32_t(z&((std::uint64_t(1)<<n)-1));
}
static Packed pack_state(State x){
    canon(x); Packed p; int bit=0;
    putbits(p,bit,x.n,5);putbits(p,bit,x.ns,5);putbits(p,bit,x.sp,4);
    for(int i=0;i<x.n;++i)putbits(p,bit,x.deg[i],2);
    for(int i=0;i<x.n;++i)putbits(p,bit,x.comp[i],5);
    for(int i=0;i<x.sp;++i)putbits(p,bit,x.stack[i],5);
    for(int q=1;q<x.ns;++q)putbits(p,bit,x.status[q],2);
    if(bit>256)throw std::runtime_error("packed state overflow");
    return p;
}
static State unpack_state(Packed const&p){
    State x{};int bit=0;x.n=getbits(p,bit,5);x.ns=getbits(p,bit,5);x.sp=getbits(p,bit,4);
    if(x.n>MAXV||x.ns>MAXC||x.sp>MAXR)throw std::runtime_error("bad packed state");
    for(int i=0;i<x.n;++i)x.deg[i]=getbits(p,bit,2);
    for(int i=0;i<x.n;++i)x.comp[i]=getbits(p,bit,5);
    for(int i=0;i<x.sp;++i)x.stack[i]=getbits(p,bit,5);
    for(int q=1;q<x.ns;++q)x.status[q]=getbits(p,bit,2);
    return x;
}
using StateVec=std::vector<Packed>;
static inline void insert_state(StateVec&s,State x){s.push_back(pack_state(std::move(x)));}
static inline void normalize(StateVec&s){std::sort(s.begin(),s.end());s.erase(std::unique(s.begin(),s.end()),s.end());}
...
struct Engine {
    ...
    std::vector<size_t> run() {
        ...
        for (int ei = 0; ei < E; ++ei) {
            ...
            bool emitted = bottomX>=0;
            int nt=std::max(1,omp_get_max_threads());
            std::vector<StateVec> locals(nt);
#pragma omp parallel
            {
                int tid=omp_get_thread_num();
                StateVec& local=locals[tid];
                local.reserve(cur.size()*2/nt + 128);
#pragma omp for schedule(static)
                for (long long pii=0; pii<(long long)cur.size(); ++pii) {
                    Packed const& pk=cur[(size_t)pii];
                    State old=unpack_state(pk);
                    ...
                    for (int take=0; take<2; ++take) {
                        ...
                        for(int sym=symLo;sym<=symHi;++sym) {
                            ...
                            insert_state(local,out);
                        }
                    }
                }
            }
            nxt.clear();
            size_t total=0;for(auto const&v:locals)total+=v.size();
            nxt.reserve(total);
            for(auto&v:locals)nxt.insert(nxt.end(),std::make_move_iterator(v.begin()),std::make_move_iterator(v.end()));
            normalize(nxt);
            cur.swap(nxt);
            if(emitted) {
        ...
        std::cout << "packed-omp-loop r="<<r<<" W="<<W<<" per_column=";
