...
static std::vector<std::vector<std::uint32_t>> build_low_storage(int low) {
    ...
    for (int h0 = 0; h0 <= low; ++h0) {
        ...
        for (std::uint32_t mask = 0; mask < nm; ++mask)
            out[h0].insert(out[h0].end(), groups[mask].begin(), groups[mask].end());
    }
    return out;
}
...
int main(int argc, char** argv) {
    ...
    for (int p = low; p >= 1; --p) {
        // This probe deliberately validates only task arithmetic. Descriptor
        // validity and CROSS semantics are independently checked by the actual
        // CUDA host-plan probe.
        ...
        for (int he = 0; he <= high + 1; ++he) {
            for (int cv = 0; cv < 3; ++cv) {
                ...
                for (std::uint32_t lr = 0; lr < lc.size(); ++lr) {
                    ...
                    if (closure_pair(w)) closure_lr[bid].push_back(lr);
                }
            }
        }
                    ...
                    for (std::uint32_t hc : hrs) {
                        for (U64 chunk = 0; chunk < chunks; ++chunk) {
                            for (U64 lane = 0; lane < 32; ++lane) {
                                ...
                                const std::uint32_t lc = low_storage[hs][lr];
