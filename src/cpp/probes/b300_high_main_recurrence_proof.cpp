...
namespace{
constexpr int W=28,HC=30,MIN_FIXED=7;
...
int main(){
    ...
    std::array<Delta,15> k_lo{},k_hi{};
    for(int k=0;k<=14;++k){bool kfirst=true;std::uint32_t fixed=0;for(int j=0;j<k;++j)fixed|=1u<<(13-j);const std::uint32_t groups=1u<<k;
    ...
    std::cout<<"b300-high-main-recurrence-proof OK min_fixed="<<MIN_FIXED<<" fallback_fixed_lt="<<MIN_FIXED<<" all_delta_min="<<all_lo<<" safe_delta_min="<<safe_lo<<" k6_delta_min="<<k_lo[6]<<" k7_delta_min="<<k_lo[7]<<" signed35=1 trit_positions=14 trit_chunks=5 trit_bits=25 height_bits=4 total_bits=64 planner_specs="<<specs<<" safe_specs="<<safe_specs<<" exhaustive_trit_states="<<total<<" exact=1\n";
}
