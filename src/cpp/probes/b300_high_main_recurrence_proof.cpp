...
namespace{
constexpr int W=28,HC=30,MIN_FIXED=7,TRITS=15,MAX_FIXED=13;
...
int main(){
    ...
    // For window p=27..14 the only planner-fixed positions are 12..0.
    // Position 13 is live because the p=14 pair reads (13,14).
    for(int k=0;k<=MAX_FIXED;++k){bool kfirst=true;std::uint32_t fixed=0;for(int j=0;j<k;++j)fixed|=1u<<(12-j);const std::uint32_t groups=1u<<k;
        for(std::uint32_t g=0;g<groups;++g){std::uint32_t occ=0;for(int j=0;j<k;++j)if((g>>j)&1u)occ|=1u<<(12-j);Spec m=make_spec(28,fixed,occ),b=make_spec(27,fixed,occ);++specs;if(k>=MIN_FIXED)++safe_specs;
    ...
    for(std::uint64_t x=0;x<total;++x){std::uint64_t q=x,packed=0;int a[TRITS]{};for(int i=0;i<TRITS;++i){a[i]=int(q%3);q/=3;}for(int c=0;c<5;++c){int p=3*c;unsigned z=chunk3(a[p],a[p+1],a[p+2]);packed|=std::uint64_t(z)<<(5*c);}for(int i=0;i<TRITS;++i){unsigned z=unsigned((packed>>(5*(i/3)))&31u);if(get3(z,i%3)!=a[i])return 6;}}
    std::cout<<"b300-high-main-recurrence-proof OK min_fixed="<<MIN_FIXED<<" fallback_fixed_lt="<<MIN_FIXED<<" p_lo=14 symbol_lo=13 symbol_hi=27 all_delta_min="<<all_lo<<" safe_delta_min="<<safe_lo<<" k6_delta_min="<<k_lo[6]<<" k7_delta_min="<<k_lo[7]<<" signed35=1 trit_positions=15 trit_chunks=5 trit_bits=25 height_bits=4 total_bits=64 planner_specs="<<specs<<" safe_specs="<<safe_specs<<" exhaustive_trit_states="<<total<<" exact=1\n";
}
