...
static std::vector<U64> greedy_bins(int bits, const std::vector<U64>& by_popcount, int ngpu) {
...
int main(int argc, char** argv) {
    ...
    U64 total = 0;
    for (int kh = 0; kh <= high; ++kh) for (int kl = 0; kl <= low; ++kl) {
    ...
    const LD lowdesc_bytes = LD(4) * LD(low) * LD(lowdesc_main_active + lowdesc_block_active);
    ...
    std::cout << std::fixed << std::setprecision(6)
              ...
              << " v04_headroom_gib=" << double(usable_gib - gib(v04)) << '\n';

    if (gib(v04) >= usable_gib) {
        std::cerr << "v0.4 exceeds requested usable HBM\n";
        return 2;
    }
    return 0;
}
