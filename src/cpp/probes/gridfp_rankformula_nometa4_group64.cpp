...
static uint64_t pack64(uint32_t start,uint32_t n,int delta,uint32_t count){
    if(start>=65536u||n>uint32_t(L)||n>15u||delta<-32768||delta>32767||!count||count>=65536u)return ~0ull;
    return uint64_t(uint16_t(start))|(uint64_t(n)<<16)|(uint64_t(uint16_t(int16_t(delta)))<<32)|(uint64_t(uint16_t(count))<<48);
}
...
int main(){
    ...
    uint32_t max_steps=0,max_count=0,max_owner_groups=0,max_n=0;
    for(int g=0;g<NG;++g){
        ...
        for(int h=0;h<=L+1;++h){
            ...
            for(auto z:v){
                ...
                uint64_t x=pack64(z.start,n,z.delta,z.count);
                if(x==~0ull||st(x)!=z.start||nn(x)!=n||dl(x)!=z.delta||ct(x)!=z.count)return 2;
                p.push_back(x);max_count=std::max(max_count,z.count);max_n=std::max(max_n,n);
            }
            uint32_t gi=0,nb=(rank+3u)/4u;blocks+=nb;
            for(uint32_t b=0;b<nb;++b){uint32_t lo=b*4u,hi=std::min(rank,lo+4u);while(gi+1<v.size()&&lo>=v[gi+1].start)++gi;for(uint32_t r=lo;r<hi;++r){uint32_t q=gi,s=0;uint64_t e=p[q];for(int k=0;k<3;++k){if(r<st(e)+ct(e))break;++q;++s;if(q>=p.size())return 3;e=p[q];}if(r<st(e)||r>=st(e)+ct(e)||nn(e)!=uint32_t(__builtin_popcount(v[q].mask)))return 4;++exact;steps_sum+=s;max_steps=std::max(max_steps,s);}}
        }
        max_owner_groups=std::max(max_owner_groups,owner_groups);
    }
    if(codes!=1201917ull||groups!=69632ull||blocks!=300524ull||exact!=codes||max_steps!=3u||max_count!=1001u||max_owner_groups!=8709u||max_n!=14u)return 5;
    ...
    std::cout<<"gridfp-rankformula-nometa4-group64 OK codes="<<codes<<" groups="<<groups<<" blocks="<<blocks
             ...
             <<" count_pack_exact=1 support_count_pack_exact=1 support_position_bits=0 group_n_bits=4 sentinel_entries=0\n";
    return 0;
}
