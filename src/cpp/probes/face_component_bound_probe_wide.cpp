...
struct Key{uint64_t lo,hi;bool operator==(Key const&o)const{return lo==o.lo&&hi==o.hi;}};
struct KeyHash{size_t operator()(Key const&k)const{uint64_t x=k.lo^(k.hi+0x9e3779b97f4a7c15ULL+(k.lo<<6)+(k.lo>>2));x^=x>>30;x*=0xbf58476d1ce4e5b9ULL;x^=x>>27;x*=0x94d049bb133111ebULL;return size_t(x^(x>>31));}};
...
static Key encode(uint32_t color,const std::vector<int>&lab,uint32_t touched,int h){
    __uint128_t z=color;int sh=h;
    for(int r=0;r<h;++r){z|=__uint128_t(lab[r])<<sh;sh+=4;}
    z|=__uint128_t(touched)<<sh;
    return {uint64_t(z),uint64_t(z>>64)};
}
static void decode(Key k,uint32_t&color,int lab[12],uint32_t&touched,int h){
    __uint128_t z=(__uint128_t(k.hi)<<64)|k.lo;color=uint32_t(z&((__uint128_t(1)<<h)-1));int sh=h;
    for(int r=0;r<h;++r){lab[r]=int((z>>sh)&15);sh+=4;}
    touched=uint32_t(z>>sh)&((1u<<h)-1);
}

static Key initial_state(uint32_t color,int h){
...
static bool advance(Key st,uint32_t ny,int h,Key&out){
...
int main(int argc,char**argv){
    ...
    if(h<1||h>12||w<1){std::cerr<<"h=1..12\n";return 2;}
    ...
    std::unordered_map<Key,cpp_int,KeyHash>dp,ndp;dp.reserve(S*8);
    for(int x=0;x<S;++x)dp[initial_state(x,h)]+=1;
    std::cerr<<"col=1 states="<<dp.size()<<"\n";
    for(int col=1;col<w;++col){
        ndp.clear();ndp.reserve(dp.size()*2);
        for(auto const&kv:dp){uint32_t x,ot;int ol[12];decode(kv.first,x,ol,ot,h);for(uint16_t y:nxt[x]){Key ns;if(advance(kv.first,y,h,ns))ndp[ns]+=kv.second;}}
