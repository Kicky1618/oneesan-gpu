...
static void verify_width(int W, int low) {
    ...
    std::cout << "highorbit-rowdepth-semantics OK W=" << W
...
int main(int argc, char** argv) {
    ...
    constexpr int HIGH = W - 1 - LOW;
    constexpr U64 D = 135015505407ULL;
    U64 active_per_high_position = 0;
    for (int row = 1; row <= W; ++row)
        active_per_high_position += capped_state_count(W - 1, std::min(row, W / 2));

    const U64 dense_bodies = D * U64(HIGH) * U64(W);
    const U64 active_bodies = active_per_high_position * U64(HIGH);
    if (dense_bodies != 49145643968148ULL
        || active_bodies != 43627644071375ULL) {
        std::cerr << "n=27 HIGH orbit body-count regression mismatch\n";
        return 2;
    }

    const long double ratio = (long double)active_bodies / (long double)dense_bodies;
    std::cout << std::fixed << std::setprecision(12)
              << "n27_highorbit_dense_bodies=" << dense_bodies
