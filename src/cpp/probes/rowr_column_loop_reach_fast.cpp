...
struct Hash {
    size_t operator()(State const& s) const noexcept {
        ...
        auto put = [&](std::uint8_t x){ h ^= x; h *= 1099511628211ULL; };
        put(s.n); put(s.ns); put(s.sp);
        for (int i = 0; i < s.n; ++i) { put(s.deg[i]); put(s.comp[i]); }
        put(0xfe);
        for (int i = 0; i < s.sp; ++i) put(s.stack[i]);
        put(0xfd);
        for (int i = 1; i < s.ns; ++i) put(s.status[i]);
        return (size_t)h;
    }
};
...
using Set = std::unordered_set<State, Hash>;

static inline void insert_state(Set& s, State x) {
    canon(x);
    s.insert(std::move(x));
}
...
struct Engine {
    ...
    std::vector<size_t> run() {
        Set cur, nxt;
        State z; z.ns = 1; insert_state(cur, z);
        ...
        std::cout << "fast-loop r="<<r<<" W="<<W<<" per_column=";
