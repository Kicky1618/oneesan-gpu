...
int main() {
    ...
    for (int h = 0; h <= L + 1; ++h) {
        ...
        for (uint32_t m = 0; m < (1u << L); ++m) {
            ...
            for (uint32_t code : v) {
                uint32_t r = next[g]++;
                if (r >= RANK16_INVALID) {
                    std::cerr << "rank16 local rank overflow owner=" << g << " h=" << h
                              << " rank=" << r << '\n';
                    return 2;
                }
                LocalCode z{code, uint16_t(r), uint8_t(h), uint8_t(g)};
                codes[g].push_back(z);
                uint32_t key = ternary_key(code, L);
                if (direct[key] != INVALID) {
                    std::cerr << "duplicate LOW ternary key=" << key << '\n';
                    return 3;
                }
    ...
    std::array<uint64_t, NG> valid{};
    std::array<uint64_t, NG> l_digits{};
    std::array<uint64_t, NG> owner_mismatch{};
    uint64_t total_valid = 0, total_l = 0;
    for (int g = 0; g < NG; ++g) {
        for (const LocalCode& z : codes[g]) {
            uint32_t key = ternary_key(z.code, L);
            uint32_t weight = 1;
            for (int pos = 0; pos < L; ++pos) {
                uint32_t v = (z.code >> (2 * pos)) & 3u;
                if (v == LL) {
                    ++l_digits[g]; ++total_l;
    ...
    uint64_t max_dense_bytes = 0, max_sparse_bytes = 0, max_prekey_bytes = 0;
    uint64_t total_dense_bytes = 0, total_sparse_bytes = 0, total_prekey_bytes = 0;
    for (int g = 0; g < NG; ++g) {
        uint64_t n = codes[g].size();
        uint64_t dense = n * uint64_t(L) * sizeof(uint16_t);
        // Candidate sparse design: uint16 valid mask + uint32 packed-rank offset
        // per code, plus uint16 for every valid L->R transition.
        uint64_t sparse = n * (sizeof(uint16_t) + sizeof(uint32_t)) + valid[g] * sizeof(uint16_t);
        uint64_t prekey = n * sizeof(uint32_t);
        max_dense_bytes = std::max(max_dense_bytes, dense);
        max_sparse_bytes = std::max(max_sparse_bytes, sparse);
        max_prekey_bytes = std::max(max_prekey_bytes, prekey);
        total_dense_bytes += dense;
        total_sparse_bytes += sparse;
        total_prekey_bytes += prekey;
        std::cout << "owner=" << g
                  << " low_codes=" << n
                  << " l_digits=" << l_digits[g]
                  << " valid_rank16=" << valid[g]
                  << " valid_per_code=" << (n ? double(valid[g]) / double(n) : 0.0)
                  << " dense_rank16_mib=" << double(dense) / double(1 << 20)
                  << " sparse_rank16_mib=" << double(sparse) / double(1 << 20)
    ...
    if (mismatches) {
        std::cerr << "rank16 occupancy-owner mismatch total=" << mismatches << '\n';
        return 4;
    }
