...
Stats primitive_stats() {
    std::array<std::array<U64, COLS>, MAX + 1> p{};
    p[0][0] = 1;
    ...
    Stats s{};
    for (const auto& row : p) for (U64 v : row) if (v) {
        ++s.nonzero;
...
Stats motzkin_stats() {
    std::array<std::array<U64, COLS>, MAX + 1> m{};
    m[0][0] = 1;
    for (int rem = 1; rem <= MAX; ++rem) {
        for (int h = 0; h <= MAX; ++h) {
            U64 z = m[rem - 1][h];
    ...
    Stats s{};
    for (const auto& row : m) for (U64 v : row) if (v) {
        ++s.nonzero;
        if (v > s.max_value) s.max_value = v;
    }
    return s;
...
U64 choose_max() {
    std::array<std::array<U64, MAX + 1>, MAX + 1> c{};
    U64 mx = 0;
    for (int n = 0; n <= MAX; ++n) {
        c[n][0] = c[n][n] = 1;
        for (int k = 1; k < n; ++k)
            c[n][k] = c[n - 1][k - 1] + c[n - 1][k];
        for (int k = 0; k <= n; ++k) if (c[n][k] > mx) mx = c[n][k];
    }
    return mx;
}
}
...
int main() {
    const Stats primitive = primitive_stats();
    const Stats motzkin = motzkin_stats();
    const U64 cmax = choose_max();
    ...
    constexpr U64 sym_total = choose_sym + primitive_compact + motzkin_tri;
    constexpr U64 tri_total = choose_tri + primitive_compact + motzkin_tri;

    static_assert(old_total == 20648);
    static_assert(sym_total == 5280);
    static_assert(tri_total == 6120);
    static_assert(old_total - sym_total == 15368);
    static_assert(old_total - tri_total == 14528);
    ...
    std::cout << "gridfp-codec-table-budget-proof OK"
              ...
              << " sym_candidate_bytes=" << sym_total
              << " sym_saved_bytes=" << (old_total - sym_total)
              << " tri_candidate_bytes=" << tri_total
              << " tri_saved_bytes=" << (old_total - tri_total)
