...
int main(int argc,char**argv){
    ...
    unordered_map<K,Count,H> cur,nxt; cur[key(z)]=1;
    ...
    for(int ei=0;ei<(int)e.size();++ei){
        ...
        for(auto const& [kk,ways]:cur){ S base=dec(kk);
            for(int take=0;take<2;++take){ S s=base; bool ok=true;
                if(take){
                    if(s.done)continue;
                    uint8_t mu=(u==st||u==tt)?1:2,mv=(v==st||v==tt)?1:2;
                    if(s.d[u]>=mu||s.d[v]>=mv)continue;
                    ++s.d[u];++s.d[v];
    ...
    Count ans=0; for(auto const& [kk,ways]:cur)if(dec(kk).done)ans+=ways;
