...
__device__ static inline void atomic_add_mod(Count* p, Count v, Count) {
    atomicAdd(p, v);
}
...
__global__ void gather_kernel(
    ...
    size_t n, Key128* out_keys, Count* out_vals, Count mod) {
    size_t i = blockIdx.x * (size_t)blockDim.x + threadIdx.x;
    if (i >= n) return;
    uint32_t slot = occupied[i];
    out_keys[i] = table_keys[slot];
    out_vals[i] = table_vals[slot] % mod;
}
...
int main(int argc, char** argv) {
    ...
    if (mod >= (1ULL << 31)) { std::cerr << "raw31 mode requires mod < 2^31\n"; return 1; }
    int w = n + 1;
    ...
    for (int ei=0; ei<(int)edges.size(); ++ei) {
        ...
        if (in_n && (mod - 1) > (~0ULL) / (2ULL * in_n)) {
            std::cerr << "raw accumulation could overflow uint64\n"; return 3;
        }
        ...
        if (rn) {
            ...
            gather_kernel<<<gb,threads>>>(table_keys,table_vals,occupied,rn,
                thrust::raw_pointer_cast(next_keys.data()),thrust::raw_pointer_cast(next_vals.data()),mod);
