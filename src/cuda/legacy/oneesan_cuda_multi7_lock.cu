...
static constexpr int NRES = 7;
__device__ __constant__ Count D_MODS[NRES] = {2305843009213693921ULL,2305843009213692799ULL,2305843009213691767ULL,2305843009213690657ULL,2305843009213689601ULL,2305843009213688569ULL,2305843009213687519ULL};
static constexpr Count H_MODS[NRES] = {2305843009213693921ULL,2305843009213692799ULL,2305843009213691767ULL,2305843009213690657ULL,2305843009213689601ULL,2305843009213688569ULL,2305843009213687519ULL};
        ...
        for (;;) {
            if (s == ready) {
                if (table_keys[slot] == key) {
                    unsigned int expected = ready;
                    if (st.compare_exchange_strong(expected, locked, cuda::memory_order_acq_rel)) {
                        #pragma unroll
                        for (int r = 0; r < NRES; ++r) {
                            Count* dst = &table_vals[size_t(slot) * NRES + r];
                            Count old = *dst, v = values[r], mod = D_MODS[r];
                            *dst = old >= mod - v ? old - (mod - v) : old + v;
                        }
            ...
            if (st.compare_exchange_strong(expected, locked, cuda::memory_order_acq_rel)) {
                table_keys[slot] = key;
                #pragma unroll
                for (int r = 0; r < NRES; ++r) table_vals[size_t(slot) * NRES + r] = values[r];
...
__global__ void expand_hash_kernel(
    ...
    if (transition_decoded(base, m, 0, out))
        hash_insert(out, v, table_keys, table_vals, table_state, occupied, occupied_n, mask, generation);
...
__global__ void gather_kernel(
    ...
    #pragma unroll
    for (int r = 0; r < NRES; ++r) out_vals[i * NRES + r] = table_vals[size_t(slot) * NRES + r];
}
...
int main(int argc, char** argv) {
    ...
    thrust::device_vector<Count> vals(NRES, 1);
    keys[0] = Key128{0,0};
    ...
    for (int ei=0; ei<(int)edges.size(); ++ei) {
        ...
        if (cap > allocated_cap) {
            ...
            ck(cudaMalloc(&table_vals, cap*NRES*sizeof(Count)), "malloc table_vals");
        ...
        expand_hash_kernel<<<blocks,threads>>>(
            thrust::raw_pointer_cast(keys.data()), thrust::raw_pointer_cast(vals.data()), in_n, m,
        ...
        thrust::device_vector<Count> next_vals(size_t(rn) * NRES);
        if (rn) {
