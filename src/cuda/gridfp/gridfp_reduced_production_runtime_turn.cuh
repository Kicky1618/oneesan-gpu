...
#endif
#ifndef RP_RUNTIME_TURN_DIRECT_LOW_COMPRESS_STEP
#define RP_RUNTIME_TURN_DIRECT_LOW_COMPRESS_STEP 1
#endif
#ifndef RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_INVERSE
#define RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_INVERSE 1
#endif
static_assert(RP_RUNTIME_TURN_LOCAL_SECTOR_TABLE == 0 || RP_RUNTIME_TURN_LOCAL_SECTOR_TABLE == 1,
              "RP_RUNTIME_TURN_LOCAL_SECTOR_TABLE must be 0 or 1");
static_assert(RP_RUNTIME_TURN_LOCAL_SECTOR_W28_TREE == 0 || RP_RUNTIME_TURN_LOCAL_SECTOR_W28_TREE == 1,
              "RP_RUNTIME_TURN_LOCAL_SECTOR_W28_TREE must be 0 or 1");
static_assert(RP_RUNTIME_TURN_DISCOVERY_NONN_SCAN == 0 || RP_RUNTIME_TURN_DISCOVERY_NONN_SCAN == 1,
              "RP_RUNTIME_TURN_DISCOVERY_NONN_SCAN must be 0 or 1");
static_assert(RP_RUNTIME_TURN_DIRECT_COMPRESS_INVERSE == 0 || RP_RUNTIME_TURN_DIRECT_COMPRESS_INVERSE == 1,
              "RP_RUNTIME_TURN_DIRECT_COMPRESS_INVERSE must be 0 or 1");
static_assert(RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_STEP == 0 || RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_STEP == 1,
              "RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_STEP must be 0 or 1");
static_assert(RP_RUNTIME_TURN_DIRECT_LOW_COMPRESS_STEP == 0 || RP_RUNTIME_TURN_DIRECT_LOW_COMPRESS_STEP == 1,
              "RP_RUNTIME_TURN_DIRECT_LOW_COMPRESS_STEP must be 0 or 1");
static_assert(RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_INVERSE == 0 || RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_INVERSE == 1,
              "RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_INVERSE must be 0 or 1");
...
__device__ __forceinline__ void runtime_turn_local_sector_w28_tree_device(
    int row, int first, bool eight, Rank64 within,
    int& local_ones, Rank64& local_within
) {
...
__device__ __forceinline__ bool runtime_turn_local_sector_device(
    int W, int L, int outer_ones, Rank64 within,
    int& local_ones, Rank64& local_within
) {
    ...
    if (base >= 0 && L == W / 2 + 1 && outer_ones >= 0 && outer_ones <= O) {
        ...
        if (W == 28 && L == 15) {
            runtime_turn_local_sector_w28_tree_device(row, first, count == 8, within, local_ones, local_within);
            return true;
        }
#endif
        int lo = 0, hi = count;
...
__device__ __forceinline__ Rank64 runtime_turn_compress_group_size_device(int L, int outer_ones) {
    Rank64 total = 0;
...
__device__ __forceinline__ MateID runtime_turn_compress_label_unrank_device(
    int W, int K, bool high, const OwnerComponentPlanDevice& plan, Rank64 rank
) {
    const int L = K + 2;
    const int O = W - L;
    ...
    if (!group) return 0;
    Rank64 outer_delta = 0, within = 0;
    runtime_fastdivmod64_magic(local, group, runtime_turn_compress_group_magic(W, outer_ones), outer_delta, within);
    ...
    int local_ones = -1;
    Rank64 local_sr = 0, primitive_rank = 0, local_within = 0;
    if (runtime_turn_local_sector_device(W, L, outer_ones, within, local_ones, local_within)) {
        const int occupied = outer_ones + local_ones;
        const Rank64 pc = RP_PRIMITIVE[occupied][1];
        runtime_fastdivmod64_magic(local_within, pc, RP_RUNTIME_PRIMITIVE1_MAGIC[occupied], local_sr, primitive_rank);
    } else {
        ...
        for (int l = 0; l <= L - 1; ++l) {
            ...
            if (scan_within < n) {
                local_ones = l;
                runtime_fastdivmod64_magic(scan_within, pc, RP_RUNTIME_PRIMITIVE1_MAGIC[occupied], local_sr, primitive_rank);
                break;
            }
            scan_within -= n;
        }
    ...
    const std::uint32_t local_support = support_unrank_mask_device(L - 1, local_ones, local_sr);
    std::uint32_t full = 0;
...
__device__ __forceinline__ DeviceKey runtime_turn_compress_seed(MateID label, int W, bool high) {
    if (!high) {
...
__device__ __forceinline__ DeviceKey runtime_turn_expand_seed(MateID label, int W, bool high) {
    return high ? DeviceKey{blocked_exclude(label, W - 1), 0}
                : DeviceKey{blocked_exclude_reverse(label, W, 1), 0};
}

__device__ __forceinline__ bool runtime_turn_compress_low_step(DeviceKey src, int W, RuntimeSmallTerms& z) {
#if RP_RUNTIME_TURN_DIRECT_LOW_COMPRESS_STEP
    if (src.blocked)
        return runtime_small_add(z, DeviceKey{blocked_exclude(src.mate, 1), 0}, 1);
    if (!runtime_small_add(z, src, 1)) return false;
    switch (mpair(src.mate, 1)) {
    case NN:
        return runtime_small_add(z, DeviceKey{msetpair(src.mate, 1, LR), 0}, 1);
    case NR:
        return runtime_small_add(z, DeviceKey{msetpair(src.mate, 1, RN), 0}, 1);
    case RN:
        return runtime_small_add(z, DeviceKey{msetpair(src.mate, 1, NR), 0}, 1);
    case RR: {
        MateID t = msetpair(src.mate, 1, NN);
        const int q = closure_match_right(t, W, 1);
        if (q < 0) return true;
        return runtime_small_add(z, DeviceKey{mset(t, q, R), 0}, 1);
    }
    default:
        return true;
    }
...
__device__ __forceinline__ bool runtime_turn_step(DeviceKey src, int W, bool high, bool expand, RuntimeSmallTerms& z) {
    if (!high) {
        if (expand) {
    ...
    return runtime_small_add(z, DeviceKey{blocked_exclude_reverse(src.mate, W, W - 1), 0}, 1);
#else
    RuntimeSmallTerms tmp;
    if (!runtime_turn_compress_low_step(mirror_key_device(src, W), W, tmp)) return false;
    for (int i = 0; i < tmp.n; ++i)
        if (!runtime_small_add(z, mirror_key_device(tmp.v[i].key, W), tmp.v[i].coef)) return false;
    return true;
#endif
}

template<class Sink>
__device__ __forceinline__ bool runtime_turn_try_compress_main(MateID x, MateID dest, int W, Sink& sink) {
    if (!valid_mate_device(x, W)) return true;
    const IncludeResult z = include_horizontal(x, W, 1);
    if (z.valid && !z.blocked && z.mate == dest) return sink.emit(DeviceKey{x, 0});
    return true;
}

template<class Sink>
__device__ __forceinline__ bool runtime_turn_discover_compress_low(DeviceKey dest, int W, Sink& sink) {
    ...
    if (pair == NN) {
        ...
        while (mask) {
            ...
            if (bal == 0 && v == R) {
                MateID x = msetpair(d, 1, RR); x = mset(x, q, L);
    ...
    if (pair == NN) {
        if (!runtime_turn_try_compress_main(msetpair(d, 1, RL), d, W, sink)) return false;
        int bal = 0;
#if RP_RUNTIME_TURN_DISCOVERY_NONN_SCAN
        std::uint32_t mask = mate_non_n_mask(d, W) & ~std::uint32_t(3u);
        while (mask) {
            const int q = mate_lsb_index32(mask); const MateValue v = mget(d, q);
            if (bal == 0 && v == R) {
                MateID x = msetpair(d, 1, RR); x = mset(x, q, L);
                if (!runtime_turn_try_compress_main(x, d, W, sink)) return false;
            }
            if (v == R) ++bal; else if (v == L) --bal;
            if (bal < 0) break; mask &= mask - 1u;
        }
#else
        for (int q = 2; q < W; ++q) {
            const MateValue v = mget(d, q);
...
template<class Sink>
__device__ __forceinline__ bool runtime_turn_discover_compress_high(DeviceKey dest, int W, Sink& sink) {
    if (dest.blocked) return false;
    const MateID d = dest.mate;
    if (!sink.emit(DeviceKey{d, 0})) return false;
    const int p = W - 1;
    ...
    if (pair == LN && !sink.emit(DeviceKey{msetpair(d, p, NL), 0})) return false;
    if (pair == NN) {
        int bal = 0;
#if RP_RUNTIME_TURN_DISCOVERY_NONN_SCAN
        const std::uint32_t limit = (std::uint32_t(1) << (W - 2)) - 1u;
    ...
    if (pair == LN && !sink.emit(DeviceKey{mshrink(d, p - 1), 1})) return false;
    return true;
}

template<class Sink>
struct RuntimeMirrorSink {
    Sink sink{}; int W = 0; bool main_only = false;
...
struct RuntimeMainOnlySink {
    Sink sink{};
    __device__ __forceinline__ bool emit(DeviceKey k) { if (k.blocked) return true; return sink.emit(k); }
};

__device__ __forceinline__ bool runtime_turn_discover_inverse(
    DeviceKey dest, int W, bool high, bool expand,
    ...
    if (!high && !expand) return runtime_turn_discover_compress_low(dest, W, base);
    if (high && !expand) {
#if RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_INVERSE
        return runtime_turn_discover_compress_high(dest, W, base);
#else
        RuntimeMirrorSink<RuntimeSharedKeySetSink> sink{base, W, false};
        return runtime_turn_discover_compress_low(mirror_key_device(dest, W), W, sink);
#endif
    }
    if (!high && expand) {
...
__device__ __forceinline__ bool runtime_turn_discover_inverse_indexed(
    DeviceKey dest, int W, bool high, bool expand,
    RuntimeSharedKey* source_set, int& source_count, int capacity,
    std::uint64_t& source_occupancy, RuntimeFindIndexCache& source_cache
) {
    RuntimeIndexedSharedKeySetSink base{source_set, &source_count, capacity, &source_occupancy, &source_cache};
    if (!high && !expand) return runtime_turn_discover_compress_low(dest, W, base);
    if (high && !expand) {
#if RP_RUNTIME_TURN_DIRECT_HIGH_COMPRESS_INVERSE
        return runtime_turn_discover_compress_high(dest, W, base);
...
__global__ void owner_turn_runtime_subwarp_kernel(
    std::uint32_t* __restrict__ state, Rank64 local_components,
    int W, int K, bool high, bool expand, int gpu_id, int ngpu,
    ...
    std::uint32_t mod, int* error
) {
    __shared__ RuntimeSharedKey sh_src[RP_RUNTIME_WARPS_PER_BLOCK][RP_RUNTIME_SUBGROUPS_PER_WARP][RP_RUNTIME_MAX_PAIRS];
    __shared__ RuntimeSharedKey sh_dst[RP_RUNTIME_WARPS_PER_BLOCK][RP_RUNTIME_SUBGROUPS_PER_WARP][RP_RUNTIME_MAX_PAIRS];
    __shared__ std::uint32_t sh_value[RP_RUNTIME_WARPS_PER_BLOCK][RP_RUNTIME_SUBGROUPS_PER_WARP][RP_RUNTIME_MAX_PAIRS];
    __shared__ GroupedComponentContextDevice sh_ctx[RP_RUNTIME_WARPS_PER_BLOCK][RP_RUNTIME_SUBGROUPS_PER_WARP];
    ...
    __shared__ RuntimeEdgeCache sh_edge[RP_RUNTIME_WARPS_PER_BLOCK][RP_RUNTIME_SUBGROUPS_PER_WARP];
#endif
#if RP_RUNTIME_FIND_INDEX_CACHE
    __shared__ RuntimeFindIndexCache sh_src_find[RP_RUNTIME_WARPS_PER_BLOCK][RP_RUNTIME_SUBGROUPS_PER_WARP];
    __shared__ RuntimeFindIndexCache sh_dst_find[RP_RUNTIME_WARPS_PER_BLOCK][RP_RUNTIME_SUBGROUPS_PER_WARP];
#endif
    ...
    const Rank64 stride = Rank64(gridDim.x) * RP_RUNTIME_WARPS_PER_BLOCK * RP_RUNTIME_SUBGROUPS_PER_WARP;
    const OwnerComponentPlanDevice plan{component_prefix, component_sr_begin, component_group};
    const int source_q = high ? W - 1 : 1;
    const bool source_reverse = high ? !expand : expand;
    const int tile_start = high ? (expand ? W - 1 : K + 1) : (expand ? 1 : K + 1);
    const int destination_q = high ? (expand ? W - 2 : W - 1) : (expand ? 2 : 1);

    for (Rank64 local_rank = first; local_rank < local_components; local_rank += stride) {
        if (sublane == 0) {
            sh_ns[warp][subgroup] = 0; sh_nd[warp][subgroup] = 0;
            const MateID label = expand
                ? owner_component_label_unrank_planned_device(W, high ? W - 1 : 1, high ? false : true, high ? W - 1 : 1, K, plan, local_rank)
                : runtime_turn_compress_label_unrank_device(W, K, high, plan, local_rank);
            const DeviceKey seed = expand ? runtime_turn_expand_seed(label, W, high)
                                          : runtime_turn_compress_seed(label, W, high);
            const GroupedComponentContextDevice ctx = grouped_component_context_device(seed, W, source_q, source_reverse, tile_start, K, ngpu, owner_begin);
            sh_ctx[warp][subgroup] = ctx;
            if (ctx.owner != gpu_id) runtime_set_error(error, 321);
            else {
#if RP_RUNTIME_FIND_INDEX_CACHE
                std::uint64_t source_occupancy = 0, destination_occupancy = 0;
#elif RP_RUNTIME_FIND_SIGNATURE_FILTER
                std::uint64_t source_signature = runtime_shared_key_signature_bit(seed), destination_signature = 0;
#endif
                sh_src[warp][subgroup][0] = runtime_shared_key_encode(seed); sh_ns[warp][subgroup] = 1;
#if RP_RUNTIME_FIND_INDEX_CACHE
                runtime_find_index_record(sh_src_find[warp][subgroup], source_occupancy, seed, 0);
#endif
                int cursor = 0;
                while (cursor < sh_ns[warp][subgroup]) {
                    ...
                    if (!runtime_turn_step(runtime_shared_key_decode(sh_src[warp][subgroup][source_ix]), W, high, expand, edge)) {
                        runtime_set_error(error, 322); break;
                    }
                    for (int ei = 0; ei < edge.n; ++ei) {
                        if (!edge.v[ei].coef) continue;
                        const DeviceKey d = edge.v[ei].key;
#if RP_RUNTIME_FIND_INDEX_CACHE
                        int destination_ix = runtime_find_shared_key_indexed(sh_dst[warp][subgroup], sh_nd[warp][subgroup], d, destination_occupancy, sh_dst_find[warp][subgroup]);
#elif RP_RUNTIME_FIND_SIGNATURE_FILTER
                        int destination_ix = runtime_find_shared_key_filtered(sh_dst[warp][subgroup], sh_nd[warp][subgroup], d, destination_signature);
#else
                        int destination_ix = runtime_find_shared_key(sh_dst[warp][subgroup], sh_nd[warp][subgroup], d);
#endif
                        if (destination_ix < 0) {
                            if (sh_nd[warp][subgroup] >= RP_RUNTIME_MAX_PAIRS) { runtime_set_error(error, 323); cursor = RP_RUNTIME_MAX_PAIRS; break; }
                            destination_ix = sh_nd[warp][subgroup]++;
                            sh_dst[warp][subgroup][destination_ix] = runtime_shared_key_encode(d);
#if RP_RUNTIME_FIND_INDEX_CACHE
                            runtime_find_index_record(sh_dst_find[warp][subgroup], destination_occupancy, d, destination_ix);
                            if (!runtime_turn_discover_inverse_indexed(d, W, high, expand, sh_src[warp][subgroup], sh_ns[warp][subgroup], RP_RUNTIME_MAX_PAIRS, source_occupancy, sh_src_find[warp][subgroup])) {
#elif RP_RUNTIME_FIND_SIGNATURE_FILTER
                            destination_signature |= runtime_shared_key_signature_bit(d);
                            if (!runtime_turn_discover_inverse(d, W, high, expand, sh_src[warp][subgroup], sh_ns[warp][subgroup], RP_RUNTIME_MAX_PAIRS, &source_signature)) {
#else
                            if (!runtime_turn_discover_inverse(d, W, high, expand, sh_src[warp][subgroup], sh_ns[warp][subgroup], RP_RUNTIME_MAX_PAIRS)) {
#endif
                                runtime_set_error(error, 324); cursor = RP_RUNTIME_MAX_PAIRS; break;
                            }
                        }
#if RP_RUNTIME_CACHE_EDGES
                        if (!runtime_edge_cache_append(sh_edge[warp][subgroup], source_ix, destination_ix, int(edge.v[ei].coef))) {
                            runtime_set_error(error, 328); cursor = RP_RUNTIME_MAX_PAIRS; break;
                        }
#endif
                    }
        ...
        const int ns = sh_ns[warp][subgroup], nd = sh_nd[warp][subgroup];
        const GroupedComponentContextDevice ctx = sh_ctx[warp][subgroup];
        for (int i = sublane; i < ns; i += RP_RUNTIME_SUBGROUP_WIDTH) {
            const GroupedDeviceRank gr = grouped_rank_in_component_device(runtime_shared_key_decode(sh_src[warp][subgroup][i]), W, source_q, source_reverse, ctx);
            if (gr.owner != gpu_id) { runtime_set_error(error, 325); sh_value[warp][subgroup][i] = 0; }
            else sh_value[warp][subgroup][i] = state[gr.local];
        }
        __syncwarp(subgroup_mask);
#if RP_RUNTIME_CACHE_EDGES
        ...
        for (int si = 0; si < ns; ++si) {
            ...
            for (std::uint8_t ei = 0; ei < nedge; ++ei) {
                ...
                if ((destination_ix & (RP_RUNTIME_SUBGROUP_WIDTH - 1)) == sublane) {
                    ...
                    accum[slot] += static_cast<long long>(runtime_edge_coefficient(packed)) * value;
                }
            }
        }
        int accumulator_ix = 0;
        for (int di = sublane; di < nd; di += RP_RUNTIME_SUBGROUP_WIDTH, ++accumulator_ix) {
            const DeviceKey mine = runtime_shared_key_decode(sh_dst[warp][subgroup][di]);
            const GroupedDeviceRank dgr = grouped_rank_in_component_device(mine, W, destination_q, source_reverse, ctx);
            if (dgr.owner != gpu_id) { runtime_set_error(error, 326); continue; }
        ...
        for (int di = sublane; di < nd; di += RP_RUNTIME_SUBGROUP_WIDTH) {
            ...
            for (int si = 0; si < ns; ++si) {
                RuntimeSmallTerms edge;
                if (!runtime_turn_step(runtime_shared_key_decode(sh_src[warp][subgroup][si]), W, high, expand, edge)) { runtime_set_error(error, 327); continue; }
                for (int ei = 0; ei < edge.n; ++ei)
                    if (key_equal(edge.v[ei].key, mine))
                        acc += static_cast<long long>(edge.v[ei].coef) * static_cast<long long>(sh_value[warp][subgroup][si]);
            }
