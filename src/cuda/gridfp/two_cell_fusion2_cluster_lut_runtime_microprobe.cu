...
#undef main
#define main two_cell_fusion2_cluster_sliced_lut_microprobe_main_unused
...
cudaLaunchConfig_t make_lut_cluster_config(
    int cluster,
    Rank dynamic_bytes,
    unsigned clusters
) {
...
LutRuntimeChoice choose_lut_runtime_bucket(
    int outer_ones,
    Rank support_count,
    ...
    for (int cluster : {1, 2, 4, 8}) {
        ...
        ck(cudaFuncSetAttribute(
           ...
           "cluster LUT runtime optin shared");
        ...
        ck(cudaOccupancyMaxPotentialClusterSize(
               ...
               reinterpret_cast<const void*>(
                   two_cell_fusion2_cluster_sliced_lut_kernel),
               &config),
           "cluster LUT runtime max potential");
        ...
        ck(cudaOccupancyMaxActiveClusters(
               ...
               reinterpret_cast<const void*>(
                   two_cell_fusion2_cluster_sliced_lut_kernel),
               &config),
           "cluster LUT runtime active clusters");
...
void launch_lut_runtime_bucket(
    ...
    ck(cudaGetLastError(), "cluster LUT runtime launch");
}
...
int main(int argc, char** argv) {
    ...
    int visible = 0;
    ck(cudaGetDeviceCount(&visible), "cluster LUT runtime device count");
    if (visible < 1) return 4;
    ck(cudaSetDevice(0), "cluster LUT runtime set device");
    cudaDeviceProp prop{};
    ck(cudaGetDeviceProperties(&prop, 0), "cluster LUT runtime props");
    if (prop.major < 9) {
        std::cerr << "cluster LUT runtime requires compute capability >= 9.0\n";
        return 5;
    }
    install_tables(rt);
    ...
    ck(cudaFuncGetAttributes(&attr, two_cell_fusion2_cluster_sliced_lut_kernel),
       "cluster LUT runtime function attributes");
    ...
    for (int start = 1; start + 1 <= W - 4; start += 2) {
        ...
        ck(cudaMalloc(&d_values, states * sizeof(std::uint32_t)),
           "cluster LUT runtime alloc values");
        ck(cudaMalloc(&d_error, sizeof(int)), "cluster LUT runtime alloc error");
        ck(cudaMemcpy(d_values, input.data(), states * sizeof(std::uint32_t),
                      cudaMemcpyHostToDevice), "cluster LUT runtime copy input");
        ck(cudaMemset(d_error, 0, sizeof(int)), "cluster LUT runtime zero error");
        ...
        for (int o = 0; o <= outer_bits; ++o) {
            ...
            const auto choice = choose_lut_runtime_bucket(
                ...
                requested_max_cluster, rt);
            if (!choice.ok) {
                std::cerr << "FAIL cluster LUT runtime no executable bucket"
                          << " W=" << W << " start=" << start
                          << " outer=" << o << '\n';
                return 7;
            }
        ...
        ck(cudaDeviceSynchronize(), "cluster LUT runtime sync");
        ...
        ck(cudaMemcpy(output.data(), d_values, states * sizeof(std::uint32_t),
                      cudaMemcpyDeviceToHost), "cluster LUT runtime copy output");
        ck(cudaMemcpy(&error, d_error, sizeof(error), cudaMemcpyDeviceToHost),
           "cluster LUT runtime copy error");
        if (error || fused_weight != states || output != reference) {
            std::cerr << "FAIL cluster LUT runtime arithmetic"
                      << " W=" << W << " start=" << start
                      << " error=" << error
        ...
        cudaFree(d_values);
        cudaFree(d_error);
        std::cout << "two-cell-fusion2-cluster-LUT-runtime"
                  << " W=" << W
                  << " start=" << start
    ...
    cudaFree(d_lut);
    cudaFree(d_offset);
    std::cout << "ALL_OK two_cell_fusion2_cluster_LUT_runtime=1 W=" << W << '\n';
    return 0;
}
