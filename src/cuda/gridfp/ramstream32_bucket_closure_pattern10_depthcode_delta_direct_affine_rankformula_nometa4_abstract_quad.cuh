...
#endif
#ifndef P10DC_RANKFORMULA_QUAD_LOCAL_DIRECT_MAX
#define P10DC_RANKFORMULA_QUAD_LOCAL_DIRECT_MAX 0
#endif
static_assert(P10DC_RANKFORMULA_QUAD_LOCAL_DIRECT_MAX >= 0 &&
              P10DC_RANKFORMULA_QUAD_LOCAL_DIRECT_MAX <= BKCZ_MAX_LOCAL,
              "QUAD_LOCAL_DIRECT_MAX must be in [0,BKCZ_MAX_LOCAL]");
...
__device__ __forceinline__ void p10dc_rankformula_quad_local_register(
    const P10DCDirectHighResolvedCtx& c,
    uint32_t lr0, uint32_t lr1, uint32_t lr2, uint32_t lr3,
    BkczCrossAccum& s0, BkczCrossAccum& s1,
    BkczCrossAccum& s2, BkczCrossAccum& s3
) {
#pragma unroll
...
#if P10DC_RANKFORMULA_CPASYNC_PAIR
    if (uint32_t(c.local_n) <= uint32_t(P10DC_RANKFORMULA_QUAD_LOCAL_DIRECT_MAX)) {
        // Small plans do not amortize 28 shared destinations, four async commits
        // and staged waits. local_n=0 is especially common enough to deserve a
        // zero-work path; larger thresholds are exposed as an A/B knob.
