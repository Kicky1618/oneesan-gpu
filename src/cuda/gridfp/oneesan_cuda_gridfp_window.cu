...
int main(int argc,char**argv){
    ...
    std::cout<<"backend=gridfp-stream n="<<n<<" residue="<<ans<<" modulus="<<mod<<" main_states="<<mainN<<" blocked_states="<<blockN<<" external_bytes="<<size_t(mainN+blockN)*8ULL<<" max_group_main="<<maxGM<<" max_group_blocked="<<maxGD<<" max_vram_bytes="<<size_t(2*maxGM+maxGD)*8ULL<<" max_intervals="<<maxIntervals<<" windows="<<total_windows<<" max_groups="<<max_groups<<" max_window_len="<<max_window_len<<" target_mib="<<target_mib<<" max_window_cfg="<<max_window<<" transfer_gib="<<transferred/(1ull<<30)<<" gpu_ms="<<ms<<"\n";
