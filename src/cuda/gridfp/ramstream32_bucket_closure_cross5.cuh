...
static constexpr uint8_t p10dc_cross5_host_entry(uint32_t key,uint32_t input_state){uint32_t s=input_state;uint8_t mask=0;bool halt=false;for(int pos=P10DC_CROSS5_CHUNK-1;pos>=0;--pos){uint32_t v=(key/p10dc_cross5_pow3_host(pos))%3u;if(v==uint32_t(R)){if(s==1u){halt=true;break;}--s;}else if(v==uint32_t(::L)){if(s==1u)mask=uint8_t(mask|uint8_t(1u<<pos));++s;}}return uint8_t(mask|(uint8_t(halt)<<P10DC_CROSS5_HALT_SHIFT));}
static constexpr int8_t p10dc_cross5_host_delta(uint32_t key){int d=0;for(int pos=0;pos<P10DC_CROSS5_CHUNK;++pos){uint32_t v=(key/p10dc_cross5_pow3_host(pos))%3u;if(v==uint32_t(R))--d;else if(v==uint32_t(::L))++d;}return int8_t(d);}
