...
static void gdmp_enable_full_p2p(int ngpu) {
    int peers=0;
    for (int a=0;a<ngpu;++a) for (int b=0;b<ngpu;++b) if (a!=b) {
        int can=0; ck(cudaDeviceCanAccessPeer(&can,a,b),"gdmp can peer");
        if (!can) continue;
        ck(cudaSetDevice(a),"gdmp set p2p device");
        cudaError_t e=cudaDeviceEnablePeerAccess(b,0);
        if (e==cudaErrorPeerAccessAlreadyEnabled) cudaGetLastError();
        else ck(e,"gdmp enable peer");
        ++peers;
    }
    if (ngpu>1 && peers!=ngpu*(ngpu-1)) {
        std::cerr<<"gdmp requires full P2P: "<<peers<<'/'<<ngpu*(ngpu-1)<<'\n';
        std::exit(183);
    }
}
...
int main(int argc,char**argv) {
    ...
    GdmpPeerStats plan_peer=gdmp_peer_stats(plan_stage,layout,plan_shard,plan_ng);
    double prepare_host_s=gdg_seconds(prep0);
    if (plan_only) {
        std::cout<<"backend=gridfp-gpu-direct-atomicfree-multigpu-peerstreams-v0.4-plan"
                 <<" n="<<n<<" gpus="<<plan_ng
                 ...
                 <<" peer_block_copies_per_row="<<plan_peer.copy_ops_per_row
                 <<" active_peer_pairs="<<plan_peer.active_peer_pairs
                 <<" max_peer_pair_gib_per_row="<<double(plan_peer.max_peer_pair_bytes_per_row)/double(1ULL<<30)
    ...
    GdmpPeerStats peer_stats=gdmp_peer_stats(stage_plan,layout,shard,ngpu);

    std::array<size_t,GDM_MAX_GPU> free_before{},total_before{};
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdmp mem device");
        ck(cudaMemGetInfo(&free_before[d],&total_before[d]),"gdmp mem info");
        size_t need=size_t(shard.total_elems[d])*sizeof(Count)+stage_bytes+transient_meta;
        if (need>free_before[d]) {
            std::cerr<<"gdmp insufficient HBM device="<<d
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdmp alloc device");
        if (shard.main_elems[d])
            ck(cudaMalloc(&main_ptr[d],size_t(shard.main_elems[d])*sizeof(Count)),"gdmp alloc main");
        if (shard.block_elems[d])
            ck(cudaMalloc(&block_ptr[d],size_t(shard.block_elems[d])*sizeof(Count)),"gdmp alloc block");
        if (shard.main_elems[d])
            ck(cudaMemset(main_ptr[d],0,size_t(shard.main_elems[d])*sizeof(Count)),"gdmp zero main");
        if (shard.block_elems[d])
            ck(cudaMemset(block_ptr[d],0,size_t(shard.block_elems[d])*sizeof(Count)),"gdmp zero block");
    }
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdmp metadata device");
        ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"gdmp modulus");
    ...
    Count one=1;
    ck(cudaSetDevice(init_loc.first),"gdmp init device");
    ck(cudaMemcpy(main_ptr[init_loc.first]+init_loc.second,&one,sizeof(one),cudaMemcpyHostToDevice),"gdmp init");
    ...
    for (int row=0;row<W;++row) {
        ...
        gdmp_enqueue_high(pipe,stage_plan,layout,shard,main_ptr,threads,grid_x,grid_y,slot);
        gdmp_enqueue_low(pipe,stage_plan,layout,shard,main_ptr,threads,grid_x,grid_y,slot);
        submit_s+=gdg_seconds(t);
        if (slot!=pipe.slots) {
            std::cerr<<"gdmp slot mismatch "<<slot<<'/'<<pipe.slots<<'\n';
            return 5;
        }
    ...
    Count answer=0;
    ck(cudaSetDevice(final_loc.first),"gdmp answer device");
    ck(cudaMemcpy(&answer,main_ptr[final_loc.first]+final_loc.second,sizeof(answer),cudaMemcpyDeviceToHost),"gdmp answer");

    std::cout<<"backend=gridfp-gpu-direct-atomicfree-multigpu-peerstreams-v0.4"
             ...
             <<" peer_block_copies_per_row="<<peer_stats.copy_ops_per_row
             <<" active_peer_pairs="<<peer_stats.active_peer_pairs
             <<" max_peer_pair_gib_per_row="<<double(peer_stats.max_peer_pair_bytes_per_row)/double(1ULL<<30)
    ...
    pipe.destroy();
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdmp release device");
        cross_dev[d].release();
        ordinary_dev[d].release();
        base[d].release();
