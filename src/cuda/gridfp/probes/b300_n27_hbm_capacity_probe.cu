...
static ForcedPeak forced_peak(int W, int hi, int lo) {
    ...
    for (uint32_t g = 0; g < ng; ++g) {
        ...
        if (bytes > z.bytes) z = {ms.size, ds.size, g, bytes};
    }
    return z;
}

static double gib(long double bytes) {
    return static_cast<double>(bytes / static_cast<long double>(1ULL << 30));
}
static double gb(long double bytes) {
    return static_cast<double>(bytes / 1.0e9L);
}
...
int main() {
    ...
    long double auth_bytes =
        (static_cast<long double>(main_chunk) + static_cast<long double>(block_chunk))
        * sizeof(Count);
    ...
    long double high_orbit_bytes =
        static_cast<long double>(highorbit.rec.size()) * sizeof(uint64_t);
    long double low_orbit_bytes =
        static_cast<long double>(loworbit.rec.size()) * sizeof(uint64_t);
    ...
    long double canonical_map_bytes = static_cast<long double>(
    ...
    long double compact_meta = desc_bytes + high_orbit_bytes + low_orbit_bytes
                             + mask_bytes + canonical_map_bytes;

    long double old_low_dense =
        static_cast<long double>(size_t(1) << (2 * LOW_LUT_K)) * sizeof(uint32_t);
    long double old_high_dense =
        static_cast<long double>(size_t(1) << (2 * HIGH_LUT_K)) * sizeof(uint32_t);

    long double need_compact = auth_bytes + static_cast<long double>(scratch_peak) + compact_meta;
    long double cap_288 = 288.0e9L;
    // B300X-279C is a conservative vGPU full-profile proxy, not a claim about
    // bare-metal cudaMemGetInfo.  Use it only as a stress-test capacity budget.
    long double cap_279 = 279.0e9L;
    long double auth_total =
        static_cast<long double>(main_n + block_n) * sizeof(Count);

    std::cout << std::fixed << std::setprecision(3)
        << "b300-hbm-capacity W=" << W
        << " main_states=" << main_n
        << " block_states=" << block_n
        << " auth_total_gib=" << gib(auth_total)
        << " auth_shard_gib=" << gib(auth_bytes)
        << '\n'
        << "forced_high groups=" << (1u << LOW_LUT_K)
        << " max_main=" << high.main_states
        << " max_block=" << high.block_states
        << " scratch_gib=" << gib(static_cast<long double>(high.bytes))
        << " group=" << high.group
        << '\n'
        << "forced_low groups=" << (1u << HIGH_LUT_K)
        << " max_main=" << low.main_states
        << " max_block=" << low.block_states
        << " scratch_gib=" << gib(static_cast<long double>(low.bytes))
        << " group=" << low.group
        << '\n'
        << "scratch_peak_gib=" << gib(static_cast<long double>(scratch_peak))
        << " descriptors_mib=" << static_cast<double>(desc_bytes / (1 << 20))
        << " high_orbit_mib=" << static_cast<double>(high_orbit_bytes / (1 << 20))
        << " low_orbit_mib=" << static_cast<double>(low_orbit_bytes / (1 << 20))
        << " mask_tables_mib=" << static_cast<double>(mask_bytes / (1 << 20))
        << " canonical_maps_mib=" << static_cast<double>(canonical_map_bytes / (1 << 20))
        << " compact_meta_mib=" << static_cast<double>(compact_meta / (1 << 20))
        << " legacy_low_dense_rank_mib=" << static_cast<double>(old_low_dense / (1 << 20))
        << " legacy_high_dense_rank_mib=" << static_cast<double>(old_high_dense / (1 << 20))
        << '\n'
    ...
    for (int reserve_gib : {2, 4, 8}) {
        long double r = static_cast<long double>(reserve_gib) * (1ULL << 30);
