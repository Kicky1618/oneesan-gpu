...
static void brf_run_low(
    ...
    for(uint32_t fixed=0;fixed<BUCKET_NGPU;++fixed){
        ...
        dt.bind_owner(fixed,phy,d);bucket_launch_reverse_low_fused(layout,256,4,4);ck(cudaDeviceSynchronize(),"brf low sync");
...
static void brf_run_high(
    ...
    for(uint32_t fixed=0;fixed<BUCKET_NGPU;++fixed){
        ...
        dt.bind_owner(fixed,phy,d);bucket_launch_reverse_high_fused(layout,256,4,4);ck(cudaDeviceSynchronize(),"brf high sync");
...
int main(){
    ...
    int visible=0;cudaError_t ce=cudaGetDeviceCount(&visible);if(ce!=cudaSuccess||visible<1){std::cout<<"bucket-reverse-fused-selftest SKIP no CUDA device\n";return 0;}
    ...
    std::mt19937_64 rng(0x1618fusedULL);std::vector<Count>im(ms.size()),ib(bs.size());for(auto&x:im)x=Count(rng()%mod);for(auto&x:ib)x=Count(rng()%mod);
