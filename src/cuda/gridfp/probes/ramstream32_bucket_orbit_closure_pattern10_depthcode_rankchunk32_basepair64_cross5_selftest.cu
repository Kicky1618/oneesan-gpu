...
#undef main
#define main pattern10_depthcode_rankchunk32_basepair64_reference_main_unused
#include "ramstream32_bucket_reverse_fused_selftest.cu"
#pragma pop_macro("main")

#include "../ramstream32_bucket_orbit_closure_pattern10_depthcode_warpstriped_delta_direct_affine_rankchunk32_basepair64_cross5.cuh"

static std::array<uint8_t, BUCKET_NGPU> P10DC_RANKCHUNK32_BASEPAIR64_TABLE_VERIFIED{};

static uint32_t p10dc_rankchunk32_basepair64_decode_host(uint32_t pair, size_t block32_index) {
    ...
    return base + ((block32_index & 1u) ? delta : 0u);
}

static void p10dc_verify_rankchunk32_basepair64_tables(
    const BucketFusedHost& bf,
    const BucketFusedDirectHighRowsRankChunk32BasePair64Tables& dt,
    uint32_t fixed
) {
    if (P10DC_RANKCHUNK32_BASEPAIR64_TABLE_VERIFIED[fixed]) return;
    ...
    const size_t base32_count = (meta_count + 31u) >> 5;
    const size_t pair_count = (base32_count + 1u) >> 1;
    if (meta_count < code_count ||
        dt.low_rankchunkblock16_count != pair_count ||
        dt.low_rankchunk_padding_count != meta_count - code_count ||
        dt.low_prekey != nullptr || dt.low_rankstream_off != nullptr) {
        std::cerr << "p10dc rankchunk32 basepair64 count/lifetime mismatch owner=" << fixed
                  << " meta=" << meta_count << " codes=" << code_count
                  << " padding=" << dt.low_rankchunk_padding_count
                  << " pairs=" << dt.low_rankchunkblock16_count << '/' << pair_count << '\n';
        std::exit(680);
    }

    std::vector<uint32_t> got_meta(meta_count), got_pairs(pair_count);
    if (!got_meta.empty()) ck(cudaMemcpy(got_meta.data(), dt.low_rankchunkmeta32,
        got_meta.size()*sizeof(uint32_t), cudaMemcpyDeviceToHost),
        "p10dc rankchunk32 basepair64 meta verify D2H");
    if (!got_pairs.empty()) ck(cudaMemcpy(got_pairs.data(), dt.low_rankchunkblock16,
        got_pairs.size()*sizeof(uint32_t), cudaMemcpyDeviceToHost),
        "p10dc rankchunk32 basepair64 pairs verify D2H");
    ...
    ck(cudaMemcpyFromSymbol(got_hoff.data(), D_P10DC_LOW_RANKCHUNK_HOFF,
        ...
        "p10dc rankchunk32 basepair64 height offsets verify");
    ...
    auto verify_block_start = [&](size_t c) {
        if ((c & 31u) != 0u) return;
        const size_t bi = c >> 5;
        const size_t pi = bi >> 1;
        current_block_base = uint32_t(stream_ix);
        const uint32_t got = pi < got_pairs.size()
            ? p10dc_rankchunk32_basepair64_decode_host(got_pairs[pi], bi)
            : 0xffffffffu;
        if (got != current_block_base) {
            std::cerr << "p10dc rankchunk32 basepair64 block base mismatch owner=" << fixed
                      << " compact=" << c << " block32=" << bi
                      << " pair=" << pi << " got=" << got
    ...
    for (uint32_t h = 0; h < uint32_t(MAXW + 2); ++h) {
        while ((compact & 31u) != 0u) {
            verify_block_start(compact);
            if (compact >= got_meta.size() || got_meta[compact] != 0u) {
                std::cerr << "p10dc rankchunk32 basepair64 padding mismatch owner=" << fixed
                          << " h=" << h << " compact=" << compact << '\n';
                std::exit(682);
            }
            ++compact;
        ...
        if (got_hoff[h] != compact || (got_hoff[h] & 31u) != 0u) {
            std::cerr << "p10dc rankchunk32 basepair64 height offset mismatch owner=" << fixed
                      << " h=" << h << " got=" << got_hoff[h]
                      << " expected=" << compact << '\n';
            std::exit(683);
        }
        ...
        for (uint32_t i = a; i < b; ++i, ++compact, ++actual_codes) {
            ...
            if (p10dc_rankchunk32_unpack_host(chunks) != key ||
                prefix >= 256u || compact >= got_meta.size() || got_meta[compact] != expected) {
                std::cerr << "p10dc rankchunk32 basepair64 meta mismatch owner=" << fixed
                          << " h=" << h << " compact=" << compact
                          << " prefix=" << prefix << '\n';
                std::exit(684);
            }
    ...
    if (actual_codes != code_count || compact != meta_count ||
        ...
        std::cerr << "p10dc rankchunk32 basepair64 walk mismatch owner=" << fixed
    ...
    P10DC_RANKCHUNK32_BASEPAIR64_TABLE_VERIFIED[fixed] = 1;
}

static void p10dc_rankchunk32_basepair64_run_high(
    BucketHostGrid& g, const StorageLayout& layout, const BucketPhysicalLayoutHost& phy,
    const BucketFusedHost& bf, BucketFusedDirectHighRowsRankChunk32BasePair64Tables& dt,
    ...
    for (uint32_t fixed = 0; fixed < BUCKET_NGPU; ++fixed) {
        ...
        for (uint32_t s = 0; s < BUCKET_NGPU; ++s) {
            ...
            if (!v.empty()) ck(cudaMemcpy(d[s], v.data(), v.size()*sizeof(Count),
                               ...
                               "p10dc rankchunk32 basepair64 high H2D");
        }
        dt.bind_owner(fixed, phy, d);
        p10dc_verify_rankchunk32_basepair64_tables(bf, dt, fixed);
        if (!rev) {
            for (int p = TARGET_W - 1; p >= LOW_LUT_K + 1; --p) {
                if (experiment)
                    bucket_high_orbit_closure_pattern10_depthcode_warpstriped_delta_direct_affine_rankchunk32_basepair64_cross5_kernel<<<grid, block, smem>>>(p);
                else bucket_high_orbit_closure_pattern10_depthcode_resolved_kernel<<<grid, block>>>(p);
                ck(cudaGetLastError(), experiment
                    ? "p10dc rankchunk32 basepair64 forward high"
                    : "p10dc rankchunk32 basepair64 forward control");
            }
        } else {
            for (int p = LOW_LUT_K + 1; p < TARGET_W; ++p) {
                if (experiment)
                    bucket_reverse_high_pattern10_depthcode_warpstriped_delta_direct_affine_rankchunk32_basepair64_cross5_kernel<<<grid, block, smem>>>(p);
                else bucket_reverse_high_pattern10_depthcode_resolved_kernel<<<grid, block>>>(p);
                ck(cudaGetLastError(), experiment
                    ? "p10dc rankchunk32 basepair64 reverse high"
                    : "p10dc rankchunk32 basepair64 reverse control");
            }
        }
        ck(cudaDeviceSynchronize(), "p10dc rankchunk32 basepair64 high sync");
        for (uint32_t s = 0; s < BUCKET_NGPU; ++s) {
            auto& v = g[s][fixed];
            if (!v.empty()) ck(cudaMemcpy(v.data(), d[s], v.size()*sizeof(Count),
                                          cudaMemcpyDeviceToHost),
                               "p10dc rankchunk32 basepair64 high D2H");
        }
        bkft_free_slots(d);
    }
}
...
int main() {
    ...
    static_assert(W <= 12, "rankchunk32 basepair64 selftest intentionally uses small width");
    static_assert(P10DC_RANKCHUNK32_BYTEPACK == 1);
    static_assert(P10DC_RANKCHUNK32_ALIGN32 == 1);
    ...
    if (ce != cudaSuccess || visible < 1) {
        std::cout << "bucket-closure-pattern10-depthcode-rankchunk32-basepair64-cross5-selftest SKIP no CUDA device\n";
        return 0;
    }
    ck(cudaSetDevice(0), "p10dc rankchunk32 basepair64 set device");
    ...
    ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"p10dc rankchunk32 basepair64 modulus");
    BucketFusedDirectHighRowsRankChunk32BasePair64Tables dt; dt.install_metadata(layout,bo,bf);
    ...
    auto g0=bkft_make_grid(ms,bs,im,ib,storage,layout,owner,phy); p10dc_rankchunk32_basepair64_run_high(g0,layout,phy,bf,dt,false,false); if(!bkft_compare("pattern10-depthcode-forward-high-resolved-rankchunk32-basepair64-control",g0,ms,bs,fhm,fhb,storage,layout,owner,phy))return 90;
    auto g1=bkft_make_grid(ms,bs,im,ib,storage,layout,owner,phy); p10dc_rankchunk32_basepair64_run_high(g1,layout,phy,bf,dt,false,true); if(!bkft_compare("pattern10-depthcode-forward-high-rankchunk32-basepair64-cross5",g1,ms,bs,fhm,fhb,storage,layout,owner,phy))return 91;
    auto g2=bkft_make_grid(ms,bs,im,ib,storage,layout,owner,phy); p10dc_rankchunk32_basepair64_run_high(g2,layout,phy,bf,dt,true,false); if(!bkft_compare("pattern10-depthcode-reverse-high-resolved-rankchunk32-basepair64-control",g2,ms,bs,rhm,rhb,storage,layout,owner,phy))return 92;
    auto g3=bkft_make_grid(ms,bs,im,ib,storage,layout,owner,phy); p10dc_rankchunk32_basepair64_run_high(g3,layout,phy,bf,dt,true,true); if(!bkft_compare("pattern10-depthcode-reverse-high-rankchunk32-basepair64-cross5",g3,ms,bs,rhm,rhb,storage,layout,owner,phy))return 93;
    for(uint32_t g=0;g<BUCKET_NGPU;++g) if(!P10DC_RANKCHUNK32_BASEPAIR64_TABLE_VERIFIED[g]) return 94;
    rdt.release(); fdt.release(); dt.release();
    std::cout << "bucket-closure-pattern10-depthcode-rankchunk32-basepair64-cross5-selftest OK W=" << W
              << " control=resolved experiment=warpstriped_delta_direct_affine_rankchunk32_basepair64_cross5"
              << " forward_exact=1 reverse_exact=1 rankchunk32_table_exact=1 pair_table_exact=1 padding_exact=1"
              << " chunk_bits=24 prefix_bits=8 block=32 height_align=32"
              << " base_bits=22 delta_bits=8 codes_per_pair=64 block_base_bytes_per_code=0.0625"
              << " block_base_loads_per_warp_max=1 byte_aligned_chunks=1"
