...
int main(){
    ...
    size_t legacy_keys=size_t(2u*2u*P10DC_LEGACY_STREAMS*uint32_t(TARGET_W)*P10DC_HDIM);size_t legacy_dense=legacy_keys*size_t(P10DC_PAIR_COUNT)/8;

    uint64_t payload_checked=0,valid_payloads=0,none_payloads=0;
    p10dc_for_each_entry_direct(layout,bo,rh.split,bf,[&](P10DepthCodeEntryView e){
        uint32_t code=uint32_t(bkcp10_id(*e.op)),want=p10dc_payload_host(e.pair,e.high,e.p),got=0;
        if(code==uint32_t(oneesan::gridfp::CLOSURE_PATTERN10_NONE)){
            if(e.pair!=P10DC_NONE_PAIR||want!=0){std::cerr<<"depthcode NONE sentinel mismatch pair="<<e.pair<<" want="<<want<<'\n';std::exit(610);}++none_payloads;
        }else{
            uint32_t key=p10dc_key(e.rev,e.high,e.sid,e.p,e.h,rh.codebook.mode);if(key>=rh.codebook.base.size()){std::cerr<<"depthcode payload key overflow\n";std::exit(611);}uint32_t base=rh.codebook.base[key];if(base==P10DC_INVALID_BASE){std::cerr<<"depthcode payload missing context\n";std::exit(612);}if(base+code>=rh.codebook.decode.size()){std::cerr<<"depthcode payload code overflow\n";std::exit(613);}got=rh.codebook.decode[base+code];
        }
        if(got!=want){std::cerr<<"depthcode payload mismatch rev="<<e.rev<<" high="<<e.high<<" p="<<e.p<<" h="<<e.h<<" sid="<<e.sid<<" code="<<code<<" got=0x"<<std::hex<<got<<" want=0x"<<want<<std::dec<<'\n';std::exit(614);}++payload_checked;valid_payloads+=p10dc_payload_valid(got);
    });
