...
int main(){
    ...
    size_t fops=bo.low_nn.size()+bo.low_nr.size()+bo.low_nl.size()+bo.high_nn.size()+bo.high_nrnl.size();size_t rops=rh.split.low.ops()+rh.split.high.ops();size_t ops=fops+rops;size_t depth4=(ops+1)/2;size_t legacy_dense=size_t(P10DC_KEY_COUNT)*size_t(P10DC_PAIR_COUNT)/8;
