...
#undef main
#define main pattern10_depthcode_rankformula_nometa4_abstract_reference_main_unused
...
static void p10dc_rankformula_nometa4_abstract_run_high(
    BucketHostGrid& g, const StorageLayout& layout, const BucketPhysicalLayoutHost& phy,
    BucketFusedDirectHighRowsRankFormulaNometa4Tables& dt,
    ...
    for (uint32_t fixed = 0; fixed < BUCKET_NGPU; ++fixed) {
        ...
        for (uint32_t s = 0; s < BUCKET_NGPU; ++s) {
            ...
            if (!v.empty()) ck(cudaMemcpy(d[s], v.data(), v.size()*sizeof(Count), cudaMemcpyHostToDevice),
                               "p10dc rankformula nometa4 abstract H2D");
        }
        dt.bind_owner(fixed, phy, d);
        if (!rev) {
            for (int p = TARGET_W - 1; p >= LOW_LUT_K + 1; --p) {
                if (experiment)
                    bucket_high_orbit_closure_pattern10_depthcode_warpstriped_delta_direct_affine_rankformula_nometa4_abstract_kernel<<<grid,block,smem>>>(p);
                else
                    bucket_high_orbit_closure_pattern10_depthcode_resolved_kernel<<<grid,block>>>(p);
                ck(cudaGetLastError(), experiment ? "p10dc nometa4 abstract forward" : "p10dc nometa4 abstract forward control");
            }
        } else {
            for (int p = LOW_LUT_K + 1; p < TARGET_W; ++p) {
                if (experiment)
                    bucket_reverse_high_pattern10_depthcode_warpstriped_delta_direct_affine_rankformula_nometa4_abstract_kernel<<<grid,block,smem>>>(p);
                else
                    bucket_reverse_high_pattern10_depthcode_resolved_kernel<<<grid,block>>>(p);
                ck(cudaGetLastError(), experiment ? "p10dc nometa4 abstract reverse" : "p10dc nometa4 abstract reverse control");
            }
        }
        ck(cudaDeviceSynchronize(), "p10dc rankformula nometa4 abstract high sync");
        for (uint32_t s = 0; s < BUCKET_NGPU; ++s) {
            auto& v = g[s][fixed];
            if (!v.empty()) ck(cudaMemcpy(v.data(), d[s], v.size()*sizeof(Count), cudaMemcpyDeviceToHost),
                               "p10dc rankformula nometa4 abstract D2H");
        }
        bkft_free_slots(d);
    }
}
...
int main() {
    ...
    static_assert(W <= 12, "rankformula nometa4 abstract selftest intentionally uses small width");
    static_assert(P10DC_RANKCHUNK32_BYTEPACK == 0);

    int visible = 0;
    cudaError_t ce = cudaGetDeviceCount(&visible);
    if (ce != cudaSuccess || visible < 1) {
        std::cout << "bucket-closure-pattern10-depthcode-rankformula-nometa4-abstract-selftest SKIP no CUDA device\n";
        return 0;
    }
    ck(cudaSetDevice(0), "p10dc rankformula nometa4 abstract set device");
    ...
    ck(cudaMemcpyToSymbol(D_MOD, &mod, sizeof(mod)), "p10dc nometa4 abstract modulus");
    BucketFusedDirectHighRowsRankFormulaNometa4Tables dt;
    ...
    p10dc_rankformula_nometa4_abstract_run_high(g0, layout, phy, dt, false, false);
    if (!bkft_compare("rankformula-nometa4-abstract-forward-control", g0, ms, bs, fhm, fhb, storage, layout, owner, phy)) return 90;
    auto g1 = bkft_make_grid(ms, bs, im, ib, storage, layout, owner, phy);
    p10dc_rankformula_nometa4_abstract_run_high(g1, layout, phy, dt, false, true);
    if (!bkft_compare("rankformula-nometa4-abstract-forward", g1, ms, bs, fhm, fhb, storage, layout, owner, phy)) return 91;
    auto g2 = bkft_make_grid(ms, bs, im, ib, storage, layout, owner, phy);
    p10dc_rankformula_nometa4_abstract_run_high(g2, layout, phy, dt, true, false);
    if (!bkft_compare("rankformula-nometa4-abstract-reverse-control", g2, ms, bs, rhm, rhb, storage, layout, owner, phy)) return 92;
    auto g3 = bkft_make_grid(ms, bs, im, ib, storage, layout, owner, phy);
    p10dc_rankformula_nometa4_abstract_run_high(g3, layout, phy, dt, true, true);
    if (!bkft_compare("rankformula-nometa4-abstract-reverse", g3, ms, bs, rhm, rhb, storage, layout, owner, phy)) return 93;

    rdt.release(); fdt.release(); dt.release();
    std::cout << "bucket-closure-pattern10-depthcode-rankformula-nometa4-abstract-selftest OK W=" << W
              << " forward_exact=1 reverse_exact=1"
              << " per_code_metadata_bytes=0 block_size=4 max_locator_steps=3"
              << " abstract_lut_bytes=94206 abstract_states=7060 abstract_transitions=32743"
              << " ballot_runtime_loads=0 source_local_lut=1 rankstream_bytes=0\n";
    return 0;
}
