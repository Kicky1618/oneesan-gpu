...
__device__ __forceinline__ BkczPlan bkcp10_build_low_plan(MateID d,int fixed_he,int p,uint16_t id){
    ...
    int depth=low_cross_preimage_partial(d,LOW_LUT_K+1,p,x);if(depth>0){uint32_t loc=0,bid=0;if(bkcz_low_source_ref(x,fixed_he+2,loc,bid))bkcz_plan_set_cross(z,bid,loc,uint32_t(depth));}return z;
}
__device__ __forceinline__ BkczPlan bkcp10_build_high_plan(MateID d,int fixed_hs,int rel,uint16_t id){
    ...
    int depth=high_cross_preimage_partial(d,HIGH_LUT_K+1,rel,x);if(depth>0){uint32_t loc=0,bid=0;if(bkcz_high_source_ref(x,fixed_hs+2,loc,bid))bkcz_plan_set_cross(z,bid,loc,uint32_t(depth));}return z;
}
