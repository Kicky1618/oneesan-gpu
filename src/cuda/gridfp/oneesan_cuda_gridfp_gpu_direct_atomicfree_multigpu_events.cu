#define main gpu_direct_atomicfree_multigpu_v01_main_unused
#include "oneesan_cuda_gridfp_gpu_direct_atomicfree_multigpu.cu"
#undef main

#include "ramstream32_gpu_direct_atomicfree_multigpu_events.cuh"
...
int main(int argc,char**argv) {
    ...
    if (plan_only) {
        std::cout<<"backend=gridfp-gpu-direct-atomicfree-multigpu-events-v0.2-plan"
                 <<" n="<<n<<" gpus="<<plan_ng
                 ...
                 <<" device_fences_per_row="<<3*(LOW_LUT_K+HIGH_LUT_K)
                 <<" host_barrier_points_per_row=1 event_slots="<<3*(LOW_LUT_K+HIGH_LUT_K)
                 <<" prepare_s="<<prepare_host_s<<'\n';
        return 0;
    }

    int visible=0; ck(cudaGetDeviceCount(&visible),"gdm event device count");
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdm event mem device"); ck(cudaMemGetInfo(&free_before[d],&total_before[d]),"gdm event mem info");
        size_t need=size_t(shard.total_elems[d])*sizeof(Count)+transient_meta;
        if (need>free_before[d]) {
            std::cerr<<"gdm event insufficient HBM device="<<d<<" need_gib="<<double(need)/double(1ULL<<30)
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdm event alloc device");
        if (shard.main_elems[d]) ck(cudaMalloc(&main_ptr[d],size_t(shard.main_elems[d])*sizeof(Count)),"gdm event alloc main");
        if (shard.block_elems[d]) ck(cudaMalloc(&block_ptr[d],size_t(shard.block_elems[d])*sizeof(Count)),"gdm event alloc block");
        if (shard.main_elems[d]) ck(cudaMemset(main_ptr[d],0,size_t(shard.main_elems[d])*sizeof(Count)),"gdm event zero main");
        if (shard.block_elems[d]) ck(cudaMemset(block_ptr[d],0,size_t(shard.block_elems[d])*sizeof(Count)),"gdm event zero block");
    }
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdm event metadata device"); ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"gdm event modulus");
    ...
    ck(cudaSetDevice(init_loc.first),"gdm event init device");
    ck(cudaMemcpy(main_ptr[init_loc.first]+init_loc.second,&one,sizeof(one),cudaMemcpyHostToDevice),"gdm event init");

    double submit_s=0,row_sync_s=0; auto wall0=std::chrono::steady_clock::now();
    for (int row=0;row<W;++row) {
        int slot=0;
        auto t=std::chrono::steady_clock::now();
        gdm_enqueue_high_events(pipe,layout,threads,grid_x,grid_y,slot);
        gdm_enqueue_low_events(pipe,layout,threads,grid_x,grid_y,slot);
        submit_s+=gdg_seconds(t);
        if (slot!=pipe.slots) { std::cerr<<"gdm event slot mismatch "<<slot<<'/'<<pipe.slots<<'\n'; return 5; }
    ...
    ck(cudaSetDevice(final_loc.first),"gdm event answer device");
    ck(cudaMemcpy(&answer,main_ptr[final_loc.first]+final_loc.second,sizeof(answer),cudaMemcpyDeviceToHost),"gdm event answer");

    std::cout<<"backend=gridfp-gpu-direct-atomicfree-multigpu-events-v0.2"
             ...
             <<" host_barrier_points_per_row=1 cross_device_events=1\n";

    pipe.destroy();
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdm event release device"); cross_dev[d].release(); ordinary_dev[d].release(); base[d].release();
