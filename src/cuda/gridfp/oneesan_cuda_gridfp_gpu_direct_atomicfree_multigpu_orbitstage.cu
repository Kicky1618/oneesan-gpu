...
static void gdpo_enable_full_p2p(int ngpu) {
    int peers=0;
    for (int a=0;a<ngpu;++a) for (int b=0;b<ngpu;++b) if (a!=b) {
        int can=0; ck(cudaDeviceCanAccessPeer(&can,a,b),"gdpo can peer");
        if (!can) continue;
        ck(cudaSetDevice(a),"gdpo set p2p device");
        cudaError_t e=cudaDeviceEnablePeerAccess(b,0);
        if (e==cudaErrorPeerAccessAlreadyEnabled) cudaGetLastError();
        else ck(e,"gdpo enable peer");
        ++peers;
    }
    if (ngpu>1 && peers!=ngpu*(ngpu-1)) {
        std::cerr<<"gdpo requires full P2P: "<<peers<<'/'<<ngpu*(ngpu-1)<<'\n';
        std::exit(185);
    }
}
...
int main(int argc,char**argv) {
    ...
    GdpoOrbitPlan plan_orbit=build_gdpo_orbit_plan(layout,plan_shard,loworbit,highdirect,plan_ng);
    double prepare_host_s=gdg_seconds(prep0);
    if (plan_only) {
        std::cout<<"backend=gridfp-gpu-direct-atomicfree-multigpu-orbitstage-v0.5-plan"
                 <<" n="<<n<<" gpus="<<plan_ng
                 ...
                 <<" orbit_bulk_p2p_gib_per_row="<<double(plan_orbit.copy_bytes_per_row)/double(1ULL<<30)
                 <<" total_bulk_p2p_gib_per_row="<<double(plan_gather.copy_bytes_per_row+plan_orbit.copy_bytes_per_row)/double(1ULL<<30)
                 <<" gather_peer_block_copies_per_row="<<plan_peer.copy_ops_per_row
                 <<" orbit_peer_block_copies_per_row="<<plan_orbit.block_copies_per_row
                 <<" orbit_max_device_phase_copy_gib="<<double(plan_orbit.max_device_phase_bytes)/double(1ULL<<30)
                 ...
                 <<" remote_scalar_gather_loads=0 remote_orbit_reads=0 remote_orbit_writes=1"
    ...
    GdpoOrbitPlan orbit_plan=build_gdpo_orbit_plan(layout,shard,loworbit,highdirect,ngpu);

    std::array<size_t,GDM_MAX_GPU> free_before{},total_before{};
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdpo mem device");
        ck(cudaMemGetInfo(&free_before[d],&total_before[d]),"gdpo mem info");
        size_t need=size_t(shard.total_elems[d])*sizeof(Count)
            +main_stage_bytes+block_stage_bytes+transient_meta;
        if (need>free_before[d]) {
            std::cerr<<"gdpo insufficient HBM device="<<d
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdpo alloc device");
        if (shard.main_elems[d])
            ck(cudaMalloc(&main_ptr[d],size_t(shard.main_elems[d])*sizeof(Count)),"gdpo alloc main");
        if (shard.block_elems[d])
            ck(cudaMalloc(&block_ptr[d],size_t(shard.block_elems[d])*sizeof(Count)),"gdpo alloc block");
        if (shard.main_elems[d])
            ck(cudaMemset(main_ptr[d],0,size_t(shard.main_elems[d])*sizeof(Count)),"gdpo zero main");
        if (shard.block_elems[d])
            ck(cudaMemset(block_ptr[d],0,size_t(shard.block_elems[d])*sizeof(Count)),"gdpo zero block");
    }
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdpo metadata device");
        ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"gdpo modulus");
        base[d].install(storage,layout,lowdesc,loworbit,highdirect,forward);
        ordinary_dev[d].install(ordinary);
        gpu_direct_gather_drop_redundant(base[d]);
        cross_dev[d].install(cross);
        gpu_direct_cross_gather_drop_redundant(base[d],ordinary_dev[d]);
    ...
    auto init_loc=gdm_locate_main(init_rank,layout,shard);
    Count one=1;
    ck(cudaSetDevice(init_loc.first),"gdpo init device");
    ck(cudaMemcpy(main_ptr[init_loc.first]+init_loc.second,&one,sizeof(one),cudaMemcpyHostToDevice),"gdpo init");

    double submit_s=0,row_sync_s=0;
    auto wall0=std::chrono::steady_clock::now();
    for (int row=0;row<W;++row) {
        int slot=0;
        auto t=std::chrono::steady_clock::now();
        gdpo_enqueue_row(pipe,orbit_plan,gather_plan,layout,shard,main_ptr,block_ptr,
                         threads,grid_x,grid_y,slot);
        submit_s+=gdg_seconds(t);
        if (slot!=pipe.slots) {
            std::cerr<<"gdpo slot mismatch "<<slot<<'/'<<pipe.slots<<'\n';
            return 5;
        }
        t=std::chrono::steady_clock::now();
        pipe.sync_row();
        row_sync_s+=gdg_seconds(t);
    ...
    auto final_loc=gdm_locate_main(final_rank,layout,shard);
    Count answer=0;
    ck(cudaSetDevice(final_loc.first),"gdpo answer device");
    ck(cudaMemcpy(&answer,main_ptr[final_loc.first]+final_loc.second,sizeof(answer),cudaMemcpyDeviceToHost),"gdpo answer");

    std::cout<<"backend=gridfp-gpu-direct-atomicfree-multigpu-orbitstage-v0.5"
             ...
             <<" orbit_bulk_p2p_gib_per_row="<<double(orbit_plan.copy_bytes_per_row)/double(1ULL<<30)
             ...
             <<" orbit_peer_block_copies_per_row="<<orbit_plan.block_copies_per_row
    ...
    pipe.destroy();
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdpo release device");
