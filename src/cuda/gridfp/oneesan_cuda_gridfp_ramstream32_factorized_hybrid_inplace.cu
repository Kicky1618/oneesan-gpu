...
static void hybrid_inplace_release_dense_host(StorageFactorHost& storage) {
...
int main(int argc, char** argv) {
    ...
    if (high_wp.max_bytes > gpu_target) {
        std::cerr << "hybrid-inplace high window does not fit: high_gib="
    ...
    for (int i = 0; i < cpu_workers && i < int(cpu_jobs.size()); ++i)
        cpu_peak_est += cpu_jobs[size_t(i)].scratch_bytes / 2;
    ...
    if (plan_only) {
        std::cout
            << "backend=gridfp-ramstream32-factorized-hybrid-inplace-v4.1-plan"
            << " n=" << n
            ...
            << " cpu_low_outplace_max_gib=" << double(low_wp.max_bytes) / double(1ULL << 30)
            << " cpu_low_inplace_max_gib=" << double(low_wp.max_bytes) / double(2ULL << 30)
    ...
    hybrid_inplace_release_dense_host(storage);
    ...
    main_auth.alloc(layout.main_size, "mmap hybrid-inplace main");
    block_auth.alloc(layout.block_size, "mmap hybrid-inplace block");
    main_auth.ptr[init_rank] = 1;

    Direct2DCtx gpu;
    gpu.init(mod);
    CpuLowInplacePool cpu(cpu_workers);
    int gpu_threads = 256;
    ...
    std::cout
        << "backend=gridfp-ramstream32-factorized-hybrid-inplace-v4.1"
        << " n=" << n
        << " residue=" << answer
        << " modulus=" << mod
        << " gpu_high_desc_mib=" << highdesc_mib
