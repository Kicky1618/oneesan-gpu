...
static void process_group_bidesc(
    ...
    for (int p = wp.p_hi; p >= wp.p_lo; --p) {
        if (ms.size) ck(cudaMemcpy(nxt, cur, size_t(ms.size) * sizeof(Count),
                                   cudaMemcpyDeviceToDevice), "bidesc identity");
        if (ds.size) ck(cudaMemset(dnext, 0, size_t(ds.size) * sizeof(Count)),
                        "bidesc clear block");

        if (fix_low) {
            if (ms.size) main_group_highdesc_kernel<<<bm, gpu_threads>>>(
        ...
        ck(cudaGetLastError(), "bidesc transition");
        std::swap(cur, nxt);
        std::swap(dcur, dnext);
    }
    ck(cudaDeviceSynchronize(), "bidesc transition sync");
...
#ifndef RAMSTREAM_BIDESC_NO_MAIN
int main(int argc, char** argv) {
    ...
    if (high_wp.max_bytes > target || low_wp.max_bytes > target) {
        std::cerr << "bidesc forced window does not fit: high_gib="
    ...
    main_auth.alloc(layout.main_size, "mmap bidesc main");
    block_auth.alloc(layout.block_size, "mmap bidesc block");
    ...
    for (int row = 0; row < W; ++row) {
        for (const auto& job : high_jobs)
            if (job.work) process_group_bidesc(ctx, main_auth, block_auth, storage, layout,
