...
constexpr int FUSION_FACE_CANDIDATES = oneesan::twocell::cuda_face::kMaxCandidates;

__global__ void two_cell_fusion2_closed_face_kernel(
    ...
    __shared__ PackedKey sh_candidate[FUSION_WARPS][FUSION_FACE_CANDIDATES];
    __shared__ std::uint8_t sh_candidate_valid[FUSION_WARPS][FUSION_FACE_CANDIDATES];
...
Rank fusion2_closed_face_workspace_estimate() {
    return Rank(FUSION_WARPS) * (
        Rank(FUSION_MAX_COMPONENT) *
            (sizeof(PackedKey) + 2 * sizeof(std::uint32_t) + sizeof(Rank)) +
        Rank(FUSION_FACE_CANDIDATES) *
            (sizeof(PackedKey) + sizeof(std::uint8_t)) +
        sizeof(PackedWord) + 3 * sizeof(int)) + 512ULL;
}

void print_fusion2_closed_face_plan(
    int W,
    Rank shared_limit,
    const RankTables& rt
) {
    const Rank reserve = fusion2_closed_face_workspace_estimate();
    ...
    std::cout << "fusion2_closed_face_plan"
              << " W=" << W
...
void run_fusion2_closed_face(
    ...
    ck(cudaFuncGetAttributes(&attr, two_cell_fusion2_closed_face_kernel),
       "fusion closed face attributes");
    cudaDeviceProp prop{};
    ck(cudaGetDeviceProperties(&prop, 0), "fusion closed face props");
    ...
    for (Rank r = 0; r < states; ++r)
        input[static_cast<std::size_t>(r)] = static_cast<std::uint32_t>(
            1 + ((r * 2654435761ULL + Rank(start) * 251ULL) % (mod - 1ULL)));
    ...
    ck(cudaMalloc(&d_values, states * sizeof(std::uint32_t)), "fusion closed face alloc values");
    ck(cudaMalloc(&d_blocks, sizeof(unsigned long long)), "fusion closed face alloc blocks");
    ck(cudaMalloc(&d_components, sizeof(unsigned long long)), "fusion closed face alloc components");
    ck(cudaMalloc(&d_deep, sizeof(unsigned long long)), "fusion closed face alloc deep");
    ck(cudaMalloc(&d_loads, sizeof(unsigned long long)), "fusion closed face alloc loads");
    ck(cudaMalloc(&d_stores, sizeof(unsigned long long)), "fusion closed face alloc stores");
    ck(cudaMalloc(&d_adds, sizeof(unsigned long long)), "fusion closed face alloc adds");
    ck(cudaMalloc(&d_rounds, sizeof(int)), "fusion closed face alloc rounds");
    ck(cudaMalloc(&d_error, sizeof(int)), "fusion closed face alloc error");
    ck(cudaMemcpy(d_values, input.data(), states * sizeof(std::uint32_t), cudaMemcpyHostToDevice),
       "fusion closed face copy values");
    ck(cudaMemset(d_blocks, 0, sizeof(unsigned long long)), "fusion closed face zero blocks");
    ck(cudaMemset(d_components, 0, sizeof(unsigned long long)), "fusion closed face zero components");
    ck(cudaMemset(d_deep, 0, sizeof(unsigned long long)), "fusion closed face zero deep");
    ck(cudaMemset(d_loads, 0, sizeof(unsigned long long)), "fusion closed face zero loads");
    ck(cudaMemset(d_stores, 0, sizeof(unsigned long long)), "fusion closed face zero stores");
    ck(cudaMemset(d_adds, 0, sizeof(unsigned long long)), "fusion closed face zero adds");
    ck(cudaMemset(d_rounds, 0, sizeof(int)), "fusion closed face zero rounds");
    ck(cudaMemset(d_error, 0, sizeof(int)), "fusion closed face zero error");
    ...
    for (int o = 0; o <= outer_bits; ++o) {
        ...
        ck(cudaFuncSetAttribute(
               two_cell_fusion2_closed_face_kernel,
               cudaFuncAttributeMaxDynamicSharedMemorySize,
               static_cast<int>(dynamic_bytes)),
           "fusion closed face optin shared");
        const unsigned grid = static_cast<unsigned>(std::min<Rank>(support_count, 65535));
        two_cell_fusion2_closed_face_kernel<<<grid, FUSION_THREADS, dynamic_bytes>>>(
            d_values, W, start, o, support_count, states, mod,
            d_blocks, d_components, d_deep, d_loads, d_stores, d_adds,
            d_rounds, d_error);
        ck(cudaGetLastError(), "fusion closed face launch");
    }
    ck(cudaDeviceSynchronize(), "fusion closed face sync");
    ...
    if (plan.fallback_states == 0 && output != reference) {
        std::cerr << "FAIL fusion closed face arithmetic W=" << W
                  << " start=" << start << '\n';
        std::exit(371);
    }

    std::cout << "two-cell-fusion2-closed-face"
              << " W=" << W
              << " start=" << start
...
int main(int argc, char** argv) {
    ...
    if (plan_only) {
        print_fusion2_closed_face_plan(W, requested_bytes, rt);
        if (W == 28) {
            for (Rank kib : {64ULL, 96ULL, 128ULL, 160ULL, 192ULL, 228ULL, 256ULL})
                print_fusion2_closed_face_plan(W, kib * 1024ULL, rt);
        }
        return 0;
    }
    if (W > 10) {
    ...
    int visible = 0;
    ck(cudaGetDeviceCount(&visible), "fusion closed face device count");
    if (visible < 1) return 4;
    ck(cudaSetDevice(0), "fusion closed face set device");
    install_tables(rt);
    install_stationary_tables(st);

    for (int start = 0; start + 2 <= W - 3; ++start)
        run_fusion2_closed_face(W, start, requested_bytes, rt, st, mod);
    std::cout << "ALL_OK two_cell_fusion2_closed_face_cuda=1 W=" << W << '\n';
    return 0;
}
