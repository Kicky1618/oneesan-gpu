...
static_assert(P10DC_RANKCHUNK32_ALIGN32 == 1,
              "rankchunk32 basepair64 currently requires aligned 32-code heights");
...
__device__ __forceinline__ void p10dc_low_rankchunk32_basepair64_row_warpstripe(
    ...
    if (lane == 0u) local_pair = D_P10DC_LOW_RANKCHUNKBLOCK16[first_compact >> 6];
    const uint32_t pair = __shfl_sync(active, local_pair, 0);
    const uint32_t block_base = p10dc_rankchunk32_basepair64_decode(pair, first_compact);
    row = D_P10DC_LOW_RANKSTREAM + block_base + prefix;
}
