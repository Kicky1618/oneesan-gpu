...
using ChooseTable =
    std::array<std::array<rp::Rank64, rp::RP_MAX_W + 1>, rp::RP_MAX_W + 1>;
...
__device__ __forceinline__ int choose_sym_row_base(int n) {
    const int m = n >> 1;
...
int main(int argc, char** argv) {
    ...
    if (blocks <= 0 || threads <= 0 || threads > 1024 ||
        iterations <= 0 || warmup < 0 || pattern < 0 || pattern > 1) {
        std::cerr << "usage: probe [blocks>0] [threads=1..1024] "
                     "[iterations>0] [warmup>=0] [pattern=0|1]\n";
        return 2;
    }

    const ChooseTable choose = make_choose_table();
    cuda_check(cudaMemcpyToSymbol(
        rp::RP_CHOOSE, choose.data(), sizeof(choose)),
        "cudaMemcpyToSymbol(RP_CHOOSE)");
    ...
    std::cout << "gridfp-choose-sym-u32-table-microprobe"
              << " mode=" << RP_CHOOSE_TABLE_MODE
              << " pattern=" << pattern
              << " n_min=0 n_max=28"
              << " blocks=" << blocks
              << " threads=" << threads
