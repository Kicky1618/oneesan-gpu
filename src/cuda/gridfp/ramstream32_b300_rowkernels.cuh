...
// One CUDA block owns one factor-row.  This moves FBlock lookup, row division,
// and (for HIGH-window transitions) descriptor/orbit fetch out of the per-state
// inner loop.  Prefixes are replaced for each group together with D_F_*BLOCKS.
__constant__ uint32_t D_CF_MAIN_ROW_OFF[65];
__constant__ uint32_t D_CF_BLOCK_ROW_OFF[33];
__constant__ uint32_t D_CF_MAIN_ROWS;
__constant__ uint32_t D_CF_BLOCK_ROWS;

static inline void install_compact_row_prefixes(
    ...
    for (size_t i = 0; i < mb.size(); ++i) {
        ...
        mr += rows;
        if (mr > 0xffffffffULL) std::exit(290);
    }
    ...
    for (size_t i = 0; i < db.size(); ++i) {
        ...
        br += rows;
        if (br > 0xffffffffULL) std::exit(291);
    }
    bo[db.size()] = uint32_t(br);
    uint32_t mru = uint32_t(mr), bru = uint32_t(br);
    ck(cudaMemcpyToSymbol(D_CF_MAIN_ROW_OFF, mo.data(), sizeof(mo)), "row main prefix");
    ck(cudaMemcpyToSymbol(D_CF_BLOCK_ROW_OFF, bo.data(), sizeof(bo)), "row block prefix");
    ck(cudaMemcpyToSymbol(D_CF_MAIN_ROWS, &mru, sizeof(mru)), "row main total");
    ck(cudaMemcpyToSymbol(D_CF_BLOCK_ROWS, &bru, sizeof(bru)), "row block total");
}

__device__ __forceinline__ int compact_find_row_block(
...
__global__ void compact_row_gather_main_kernel(Count* out) {
    uint32_t rr = blockIdx.x;
    if (rr >= D_CF_MAIN_ROWS) return;
    int bid = compact_find_row_block(rr, D_CF_MAIN_ROW_OFF, D_F_MAIN_NBLOCKS);
    FBlock x = D_F_MAIN_BLOCKS[bid];
    uint32_t hr = rr - D_CF_MAIN_ROW_OFF[bid];
    Code gbase = compact_main_canonical_row_base(x, hr);
    Code lbase = x.off + Code(hr) * x.stride;
    for (uint32_t lr = threadIdx.x; lr < x.stride; lr += blockDim.x) {
        uint32_t lar = compact_low_all_rank(x, lr);
        out[lbase + lr] = global_load_main(gbase + lar);
    }
}

__global__ void compact_row_gather_block_kernel(Count* out) {
    uint32_t rr = blockIdx.x;
    if (rr >= D_CF_BLOCK_ROWS) return;
    int bid = compact_find_row_block(rr, D_CF_BLOCK_ROW_OFF, D_F_BLOCK_NBLOCKS);
    FBlock x = D_F_BLOCK_BLOCKS[bid];
    uint32_t hr = rr - D_CF_BLOCK_ROW_OFF[bid];
    Code gbase = compact_block_canonical_row_base(x, hr);
    Code lbase = x.off + Code(hr) * x.stride;
    for (uint32_t lr = threadIdx.x; lr < x.stride; lr += blockDim.x) {
        uint32_t lar = compact_low_all_rank(x, lr);
...
__global__ void compact_row_scatter_main_kernel(const Count* in) {
    uint32_t rr = blockIdx.x;
    if (rr >= D_CF_MAIN_ROWS) return;
    int bid = compact_find_row_block(rr, D_CF_MAIN_ROW_OFF, D_F_MAIN_NBLOCKS);
    FBlock x = D_F_MAIN_BLOCKS[bid];
...
__global__ void compact_row_scatter_block_kernel(const Count* in) {
    uint32_t rr = blockIdx.x;
    if (rr >= D_CF_BLOCK_ROWS) return;
    int bid = compact_find_row_block(rr, D_CF_BLOCK_ROW_OFF, D_F_BLOCK_NBLOCKS);
    FBlock x = D_F_BLOCK_BLOCKS[bid];
...
// HIGH window: the orbit record and descriptor depend on HIGH row only.  One
// row block loads each once, then applies it to every LOW-mask-local column.
__global__ void compact_row_high_orbit_kernel(Count* mainv, Count* blockv, int p) {
    uint32_t rr = blockIdx.x;
    if (rr >= D_CF_MAIN_ROWS) return;
    int bid = compact_find_row_block(rr, D_CF_MAIN_ROW_OFF, D_F_MAIN_NBLOCKS);
    FBlock x = D_F_MAIN_BLOCKS[bid];
...
__global__ void compact_row_high_closure_kernel(Count* mainv, Count* blockv, int p) {
    constexpr int S = MAXW + 2;
    uint32_t rr = blockIdx.x;
    if (rr >= D_CF_MAIN_ROWS) return;
    int bid = compact_find_row_block(rr, D_CF_MAIN_ROW_OFF, D_F_MAIN_NBLOCKS);
    ...
    FBlock y = D_F_BLOCK_BLOCKS[dbid];
    Code ib = x.off + Code(hr) * x.stride;
...
// LOW window: orbit/descriptor depend on LOW all-rank, so threads still fetch
// one record each; FBlock lookup and row division remain amortized per block.
__global__ void compact_row_low_orbit_kernel(Count* mainv, Count* blockv, int p) {
    uint32_t rr = blockIdx.x;
    if (rr >= D_CF_MAIN_ROWS) return;
    int bid = compact_find_row_block(rr, D_CF_MAIN_ROW_OFF, D_F_MAIN_NBLOCKS);
    FBlock x = D_F_MAIN_BLOCKS[bid];
    ...
    for (uint32_t lr = threadIdx.x; lr < x.stride; lr += blockDim.x) {
        ...
        Code d = dy.off + Code(hr) * dy.stride + low_orbit_dlr_dev(ow);
...
__global__ void compact_row_low_closure_kernel(Count* mainv, Count* blockv, int p) {
    constexpr int S = MAXW + 2;
    uint32_t rr = blockIdx.x;
    if (rr >= D_CF_MAIN_ROWS) return;
    int bid = compact_find_row_block(rr, D_CF_MAIN_ROW_OFF, D_F_MAIN_NBLOCKS);
    ...
    uint32_t pi = uint32_t(LOW_LUT_K - p);
    Code ib = x.off + Code(hr) * x.stride;
    uint32_t ha = D_F_HIGH_MASK_OFF[size_t(D_F_MASK) * S + x.he];
    uint32_t hc = D_F_HIGH_MASK_CODES[ha + hr];
    for (uint32_t lr = threadIdx.x; lr < x.stride; lr += blockDim.x) {
        uint64_t ow = D_LOW_ORBIT[size_t(pi) * D_LOW_ORBIT_MAIN_TOTAL
                                 + D_LOW_ORBIT_MAIN_BASE[bid] + lr];
