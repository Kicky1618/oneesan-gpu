...
#undef main
#define main two_cell_left_boundary_cluster_sliced_microprobe_main_unused
#include "two_cell_left_boundary_cluster_sliced_microprobe.cu"
...
struct LeftBoundaryClusterChoice {
    int cluster = 0;
    Rank local_states = 0;
...
cudaLaunchConfig_t make_left_boundary_cluster_config(
    int cluster,
    Rank dynamic_bytes,
    unsigned clusters
) {
...
LeftBoundaryClusterChoice forced_left_boundary_choice(
    int cluster,
    int outer_ones,
    Rank support_count,
    ...
    LeftBoundaryClusterChoice out{};
    const Rank local_states = left_boundary_slice_max_owner_states(
    ...
    ck(cudaFuncSetAttribute(
           two_cell_left_boundary_cluster_sliced_kernel,
           cudaFuncAttributeMaxDynamicSharedMemorySize,
           static_cast<int>(dynamic_bytes)),
       "forced left boundary DSM optin shared");

    const unsigned clusters = static_cast<unsigned>(
        std::max<Rank>(1, std::min<Rank>(support_count, 256)));
    cudaLaunchConfig_t config = make_left_boundary_cluster_config(
    ...
    ck(cudaOccupancyMaxPotentialClusterSize(
           ...
           reinterpret_cast<const void*>(
               two_cell_left_boundary_cluster_sliced_kernel),
           &config),
       "forced left boundary DSM max potential");
    ...
    ck(cudaOccupancyMaxActiveClusters(
           ...
           reinterpret_cast<const void*>(
               two_cell_left_boundary_cluster_sliced_kernel),
           &config),
       "forced left boundary DSM active clusters");
...
void launch_forced_left_boundary_bucket(
    ...
    std::uint32_t mod,
    int* d_error,
    const LeftBoundaryClusterChoice& choice
) {
    const unsigned clusters = static_cast<unsigned>(
    ...
    ck(cudaGetLastError(), "forced left boundary DSM launch");
}
...
int main(int argc, char** argv) {
    ...
    const LeftBoundaryPrimitiveLut host_lut =
        build_left_boundary_primitive_lut(W, rt);

    if (plan_only) {
        std::cout << "forced-left-boundary-DSM-plan"
                  << " W=" << W
    ...
    int visible = 0;
    ck(cudaGetDeviceCount(&visible), "forced left boundary DSM device count");
    if (visible < 1) return 3;
    ck(cudaSetDevice(0), "forced left boundary DSM set device");
    cudaDeviceProp prop{};
    ck(cudaGetDeviceProperties(&prop, 0), "forced left boundary DSM props");
    if (prop.major < 9) {
        std::cerr << "forced left boundary DSM requires compute capability >= 9.0\n";
        return 4;
    }
    install_tables(rt);
    ...
    ck(cudaFuncGetAttributes(
           &attr, two_cell_left_boundary_cluster_sliced_kernel),
       "forced left boundary DSM function attrs");
    ...
    for (Rank r = 0; r < states; ++r)
        input[static_cast<std::size_t>(r)] = static_cast<std::uint32_t>(
            1 + ((r * 2654435761ULL + 769ULL) % (mod - 1ULL)));
    ...
    ck(cudaMalloc(&d_values, states * sizeof(std::uint32_t)),
       "forced left boundary DSM alloc values");
    ck(cudaMalloc(&d_error, sizeof(int)),
       "forced left boundary DSM alloc error");
    ck(cudaMemcpy(d_values, input.data(), states * sizeof(std::uint32_t),
                  cudaMemcpyHostToDevice),
       "forced left boundary DSM copy input");
    ck(cudaMemset(d_error, 0, sizeof(int)),
       "forced left boundary DSM zero error");

    const int outer_bits = W - 4;
    for (int o = 0; o <= outer_bits; ++o) {
        const Rank support_count = rt.choose[outer_bits][o];
        const auto choice = forced_left_boundary_choice(
            forced_cluster, o, support_count,
            per_block_limit, static_shared, rt);
        if (!choice.ok) {
            std::cerr << "SKIP forced left boundary DSM unsupported bucket"
                      << " W=" << W
                      << " outer=" << o
                      << " cluster=" << forced_cluster << '\n';
            return 5;
        }
        launch_forced_left_boundary_bucket(
            d_values, d_left, d_reflection, d_offset,
            W, o, support_count, states, mod, d_error, choice);
    }
    ck(cudaDeviceSynchronize(), "forced left boundary DSM sync");
    ...
    ck(cudaMemcpy(output.data(), d_values, states * sizeof(std::uint32_t),
       ...
       "forced left boundary DSM copy output");
    ck(cudaMemcpy(&error, d_error, sizeof(error),
                  cudaMemcpyDeviceToHost),
       "forced left boundary DSM copy error");
    if (error || output != reference) {
        std::cerr << "FAIL forced left boundary DSM arithmetic"
                  << " W=" << W
                  << " cluster=" << forced_cluster
                  << " error=" << error << '\n';
        return 6;
    ...
    std::cout << "forced-left-boundary-DSM arithmetic=OK"
              << " W=" << W
              << " cluster=" << forced_cluster
