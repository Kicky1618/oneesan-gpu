...
#undef main
#define main two_cell_left_boundary_cluster_forced_microprobe_main_unused
#include "two_cell_left_boundary_cluster_forced_microprobe.cu"
#pragma pop_macro("main")

namespace {

LeftBoundaryClusterChoice choose_left_boundary_runtime_bucket(
    int outer_ones,
    Rank support_count,
    ...
    for (int cluster : {1, 2, 4, 8}) {
        if (cluster > requested_max_cluster ||
            cluster > LEFT_BOUNDARY_CLUSTER_MAX) break;
        ...
        ck(cudaFuncSetAttribute(
           ...
           "left boundary runtime optin shared");
        ...
        ck(cudaOccupancyMaxPotentialClusterSize(
               ...
               reinterpret_cast<const void*>(
                   two_cell_left_boundary_cluster_sliced_kernel),
               &config),
           "left boundary runtime max potential");
        ...
        ck(cudaOccupancyMaxActiveClusters(
               ...
               reinterpret_cast<const void*>(
                   two_cell_left_boundary_cluster_sliced_kernel),
               &config),
           "left boundary runtime active clusters");
...
int main(int argc, char** argv) {
    ...
    if (plan_only) {
        ...
        for (int o = 0; o <= outer_bits; ++o) {
            ...
            for (int c : {1, 2, 4, 8}) {
                ...
                if (left_boundary_slice_max_owner_states(o, c, rt) *
                            sizeof(std::uint32_t) +
        ...
        std::cout << "left-boundary-runtime-plan"
                  << " W=" << W
                  << " max_cluster=" << requested_max_cluster
                  << " fused_state_fraction="
                  << (total ? double(fused) / double(total) : 0.0)
                  << " capacity_same_as_right=1\n";
        return 0;
    }
    if (W > 10) return 3;

    int visible = 0;
    ck(cudaGetDeviceCount(&visible), "left boundary runtime device count");
    if (visible < 1) return 4;
    ck(cudaSetDevice(0), "left boundary runtime set device");
    cudaDeviceProp prop{};
    ck(cudaGetDeviceProperties(&prop, 0), "left boundary runtime props");
    if (prop.major < 9) {
        std::cerr << "left boundary runtime DSM requires compute capability >= 9.0\n";
        return 5;
    }
    install_tables(rt);
    ...
    ck(cudaFuncGetAttributes(
       ...
       "left boundary runtime function attrs");
    ...
    for (Rank r = 0; r < states; ++r)
        input[static_cast<std::size_t>(r)] = static_cast<std::uint32_t>(
            1 + ((r * 2654435761ULL + 839ULL) % (mod - 1ULL)));
    ...
    ck(cudaMalloc(&d_values, states * sizeof(std::uint32_t)),
       "left boundary runtime alloc values");
    ck(cudaMalloc(&d_error, sizeof(int)),
       "left boundary runtime alloc error");
    ck(cudaMemcpy(d_values, input.data(), states * sizeof(std::uint32_t),
                  cudaMemcpyHostToDevice),
       "left boundary runtime copy input");
    ck(cudaMemset(d_error, 0, sizeof(int)),
       "left boundary runtime zero error");
    ...
    for (int o = 0; o <= outer_bits; ++o) {
        ...
        std::cout << "left-boundary-runtime-bucket"
                  << " outer=" << o
    ...
    if (fallback) {
        std::cerr << "left boundary runtime small-width correctness requires all buckets fused;"
                  << " fallback_states=" << fallback << '\n';
        return 6;
    }
    ck(cudaDeviceSynchronize(), "left boundary runtime sync");
    ...
    ck(cudaMemcpy(output.data(), d_values, states * sizeof(std::uint32_t),
       ...
       "left boundary runtime copy output");
    ck(cudaMemcpy(&error, d_error, sizeof(error),
                  cudaMemcpyDeviceToHost),
       "left boundary runtime copy error");
    if (error || output != reference) {
        std::cerr << "FAIL left boundary runtime arithmetic"
                  << " W=" << W
                  << " error=" << error << '\n';
        return 7;
    }
    ...
    cudaFree(d_offset);
    std::cout << "left-boundary-runtime arithmetic=OK W=" << W << '\n';
    return 0;
}
