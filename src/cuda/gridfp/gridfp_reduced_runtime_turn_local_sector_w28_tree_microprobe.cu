...
__global__ void turn_local_sector_probe_kernel(
    std::uint64_t* output,
    int iterations
) {
    ...
    for (int i = 0; i < iterations; ++i) {
        const int row = 445 + ((outer_ones + 1) >> 1) * 7 +
                        (outer_ones >> 1) * 8;
        const int count = (outer_ones & 1) ? 8 : 7;
        const std::uint32_t group =
            rp::RP_RUNTIME_TURN_LOCAL_SECTOR_END[row + count - 1];
        seed = seed * 1664525u + 1013904223u;
        const rp::Rank64 within = (std::uint64_t(seed) * group) >> 32;

        int local_ones = -1;
        rp::Rank64 local_within = 0;
        const bool ok = rp::runtime_turn_local_sector_device(
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < warmup; ++i) {
        turn_local_sector_probe_kernel<<<blocks, threads>>>(d_output, iterations);
    ...
    std::cout << "gridfp-runtime-turn-local-sector-w28-tree-microprobe"
              << " w28_tree=" << RP_RUNTIME_TURN_LOCAL_SECTOR_W28_TREE
