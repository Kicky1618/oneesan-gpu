...
#undef main
#define main two_cell_fusion2_register_face_microprobe_main_unused
#include "two_cell_fusion2_register_face_microprobe.cu"
#pragma pop_macro("main")

namespace {

__global__ void two_cell_fusion2_register_reuse_kernel(
...
Rank fusion2_register_reuse_workspace_estimate() {
    return Rank(FUSION_WARPS) * (
        Rank(FUSION_MAX_COMPONENT) * sizeof(PackedKey) +
        sizeof(PackedWord) + sizeof(Rank) + 3 * sizeof(int)) + 256ULL;
}

void print_fusion2_register_reuse_plan(
    int W,
    Rank shared_limit,
    const RankTables& rt
) {
    const Rank reserve = fusion2_register_reuse_workspace_estimate();
    ...
    std::cout << "fusion2_register_reuse_plan"
              << " W=" << W
...
int main(int argc, char** argv) {
    ...
    if (!plan_only) {
        std::cerr << "execution harness intentionally deferred; use --plan-only until the A/B kernel is compiled on CUDA hardware\n";
        return 3;
    }
    print_fusion2_register_reuse_plan(W, shared_kib * 1024ULL, rt);
    if (W == 28) {
        for (Rank kib : {64ULL, 96ULL, 128ULL, 160ULL, 192ULL, 228ULL, 256ULL})
            print_fusion2_register_reuse_plan(W, kib * 1024ULL, rt);
    }
    return 0;
}
