...
// A second lane-0 collision mask records buckets that received two or more
// distinct inserted keys; candidate mismatch in a non-collided bucket is then
// an exact miss and needs no linear fallback.
...
__device__ __forceinline__ void runtime_find_index_record(
    ...
    const int bucket = runtime_find_index_bucket(k);
    const std::uint64_t bit = 1ULL << bucket;
    if (occupancy & bit) collisions |= bit;
...
__device__ __forceinline__ bool runtime_discover_inverse_direction_to_shared_indexed(
    ...
    std::uint64_t& source_collisions,
    RuntimeFindIndexCache& source_cache
) {
    RuntimeIndexedSharedKeySetSink base{
        source_set, &source_count, capacity,
        &source_occupancy, &source_collisions, &source_cache};
    if (!reverse)
