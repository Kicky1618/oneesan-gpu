...
#undef main
#define main two_cell_left_runtime_main_unused_for_stage
#include "two_cell_left_boundary_cluster_runtime_microprobe.cu"
...
int run_left_stage(
    ...
    cudaDeviceProp prop{};
    ck(cudaGetDeviceProperties(&prop, 0), "snake left props");
    if (prop.major < 9) return 132;
    ...
    if (static_shared >= per_block_limit) return 133;
