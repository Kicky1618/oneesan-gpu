#define main gpu_direct_atomicfree_reference_main_unused
#include "oneesan_cuda_gridfp_gpu_direct_atomicfree.cu"
#undef main

#include "ramstream32_gpu_direct_atomicfree_multigpu.cuh"

static void gdm_enable_full_p2p(int ngpu) {
    int peers=0;
    for (int a=0;a<ngpu;++a) for (int b=0;b<ngpu;++b) if (a!=b) {
        int can=0; ck(cudaDeviceCanAccessPeer(&can,a,b),"gdm can peer");
        if (!can) continue;
        ck(cudaSetDevice(a),"gdm set p2p device");
        cudaError_t e=cudaDeviceEnablePeerAccess(b,0);
        if (e==cudaErrorPeerAccessAlreadyEnabled) cudaGetLastError(); else ck(e,"gdm enable peer");
        ++peers;
    }
    if (ngpu>1 && peers!=ngpu*(ngpu-1)) {
        std::cerr<<"gdm requires full P2P: "<<peers<<'/'<<ngpu*(ngpu-1)<<'\n'; std::exit(174);
    }
}
...
int main(int argc,char**argv) {
    ...
    int plan_ng=requested>0?requested:4;
    GdmShardHost plan_shard=build_gdm_shards(layout,plan_ng);
    double prepare_host_s=gdg_seconds(prep0);
    if (plan_only) {
        std::cout<<"backend=gridfp-gpu-direct-atomicfree-multigpu-v0.1-plan"
                 <<" n="<<n<<" gpus="<<plan_ng
                 ...
                 <<" transient_metadata_per_gpu_mib="<<double(transient_meta)/double(1ULL<<20)
                 <<" peak_per_gpu_gib="<<double(plan_shard.max_elems*sizeof(Count)+transient_meta)/double(1ULL<<30)
                 ...
                 <<" global_barriers_per_row="<<3*(LOW_LUT_K+HIGH_LUT_K)
                 <<" prepare_s="<<prepare_host_s<<'\n';
        return 0;
    }

    int visible=0; ck(cudaGetDeviceCount(&visible),"gdm device count");
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdm mem device"); ck(cudaMemGetInfo(&free_before[d],&total_before[d]),"gdm mem info");
        size_t need=size_t(shard.total_elems[d])*sizeof(Count)+transient_meta;
        if (need>free_before[d]) {
            std::cerr<<"gdm insufficient HBM device="<<d<<" need_gib="<<double(need)/double(1ULL<<30)
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdm alloc device");
        if (shard.main_elems[d]) ck(cudaMalloc(&main_ptr[d],size_t(shard.main_elems[d])*sizeof(Count)),"gdm alloc main");
        if (shard.block_elems[d]) ck(cudaMalloc(&block_ptr[d],size_t(shard.block_elems[d])*sizeof(Count)),"gdm alloc block");
        if (shard.main_elems[d]) ck(cudaMemset(main_ptr[d],0,size_t(shard.main_elems[d])*sizeof(Count)),"gdm zero main");
        if (shard.block_elems[d]) ck(cudaMemset(block_ptr[d],0,size_t(shard.block_elems[d])*sizeof(Count)),"gdm zero block");
    }
    ...
    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdm metadata device"); ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"gdm modulus");
    ...
    ck(cudaSetDevice(init_loc.first),"gdm init device");
    ck(cudaMemcpy(main_ptr[init_loc.first]+init_loc.second,&one,sizeof(one),cudaMemcpyHostToDevice),"gdm init");

    double high_s=0,low_s=0; auto wall0=std::chrono::steady_clock::now();
    for (int row=0;row<W;++row) {
        auto t=std::chrono::steady_clock::now(); gdm_run_high(ngpu,layout,threads,grid_x,grid_y); high_s+=gdg_seconds(t);
        t=std::chrono::steady_clock::now(); gdm_run_low(ngpu,layout,threads,grid_x,grid_y); low_s+=gdg_seconds(t);
        std::cerr<<"row "<<row+1<<'/'<<W<<" high_s="<<high_s<<" low_s="<<low_s<<'\n';
    }
    ...
    ck(cudaSetDevice(final_loc.first),"gdm answer device");
    ck(cudaMemcpy(&answer,main_ptr[final_loc.first]+final_loc.second,sizeof(answer),cudaMemcpyDeviceToHost),"gdm answer");

    std::cout<<"backend=gridfp-gpu-direct-atomicfree-multigpu-v0.1"
             ...
             <<" high_s="<<high_s<<" low_s="<<low_s<<" prepare_s="<<prepare_s<<" wall_s="<<wall_s
             <<" closure_atomic=0 scratch_bytes=0 shard=storage-block"
             <<" global_barriers_per_row="<<3*(LOW_LUT_K+HIGH_LUT_K)<<'\n';

    for (int d=0;d<ngpu;++d) {
        ck(cudaSetDevice(d),"gdm release device"); cross_dev[d].release(); ordinary_dev[d].release(); base[d].release();
