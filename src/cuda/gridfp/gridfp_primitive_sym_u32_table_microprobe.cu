...
#endif
static_assert(RP_PRIMITIVE_SYM_U32_MODE >= 0 && RP_PRIMITIVE_SYM_U32_MODE <= 2,
              "RP_PRIMITIVE_SYM_U32_MODE must be 0, 1, or 2");
...
__device__ __forceinline__ int row_base(int rem) {
    const int m = rem >> 1; return (rem & 1) ? (m + 1) * (m + 1) : m * (m + 1);
}
...
#elif RP_PRIMITIVE_SYM_U32_MODE == 1
    if (rem < 0 || rem > rp::RP_MAX_W || h < 0 || h > rem || ((rem ^ h) & 1)) return 0;
    return RP_PRIMITIVE_SYM_U32_PROBE[row_base(rem) + (h >> 1)];
#else
    return RP_PRIMITIVE_FULL_U32_PROBE[rem * 30 + h];
#endif
}
__global__ void probe_kernel(std::uint64_t* output, int iterations, int pattern) {
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        int rem = 0, h = 0;
        if (pattern == 0) {
            rem = int((seed >> 16) % 29u); h = int((seed >> 8) % 30u);
        } else if (pattern == 1) {
            const int sector = int((tid + std::uint32_t(i)) % 14u); rem = 1 + (sector << 1); h = 1;
        } else {
...
int main(int argc, char** argv) {
    ...
    if (blocks <= 0 || threads <= 0 || threads > 1024 || iterations <= 0 || pattern < 0 || pattern > 2 || warmup < 0) return 2;

    const PrimitiveTable primitive = make_primitive_table();
    std::array<std::uint32_t, 29 * 30> full{};
    for (int rem = 0; rem <= rp::RP_MAX_W; ++rem)
        for (int h = 0; h < 30; ++h) {
            if (primitive[rem][h] > 0xffffffffULL) return 3;
            full[std::size_t(rem * 30 + h)] = static_cast<std::uint32_t>(primitive[rem][h]);
        }
    cuda_check(cudaMemcpyToSymbol(rp::RP_PRIMITIVE, primitive.data(), sizeof(primitive)), "cudaMemcpyToSymbol(RP_PRIMITIVE)");
    cuda_check(cudaMemcpyToSymbol(RP_PRIMITIVE_FULL_U32_PROBE, full.data(), sizeof(full)), "cudaMemcpyToSymbol(RP_PRIMITIVE_FULL_U32_PROBE)");

    const std::size_t nthreads = std::size_t(blocks) * std::size_t(threads);
    std::uint64_t* d_output = nullptr; cuda_check(cudaMalloc(&d_output, nthreads * sizeof(std::uint64_t)), "cudaMalloc");
    ...
    std::cout << "gridfp-primitive-sym-u32-table-microprobe mode=" << RP_PRIMITIVE_SYM_U32_MODE
              << " pattern=" << pattern << " blocks=" << blocks << " threads=" << threads
              << " iterations=" << iterations << " calls=" << calls << " kernel_ms=" << elapsed_ms
              << " ns_per_call=" << (double(elapsed_ms) * 1.0e6 / double(calls)) << " checksum=" << checksum << '\n';
