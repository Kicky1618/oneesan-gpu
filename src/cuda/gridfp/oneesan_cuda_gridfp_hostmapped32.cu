...
struct HostMappedCounts {
    Count* h=nullptr;
    Count* d=nullptr;
    size_t bytes=0;
    void alloc(Code n,const char* what){
        bytes=size_t(n)*sizeof(Count);
        void* q=mmap(nullptr,bytes,PROT_READ|PROT_WRITE,MAP_PRIVATE|MAP_ANONYMOUS,-1,0);
        if(q==MAP_FAILED){perror("mmap hostmapped");std::exit(2);}
        h=(Count*)q;
        madvise(h,bytes,MADV_HUGEPAGE);
        ck(cudaHostRegister(h,bytes,cudaHostRegisterMapped),what);
        ck(cudaHostGetDevicePointer((void**)&d,h,0),"hostmapped device ptr");
    }
    void release(){
        if(h){cudaHostUnregister(h);munmap(h,bytes);h=nullptr;d=nullptr;}
    }
};
...
int main(int argc,char**argv){int n=argc>1?std::atoi(argv[1]):16;unsigned long long raw_mod=2147483647ULL;if(argc>2){char*end=nullptr;errno=0;raw_mod=std::strtoull(argv[2],&end,10);if(errno==ERANGE||end==argv[2]||*end!='\0'||argv[2][0]=='-'){std::cerr<<"invalid modulus: "<<argv[2]<<"\n";return 1;}}if(raw_mod<2||raw_mod>0xffffffffULL){std::cerr<<"32-bit backend modulus must be in [2, 4294967295], got "<<raw_mod<<"\n";return 1;}Count mod=(Count)raw_mod;int target_mib=argc>3?std::atoi(argv[3]):16384;int max_window=argc>4?std::atoi(argv[4]):14;int requested=argc>5?std::atoi(argv[5]):0;int W=n+1;if(n<2||W>MAXW){std::cerr<<"n=2..27\n";return 1;}if(W!=TARGET_W){std::cerr<<"specialized for width "<<TARGET_W<<" (n="<<(TARGET_W-1)<<")\n";return 1;}build_full_dp();ck(cudaSetDeviceFlags(cudaDeviceMapHost),"map host flags");int visible=0;ck(cudaGetDeviceCount(&visible),"count");int ng=requested<=0?visible:std::min(requested,visible);if(ng<1||ng>MAXGPU){std::cerr<<"need 1..8 GPUs\n";return 2;}
    ...
    if(ng!=1){std::cerr<<"hostmapped prototype supports one GPU\n";return 3;}
    Code mainN=H_DP[W][1],blockN=H_DP[W-1][1];Code mc=mainN,bc=blockN;Count*mp[MAXGPU]{},*bp[MAXGPU]{};
    cudaSetDevice(0);HostMappedCounts hm,hb;hm.alloc(mainN,"register auth main");hb.alloc(blockN,"register auth block");mp[0]=hm.d;bp[0]=hb.d;
    std::vector<DeviceCtx>ctx(ng);for(int d=0;d<ng;++d)ctx[d].init(d,mod,mp,bp,mc,bc,ng);
    MateID init=MateID(R)<<(2*(W-1));Code ig=rank_full(init,W);hm.h[ig]=1;
    ...
    double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();Code fg=rank_full(MateID(R),W);Count ans=hm.h[fg];double mx=0,sum=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;std::cerr<<"gpu "<<c.dev<<" groups="<<c.groups<<" active_s="<<c.active<<"\n";}std::cout<<"backend=gridfp-hostmapped32 n="<<n<<" residue="<<ans<<" modulus="<<mod<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" auth_gib="<<double(mainN+blockN)*sizeof(Count)/(1ull<<30)<<" auth_per_gpu_gib="<<double(mainN+blockN)*sizeof(Count)/ng/(1ull<<30)<<" scratch_target_mib="<<target_mib<<" windows="<<total_windows<<" max_groups="<<maxgroups<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" wall_s="<<wall<<"\n";
    for(auto&c:ctx)c.destroy();hm.release();hb.release();
}
