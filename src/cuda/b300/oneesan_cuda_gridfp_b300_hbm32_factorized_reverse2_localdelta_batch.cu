...
// Rank a main predecessor obtained by changing only the pair (p,p-1),
// using the factorized local layout directly.  All simple main->main reverse
// transitions preserve the segment end height.  Away from the center split,
// only one segment rank changes, so no FBlock search / seg_end_height is needed.
__device__ __forceinline__ Code factor_rank_main_pair_from(Code ti,MateID pred,int p){
    ...
    uint32_t lr=x.stride?uint32_t(r-Code(r/x.stride)*x.stride):0;
    if(p<L){
        constexpr uint32_t LM=(1u<<(2*L))-1u;
        uint32_t lc=uint32_t(pred)&LM, lp=D_F_LOW_PACKED_RANK[lc];
        uint32_t nlr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);
        return ti-Code(lr)+Code(nlr);
    }
    if(p>=L+2){
        constexpr uint32_t HM=(1u<<(2*H))-1u;
        uint32_t hc=uint32_t((pred>>(2*(L+1)))&HM),hp=D_F_HIGH_PACKED_RANK[hc];
        uint32_t nhr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));
...
__device__ __forceinline__ Count reverse_eval_main_step(const Count*in,const Count*din,MateID t,int p){
    return reverse_eval_main_step_known(in,din,t,factor_rank_main(t),p);
}
