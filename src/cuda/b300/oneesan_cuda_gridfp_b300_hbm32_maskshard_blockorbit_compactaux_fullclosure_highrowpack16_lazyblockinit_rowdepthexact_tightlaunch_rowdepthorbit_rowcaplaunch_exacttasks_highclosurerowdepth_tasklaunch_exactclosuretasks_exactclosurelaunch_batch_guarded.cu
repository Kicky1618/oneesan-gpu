// v0.23 experiment: v0.22 plus exact row-depth HIGH closure host grid sizing.
// Kernel task mapping is unchanged; only launched CTAs follow the exact active
// warp-task count for the current LOW mask, HIGH position and DP row cap.
...
#define main oneesan_maskshard_v23_highclosureexactlaunch_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.23 exact HIGH closure launch requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.23-highrowpack16-rowdepthexact-exacttasks-highclosureexacttasks-exactlaunch-batch"
    ...
    return oneesan_maskshard_v23_highclosureexactlaunch_inner_main(argc, argv);
}
