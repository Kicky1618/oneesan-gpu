// v0.42 experiment: v0.41 prepacked closure prefixes plus row-resolved
// warp-uniform closure metadata in the same LOW group constant object.
...
#define main oneesan_maskshard_v42_lowclosurepackedmeta_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) return 1;
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.42-lowclosure-packedmeta-batch"
        << " low_closure_packed_meta=1 low_closure_packed_prefix=1"
        << " low_closure_task_u32=1 low_group_sync=1 guarded_hbm=1\n";
    const int rc = oneesan_maskshard_v42_lowclosurepackedmeta_inner_main(argc, argv);
