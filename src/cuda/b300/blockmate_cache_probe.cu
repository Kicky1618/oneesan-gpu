...
__global__ void gather_block_kernel(Count*out,MateID*mates,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){MateID m=factor_unrank_block(i);Code g=factor_global_rank_block(m);out[i]=global_load_block(g);if(mates)mates[i]=m;}}
...
__global__ void scatter_block_kernel(const Count*in,const MateID*mates,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){MateID m=mates?mates[i]:factor_unrank_block(i);Code g=factor_global_rank_block(m);global_store_block(g,in[i]);}}
...
__global__ void blocked_group_kernel(const Count*in,const MateID*mates,Code n,Count*out_main,int p){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID sm=mates?mates[i]:factor_unrank_block(i);MateID t=oneesan::gridfp::blocked_exclude(sm,p);Code j=factor_rank_main(t);atomic_add_mod(out_main+j,c);}}
...
__global__ void reverse_block_group_kernel(const Count*in,const MateID*mates,Code n,Count*out,Code dn,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<dn;i+=stride){
        MateID b=mates?mates[i]:factor_unrank_block(i); Count acc=0; auto low=oneesan::gridfp::mget(b,p-1);
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    size_t countBytes=size_t(2*ms.size+2*ds.size)*sizeof(Count),mateBytes=size_t(ms.size)*sizeof(MateID),blockMateBytes=size_t(ds.size)*sizeof(MateID);bool useMate=!pg.use_mi&&(countBytes+mateBytes<=target);bool useBlockMate=!pg.use_di&&(countBytes+(useMate?mateBytes:0)+blockMateBytes<=target);if(!useBlockMate&&!pg.use_di&&countBytes+blockMateBytes<=target){useMate=false;useBlockMate=true;}if(const char*e=std::getenv("GRIDFP_BLOCK_MATE_CACHE"))if(std::atoi(e)==0)useBlockMate=false;
    ...
    if(ds.size){if(pg.use_di)interval_io_kernel<true,false><<<interval_blocks(pg.di.size(),threads),threads>>>(c.dD,c.dID,pg.di.size());else gather_block_kernel<<<bd,threads>>>(c.dD,useBlockMate?c.dBlockMate:nullptr,ds.size);}
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;){
        if(p>2 && p-1>=wp.p_lo){
            ...
            if(ds.size)reverse2_block_group_kernel<<<bd,threads,0,c.sBlock>>>(cur,dcur,useBlockMate?c.dBlockMate:nullptr,dnext,ds.size,p);
            p-=2;
        }else{
            if(p>1){
                if(ms.size)reverse_main_group_kernel<<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,ds.size,nxt,p);
                if(ds.size)reverse_block_group_kernel<<<bd,threads,0,c.sBlock>>>(cur,useBlockMate?c.dBlockMate:nullptr,ms.size,dnext,ds.size,p);
            }else{
                ...
                if(ds.size)blocked_group_kernel<<<bd,threads,0,c.sBlock>>>(dcur,useBlockMate?c.dBlockMate:nullptr,ds.size,nxt,p);
            }
            --p;
        }
    ...
    if(ds.size){if(pg.use_di)interval_io_kernel<true,true><<<interval_blocks(pg.di.size(),threads),threads>>>(dcur,c.dID,pg.di.size());else scatter_block_kernel<<<bd,threads>>>(dcur,useBlockMate?c.dBlockMate:nullptr,ds.size);}
    ck(cudaGetLastError(),"doubleD scatter");ck(cudaDeviceSynchronize(),"group sync");auto tScatter=std::chrono::steady_clock::now();c.groups++;c.groupsMainMate+=useMate;c.groupsBlockMate+=useBlockMate;c.gather_s+=std::chrono::duration<double>(tGather-t0).count();c.transition_s+=std::chrono::duration<double>(tTransition-tGather).count();c.scatter_s+=std::chrono::duration<double>(tScatter-tTransition).count();c.active+=std::chrono::duration<double>(tScatter-t0).count();
}
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        std::cerr<<"PLAN n="<<n<<" W="<<W<<" gpus="<<png<<" main="<<pmain<<" blocked="<<pblock<<" auth_gib="<<double(pmain+pblock)*4.0/(1ull<<30)<<" auth_per_gpu_gib="<<double(pmain+pblock)*4.0/png/(1ull<<30)<<" row6_states="<<r6n<<" pref_mib="<<(pvals*4>>20)<<" suff_mib="<<(svals*4>>20)<<" task_chunk="<<tch<<"\n";
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"set residue reset");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"set modulus");if(ml[d])ck(cudaMemset(mp[d],0,size_t(ml[d])*sizeof(Count)),"zero main");if(bl[d])ck(cudaMemset(bp[d],0,size_t(bl[d])*sizeof(Count)),"zero block");ck(cudaDeviceSynchronize(),"zero sync");ctx[d].active=0;ctx[d].gather_s=ctx[d].transition_s=ctx[d].scatter_s=0;ctx[d].groups=ctx[d].groupsMainMate=ctx[d].groupsBlockMate=0;}
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;uint64_t gmm=0,gbm=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;gmm+=c.groupsMainMate;gbm+=c.groupsBlockMate;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" main_mate_groups="<<gmm<<" block_mate_groups="<<gbm<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }
