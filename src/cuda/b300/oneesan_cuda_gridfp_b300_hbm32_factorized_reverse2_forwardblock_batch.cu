...
// Build only the blocked output of two consecutive updates by following the
// few forward branches from each source. Main remains target-gather/reverse.
...
__global__ void forward2_block_from_main_kernel(const Count*in,const MateID*mates,Code n,Count*out,int p){
    ...
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=mates?mates[i]:factor_unrank_main(i);
        // First-step identity branch remains main.
        forward2_try_block(out,m,q,c);
        // First-step include branch contributes only if it is still main.
        auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);
        if(z.valid&&!z.blocked)forward2_try_block(out,z.mate,q,c);
    }
}
__global__ void forward2_block_from_block_kernel(const Count*din,const MateID*mates,Code dn,Count*out,int p){
    int q=p-1; Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<dn;i+=stride){Count c=din[i];if(!c)continue;MateID b=mates?mates[i]:factor_unrank_block(i);
        MateID m=oneesan::gridfp::blocked_exclude(b,p);forward2_try_block(out,m,q,c);
    }
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;){
        if(p>2 && p-1>=wp.p_lo){
            ...
            if(ds.size)forward2_block_from_block_kernel<<<bd,threads,0,c.sBlock>>>(dcur,useBlockMate?c.dBlockMate:nullptr,ds.size,dnext,p);
            p-=2;
        }else{
            if(p>1){
