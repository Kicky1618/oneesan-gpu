...
#include "maskshard_low_maskbatch_range_config.cuh"

#ifndef MASKSHARD_LOW_MASKBATCH_REBUILD_DYNAMIC
#error "LOW mask-batch range kernels require rebuild-dynamic mode"
#endif

struct MaskShardLowBatchRangeJob {
    std::uint16_t mask = 0;
...
__device__ __forceinline__ MaskShardLowBatchRangeJob
maskshard_low_batch_decode_range_job(
    const MaskShardLowBatchRangeDeviceDesc* groups,
    int group_count,
    std::uint32_t cta
) {
    int lo = 0, hi = group_count;
    while (lo < hi) {
        const int mid = (lo + hi) >> 1;
        if (groups[mid].cta_end <= cta) lo = mid + 1;
        else hi = mid;
    }
    const int g = lo;
    const std::uint32_t begin = g ? groups[g - 1].cta_end : 0u;
    const std::uint32_t end = groups[g].cta_end;
    return {groups[g].mask, groups[g].local, cta - begin, end - begin};
}

struct MaskShardLowBatchRangeOrbitShared {
...
struct MaskShardLowBatchRangeClosureShared {
    FBlock main_blocks[64];
    FBlock block_blocks[32];
...
__device__ __forceinline__ int maskshard_low_batch_find_prefix_range(
...
__global__ void maskshard_low_maskbatch_range_orbit_kernel(
    ...
    __shared__ MaskShardLowBatchRangeOrbitShared s;

    const MaskShardLowBatchRangeJob job = maskshard_low_batch_decode_range_job(
        groups, group_count, std::uint32_t(blockIdx.x));
    ...
    std::uint32_t warp_task = job.replica * warps_per_block
                            + std::uint32_t(warp_in_block);
    const std::uint32_t warp_step = job.replicas * warps_per_block;
    ...
    for (; warp_task < total; warp_task += warp_step) {
        ...
        if (lane == 0) {
            dbid = maskshard_low_batch_find_prefix_range(
...
__global__ void maskshard_low_maskbatch_range_closure_kernel(
    Count* authoritative_main,
    Count* authoritative_block,
    const MaskShardLowBatchRangeDeviceDesc* groups,
    int group_count,
    ...
    __shared__ MaskShardLowBatchRangeClosureShared s;
    ...
    const std::uint32_t warps_per_block =
        std::uint32_t((blockDim.x + 31) >> 5);
    std::uint32_t task = job.replica * warps_per_block
                       + std::uint32_t(warp_in_block);
    const std::uint32_t task_step = job.replicas * warps_per_block;
    ...
    for (; task < total; task += task_step) {
        ...
        if (lane == 0) {
            bid = maskshard_low_batch_find_prefix_range(s.prefix, task, main_nb);
