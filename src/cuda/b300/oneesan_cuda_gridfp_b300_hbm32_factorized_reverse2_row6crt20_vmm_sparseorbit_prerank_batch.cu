...
__constant__ uint32_t* D_PR_SLOW_LR;
...
struct PreRankOrbitTablesHost {
    ...
    std::vector<uint32_t> slow_lr;
    std::vector<SparseSel> owner_sel,closure_sel,slow_sel;
};
...
static PreRankOrbitTablesHost build_prerank_orbit_tables(){
    ...
    z.owner_sel.assign(keys,{0,0});z.closure_sel.assign(keys,{0,0});z.slow_sel.assign(keys,{0,0});
    if constexpr(L<1)return z;
    for(int p=1;p<=L;++p)for(int hs=0;hs<=L+1;++hs)for(int c=0;c<3;++c){
        auto key=sparse_key(p,hs,c);SparseSel os{uint32_t(z.owner_rec.size()),0},cs{uint32_t(z.closure_rec.size()),0},ss{uint32_t(z.slow_lr.size()),0};
        int dc=(c==int(::L)?1:c==int(::R)?-1:0),he=hs-dc;
        if(he<0||he>HIGH_LUT_K+1){z.owner_sel[key]=os;z.closure_sel[key]=cs;z.slow_sel[key]=ss;continue;}
        uint32_t beg=G_FACTOR.low_all_off[hs],end=G_FACTOR.low_all_off[hs+1];
        for(uint32_t a=beg;a<end;++a){
            uint32_t lr=a-beg,lc=G_FACTOR.low_all_codes[a];
            oneesan::gridfp::MateID m=oneesan::gridfp::MateID(lc)|(oneesan::gridfp::MateID(c)<<(2*L));
            ...
            if(w==oneesan::gridfp::LL||w==oneesan::gridfp::RR||w==oneesan::gridfp::RL){
                ...
                if(iz.valid){
                    ...
                    // The matching L lies in HIGH; retain only this case for the generic path.
                    z.slow_lr.push_back(lr);
                }
            }
        }
        os.count=uint32_t(z.owner_rec.size())-os.off;cs.count=uint32_t(z.closure_rec.size())-cs.off;ss.count=uint32_t(z.slow_lr.size())-ss.off;
        z.owner_sel[key]=os;z.closure_sel[key]=cs;z.slow_sel[key]=ss;
    }
    double mib=(double(z.owner_rec.size()+z.closure_rec.size())*8.0+double(z.slow_lr.size())*4.0)/(1<<20);
    std::cerr<<"pre-rank orbit owner="<<z.owner_rec.size()<<" closure_fast="<<z.closure_rec.size()<<" closure_slow="<<z.slow_lr.size()<<" mib="<<mib<<"\n";
    return z;
}
...
static PreRankOrbitMap make_prerank_orbit_map(const std::vector<FBlock>&fb,int p,int kind){
    ...
    for(int b=0;b<m.nblocks;++b){auto const&x=fb[b];auto key=sparse_key(p,x.hs,x.c);SparseSel q=kind==0?G_PRERANK.owner_sel[key]:(kind==1?G_PRERANK.closure_sel[key]:G_PRERANK.slow_sel[key]);Code hc=(x.stride&&x.end>x.off)?((x.end-x.off)/x.stride):0;sum+=hc*Code(q.count);m.end[b]=sum;m.sel_off[b]=q.off;m.sel_count[b]=q.count;}
    return m;
}
...
__global__ void factor_prerank_slow_closure_kernel(Count*mainv,Count*blockv,const MateID*mates,PreRankOrbitMap map,int p){
    Code total=map.nblocks?map.end[map.nblocks-1]:0;
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){
        int bid=prerank_find_block(q,map);Code base=bid?map.end[bid-1]:0,loc=q-base;uint32_t sc=map.sel_count[bid],hr=uint32_t(loc/sc),k=uint32_t(loc-Code(hr)*sc),lr=D_PR_SLOW_LR[map.sel_off[bid]+k];FBlock x=D_F_MAIN_BLOCKS[bid];Code i=x.off+Code(hr)*x.stride+lr;Count c=mainv[i];if(!c)continue;MateID m=mates?mates[i]:sparse_unrank_main_known(bid,hr,lr);auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);if(z.valid){if(z.blocked)atomic_add_mod(blockv+factor_rank_block(z.mate),c);else atomic_add_mod(mainv+factor_rank_main(z.mate),c);}
    }
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    bool useOrbit=wp.p_hi<=LOW_LUT_K;size_t countBytes=size_t(useOrbit?(ms.size+ds.size):(2*ms.size+2*ds.size))*sizeof(Count),mateBytes=size_t(ms.size)*sizeof(MateID);bool useMate=!pg.use_mi&&(countBytes+mateBytes<=target);
        ...
        for(int p=wp.p_hi;p>=wp.p_lo;--p){
            {
                ...
                auto cm=make_prerank_orbit_map(fmb,p,1),sm=make_prerank_orbit_map(fmb,p,2);Code cn=cm.nblocks?cm.end[cm.nblocks-1]:0,sn=sm.nblocks?sm.end[sm.nblocks-1]:0;launchN(cn,[&](int nb){factor_prerank_closure_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,cm,p);},"factor prerank closure");launchN(sn,[&](int nb){factor_prerank_slow_closure_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,sm,p);},"factor prerank slow closure");
            }
        }
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t sparseBytes=0;uint64_t preRankBytes=uint64_t(G_PRERANK.owner_rec.size()+G_PRERANK.closure_rec.size())*8+vb4(G_PRERANK.slow_lr);uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+sparseBytes+preRankBytes;
        ...
        uint64_t minScratchBytes=0,actualScratchBytes=0,totalGroups=0,mateGroups=0;{const int rr[2][2]={{W-1,LOW_LUT_K+1},{LOW_LUT_K,1}};for(auto const&r:rr){auto fp=window_candidates(W,r[0],r[1]);int nj=1<<int(fp.size());for(int g=0;g<nj;++g){uint32_t mf,mo,bf,bo;window_masks(W,r[0],r[1],fp,uint32_t(g),mf,mo,bf,bo);auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);bool orbit=(r[0]<=LOW_LUT_K);uint64_t countBytes=uint64_t(orbit?(ms.size+ds.size):(2*ms.size+2*ds.size))*sizeof(Count),mateBytes=uint64_t(ms.size)*sizeof(MateID);minScratchBytes=std::max(minScratchBytes,countBytes);bool useMate=countBytes+mateBytes<=scratchLimit;actualScratchBytes=std::max(actualScratchBytes,countBytes+(useMate?mateBytes:0));totalGroups++;mateGroups+=useMate;}}}
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-vmm-sparseorbit-prerank-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prO[d])cudaFree(prO[d]);if(prC[d])cudaFree(prC[d]);if(prS[d])cudaFree(prS[d]);}
}
