...
__constant__ uint32_t* D_F_LOW_MASK_ALL_RANK;
...
__constant__ uint32_t* D_F_HIGH_MASK_ALL_RANK;
...
__constant__ uint32_t* D_PR_OWNER_BLOCK_CLOSURE_META;
__constant__ uint32_t* D_PR_OWNER_BLOCK_CLOSURE_SRC;
...
__constant__ unsigned long long* D_P1_DEST_REC;
__constant__ uint32_t* D_P1_DEST_SRC;
__constant__ unsigned long long* D_CROSS_LOW_DEST_REC;
__constant__ uint32_t* D_CROSS_LOW_DEST_SRC;
__constant__ uint32_t* D_CROSS_LOW_REC_IDX;
__constant__ unsigned long long* D_CROSS_HIGH_DEST_REC;
__constant__ uint32_t* D_CROSS_HIGH_DEST_SRC;
__constant__ uint32_t* D_CROSS_HIGH_REC_IDX;
__constant__ unsigned long long* D_HP_OWNER_REC;
__constant__ unsigned long long* D_HP_OWNER_BLOCK_REC;
__constant__ uint32_t* D_HP_OWNER_BLOCK_CLOSURE_META;
__constant__ uint32_t* D_HP_OWNER_BLOCK_CLOSURE_SRC;
...
struct FactorTablesHost {
    ...
    std::vector<uint32_t> low_all_codes,low_packed_values,low_mask_codes,low_mask_off,low_mask_all_rank;
    std::vector<uint32_t> high_all_codes,high_packed_values,high_mask_codes,high_mask_off,high_mask_all_rank;
...
static FactorTablesHost build_factor_tables(){
    ...
    for(int h=0;h<=MAXW;++h){f.low_all_off[h]=f.low_all_codes.size();for(uint32_t r=0;r<la[h].size();++r){if(uint64_t(r)>=(uint64_t(1)<<(32-L)))throw std::runtime_error("low all rank exceeds packed field");auto c=la[h][r];f.low_all_codes.push_back(c);f.low_packed_values.push_back(r<<L);} }
    f.low_all_off[MAXW+1]=f.low_all_codes.size();
    std::vector<uint64_t> lowIndex;lowIndex.reserve(f.low_all_codes.size());for(uint32_t i=0;i<f.low_all_codes.size();++i)lowIndex.push_back((uint64_t(f.low_all_codes[i])<<32)|i);std::sort(lowIndex.begin(),lowIndex.end());
    auto lowPos=[&](uint32_t c)->uint32_t{auto it=std::lower_bound(lowIndex.begin(),lowIndex.end(),uint64_t(c)<<32);if(it==lowIndex.end()||uint32_t(*it>>32)!=c)throw std::runtime_error("low sparse rank code missing");return uint32_t(*it);};
    f.low_mask_off.resize(size_t(LM)*S);
    for(uint32_t m=0;m<LM;++m)for(int h=0;h<S;++h){size_t ix=size_t(m)*S+h;f.low_mask_off[ix]=f.low_mask_codes.size();if(h<=MAXW)for(uint32_t r=0;r<lm[ix].size();++r){if(uint64_t(r)>=(uint64_t(1)<<L))throw std::runtime_error("low local rank exceeds packed field");auto c=lm[ix][r];f.low_mask_codes.push_back(c);uint32_t ap=lowPos(c);f.low_mask_all_rank.push_back(ap-f.low_all_off[h]);auto &v=f.low_packed_values[ap];v=(v&~((1u<<L)-1u))|r;}}
    lowIndex.clear();lowIndex.shrink_to_fit();
    for(int h=0;h<=MAXW;++h){f.high_all_off[h]=f.high_all_codes.size();for(uint32_t r=0;r<ha[h].size();++r){if(uint64_t(r)>=(uint64_t(1)<<(32-H)))throw std::runtime_error("high all rank exceeds packed field");auto c=ha[h][r];f.high_all_codes.push_back(c);f.high_packed_values.push_back(r<<H);}}
    f.high_all_off[MAXW+1]=f.high_all_codes.size();
    std::vector<uint64_t> highIndex;highIndex.reserve(f.high_all_codes.size());for(uint32_t i=0;i<f.high_all_codes.size();++i)highIndex.push_back((uint64_t(f.high_all_codes[i])<<32)|i);std::sort(highIndex.begin(),highIndex.end());
    auto highPos=[&](uint32_t c)->uint32_t{auto it=std::lower_bound(highIndex.begin(),highIndex.end(),uint64_t(c)<<32);if(it==highIndex.end()||uint32_t(*it>>32)!=c)throw std::runtime_error("high sparse rank code missing");return uint32_t(*it);};
    f.high_mask_off.resize(size_t(HM)*S);
    for(uint32_t m=0;m<HM;++m)for(int h=0;h<S;++h){size_t ix=size_t(m)*S+h;f.high_mask_off[ix]=f.high_mask_codes.size();if(h<=MAXW)for(uint32_t r=0;r<hm[ix].size();++r){if(uint64_t(r)>=(uint64_t(1)<<H))throw std::runtime_error("high local rank exceeds packed field");auto c=hm[ix][r];f.high_mask_codes.push_back(c);uint32_t ap=highPos(c);f.high_mask_all_rank.push_back(ap-f.high_all_off[h]);auto &v=f.high_packed_values[ap];v=(v&~((1u<<H)-1u))|r;}}
    highIndex.clear();highIndex.shrink_to_fit();
    ...
    std::cerr<<"factor tables low_all="<<f.low_all_codes.size()<<" low_mask="<<f.low_mask_codes.size()<<" low_sparse_rank_mib="<<double(f.low_packed_values.size()*4)/(1<<20)<<" high_all="<<f.high_all_codes.size()<<" high_mask="<<f.high_mask_codes.size()<<" high_sparse_rank_mib="<<double(f.high_packed_values.size()*4)/(1<<20)<<"\n";
    return f;
}
...
struct PreRankOrbitTablesHost {
    ...
    std::vector<uint32_t> closure_inv_src,owner_block_closure_meta,owner_block_closure_src;
...
static uint32_t low_all_rank_host(uint32_t code,int h){
    ...
    auto b=G_FACTOR.low_all_codes.begin()+G_FACTOR.low_all_off[h],e=G_FACTOR.low_all_codes.begin()+G_FACTOR.low_all_off[h+1];
    auto it=std::lower_bound(b,e,code);
    if(it==e||*it!=code)throw std::runtime_error("pre-rank low destination missing");
    return uint32_t(it-b);
}
...
static PreRankOrbitTablesHost build_prerank_orbit_tables(){
    ...
    z.owner_block_closure_meta.assign(z.owner_block_rec.size(),0u);
    { constexpr uint32_t B=20,M=(1u<<B)-1u,RB=18,RM=(1u<<RB)-1u;
      for(int p=2;p<=L;++p)for(int he=0;he<=HIGH_LUT_K+1;++he){
        uint32_t nb=G_FACTOR.low_all_off[he+1]-G_FACTOR.low_all_off[he];std::vector<std::vector<uint32_t>> srcs(nb);
        for(int c=0;c<3;++c){int dc=(c==int(::L)?1:c==int(::R)?-1:0),hs=he+dc;if(hs<0||hs>L+1)continue;auto sel=z.closure_sel[sparse_key(p,hs,c)];for(uint32_t k=0;k<sel.count;++k){auto r=z.closure_rec[sel.off+k];uint32_t src=uint32_t(r)&M,dst=uint32_t(r>>B)&M;if(src>RM||dst>=nb)throw std::runtime_error("low block-fused closure rank overflow");srcs[dst].push_back(src|(uint32_t(c)<<RB));}}
        for(uint32_t dst=0;dst<nb;++dst){auto const&v=srcs[dst];if(v.empty())continue;if(v.size()>15)throw std::runtime_error("low block-fused indegree overflow");uint32_t off=uint32_t(z.owner_block_closure_src.size());if(off>0x0fffffffu)throw std::runtime_error("low block-fused source offset overflow");z.owner_block_closure_src.insert(z.owner_block_closure_src.end(),v.begin(),v.end());size_t obi=size_t(p-1)*lowTotal+G_FACTOR.low_all_off[he]+dst;z.owner_block_closure_meta[obi]=(off<<4)|uint32_t(v.size());}
      }
    }
    for(auto r:z.owner_block_rec)if(r==~0ULL)throw std::runtime_error("low blocked-order owner missing");
    double mib=(double(z.owner_block_rec.size()+z.highrr_rec.size()+z.closure_inv_rec.size())*8.0+double(z.closure_inv_src.size()+z.owner_block_closure_meta.size()+z.owner_block_closure_src.size())*4.0)/(1<<20);
...
static uint32_t high_all_rank_host(uint32_t code,int h){
    if(h<0||h>MAXW)throw std::runtime_error("high rank height");auto b=G_FACTOR.high_all_codes.begin()+G_FACTOR.high_all_off[h],e=G_FACTOR.high_all_codes.begin()+G_FACTOR.high_all_off[h+1];auto it=std::lower_bound(b,e,code);if(it==e||*it!=code)throw std::runtime_error("high pre-rank destination missing");return uint32_t(it-b);
}
...
static HighPreRankTablesHost build_high_prerank_tables(){
    ...
    z.owner_block_closure_meta.assign(z.owner_block_rec.size(),0u);
    { constexpr uint32_t B=20,M=(1u<<B)-1u,RB=18,RM=(1u<<RB)-1u;
      for(int p=L+1;p<=L+H;++p)for(int hs=0;hs<=L+1;++hs){
        uint32_t nb=G_FACTOR.high_all_off[hs+1]-G_FACTOR.high_all_off[hs];std::vector<std::vector<uint32_t>> srcs(nb);
        for(int c=0;c<3;++c){int dc=(c==int(::L)?1:c==int(::R)?-1:0),he=hs-dc;if(he<0||he>H+1)continue;auto sel=z.closure_sel[high_pr_key(p,he,c)];for(uint32_t k=0;k<sel.count;++k){auto r=z.closure_rec[sel.off+k];uint32_t src=uint32_t(r)&M,dst=uint32_t(r>>B)&M;if(src>RM||dst>=nb)throw std::runtime_error("high block-fused closure rank overflow");srcs[dst].push_back(src|(uint32_t(c)<<RB));}}
        for(uint32_t dst=0;dst<nb;++dst){auto const&v=srcs[dst];if(v.empty())continue;if(v.size()>15)throw std::runtime_error("high block-fused indegree overflow");uint32_t off=uint32_t(z.owner_block_closure_src.size());if(off>0x0fffffffu)throw std::runtime_error("high block-fused source offset overflow");z.owner_block_closure_src.insert(z.owner_block_closure_src.end(),v.begin(),v.end());size_t obi=size_t(p-L-1)*highTotal+G_FACTOR.high_all_off[hs]+dst;z.owner_block_closure_meta[obi]=(off<<4)|uint32_t(v.size());}
      }
    }
    for(auto r:z.owner_block_rec)if(r==~0ULL)throw std::runtime_error("high blocked-order owner missing");double mib=(double(z.owner_block_rec.size()+z.crossll_rec.size()+z.closure_inv_rec.size())*8.0+double(z.closure_inv_src.size()+z.owner_block_closure_meta.size()+z.owner_block_closure_src.size())*4.0)/(1<<20);std::cerr<<"high pre-rank owner="<<z.owner_block_rec.size()<<" closure_fast="<<z.closure_inv_src.size()<<" closure_inv="<<z.closure_inv_rec.size()<<" closure_crossll="<<z.crossll_rec.size()<<" device_mib="<<mib<<"\n";z.owner_rec.clear();z.owner_rec.shrink_to_fit();z.closure_rec.clear();z.closure_rec.shrink_to_fit();return z;
}
static void print_p1_inverse_merge_stats(){
    constexpr uint32_t B=18,IM=(1u<<B)-1u,P=36;constexpr unsigned long long PM=(1ULL<<P)-1ULL;
    uint64_t oldrec=0,newrec=0,srcs=0;uint32_t maxdeg=0;std::array<uint64_t,8> hist{};
    for(int he=0;he<=HIGH_LUT_K+1;++he){std::unordered_map<uint32_t,uint32_t>deg;
        for(int c=0;c<3;++c){int dc0=(c==int(::L)?1:c==int(R)?-1:0),hs=he+dc0;if(hs<0||hs>LOW_LUT_K+1)continue;auto sel=G_PRERANK.closure_inv_sel[sparse_key(1,hs,c)];oldrec+=sel.count;
            for(uint32_t k=0;k<sel.count;++k){auto r=G_PRERANK.closure_inv_rec[sel.off+k];uint32_t dlr=uint32_t(r)&IM,cnt=uint32_t(r>>(B+P))&7u,dc=uint32_t(r>>(B+P+3))&3u;srcs+=cnt;deg[(dc<<B)|dlr]+=cnt;}}
        newrec+=deg.size();for(auto const&kv:deg){uint32_t d=kv.second;maxdeg=std::max(maxdeg,d);hist[std::min<uint32_t>(7,d)]++;}
    }
    std::cerr<<"p1 inverse merge old_records="<<oldrec<<" merged_dest_records="<<newrec<<" record_ratio="<<(oldrec?double(newrec)/oldrec:0.0)<<" sources="<<srcs<<" max_indegree="<<maxdeg<<" hist";for(int i=1;i<=7;++i)std::cerr<<" d"<<i<<"="<<hist[i];std::cerr<<"\n";
}
struct P1DestTablesHost {
    std::vector<unsigned long long> rec;
    std::vector<uint32_t> src;
    std::array<uint32_t,MAXW+3> off{};
};
static P1DestTablesHost G_P1_DEST;
static bool groupbatch_p1_dest_mode(){
    static int mode=[](){int v=1;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_P1_DEST"))v=std::atoi(e);if(v<0||v>1)throw std::runtime_error("GRIDFP_GROUPBATCH_P1_DEST must be 0 or 1");return v;}();
    return mode!=0;
}
static bool groupbatch_wrap32_mode(){
    static int mode=[](){int v=0;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_WRAP32"))v=std::atoi(e);if(v<0||v>1)throw std::runtime_error("GRIDFP_GROUPBATCH_WRAP32 must be 0 or 1");return v;}();
    return mode!=0;
}
static bool groupbatch_wrap32_mod_safe(Count mod){uint64_t gap=(1ull<<32)-uint64_t(mod);return mod>0&&256ull*gap<=uint64_t(mod);}
static bool G_GROUPBATCH_WRAP32_ACTIVE=false;
static unsigned long long pack_p1_dest_rec(uint32_t dlr,uint32_t dc,uint32_t off,uint32_t cnt){
    constexpr uint32_t IM=(1u<<18)-1u;if(dlr>IM||dc>2||off>0x0fffffffu||cnt==0||cnt>7)throw std::runtime_error("p1 destination record overflow");
    return (unsigned long long)dlr|((unsigned long long)dc<<18)|((unsigned long long)off<<20)|((unsigned long long)cnt<<48);
}
static P1DestTablesHost build_p1_dest_tables(){
    constexpr uint32_t B=18,IM=(1u<<B)-1u,P=36;constexpr unsigned long long PM=(1ULL<<P)-1ULL;P1DestTablesHost z;
    for(int he=0;he<=HIGH_LUT_K+1;++he){z.off[he]=uint32_t(z.rec.size());std::unordered_map<uint32_t,std::vector<uint32_t>> bydst;
        for(int sc=0;sc<3;++sc){int dc0=(sc==int(::L)?1:sc==int(R)?-1:0),hs=he+dc0;if(hs<0||hs>LOW_LUT_K+1)continue;auto sel=G_PRERANK.closure_inv_sel[sparse_key(1,hs,sc)];
            for(uint32_t k=0;k<sel.count;++k){auto r=G_PRERANK.closure_inv_rec[sel.off+k];uint32_t dlr=uint32_t(r)&IM;unsigned long long payload=(r>>B)&PM;uint32_t cnt=uint32_t(r>>(B+P))&7u,dc=uint32_t(r>>(B+P+3))&3u;auto&v=bydst[(dc<<B)|dlr];
                for(uint32_t j=0;j<cnt;++j){uint32_t lr=cnt<=2?uint32_t(payload>>(18*j))&IM:G_PRERANK.closure_inv_src[uint32_t(payload)+j];v.push_back(lr|(uint32_t(sc)<<B));}
            }
        }
        std::vector<uint32_t> keys;keys.reserve(bydst.size());for(auto const&kv:bydst)keys.push_back(kv.first);std::sort(keys.begin(),keys.end());
        for(uint32_t key:keys){auto&v=bydst[key];if(v.empty()||v.size()>7)throw std::runtime_error("p1 destination indegree overflow");uint32_t so=uint32_t(z.src.size());z.src.insert(z.src.end(),v.begin(),v.end());z.rec.push_back(pack_p1_dest_rec(key&IM,key>>B,so,uint32_t(v.size())));}
    }
    z.off[HIGH_LUT_K+2]=uint32_t(z.rec.size());
    std::cerr<<"p1 destination table records="<<z.rec.size()<<" sources="<<z.src.size()<<" device_mib="<<(double(z.rec.size()*8+z.src.size()*4)/(1<<20))<<"\n";
    return z;
}
struct CrossDestTablesHost {
    std::vector<unsigned long long> low_rec,high_rec;
    std::vector<uint32_t> low_src,high_src;
    std::vector<uint32_t> low_cross_idx,high_cross_idx;
    std::unordered_map<unsigned long long,SparseSel> low_sel,high_sel;
    std::unordered_map<unsigned long long,SparseSel> low_cross_sel,high_cross_sel;
};
static CrossDestTablesHost G_CROSS_DEST;
static bool groupbatch_cross_dest_mode(){
    static int mode=[](){int v=0;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_CROSS_DEST"))v=std::atoi(e);if(v<0||v>1)throw std::runtime_error("GRIDFP_GROUPBATCH_CROSS_DEST must be 0 or 1");return v;}();
    return mode!=0;
}
static unsigned long long cross_dest_key(uint32_t mask,int h,int depth){return (static_cast<unsigned long long>(mask)<<16)|(static_cast<unsigned long long>(uint32_t(h))<<8)|uint32_t(depth);}
static unsigned long long pack_cross_dest_rec(uint32_t dst,uint32_t off,uint32_t cnt){
    if(dst>0xfffffu||off>0x0fffffffu||cnt==0||cnt>15)throw std::runtime_error("cross destination record overflow");
    return (unsigned long long)dst|((unsigned long long)off<<20)|((unsigned long long)cnt<<48);
}
static SparseSel cross_dest_find(bool low,uint32_t mask,int h,int depth){
    auto const&tab=low?G_CROSS_DEST.low_sel:G_CROSS_DEST.high_sel;auto it=tab.find(cross_dest_key(mask,h,depth));return it==tab.end()?SparseSel{0,0}:it->second;
}
static unsigned long long cross_record_depth_key(size_t key,int depth){return (static_cast<unsigned long long>(key)<<8)|uint32_t(depth);}
static SparseSel cross_record_find(bool low,size_t key,int depth){
    auto const&tab=low?G_CROSS_DEST.low_cross_sel:G_CROSS_DEST.high_cross_sel;auto it=tab.find(cross_record_depth_key(key,depth));return it==tab.end()?SparseSel{0,0}:it->second;
}
static CrossDestTablesHost build_cross_dest_tables(){
    CrossDestTablesHost z;constexpr int S=FactorTablesHost::STRIDE;
    auto build_side=[&](bool low){
        int K=low?LOW_LUT_K:HIGH_LUT_K;uint32_t NM=1u<<K;
        auto const&off=low?G_FACTOR.low_mask_off:G_FACTOR.high_mask_off;
        auto const&codes=low?G_FACTOR.low_mask_codes:G_FACTOR.high_mask_codes;
        auto&rec=low?z.low_rec:z.high_rec;auto&src=low?z.low_src:z.high_src;auto&sels=low?z.low_sel:z.high_sel;
        std::array<std::vector<std::pair<uint32_t,uint32_t>>,MAXW+1> bydepth;
        for(uint32_t mask=0;mask<NM;++mask)for(int h=2;h<=K+1;++h){
            for(auto&v:bydepth)v.clear();size_t so=off[size_t(mask)*S+h],se=off[size_t(mask)*S+h+1];
            for(uint32_t sr=0;so+sr<se;++sr){uint32_t code=codes[so+sr];int bal=0,minbal=0;
                if(low){for(int a=K-1;a>=0;--a){auto mv=MateValue((code>>(2*a))&3u);if(mv==R)--bal;else if(mv==L)++bal;if(bal<minbal){minbal=bal;int depth=-bal;uint32_t nc=(code&~(3u<<(2*a)))|(uint32_t(L)<<(2*a));if(seg_occ(nc,K)!=mask)throw std::runtime_error("cross low destination mask changed");auto ab=G_FACTOR.low_all_codes.begin()+G_FACTOR.low_all_off[h-2],ae=G_FACTOR.low_all_codes.begin()+G_FACTOR.low_all_off[h-1],ai=std::lower_bound(ab,ae,nc);if(ai==ae||*ai!=nc)continue;uint32_t ap=G_FACTOR.low_all_off[h-2]+uint32_t(ai-ab),dr=G_FACTOR.low_packed_values[ap]&((1u<<LOW_LUT_K)-1u);bydepth[depth].push_back({dr,sr});}}}
                else{for(int a=0;a<K;++a){auto mv=MateValue((code>>(2*a))&3u);if(mv==L)--bal;else if(mv==R)++bal;if(bal<minbal){minbal=bal;int depth=-bal;uint32_t nc=(code&~(3u<<(2*a)))|(uint32_t(R)<<(2*a));if(seg_occ(nc,K)!=mask)throw std::runtime_error("cross high destination mask changed");auto ab=G_FACTOR.high_all_codes.begin()+G_FACTOR.high_all_off[h-2],ae=G_FACTOR.high_all_codes.begin()+G_FACTOR.high_all_off[h-1],ai=std::lower_bound(ab,ae,nc);if(ai==ae||*ai!=nc)continue;uint32_t ap=G_FACTOR.high_all_off[h-2]+uint32_t(ai-ab),dr=G_FACTOR.high_packed_values[ap]&((1u<<HIGH_LUT_K)-1u);bydepth[depth].push_back({dr,sr});}}}
            }
            for(int depth=1;depth<=K;++depth){auto&v=bydepth[depth];if(v.empty())continue;std::sort(v.begin(),v.end());SparseSel sel{uint32_t(rec.size()),0};for(size_t q=0;q<v.size();){size_t e=q+1;while(e<v.size()&&v[e].first==v[q].first)++e;uint32_t cnt=uint32_t(e-q),oo=uint32_t(src.size());if(cnt>7)throw std::runtime_error("cross destination indegree exceeds measured bound");for(size_t j=q;j<e;++j)src.push_back(v[j].second);rec.push_back(pack_cross_dest_rec(v[q].first,oo,cnt));q=e;}sel.count=uint32_t(rec.size())-sel.off;sels.emplace(cross_dest_key(mask,h,depth),sel);}
        }
    };
    build_side(true);build_side(false);
    auto build_cross_groups=[&](bool low){
        auto&idx=low?z.low_cross_idx:z.high_cross_idx;auto&sels=low?z.low_cross_sel:z.high_cross_sel;
        std::array<std::vector<uint32_t>,MAXW+1> bydepth;
        if(low){
            for(int p=LOW_LUT_K+1;p<=LOW_LUT_K+HIGH_LUT_K;++p)for(int he=0;he<=HIGH_LUT_K+1;++he)for(int c=0;c<3;++c){
                for(auto&v:bydepth)v.clear();size_t key=high_pr_key(p,he,c);auto sel=G_HPR.crossll_sel[key];
                for(uint32_t k=0;k<sel.count;++k){uint32_t ri=sel.off+k;auto r=G_HPR.crossll_rec[ri];int depth=int((r>>40)&63u);if(depth>0&&depth<=LOW_LUT_K)bydepth[depth].push_back(ri);}
                for(int depth=1;depth<=LOW_LUT_K;++depth)if(!bydepth[depth].empty()){SparseSel ds{uint32_t(idx.size()),uint32_t(bydepth[depth].size())};idx.insert(idx.end(),bydepth[depth].begin(),bydepth[depth].end());sels.emplace(cross_record_depth_key(key,depth),ds);}
            }
        }else{
            for(int p=1;p<=LOW_LUT_K;++p)for(int hs=0;hs<=LOW_LUT_K+1;++hs)for(int c=0;c<3;++c){
                for(auto&v:bydepth)v.clear();size_t key=sparse_key(p,hs,c);auto sel=G_PRERANK.highrr_sel[key];
                for(uint32_t k=0;k<sel.count;++k){uint32_t ri=sel.off+k;auto r=G_PRERANK.highrr_rec[ri];int depth=int((r>>40)&63u);if(depth>0&&depth<=HIGH_LUT_K)bydepth[depth].push_back(ri);}
                for(int depth=1;depth<=HIGH_LUT_K;++depth)if(!bydepth[depth].empty()){SparseSel ds{uint32_t(idx.size()),uint32_t(bydepth[depth].size())};idx.insert(idx.end(),bydepth[depth].begin(),bydepth[depth].end());sels.emplace(cross_record_depth_key(key,depth),ds);}
            }
        }
    };
    build_cross_groups(true);build_cross_groups(false);
    std::cerr<<"cross destination tables low_rec="<<z.low_rec.size()<<" low_src="<<z.low_src.size()<<" low_cross_idx="<<z.low_cross_idx.size()<<" high_rec="<<z.high_rec.size()<<" high_src="<<z.high_src.size()<<" high_cross_idx="<<z.high_cross_idx.size()<<" device_mib="<<double((z.low_rec.size()+z.high_rec.size())*8+(z.low_src.size()+z.high_src.size()+z.low_cross_idx.size()+z.high_cross_idx.size())*4)/(1<<20)<<"\n";
    return z;
}
...
__device__ __forceinline__ Code factor_global_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[hc],lp=D_F_LOW_DENSE_PACKED_RANK[lc];uint32_t har=hp>>H,lar=lp>>L;Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[he]+har];MateValue c=mget(m,L);if(c>N)rank+=D_FULL_DP[L][he];if(c>R&&he>0)rank+=D_FULL_DP[L][he-1];return rank+lar;
}
__device__ __forceinline__ Code factor_global_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[hc],lp=D_F_LOW_DENSE_PACKED_RANK[lc];uint32_t har=hp>>H,lar=lp>>L;return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[he]+har]+lar;
}
...
__device__ __forceinline__ Code factor_global_rank_main_index(Code i){
    ...
    Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[x.he]+har];if(x.c>N)rank+=D_FULL_DP[L][x.he];if(x.c>R&&x.he>0)rank+=D_FULL_DP[L][x.he-1];return rank+lar;
}
...
struct BatchFactorIoTask { uint32_t cfg,block; Code begin; };
...
__device__ __forceinline__ int batch_factor_find(Code i,const FBlock*b,int nb){int lo=0,hi=nb;while(lo<hi){int m=(lo+hi)>>1;if(i<b[m].end)hi=m;else lo=m+1;}return lo;}
__device__ __forceinline__ Code batch_factor_global_main(Code i,const BatchFactorIoCfg*cp){
    const BatchFactorIoCfg&c=*cp;
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;int bi=batch_factor_find(i,c.mainb,c.main_nb);FBlock x=c.mainb[bi];Code r=i-x.off;uint32_t hr=x.stride?uint32_t(r/x.stride):0,lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;uint32_t har,lar;
    if(c.fix_low){har=hr;uint32_t lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(c.mask)*S+x.hs]+lr];lar=D_F_LOW_DENSE_PACKED_RANK[lc]>>L;}
    else{uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(c.mask)*S+x.he]+hr];har=D_F_HIGH_PACKED_RANK[hc]>>H;lar=lr;}
    Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[x.he]+har];if(x.c>N)rank+=D_FULL_DP[L][x.he];if(x.c>R&&x.he>0)rank+=D_FULL_DP[L][x.he-1];return rank+lar;
}
__device__ __forceinline__ Code batch_factor_global_block(Code i,const BatchFactorIoCfg*cp){
    const BatchFactorIoCfg&c=*cp;
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;int bi=batch_factor_find(i,c.blockb,c.block_nb);FBlock x=c.blockb[bi];Code r=i-x.off;uint32_t hr=x.stride?uint32_t(r/x.stride):0,lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;uint32_t har,lar;
    if(c.fix_low){har=hr;uint32_t lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(c.mask)*S+x.hs]+lr];lar=D_F_LOW_DENSE_PACKED_RANK[lc]>>L;}
    else{uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(c.mask)*S+x.he]+hr];har=D_F_HIGH_PACKED_RANK[hc]>>H;lar=lr;}
    return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[x.he]+har]+lar;
}
__device__ __forceinline__ Count groupbatch_wrap32_normalize(Count x){Count mod=D_MOD;return x>=mod?x-mod:x;}
__device__ __forceinline__ void groupbatch_wrap32_atomic_add(Count*p,Count v){
    if(!v)return;unsigned int add=unsigned(v),gap=0u-unsigned(D_MOD);
    for(;;){unsigned int old=atomicAdd(reinterpret_cast<unsigned int*>(p),add);if(old<=0xffffffffu-add)return;add=gap;}
}
template<bool SCATTER,bool NORMALIZE_BLOCK> __global__ void batch_factor_io_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchFactorIoTask*tasks,Code nt){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;constexpr Code CH=GROUPBATCH_CHUNK;Code n=t.block?c->block_n:c->main_n,end=min(n,t.begin+CH),bo=t.block?c->block_off:c->main_off;constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;
    for(Code i=t.begin+threadIdx.x;i<end;i+=blockDim.x){int bi=t.block?batch_factor_find(i,c->blockb,c->block_nb):batch_factor_find(i,c->mainb,c->main_nb);FBlock x=t.block?c->blockb[bi]:c->mainb[bi];Code z=i-x.off;uint32_t hr=x.stride?uint32_t(z/x.stride):0,lr=x.stride?uint32_t(z-Code(hr)*x.stride):0;uint32_t har,lar;if(c->fix_low){har=hr;lar=D_F_LOW_MASK_ALL_RANK[D_F_LOW_MASK_OFF[size_t(c->mask)*S+(t.block?x.he:x.hs)]+lr];}else{har=D_F_HIGH_MASK_ALL_RANK[D_F_HIGH_MASK_OFF[size_t(c->mask)*S+x.he]+hr];lar=lr;}Code g;if(t.block)g=D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[x.he]+har]+lar;else{g=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[x.he]+har];if(x.c>N)g+=D_FULL_DP[L][x.he];if(x.c>R&&x.he>0)g+=D_FULL_DP[L][x.he-1];g+=lar;}Count*bp=arena+bo+i;if constexpr(SCATTER){Count v=*bp;if constexpr(NORMALIZE_BLOCK){if(t.block)v=groupbatch_wrap32_normalize(v);}if(t.block)global_store_block(g,v);else global_store_main(g,v);}else *bp=t.block?global_load_block(g):global_load_main(g);}
}



__device__ __forceinline__ Count add_mod_plain(Count a,Count b);
struct BatchOwnerTask { uint32_t cfg,bid; Code begin; };
struct BatchCrossTask { uint32_t cfg,bid,sel_off,sel_count; Code begin; };
struct BatchP1DestTask { uint32_t cfg,rec_off,rec_count,he; Code begin; };
struct BatchCrossDestTask { uint32_t cfg,bid,cross_idx_off,cross_count,fac_off,fac_count; Code begin; };

template<bool WRAP32> __global__ void batch_low_owner_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchOwnerTask*tasks,Code nt,int p){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;FBlock bx=c->blockb[t.bid];constexpr Code CH=GROUPBATCH_CHUNK;Code len=bx.end-bx.off,end=min(len,t.begin+CH);constexpr uint32_t B=20,M=(1u<<B)-1u;Code lowTotal=D_F_LOW_ALL_OFF[MAXW+1];Count*mainv=arena+c->main_off,*blockv=arena+c->block_off;
    for(Code z=t.begin+threadIdx.x;z<end;z+=blockDim.x){uint32_t hr=uint32_t(z/bx.stride),blr=uint32_t(z-Code(hr)*bx.stride);Code obi=Code(p-1)*lowTotal+D_F_LOW_ALL_OFF[bx.he]+blr;unsigned long long r=D_PR_OWNER_BLOCK_REC[obi];uint32_t lr=uint32_t(r)&M,mlr=uint32_t(r>>B)&M,cvv=uint32_t(r>>(2*B))&3u,ty=uint32_t(r>>(2*B+2))&3u;FBlock x=c->mainb[3*int(bx.he)+int(cvv)],dx=x;if(p==LOW_LUT_K){int dc=(ty==1?int(R):int(::L));dx=c->mainb[3*int(bx.he)+dc];}Code i=x.off+Code(hr)*x.stride+lr,j=dx.off+Code(hr)*dx.stride+mlr,q=bx.off+z;Count cv=mainv[i],d=blockv[q],ca=0;if constexpr(WRAP32)d=groupbatch_wrap32_normalize(d);uint32_t meta=D_PR_OWNER_BLOCK_CLOSURE_META[obi],co=meta>>4,cn=meta&15u;for(uint32_t a=0;a<cn;++a){uint32_t sr=D_PR_OWNER_BLOCK_CLOSURE_SRC[co+a],cc=sr>>18,sl=sr&((1u<<18)-1u);FBlock sx=c->mainb[3*int(bx.he)+int(cc)];Count v=mainv[sx.off+Code(hr)*sx.stride+sl];if(v)ca=add_mod_plain(ca,v);}if(ty==0){mainv[j]=add_mod_plain(mainv[j],cv);mainv[i]=add_mod_plain(cv,d);blockv[q]=ca;}else{Count cc=mainv[j],pair=add_mod_plain(cv,cc);mainv[i]=add_mod_plain(pair,d);if(p==1){mainv[j]=pair;blockv[q]=0;}else blockv[q]=add_mod_plain(cv,ca);}}
}

template<bool WRAP32> __global__ void batch_high_owner_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchOwnerTask*tasks,Code nt,int p){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;FBlock bx=c->blockb[t.bid];constexpr Code CH=GROUPBATCH_CHUNK;Code len=bx.end-bx.off,end=min(len,t.begin+CH);constexpr uint32_t B=20,M=(1u<<B)-1u;Code highTotal=D_F_HIGH_ALL_OFF[MAXW+1];Count*mainv=arena+c->main_off,*blockv=arena+c->block_off;
    for(Code z=t.begin+threadIdx.x;z<end;z+=blockDim.x){uint32_t bhr=uint32_t(z/bx.stride),lr=uint32_t(z-Code(bhr)*bx.stride);Code obi=Code(p-LOW_LUT_K-1)*highTotal+D_F_HIGH_ALL_OFF[bx.he]+bhr;unsigned long long r=D_HP_OWNER_BLOCK_REC[obi];uint32_t hr=uint32_t(r)&M,mhr=uint32_t(r>>B)&M,cvv=uint32_t(r>>(2*B))&3u,ty=uint32_t(r>>(2*B+2))&3u,tc=uint32_t(r>>(2*B+4))&3u;int sd=(cvv==uint32_t(::L)?1:cvv==uint32_t(R)?-1:0),he=int(bx.he)-sd;FBlock x=c->mainb[3*he+int(cvv)];int td=(tc==uint32_t(::L)?1:tc==uint32_t(R)?-1:0),the=int(bx.he)-td;FBlock dx=c->mainb[3*the+int(tc)];Code i=x.off+Code(hr)*x.stride+lr,j=dx.off+Code(mhr)*dx.stride+lr,q=bx.off+z;Count cv=mainv[i],d=blockv[q],ca=0;if constexpr(WRAP32)d=groupbatch_wrap32_normalize(d);uint32_t meta=D_HP_OWNER_BLOCK_CLOSURE_META[obi],co=meta>>4,cn=meta&15u;for(uint32_t a=0;a<cn;++a){uint32_t sr=D_HP_OWNER_BLOCK_CLOSURE_SRC[co+a],cc=sr>>18,sh=sr&((1u<<18)-1u);int sdc=(cc==uint32_t(::L)?1:cc==uint32_t(R)?-1:0),she=int(bx.he)-sdc;FBlock sx=c->mainb[3*she+int(cc)];Count v=mainv[sx.off+Code(sh)*sx.stride+lr];if(v)ca=add_mod_plain(ca,v);}if(ty==0){mainv[j]=add_mod_plain(mainv[j],cv);mainv[i]=add_mod_plain(cv,d);blockv[q]=ca;}else{Count cc=mainv[j];mainv[i]=add_mod_plain(add_mod_plain(cv,cc),d);blockv[q]=add_mod_plain(cv,ca);}}
}

__global__ void batch_low_highrr_dest_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchCrossDestTask*tasks,Code nt){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;FBlock x=c->mainb[t.bid];constexpr uint32_t B=20,M=(1u<<B)-1u,DM=(1u<<20)-1u;Count*mainv=arena+c->main_off,*blockv=arena+c->block_off;Code total=Code(t.cross_count)*t.fac_count,end=min(total,t.begin+Code(GROUPBATCH_CHUNK));int nhe=int(x.he)-2;FBlock bx=c->blockb[nhe];
    for(Code z=t.begin+threadIdx.x;z<end;z+=blockDim.x){uint32_t ci=uint32_t(z/t.fac_count),fk=uint32_t(z-Code(ci)*t.fac_count),cri=D_CROSS_HIGH_REC_IDX[t.cross_idx_off+ci];unsigned long long cr=D_PR_HIGHRR_REC[cri];uint32_t slr=uint32_t(cr)&M,dlr=uint32_t(cr>>B)&M;unsigned long long r=D_CROSS_HIGH_DEST_REC[t.fac_off+fk];uint32_t dhr=uint32_t(r)&DM,so=uint32_t(r>>20)&0x0fffffffu,cnt=uint32_t(r>>48)&15u;Count acc=0;for(uint32_t j=0;j<cnt;++j){uint32_t hr=D_CROSS_HIGH_DEST_SRC[so+j];Count v=mainv[x.off+Code(hr)*x.stride+slr];if(v)acc=add_mod_plain(acc,v);}if(acc){Count*dp=blockv+bx.off+Code(dhr)*bx.stride+dlr;*dp=add_mod_plain(*dp,acc);}}
}
__global__ void batch_high_crossll_dest_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchCrossDestTask*tasks,Code nt){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;FBlock x=c->mainb[t.bid];constexpr uint32_t B=20,M=(1u<<B)-1u,DM=(1u<<20)-1u;Count*mainv=arena+c->main_off,*blockv=arena+c->block_off;Code total=Code(t.cross_count)*t.fac_count,end=min(total,t.begin+Code(GROUPBATCH_CHUNK));int nh=int(x.hs)-2;FBlock bx=c->blockb[nh];
    for(Code z=t.begin+threadIdx.x;z<end;z+=blockDim.x){uint32_t ci=uint32_t(z/t.fac_count),fk=uint32_t(z-Code(ci)*t.fac_count),cri=D_CROSS_LOW_REC_IDX[t.cross_idx_off+ci];unsigned long long cr=D_HP_CROSSLL_REC[cri];uint32_t hr=uint32_t(cr)&M,bhr=uint32_t(cr>>B)&M;unsigned long long r=D_CROSS_LOW_DEST_REC[t.fac_off+fk];uint32_t dlr=uint32_t(r)&DM,so=uint32_t(r>>20)&0x0fffffffu,cnt=uint32_t(r>>48)&15u;Count acc=0;for(uint32_t j=0;j<cnt;++j){uint32_t lr=D_CROSS_LOW_DEST_SRC[so+j];Count v=mainv[x.off+Code(hr)*x.stride+lr];if(v)acc=add_mod_plain(acc,v);}if(acc){Count*dp=blockv+bx.off+Code(bhr)*bx.stride+dlr;*dp=add_mod_plain(*dp,acc);}}
}

template<bool WRAP32,bool P1> __global__ void batch_low_highrr_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchCrossTask*tasks,Code nt){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;FBlock x=c->mainb[t.bid];constexpr Code CH=GROUPBATCH_CHUNK;Code rows=x.stride?((x.end-x.off)/x.stride):0,total=rows*Code(t.sel_count),end=min(total,t.begin+CH);constexpr uint32_t B=20,M=(1u<<B)-1u;constexpr int H=HIGH_LUT_K,S=MAXW+2;constexpr uint32_t HM=(1u<<H)-1u;Count*mainv=arena+c->main_off,*blockv=arena+c->block_off;
    for(Code z=t.begin+threadIdx.x;z<end;z+=blockDim.x){uint32_t hr=uint32_t(z/t.sel_count),k=uint32_t(z-Code(hr)*t.sel_count);unsigned long long rec=D_PR_HIGHRR_REC[t.sel_off+k];uint32_t lr=uint32_t(rec)&M,dlr=uint32_t(rec>>B)&M;int depth=int((rec>>(2*B))&63u);Count v=mainv[x.off+Code(hr)*x.stride+lr];if(!v)continue;uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(c->mask)*S+x.he]+hr];int match=-1,s=depth;
#pragma unroll
        for(int a=0;a<H;++a){auto mv=MateValue((hc>>(2*a))&3u);if(mv==L){if(--s==0){match=a;break;}}else if(mv==R)++s;}if(match<0)continue;uint32_t nhc=(hc&~(3u<<(2*match)))|(uint32_t(R)<<(2*match));uint32_t hp=D_F_HIGH_PACKED_RANK[nhc],nhr=hp&HM;int nhe=int(x.he)-2;if constexpr(P1){FBlock dx=c->mainb[3*nhe+int(x.c)];atomic_add_mod(mainv+dx.off+Code(nhr)*dx.stride+dlr,v);}else{FBlock bx=c->blockb[nhe];Code di=bx.off+Code(nhr)*bx.stride+dlr;if constexpr(WRAP32)groupbatch_wrap32_atomic_add(blockv+di,v);else atomic_add_mod(blockv+di,v);}}
}

__global__ void batch_low_inv_p1_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchCrossTask*tasks,Code nt){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;FBlock x=c->mainb[t.bid];constexpr Code CH=GROUPBATCH_CHUNK;Code rows=x.stride?((x.end-x.off)/x.stride):0,total=rows*Code(t.sel_count),end=min(total,t.begin+CH);constexpr uint32_t IB=18,IM=(1u<<IB)-1u,P=36;constexpr unsigned long long PM=(1ULL<<P)-1ULL;Count*mainv=arena+c->main_off;
    for(Code z=t.begin+threadIdx.x;z<end;z+=blockDim.x){uint32_t hr=uint32_t(z/t.sel_count),k=uint32_t(z-Code(hr)*t.sel_count);unsigned long long r=D_PR_CLOSURE_INV_REC[t.sel_off+k];uint32_t dlr=uint32_t(r)&IM;unsigned long long payload=(r>>IB)&PM;uint32_t cnt=uint32_t(r>>(IB+P))&7u,dc=uint32_t(r>>(IB+P+3))&3u;Count acc=0;for(uint32_t j=0;j<cnt;++j){uint32_t lr=cnt<=2?uint32_t(payload>>(18*j))&IM:D_PR_CLOSURE_INV_SRC[uint32_t(payload)+j];Count v=mainv[x.off+Code(hr)*x.stride+lr];if(v)acc=add_mod_plain(acc,v);}if(acc){FBlock dx=c->mainb[3*x.he+int(dc)];atomic_add_mod(mainv+dx.off+Code(hr)*dx.stride+dlr,acc);}}
}
__global__ void batch_low_p1_dest_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchP1DestTask*tasks,Code nt){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;constexpr Code CH=GROUPBATCH_CHUNK;Code total=Code(t.rec_count)*(D_F_HIGH_MASK_OFF[size_t(c->mask)*(MAXW+2)+int(t.he)+1]-D_F_HIGH_MASK_OFF[size_t(c->mask)*(MAXW+2)+int(t.he)]),end=min(total,t.begin+CH);constexpr uint32_t IM=(1u<<18)-1u;Count*mainv=arena+c->main_off;
    for(Code z=t.begin+threadIdx.x;z<end;z+=blockDim.x){uint32_t hr=uint32_t(z/t.rec_count),k=uint32_t(z-Code(hr)*t.rec_count);unsigned long long r=D_P1_DEST_REC[t.rec_off+k];uint32_t dlr=uint32_t(r)&IM,dc=uint32_t(r>>18)&3u,so=uint32_t(r>>20)&0x0fffffffu,cnt=uint32_t(r>>48)&7u;Count acc=0;for(uint32_t j=0;j<cnt;++j){uint32_t sr=D_P1_DEST_SRC[so+j],lr=sr&IM,sc=sr>>18;FBlock sx=c->mainb[3*int(t.he)+int(sc)];Count v=mainv[sx.off+Code(hr)*sx.stride+lr];if(v)acc=add_mod_plain(acc,v);}if(acc){FBlock dx=c->mainb[3*int(t.he)+int(dc)];Count*dp=mainv+dx.off+Code(hr)*dx.stride+dlr;*dp=add_mod_plain(*dp,acc);}}
}

template<bool WRAP32> __global__ void batch_high_crossll_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchCrossTask*tasks,Code nt){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;FBlock x=c->mainb[t.bid];constexpr Code CH=GROUPBATCH_CHUNK;Code lc=x.stride,total=lc*Code(t.sel_count),end=min(total,t.begin+CH);constexpr uint32_t B=20,M=(1u<<B)-1u;constexpr int LOWK=LOW_LUT_K,S=MAXW+2;constexpr uint32_t LM=(1u<<LOWK)-1u;Count*mainv=arena+c->main_off,*blockv=arena+c->block_off;
    for(Code z=t.begin+threadIdx.x;z<end;z+=blockDim.x){uint32_t k=uint32_t(z/lc),lr=uint32_t(z-Code(k)*lc);unsigned long long r=D_HP_CROSSLL_REC[t.sel_off+k];uint32_t hr=uint32_t(r)&M,bhr=uint32_t(r>>B)&M;int depth=int((r>>(2*B))&63u);Count v=mainv[x.off+Code(hr)*x.stride+lr];if(!v)continue;uint32_t code=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(c->mask)*S+x.hs]+lr];int match=-1,s=depth;
#pragma unroll
        for(int a=LOWK-1;a>=0;--a){auto mv=MateValue((code>>(2*a))&3u);if(mv==R){if(--s==0){match=a;break;}}else if(mv==oneesan::gridfp::L)++s;}if(match<0)continue;uint32_t nc=(code&~(3u<<(2*match)))|(uint32_t(::L)<<(2*match));uint32_t nlr=D_F_LOW_DENSE_PACKED_RANK[nc]&LM;int nh=int(x.hs)-2;FBlock bx=c->blockb[nh];Code di=bx.off+Code(bhr)*bx.stride+nlr;if constexpr(WRAP32)groupbatch_wrap32_atomic_add(blockv+di,v);else atomic_add_mod(blockv+di,v);}
}
...
__global__ void factor_prerank_owner_blockorder_kernel(Count*mainv,Count*blockv,Code n,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<n;q+=st){
        int bb=f_find_block(q);FBlock bx=D_F_BLOCK_BLOCKS[bb];Code loc=q-bx.off;uint32_t hr=uint32_t(loc/bx.stride),blr=uint32_t(loc-Code(hr)*bx.stride);Code obi=Code(p-1)*lowTotal+D_F_LOW_ALL_OFF[bx.he]+blr;
        unsigned long long r=D_PR_OWNER_BLOCK_REC[obi];uint32_t lr=uint32_t(r)&M,mlr=uint32_t(r>>B)&M,c=uint32_t(r>>(2*B))&3u,ty=uint32_t(r>>(2*B+2))&3u;
        FBlock x=D_F_MAIN_BLOCKS[3*int(bx.he)+int(c)],dx=x;if(p==LOW_LUT_K){int dc=(ty==1?int(R):int(::L));dx=D_F_MAIN_BLOCKS[3*int(bx.he)+dc];}
        Code i=x.off+Code(hr)*x.stride+lr,j=dx.off+Code(hr)*dx.stride+mlr;Count cv=mainv[i],d=blockv[q],ca=0;uint32_t meta=D_PR_OWNER_BLOCK_CLOSURE_META[obi],co=meta>>4,cn=meta&15u;for(uint32_t z=0;z<cn;++z){uint32_t sr=D_PR_OWNER_BLOCK_CLOSURE_SRC[co+z],cc=sr>>18,sl=sr&((1u<<18)-1u);FBlock sx=D_F_MAIN_BLOCKS[3*int(bx.he)+int(cc)];Count v=mainv[sx.off+Code(hr)*sx.stride+sl];if(v)ca=add_mod_plain(ca,v);}
        if(ty==0){mainv[j]=add_mod_plain(mainv[j],cv);mainv[i]=add_mod_plain(cv,d);blockv[q]=ca;}
        else{Count cc=mainv[j],pair=add_mod_plain(cv,cc);mainv[i]=add_mod_plain(pair,d);if(p==1){mainv[j]=pair;blockv[q]=0;}else blockv[q]=add_mod_plain(cv,ca);}
    }
}
...
__global__ void factor_high_prerank_owner_blockorder_kernel(Count*mainv,Count*blockv,Code n,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<n;q+=st){
        int bb=f_find_block(q);FBlock bx=D_F_BLOCK_BLOCKS[bb];Code loc=q-bx.off;uint32_t bhr=uint32_t(loc/bx.stride),lr=uint32_t(loc-Code(bhr)*bx.stride);Code obi=Code(p-LOW_LUT_K-1)*highTotal+D_F_HIGH_ALL_OFF[bx.he]+bhr;
        unsigned long long r=D_HP_OWNER_BLOCK_REC[obi];uint32_t hr=uint32_t(r)&M,mhr=uint32_t(r>>B)&M,c=uint32_t(r>>(2*B))&3u,ty=uint32_t(r>>(2*B+2))&3u,tc=uint32_t(r>>(2*B+4))&3u;
        ...
        Code i=x.off+Code(hr)*x.stride+lr,j=dx.off+Code(mhr)*dx.stride+lr;Count cv=mainv[i],d=blockv[q],ca=0;uint32_t meta=D_HP_OWNER_BLOCK_CLOSURE_META[obi],co=meta>>4,cn=meta&15u;for(uint32_t z=0;z<cn;++z){uint32_t sr=D_HP_OWNER_BLOCK_CLOSURE_SRC[co+z],cc=sr>>18,sh=sr&((1u<<18)-1u);int sdc=(cc==uint32_t(::L)?1:cc==uint32_t(R)?-1:0),she=int(bx.he)-sdc;FBlock sx=D_F_MAIN_BLOCKS[3*she+int(cc)];Count v=mainv[sx.off+Code(sh)*sx.stride+lr];if(v)ca=add_mod_plain(ca,v);}
        if(ty==0){mainv[j]=add_mod_plain(mainv[j],cv);mainv[i]=add_mod_plain(cv,d);blockv[q]=ca;}else{Count cc=mainv[j];mainv[i]=add_mod_plain(add_mod_plain(cv,cc),d);blockv[q]=add_mod_plain(cv,ca);}
    }
}
...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;MateID*dMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;
    BatchFactorIoCfg*dBatchCfg=nullptr;BatchFactorIoTask*dBatchIo=nullptr;BatchOwnerTask*dBatchOwner=nullptr;BatchCrossTask*dBatchCross=nullptr,*dBatchInv=nullptr;size_t capBatchCfg=0,capBatchIo=0,capBatchOwner=0,capBatchCross=0,capBatchInv=0;
    double active=0,gather_s=0,transition_s=0,scatter_s=0,pipeline_graph_s=0;uint64_t groups=0,local_batches=0,stolen_batches=0,sticky_migrations=0,meta_upload_batches=0,meta_upload_bytes=0,meta_reclaim_batches=0,meta_reclaim_bytes=0,graph_instantiates=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
    ...
    template<class T> void ensure_batch(T*&ptr,size_t&cap,size_t need,const char*w){if(need<=cap)return;if(ptr)cudaFree(ptr);cap=std::max(need,cap?cap*2:size_t(256));ck(cudaMalloc(&ptr,cap*sizeof(T)),w);}
    void destroy(){if(dev<0)return;cudaSetDevice(dev);if(arena)cudaFree(arena);if(dIM)cudaFree(dIM);if(dID)cudaFree(dID);if(dBatchCfg)cudaFree(dBatchCfg);if(dBatchIo)cudaFree(dBatchIo);if(dBatchOwner)cudaFree(dBatchOwner);if(dBatchCross)cudaFree(dBatchCross);if(dBatchInv)cudaFree(dBatchInv);if(copyDone)cudaEventDestroy(copyDone);if(clearDone)cudaEventDestroy(clearDone);if(mainDone)cudaEventDestroy(mainDone);if(blockDone)cudaEventDestroy(blockDone);if(sMain)cudaStreamDestroy(sMain);if(sBlock)cudaStreamDestroy(sBlock);}
};
...
struct BatchDeviceMeta{
    BatchFactorIoCfg*cfg=nullptr;BatchFactorIoTask*io=nullptr;BatchOwnerTask*owner=nullptr;BatchCrossTask*cross=nullptr,*inv=nullptr;BatchP1DestTask*p1=nullptr;BatchCrossDestTask*cross_dest=nullptr;cudaGraphExec_t graph=nullptr;bool ready=false;
};
struct PreparedBatchHost{
    Code used=0,work=0;bool fix_low=false;
    std::vector<BatchFactorIoCfg> cfg;
    std::vector<BatchFactorIoTask> io;
    std::vector<BatchOwnerTask> owner;
    std::vector<BatchCrossTask> cross,inv;
    std::vector<BatchP1DestTask> p1;
    std::vector<BatchCrossDestTask> cross_dest;
    std::array<size_t,MAXW+1> cross_off{},cross_count{},cross_dest_off{},cross_dest_count{},inv_off{},inv_count{};
    mutable BatchDeviceMeta device[MAXGPU]{};
};
struct PreparedWindow{
    WindowPlan wp;
    std::vector<PreparedGroup> groups;
    std::vector<PreparedBatchHost> batches;
};
...
static void prepare_window_batches(int W,PreparedWindow&pw,size_t target){
    constexpr Code ALIGN=64;auto al=[](Code x){return (x+ALIGN-1)&~Code(ALIGN-1);};pw.batches.clear();
    size_t q0=0;while(q0<pw.groups.size()){
        PreparedBatchHost b;Code used=0;size_t q=q0;bool fixLow=pw.wp.p_hi>LOW_LUT_K;
        for(;q<pw.groups.size();++q){auto const&pg=pw.groups[q];if(!pg.ms.size&&!pg.ds.size)continue;Code need=al(pg.ms.size)+al(pg.ds.size);uint64_t nextBytes=uint64_t(used+need)*sizeof(Count),oneBytes=uint64_t(need)*sizeof(Count);if(!b.cfg.empty()&&nextBytes>target)break;if(oneBytes>target)throw std::runtime_error("groupbatch single group exceeds target");
            b.work+=pg.work;uint32_t fmask=fixLow?(pg.mo&((1u<<LOW_LUT_K)-1u)):((pg.mo>>(LOW_LUT_K+1))&((1u<<HIGH_LUT_K)-1u));auto fmb=make_factor_main_blocks(fixLow,fmask),fdb=make_factor_block_blocks(fixLow,fmask);if(fmb.back().end!=pg.ms.size||fdb.back().end!=pg.ds.size)throw std::runtime_error("groupbatch factor size mismatch");BatchFactorIoCfg io{};std::memcpy(io.mainb,fmb.data(),fmb.size()*sizeof(FBlock));std::memcpy(io.blockb,fdb.data(),fdb.size()*sizeof(FBlock));io.main_n=pg.ms.size;io.block_n=pg.ds.size;io.main_off=used;io.block_off=used+al(pg.ms.size);io.mask=fmask;io.main_nb=fmb.size();io.block_nb=fdb.size();io.fix_low=fixLow?1:0;b.cfg.push_back(io);used+=need;}
        if(b.cfg.empty()){q0=q+1;continue;}b.used=used;b.fix_low=fixLow;
        for(uint32_t gi=0;gi<b.cfg.size();++gi){auto const&io=b.cfg[gi];for(Code z=0;z<io.main_n;z+=GROUPBATCH_CHUNK)b.io.push_back({gi,0,z});for(Code z=0;z<io.block_n;z+=GROUPBATCH_CHUNK)b.io.push_back({gi,1,z});for(uint32_t bid=0;bid<io.block_nb;++bid){auto bx=io.blockb[bid];Code len=bx.end-bx.off;for(Code z=0;z<len;z+=GROUPBATCH_CHUNK)b.owner.push_back({gi,bid,z});}}
        for(int pp=pw.wp.p_hi;pp>=pw.wp.p_lo;--pp){b.cross_off[pp]=b.cross.size();b.cross_dest_off[pp]=b.cross_dest.size();b.inv_off[pp]=b.inv.size();for(uint32_t gi=0;gi<b.cfg.size();++gi){auto const&io=b.cfg[gi];for(uint32_t bid=0;bid<io.main_nb;++bid){auto x=io.mainb[bid];if(!x.stride||x.end<=x.off)continue;if(fixLow){auto hkey=high_pr_key(pp,x.he,x.c);auto sel=G_HPR.crossll_sel[hkey];if(groupbatch_cross_dest_mode()){for(int depth=1;depth<=LOW_LUT_K;++depth){auto cs=cross_record_find(true,hkey,depth),fs=cross_dest_find(true,io.mask,x.hs,depth);if(!cs.count||!fs.count)continue;Code total=Code(cs.count)*fs.count;for(Code z=0;z<total;z+=GROUPBATCH_CHUNK)b.cross_dest.push_back({gi,bid,cs.off,cs.count,fs.off,fs.count,z});}}else{Code total=Code(x.stride)*sel.count;for(Code z=0;z<total;z+=GROUPBATCH_CHUNK)b.cross.push_back({gi,bid,sel.off,sel.count,z});}}else{auto key=sparse_key(pp,x.hs,x.c);auto sel=G_PRERANK.highrr_sel[key];Code rows=(x.end-x.off)/x.stride;if(groupbatch_cross_dest_mode()&&pp>1){for(int depth=1;depth<=HIGH_LUT_K;++depth){auto cs=cross_record_find(false,key,depth),fs=cross_dest_find(false,io.mask,x.he,depth);if(!cs.count||!fs.count)continue;Code total=Code(cs.count)*fs.count;for(Code z=0;z<total;z+=GROUPBATCH_CHUNK)b.cross_dest.push_back({gi,bid,cs.off,cs.count,fs.off,fs.count,z});}}else{Code total=rows*Code(sel.count);for(Code z=0;z<total;z+=GROUPBATCH_CHUNK)b.cross.push_back({gi,bid,sel.off,sel.count,z});}if(pp==1&&!groupbatch_p1_dest_mode()){auto iv=G_PRERANK.closure_inv_sel[key];Code itotal=rows*Code(iv.count);for(Code z=0;z<itotal;z+=GROUPBATCH_CHUNK)b.inv.push_back({gi,bid,iv.off,iv.count,z});}}}}b.cross_count[pp]=b.cross.size()-b.cross_off[pp];b.cross_dest_count[pp]=b.cross_dest.size()-b.cross_dest_off[pp];b.inv_count[pp]=b.inv.size()-b.inv_off[pp];}
        if(!fixLow&&pw.wp.p_lo<=1&&pw.wp.p_hi>=1&&groupbatch_p1_dest_mode()){for(uint32_t gi=0;gi<b.cfg.size();++gi){auto const&io=b.cfg[gi];for(int he=0;he<=HIGH_LUT_K+1;++he){uint32_t ro=G_P1_DEST.off[he],rc=G_P1_DEST.off[he+1]-ro;if(!rc)continue;uint32_t rows=factor_count(G_FACTOR.high_mask_off,io.mask,he);Code total=Code(rows)*rc;for(Code z=0;z<total;z+=GROUPBATCH_CHUNK)b.p1.push_back({gi,ro,rc,uint32_t(he),z});}}}
        pw.batches.push_back(std::move(b));q0=q;
    }
}

struct GroupBatchAffinity{
    std::vector<std::vector<size_t>> home;
    std::vector<size_t> steal_order;
    std::vector<uint64_t> load;
    std::vector<int8_t> owner;
};

static GroupBatchAffinity make_groupbatch_affinity(const PreparedWindow&pw,int ng){
    GroupBatchAffinity a;a.home.resize(ng);a.load.assign(ng,0);a.steal_order.resize(pw.batches.size());a.owner.assign(pw.batches.size(),int8_t(-1));
    std::iota(a.steal_order.begin(),a.steal_order.end(),size_t(0));
    std::sort(a.steal_order.begin(),a.steal_order.end(),[&](size_t x,size_t y){return pw.batches[x].work>pw.batches[y].work;});
    for(size_t bi:a.steal_order){int best=0;for(int d=1;d<ng;++d)if(a.load[d]<a.load[best])best=d;a.home[best].push_back(bi);a.load[best]+=pw.batches[bi].work;a.owner[bi]=int8_t(best);}
    return a;
}

struct GroupBatchStats{
    uint64_t used_bytes=0,max_batch_bytes=0,max_total_bytes=0,meta_bytes=0;
    uint64_t cfg=0,io=0,owner=0,cross=0,cross_dest=0,inv=0,p1=0;
};

static std::pair<unsigned long long,unsigned long long> groupbatch_cross_work(const PreparedWindow&pw){
    unsigned long long src=0,dst=0;bool fixLow=pw.wp.p_hi>LOW_LUT_K;
    for(auto const&b:pw.batches)for(auto const&io:b.cfg)for(int pp=pw.wp.p_hi;pp>=pw.wp.p_lo;--pp)for(uint32_t bid=0;bid<uint32_t(io.main_nb);++bid){auto x=io.mainb[bid];if(!x.stride||x.end<=x.off)continue;
        if(fixLow){auto sel=G_HPR.crossll_sel[high_pr_key(pp,x.he,x.c)];if(!sel.count)continue;uint64_t slc=x.stride;int dh=int(x.hs)-2;if(dh<0||dh>=io.block_nb)continue;uint64_t dlc=io.blockb[dh].stride;src+=slc*sel.count;dst+=dlc*sel.count;}
        else{if(pp==1)continue;auto sel=G_PRERANK.highrr_sel[sparse_key(pp,x.hs,x.c)];if(!sel.count)continue;uint64_t shr=(x.end-x.off)/x.stride;int dh=int(x.he)-2;if(dh<0||dh>=io.block_nb)continue;auto bx=io.blockb[dh];uint64_t dhr=bx.stride?((bx.end-bx.off)/bx.stride):0;src+=shr*sel.count;dst+=dhr*sel.count;}
    }
    return {src,dst};
}

static GroupBatchStats groupbatch_stats(const PreparedWindow&pw){
    GroupBatchStats s;
    for(auto const&b:pw.batches){
        uint64_t ub=uint64_t(b.used)*sizeof(Count);s.used_bytes+=ub;s.max_batch_bytes=std::max(s.max_batch_bytes,ub);s.max_total_bytes=std::max(s.max_total_bytes,ub);
        s.cfg+=b.cfg.size();s.io+=b.io.size();s.owner+=b.owner.size();s.cross+=b.cross.size();s.cross_dest+=b.cross_dest.size();s.inv+=b.inv.size();s.p1+=b.p1.size();
        s.meta_bytes+=uint64_t(b.cfg.size())*sizeof(BatchFactorIoCfg)+uint64_t(b.io.size())*sizeof(BatchFactorIoTask)+uint64_t(b.owner.size())*sizeof(BatchOwnerTask)+uint64_t(b.cross.size()+b.inv.size())*sizeof(BatchCrossTask)+uint64_t(b.p1.size())*sizeof(BatchP1DestTask)+uint64_t(b.cross_dest.size())*sizeof(BatchCrossDestTask);
    }
    return s;
}

static size_t groupbatch_target_for_window(const PreparedWindow&pw,size_t hardTarget,int ng,bool dynamicMode){
    if(!dynamicMode||ng<=1)return hardTarget;
    int batchesPerGpu=6;
    if(const char*e=std::getenv("GRIDFP_GROUPBATCH_BATCHES_PER_GPU"))batchesPerGpu=std::max(0,std::atoi(e));
    if(batchesPerGpu<=0)return hardTarget;
    constexpr Code ALIGN=64;auto al=[](Code x){return(x+ALIGN-1)&~Code(ALIGN-1);};
    uint64_t total=0,maxGroup=0;
    for(auto const&pg:pw.groups){uint64_t b=uint64_t(al(pg.ms.size)+al(pg.ds.size))*sizeof(Count);total+=b;maxGroup=std::max(maxGroup,b);}
    uint64_t wanted=uint64_t(ng)*uint64_t(batchesPerGpu);
    uint64_t fair=wanted?((total+wanted-1)/wanted):uint64_t(hardTarget);
    uint64_t tuned=std::max(maxGroup,std::min<uint64_t>(hardTarget,fair));
    return size_t(std::min<uint64_t>(tuned,hardTarget));
}

static void reserve_groupbatch_buffers(DeviceCtx&c,const PreparedWindow&pw){
    ck(cudaSetDevice(c.dev),"groupbatch reserve set");size_t arenaBytes=0;for(auto const&b:pw.batches)arenaBytes=std::max(arenaBytes,size_t(b.used)*sizeof(Count));if(arenaBytes>c.capArena){if(c.arena)cudaFree(c.arena);c.capArena=arenaBytes;ck(cudaMalloc(&c.arena,c.capArena),"groupbatch reserve arena");}
}

static void release_groupbatch_arena(DeviceCtx&c){
    ck(cudaSetDevice(c.dev),"groupbatch release arena set");if(c.arena)ck(cudaFree(c.arena),"groupbatch release arena");c.arena=nullptr;c.capArena=0;c.dA=c.dB=c.dD=c.dE=nullptr;c.dMate=nullptr;
}

static uint64_t groupbatch_device_meta_bytes(const PreparedBatchHost&b){
    return uint64_t(b.cfg.size())*sizeof(BatchFactorIoCfg)+uint64_t(b.io.size())*sizeof(BatchFactorIoTask)+uint64_t(b.owner.size())*sizeof(BatchOwnerTask)+uint64_t(b.cross.size()+b.inv.size())*sizeof(BatchCrossTask)+uint64_t(b.p1.size())*sizeof(BatchP1DestTask)+uint64_t(b.cross_dest.size())*sizeof(BatchCrossDestTask);
}

static bool destroy_groupbatch_device_meta_one(PreparedBatchHost&b,int d){
    auto&x=b.device[d];if(!x.ready&&!x.graph)return false;ck(cudaSetDevice(d),"groupbatch meta one destroy set");if(x.graph)ck(cudaGraphExecDestroy(x.graph),"groupbatch graph one destroy exec");if(x.cfg)ck(cudaFree(x.cfg),"groupbatch cfg one free");if(x.io)ck(cudaFree(x.io),"groupbatch io one free");if(x.owner)ck(cudaFree(x.owner),"groupbatch owner one free");if(x.cross)ck(cudaFree(x.cross),"groupbatch cross one free");if(x.inv)ck(cudaFree(x.inv),"groupbatch inv one free");if(x.p1)ck(cudaFree(x.p1),"groupbatch p1 one free");if(x.cross_dest)ck(cudaFree(x.cross_dest),"groupbatch cross dest one free");x=BatchDeviceMeta{};return true;
}

static void invalidate_groupbatch_graphs(PreparedWindow&pw,int ng){
    for(auto&b:pw.batches)for(int d=0;d<ng;++d){auto&x=b.device[d];if(!x.graph)continue;ck(cudaSetDevice(d),"groupbatch graph invalidate set");ck(cudaGraphExecDestroy(x.graph),"groupbatch graph invalidate");x.graph=nullptr;}
}

static void destroy_groupbatch_device_meta(PreparedWindow&pw,int ng){
    for(auto&b:pw.batches)for(int d=0;d<ng;++d)destroy_groupbatch_device_meta_one(b,d);
}

static int groupbatch_graph_mode(){
    static int mode=[](){int v=1;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_GRAPH"))v=std::atoi(e);if(v<0||v>2)throw std::runtime_error("GRIDFP_GROUPBATCH_GRAPH must be 0,1,2");return v;}();
    return mode;
}

static void process_window_batched_io(DeviceCtx&c,int W,const PreparedWindow&pw,int threads,size_t target,size_t begin_batch=0,size_t end_batch=~size_t(0)){
    ck(cudaSetDevice(c.dev),"batchio set");(void)W;(void)target;
    size_t bend=std::min(end_batch,pw.batches.size());
    for(size_t bi=begin_batch;bi<bend;++bi){
        auto const&b=pw.batches[bi];size_t needBytes=size_t(b.used)*sizeof(Count);
        if(needBytes>c.capArena){if(c.arena)cudaFree(c.arena);c.capArena=needBytes;ck(cudaMalloc(&c.arena,c.capArena),"batchio arena");}
        auto&dm=b.device[c.dev];
        if(!dm.ready){
            auto upload=[&](auto*&dst,auto const&v,const char*w){using T=typename std::decay_t<decltype(v)>::value_type;if(v.empty())return;ck(cudaMalloc(&dst,v.size()*sizeof(T)),w);ck(cudaMemcpy(dst,v.data(),v.size()*sizeof(T),cudaMemcpyHostToDevice),w);};
            upload(dm.cfg,b.cfg,"groupbatch cfg upload");upload(dm.io,b.io,"groupbatch io upload");upload(dm.owner,b.owner,"groupbatch owner upload");upload(dm.cross,b.cross,"groupbatch cross upload");upload(dm.inv,b.inv,"groupbatch inv upload");upload(dm.p1,b.p1,"groupbatch p1 upload");upload(dm.cross_dest,b.cross_dest,"groupbatch cross dest upload");dm.ready=true;c.meta_upload_batches++;c.meta_upload_bytes+=groupbatch_device_meta_bytes(b);
        }
        BatchFactorIoCfg*dcfg=dm.cfg;BatchFactorIoTask*dt=dm.io;BatchOwnerTask*dot=dm.owner;BatchCrossTask*dct=dm.cross,*dit=dm.inv;BatchP1DestTask*dpt=dm.p1;BatchCrossDestTask*dcet=dm.cross_dest;Count*arena=reinterpret_cast<Count*>(c.arena);bool wrap32=G_GROUPBATCH_WRAP32_ACTIVE;
        auto enqueue_gather=[&]{batch_factor_io_kernel<false,false><<<std::max<size_t>(1,b.io.size()),threads,0,c.sMain>>>(arena,dcfg,dt,b.io.size());};
        auto enqueue_transition=[&]{
            for(int pp=pw.wp.p_hi;pp>=pw.wp.p_lo;--pp){
                if(!b.owner.empty()){if(b.fix_low){if(wrap32)batch_high_owner_kernel<true><<<b.owner.size(),threads,0,c.sMain>>>(arena,dcfg,dot,b.owner.size(),pp);else batch_high_owner_kernel<false><<<b.owner.size(),threads,0,c.sMain>>>(arena,dcfg,dot,b.owner.size(),pp);}else{if(wrap32)batch_low_owner_kernel<true><<<b.owner.size(),threads,0,c.sMain>>>(arena,dcfg,dot,b.owner.size(),pp);else batch_low_owner_kernel<false><<<b.owner.size(),threads,0,c.sMain>>>(arena,dcfg,dot,b.owner.size(),pp);}}
                if(!b.fix_low&&pp==1){if(groupbatch_p1_dest_mode()){if(!b.p1.empty())batch_low_p1_dest_kernel<<<b.p1.size(),threads,0,c.sMain>>>(arena,dcfg,dpt,b.p1.size());}else if(b.inv_count[pp])batch_low_inv_p1_kernel<<<b.inv_count[pp],threads,0,c.sMain>>>(arena,dcfg,dit+b.inv_off[pp],b.inv_count[pp]);}
                if(b.cross_dest_count[pp]){if(b.fix_low)batch_high_crossll_dest_kernel<<<b.cross_dest_count[pp],threads,0,c.sMain>>>(arena,dcfg,dcet+b.cross_dest_off[pp],b.cross_dest_count[pp]);else batch_low_highrr_dest_kernel<<<b.cross_dest_count[pp],threads,0,c.sMain>>>(arena,dcfg,dcet+b.cross_dest_off[pp],b.cross_dest_count[pp]);}
        ...
        auto enqueue_scatter=[&]{if(wrap32&&pw.wp.p_lo>1)batch_factor_io_kernel<true,true><<<std::max<size_t>(1,b.io.size()),threads,0,c.sMain>>>(arena,dcfg,dt,b.io.size());else batch_factor_io_kernel<true,false><<<std::max<size_t>(1,b.io.size()),threads,0,c.sMain>>>(arena,dcfg,dt,b.io.size());};
        int graphMode=groupbatch_graph_mode();auto t0=std::chrono::steady_clock::now();
        if(graphMode==2){
            if(dm.graph)ck(cudaGraphLaunch(dm.graph,c.sMain),"groupbatch pipeline graph launch");
            else{
                cudaGraph_t graph=nullptr;ck(cudaStreamBeginCapture(c.sMain,cudaStreamCaptureModeThreadLocal),"groupbatch pipeline graph begin");enqueue_gather();enqueue_transition();enqueue_scatter();ck(cudaStreamEndCapture(c.sMain,&graph),"groupbatch pipeline graph end");ck(cudaGraphInstantiate(&dm.graph,graph,0),"groupbatch pipeline graph instantiate");c.graph_instantiates++;ck(cudaGraphDestroy(graph),"groupbatch pipeline graph destroy");ck(cudaGraphLaunch(dm.graph,c.sMain),"groupbatch pipeline graph first launch");
            }
            ck(cudaGetLastError(),"groupbatch pipeline graph launch check");ck(cudaStreamSynchronize(c.sMain),"groupbatch pipeline graph sync");auto ts=std::chrono::steady_clock::now();double sec=std::chrono::duration<double>(ts-t0).count();c.pipeline_graph_s+=sec;c.active+=sec;
        }else{
            enqueue_gather();ck(cudaGetLastError(),"batchio gather");ck(cudaStreamSynchronize(c.sMain),"batchio gather sync");auto tg=std::chrono::steady_clock::now();
            if(graphMode==1){
                if(dm.graph)ck(cudaGraphLaunch(dm.graph,c.sMain),"groupbatch graph launch");
                else{cudaGraph_t graph=nullptr;ck(cudaStreamBeginCapture(c.sMain,cudaStreamCaptureModeThreadLocal),"groupbatch graph begin");enqueue_transition();ck(cudaStreamEndCapture(c.sMain,&graph),"groupbatch graph end");ck(cudaGraphInstantiate(&dm.graph,graph,0),"groupbatch graph instantiate");c.graph_instantiates++;ck(cudaGraphDestroy(graph),"groupbatch graph destroy");ck(cudaGraphLaunch(dm.graph,c.sMain),"groupbatch graph first launch");}
            }else enqueue_transition();
            ck(cudaGetLastError(),"groupbatch transition launch");ck(cudaStreamSynchronize(c.sMain),"batchio transition batch sync");auto tt=std::chrono::steady_clock::now();
            enqueue_scatter();ck(cudaGetLastError(),"batchio scatter");ck(cudaStreamSynchronize(c.sMain),"batchio scatter sync");auto ts=std::chrono::steady_clock::now();
            c.gather_s+=std::chrono::duration<double>(tg-t0).count();c.transition_s+=std::chrono::duration<double>(tt-tg).count();c.scatter_s+=std::chrono::duration<double>(ts-tt).count();c.active+=std::chrono::duration<double>(ts-t0).count();
        }
        c.groups+=b.cfg.size();
    }
}

static void process_window_dynamic_affinity(std::vector<DeviceCtx>&ctx,int W,PreparedWindow&pw,GroupBatchAffinity&aff,int threads,size_t target,bool sticky,bool reclaim){
    const int ng=int(ctx.size());size_t nb=pw.batches.size();
    std::unique_ptr<std::atomic<uint8_t>[]>claimed(new std::atomic<uint8_t>[nb]);for(size_t bi=0;bi<nb;++bi)claimed[bi].store(0,std::memory_order_relaxed);
    std::vector<int8_t>executed(nb,int8_t(-1));std::atomic<size_t>stealPos{0};std::vector<std::thread>ths;ths.reserve(ng);
    for(int d=0;d<ng;++d)ths.emplace_back([&,d]{
        for(size_t bi:aff.home[d]){uint8_t expected=0;if(claimed[bi].compare_exchange_strong(expected,1,std::memory_order_relaxed)){ctx[d].local_batches++;process_window_batched_io(ctx[d],W,pw,threads,target,bi,bi+1);executed[bi]=int8_t(d);}}
        for(;;){size_t z=stealPos.fetch_add(1,std::memory_order_relaxed);if(z>=aff.steal_order.size())break;size_t bi=aff.steal_order[z];uint8_t expected=0;if(claimed[bi].compare_exchange_strong(expected,1,std::memory_order_relaxed)){ctx[d].stolen_batches++;process_window_batched_io(ctx[d],W,pw,threads,target,bi,bi+1);executed[bi]=int8_t(d);}}
    });
    for(auto&t:ths)t.join();
    if(!sticky)return;
    std::vector<std::vector<size_t>>next(ng);std::vector<uint64_t>load(ng,0);
    for(size_t bi:aff.steal_order){int d=executed[bi];if(d<0||d>=ng)throw std::runtime_error("groupbatch affinity batch was not executed");int old=aff.owner[bi];if(old!=d){ctx[d].sticky_migrations++;if(reclaim&&old>=0&&destroy_groupbatch_device_meta_one(pw.batches[bi],old)){ctx[d].meta_reclaim_batches++;ctx[d].meta_reclaim_bytes+=groupbatch_device_meta_bytes(pw.batches[bi]);}aff.owner[bi]=int8_t(d);}next[d].push_back(bi);load[d]+=pw.batches[bi].work;}
    aff.home.swap(next);aff.load.swap(load);
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(ms.size){if(pg.use_mi)interval_io_kernel<false,false><<<interval_blocks(pg.mi.size(),threads),threads,0,c.sMain>>>(c.dA,c.dIM,pg.mi.size());else gather_main_kernel<<<bm,threads,0,c.sMain>>>(c.dA,useMate?c.dMate:nullptr,ms.size);}
    if(ds.size){if(pg.use_di)interval_io_kernel<true,false><<<interval_blocks(pg.di.size(),threads),threads,0,c.sBlock>>>(c.dD,c.dID,pg.di.size());else gather_block_kernel<<<bd,threads,0,c.sBlock>>>(c.dD,ds.size);}
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){auto launchN=[&](Code n,auto fn,const char*w){if(!n)return;int nb=int(std::min<Code>(65535,(n+threads-1)/threads));fn(nb);ck(cudaGetLastError(),w);};
            if(fixLow){auto xm=make_high_prerank_map(fmb,p,2);Code on=ds.size,xn=xm.nblocks?xm.end[xm.nblocks-1]:0;launchN(on,[&](int nb){factor_high_prerank_owner_blockorder_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,on,p);},"factor high prerank owner blockfused");if(xn){HighPreRankMap empty{};launchN(xn,[&](int nb){factor_high_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,empty,xm);},"factor high prerank cross closure");}}
            else{Code on=ds.size;launchN(on,[&](int nb){factor_prerank_owner_blockorder_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,on,p);},"factor prerank owner blockfused");auto sm=make_prerank_orbit_map(fmb,p,2);Code sn=sm.nblocks?sm.end[sm.nblocks-1]:0;if(p==1){auto im=make_prerank_orbit_map(fmb,p,3);Code in=im.nblocks?im.end[im.nblocks-1]:0;launchN(in+sn,[&](int nb){factor_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,im,sm,p);},"factor prerank p1 closure");}else launchN(sn,[&](int nb){factor_prerank_highrr_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,sm,p);},"factor prerank cross closure");}
        }
    ...
    if(ms.size){if(pg.use_mi)interval_io_kernel<false,true><<<interval_blocks(pg.mi.size(),threads),threads,0,c.sMain>>>(cur,c.dIM,pg.mi.size());else scatter_main_kernel<<<bm,threads,0,c.sMain>>>(cur,ms.size);}
    if(ds.size){if(pg.use_di)interval_io_kernel<true,true><<<interval_blocks(pg.di.size(),threads),threads,0,c.sBlock>>>(dcur,c.dID,pg.di.size());else scatter_block_kernel<<<bd,threads,0,c.sBlock>>>(dcur,ds.size);}
...
struct Row6CartLists{std::array<std::vector<uint32_t>,7> pc,sc;std::array<std::vector<Code>,7> pr,sr;};
static Row6CartLists make_row6_cart_lists(int W){
    constexpr int lo=TARGET_W/2,hi=TARGET_W-lo;Row6CartLists z;size_t pn=1,sn=1;for(int i=0;i<hi;++i)pn*=3;for(int i=0;i<lo;++i)sn*=3;
    auto digits=[](size_t code,int len,int*d){for(int i=len-1;i>=0;--i){d[i]=int(code%3);code/=3;}};
    for(size_t c=0;c<pn;++c){int d[32];digits(c,hi,d);int h=1;Code gr=0;bool ok=true;for(int k=0;k<hi;++k){int pos=W-1-k,a=d[k];if(a>0)gr+=H_DP[pos][h];if(a>1&&h>0)gr+=H_DP[pos][h-1];if(a==1)--h;else if(a==2)++h;if(h<0||h>6){ok=false;break;}}if(ok){z.pc[h].push_back(uint32_t(c));z.pr[h].push_back(gr);}}
    for(size_t c=0;c<sn;++c){int d[32];digits(c,lo,d);int h=0;bool ok=true;for(int k=lo-1;k>=0;--k){int a=d[k];if(a==1)++h;else if(a==2)--h;if(h<0||h>6){ok=false;break;}}if(!ok)continue;int hs=h;Code gr=0;h=hs;for(int k=0;k<lo;++k){int pos=lo-1-k,a=d[k];if(a>0)gr+=H_DP[pos][h];if(a>1&&h>0)gr+=H_DP[pos][h-1];if(a==1)--h;else if(a==2)++h;if(h<0||h>6){ok=false;break;}}if(ok&&h==0){z.sc[hs].push_back(uint32_t(c));z.sr[hs].push_back(gr);}}
    return z;
}
...
static void init_direct_row6_lut_full_multi_gpu(int W,Count mod,int threads,Count**fullMain,const std::vector<Code>&mainLen,Code mainChunk,int ng){
    ...
    for(int dev=0;dev<ng;++dev)workers.emplace_back([&,dev]{ck(cudaSetDevice(dev),"r6 levelpack set");ck(cudaMemcpyToSymbol(D_BOUND_DP,dp,sizeof(dp)),"r6 levelpack dp");int cap=6;ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"r6 levelpack cap");ck(cudaMemcpyToSymbol(D_R6_GPU_MAT_OFF,moff,sizeof(moff)),"r6 levelpack mat off");Count*dmat=nullptr,*dbeta=nullptr;uint32_t*dpidx=nullptr,*dsidx=nullptr;ck(cudaMalloc(&dmat,mat.size()*4),"r6 levelpack mat");ck(cudaMalloc(&dbeta,beta.size()*4),"r6 levelpack beta");ck(cudaMemcpy(dmat,mat.data(),mat.size()*4,cudaMemcpyHostToDevice),"r6 levelpack mat cp");ck(cudaMemcpy(dbeta,beta.data(),beta.size()*4,cudaMemcpyHostToDevice),"r6 levelpack beta cp");auto tb=std::chrono::steady_clock::now();
        ...
        auto PP=build_pref();Count*P=PP.first;dpidx=PP.second;auto SS=build_suff();Count*S=SS.first;dsidx=SS.second;ck(cudaDeviceSynchronize(),"r6 levelpack tables");table_s[dev]=std::chrono::duration<double>(std::chrono::steady_clock::now()-tb).count();ck(cudaMemcpyToSymbol(D_R6_PREF,&P,sizeof(P)),"r6 levelpack pref ptr");ck(cudaMemcpyToSymbol(D_R6_SUFF,&S,sizeof(S)),"r6 levelpack suff ptr");ck(cudaMemcpyToSymbol(D_R6_PREF_IDX,&dpidx,sizeof(dpidx)),"r6 levelpack pidx ptr");ck(cudaMemcpyToSymbol(D_R6_SUFF_IDX,&dsidx,sizeof(dsidx)),"r6 levelpack sidx ptr");Code cnt=cut[dev+1]-cut[dev],base=Code(dev)*mainChunk;int blocks=lanes?65535:int(std::min<Code>(65535,(cnt+threads-1)/threads));auto ib=std::chrono::steady_clock::now();bool cartesian=true;if(const char*e=std::getenv("GRIDFP_ROW6_CARTESIAN"))cartesian=std::atoi(e)!=0;if(cnt&&cartesian){
            for(int h=0;h<=6;++h){Code np=cart.pc[h].size(),ns=cart.sc[h].size();if(!np||!ns)continue;uint32_t*dpc=nullptr,*dsc=nullptr;Code*dpr=nullptr,*dsr=nullptr;ck(cudaMalloc(&dpc,np*4),"r6 cart pc");ck(cudaMalloc(&dsc,ns*4),"r6 cart sc");ck(cudaMalloc(&dpr,np*sizeof(Code)),"r6 cart pr");ck(cudaMalloc(&dsr,ns*sizeof(Code)),"r6 cart sr");ck(cudaMemcpy(dpc,cart.pc[h].data(),np*4,cudaMemcpyHostToDevice),"r6 cart pc cp");ck(cudaMemcpy(dsc,cart.sc[h].data(),ns*4,cudaMemcpyHostToDevice),"r6 cart sc cp");ck(cudaMemcpy(dpr,cart.pr[h].data(),np*sizeof(Code),cudaMemcpyHostToDevice),"r6 cart pr cp");ck(cudaMemcpy(dsr,cart.sr[h].data(),ns*sizeof(Code),cudaMemcpyHostToDevice),"r6 cart sr cp");bool useGemm=false;if(const char*e=std::getenv("GRIDFP_ROW6_GEMM"))useGemm=std::atoi(e)!=0;bool useTasks=true;if(const char*e=std::getenv("GRIDFP_ROW6_TASKS"))useTasks=std::atoi(e)!=0;if(useGemm){dim3 gd((unsigned)((np+7)/8),(unsigned)((ns+7)/8));row6_cartesian_gemm_kernel<8,4><<<gd,256>>>(fullMain[dev],base,base+mainLen[dev],dpc,dpr,np,dsc,dsr,ns,r6_block_size(h));ck(cudaGetLastError(),"r6 gemm launch");cartPairCount[dev]+=uint64_t(np)*ns;}else if(useTasks){std::vector<Row6CartTask> ht;ht.reserve(np);Code fend=base+mainLen[dev];auto const&sv=cart.sr[h];for(uint32_t ip=0;ip<np;++ip){Code rp=cart.pr[h][ip];if(rp>=fend)break;auto lb=(base<=rp)?sv.begin():std::lower_bound(sv.begin(),sv.end(),base-rp);auto ub=(fend<=rp)?sv.begin():std::lower_bound(sv.begin(),sv.end(),fend-rp);uint32_t a=uint32_t(lb-sv.begin()),b=uint32_t(ub-sv.begin());if(a<b){if(!(base<=rp+sv[a]&&rp+sv[b-1]<fend))throw std::runtime_error("row6 Cartesian owner interval mismatch");cartPairCount[dev]+=uint64_t(b-a);for(uint32_t x=a;x<b;x+=taskChunk){ht.push_back({ip,x,std::min<uint32_t>(b,x+taskChunk)});++cartTaskCount[dev];}}}Row6CartTask*dt=nullptr;if(!ht.empty()){ck(cudaMalloc(&dt,ht.size()*sizeof(Row6CartTask)),"r6 cart tasks");ck(cudaMemcpy(dt,ht.data(),ht.size()*sizeof(Row6CartTask),cudaMemcpyHostToDevice),"r6 cart tasks cp");int gb=int(ht.size());int hlanes=lanes;{char en[64];std::snprintf(en,sizeof(en),"GRIDFP_ROW6_LANES_H%d",h);if(const char*e=std::getenv(en))hlanes=std::atoi(e);}if(hlanes==16)row6_cartesian_task_kernel<16><<<gb,threads>>>(fullMain[dev],base,dpc,dpr,dsc,dsr,dt,ht.size(),r6_block_size(h));else if(hlanes==8)row6_cartesian_task_kernel<8><<<gb,threads>>>(fullMain[dev],base,dpc,dpr,dsc,dsr,dt,ht.size(),r6_block_size(h));else if(hlanes==2)row6_cartesian_task_kernel<2><<<gb,threads>>>(fullMain[dev],base,dpc,dpr,dsc,dsr,dt,ht.size(),r6_block_size(h));else if(hlanes==1)row6_cartesian_task_kernel<1><<<gb,threads>>>(fullMain[dev],base,dpc,dpr,dsc,dsr,dt,ht.size(),r6_block_size(h));else row6_cartesian_task_kernel<4><<<gb,threads>>>(fullMain[dev],base,dpc,dpr,dsc,dsr,dt,ht.size(),r6_block_size(h));ck(cudaGetLastError(),"r6 task launch");cudaFree(dt);}}else{int cb=65535;if(lanes==8)row6_cartesian_group_kernel<8><<<cb,threads>>>(fullMain[dev],base,dpc,dpr,np,dsc,dsr,ns,r6_block_size(h));else if(lanes==2)row6_cartesian_group_kernel<2><<<cb,threads>>>(fullMain[dev],base,dpc,dpr,np,dsc,dsr,ns,r6_block_size(h));else row6_cartesian_group_kernel<4><<<cb,threads>>>(fullMain[dev],base,dpc,dpr,np,dsc,dsr,ns,r6_block_size(h));ck(cudaGetLastError(),"r6 cart launch");}cudaFree(dpc);cudaFree(dsc);cudaFree(dpr);cudaFree(dsr);}
...
int main(int argc,char**argv){
    ...
    if(groupbatch_p1_dest_mode())G_P1_DEST=build_p1_dest_tables();
    if(groupbatch_cross_dest_mode())G_CROSS_DEST=build_cross_dest_tables();
    if(const char*e=std::getenv("GRIDFP_GROUPBATCH_P1_DIAG"))if(std::atoi(e)!=0)print_p1_inverse_merge_stats();
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t p1DestBytes=groupbatch_p1_dest_mode()?uint64_t(G_P1_DEST.rec.size())*8+uint64_t(G_P1_DEST.src.size())*4:0;uint64_t crossDestBytes=groupbatch_cross_dest_mode()?uint64_t(G_CROSS_DEST.low_rec.size()+G_CROSS_DEST.high_rec.size())*8+uint64_t(G_CROSS_DEST.low_src.size()+G_CROSS_DEST.high_src.size()+G_CROSS_DEST.low_cross_idx.size()+G_CROSS_DEST.high_cross_idx.size())*4:0;uint64_t sparseBytes=0;uint64_t preRankBytes=uint64_t(G_PRERANK.owner_block_rec.size()+G_PRERANK.highrr_rec.size()+G_PRERANK.closure_inv_rec.size()+G_HPR.owner_block_rec.size()+G_HPR.crossll_rec.size()+G_HPR.closure_inv_rec.size())*8+uint64_t(G_PRERANK.closure_inv_src.size()+G_HPR.closure_inv_src.size()+G_PRERANK.owner_block_closure_meta.size()+G_PRERANK.owner_block_closure_src.size()+G_HPR.owner_block_closure_meta.size()+G_HPR.owner_block_closure_src.size())*4;uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+vb4(G_FACTOR.low_mask_all_rank)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+vb4(G_FACTOR.high_mask_all_rank)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+sparseBytes+preRankBytes+p1DestBytes+crossDestBytes;
        ...
        uint64_t scratchLimit=uint64_t(std::max(1,target_mib))<<20;
        uint64_t minScratchBytes=0,actualScratchBytes=0,totalGroups=0,mateGroups=0;{const int rr[2][2]={{W-1,LOW_LUT_K+1},{LOW_LUT_K,1}};for(auto const&r:rr){auto fp=window_candidates(W,r[0],r[1]);int nj=1<<int(fp.size());for(int g=0;g<nj;++g){uint32_t mf,mo,bf,bo;window_masks(W,r[0],r[1],fp,uint32_t(g),mf,mo,bf,bo);auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);bool orbit=true;uint64_t countBytes=uint64_t(ms.size+ds.size)*sizeof(Count),mateBytes=uint64_t(ms.size)*sizeof(MateID);minScratchBytes=std::max(minScratchBytes,countBytes);bool useMate=!orbit&&countBytes+mateBytes<=scratchLimit;actualScratchBytes=std::max(actualScratchBytes,countBytes+(useMate?mateBytes:0));totalGroups++;mateGroups+=useMate;}}}
        bool scratchFits=minScratchBytes<=scratchLimit;
        bool planDynamic=png>1;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_DYNAMIC"))planDynamic=std::atoi(e)!=0;
        uint64_t totalMeta=0,totalBatches=0,totalCfg=0,totalIo=0,totalOwner=0,totalCross=0,totalCrossDest=0,totalInv=0,totalP1=0,maxBatch=0;
        if(scratchFits){
            const int gr[2][2]={{W-1,LOW_LUT_K+1},{LOW_LUT_K,1}};
            for(int wi=0;wi<2;++wi){
                WindowPlan wp;wp.p_hi=gr[wi][0];wp.p_lo=gr[wi][1];wp.fixed_pos=window_candidates(W,wp.p_hi,wp.p_lo);int nj=1<<int(wp.fixed_pos.size());PreparedWindow pw;pw.wp=wp;pw.groups.reserve(nj);
                for(int g=0;g<nj;++g)pw.groups.push_back(prepare_group(W,pw.wp,g,pch,pbch,png));
                std::sort(pw.groups.begin(),pw.groups.end(),[](auto const&a,auto const&b){return a.work>b.work;});size_t bt=groupbatch_target_for_window(pw,scratchLimit,png,planDynamic);prepare_window_batches(W,pw,bt);auto st=groupbatch_stats(pw);auto aff=make_groupbatch_affinity(pw,png);uint64_t affSum=0,affMax=0;for(auto x:aff.load){affSum+=x;affMax=std::max(affMax,x);}
                totalMeta+=st.meta_bytes;totalBatches+=pw.batches.size();totalCfg+=st.cfg;totalIo+=st.io;totalOwner+=st.owner;totalCross+=st.cross;totalCrossDest+=st.cross_dest;totalInv+=st.inv;totalP1+=st.p1;maxBatch=std::max(maxBatch,groupbatch_wrap32_mode()?st.max_total_bytes:st.max_batch_bytes);auto cw=groupbatch_cross_work(pw);
                std::cerr<<"PLAN groupbatch_window="<<wi<<" p="<<wp.p_hi<<".."<<wp.p_lo<<" groups="<<pw.groups.size()<<" batches="<<pw.batches.size()<<" batch_target_mib="<<double(bt)/(1<<20)<<" max_batch_gib="<<double(st.max_batch_bytes)/(1ull<<30)<<" wrap32_extra_mib=0 total_scratch_gib="<<double(st.max_total_bytes)/(1ull<<30)<<" meta_mib="<<double(st.meta_bytes)/(1<<20)<<" affinity_max_over_avg="<<(affSum?double(affMax)*png/double(affSum):0.0)<<" cfg="<<st.cfg<<" io="<<st.io<<" owner="<<st.owner<<" cross="<<st.cross<<" cross_dest="<<st.cross_dest<<" inv="<<st.inv<<" p1="<<st.p1<<" cross_src_work="<<cw.first<<" cross_dst_work="<<cw.second<<" cross_dst_over_src="<<(cw.first?double(cw.second)/double(cw.first):0.0)<<"\n";
            }
        }
        // Dynamic scheduling may eventually touch every batch on every GPU, so one full metadata copy per GPU is the conservative resident bound.
        uint64_t metaPerGpu=totalMeta;
        bool groupbatchFits=scratchFits&&maxBatch<=scratchLimit;
        uint64_t row6PeakNoMeta=authMaxBytes+factorBytes+row6Extra;
        uint64_t row6PeakWithMeta=row6PeakNoMeta+metaPerGpu;
        uint64_t dpPeak=authMaxBytes+factorBytes+maxBatch+metaPerGpu;
        uint64_t minDpPeak=authMaxBytes+factorBytes+minScratchBytes+metaPerGpu;
        std::cerr<<"PLAN n="<<n<<" W="<<W<<" gpus="<<png<<" main="<<pmain<<" blocked="<<pblock<<" auth_gib="<<double(pmain+pblock)*4.0/(1ull<<30)<<" auth_per_gpu_gib="<<double(authMaxBytes)/(1ull<<30)<<" factor_gpu_gib="<<double(factorBytes)/(1ull<<30)<<" sparse_orbit_mib="<<(sparseBytes>>20)<<" prerank_mib="<<(preRankBytes>>20)<<" vmm_gran_kib="<<(vmmGran>>10)<<" low_vmm_mib="<<(lowVmmBytes>>20)<<" high_vmm_mib="<<(highVmmBytes>>20)<<" row6_extra_gib="<<double(row6Extra)/(1ull<<30)<<" row6_peak_no_meta_gib="<<double(row6PeakNoMeta)/(1ull<<30)<<" row6_peak_gib="<<double(row6PeakWithMeta)/(1ull<<30)<<" scratch_limit_mib="<<target_mib<<" scratch_fits="<<(groupbatchFits?1:0)<<" count_scratch_gib="<<double(minScratchBytes)/(1ull<<30)<<" actual_scratch_gib="<<double(maxBatch)/(1ull<<30)<<" metadata_per_gpu_mib="<<double(metaPerGpu)/(1<<20)<<" mate_groups="<<mateGroups<<"/"<<totalGroups<<" dp_peak_gib="<<double(dpPeak)/(1ull<<30)<<" min_dp_peak_gib="<<double(minDpPeak)/(1ull<<30)<<" row6_states="<<r6n<<" pref_mib="<<(pvals*4>>20)<<" suff_mib="<<(svals*4>>20)<<" row6_task_chunk="<<tch<<" groupbatch_task_chunk="<<GROUPBATCH_CHUNK<<"\n";
        bool planAffinity=planDynamic;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_AFFINITY"))planAffinity=planDynamic&&std::atoi(e)!=0;bool planSticky=planAffinity;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_STICKY"))planSticky=planAffinity&&std::atoi(e)!=0;bool planReclaim=planSticky;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_RECLAIM"))planReclaim=planSticky&&std::atoi(e)!=0;
        std::cerr<<"PLAN groupbatch mode="<<(planDynamic?(planAffinity?(planSticky?(planReclaim?"dynamic-affinity-sticky-reclaim":"dynamic-affinity-sticky"):"dynamic-affinity"):"dynamic-batch"):"static-shard")<<" batches="<<totalBatches<<" max_batch_gib="<<double(maxBatch)/(1ull<<30)<<" meta_per_gpu_worst_mib="<<double(metaPerGpu)/(1<<20)<<" meta_all_gpus_worst_mib="<<double(metaPerGpu)*png/(1<<20)<<" cfg="<<totalCfg<<" io="<<totalIo<<" owner="<<totalOwner<<" cross="<<totalCross<<" cross_dest="<<totalCrossDest<<" inv="<<totalInv<<" p1="<<totalP1<<" p1_table_mib="<<double(p1DestBytes)/(1<<20)<<" cross_dest_table_mib="<<double(crossDestBytes)/(1<<20)<<"\n";
        uint64_t sumPairs=0,sumTasks=0;for(int d=0;d<png;++d){Code base=Code(d)*pch,end=std::min<Code>(pmain,base+pch);uint64_t npair=0,ntask=0,work=0;for(int h=0;h<=6;++h){auto const&sv=cart.sr[h];for(size_t ip=0;ip<cart.pr[h].size();++ip){Code rp=cart.pr[h][ip];if(rp>=end)break;auto lb=(base<=rp)?sv.begin():std::lower_bound(sv.begin(),sv.end(),base-rp);auto ub=(end<=rp)?sv.begin():std::lower_bound(sv.begin(),sv.end(),end-rp);uint64_t z=uint64_t(ub-lb);if(z){npair+=z;work+=z*uint64_t(r6_block_size(h));ntask+=(z+tch-1)/tch;}}}sumPairs+=npair;sumTasks+=ntask;std::cerr<<"PLAN gpu="<<d<<" pairs="<<npair<<" work="<<work<<" tasks="<<ntask<<" owner=["<<base<<","<<end<<")\n";}
    ...
    SparseVmmU32 fLDv[MAXGPU]{},fHRv[MAXGPU]{};uint32_t *fLA[MAXGPU]{},*fLM[MAXGPU]{},*fLO[MAXGPU]{},*fLAR[MAXGPU]{},*fLD[MAXGPU]{},*fHA[MAXGPU]{},*fHM[MAXGPU]{},*fHO[MAXGPU]{},*fHAR[MAXGPU]{},*fHR[MAXGPU]{};unsigned long long *prOB[MAXGPU]{},*prCI[MAXGPU]{},*prH[MAXGPU]{},*hpOB[MAXGPU]{},*hpCI[MAXGPU]{},*hpX[MAXGPU]{};uint32_t *prCIS[MAXGPU]{},*hpCIS[MAXGPU]{},*prBCM[MAXGPU]{},*prBCS[MAXGPU]{},*hpBCM[MAXGPU]{},*hpBCS[MAXGPU]{};Code *fHMB[MAXGPU]{},*fHBB[MAXGPU]{};unsigned long long*p1DR[MAXGPU]{},*cdLR[MAXGPU]{},*cdHR[MAXGPU]{};uint32_t*p1DS[MAXGPU]{},*cdLS[MAXGPU]{},*cdHS[MAXGPU]{},*cdLCI[MAXGPU]{},*cdHCI[MAXGPU]{};
    for(int d=0;d<ng;++d){cudaSetDevice(d);auto cp=[&](uint32_t**dst,const std::vector<uint32_t>&v,const char*w){if(v.empty())return;ck(cudaMalloc(dst,v.size()*sizeof(uint32_t)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),w);};cp(&fLA[d],G_FACTOR.low_all_codes,"f low all");cp(&fLM[d],G_FACTOR.low_mask_codes,"f low mask");cp(&fLO[d],G_FACTOR.low_mask_off,"f low off");cp(&fLAR[d],G_FACTOR.low_mask_all_rank,"f low mask all rank");fLDv[d]=make_sparse_vmm_u32(d,size_t(1)<<(2*LOW_LUT_K),G_FACTOR.low_all_codes,G_FACTOR.low_packed_values,"LOW");fLD[d]=fLDv[d].ptr();cp(&fHA[d],G_FACTOR.high_all_codes,"f high all");cp(&fHM[d],G_FACTOR.high_mask_codes,"f high mask");cp(&fHO[d],G_FACTOR.high_mask_off,"f high off");cp(&fHAR[d],G_FACTOR.high_mask_all_rank,"f high mask all rank");fHRv[d]=make_sparse_vmm_u32(d,size_t(1)<<(2*HIGH_LUT_K),G_FACTOR.high_all_codes,G_FACTOR.high_packed_values,"HIGH");fHR[d]=fHRv[d].ptr();auto cpc=[&](Code**dst,const std::vector<Code>&v,const char*w){ck(cudaMalloc(dst,v.size()*sizeof(Code)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(Code),cudaMemcpyHostToDevice),w);};cpc(&fHMB[d],G_FACTOR.high_main_base,"f high main base");cpc(&fHBB[d],G_FACTOR.high_block_base,"f high block base");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_CODES,&fLA[d],sizeof(fLA[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_MASK_CODES,&fLM[d],sizeof(fLM[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_MASK_OFF,&fLO[d],sizeof(fLO[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_MASK_ALL_RANK,&fLAR[d],sizeof(fLAR[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_DENSE_PACKED_RANK,&fLD[d],sizeof(fLD[d])),"f dense ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_ALL_CODES,&fHA[d],sizeof(fHA[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MASK_CODES,&fHM[d],sizeof(fHM[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MASK_OFF,&fHO[d],sizeof(fHO[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MASK_ALL_RANK,&fHAR[d],sizeof(fHAR[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_PACKED_RANK,&fHR[d],sizeof(fHR[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MAIN_BASE,&fHMB[d],sizeof(fHMB[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_BLOCK_BASE,&fHBB[d],sizeof(fHBB[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_OFF,G_FACTOR.low_all_off.data(),sizeof(uint32_t)*(MAXW+2)),"f low all off");ck(cudaMemcpyToSymbol(D_F_HIGH_ALL_OFF,G_FACTOR.high_all_off.data(),sizeof(uint32_t)*(MAXW+2)),"f high all off");auto cp8=[&](unsigned long long**dst,const std::vector<unsigned long long>&v,const char*w){if(v.empty())return;ck(cudaMalloc(dst,v.size()*sizeof(unsigned long long)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(unsigned long long),cudaMemcpyHostToDevice),w);};cp8(&prOB[d],G_PRERANK.owner_block_rec,"pr owner blockorder");cp(&prBCM[d],G_PRERANK.owner_block_closure_meta,"pr block closure meta");cp(&prBCS[d],G_PRERANK.owner_block_closure_src,"pr block closure src");cp8(&prCI[d],G_PRERANK.closure_inv_rec,"pr closure inv");cp(&prCIS[d],G_PRERANK.closure_inv_src,"pr closure inv src");cp8(&prH[d],G_PRERANK.highrr_rec,"pr highrr");if(groupbatch_p1_dest_mode()){cp8(&p1DR[d],G_P1_DEST.rec,"p1 dest rec");cp(&p1DS[d],G_P1_DEST.src,"p1 dest src");ck(cudaMemcpyToSymbol(D_P1_DEST_REC,&p1DR[d],sizeof(p1DR[d])),"p1 dest rec ptr");ck(cudaMemcpyToSymbol(D_P1_DEST_SRC,&p1DS[d],sizeof(p1DS[d])),"p1 dest src ptr");}if(groupbatch_cross_dest_mode()){cp8(&cdLR[d],G_CROSS_DEST.low_rec,"cross low dest rec");cp(&cdLS[d],G_CROSS_DEST.low_src,"cross low dest src");cp(&cdLCI[d],G_CROSS_DEST.low_cross_idx,"cross low rec idx");cp8(&cdHR[d],G_CROSS_DEST.high_rec,"cross high dest rec");cp(&cdHS[d],G_CROSS_DEST.high_src,"cross high dest src");cp(&cdHCI[d],G_CROSS_DEST.high_cross_idx,"cross high rec idx");ck(cudaMemcpyToSymbol(D_CROSS_LOW_DEST_REC,&cdLR[d],sizeof(cdLR[d])),"cross low dest rec ptr");ck(cudaMemcpyToSymbol(D_CROSS_LOW_DEST_SRC,&cdLS[d],sizeof(cdLS[d])),"cross low dest src ptr");ck(cudaMemcpyToSymbol(D_CROSS_LOW_REC_IDX,&cdLCI[d],sizeof(cdLCI[d])),"cross low rec idx ptr");ck(cudaMemcpyToSymbol(D_CROSS_HIGH_DEST_REC,&cdHR[d],sizeof(cdHR[d])),"cross high dest rec ptr");ck(cudaMemcpyToSymbol(D_CROSS_HIGH_DEST_SRC,&cdHS[d],sizeof(cdHS[d])),"cross high dest src ptr");ck(cudaMemcpyToSymbol(D_CROSS_HIGH_REC_IDX,&cdHCI[d],sizeof(cdHCI[d])),"cross high rec idx ptr");}ck(cudaMemcpyToSymbol(D_PR_OWNER_BLOCK_REC,&prOB[d],sizeof(prOB[d])),"pr owner block ptr");ck(cudaMemcpyToSymbol(D_PR_OWNER_BLOCK_CLOSURE_META,&prBCM[d],sizeof(prBCM[d])),"pr block closure meta ptr");ck(cudaMemcpyToSymbol(D_PR_OWNER_BLOCK_CLOSURE_SRC,&prBCS[d],sizeof(prBCS[d])),"pr block closure src ptr");ck(cudaMemcpyToSymbol(D_PR_CLOSURE_INV_REC,&prCI[d],sizeof(prCI[d])),"pr inv ptr");ck(cudaMemcpyToSymbol(D_PR_CLOSURE_INV_SRC,&prCIS[d],sizeof(prCIS[d])),"pr inv src ptr");ck(cudaMemcpyToSymbol(D_PR_HIGHRR_REC,&prH[d],sizeof(prH[d])),"pr ptr");cp8(&hpOB[d],G_HPR.owner_block_rec,"hp owner blockorder");cp(&hpBCM[d],G_HPR.owner_block_closure_meta,"hp block closure meta");cp(&hpBCS[d],G_HPR.owner_block_closure_src,"hp block closure src");cp8(&hpCI[d],G_HPR.closure_inv_rec,"hp closure inv");cp(&hpCIS[d],G_HPR.closure_inv_src,"hp closure inv src");cp8(&hpX[d],G_HPR.crossll_rec,"hp crossll");ck(cudaMemcpyToSymbol(D_HP_OWNER_BLOCK_REC,&hpOB[d],sizeof(hpOB[d])),"hp owner block ptr");ck(cudaMemcpyToSymbol(D_HP_OWNER_BLOCK_CLOSURE_META,&hpBCM[d],sizeof(hpBCM[d])),"hp block closure meta ptr");ck(cudaMemcpyToSymbol(D_HP_OWNER_BLOCK_CLOSURE_SRC,&hpBCS[d],sizeof(hpBCS[d])),"hp block closure src ptr");ck(cudaMemcpyToSymbol(D_HP_CLOSURE_INV_REC,&hpCI[d],sizeof(hpCI[d])),"hp inv ptr");ck(cudaMemcpyToSymbol(D_HP_CLOSURE_INV_SRC,&hpCIS[d],sizeof(hpCIS[d])),"hp inv src ptr");ck(cudaMemcpyToSymbol(D_HP_CROSSLL_REC,&hpX[d],sizeof(hpX[d])),"hp ptr");}
    ...
    bool dynamicGroupBatch=ng>1;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_DYNAMIC"))dynamicGroupBatch=std::atoi(e)!=0;
    int threads=256;if(const char*e=std::getenv("GRIDFP_THREADS")){int v=std::atoi(e);if(v==64||v==128||v==256||v==512||v==1024)threads=v;else throw std::runtime_error("GRIDFP_THREADS must be 64,128,256,512,1024");}int maxgroups=0;auto prep0=std::chrono::steady_clock::now();std::vector<PreparedWindow> schedule;
    {
        ...
        for(auto const& r:ranges){
            ...
            maxgroups=std::max(maxgroups,nj);std::sort(pw.groups.begin(),pw.groups.end(),[](auto const&a,auto const&b){return a.work>b.work;});size_t batchTarget=groupbatch_target_for_window(pw,target,ng,dynamicGroupBatch);prepare_window_batches(W,pw,batchTarget);
            auto gbs=groupbatch_stats(pw);std::cerr<<"forced window p="<<hi<<".."<<lo<<" fixed="<<k<<" groups="<<nj<<" batches="<<pw.batches.size()<<" max_mib="<<(mx>>20)<<" batch_target_mib="<<double(batchTarget)/(1<<20)<<" max_batch_mib="<<double(gbs.max_batch_bytes)/(1<<20)<<" wrap32_extra_mib=0\n";schedule.push_back(std::move(pw));
        }
    }
    bool affinityGroupBatch=dynamicGroupBatch;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_AFFINITY"))affinityGroupBatch=dynamicGroupBatch&&std::atoi(e)!=0;bool stickyGroupBatch=affinityGroupBatch;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_STICKY"))stickyGroupBatch=affinityGroupBatch&&std::atoi(e)!=0;bool reclaimGroupBatch=stickyGroupBatch;if(const char*e=std::getenv("GRIDFP_GROUPBATCH_RECLAIM"))reclaimGroupBatch=stickyGroupBatch&&std::atoi(e)!=0;
    std::vector<GroupBatchAffinity> batchAffinity;if(affinityGroupBatch){batchAffinity.reserve(schedule.size());for(auto const&pw:schedule)batchAffinity.push_back(make_groupbatch_affinity(pw,ng));}
    ...
    std::vector<std::vector<PreparedWindow>> schedule_shards;
    if(ng>1&&!dynamicGroupBatch){schedule_shards.resize(schedule.size(),std::vector<PreparedWindow>(ng));for(size_t wi=0;wi<schedule.size();++wi){std::vector<unsigned long long> load(ng,0);for(int d=0;d<ng;++d)schedule_shards[wi][d].wp=schedule[wi].wp;for(auto const&pg:schedule[wi].groups){int best=0;for(int d=1;d<ng;++d)if(load[d]<load[best])best=d;schedule_shards[wi][best].groups.push_back(pg);load[best]+=pg.work;}for(int d=0;d<ng;++d)prepare_window_batches(W,schedule_shards[wi][d],target);std::cerr<<"groupbatch shard window="<<wi;for(int d=0;d<ng;++d)std::cerr<<" gpu"<<d<<"_groups="<<schedule_shards[wi][d].groups.size()<<"_work="<<load[d];std::cerr<<"\n";}}

    std::cerr<<"groupbatch multi_gpu_mode="<<(ng==1?"single":dynamicGroupBatch?(affinityGroupBatch?(stickyGroupBatch?(reclaimGroupBatch?"dynamic-affinity-sticky-reclaim":"dynamic-affinity-sticky"):"dynamic-affinity"):"dynamic-batch"):"static-shard")<<"\n";
    if(affinityGroupBatch)for(size_t wi=0;wi<batchAffinity.size();++wi){uint64_t sum=0,mx=0;std::cerr<<"groupbatch affinity window="<<wi;for(int d=0;d<ng;++d){sum+=batchAffinity[wi].load[d];mx=std::max(mx,batchAffinity[wi].load[d]);std::cerr<<" gpu"<<d<<"_batches="<<batchAffinity[wi].home[d].size()<<"_work="<<batchAffinity[wi].load[d];}std::cerr<<" max_over_avg="<<(sum?double(mx)*ng/double(sum):0.0)<<"\n";}
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        if(groupbatch_wrap32_mode())std::cerr<<"groupbatch wrap32="<<(G_GROUPBATCH_WRAP32_ACTIVE?"active":"fallback-cas")<<" modulus="<<mod<<" gap="<<((1ull<<32)-uint64_t(mod))<<" carry_bytes_per_block_state=0 method=end-around\n";
        if(ri){if(ng==1){for(auto&pw:schedule)invalidate_groupbatch_graphs(pw,ng);}else if(dynamicGroupBatch){for(auto&pw:schedule)invalidate_groupbatch_graphs(pw,ng);}else{for(auto&v:schedule_shards)for(auto&pw:v)invalidate_groupbatch_graphs(pw,ng);}}
        for(int d=0;d<ng;++d)release_groupbatch_arena(ctx[d]);
        for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"set residue reset");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"set modulus");if(ml[d])ck(cudaMemset(mp[d],0,size_t(ml[d])*sizeof(Count)),"zero main");if(bl[d])ck(cudaMemset(bp[d],0,size_t(bl[d])*sizeof(Count)),"zero block");ck(cudaDeviceSynchronize(),"zero sync");ctx[d].active=0;ctx[d].gather_s=ctx[d].transition_s=ctx[d].scatter_s=ctx[d].pipeline_graph_s=0;ctx[d].groups=0;ctx[d].local_batches=ctx[d].stolen_batches=ctx[d].sticky_migrations=ctx[d].meta_upload_batches=ctx[d].meta_upload_bytes=ctx[d].meta_reclaim_batches=ctx[d].meta_reclaim_bytes=ctx[d].graph_instantiates=0;}
        auto wall0=std::chrono::steady_clock::now();
        int prefixK=(W>=21?6:5);if(const char*e=std::getenv("GRIDFP_BOUNDED_PREFIX_K"))prefixK=std::max(1,std::min(W-2,std::atoi(e)));
        ...
        if(ng==1){for(auto const&pw:schedule)reserve_groupbatch_buffers(ctx[0],pw);}else if(dynamicGroupBatch){for(int d=0;d<ng;++d)for(auto const&pw:schedule)reserve_groupbatch_buffers(ctx[d],pw);}else{for(size_t wi=0;wi<schedule_shards.size();++wi)for(int d=0;d<ng;++d)reserve_groupbatch_buffers(ctx[d],schedule_shards[wi][d]);}
        int done_windows=0;
        for(int row=prefixK;row<W-1;++row){for(size_t wi=0;wi<schedule.size();++wi){if(ng==1)process_window_batched_io(ctx[0],W,schedule[wi],threads,target);else if(dynamicGroupBatch){if(affinityGroupBatch){process_window_dynamic_affinity(ctx,W,schedule[wi],batchAffinity[wi],threads,target,stickyGroupBatch,reclaimGroupBatch);}else{std::atomic<size_t>nextBatch{0};std::vector<std::thread>ths;ths.reserve(ng);for(int d=0;d<ng;++d)ths.emplace_back([&,d,wi]{for(;;){size_t bi=nextBatch.fetch_add(1,std::memory_order_relaxed);if(bi>=schedule[wi].batches.size())break;process_window_batched_io(ctx[d],W,schedule[wi],threads,target,bi,bi+1);}});for(auto&t:ths)t.join();}}else{std::vector<std::thread>ths;ths.reserve(ng);for(int d=0;d<ng;++d)ths.emplace_back([&,d,wi]{process_window_batched_io(ctx[d],W,schedule_shards[wi][d],threads,target);});for(auto&t:ths)t.join();}++done_windows;}std::cerr<<"mod "<<(ri+1)<<"/"<<mods.size()<<" p="<<mod<<" row "<<row+1<<"/"<<W<<"\n";}
        Count ans=0;
        for(int d=0;d<ng;++d){
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0,pgs=0;uint64_t localBatches=0,stolenBatches=0,stickyMigrations=0,metaUploadBatches=0,metaUploadBytes=0,metaReclaimBatches=0,metaReclaimBytes=0,graphInstantiates=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;pgs+=c.pipeline_graph_s;localBatches+=c.local_batches;stolenBatches+=c.stolen_batches;stickyMigrations+=c.sticky_migrations;metaUploadBatches+=c.meta_upload_batches;metaUploadBytes+=c.meta_upload_bytes;metaReclaimBatches+=c.meta_reclaim_batches;metaReclaimBytes+=c.meta_reclaim_bytes;graphInstantiates+=c.graph_instantiates;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-fullprerank-orbit-blockfused-groupbatch-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" pipeline_graph_sum_s="<<pgs<<" local_batches="<<localBatches<<" stolen_batches="<<stolenBatches<<" sticky_migrations="<<stickyMigrations<<" meta_upload_batches="<<metaUploadBatches<<" meta_upload_mib="<<double(metaUploadBytes)/(1<<20)<<" meta_reclaim_batches="<<metaReclaimBatches<<" meta_reclaim_mib="<<double(metaReclaimBytes)/(1<<20)<<" graph_instantiates="<<graphInstantiates<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    if(ng==1||dynamicGroupBatch){for(auto&pw:schedule)destroy_groupbatch_device_meta(pw,ng);}else{for(auto&wv:schedule_shards)for(auto&pw:wv)destroy_groupbatch_device_meta(pw,ng);}
    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);if(fLAR[d])cudaFree(fLAR[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);if(fHAR[d])cudaFree(fHAR[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prOB[d])cudaFree(prOB[d]);if(prBCM[d])cudaFree(prBCM[d]);if(prBCS[d])cudaFree(prBCS[d]);if(prCI[d])cudaFree(prCI[d]);if(prCIS[d])cudaFree(prCIS[d]);if(prH[d])cudaFree(prH[d]);if(p1DR[d])cudaFree(p1DR[d]);if(p1DS[d])cudaFree(p1DS[d]);if(cdLR[d])cudaFree(cdLR[d]);if(cdLS[d])cudaFree(cdLS[d]);if(cdLCI[d])cudaFree(cdLCI[d]);if(cdHR[d])cudaFree(cdHR[d]);if(cdHS[d])cudaFree(cdHS[d]);if(cdHCI[d])cudaFree(cdHCI[d]);if(hpOB[d])cudaFree(hpOB[d]);if(hpBCM[d])cudaFree(hpBCM[d]);if(hpBCS[d])cudaFree(hpBCS[d]);if(hpCI[d])cudaFree(hpCI[d]);if(hpCIS[d])cudaFree(hpCIS[d]);if(hpX[d])cudaFree(hpX[d]);}
}
