...
struct BatchFactorIoCfg {
    FBlock mainb[64];
    FBlock blockb[32];
    Code main_n,block_n,main_off,block_off,wrap_off;
    uint32_t mask;
...
__device__ __forceinline__ Count groupbatch_wrap32_normalize(Count x,uint8_t wraps){unsigned long long gap=0x100000000ULL-(unsigned long long)D_MOD,z=(unsigned long long)x+(unsigned long long)wraps*gap;if(z>=D_MOD)z-=D_MOD;return Count(z);}
__device__ __forceinline__ void groupbatch_wrap32_atomic_add(Count*p,uint8_t*wrap,Code wi,Count v){if(!v)return;unsigned int old=atomicAdd(reinterpret_cast<unsigned int*>(p),unsigned(v));if(old>0xffffffffu-unsigned(v)){unsigned sh=unsigned(wi&3u)*8u;unsigned int*wp=reinterpret_cast<unsigned int*>(wrap)+(wi>>2);unsigned int ow=atomicAdd(wp,1u<<sh);if(((ow>>sh)&0xffu)==0xffu)asm("trap;");}}
template<bool SCATTER> __global__ void batch_factor_io_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchFactorIoTask*tasks,Code nt,uint8_t*wrap){
    ...
    for(Code i=t.begin+threadIdx.x;i<end;i+=blockDim.x){int bi=t.block?batch_factor_find(i,c->blockb,c->block_nb):batch_factor_find(i,c->mainb,c->main_nb);FBlock x=t.block?c->blockb[bi]:c->mainb[bi];Code z=i-x.off;uint32_t hr=x.stride?uint32_t(z/x.stride):0,lr=x.stride?uint32_t(z-Code(hr)*x.stride):0;uint32_t har,lar;if(c->fix_low){har=hr;lar=D_F_LOW_MASK_ALL_RANK[D_F_LOW_MASK_OFF[size_t(c->mask)*S+(t.block?x.he:x.hs)]+lr];}else{har=D_F_HIGH_MASK_ALL_RANK[D_F_HIGH_MASK_OFF[size_t(c->mask)*S+x.he]+hr];lar=lr;}Code g;if(t.block)g=D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[x.he]+har]+lar;else{g=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[x.he]+har];if(x.c>N)g+=D_FULL_DP[L][x.he];if(x.c>R&&x.he>0)g+=D_FULL_DP[L][x.he-1];g+=lar;}Count*bp=arena+bo+i;if constexpr(SCATTER){Count v=*bp;if(t.block&&wrap)v=groupbatch_wrap32_normalize(v,wrap[c->wrap_off+i]);if(t.block)global_store_block(g,v);else global_store_main(g,v);}else *bp=t.block?global_load_block(g):global_load_main(g);}
}
...
__global__ void batch_low_owner_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchOwnerTask*tasks,Code nt,int p,uint8_t*wrap){
...
#pragma unroll
        for(int a=0;a<H;++a){auto mv=MateValue((hc>>(2*a))&3u);if(mv==L){if(--s==0){match=a;break;}}else if(mv==R)++s;}if(match<0)continue;uint32_t nhc=(hc&~(3u<<(2*match)))|(uint32_t(R)<<(2*match));uint32_t hp=D_F_HIGH_PACKED_RANK[nhc],nhr=hp&HM;int nhe=int(x.he)-2;if(p==1){FBlock dx=c->mainb[3*nhe+int(x.c)];atomic_add_mod(mainv+dx.off+Code(nhr)*dx.stride+dlr,v);}else{FBlock bx=c->blockb[nhe];Code di=bx.off+Code(nhr)*bx.stride+dlr;if(wrap)groupbatch_wrap32_atomic_add(blockv+di,wrap,c->wrap_off+di,v);else atomic_add_mod(blockv+di,v);}}
}
...
__global__ void batch_high_crossll_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchCrossTask*tasks,Code nt,uint8_t*wrap){
...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr,*dWrap=nullptr;size_t capArena=0,capWrap=0;Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;MateID*dMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;
...
struct BatchDeviceMeta{
    BatchFactorIoCfg*cfg=nullptr;BatchFactorIoTask*io=nullptr;BatchOwnerTask*owner=nullptr;BatchCrossTask*cross=nullptr,*inv=nullptr;BatchP1DestTask*p1=nullptr;cudaGraphExec_t graph=nullptr;bool ready=false;
};
struct PreparedBatchHost{
    Code used=0,wrap_used=0,work=0;bool fix_low=false;
    ...
    std::array<size_t,MAXW+1> cross_off{},cross_count{},inv_off{},inv_count{};
...
static void prepare_window_batches(int W,PreparedWindow&pw,size_t target){
    ...
    size_t q0=0;while(q0<pw.groups.size()){
        PreparedBatchHost b;Code used=0,wrapUsed=0;size_t q=q0;bool fixLow=pw.wp.p_hi>LOW_LUT_K;
        for(;q<pw.groups.size();++q){auto const&pg=pw.groups[q];if(!pg.ms.size&&!pg.ds.size)continue;Code need=al(pg.ms.size)+al(pg.ds.size);uint64_t nextBytes=uint64_t(used+need)*sizeof(Count)+(groupbatch_wrap32_mode()?uint64_t(wrapUsed+pg.ds.size):0),oneBytes=uint64_t(need)*sizeof(Count)+(groupbatch_wrap32_mode()?uint64_t(pg.ds.size):0);if(!b.cfg.empty()&&nextBytes>target)break;if(oneBytes>target)throw std::runtime_error("groupbatch single group exceeds target");
            b.work+=pg.work;uint32_t fmask=fixLow?(pg.mo&((1u<<LOW_LUT_K)-1u)):((pg.mo>>(LOW_LUT_K+1))&((1u<<HIGH_LUT_K)-1u));auto fmb=make_factor_main_blocks(fixLow,fmask),fdb=make_factor_block_blocks(fixLow,fmask);if(fmb.back().end!=pg.ms.size||fdb.back().end!=pg.ds.size)throw std::runtime_error("groupbatch factor size mismatch");BatchFactorIoCfg io{};std::memcpy(io.mainb,fmb.data(),fmb.size()*sizeof(FBlock));std::memcpy(io.blockb,fdb.data(),fdb.size()*sizeof(FBlock));io.main_n=pg.ms.size;io.block_n=pg.ds.size;io.main_off=used;io.block_off=used+al(pg.ms.size);io.wrap_off=wrapUsed;wrapUsed+=pg.ds.size;io.mask=fmask;io.main_nb=fmb.size();io.block_nb=fdb.size();io.fix_low=fixLow?1:0;b.cfg.push_back(io);used+=need;}
        if(b.cfg.empty()){q0=q+1;continue;}b.used=used;b.wrap_used=wrapUsed;b.fix_low=fixLow;
        ...
        for(int pp=pw.wp.p_hi;pp>=pw.wp.p_lo;--pp){b.cross_off[pp]=b.cross.size();b.inv_off[pp]=b.inv.size();for(uint32_t gi=0;gi<b.cfg.size();++gi){auto const&io=b.cfg[gi];for(uint32_t bid=0;bid<io.main_nb;++bid){auto x=io.mainb[bid];if(!x.stride||x.end<=x.off)continue;if(fixLow){auto sel=G_HPR.crossll_sel[high_pr_key(pp,x.he,x.c)];Code total=Code(x.stride)*sel.count;for(Code z=0;z<total;z+=GROUPBATCH_CHUNK)b.cross.push_back({gi,bid,sel.off,sel.count,z});}else{auto key=sparse_key(pp,x.hs,x.c);auto sel=G_PRERANK.highrr_sel[key];Code rows=(x.end-x.off)/x.stride,total=rows*Code(sel.count);for(Code z=0;z<total;z+=GROUPBATCH_CHUNK)b.cross.push_back({gi,bid,sel.off,sel.count,z});if(pp==1&&!groupbatch_p1_dest_mode()){auto iv=G_PRERANK.closure_inv_sel[key];Code itotal=rows*Code(iv.count);for(Code z=0;z<itotal;z+=GROUPBATCH_CHUNK)b.inv.push_back({gi,bid,iv.off,iv.count,z});}}}}b.cross_count[pp]=b.cross.size()-b.cross_off[pp];b.inv_count[pp]=b.inv.size()-b.inv_off[pp];}
...
struct GroupBatchStats{
    uint64_t used_bytes=0,max_batch_bytes=0,wrap_bytes=0,max_wrap_bytes=0,max_total_bytes=0,meta_bytes=0;
    uint64_t cfg=0,io=0,owner=0,cross=0,inv=0,p1=0;
};
...
static std::pair<unsigned long long,unsigned long long> groupbatch_cross_compact_dest_work(const PreparedWindow&pw){
    bool fixLow=pw.wp.p_hi>LOW_LUT_K;unsigned long long src=0,dst=0;std::unordered_map<unsigned long long,uint32_t>cache;
    auto dest_count=[&](bool low,uint32_t mask,int sh,int depth)->uint32_t{
        unsigned long long key=(uint64_t(low)<<63)|(uint64_t(mask)<<24)|(uint64_t(sh)<<16)|uint64_t(depth);auto it=cache.find(key);if(it!=cache.end())return it->second;
        std::unordered_set<uint32_t> out;
        if(low){size_t o=G_FACTOR.low_mask_off[size_t(mask)*FactorTablesHost::STRIDE+sh];uint32_t n=factor_count(G_FACTOR.low_mask_off,mask,sh);out.reserve(n*2+1);for(uint32_t r=0;r<n;++r){uint32_t code=G_FACTOR.low_mask_codes[o+r];int ss=depth,match=-1;for(int a=LOW_LUT_K-1;a>=0;--a){auto mv=MateValue((code>>(2*a))&3u);if(mv==R){if(--ss==0){match=a;break;}}else if(mv==L)++ss;}if(match>=0)out.insert((code&~(3u<<(2*match)))|(uint32_t(L)<<(2*match)));}}
        else{size_t o=G_FACTOR.high_mask_off[size_t(mask)*FactorTablesHost::STRIDE+sh];uint32_t n=factor_count(G_FACTOR.high_mask_off,mask,sh);out.reserve(n*2+1);for(uint32_t r=0;r<n;++r){uint32_t code=G_FACTOR.high_mask_codes[o+r];int ss=depth,match=-1;for(int a=0;a<HIGH_LUT_K;++a){auto mv=MateValue((code>>(2*a))&3u);if(mv==L){if(--ss==0){match=a;break;}}else if(mv==R)++ss;}if(match>=0)out.insert((code&~(3u<<(2*match)))|(uint32_t(R)<<(2*match)));}}
        uint32_t z=out.size();cache.emplace(key,z);return z;
    };
    constexpr uint32_t B=20;
    for(auto const&b:pw.batches)for(auto const&io:b.cfg)for(int pp=pw.wp.p_hi;pp>=pw.wp.p_lo;--pp)for(uint32_t bid=0;bid<uint32_t(io.main_nb);++bid){auto x=io.mainb[bid];if(!x.stride||x.end<=x.off)continue;
        if(fixLow){auto sel=G_HPR.crossll_sel[high_pr_key(pp,x.he,x.c)];if(!sel.count)continue;for(uint32_t k=0;k<sel.count;++k){auto r=G_HPR.crossll_rec[sel.off+k];int depth=int((r>>(2*B))&63u);src+=x.stride;dst+=dest_count(true,io.mask,x.hs,depth);}}
        else{if(pp==1)continue;auto sel=G_PRERANK.highrr_sel[sparse_key(pp,x.hs,x.c)];if(!sel.count)continue;uint32_t rows=(x.end-x.off)/x.stride;for(uint32_t k=0;k<sel.count;++k){auto r=G_PRERANK.highrr_rec[sel.off+k];int depth=int((r>>(2*B))&63u);src+=rows;dst+=dest_count(false,io.mask,x.he,depth);}}
    }
    return {src,dst};
}
...
static GroupBatchStats groupbatch_stats(const PreparedWindow&pw){
    ...
    for(auto const&b:pw.batches){
        uint64_t ub=uint64_t(b.used)*sizeof(Count),wb=groupbatch_wrap32_mode()?uint64_t(b.wrap_used):0;s.used_bytes+=ub;s.max_batch_bytes=std::max(s.max_batch_bytes,ub);s.wrap_bytes+=wb;s.max_wrap_bytes=std::max(s.max_wrap_bytes,wb);s.max_total_bytes=std::max(s.max_total_bytes,ub+wb);
        s.cfg+=b.cfg.size();s.io+=b.io.size();s.owner+=b.owner.size();s.cross+=b.cross.size();s.inv+=b.inv.size();s.p1+=b.p1.size();
        s.meta_bytes+=uint64_t(b.cfg.size())*sizeof(BatchFactorIoCfg)+uint64_t(b.io.size())*sizeof(BatchFactorIoTask)+uint64_t(b.owner.size())*sizeof(BatchOwnerTask)+uint64_t(b.cross.size()+b.inv.size())*sizeof(BatchCrossTask)+uint64_t(b.p1.size())*sizeof(BatchP1DestTask);
    }
    return s;
}
...
static size_t groupbatch_target_for_window(const PreparedWindow&pw,size_t hardTarget,int ng,bool dynamicMode){
    ...
    for(auto const&pg:pw.groups){uint64_t b=uint64_t(al(pg.ms.size)+al(pg.ds.size))*sizeof(Count)+(groupbatch_wrap32_mode()?uint64_t(pg.ds.size):0);total+=b;maxGroup=std::max(maxGroup,b);}
...
static void reserve_groupbatch_buffers(DeviceCtx&c,const PreparedWindow&pw){
    ck(cudaSetDevice(c.dev),"groupbatch reserve set");size_t arenaBytes=0,wrapBytes=0;for(auto const&b:pw.batches){arenaBytes=std::max(arenaBytes,size_t(b.used)*sizeof(Count));if(groupbatch_wrap32_mode())wrapBytes=std::max(wrapBytes,(size_t(b.wrap_used)+3)&~size_t(3));}if(arenaBytes>c.capArena){if(c.arena)cudaFree(c.arena);c.capArena=arenaBytes;ck(cudaMalloc(&c.arena,c.capArena),"groupbatch reserve arena");}if(wrapBytes>c.capWrap){if(c.dWrap)cudaFree(c.dWrap);c.capWrap=wrapBytes;ck(cudaMalloc(&c.dWrap,c.capWrap),"groupbatch reserve wrap32");}
}
...
static void process_window_batched_io(DeviceCtx&c,int W,const PreparedWindow&pw,int threads,size_t target,size_t begin_batch=0,size_t end_batch=~size_t(0)){
    ...
    for(size_t bi=begin_batch;bi<bend;++bi){
        ...
        if(G_GROUPBATCH_WRAP32_ACTIVE&&size_t(b.wrap_used)>c.capWrap)throw std::runtime_error("groupbatch wrap32 buffer was not reserved before graph capture");
        ...
        if(!dm.ready){
            ...
            upload(dm.cfg,b.cfg,"groupbatch cfg upload");upload(dm.io,b.io,"groupbatch io upload");upload(dm.owner,b.owner,"groupbatch owner upload");upload(dm.cross,b.cross,"groupbatch cross upload");upload(dm.inv,b.inv,"groupbatch inv upload");upload(dm.p1,b.p1,"groupbatch p1 upload");dm.ready=true;c.meta_upload_batches++;c.meta_upload_bytes+=groupbatch_device_meta_bytes(b);
        }
        BatchFactorIoCfg*dcfg=dm.cfg;BatchFactorIoTask*dt=dm.io;BatchOwnerTask*dot=dm.owner;BatchCrossTask*dct=dm.cross,*dit=dm.inv;BatchP1DestTask*dpt=dm.p1;Count*arena=reinterpret_cast<Count*>(c.arena);uint8_t*wrap=G_GROUPBATCH_WRAP32_ACTIVE?c.dWrap:nullptr;
        auto enqueue_gather=[&]{if(wrap&&b.wrap_used)ck(cudaMemsetAsync(wrap,0,size_t(b.wrap_used),c.sMain),"groupbatch wrap32 clear");batch_factor_io_kernel<false><<<std::max<size_t>(1,b.io.size()),threads,0,c.sMain>>>(arena,dcfg,dt,b.io.size(),wrap);};
        auto enqueue_transition=[&]{
            for(int pp=pw.wp.p_hi;pp>=pw.wp.p_lo;--pp){
                if(!b.owner.empty()){if(b.fix_low)batch_high_owner_kernel<<<b.owner.size(),threads,0,c.sMain>>>(arena,dcfg,dot,b.owner.size(),pp,wrap);else batch_low_owner_kernel<<<b.owner.size(),threads,0,c.sMain>>>(arena,dcfg,dot,b.owner.size(),pp,wrap);}
                ...
                if(b.cross_count[pp]){if(b.fix_low)batch_high_crossll_kernel<<<b.cross_count[pp],threads,0,c.sMain>>>(arena,dcfg,dct+b.cross_off[pp],b.cross_count[pp],wrap);else batch_low_highrr_kernel<<<b.cross_count[pp],threads,0,c.sMain>>>(arena,dcfg,dct+b.cross_off[pp],b.cross_count[pp],pp,wrap);}
            }
        };
        auto enqueue_scatter=[&]{batch_factor_io_kernel<true><<<std::max<size_t>(1,b.io.size()),threads,0,c.sMain>>>(arena,dcfg,dt,b.io.size(),wrap);};
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t p1DestBytes=groupbatch_p1_dest_mode()?uint64_t(G_P1_DEST.rec.size())*8+uint64_t(G_P1_DEST.src.size())*4:0;uint64_t sparseBytes=0;uint64_t preRankBytes=uint64_t(G_PRERANK.owner_block_rec.size()+G_PRERANK.highrr_rec.size()+G_PRERANK.closure_inv_rec.size()+G_HPR.owner_block_rec.size()+G_HPR.crossll_rec.size()+G_HPR.closure_inv_rec.size())*8+uint64_t(G_PRERANK.closure_inv_src.size()+G_HPR.closure_inv_src.size()+G_PRERANK.owner_block_closure_meta.size()+G_PRERANK.owner_block_closure_src.size()+G_HPR.owner_block_closure_meta.size()+G_HPR.owner_block_closure_src.size())*4;uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+vb4(G_FACTOR.low_mask_all_rank)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+vb4(G_FACTOR.high_mask_all_rank)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+sparseBytes+preRankBytes+p1DestBytes;
        ...
        uint64_t totalMeta=0,totalBatches=0,totalCfg=0,totalIo=0,totalOwner=0,totalCross=0,totalInv=0,totalP1=0,maxBatch=0;
        if(scratchFits){
            ...
            for(int wi=0;wi<2;++wi){
                ...
                totalMeta+=st.meta_bytes;totalBatches+=pw.batches.size();totalCfg+=st.cfg;totalIo+=st.io;totalOwner+=st.owner;totalCross+=st.cross;totalInv+=st.inv;totalP1+=st.p1;maxBatch=std::max(maxBatch,groupbatch_wrap32_mode()?st.max_total_bytes:st.max_batch_bytes);auto cw=groupbatch_cross_work(pw);auto ccw=groupbatch_cross_compact_dest_work(pw);
                std::cerr<<"PLAN groupbatch_window="<<wi<<" p="<<wp.p_hi<<".."<<wp.p_lo<<" groups="<<pw.groups.size()<<" batches="<<pw.batches.size()<<" batch_target_mib="<<double(bt)/(1<<20)<<" max_batch_gib="<<double(st.max_batch_bytes)/(1ull<<30)<<" wrap32_mib="<<double(st.max_wrap_bytes)/(1<<20)<<" total_scratch_gib="<<double(st.max_total_bytes)/(1ull<<30)<<" meta_mib="<<double(st.meta_bytes)/(1<<20)<<" affinity_max_over_avg="<<(affSum?double(affMax)*png/double(affSum):0.0)<<" cfg="<<st.cfg<<" io="<<st.io<<" owner="<<st.owner<<" cross="<<st.cross<<" inv="<<st.inv<<" p1="<<st.p1<<" cross_src_work="<<cw.first<<" cross_dst_work="<<cw.second<<" cross_dst_over_src="<<(cw.first?double(cw.second)/double(cw.first):0.0)<<" compact_cross_dst_work="<<ccw.second<<" compact_over_src="<<(ccw.first?double(ccw.second)/double(ccw.first):0.0)<<"\n";
            }
        }
    ...
    int threads=256;if(const char*e=std::getenv("GRIDFP_THREADS")){int v=std::atoi(e);if(v==64||v==128||v==256||v==512||v==1024)threads=v;else throw std::runtime_error("GRIDFP_THREADS must be 64,128,256,512,1024");}int maxgroups=0;auto prep0=std::chrono::steady_clock::now();std::vector<PreparedWindow> schedule;
    {
        ...
        for(auto const& r:ranges){
            ...
            auto gbs=groupbatch_stats(pw);std::cerr<<"forced window p="<<hi<<".."<<lo<<" fixed="<<k<<" groups="<<nj<<" batches="<<pw.batches.size()<<" max_mib="<<(mx>>20)<<" batch_target_mib="<<double(batchTarget)/(1<<20)<<" max_batch_mib="<<double(groupbatch_wrap32_mode()?gbs.max_total_bytes:gbs.max_batch_bytes)/(1<<20)<<" wrap32_mib="<<double(gbs.max_wrap_bytes)/(1<<20)<<"\n";schedule.push_back(std::move(pw));
        }
    }
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        if(groupbatch_wrap32_mode())std::cerr<<"groupbatch wrap32="<<(G_GROUPBATCH_WRAP32_ACTIVE?"active":"fallback-cas")<<" modulus="<<mod<<" gap="<<((1ull<<32)-uint64_t(mod))<<" carry_bytes_per_block_state=1\n";
    ...
    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);if(fLAR[d])cudaFree(fLAR[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);if(fHAR[d])cudaFree(fHAR[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prOB[d])cudaFree(prOB[d]);if(prBCM[d])cudaFree(prBCM[d]);if(prBCS[d])cudaFree(prBCS[d]);if(prCI[d])cudaFree(prCI[d]);if(prCIS[d])cudaFree(prCIS[d]);if(prH[d])cudaFree(prH[d]);if(p1DR[d])cudaFree(p1DR[d]);if(p1DS[d])cudaFree(p1DS[d]);if(hpOB[d])cudaFree(hpOB[d]);if(hpBCM[d])cudaFree(hpBCM[d]);if(hpBCS[d])cudaFree(hpBCS[d]);if(hpCI[d])cudaFree(hpCI[d]);if(hpCIS[d])cudaFree(hpCIS[d]);if(hpX[d])cudaFree(hpX[d]);}
}
