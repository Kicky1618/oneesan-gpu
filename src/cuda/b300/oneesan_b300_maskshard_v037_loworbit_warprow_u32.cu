// v0.37 experiment: v0.36 warp-row mapping with uint32 task/prefix arithmetic.
// n=27 maximum group warp ordinal is < 16 million, so 64-bit task division is
// unnecessary; state addressing remains 64-bit Code.
...
#define main oneesan_maskshard_v37_loworbitwarprowu32_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.37 LOW orbit warp-row u32 requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.37-loworbit-warprow-u32-batch"
        << " low_orbit_warp_row_tasks=1 low_orbit_warp_row_u32=1"
        << " low_group_sync=1 low_orbit_row_depth_compact_launch=1 guarded_hbm=1\n";
    const int rc = oneesan_maskshard_v37_loworbitwarprowu32_inner_main(argc, argv);
    ...
    if (rc == 0 && skipped != executed * skips_per_group) {
        std::cerr << "v0.37 LOW group sync count mismatch skipped=" << skipped
                  << " executed=" << executed << '\n';
        return 4;
    }
    return rc;
