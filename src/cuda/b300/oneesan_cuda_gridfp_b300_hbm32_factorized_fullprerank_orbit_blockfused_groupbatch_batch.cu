...
#include "../../common/shard_address.hpp"
#include "../../common/invariant_division.hpp"
#include <cuda.h>
#include <cuda/atomic>
#include <algorithm>
#include <atomic>
...
struct FBlock { Code off,end; uint32_t stride; uint8_t he,hs,c,pad; uint64_t reciprocal; };
...
static bool groupbatch_wrap32_mod_safe(Count mod){return uint64_t(mod)*2ull>(1ull<<32);}
...
static std::vector<FBlock> make_factor_main_blocks(bool fix_low,uint32_t mask){
    ...
    for(auto& x:v)x.reciprocal=oneesan::division_reciprocal(x.stride);return v;
}
...
__device__ __forceinline__ Count global_load_main(Code g){auto a=oneesan::shard_address(g,D_MAIN_CHUNK,D_NGPU);return D_MAIN_PTR[a.owner][a.offset];}
__device__ __forceinline__ Count global_load_block(Code g){auto a=oneesan::shard_address(g,D_BLOCK_CHUNK,D_NGPU);return D_BLOCK_PTR[a.owner][a.offset];}
__device__ __forceinline__ void global_store_main(Code g,Count v){auto a=oneesan::shard_address(g,D_MAIN_CHUNK,D_NGPU);D_MAIN_PTR[a.owner][a.offset]=v;}
__device__ __forceinline__ void global_store_block(Code g,Count v){auto a=oneesan::shard_address(g,D_BLOCK_CHUNK,D_NGPU);D_BLOCK_PTR[a.owner][a.offset]=v;}
...
#include "groupbatch_interleaved.cuh"
...
#include "groupbatch_schedule.cuh"
...
static size_t groupbatch_target_for_window(const PreparedWindow&pw,size_t hardTarget,int ng,bool dynamicMode){
    ...
    for(auto const&pg:pw.groups){uint64_t b=groupbatch_group_bytes(pg.ms.size,pg.ds.size);total+=b;maxGroup=std::max(maxGroup,b);}
...
static void process_window_batched_io(DeviceCtx&c,int W,const PreparedWindow&pw,int threads,size_t target,size_t begin_batch=0,size_t end_batch=~size_t(0)){
    ...
    for(size_t bi=begin_batch;bi<bend;++bi){
        ...
        if(!dm.ready){
            // Metadata must precede kernels on the nonblocking execution stream.
            auto upload=[&](auto*&dst,auto const&v,const char*w){using T=typename std::decay_t<decltype(v)>::value_type;if(v.empty())return;ck(cudaMalloc(&dst,v.size()*sizeof(T)),w);ck(cudaMemcpyAsync(dst,v.data(),v.size()*sizeof(T),cudaMemcpyHostToDevice,c.sMain),w);};
        ...
        BatchFactorIoCfg*dcfg=dm.cfg;BatchFactorIoTask*dt=dm.io;BatchOwnerTask*dot=dm.owner;BatchCrossTask*dct=dm.cross,*dit=dm.inv;BatchP1DestTask*dpt=dm.p1;Count*arena=reinterpret_cast<Count*>(c.arena);bool wrap32=G_GROUPBATCH_WRAP32_ACTIVE,matchlut=groupbatch_matchlut_mode();
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t minScratchBytes=0,actualScratchBytes=0,totalGroups=0,mateGroups=0;{const int rr[2][2]={{W-1,LOW_LUT_K+1},{LOW_LUT_K,1}};for(auto const&r:rr){auto fp=window_candidates(W,r[0],r[1]);int nj=1<<int(fp.size());for(int g=0;g<nj;++g){uint32_t mf,mo,bf,bo;window_masks(W,r[0],r[1],fp,uint32_t(g),mf,mo,bf,bo);auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);bool orbit=true;uint64_t countBytes=groupbatch_group_bytes(ms.size,ds.size),mateBytes=uint64_t(ms.size)*sizeof(MateID);minScratchBytes=std::max(minScratchBytes,countBytes);bool useMate=!orbit&&countBytes+mateBytes<=scratchLimit;actualScratchBytes=std::max(actualScratchBytes,countBytes+(useMate?mateBytes:0));totalGroups++;mateGroups+=useMate;}}}
        ...
        if(scratchFits){
            ...
            for(int wi=0;wi<2;++wi){
                ...
                std::sort(pw.groups.begin(),pw.groups.end(),groupbatch_work_order);size_t bt=groupbatch_target_for_window(pw,scratchLimit,png,planDynamic);prepare_window_batches(W,pw,bt);auto st=groupbatch_stats(pw);auto aff=make_groupbatch_affinity(pw,png);uint64_t affSum=0,affMax=0;for(auto x:aff.load){affSum+=x;affMax=std::max(affMax,x);}
                totalMeta+=st.meta_bytes;totalBatches+=pw.batches.size();totalCfg+=st.cfg;totalIo+=st.io;totalOwner+=st.owner;totalCross+=st.cross;totalInv+=st.inv;totalP1+=st.p1;maxBatch=std::max(maxBatch,groupbatch_wrap32_mode()?st.max_total_bytes:st.max_batch_bytes);auto cw=groupbatch_cross_work(pw);
    ...
    int threads=256;if(const char*e=std::getenv("GRIDFP_THREADS")){int v=std::atoi(e);if(v==64||v==128||v==256||v==512||v==1024)threads=v;else throw std::runtime_error("GRIDFP_THREADS must be 64,128,256,512,1024");}int maxgroups=0;auto prep0=std::chrono::steady_clock::now();std::vector<PreparedWindow> schedule;
    {
        ...
        for(auto const& r:ranges){
            ...
            for(int g=0;g<nj;++g){auto pg=prepare_group(W,pw.wp,g,mc,bc,ng);size_t b=groupbatch_group_bytes(pg.ms.size,pg.ds.size);mx=std::max(mx,b);pw.groups.push_back(std::move(pg));}
            if(mx>target){std::cerr<<"forced window does not fit p="<<hi<<".."<<lo<<" max_bytes="<<mx<<" target="<<target<<"\n";return 4;}
            maxgroups=std::max(maxgroups,nj);std::sort(pw.groups.begin(),pw.groups.end(),groupbatch_work_order);size_t batchTarget=groupbatch_target_for_window(pw,target,ng,dynamicGroupBatch);prepare_window_batches(W,pw,batchTarget);
    ...
    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);if(fLAR[d])cudaFree(fLAR[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);if(fHAR[d])cudaFree(fHAR[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prOB[d])cudaFree(prOB[d]);if(prBCM[d])cudaFree(prBCM[d]);if(prBCS[d])cudaFree(prBCS[d]);if(prCI[d])cudaFree(prCI[d]);if(prCIS[d])cudaFree(prCIS[d]);if(prH[d])cudaFree(prH[d]);if(p1DR[d])cudaFree(p1DR[d]);if(p1DS[d])cudaFree(p1DS[d]);if(cmL[d])cudaFree(cmL[d]);if(cmH[d])cudaFree(cmH[d]);if(hpOB[d])cudaFree(hpOB[d]);if(hpBCM[d])cudaFree(hpBCM[d]);if(hpBCS[d])cudaFree(hpBCS[d]);if(hpCI[d])cudaFree(hpCI[d]);if(hpCIS[d])cudaFree(hpCIS[d]);if(hpX[d])cudaFree(hpX[d]);}
}
