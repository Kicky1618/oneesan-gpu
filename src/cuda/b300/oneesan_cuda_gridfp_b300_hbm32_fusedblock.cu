...
template<int BW>
__device__ __forceinline__ Code unrank_block_insert_rank_main(Code rank,int k){
    int h=1;Code out=0;
#pragma unroll
    for(int pos=BW-1;pos>=0;--pos){
        MateValue v=N;
        if(allowed(D_BLOCK_FIXED,D_BLOCK_OCC,pos,N)){Code z=D_BLOCK_DP[pos][h];if(rank<z)v=N;else{rank-=z;v=X;}}
        else v=X;
        if(v==X){
            if(h>0&&allowed(D_BLOCK_FIXED,D_BLOCK_OCC,pos,R)){Code z=D_BLOCK_DP[pos][h-1];if(rank<z)v=R;else{rank-=z;v=L;}}
            else v=L;
        }
        int q=pos>=k?pos+1:pos;
        if(v>N&&allowed(D_MAIN_FIXED,D_MAIN_OCC,q,N))out+=D_MAIN_DP[q][h];
        if(v>R&&h>0&&allowed(D_MAIN_FIXED,D_MAIN_OCC,q,R))out+=D_MAIN_DP[q][h-1];
        if(v==R)--h;else if(v==L)++h;
    }
    return out;
}
__global__ void blocked_group_kernel(const Count*in,Code n,Count*out_main,int p){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){Count c=in[i];if(!c)continue;Code j=unrank_block_insert_rank_main<TARGET_W-1>(i,p);atomic_add_mod(out_main+j,c);}}
