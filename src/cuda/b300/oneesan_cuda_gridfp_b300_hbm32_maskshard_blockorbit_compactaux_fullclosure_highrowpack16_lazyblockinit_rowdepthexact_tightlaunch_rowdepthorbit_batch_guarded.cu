// v0.17 experiment: v0.16 plus exact row-depth pruning inside the
// BLOCKED-domain HIGH orbit. States above the current row's structural depth
// cap skip descriptor/aux/state loads and arithmetic before blockv is touched.
...
#define main oneesan_maskshard_v17_rowdepthorbit_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.17 row-depth orbit requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.17-highrowpack16-rowdepthexact-tightlaunch-rowdepthorbit-batch"
    ...
    return oneesan_maskshard_v17_rowdepthorbit_inner_main(argc, argv);
}
