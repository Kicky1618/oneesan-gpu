...
__device__ __forceinline__ int prerank_find_block_warp(Code q,const PreRankOrbitMap&m){
    unsigned mask=__activemask(); int lane=threadIdx.x&31,first=__ffs(mask)-1,last=31-__clz(mask);
    int a=0,b=0; Code qlast=__shfl_sync(mask,q,last);
    if(lane==first)a=prerank_find_block(q,m); if(lane==last)b=prerank_find_block(qlast,m);
    a=__shfl_sync(mask,a,first); b=__shfl_sync(mask,b,last);
    return a==b?a:prerank_find_block(q,m);
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){auto launchN=[&](Code n,auto fn,const char*w){if(!n)return;int nb=int(std::min<Code>(65535,(n+threads-1)/threads));fn(nb);ck(cudaGetLastError(),w);};
            if(fixLow){auto om=make_high_prerank_map(fmb,p,0),cm=make_high_prerank_map(fmb,p,1),xm=make_high_prerank_map(fmb,p,2);Code on=om.nblocks?om.end[om.nblocks-1]:0,cn=cm.nblocks?cm.end[cm.nblocks-1]:0,xn=xm.nblocks?xm.end[xm.nblocks-1]:0;launchN(on,[&](int nb){factor_high_prerank_owner_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,om);},"factor high prerank owner");launchN(cn+xn,[&](int nb){factor_high_prerank_closure_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,cm,xm);},"factor high prerank closure");}
