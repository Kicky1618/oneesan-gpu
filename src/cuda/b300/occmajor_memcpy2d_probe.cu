...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    bool upper2d=fixLow&&c.authUnifiedHost;if(const char*e=std::getenv("GRIDFP_UPPER_MEMCPY2D"))upper2d=upper2d&&std::atoi(e)!=0;else upper2d=false;
    if(upper2d){constexpr int S=FactorTablesHost::STRIDE;for(auto const&x:fmb){if(!x.stride||x.end==x.off)continue;FBlock gx=G_F_FULL_MAIN_BLOCKS[3*int(x.he)+int(x.c)];uint32_t lb=G_FACTOR.low_occ_base[size_t(fmask)*S+x.hs];size_t rows=size_t((x.end-x.off)/x.stride);ck(cudaMemcpy2DAsync(c.dA+x.off,size_t(x.stride)*sizeof(Count),c.mainBase+gx.off+lb,size_t(gx.stride)*sizeof(Count),size_t(x.stride)*sizeof(Count),rows,cudaMemcpyDeviceToDevice,c.sMain),"upper2d gather main");}for(auto const&x:fdb){if(!x.stride||x.end==x.off)continue;FBlock gx=G_F_FULL_BLOCK_BLOCKS[x.he];uint32_t lb=G_FACTOR.low_occ_base[size_t(fmask)*S+x.he];size_t rows=size_t((x.end-x.off)/x.stride);ck(cudaMemcpy2DAsync(c.dD+x.off,size_t(x.stride)*sizeof(Count),c.blockBase+gx.off+lb,size_t(gx.stride)*sizeof(Count),size_t(x.stride)*sizeof(Count),rows,cudaMemcpyDeviceToDevice,c.sBlock),"upper2d gather block");}}
    else{if(ms.size){if(lowerFlatIo){factor_lower_flat_io_kernel<false,false><<<bm,threads,0,c.sMain>>>(c.dA,ms.size);}else if(lowerRowIo){dim3 gd(std::max(1u,(mainMaxStride+unsigned(threads)-1)/unsigned(threads)),unsigned(std::max<Code>(1,std::min<Code>(65535,mainRows))));factor_lower_row_io_kernel<false,false><<<gd,threads,0,c.sMain>>>(c.dA,mainRows);}else if(pg.use_mi)interval_io_kernel<false,false><<<interval_blocks(pg.mi.size(),threads),threads,0,c.sMain>>>(c.dA,c.dIM,pg.mi.size());else gather_main_kernel<<<bm,threads,0,c.sMain>>>(c.dA,useMate?c.dMate:nullptr,ms.size);}if(ds.size){if(lowerFlatIo){factor_lower_flat_io_kernel<true,false><<<bd,threads,0,c.sBlock>>>(c.dD,ds.size);}else if(lowerRowIo){dim3 gd(std::max(1u,(blockMaxStride+unsigned(threads)-1)/unsigned(threads)),unsigned(std::max<Code>(1,std::min<Code>(65535,blockRows))));factor_lower_row_io_kernel<true,false><<<gd,threads,0,c.sBlock>>>(c.dD,blockRows);}else if(pg.use_di)interval_io_kernel<true,false><<<interval_blocks(pg.di.size(),threads),threads,0,c.sBlock>>>(c.dD,c.dID,pg.di.size());else gather_block_kernel<<<bd,threads,0,c.sBlock>>>(c.dD,ds.size);}}
