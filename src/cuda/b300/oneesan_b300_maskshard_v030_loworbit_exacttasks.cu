// v0.30 experiment: v0.29 tight LOW BLOCKED launch plus exact row-depth task
// mapping.  Host launch size remains the v0.29 dense BLOCKED bd; the kernel
// grid-strides only over structurally reachable BLOCKED coordinates before
// saturation and falls back to physical order at full cap.
...
#define main oneesan_maskshard_v30_loworbitexacttasks_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.30 exact LOW orbit tasks require threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.30-loworbit-exacttasks-batch"
    ...
    return oneesan_maskshard_v30_loworbitexacttasks_inner_main(argc, argv);
}
