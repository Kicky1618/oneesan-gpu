...
static uint32_t factor_high_all_rank_host(uint32_t code,int h){
    auto b=G_FACTOR.high_all_codes.begin()+G_FACTOR.high_all_off[h],e=G_FACTOR.high_all_codes.begin()+G_FACTOR.high_all_off[h+1];
    auto it=std::lower_bound(b,e,code);if(it==e||*it!=code)throw std::runtime_error("factor interval high code missing");return uint32_t(it-b);
}
static void factor_append_peer_interval(std::vector<PeerInterval>&out,Code g,Code l,Code n,Code chunk,int ng){
    while(n){int owner=int(g/chunk);if(owner>=ng)owner=ng-1;Code shard0=Code(owner)*chunk,shardEnd=(owner+1<ng)?shard0+chunk:~Code(0);Code take=n;if(shardEnd!=~Code(0)&&g+take>shardEnd)take=shardEnd-g;Code remote=g-shard0;
        if(!out.empty()&&out.back().owner==(uint32_t)owner&&out.back().remote+out.back().len==remote&&out.back().local+out.back().len==l)out.back().len+=take;else out.push_back({remote,l,take,(uint32_t)owner,0});g+=take;l+=take;n-=take;}
}
static std::vector<PeerInterval> make_factor_peer_intervals(const std::vector<FBlock>&fb,uint32_t mask,Code chunk,int ng,bool block){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=FactorTablesHost::STRIDE;std::vector<PeerInterval> out;
    for(auto const&x:fb){if(!x.stride||x.end<=x.off)continue;size_t ix=size_t(mask)*S+x.he;uint32_t hb=G_FACTOR.high_mask_off[ix],he=G_FACTOR.high_mask_off[ix+1];Code hc=(x.end-x.off)/x.stride;if(Code(he-hb)!=hc)throw std::runtime_error("factor interval high count mismatch");
        for(uint32_t hr=0;hr<uint32_t(hc);++hr){uint32_t code=G_FACTOR.high_mask_codes[hb+hr],har=factor_high_all_rank_host(code,x.he);Code g=block?G_FACTOR.high_block_base[G_FACTOR.high_all_off[x.he]+har]:G_FACTOR.high_main_base[G_FACTOR.high_all_off[x.he]+har];if(!block){if(x.c>N)g+=H_DP[L][x.he];if(x.c>R&&x.he>0)g+=H_DP[L][x.he-1];}Code l=x.off+Code(hr)*x.stride;factor_append_peer_interval(out,g,l,x.stride,chunk,ng);}
    }
    // Keep intervals as natural LOW-rank runs; unlike the generic interval path they are already long and factor-aligned.
    return out;
}
...
static PreparedGroup prepare_group(int W,const WindowPlan&wp,int g,Code mc,Code bc,int ng){
    ...
    pg.use_mi=false;pg.use_di=false;pg.mi.clear();pg.di.clear();
    if(wp.p_hi<=LOW_LUT_K){uint32_t fmask=(pg.mo>>(LOW_LUT_K+1))&((1u<<HIGH_LUT_K)-1u);auto fmb=make_factor_main_blocks(false,fmask),fdb=make_factor_block_blocks(false,fmask);pg.mi=make_factor_peer_intervals(fmb,fmask,mc,ng,false);pg.di=make_factor_peer_intervals(fdb,fmask,bc,ng,true);pg.use_mi=true;pg.use_di=true;}
    return pg;
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    bool useOrbit=wp.p_hi<=LOW_LUT_K;size_t countBytes=size_t(useOrbit?(ms.size+ds.size):(2*ms.size+2*ds.size))*sizeof(Count),mateBytes=size_t(ms.size)*sizeof(MateID);bool useMate=!useOrbit&&!pg.use_mi&&(countBytes+mateBytes<=target);
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){
            {
                auto om=make_prerank_orbit_map(fmb,p,0);auto launchN=[&](Code n,auto fn,const char*w){if(!n)return;int nb=int(std::min<Code>(65535,(n+threads-1)/threads));fn(nb);ck(cudaGetLastError(),w);};Code on=om.nblocks?om.end[om.nblocks-1]:0;
                launchN(on,[&](int nb){factor_prerank_owner_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,om,p);},"factor prerank owner");
                auto cm=make_prerank_orbit_map(fmb,p,1),sm=make_prerank_orbit_map(fmb,p,2);Code cn=cm.nblocks?cm.end[cm.nblocks-1]:0,sn=sm.nblocks?sm.end[sm.nblocks-1]:0;launchN(cn,[&](int nb){factor_prerank_closure_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,cm,p);},"factor prerank closure");launchN(sn,[&](int nb){factor_prerank_highrr_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,sm,p);},"factor prerank high-RR closure");
            }
        }
        ck(cudaStreamSynchronize(c.sMain),"hybrid orbit transition sync");
    }else{
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t sparseBytes=0;uint64_t preRankBytes=uint64_t(G_PRERANK.owner_rec.size()+G_PRERANK.closure_rec.size()+G_PRERANK.highrr_rec.size())*8;uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+sparseBytes+preRankBytes;
        ...
        uint64_t minScratchBytes=0,actualScratchBytes=0,totalGroups=0,mateGroups=0;{const int rr[2][2]={{W-1,LOW_LUT_K+1},{LOW_LUT_K,1}};for(auto const&r:rr){auto fp=window_candidates(W,r[0],r[1]);int nj=1<<int(fp.size());for(int g=0;g<nj;++g){uint32_t mf,mo,bf,bo;window_masks(W,r[0],r[1],fp,uint32_t(g),mf,mo,bf,bo);auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);bool orbit=(r[0]<=LOW_LUT_K);uint64_t countBytes=uint64_t(orbit?(ms.size+ds.size):(2*ms.size+2*ds.size))*sizeof(Count),mateBytes=uint64_t(ms.size)*sizeof(MateID);minScratchBytes=std::max(minScratchBytes,countBytes);bool useMate=!orbit&&countBytes+mateBytes<=scratchLimit;actualScratchBytes=std::max(actualScratchBytes,countBytes+(useMate?mateBytes:0));totalGroups++;mateGroups+=useMate;}}}
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-vmm-sparseorbit-prerank-highrr-interval-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prO[d])cudaFree(prO[d]);if(prC[d])cudaFree(prC[d]);if(prH[d])cudaFree(prH[d]);}
}
