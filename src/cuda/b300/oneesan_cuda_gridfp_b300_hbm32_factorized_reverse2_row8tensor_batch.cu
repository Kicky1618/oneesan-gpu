...
struct DeviceCtx{
    ...
    void release_workspace(){if(dev<0)return;ck(cudaSetDevice(dev),"set release workspace");if(arena){cudaFree(arena);arena=nullptr;}if(dIM){cudaFree(dIM);dIM=nullptr;}if(dID){cudaFree(dID);dID=nullptr;}capArena=capIM=capID=0;dA=dB=dD=dE=nullptr;dMate=nullptr;}
    void destroy(){if(dev<0)return;cudaSetDevice(dev);release_workspace();if(copyDone)cudaEventDestroy(copyDone);if(clearDone)cudaEventDestroy(clearDone);if(mainDone)cudaEventDestroy(mainDone);if(blockDone)cudaEventDestroy(blockDone);if(sMain)cudaStreamDestroy(sMain);if(sBlock)cudaStreamDestroy(sBlock);}
};
...
#include "row8_runtime_bounded.cuh"
#include "row8_tensor_init.cuh"
#include "row8_runtime_mod_builder.cuh"
...
int main(int argc,char**argv){
    ...
    int configuredPrefixK=5;if(const char*e=std::getenv("GRIDFP_BOUNDED_PREFIX_K"))configuredPrefixK=std::max(1,std::min(W-2,std::atoi(e)));
    bool configuredDirect8=(configuredPrefixK==8);if(const char*e=std::getenv("GRIDFP_DIRECT_ROW8_TENSOR"))configuredDirect8=std::atoi(e)!=0;
    bool configuredStructural8=false;if(const char*e=std::getenv("GRIDFP_ROW8_STRUCTURAL"))configuredStructural8=std::atoi(e)!=0;
    bool configuredGap8=false;if(const char*e=std::getenv("GRIDFP_ROW8_GAP"))configuredGap8=std::atoi(e)!=0;
    bool row8CertCompare=false;if(const char*e=std::getenv("GRIDFP_ROW8_CERT_COMPARE"))row8CertCompare=std::atoi(e)!=0;
    if((configuredStructural8||configuredGap8)&&(!configuredDirect8||configuredPrefixK!=8)){std::cerr<<"GRIDFP_ROW8_STRUCTURAL/GRIDFP_ROW8_GAP requires direct row8 tensor prefixK=8\n";return 31;}
    if(configuredStructural8&&configuredGap8){std::cerr<<"GRIDFP_ROW8_STRUCTURAL and GRIDFP_ROW8_GAP are mutually exclusive\n";return 31;}
    if(row8CertCompare&&W-1<8){std::cerr<<"GRIDFP_ROW8_CERT_COMPARE requires n>=8\n";return 31;}
    double row8PrebuildS=0;bool row8Prebuild=!configuredStructural8&&!configuredGap8&&configuredDirect8&&configuredPrefixK==8&&mods.size()>1&&ng>1;if(const char*e=std::getenv("GRIDFP_ROW8_PREBUILD_CACHE"))row8Prebuild=!configuredStructural8&&!configuredGap8&&std::atoi(e)!=0;if(row8Prebuild){bool validate=false;if(const char*e=std::getenv("GRIDFP_ROW8_VALIDATE_BUILD"))validate=std::atoi(e)!=0;row8PrebuildS=oneesan::row8runtime::prebuild_mod_caches(mods,ng,validate);}
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        if(row8CertCompare){
            for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"row8 cert row1 set");Code total=Code(1)<<(W-1);Code mine=(total+ng-1-d)/ng;int blocks=int(std::min<Code>(65535,(mine+threads-1)/threads));if(mine)init_after_first_row_kernel<<<std::max(1,blocks),threads>>>(d,ng);}
            for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"row8 cert row1 sync set");ck(cudaDeviceSynchronize(),"row8 cert row1 sync");}
            for(int row=1;row<8;++row){for(auto const&pw:schedule){int nj=(int)pw.groups.size();std::atomic<int>next{0};std::vector<std::thread>ths;ths.reserve(ng);for(int d=0;d<ng;++d)ths.emplace_back([&,d]{for(;;){int q=next.fetch_add(1,std::memory_order_relaxed);if(q>=nj)break;process_group(ctx[d],W,pw.wp,pw.groups[q],threads,target);}});for(auto&t:ths)t.join();}std::cerr<<"row8-cert baseline row="<<row+1<<"/8 mod="<<mod<<"\n";}
            for(auto&c:ctx)c.release_workspace();
            unsigned long long mismatch=configuredGap8?oneesan::row8gap::compare_multi_gpu(W,mod,mp,ml,mc,ng):oneesan::row8struct::compare_multi_gpu(W,mod,mp,ml,mc,ng);
            double certWall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();
            std::cout<<"row8_cert_compare n="<<n<<" width="<<W<<" modulus="<<mod<<" gpus="<<ng<<" main_states="<<mainN<<" mismatch="<<mismatch<<" exact="<<(mismatch==0)<<" wall_s="<<certWall<<std::endl;
            if(mismatch)return 32;
            continue;
        }
        int prefixK=configuredPrefixK;
        ...
        bool requestDirect8=(prefixK==8);if(const char*e=std::getenv("GRIDFP_DIRECT_ROW8_TENSOR"))requestDirect8=std::atoi(e)!=0;
        if(ng!=1&&prefixK>1&&!((requestDirect5&&prefixK==5)||(requestDirect6&&prefixK==6)||(requestDirect8&&prefixK==8))){std::cerr<<"bounded prefix prototype currently requires NGPU=1 except direct row5/6/8\n";return 31;}
        if(prefixK==1){
            ...
            if(direct8&&prefixK==8){
                if(configuredGap8){
                    oneesan::row8gap::init_multi_gpu(W,mod,mp,ml,mc,ng);
                }else if(configuredStructural8){
                    oneesan::row8struct::init_multi_gpu(W,mod,mp,ml,mc,ng);
                }else{
                    bool row8Validate=false;if(const char*e=std::getenv("GRIDFP_ROW8_VALIDATE_BUILD"))row8Validate=std::atoi(e)!=0;auto row8md=oneesan::row8runtime::load_or_build_mod(mod,row8Validate);oneesan::row8tensor::init_multi_gpu_prebuilt(W,mod,mp,ml,mc,ng,row8md);
                }
            }else if(direct6&&prefixK==6){
        ...
        int done_windows=0;
        bool profileRows=false;if(const char*e=std::getenv("GRIDFP_PROFILE_ROWS"))profileRows=std::atoi(e)!=0;
        for(int row=prefixK;row<W-1;++row){
            auto rowWall0=std::chrono::steady_clock::now();
            std::vector<double> rowActive0;if(profileRows){rowActive0.resize(ng);for(int d=0;d<ng;++d)rowActive0[d]=ctx[d].active;}
            for(auto const&pw:schedule){int nj=(int)pw.groups.size();std::atomic<int>next{0};std::vector<std::thread>ths;ths.reserve(ng);for(int d=0;d<ng;++d)ths.emplace_back([&,d]{for(;;){int q=next.fetch_add(1,std::memory_order_relaxed);if(q>=nj)break;process_group(ctx[d],W,pw.wp,pw.groups[q],threads,target);}});for(auto&t:ths)t.join();++done_windows;}
            if(profileRows){double wallRow=std::chrono::duration<double>(std::chrono::steady_clock::now()-rowWall0).count(),amax=0,asum=0;for(int d=0;d<ng;++d){double z=ctx[d].active-rowActive0[d];amax=std::max(amax,z);asum+=z;}std::cerr<<"row-profile mod="<<mod<<" row="<<row+1<<"/"<<W<<" wall_s="<<wallRow<<" active_max_delta_s="<<amax<<" active_sum_delta_s="<<asum<<"\n";}
