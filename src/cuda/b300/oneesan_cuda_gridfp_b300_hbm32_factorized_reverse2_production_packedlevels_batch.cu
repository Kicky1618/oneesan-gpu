...
static PackedRow5Lut make_row5_prefix_lut_packed_levels(int len){
    PackedRow5Lut cur;std::vector<int8_t> h(1,1);cur.idx.assign(1,0);cur.data.assign(R5_BSZ[1],0);cur.data[row5_local_index()[0]]=1;size_t cnt=1;
    for(int d=0;d<len;++d){size_t nc=cnt*3;PackedRow5Lut nxt;nxt.idx.assign(nc,0xffffffffu);std::vector<int8_t> nh(nc,-127);uint64_t total=0;
        for(size_t c=0;c<cnt;++c)if(h[c]>=0){for(int a=0;a<3;++a){int h2=h[c]+row5_delta(a);if(h2<0||h2>5)continue;size_t q=c*3+a;nh[q]=int8_t(h2);nxt.idx[q]=uint32_t(total);total+=R5_BSZ[h2];}}
        if(total>0xffffffffULL)throw std::runtime_error("row5 packed level offset overflow");nxt.data.assign(size_t(total),0);
        row5_parallel_for(cnt,[&](size_t c){int hc=h[c];if(hc<0)return;const Count*in=cur.data.data()+cur.idx[c];for(int a=0;a<3;++a){size_t q=c*3+a;if(nh[q]<0)continue;row5_apply_row_compact(in,nxt.data.data()+nxt.idx[q],hc,a);}});
        cur=std::move(nxt);h=std::move(nh);cnt=nc;
    }return cur;
}
static PackedRow5Lut make_row5_suffix_lut_packed_levels(int len){
    PackedRow5Lut cur;std::vector<int8_t>h(1,0);cur.idx.assign(1,0);cur.data.assign(R5_BSZ[0],0);const auto&li=row5_local_index();for(int i=0;i<176;++i)if(G_R5_BETA[i])cur.data[li[i]]=G_R5_BETA[i];size_t cnt=1;
    for(int d=0;d<len;++d){size_t nc=cnt*3;PackedRow5Lut nxt;nxt.idx.assign(nc,0xffffffffu);std::vector<int8_t>nh(nc,-127);uint64_t total=0;
        for(int a=0;a<3;++a)for(size_t c=0;c<cnt;++c)if(h[c]>=0){int h2=h[c]-row5_delta(a);if(h2<0||h2>5)continue;size_t q=size_t(a)*cnt+c;nh[q]=int8_t(h2);nxt.idx[q]=uint32_t(total);total+=R5_BSZ[h2];}
        if(total>0xffffffffULL)throw std::runtime_error("row5 packed level offset overflow");nxt.data.assign(size_t(total),0);
        row5_parallel_for(cnt,[&](size_t c){int hc=h[c];if(hc<0)return;const Count*in=cur.data.data()+cur.idx[c];for(int a=0;a<3;++a){int h2=hc-row5_delta(a);if(h2<0||h2>5)continue;size_t q=size_t(a)*cnt+c;row5_apply_col_compact(in,nxt.data.data()+nxt.idx[q],hc,a);}});
        cur=std::move(nxt);h=std::move(nh);cnt=nc;
    }return cur;
}
...
static void init_direct_row5_lut_full_multi_gpu(int W,Count mod,int threads,Count**fullMain,const std::vector<Code>&mainLen,Code mainChunk,int ng){
    ...
    auto t0=std::chrono::steady_clock::now();PackedRow5Lut pref,suff;std::thread tp([&]{pref=make_row5_prefix_lut_packed_levels(hi);});std::thread ts([&]{suff=make_row5_suffix_lut_packed_levels(lo);});tp.join();ts.join();double host_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
