...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    ck(cudaGetLastError(),"doubleD gather");ck(cudaEventRecord(c.blockDone,c.sBlock),"record block gather done");ck(cudaStreamWaitEvent(c.sMain,c.blockDone,0),"main wait block gather");ck(cudaStreamSynchronize(c.sMain),"joined gather sync");auto tGather=std::chrono::steady_clock::now();
    ...
    ck(cudaGetLastError(),"orbit scatter");ck(cudaEventRecord(c.blockDone,c.sBlock),"record block scatter done");ck(cudaStreamWaitEvent(c.sMain,c.blockDone,0),"main wait block scatter");ck(cudaStreamSynchronize(c.sMain),"joined scatter sync");auto tScatter=std::chrono::steady_clock::now();c.groups++;c.gather_s+=std::chrono::duration<double>(tGather-t0).count();c.transition_s+=std::chrono::duration<double>(tTransition-tGather).count();c.scatter_s+=std::chrono::duration<double>(tScatter-tTransition).count();c.active+=std::chrono::duration<double>(tScatter-t0).count();
}
