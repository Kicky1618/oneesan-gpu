// v0.20 experiment: v0.19 exact BLOCKED tasks plus exact row-depth pruning
// inside HIGH closure source bodies. Closure launch/task geometry remains v0.11
// so v0.19 -> v0.20 isolates the source-body predicate.
...
#define main oneesan_maskshard_v20_highclosurerowdepth_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.20 HIGH closure row-depth requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.20-highrowpack16-rowdepthexact-exacttasks-highclosurerowdepth-batch"
    ...
    return oneesan_maskshard_v20_highclosurerowdepth_inner_main(argc, argv);
}
