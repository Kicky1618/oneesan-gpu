...
__global__ void factor_prerank_owner_blockorder_kernel(Count*mainv,Count*blockv,Code n,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<n;q+=st){
        int bb=f_find_block(q);FBlock bx=D_F_BLOCK_BLOCKS[bb];uint32_t loc=uint32_t(q-bx.off);uint32_t hr=loc/bx.stride,blr=loc-hr*bx.stride;Code obi=Code(p-1)*lowTotal+D_F_LOW_ALL_OFF[bx.he]+blr;
...
__global__ void factor_high_prerank_owner_blockorder_kernel(Count*mainv,Count*blockv,Code n,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<n;q+=st){
        int bb=f_find_block(q);FBlock bx=D_F_BLOCK_BLOCKS[bb];uint32_t loc=uint32_t(q-bx.off);uint32_t bhr=loc/bx.stride,lr=loc-bhr*bx.stride;Code obi=Code(p-LOW_LUT_K-1)*highTotal+D_F_HIGH_ALL_OFF[bx.he]+bhr;
