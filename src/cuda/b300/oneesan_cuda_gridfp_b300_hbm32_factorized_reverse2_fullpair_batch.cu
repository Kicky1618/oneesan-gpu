...
// Compose p=2 with the special p=1 transition.  p=1 drains the blocked
// vector completely, so only a main output kernel is needed.
__global__ void reverse2_p1_main_kernel(const Count*in,const Count*din,const MateID*mates,Code n,Count*out){
    constexpr int first=2, q=1;
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        MateID t=mates?mates[i]:factor_unrank_main(i); Count acc=reverse_eval_main_step(in,din,t,first);
        auto addp=[&](MateID m){Count c=reverse_eval_main_step(in,din,m,first);if(c)acc=add_mod_plain(acc,c);};
        switch(oneesan::gridfp::mpair(t,q)){
        case oneesan::gridfp::LR: addp(oneesan::gridfp::msetpair(t,q,oneesan::gridfp::NN)); break;
        case oneesan::gridfp::RN: addp(oneesan::gridfp::msetpair(t,q,oneesan::gridfp::NR)); break;
        case oneesan::gridfp::LN: addp(oneesan::gridfp::msetpair(t,q,oneesan::gridfp::NL)); break;
        case oneesan::gridfp::NR: addp(oneesan::gridfp::msetpair(t,q,oneesan::gridfp::RN)); break;
        case oneesan::gridfp::NL: addp(oneesan::gridfp::msetpair(t,q,oneesan::gridfp::LN)); break;
        case oneesan::gridfp::NN:{
            if(height_before_rank_pos<TARGET_W>(t,q)>0)addp(oneesan::gridfp::msetpair(t,q,oneesan::gridfp::RL));
            int ss=1;
            for(int r=2;r<TARGET_W&&ss>0;++r){
                auto v=oneesan::gridfp::mget(t,r);
                if(v==oneesan::gridfp::R&&ss==1){MateID c=oneesan::gridfp::msetpair(t,q,oneesan::gridfp::RR);c=oneesan::gridfp::mset(c,r,oneesan::gridfp::L);addp(c);}
                if(v==oneesan::gridfp::L)--ss;else if(v==oneesan::gridfp::R)++ss;
            }
            break;
        }
        default: break;
        }
        if(oneesan::gridfp::mget(t,q)==oneesan::gridfp::N){MateID b=oneesan::gridfp::mshrink(t,q);Count c=reverse_eval_block_step(in,b,first);if(c)acc=add_mod_plain(acc,c);}
        out[i]=acc;
    }
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;){
        if(p==2 && wp.p_lo==1){
            if(ms.size)reverse2_p1_main_kernel<<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,nxt);
            if(ds.size)ck(cudaMemsetAsync(dnext,0,size_t(ds.size)*sizeof(Count),c.sBlock),"p2p1 clear D");
            p-=2;
