...
__device__ unsigned long long D_HALFDBG[4];
...
__device__ __forceinline__ Code factor_rank_main_from_block(Code bi,MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;
    int bb=f_find_block(bi);FBlock bx=D_F_BLOCK_BLOCKS[bb];Code r=bi-bx.off;
    uint32_t hr=bx.stride?uint32_t(r/bx.stride):0;
    uint32_t lr=bx.stride?uint32_t(r-Code(hr)*bx.stride):0;
    int cv=int(mget(m,L));
    if(D_F_FIX_LOW){
        constexpr uint32_t HM=(1u<<(2*H))-1u;
        uint32_t hc=uint32_t((m>>(2*(L+1)))&HM);
        int he=seg_end_height(hc,H);
        uint32_t hp=D_F_HIGH_PACKED_RANK[hc],nhr=hp>>H;
        FBlock mx=D_F_MAIN_BLOCKS[3*he+cv];
        return mx.off+Code(nhr)*mx.stride+lr;
    }else{
        constexpr uint32_t LM=(1u<<(2*L))-1u;
        uint32_t lc=uint32_t(m)&LM;
        uint32_t lp=D_F_LOW_PACKED_RANK[ternary_index_dev(lc,L)],nlr=lp>>L;
        FBlock mx=D_F_MAIN_BLOCKS[3*int(bx.he)+cv];
        return mx.off+Code(hr)*mx.stride+nlr;
    }
}
...
__device__ __forceinline__ Count reverse_eval_main_step_from_block(const Count*in,const Count*din,Code bi,MateID t,int p){
    Code fasti=factor_rank_main_from_block(bi,t),fulli=factor_rank_main(t);if(fasti!=fulli){atomicAdd(&D_HALFDBG[D_F_FIX_LOW?0:1],1ULL);fasti=fulli;}else atomicAdd(&D_HALFDBG[D_F_FIX_LOW?2:3],1ULL);Count acc=in[fasti];
    MateID pred=0;bool have=false;
    switch(oneesan::gridfp::mpair(t,p)){
    case oneesan::gridfp::LR:pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::NN);have=true;break;
    case oneesan::gridfp::NR:pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::RN);have=true;break;
...
__device__ __forceinline__ void reverse2_add_main_from_block(Count&acc,const Count*in,const Count*din,Code bi,MateID m,int p){Count c=reverse_eval_main_step_from_block(in,din,bi,m,p);if(c)acc=add_mod_plain(acc,c);}
...
__global__ void reverse2_block_group_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<dn;i+=stride){
        ...
        if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
            reverse2_add_main_from_block(acc,in,din,i,oneesan::gridfp::minsert(b,q,oneesan::gridfp::N),p);
        }else if(low==oneesan::gridfp::N){
            MateID u=oneesan::gridfp::minsert(b,q-1,oneesan::gridfp::N);
            if(height_before_rank_pos<TARGET_W>(u,q)>0)
                reverse2_add_main_from_block(acc,in,din,i,oneesan::gridfp::msetpair(u,q,oneesan::gridfp::RL),p);
            int s=1;
            for(int r=q-2;r>=0&&s>0;--r){
                auto v=oneesan::gridfp::mget(u,r);
                if(v==oneesan::gridfp::L&&s==1){MateID c=oneesan::gridfp::msetpair(u,q,oneesan::gridfp::LL);c=oneesan::gridfp::mset(c,r,oneesan::gridfp::R);reverse2_add_main_from_block(acc,in,din,i,c,p);}
                if(v==oneesan::gridfp::L)++s;else if(v==oneesan::gridfp::R)--s;
            }
            s=1;
            for(int r=q+1;r<TARGET_W&&s>0;++r){
                auto v=oneesan::gridfp::mget(u,r);
                if(v==oneesan::gridfp::R&&s==1){MateID c=oneesan::gridfp::msetpair(u,q,oneesan::gridfp::RR);c=oneesan::gridfp::mset(c,r,oneesan::gridfp::L);reverse2_add_main_from_block(acc,in,din,i,c,p);}
                if(v==oneesan::gridfp::L)--s;else if(v==oneesan::gridfp::R)++s;
            }
        }
        out[i]=acc;
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        unsigned long long hz[4]{};for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"halfdbg set");ck(cudaMemcpyToSymbol(D_HALFDBG,hz,sizeof(hz)),"halfdbg zero");}
