...
// Evaluate one reverse Grid-FP step at an arbitrary main target when its
// local group rank is already known.  Main->main predecessors change only a
// local Mate slice, so use rank_known_t instead of re-ranking the whole
// factorized state through the dense segment tables.
__device__ __forceinline__ Count reverse_eval_main_step_known(const Count*in,const Count*din,MateID t,Code ti,int p){
    ...
    if(have){
        Code pi=rank_known_t<TARGET_W>(ti,t,pred,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);
        Count c=in[pi];if(c)acc=add_mod_plain(acc,c);
    }
    if(oneesan::gridfp::mget(t,p)==oneesan::gridfp::N){
        MateID b=oneesan::gridfp::mshrink(t,p);Count c=din[factor_rank_block(b)];if(c)acc=add_mod_plain(acc,c);
    }
...
__global__ void reverse2_main_group_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<n;i+=stride){
        ...
        if(have){
            Code pi=rank_known_t<TARGET_W>(i,t,pred,q,q-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);
            Count c=reverse_eval_main_step_known(in,din,pred,pi,p);if(c)acc=add_mod_plain(acc,c);
        }
