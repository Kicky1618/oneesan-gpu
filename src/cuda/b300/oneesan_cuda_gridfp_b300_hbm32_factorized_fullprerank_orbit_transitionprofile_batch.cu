...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    ck(cudaEventRecord(c.transStart,c.sMain),"transition event start");
    if(useOrbit){
        ...
        ck(cudaEventRecord(c.transEnd,c.sMain),"transition event end");ck(cudaEventSynchronize(c.transEnd),"transition event sync");float transMs=0;ck(cudaEventElapsedTime(&transMs,c.transStart,c.transEnd),"transition elapsed");c.gpu_transition_s+=double(transMs)*1e-3;
    }else{
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"set residue reset");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"set modulus");if(ml[d])ck(cudaMemset(mp[d],0,size_t(ml[d])*sizeof(Count)),"zero main");if(bl[d])ck(cudaMemset(bp[d],0,size_t(bl[d])*sizeof(Count)),"zero block");ck(cudaDeviceSynchronize(),"zero sync");ctx[d].active=0;ctx[d].gather_s=ctx[d].transition_s=ctx[d].gpu_transition_s=ctx[d].scatter_s=0;ctx[d].groups=0;}
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,gts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;gts+=c.gpu_transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-fullprerank-orbit-transitionprofile-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" gpu_transition_sum_s="<<gts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }
