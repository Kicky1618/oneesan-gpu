...
__constant__ int D_R2_STATS_ON;
__device__ unsigned long long D_R2_STATS[8];
...
__global__ void reverse2_block_group_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<dn;i+=stride){
        ...
        bool sample=D_R2_STATS_ON && (((i + Code(p)*0x9e3779b97f4a7c15ULL)&4095ULL)==0); unsigned ls=0,lc=0,rs=0,rc=0; bool rl=false;
        if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
            ...
            rl=height_before_rank_pos<TARGET_W>(u,q)>0;
            if(rl) reverse2_add_main(acc,in,din,oneesan::gridfp::msetpair(u,q,oneesan::gridfp::RL),p);
            int s=1;
            for(int r=q-2;r>=0&&s>0;--r){
                auto v=oneesan::gridfp::mget(u,r); if(sample)++ls;
                if(v==oneesan::gridfp::L&&s==1){if(sample)++lc;MateID c=oneesan::gridfp::msetpair(u,q,oneesan::gridfp::LL);c=oneesan::gridfp::mset(c,r,oneesan::gridfp::R);reverse2_add_main(acc,in,din,c,p);}
                if(v==oneesan::gridfp::L)++s;else if(v==oneesan::gridfp::R)--s;
            }
            s=1;
            for(int r=q+1;r<TARGET_W&&s>0;++r){
                auto v=oneesan::gridfp::mget(u,r); if(sample)++rs;
                if(v==oneesan::gridfp::R&&s==1){if(sample)++rc;MateID c=oneesan::gridfp::msetpair(u,q,oneesan::gridfp::RR);c=oneesan::gridfp::mset(c,r,oneesan::gridfp::L);reverse2_add_main(acc,in,din,c,p);}
                if(v==oneesan::gridfp::L)--s;else if(v==oneesan::gridfp::R)++s;
            }
        }
        if(sample){atomicAdd(&D_R2_STATS[0],1ULL);atomicAdd(&D_R2_STATS[low==oneesan::gridfp::N?2:1],1ULL);atomicAdd(&D_R2_STATS[3],(unsigned long long)rl);atomicAdd(&D_R2_STATS[4],ls);atomicAdd(&D_R2_STATS[5],lc);atomicAdd(&D_R2_STATS[6],rs);atomicAdd(&D_R2_STATS[7],rc);}
        out[i]=acc;
    }
}
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        int statsOn=1;unsigned long long statsZero[8]{};for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"stats set");ck(cudaMemcpyToSymbol(D_R2_STATS_ON,&statsOn,sizeof(statsOn)),"stats on");ck(cudaMemcpyToSymbol(D_R2_STATS,statsZero,sizeof(statsZero)),"stats zero");}
        ...
        unsigned long long hs[8]{};ck(cudaSetDevice(0),"stats copy set");ck(cudaMemcpyFromSymbol(hs,D_R2_STATS,sizeof(hs)),"stats copy");std::cerr<<"R2STATS samples="<<hs[0]<<" simple="<<hs[1]<<" N="<<hs[2]<<" rl="<<hs[3]<<" left_steps="<<hs[4]<<" left_cand="<<hs[5]<<" right_steps="<<hs[6]<<" right_cand="<<hs[7]<<"\n";
