...
__constant__ uint16_t D_TRIT7[1u<<14];
...
__device__ __forceinline__ uint32_t ternary_index_dev(uint32_t code,int len){uint32_t lo=D_TRIT7[code&0x3fffu];if(len<=7)return lo;uint32_t hi=D_TRIT7[(code>>14)&0x3fffu];return lo+POW3_7*hi;}
