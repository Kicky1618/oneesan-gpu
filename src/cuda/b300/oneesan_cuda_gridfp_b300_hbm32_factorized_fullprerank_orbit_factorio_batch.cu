...
static std::vector<PeerInterval> make_factor_lowall_peer_intervals(const std::vector<FBlock>& blocks,bool isBlock,uint32_t mask,Code chunk,int ng,bool& use){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=FactorTablesHost::STRIDE;
    std::vector<PeerInterval> raw;
    Code totalElems=0;
    for(auto const& x:blocks){
        if(!x.stride || x.end<=x.off) continue;
        uint32_t hc=uint32_t((x.end-x.off)/x.stride);
        uint32_t hb=G_FACTOR.high_mask_off[size_t(mask)*S+x.he];
        for(uint32_t hr=0;hr<hc;++hr){
            uint32_t code=G_FACTOR.high_mask_codes[hb+hr];
            uint32_t har=high_all_rank_host(code,x.he);
            Code g=isBlock?G_FACTOR.high_block_base[G_FACTOR.high_all_off[x.he]+har]:G_FACTOR.high_main_base[G_FACTOR.high_all_off[x.he]+har];
            if(!isBlock){if(x.c>N)g+=H_DP[L][x.he];if(x.c>R&&x.he>0)g+=H_DP[L][x.he-1];}
            Code l=x.off+Code(hr)*x.stride,left=x.stride; totalElems+=left;
            while(left){int owner=int(g/chunk);if(owner>=ng)owner=ng-1;Code shard0=Code(owner)*chunk;Code shardEnd=(owner+1<ng)?shard0+chunk:~Code(0);Code take=left;if(shardEnd!=~Code(0)&&g+take>shardEnd)take=shardEnd-g;Code remote=g-shard0;
                if(!raw.empty()&&raw.back().owner==uint32_t(owner)&&raw.back().remote+raw.back().len==remote&&raw.back().local+raw.back().len==l)raw.back().len+=take;else raw.push_back({remote,l,take,uint32_t(owner),0});
                g+=take;l+=take;left-=take;}
        }
    }
    if(raw.empty()){use=false;return {};}
    Code avg=totalElems/Code(raw.size());
    if(avg<256){use=false;return {};}
    std::vector<PeerInterval> tiled;size_t nt=0;for(auto const&x:raw)nt+=size_t((x.len+IO_TILE_ELEMS-1)/IO_TILE_ELEMS);tiled.reserve(nt);
    for(auto const&x:raw)for(Code off=0;off<x.len;off+=IO_TILE_ELEMS){Code take=std::min<Code>(x.len-off,Code(IO_TILE_ELEMS));tiled.push_back({x.remote+off,x.local+off,take,x.owner,0});}
    use=true;return tiled;
}
...
static PreparedGroup prepare_group(int W,const WindowPlan&wp,int g,Code mc,Code bc,int ng){
    ...
    if(!fixLow){uint32_t fmask=(pg.mo>>(LOW_LUT_K+1))&((1u<<HIGH_LUT_K)-1u);auto mb=make_factor_main_blocks(false,fmask),db=make_factor_block_blocks(false,fmask);pg.mi=make_factor_lowall_peer_intervals(mb,false,fmask,mc,ng,pg.use_mi);pg.di=make_factor_lowall_peer_intervals(db,true,fmask,bc,ng,pg.use_di);}
    else {pg.use_mi=false;pg.use_di=false;}
    return pg;
}
