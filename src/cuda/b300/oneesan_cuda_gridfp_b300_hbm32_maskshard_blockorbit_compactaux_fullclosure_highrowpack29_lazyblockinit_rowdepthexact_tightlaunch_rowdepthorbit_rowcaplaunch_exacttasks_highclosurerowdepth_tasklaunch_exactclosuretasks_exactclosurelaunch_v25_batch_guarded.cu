// v0.25 experiment: v0.24 LOW-closure work remains the numbered predecessor;
// this candidate isolates the HIGH closure row-pack threshold on top of the
// v0.23 HIGH exact-launch stack.  The task map and host launch table already use
// the compile-time threshold consistently; only dense LOW stride < 16 becomes < 29.
...
#define main oneesan_maskshard_v25_highclosurethreshold29_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.25 threshold-29 HIGH closure requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.25-highrowpack29-rowdepthexact-exacttasks-highclosureexacttasks-exactlaunch-batch"
    ...
    return oneesan_maskshard_v25_highclosurethreshold29_inner_main(argc, argv);
}
