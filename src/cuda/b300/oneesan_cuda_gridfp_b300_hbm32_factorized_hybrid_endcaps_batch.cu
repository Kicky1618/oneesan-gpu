...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        for(int d=0;d<ng;++d){
            ck(cudaSetDevice(d),"set row1 init device");
            Code total=Code(1)<<(W-1); Code mine=(total+ng-1-d)/ng;
            int blocks=int(std::min<Code>(65535,(mine+threads-1)/threads));
            if(mine)init_after_first_row_kernel<<<std::max(1,blocks),threads>>>(d,ng);
        }
        for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"row1 init sync set");ck(cudaDeviceSynchronize(),"row1 init sync");}
        int done_windows=0;
        for(int row=1;row<W-1;++row){for(auto const&pw:schedule){int nj=(int)pw.groups.size();std::atomic<int>next{0};std::vector<std::thread>ths;ths.reserve(ng);for(int d=0;d<ng;++d)ths.emplace_back([&,d]{for(;;){int q=next.fetch_add(1,std::memory_order_relaxed);if(q>=nj)break;process_group(ctx[d],W,pw.wp,pw.groups[q],threads,target);}});for(auto&t:ths)t.join();++done_windows;}std::cerr<<"mod "<<(ri+1)<<"/"<<mods.size()<<" p="<<mod<<" row "<<row+1<<"/"<<W<<"\n";}
        Count ans=0;
        for(int d=0;d<ng;++d){
