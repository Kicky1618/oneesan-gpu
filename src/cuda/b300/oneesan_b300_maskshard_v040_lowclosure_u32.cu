// v0.40 experiment: v0.39 packed/cached LOW group config plus uint32 task
// prefix/local arithmetic in the exact LOW closure kernel.  State addresses
// remain 64-bit Code.
...
#define main oneesan_maskshard_v40_lowclosureu32_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.40 LOW closure u32 requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.40-lowclosure-u32-batch"
        << " low_closure_task_u32=1 low_group_packed_cache=1"
        << " low_orbit_warp_row_u32=1 low_group_sync=1 guarded_hbm=1\n";
    const int rc = oneesan_maskshard_v40_lowclosureu32_inner_main(argc, argv);
    ...
    if (rc == 0 && skipped != executed * skips_per_group) {
        std::cerr << "v0.40 LOW group sync count mismatch skipped=" << skipped
                  << " executed=" << executed << '\n';
        return 4;
    }
    return rc;
