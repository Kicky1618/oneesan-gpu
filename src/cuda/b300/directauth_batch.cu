...
__device__ __forceinline__ Count* global_main_ptr(Code g){int o=int(g/D_MAIN_CHUNK);if(o>=D_NGPU)o=D_NGPU-1;return D_MAIN_PTR[o]+g-Code(o)*D_MAIN_CHUNK;}
__device__ __forceinline__ Count* global_block_ptr(Code g){int o=int(g/D_BLOCK_CHUNK);if(o>=D_NGPU)o=D_NGPU-1;return D_BLOCK_PTR[o]+g-Code(o)*D_BLOCK_CHUNK;}
...
__device__ __forceinline__ uint32_t direct_low_all_rank(int h,uint32_t lr){
    constexpr int L=LOW_LUT_K,S=MAXW+2;
    uint32_t lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+h]+lr];
    return D_F_LOW_DENSE_PACKED_RANK[lc]>>L;
}
__device__ __forceinline__ uint32_t direct_high_all_rank(int h,uint32_t hr){
    constexpr int H=HIGH_LUT_K,S=MAXW+2;
    uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+h]+hr];
    return D_F_HIGH_PACKED_RANK[hc]>>H;
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    bool directAuth=true;if(const char*e=std::getenv("GRIDFP_DIRECT_AUTH"))directAuth=std::atoi(e)!=0;
    if(!directAuth){
