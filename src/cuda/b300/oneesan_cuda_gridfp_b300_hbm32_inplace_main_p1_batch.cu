...
__global__ void main_p1_pairs_kernel(Count*io,const MateID*mates,Code n){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=io[i];if(!c)continue;MateID m=mates?mates[i]:unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,1);MateID t;Code j;
        ...
        else if(w==NL){t=msetpair(m,1,LN);j=rank_known_t<TARGET_W>(i,m,t,1,0,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);Count z=add_mod_plain(c,io[j]);io[i]=z;io[j]=z;}
    }
}
__global__ void main_p1_closure_kernel(Count*io,const MateID*mates,Code n){
    ...
    for(;i<n;i+=stride){Count c=io[i];if(!c)continue;MateID m=mates?mates[i]:unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,1);MateID t;int q,s;Code j;
        if(w==LL){t=msetpair(m,1,NN);q=0;s=1;while(s){--q;auto v=mget(t,q);if(v==L)++s;else if(v==R)--s;}t=mset(t,q,L);j=rank_known_t<TARGET_W>(i,m,t,1,q,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(io+j,c);}
        else if(w==RR){t=msetpair(m,1,NN);q=1;s=1;while(s){++q;auto v=mget(t,q);if(v==L)--s;else if(v==R)++s;}t=mset(t,q,R);j=rank_known_t<TARGET_W>(i,m,t,q,0,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(io+j,c);}
        else if(w==RL){t=msetpair(m,1,NN);j=rank_known_t<TARGET_W>(i,m,t,1,0,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(io+j,c);}
    }
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(p>1){
            ...
            if(ds.size)blocked_group_kernel<<<bd,threads>>>(dcur,ds.size,cur,p);
...
int main(int argc,char**argv){
    ...
    for(int hi=W-1;hi>=1;){WindowPlan wp;bool found=false;for(int lo=std::max(1,hi-max_window+1);lo<=hi;++lo){auto t=plan_window(W,hi,lo,target);if(t.max_bytes&&t.max_bytes<=target){wp=std::move(t);found=true;break;}}if(!found){std::cerr<<"cannot fit window hi="<<hi<<"\n";return 4;}int k=wp.fixed_pos.size();int nj=1<<k;maxgroups=std::max(maxgroups,nj);PreparedWindow pw;pw.wp=wp;pw.groups.reserve(nj);for(int g=0;g<nj;++g)pw.groups.push_back(prepare_group(W,pw.wp,g,mc,bc,ng));std::sort(pw.groups.begin(),pw.groups.end(),[](auto const&a,auto const&b){return a.work>b.work;});schedule.push_back(std::move(pw));hi=wp.p_lo-1;}
