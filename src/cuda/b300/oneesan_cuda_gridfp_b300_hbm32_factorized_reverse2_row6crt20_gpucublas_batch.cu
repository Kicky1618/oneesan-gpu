...
static FactorTablesHost build_factor_tables(){
    ...
    f.low_dense_packed_rank.assign(size_t(1)<<(2*L),0xffffffffu);
    f.high_packed_rank.assign(size_t(1)<<(2*H),0xffffffffu);
    for(int h=0;h<=MAXW;++h){f.low_all_off[h]=f.low_all_codes.size();for(uint32_t r=0;r<la[h].size();++r){auto c=la[h][r];f.low_all_codes.push_back(c);f.low_dense_packed_rank[c]=r<<L;} }
    f.low_all_off[MAXW+1]=f.low_all_codes.size();
    f.low_mask_off.resize(size_t(LM)*S);
    for(uint32_t m=0;m<LM;++m)for(int h=0;h<S;++h){size_t ix=size_t(m)*S+h;f.low_mask_off[ix]=f.low_mask_codes.size();if(h<=MAXW)for(uint32_t r=0;r<lm[ix].size();++r){auto c=lm[ix][r];f.low_mask_codes.push_back(c);f.low_dense_packed_rank[c]=(f.low_dense_packed_rank[c]&~((1u<<L)-1u))|r;}}
    ...
    std::cerr<<"factor tables low_all="<<f.low_all_codes.size()<<" low_mask="<<f.low_mask_codes.size()<<" low_4ary_dense_mib="<<double(f.low_dense_packed_rank.size()*4)/(1<<20)<<" high_all="<<f.high_all_codes.size()<<" high_mask="<<f.high_mask_codes.size()<<" high_dense_mib="<<double(f.high_packed_rank.size()*4)/(1<<20)<<"\n";
    return f;
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    ck(cudaGetLastError(),"doubleD gather");ck(cudaDeviceSynchronize(),"doubleD gather sync");auto tGather=std::chrono::steady_clock::now();
    ...
    ck(cudaStreamSynchronize(c.sMain),"main sync");ck(cudaStreamSynchronize(c.sBlock),"block sync");auto tTransition=std::chrono::steady_clock::now();
...
static Code row6_prefix_full_base(uint32_t code,int len,int width){int d[32];for(int i=len-1;i>=0;--i){d[i]=code%3;code/=3;}Code rank=0;int h=1;for(int i=0;i<len;++i){int pos=width-1-i,v=d[i];if(v>0)rank+=H_DP[pos][h];if(v>1&&h>0)rank+=H_DP[pos][h-1];if(v==1)--h;else if(v==2)++h;}return rank;}
static Code row6_suffix_full_rank(uint32_t code,int len,int start_h){int d[32];for(int i=len-1;i>=0;--i){d[i]=code%3;code/=3;}Code rank=0;int h=start_h;for(int i=0;i<len;++i){int pos=len-1-i,v=d[i];if(v>0)rank+=H_DP[pos][h];if(v>1&&h>0)rank+=H_DP[pos][h-1];if(v==1)--h;else if(v==2)++h;}return rank;}
...
static std::pair<PackedRow6Lut,PackedRow6Lut> make_row6_luts_gpu_roundtrip_crt(
        int plen,int slen,Count mod,const std::vector<Count>&mat,const std::vector<Count>&beta,
        const uint32_t (&moff)[3][7],int initLocal,
        const std::vector<uint32_t>&pidxHost,uint64_t ptotal,
        const std::vector<uint32_t>&sidxHost,uint64_t stotal){
    auto all0=std::chrono::steady_clock::now();PackedRow6Lut pref,suff;pref.idx=pidxHost;suff.idx=sidxHost;pref.data.resize(ptotal);suff.data.resize(stotal);
    ck(cudaSetDevice(0),"r6 CRT GPU roundtrip set");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"r6 CRT GPU roundtrip mod");ck(cudaMemcpyToSymbol(D_R6_GPU_MAT_OFF,moff,sizeof(moff)),"r6 CRT GPU roundtrip mat off");
    Count*dmat=nullptr,*dbeta=nullptr;uint32_t*dpidx=nullptr,*dsidx=nullptr;ck(cudaMalloc(&dmat,mat.size()*sizeof(Count)),"r6 CRT GPU roundtrip mat");ck(cudaMalloc(&dbeta,beta.size()*sizeof(Count)),"r6 CRT GPU roundtrip beta");ck(cudaMalloc(&dpidx,pref.idx.size()*sizeof(uint32_t)),"r6 CRT GPU roundtrip pidx");ck(cudaMalloc(&dsidx,suff.idx.size()*sizeof(uint32_t)),"r6 CRT GPU roundtrip sidx");ck(cudaMemcpy(dmat,mat.data(),mat.size()*sizeof(Count),cudaMemcpyHostToDevice),"r6 CRT GPU roundtrip mat cp");ck(cudaMemcpy(dbeta,beta.data(),beta.size()*sizeof(Count),cudaMemcpyHostToDevice),"r6 CRT GPU roundtrip beta cp");ck(cudaMemcpy(dpidx,pref.idx.data(),pref.idx.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"r6 CRT GPU roundtrip pidx cp");ck(cudaMemcpy(dsidx,suff.idx.data(),suff.idx.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"r6 CRT GPU roundtrip sidx cp");
    int rt=128;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_THREADS"))rt=std::max(32,std::min(256,std::atoi(e)));int wt=128;if(const char*e=std::getenv("GRIDFP_ROW6_WIDE_THREADS"))wt=std::max(32,std::min(256,std::atoi(e)));Code wideMin=243;if(const char*e=std::getenv("GRIDFP_ROW6_WIDE_MIN"))wideMin=std::max<Code>(1,std::strtoull(e,nullptr,10));
    auto build_pref=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*sizeof(Count)),"r6 CRT GPU roundtrip pref base");ck(cudaMemset(cur,0,size_t(182)*sizeof(Count)),"r6 CRT GPU roundtrip pref zero");Count one=1;ck(cudaMemcpy(cur+initLocal,&one,sizeof(one),cudaMemcpyHostToDevice),"r6 CRT GPU roundtrip pref one");ck(cudaMalloc(&hp,1),"r6 CRT GPU roundtrip pref h");int8_t h1=1;ck(cudaMemcpy(hp,&h1,1,cudaMemcpyHostToDevice),"r6 CRT GPU roundtrip pref h1");for(int lev=0;lev<plen;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*sizeof(Count)),"r6 CRT GPU roundtrip pref next");ck(cudaMalloc(&hn,size_t(children)),"r6 CRT GPU roundtrip pref hn");bool wide=true;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_WIDE"))wide=std::atoi(e)!=0;if(wide&&children>=wideMin){int gb=int(std::min<Code>(children,2147483647));r6_prefix_level_wide_kernel<<<std::max(1,gb),wt>>>(parents,cur,hp,nxt,hn,dmat);}else{int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_prefix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);}ck(cudaGetLastError(),"r6 CRT GPU roundtrip pref launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}Count*packed=nullptr;ck(cudaMalloc(&packed,pref.data.size()*sizeof(Count)),"r6 CRT GPU roundtrip pref packed");int bl=int(std::min<Code>(65535,(parents+255)/256));r6_pack_level_kernel<<<std::max(1,bl),256>>>(parents,cur,hp,dpidx,packed);ck(cudaGetLastError(),"r6 CRT GPU roundtrip pref pack");cudaFree(cur);cudaFree(hp);return packed;};
    auto build_suff=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*sizeof(Count)),"r6 CRT GPU roundtrip suff base");ck(cudaMemset(cur,0,size_t(182)*sizeof(Count)),"r6 CRT GPU roundtrip suff zero");ck(cudaMemcpy(cur,dbeta,beta.size()*sizeof(Count),cudaMemcpyDeviceToDevice),"r6 CRT GPU roundtrip suff beta");ck(cudaMalloc(&hp,1),"r6 CRT GPU roundtrip suff h");int8_t h0=0;ck(cudaMemcpy(hp,&h0,1,cudaMemcpyHostToDevice),"r6 CRT GPU roundtrip suff h0");for(int lev=0;lev<slen;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*sizeof(Count)),"r6 CRT GPU roundtrip suff next");ck(cudaMalloc(&hn,size_t(children)),"r6 CRT GPU roundtrip suff hn");bool wide=true;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_WIDE"))wide=std::atoi(e)!=0;if(wide&&children>=wideMin){int gb=int(std::min<Code>(children,2147483647));r6_suffix_level_wide_kernel<<<std::max(1,gb),wt>>>(parents,cur,hp,nxt,hn,dmat);}else{int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_suffix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);}ck(cudaGetLastError(),"r6 CRT GPU roundtrip suff launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}Count*packed=nullptr;ck(cudaMalloc(&packed,suff.data.size()*sizeof(Count)),"r6 CRT GPU roundtrip suff packed");int bl=int(std::min<Code>(65535,(parents+255)/256));r6_pack_level_kernel<<<std::max(1,bl),256>>>(parents,cur,hp,dsidx,packed);ck(cudaGetLastError(),"r6 CRT GPU roundtrip suff pack");cudaFree(cur);cudaFree(hp);return packed;};
    auto k0=std::chrono::steady_clock::now();Count*dpref=build_pref();Count*dsuff=build_suff();ck(cudaDeviceSynchronize(),"r6 CRT GPU roundtrip tables");double kernel_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-k0).count();auto cp0=std::chrono::steady_clock::now();ck(cudaMemcpy(pref.data.data(),dpref,pref.data.size()*sizeof(Count),cudaMemcpyDeviceToHost),"r6 CRT GPU roundtrip pref back");ck(cudaMemcpy(suff.data.data(),dsuff,suff.data.size()*sizeof(Count),cudaMemcpyDeviceToHost),"r6 CRT GPU roundtrip suff back");double copy_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-cp0).count();cudaFree(dpref);cudaFree(dsuff);cudaFree(dmat);cudaFree(dbeta);cudaFree(dpidx);cudaFree(dsidx);std::cerr<<"row6 CRT GPU-LUT roundtrip kernel_s="<<kernel_s<<" copy_s="<<copy_s<<" total_s="<<std::chrono::duration<double>(std::chrono::steady_clock::now()-all0).count()<<" pref_mib="<<(pref.data.size()*4>>20)<<" suff_mib="<<(suff.data.size()*4>>20)<<"\\n";return {std::move(pref),std::move(suff)};
}

struct DevicePackedRow6Lut { Count* data=nullptr; uint32_t* idx=nullptr; size_t codes=0,elems=0; };
struct DeviceRow6LutPair { DevicePackedRow6Lut pref,suff; };
static void free_device_row6_lut(DevicePackedRow6Lut&z){if(z.data)cudaFree(z.data);if(z.idx)cudaFree(z.idx);z={};}
static DeviceRow6LutPair make_row6_luts_gpu_device_crt(
        int plen,int slen,Count mod,const std::vector<Count>&mat,const std::vector<Count>&beta,
        const uint32_t (&moff)[3][7],int initLocal,
        const std::vector<uint32_t>&pidxHost,uint64_t ptotal,
        const std::vector<uint32_t>&sidxHost,uint64_t stotal){
    auto all0=std::chrono::steady_clock::now();DeviceRow6LutPair z;z.pref.codes=pidxHost.size();z.pref.elems=ptotal;z.suff.codes=sidxHost.size();z.suff.elems=stotal;
    ck(cudaSetDevice(0),"r6 CRT GPU device set");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"r6 CRT GPU device mod");ck(cudaMemcpyToSymbol(D_R6_GPU_MAT_OFF,moff,sizeof(moff)),"r6 CRT GPU device mat off");
    Count*dmat=nullptr,*dbeta=nullptr;ck(cudaMalloc(&dmat,mat.size()*sizeof(Count)),"r6 CRT GPU device mat");ck(cudaMalloc(&dbeta,beta.size()*sizeof(Count)),"r6 CRT GPU device beta");ck(cudaMalloc(&z.pref.idx,pidxHost.size()*sizeof(uint32_t)),"r6 CRT GPU device pidx");ck(cudaMalloc(&z.suff.idx,sidxHost.size()*sizeof(uint32_t)),"r6 CRT GPU device sidx");ck(cudaMemcpy(dmat,mat.data(),mat.size()*sizeof(Count),cudaMemcpyHostToDevice),"r6 CRT GPU device mat cp");ck(cudaMemcpy(dbeta,beta.data(),beta.size()*sizeof(Count),cudaMemcpyHostToDevice),"r6 CRT GPU device beta cp");ck(cudaMemcpy(z.pref.idx,pidxHost.data(),pidxHost.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"r6 CRT GPU device pidx cp");ck(cudaMemcpy(z.suff.idx,sidxHost.data(),sidxHost.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"r6 CRT GPU device sidx cp");
    int rt=128;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_THREADS"))rt=std::max(32,std::min(256,std::atoi(e)));int wt=128;if(const char*e=std::getenv("GRIDFP_ROW6_WIDE_THREADS"))wt=std::max(32,std::min(256,std::atoi(e)));Code wideMin=243;if(const char*e=std::getenv("GRIDFP_ROW6_WIDE_MIN"))wideMin=std::max<Code>(1,std::strtoull(e,nullptr,10));
    auto build_pref=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*sizeof(Count)),"r6 CRT GPU device pref base");ck(cudaMemset(cur,0,size_t(182)*sizeof(Count)),"r6 CRT GPU device pref zero");Count one=1;ck(cudaMemcpy(cur+initLocal,&one,sizeof(one),cudaMemcpyHostToDevice),"r6 CRT GPU device pref one");ck(cudaMalloc(&hp,1),"r6 CRT GPU device pref h");int8_t h1=1;ck(cudaMemcpy(hp,&h1,1,cudaMemcpyHostToDevice),"r6 CRT GPU device pref h1");for(int lev=0;lev<plen;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*sizeof(Count)),"r6 CRT GPU device pref next");ck(cudaMalloc(&hn,size_t(children)),"r6 CRT GPU device pref hn");bool wide=true;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_WIDE"))wide=std::atoi(e)!=0;if(wide&&children>=wideMin){int gb=int(std::min<Code>(children,2147483647));r6_prefix_level_wide_kernel<<<std::max(1,gb),wt>>>(parents,cur,hp,nxt,hn,dmat);}else{int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_prefix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);}ck(cudaGetLastError(),"r6 CRT GPU device pref launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}ck(cudaMalloc(&z.pref.data,z.pref.elems*sizeof(Count)),"r6 CRT GPU device pref packed");int bl=int(std::min<Code>(65535,(parents+255)/256));r6_pack_level_kernel<<<std::max(1,bl),256>>>(parents,cur,hp,z.pref.idx,z.pref.data);ck(cudaGetLastError(),"r6 CRT GPU device pref pack");cudaFree(cur);cudaFree(hp);};
    auto build_suff=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*sizeof(Count)),"r6 CRT GPU device suff base");ck(cudaMemset(cur,0,size_t(182)*sizeof(Count)),"r6 CRT GPU device suff zero");ck(cudaMemcpy(cur,dbeta,beta.size()*sizeof(Count),cudaMemcpyDeviceToDevice),"r6 CRT GPU device suff beta");ck(cudaMalloc(&hp,1),"r6 CRT GPU device suff h");int8_t h0=0;ck(cudaMemcpy(hp,&h0,1,cudaMemcpyHostToDevice),"r6 CRT GPU device suff h0");for(int lev=0;lev<slen;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*sizeof(Count)),"r6 CRT GPU device suff next");ck(cudaMalloc(&hn,size_t(children)),"r6 CRT GPU device suff hn");bool wide=true;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_WIDE"))wide=std::atoi(e)!=0;if(wide&&children>=wideMin){int gb=int(std::min<Code>(children,2147483647));r6_suffix_level_wide_kernel<<<std::max(1,gb),wt>>>(parents,cur,hp,nxt,hn,dmat);}else{int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_suffix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);}ck(cudaGetLastError(),"r6 CRT GPU device suff launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}ck(cudaMalloc(&z.suff.data,z.suff.elems*sizeof(Count)),"r6 CRT GPU device suff packed");int bl=int(std::min<Code>(65535,(parents+255)/256));r6_pack_level_kernel<<<std::max(1,bl),256>>>(parents,cur,hp,z.suff.idx,z.suff.data);ck(cudaGetLastError(),"r6 CRT GPU device suff pack");cudaFree(cur);cudaFree(hp);};
    auto k0=std::chrono::steady_clock::now();build_pref();build_suff();ck(cudaDeviceSynchronize(),"r6 CRT GPU device tables");double kernel_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-k0).count();cudaFree(dmat);cudaFree(dbeta);std::cerr<<"row6 CRT GPU-LUT device kernel_s="<<kernel_s<<" total_s="<<std::chrono::duration<double>(std::chrono::steady_clock::now()-all0).count()<<" pref_mib="<<(z.pref.elems*4>>20)<<" suff_mib="<<(z.suff.elems*4>>20)<<"\\n";return z;
}
__global__ void row6_expand_bytes_kernel(const Count*packed,const uint32_t*idx,const uint32_t*codes,int rows,int k,int ld,int8_t*a0,int8_t*a1,int8_t*a2,int8_t*a3){
    unsigned long long total=static_cast<unsigned long long>(rows)*k;for(unsigned long long q=unsigned(blockIdx.x)*blockDim.x+threadIdx.x,st=unsigned(gridDim.x)*blockDim.x;q<total;q+=st){int r=int(q/k),j=int(q-r*static_cast<unsigned long long>(k));Count x=packed[Code(idx[codes[r]])+j];size_t o=size_t(ld)*r+j;a0[o]=int8_t(int(x&255u)-128);a1[o]=int8_t(int((x>>8)&255u)-128);a2[o]=int8_t(int((x>>16)&255u)-128);a3[o]=int8_t(int((x>>24)&255u)-128);}
}
__global__ void row6_byte_sums_kernel(const Count*packed,const uint32_t*idx,const uint32_t*codes,int rows,int k,int32_t*sums){
    for(int r=int(blockIdx.x)*blockDim.x+threadIdx.x,st=int(gridDim.x)*blockDim.x;r<rows;r+=st){const Count*v=packed+idx[codes[r]];int s0=0,s1=0,s2=0,s3=0;for(int j=0;j<k;++j){Count x=v[j];s0+=x&255u;s1+=(x>>8)&255u;s2+=(x>>16)&255u;s3+=(x>>24)&255u;}sums[r]=s0;sums[rows+r]=s1;sums[2*rows+r]=s2;sums[3*rows+r]=s3;}
}
__global__ void row6_make_corr_kernel(const int32_t*sums,int rows,int32_t*corr){
    int total=7*rows;for(int q=int(blockIdx.x)*blockDim.x+threadIdx.x,st=int(gridDim.x)*blockDim.x;q<total;q+=st){int e=q/rows,r=q-e*rows;int z=0;for(int a=0;a<4;++a){int b=e-a;if(b>=0&&b<4)z+=sums[a*rows+r];}corr[q]=128*z;}
}
__global__ void row6_gemm_update_kernel_crt(const int32_t* cmat,int ldc,int m,int n,
        const int32_t* rowCorr,const int32_t* colCorr,unsigned long long* acc,
        int npairs,int k,int reduceNow,const Code* pbase,const Code* srank,Count* fullMain,int writeNow){
    const unsigned long long mod=D_MOD;
    for(unsigned long long q=unsigned(blockIdx.x)*blockDim.x+threadIdx.x,st=unsigned(gridDim.x)*blockDim.x;q<static_cast<unsigned long long>(m)*n;q+=st){int r=int(q%m),c=int(q/m);long long de=static_cast<long long>(cmat[r+size_t(ldc)*c])+rowCorr[r]+colCorr[c]-static_cast<long long>(npairs)*16384LL*k;unsigned long long x=acc[q]*256ULL+static_cast<unsigned long long>(de);if(reduceNow)x%=mod;acc[q]=x;if(writeNow)fullMain[pbase[r]+srank[c]]=Count(x);}
}
static void row6_cublas_check_crt(cublasStatus_t st,const char*what){if(st!=CUBLAS_STATUS_SUCCESS)throw std::runtime_error(std::string("cuBLAS ")+what+" status="+std::to_string(int(st)));}
static double row6_gemm_fill_all_heights_crt(const PackedRow6Lut&pref,int plen,const PackedRow6Lut&suff,int slen,Count*fullMain,Count mod){
    cublasHandle_t handle=nullptr;row6_cublas_check_crt(cublasCreate(&handle),"create fill");auto all0=std::chrono::steady_clock::now();int tileMiB=192;if(const char*e=std::getenv("GRIDFP_ROW6_GEMM_TILE_MIB"))tileMiB=std::max(32,std::atoi(e));
    for(int h=0;h<=6;++h){std::vector<uint32_t>pc,sc;for(uint32_t c=0;c<pref.idx.size();++c)if(row6_prefix_code_height(c,plen)==h)pc.push_back(c);for(uint32_t c=0;c<suff.idx.size();++c)if(row6_suffix_code_height(c,slen)==h)sc.push_back(c);if(pc.empty()||sc.empty())continue;const int m=int(pc.size()),n=int(sc.size()),k=r6_block_size(h),kp=(k+15)&~15,mp=(m+15)&~15,nPad=(n+15)&~15,lda=kp,ldb=kp,ldc=mp;auto prep0=std::chrono::steady_clock::now();
        std::array<std::vector<int8_t>,4>A,B;std::array<std::vector<int32_t>,4>ars,bcs;for(int a=0;a<4;++a){A[a].assign(size_t(lda)*mp,0);B[a].assign(size_t(ldb)*nPad,0);ars[a].assign(m,0);bcs[a].assign(n,0);}for(int r=0;r<m;++r){const Count*v=pref.data.data()+pref.idx[pc[r]];for(int j=0;j<k;++j){Count x=v[j];for(int a=0;a<4;++a){int z=int((x>>(8*a))&255u);A[a][j+size_t(lda)*r]=int8_t(z-128);ars[a][r]+=z;}}}for(int r=0;r<n;++r){const Count*v=suff.data.data()+suff.idx[sc[r]];for(int j=0;j<k;++j){Count x=v[j];for(int a=0;a<4;++a){int z=int((x>>(8*a))&255u);B[a][j+size_t(ldb)*r]=int8_t(z-128);bcs[a][r]+=z;}}}
        std::array<std::vector<int32_t>,7>rc,cc;for(int e=0;e<7;++e){rc[e].resize(m);cc[e].resize(n);for(int r=0;r<m;++r){long long z=0;for(int a=0;a<4;++a){int b=e-a;if(b>=0&&b<4)z+=ars[a][r];}rc[e][r]=int32_t(128*z);}for(int c=0;c<n;++c){long long z=0;for(int b=0;b<4;++b){int a=e-b;if(a>=0&&a<4)z+=bcs[b][c];}cc[e][c]=int32_t(128*z);}}
        std::vector<Code>pbase(m),srank(n);for(int r=0;r<m;++r)pbase[r]=row6_prefix_full_base(pc[r],plen,plen+slen);for(int c=0;c<n;++c)srank[c]=row6_suffix_full_rank(sc[c],slen,h);double prep_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-prep0).count();std::array<int8_t*,4>dA{},dB{};for(int a=0;a<4;++a){ck(cudaMalloc(&dA[a],A[a].size()),"r6 CRT fill A");ck(cudaMalloc(&dB[a],B[a].size()),"r6 CRT fill B");ck(cudaMemcpy(dA[a],A[a].data(),A[a].size(),cudaMemcpyHostToDevice),"r6 CRT fill Acp");ck(cudaMemcpy(dB[a],B[a].data(),B[a].size(),cudaMemcpyHostToDevice),"r6 CRT fill Bcp");}
        int32_t*dRow=nullptr;Code*dPB=nullptr;ck(cudaMalloc(&dRow,size_t(m)*sizeof(int32_t)),"r6 CRT fill row");ck(cudaMalloc(&dPB,size_t(m)*sizeof(Code)),"r6 CRT fill pbase");ck(cudaMemcpy(dPB,pbase.data(),size_t(m)*sizeof(Code),cudaMemcpyHostToDevice),"r6 CRT fill pbase cp");const size_t budget=size_t(tileMiB)<<20;size_t maxCells=std::max<size_t>(size_t(m)*16,budget/12);int tileCols=int(std::min<size_t>(n,std::max<size_t>(16,maxCells/size_t(m))));if(tileCols<n)tileCols=std::max(16,(tileCols/16)*16);int maxTilePad=(std::min(tileCols,n)+15)&~15;int32_t*dC=nullptr,*dCol=nullptr;unsigned long long*dAcc=nullptr;Code*dSR=nullptr;ck(cudaMalloc(&dC,size_t(ldc)*maxTilePad*sizeof(int32_t)),"r6 CRT fill tiled C");ck(cudaMalloc(&dAcc,size_t(m)*std::min(tileCols,n)*sizeof(unsigned long long)),"r6 CRT fill tiled acc");ck(cudaMalloc(&dCol,size_t(std::min(tileCols,n))*sizeof(int32_t)),"r6 CRT fill tiled col");ck(cudaMalloc(&dSR,size_t(std::min(tileCols,n))*sizeof(Code)),"r6 CRT fill tiled srank");auto gpu0=std::chrono::steady_clock::now();int tiles=0;
        for(int c0=0;c0<n;c0+=tileCols){++tiles;int nt=std::min(tileCols,n-c0),ntPad=(nt+15)&~15;size_t outN=size_t(m)*nt;ck(cudaMemset(dAcc,0,outN*sizeof(unsigned long long)),"r6 CRT fill tiled acc zero");ck(cudaMemcpy(dSR,srank.data()+c0,size_t(nt)*sizeof(Code),cudaMemcpyHostToDevice),"r6 CRT fill tiled srank cp");for(int e=6;e>=0;--e){int nPairs=0;bool first=true;int32_t alpha=1;for(int a=0;a<4;++a){int b=e-a;if(b<0||b>=4)continue;++nPairs;int32_t beta0=first?0:1;row6_cublas_check_crt(cublasGemmEx(handle,CUBLAS_OP_T,CUBLAS_OP_N,mp,ntPad,kp,&alpha,dA[a],CUDA_R_8I,lda,dB[b]+size_t(ldb)*c0,CUDA_R_8I,ldb,&beta0,dC,CUDA_R_32I,ldc,CUBLAS_COMPUTE_32I,CUBLAS_GEMM_DEFAULT),"fill tiled gemmEx");first=false;}ck(cudaMemcpy(dRow,rc[e].data(),size_t(m)*sizeof(int32_t),cudaMemcpyHostToDevice),"r6 CRT fill tiled row cp");ck(cudaMemcpy(dCol,cc[e].data()+c0,size_t(nt)*sizeof(int32_t),cudaMemcpyHostToDevice),"r6 CRT fill tiled col cp");int blocks=int(std::min<size_t>(65535,(outN+255)/256));row6_gemm_update_kernel_crt<<<std::max(1,blocks),256>>>(dC,ldc,m,nt,dRow,dCol,dAcc,nPairs,k,e==3||e==0,dPB,dSR,fullMain,e==0);}}
        ck(cudaDeviceSynchronize(),"r6 CRT gemm tiled fill sync");double gpu_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-gpu0).count();bool ok=true;for(int q=0;q<8&&ok;++q){int r=(q*7919+17)%m,c=(q*1543+11)%n;Count got=0;ck(cudaMemcpy(&got,fullMain+pbase[r]+srank[c],sizeof(got),cudaMemcpyDeviceToHost),"r6 CRT tiled fill verify");const Count*a=pref.data.data()+pref.idx[pc[r]],*b=suff.data.data()+suff.idx[sc[c]];uint64_t want=0;for(int j=0;j<k;++j)want=(want+uint64_t(a[j])*b[j])%mod;if(got!=want){ok=false;std::cerr<<"row6 CRT tiled fill mismatch p="<<mod<<" h="<<h<<" r="<<r<<" c="<<c<<" got="<<got<<" want="<<want<<"\\n";}}std::cerr<<"row6 CRT gemm tiled fill p="<<mod<<" h="<<h<<" states="<<size_t(m)*n<<" m="<<m<<" n="<<n<<" k="<<k<<" tiles="<<tiles<<" tile_cols="<<tileCols<<" tile_mib="<<tileMiB<<" prep_s="<<prep_s<<" gpu_s="<<gpu_s<<" exact="<<(ok?1:0)<<"\\n";for(int a=0;a<4;++a){cudaFree(dA[a]);cudaFree(dB[a]);}cudaFree(dC);cudaFree(dAcc);cudaFree(dRow);cudaFree(dCol);cudaFree(dPB);cudaFree(dSR);if(!ok)throw std::runtime_error("row6 CRT tiled gemm fill verification failed");}
    cublasDestroy(handle);double sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-all0).count();std::cerr<<"row6 CRT gemm tiled fill all p="<<mod<<" total_s="<<sec<<"\\n";return sec;
}

static double row6_gemm_fill_all_heights_device_crt(const DevicePackedRow6Lut&pref,const std::vector<uint32_t>&pidxHost,int plen,const DevicePackedRow6Lut&suff,const std::vector<uint32_t>&sidxHost,int slen,Count*fullMain,Count mod){
    cublasHandle_t handle=nullptr;row6_cublas_check_crt(cublasCreate(&handle),"create device fill");auto all0=std::chrono::steady_clock::now();int tileMiB=192;if(const char*e=std::getenv("GRIDFP_ROW6_GEMM_TILE_MIB"))tileMiB=std::max(32,std::atoi(e));
    for(int h=0;h<=6;++h){std::vector<uint32_t>pc,sc;for(uint32_t c=0;c<pref.codes;++c)if(row6_prefix_code_height(c,plen)==h)pc.push_back(c);for(uint32_t c=0;c<suff.codes;++c)if(row6_suffix_code_height(c,slen)==h)sc.push_back(c);if(pc.empty()||sc.empty())continue;const int m=int(pc.size()),n=int(sc.size()),k=r6_block_size(h),kp=(k+15)&~15,mp=(m+15)&~15,nPad=(n+15)&~15,lda=kp,ldb=kp,ldc=mp;auto prep0=std::chrono::steady_clock::now();
        uint32_t*dPC=nullptr,*dSC=nullptr;ck(cudaMalloc(&dPC,size_t(m)*4),"r6 CRT device pc");ck(cudaMalloc(&dSC,size_t(n)*4),"r6 CRT device sc");ck(cudaMemcpy(dPC,pc.data(),size_t(m)*4,cudaMemcpyHostToDevice),"r6 CRT device pc cp");ck(cudaMemcpy(dSC,sc.data(),size_t(n)*4,cudaMemcpyHostToDevice),"r6 CRT device sc cp");std::array<int8_t*,4>dA{},dB{};for(int a=0;a<4;++a){ck(cudaMalloc(&dA[a],size_t(lda)*mp),"r6 CRT device A");ck(cudaMalloc(&dB[a],size_t(ldb)*nPad),"r6 CRT device B");ck(cudaMemset(dA[a],0,size_t(lda)*mp),"r6 CRT device A zero");ck(cudaMemset(dB[a],0,size_t(ldb)*nPad),"r6 CRT device B zero");}
        int blocksA=int(std::min<unsigned long long>(65535,(static_cast<unsigned long long>(m)*k+255)/256)),blocksB=int(std::min<unsigned long long>(65535,(static_cast<unsigned long long>(n)*k+255)/256));row6_expand_bytes_kernel<<<std::max(1,blocksA),256>>>(pref.data,pref.idx,dPC,m,k,lda,dA[0],dA[1],dA[2],dA[3]);row6_expand_bytes_kernel<<<std::max(1,blocksB),256>>>(suff.data,suff.idx,dSC,n,k,ldb,dB[0],dB[1],dB[2],dB[3]);int32_t*dAS=nullptr,*dBS=nullptr,*dRC=nullptr,*dCC=nullptr;ck(cudaMalloc(&dAS,size_t(4)*m*sizeof(int32_t)),"r6 CRT device Asum");ck(cudaMalloc(&dBS,size_t(4)*n*sizeof(int32_t)),"r6 CRT device Bsum");ck(cudaMalloc(&dRC,size_t(7)*m*sizeof(int32_t)),"r6 CRT device rc");ck(cudaMalloc(&dCC,size_t(7)*n*sizeof(int32_t)),"r6 CRT device cc");int rb=(m+255)/256,cb=(n+255)/256;row6_byte_sums_kernel<<<std::max(1,rb),256>>>(pref.data,pref.idx,dPC,m,k,dAS);row6_byte_sums_kernel<<<std::max(1,cb),256>>>(suff.data,suff.idx,dSC,n,k,dBS);row6_make_corr_kernel<<<std::max(1,(7*m+255)/256),256>>>(dAS,m,dRC);row6_make_corr_kernel<<<std::max(1,(7*n+255)/256),256>>>(dBS,n,dCC);ck(cudaGetLastError(),"r6 CRT device byte prep");
        std::vector<Code>pbase(m),srank(n);for(int r=0;r<m;++r)pbase[r]=row6_prefix_full_base(pc[r],plen,plen+slen);for(int c=0;c<n;++c)srank[c]=row6_suffix_full_rank(sc[c],slen,h);Code*dPB=nullptr;ck(cudaMalloc(&dPB,size_t(m)*sizeof(Code)),"r6 CRT device pbase");ck(cudaMemcpy(dPB,pbase.data(),size_t(m)*sizeof(Code),cudaMemcpyHostToDevice),"r6 CRT device pbase cp");double prep_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-prep0).count();
        const size_t budget=size_t(tileMiB)<<20;size_t maxCells=std::max<size_t>(size_t(m)*16,budget/12);int tileCols=int(std::min<size_t>(n,std::max<size_t>(16,maxCells/size_t(m))));if(tileCols<n)tileCols=std::max(16,(tileCols/16)*16);int maxTilePad=(std::min(tileCols,n)+15)&~15;int32_t*dC=nullptr;unsigned long long*dAcc=nullptr;Code*dSR=nullptr;ck(cudaMalloc(&dC,size_t(ldc)*maxTilePad*sizeof(int32_t)),"r6 CRT device tiled C");ck(cudaMalloc(&dAcc,size_t(m)*std::min(tileCols,n)*sizeof(unsigned long long)),"r6 CRT device tiled acc");ck(cudaMalloc(&dSR,size_t(std::min(tileCols,n))*sizeof(Code)),"r6 CRT device tiled srank");auto gpu0=std::chrono::steady_clock::now();int tiles=0;
        for(int c0=0;c0<n;c0+=tileCols){++tiles;int nt=std::min(tileCols,n-c0),ntPad=(nt+15)&~15;size_t outN=size_t(m)*nt;ck(cudaMemset(dAcc,0,outN*sizeof(unsigned long long)),"r6 CRT device tiled acc zero");ck(cudaMemcpy(dSR,srank.data()+c0,size_t(nt)*sizeof(Code),cudaMemcpyHostToDevice),"r6 CRT device tiled srank cp");for(int e=6;e>=0;--e){int nPairs=0;bool first=true;int32_t alpha=1;for(int a=0;a<4;++a){int b=e-a;if(b<0||b>=4)continue;++nPairs;int32_t beta0=first?0:1;row6_cublas_check_crt(cublasGemmEx(handle,CUBLAS_OP_T,CUBLAS_OP_N,mp,ntPad,kp,&alpha,dA[a],CUDA_R_8I,lda,dB[b]+size_t(ldb)*c0,CUDA_R_8I,ldb,&beta0,dC,CUDA_R_32I,ldc,CUBLAS_COMPUTE_32I,CUBLAS_GEMM_DEFAULT),"device fill tiled gemmEx");first=false;}int blocks=int(std::min<size_t>(65535,(outN+255)/256));row6_gemm_update_kernel_crt<<<std::max(1,blocks),256>>>(dC,ldc,m,nt,dRC+size_t(e)*m,dCC+size_t(e)*n+c0,dAcc,nPairs,k,e==3||e==0,dPB,dSR,fullMain,e==0);}}
        ck(cudaDeviceSynchronize(),"r6 CRT device gemm fill sync");double gpu_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-gpu0).count();bool ok=true;for(int q=0;q<8&&ok;++q){int r=(q*7919+17)%m,c=(q*1543+11)%n;Count got=0;ck(cudaMemcpy(&got,fullMain+pbase[r]+srank[c],sizeof(got),cudaMemcpyDeviceToHost),"r6 CRT device verify out");std::vector<Count>av(k),bv(k);ck(cudaMemcpy(av.data(),pref.data+pidxHost[pc[r]],size_t(k)*sizeof(Count),cudaMemcpyDeviceToHost),"r6 CRT device verify A");ck(cudaMemcpy(bv.data(),suff.data+sidxHost[sc[c]],size_t(k)*sizeof(Count),cudaMemcpyDeviceToHost),"r6 CRT device verify B");uint64_t want=0;for(int j=0;j<k;++j)want=(want+uint64_t(av[j])*bv[j])%mod;if(got!=want){ok=false;std::cerr<<"row6 CRT device fill mismatch p="<<mod<<" h="<<h<<" got="<<got<<" want="<<want<<"\\n";}}std::cerr<<"row6 CRT device gemm p="<<mod<<" h="<<h<<" states="<<size_t(m)*n<<" tiles="<<tiles<<" prep_s="<<prep_s<<" gpu_s="<<gpu_s<<" exact="<<(ok?1:0)<<"\\n";
        for(int a=0;a<4;++a){cudaFree(dA[a]);cudaFree(dB[a]);}cudaFree(dPC);cudaFree(dSC);cudaFree(dAS);cudaFree(dBS);cudaFree(dRC);cudaFree(dCC);cudaFree(dPB);cudaFree(dC);cudaFree(dAcc);cudaFree(dSR);if(!ok)throw std::runtime_error("row6 CRT device gemm verification failed");}
    cublasDestroy(handle);double sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-all0).count();std::cerr<<"row6 CRT device gemm all p="<<mod<<" total_s="<<sec<<"\\n";return sec;
}
...
static void init_direct_row6_lut_full_multi_gpu(int W,Count mod,int threads,Count**fullMain,const std::vector<Code>&mainLen,Code mainChunk,int ng){
    ...
    bool cublasInit=false;if(const char*e=std::getenv("GRIDFP_ROW6_CUBLAS_INIT"))cublasInit=std::atoi(e)!=0;
    if(cublasInit){if(ng!=1)throw std::runtime_error("row6 CRT cuBLAS initializer currently requires NGPU=1");bool devicePath=true;if(const char*e=std::getenv("GRIDFP_ROW6_CUBLAS_DEVICE"))devicePath=std::atoi(e)!=0;ck(cudaSetDevice(0),"r6 CRT cublas set");ck(cudaMemcpyToSymbol(D_BOUND_DP,dp,sizeof(dp)),"r6 CRT cublas dp");int cap=6;ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"r6 CRT cublas cap");if(devicePath){auto l0=std::chrono::steady_clock::now();auto z=make_row6_luts_gpu_device_crt(hi,lo,mod,mat,beta,moff,initLocal,pi.first,pi.second,si.first,si.second);double lut_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-l0).count();double gemm_s=row6_gemm_fill_all_heights_device_crt(z.pref,pi.first,hi,z.suff,si.first,lo,fullMain[0],mod);free_device_row6_lut(z.pref);free_device_row6_lut(z.suff);std::cerr<<"direct row6 CRT cuBLAS-device states="<<n<<" p="<<mod<<" setup_host_s="<<host_s<<" lut_s="<<lut_s<<" gemm_s="<<gemm_s<<"\n";}else{auto l0=std::chrono::steady_clock::now();auto z=make_row6_luts_gpu_roundtrip_crt(hi,lo,mod,mat,beta,moff,initLocal,pi.first,pi.second,si.first,si.second);double lut_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-l0).count();double gemm_s=row6_gemm_fill_all_heights_crt(z.first,hi,z.second,lo,fullMain[0],mod);std::cerr<<"direct row6 CRT cuBLAS-host states="<<n<<" p="<<mod<<" setup_host_s="<<host_s<<" lut_s="<<lut_s<<" gemm_s="<<gemm_s<<"\n";}return;}
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+vb4(G_FACTOR.low_dense_packed_rank)+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+vb4(G_FACTOR.high_packed_rank)+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base);
