...
#define HIGH_LUT_K 0
#endif
#ifndef FAST_MOD_SHIFT
#define FAST_MOD_SHIFT 26
#endif
#ifndef FAST_MOD_K
#define FAST_MOD_K 7
#endif
static constexpr uint32_t FAST_MOD_VALUE=(uint32_t(FAST_MOD_K)<<FAST_MOD_SHIFT)+1u;
...
__device__ __forceinline__ Count reduce_small(Count x){
    constexpr uint32_t B=1u<<FAST_MOD_SHIFT, MASK=B-1u;
    uint32_t a=x>>FAST_MOD_SHIFT,b=x&MASK,q=a/FAST_MOD_K,r=a-q*FAST_MOD_K;
    uint32_t y=(r<<FAST_MOD_SHIFT)+b;
    return y<q?y+FAST_MOD_VALUE-q:y-q;
}
