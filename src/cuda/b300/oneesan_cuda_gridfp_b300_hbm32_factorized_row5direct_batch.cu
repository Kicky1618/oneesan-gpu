...
__global__ void r5_build_prefix_kernel(Code total,int len,Count* out,const uint8_t* csrc,const Count* cco){
    ...
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<total;q+=st){
        ...
        for(int j=0;j<oneesan::row5mod::DIM;++j)out[Code(j)*total+q]=v[j*bd+tid];
    }
}
...
__global__ void init_after_five_rows_compactlut_kernel(Code total,int shard,int ng,const Count* pref,Code pn,const Count* suff,Code sn,int hlen){
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;;q+=st){Code brank=Code(shard)+q*Code(ng);if(brank>=total)break;Code r=brank,grank=0,pr=0,lr=0;int h=1,mid=0,idx=0;
        ...
        for(int j=0;j<oneesan::row5mod::DIM;++j){Count a=pref[Code(j)*pn+pr],b=suff[Code(j)*sn+si];if(a&&b)ans=r3_add(ans,pm_mul(a,b));}
        global_store_main(grank,ans);
    }
}
