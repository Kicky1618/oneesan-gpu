...
static HighPreRankTablesHost build_high_prerank_tables(){
    unsigned long long shOwner=0,shOwnerCross=0,shClosure=0,shClosureCross=0,shCrossLL=0,shCrossLLCross=0;
    auto shard_of=[](uint32_t m){const uint32_t b[9]={0,2151,3575,4593,5822,6647,7396,7927,8192};for(int i=0;i<8;++i)if(m>=b[i]&&m<b[i+1])return i;return 7;};
    std::unordered_map<uint32_t,unsigned long long> graph;graph.reserve(1<<18);
    auto edge=[&](uint32_t a,uint32_t b,unsigned long long w){if(a==b||!w)return;if(a>b)std::swap(a,b);uint32_t k=(a<<13)|b;graph[k]+=w;};
    ...
    for(auto r:z.owner_block_rec)if(r==~0ULL)throw std::runtime_error("high blocked-order owner missing");double mib=(double(z.owner_block_rec.size()+z.crossll_rec.size()+z.closure_inv_rec.size())*8.0+double(z.closure_inv_src.size())*4.0)/(1<<20);std::cerr<<"high pre-rank owner="<<z.owner_block_rec.size()<<" closure_fast="<<z.closure_inv_src.size()<<" closure_inv="<<z.closure_inv_rec.size()<<" closure_crossll="<<z.crossll_rec.size()<<" device_mib="<<mib<<"\n";z.owner_rec.clear();z.owner_rec.shrink_to_fit();z.closure_rec.clear();z.closure_rec.shrink_to_fit();std::cerr<<"SHARDPROBE upper owner_tasks="<<shOwner<<" owner_cross="<<shOwnerCross<<" owner_cross_pct="<<(100.0*shOwnerCross/std::max<unsigned long long>(1,shOwner))<<" closure_tasks="<<shClosure<<" closure_cross="<<shClosureCross<<" closure_cross_pct="<<(100.0*shClosureCross/std::max<unsigned long long>(1,shClosure))<<" crossll_tasks="<<shCrossLL<<" crossll_cross="<<shCrossLLCross<<" crossll_cross_pct="<<(100.0*shCrossLLCross/std::max<unsigned long long>(1,shCrossLL))<<" graph_edges="<<graph.size()<<"\n";if(const char*e=std::getenv("GRIDFP_DUMP_SHARD_GRAPH");e&&std::atoi(e)!=0){std::ofstream f("work/high_occ_edges.tsv");for(auto const&kv:graph){uint32_t a=kv.first>>13,b=kv.first&8191;f<<a<<'\t'<<b<<'\t'<<kv.second<<'\n';}std::cerr<<"SHARDGRAPH dumped work/high_occ_edges.tsv\n";}return z;
}
