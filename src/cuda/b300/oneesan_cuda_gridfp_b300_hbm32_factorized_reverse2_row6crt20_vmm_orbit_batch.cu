...
__global__ void factor_main_block_orbit_kernel(Count*mainv,Count*blockv,const MateID*mates,Code n,int p){
    ...
    for(;i<n;i+=stride){
        ...
        Code dj=orbit_rank_block_from_main(i,m,p);
        Count d=blockv[dj];
        if(w==oneesan::gridfp::NN){
            MateID t=oneesan::gridfp::msetpair(m,p,oneesan::gridfp::LR);
            Code j=orbit_rank_main_pair_from(i,t,p);
...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dD=nullptr;MateID*dMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0,gather_s=0,transition_s=0,scatter_s=0;uint64_t groups=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
    ...
    void ensure(Code m,Code b,bool useMate,size_t im,size_t id){ck(cudaSetDevice(dev),"set ensure");auto al=[](size_t x){return(x+255)&~size_t(255);};size_t ab=al(size_t(m)*sizeof(Count)),db=al(size_t(b)*sizeof(Count)),mb=useMate?al(size_t(m)*sizeof(MateID)):0,need=ab+db+mb;if(need>capArena){if(arena)cudaFree(arena);capArena=need;ck(cudaMalloc(&arena,capArena),"scratch arena");}size_t off=0;dA=(Count*)(arena+off);off+=ab;dD=(Count*)(arena+off);off+=db;dMate=useMate?(MateID*)(arena+off):nullptr;if(im>capIM){if(dIM)cudaFree(dIM);capIM=im;ck(cudaMalloc(&dIM,capIM*sizeof(PeerInterval)),"interval main");}if(id>capID){if(dID)cudaFree(dID);capID=id;ck(cudaMalloc(&dID,capID*sizeof(PeerInterval)),"interval block");}maxIntervals=std::max({maxIntervals,im,id});}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    size_t countBytes=size_t(ms.size+ds.size)*sizeof(Count),mateBytes=size_t(ms.size)*sizeof(MateID);bool useMate=!pg.use_mi&&(countBytes+mateBytes<=target);
    ...
    ck(cudaStreamSynchronize(c.sMain),"orbit transition sync");auto tTransition=std::chrono::steady_clock::now();
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t minScratchBytes=0,actualScratchBytes=0,totalGroups=0,mateGroups=0;{const int rr[2][2]={{W-1,LOW_LUT_K+1},{LOW_LUT_K,1}};for(auto const&r:rr){auto fp=window_candidates(W,r[0],r[1]);int nj=1<<int(fp.size());for(int g=0;g<nj;++g){uint32_t mf,mo,bf,bo;window_masks(W,r[0],r[1],fp,uint32_t(g),mf,mo,bf,bo);auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);uint64_t countBytes=uint64_t(ms.size+ds.size)*sizeof(Count),mateBytes=uint64_t(ms.size)*sizeof(MateID);minScratchBytes=std::max(minScratchBytes,countBytes);bool useMate=countBytes+mateBytes<=scratchLimit;actualScratchBytes=std::max(actualScratchBytes,countBytes+(useMate?mateBytes:0));totalGroups++;mateGroups+=useMate;}}}
