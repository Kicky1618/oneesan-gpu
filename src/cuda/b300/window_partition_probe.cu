...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t minScratchBytes=0;{const int rr[2][2]={{W-1,LOW_LUT_K+1},{LOW_LUT_K,1}};for(auto const&r:rr){auto fp=window_candidates(W,r[0],r[1]);int nj=1<<int(fp.size());uint64_t mx=0;for(int g=0;g<nj;++g){uint32_t mf,mo,bf,bo;window_masks(W,r[0],r[1],fp,uint32_t(g),mf,mo,bf,bo);auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);mx=std::max<uint64_t>(mx,uint64_t(2*ms.size+2*ds.size)*sizeof(Count));}std::cerr<<"PLAN base_window hi="<<r[0]<<" lo="<<r[1]<<" active="<<(r[0]-r[1]+1)<<" fixed="<<fp.size()<<" groups="<<nj<<" max_gib="<<double(mx)/(1ull<<30)<<"\n";minScratchBytes=std::max(minScratchBytes,mx);}
        if(const char*wp=getenv("GRIDFP_WINDOW_PARTITION_SCAN");wp&&atoi(wp)){for(int parts=2;parts<=4;++parts){uint64_t peak=0,totalGroups=0;int pHi=W-1;std::cerr<<"PLAN partition parts="<<parts;for(int z=0;z<parts;++z){int remain=pHi;int left=parts-z;int len=(remain+left-1)/left;int pLo=std::max(1,pHi-len+1);auto fp=window_candidates(W,pHi,pLo);if(fp.size()>24){std::cerr<<" too_many_fixed="<<fp.size();peak=~0ull;break;}uint64_t nj=1ull<<fp.size(),mx=0;for(uint64_t g=0;g<nj;++g){uint32_t mf,mo,bf,bo;window_masks(W,pHi,pLo,fp,uint32_t(g),mf,mo,bf,bo);auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);mx=std::max<uint64_t>(mx,uint64_t(2*ms.size+2*ds.size)*sizeof(Count));}peak=std::max(peak,mx);totalGroups+=nj;std::cerr<<" ["<<pHi<<".."<<pLo<<" g="<<nj<<" gib="<<double(mx)/(1ull<<30)<<"]";pHi=pLo-1;}if(peak!=~0ull)std::cerr<<" peak_gib="<<double(peak)/(1ull<<30)<<" groups_per_row="<<totalGroups;std::cerr<<"\n";}}
    }
