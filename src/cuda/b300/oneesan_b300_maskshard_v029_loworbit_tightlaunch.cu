// v0.29 experiment: v0.28 plus BLOCKED-domain-sized LOW orbit launch.  The LOW
// orbit kernel and row-depth predicate are unchanged; only the host grid changes
// from MAIN-domain bm to BLOCKED-domain bd.
...
#define main oneesan_maskshard_v29_loworbittightlaunch_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.29 tight LOW orbit launch requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.29-loworbitrowdepth-tightlaunch-batch"
        ...
        << " low_block_orbit_tight_launch=1"
    ...
    return oneesan_maskshard_v29_loworbittightlaunch_inner_main(argc, argv);
}
