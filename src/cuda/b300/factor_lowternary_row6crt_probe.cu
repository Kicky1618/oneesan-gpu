...
static uint32_t ternary_index_host(uint32_t code,int len){uint32_t z=0,m=1;for(int p=0;p<len;++p){uint32_t v=(code>>(2*p))&3u;if(v>2)throw std::runtime_error("bad ternary symbol");z+=v*m;m*=3u;}return z;}
...
static FactorTablesHost build_factor_tables(){
    ...
    std::cerr<<"factor tables low_all="<<f.low_all_codes.size()<<" low_mask="<<f.low_mask_codes.size()<<" low_ternary_mib="<<double(f.low_dense_packed_rank.size()*4)/(1<<20)<<" high_all="<<f.high_all_codes.size()<<" high_mask="<<f.high_mask_codes.size()<<" high_dense_mib="<<double(f.high_packed_rank.size()*4)/(1<<20)<<"\n";
    return f;
}
...
__device__ __forceinline__ Code factor_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);int cv=int(mget(m,L));int bid=3*he+cv;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lp=D_F_LOW_DENSE_PACKED_RANK[ternary_index_dev(lc,L)],hp=D_F_HIGH_PACKED_RANK[hc];uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int h=seg_end_height(hc,H);FBlock x=D_F_BLOCK_BLOCKS[h];uint32_t lp=D_F_LOW_DENSE_PACKED_RANK[ternary_index_dev(lc,L)],hp=D_F_HIGH_PACKED_RANK[hc];uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_global_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[hc],lp=D_F_LOW_DENSE_PACKED_RANK[ternary_index_dev(lc,L)];uint32_t har=hp>>H,lar=lp>>L;Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[he]+har];MateValue c=mget(m,L);if(c>N)rank+=D_FULL_DP[L][he];if(c>R&&he>0)rank+=D_FULL_DP[L][he-1];return rank+lar;
}
__device__ __forceinline__ Code factor_global_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[hc],lp=D_F_LOW_DENSE_PACKED_RANK[ternary_index_dev(lc,L)];uint32_t har=hp>>H,lar=lp>>L;return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[he]+har]+lar;
}
