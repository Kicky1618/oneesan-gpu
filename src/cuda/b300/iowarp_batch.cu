...
__device__ __forceinline__ Code factor_main_high_base(FBlock x,uint32_t hr){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;uint32_t har;
    if(D_F_FIX_LOW)har=hr;
    else{uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];har=D_F_HIGH_PACKED_RANK[hc]>>H;}
    Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[x.he]+har];if(x.c>N)rank+=D_FULL_DP[L][x.he];if(x.c>R&&x.he>0)rank+=D_FULL_DP[L][x.he-1];return rank;
}
__device__ __forceinline__ Code factor_block_high_base(FBlock x,uint32_t hr){
    constexpr int H=HIGH_LUT_K,S=MAXW+2;uint32_t har;
    if(D_F_FIX_LOW)har=hr;
    else{uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];har=D_F_HIGH_PACKED_RANK[hc]>>H;}
    return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[x.he]+har];
}
__device__ __forceinline__ Code factor_global_rank_main_index(Code i){
    constexpr int L=LOW_LUT_K,S=MAXW+2;int b=f_find_main(i);FBlock x=D_F_MAIN_BLOCKS[b];Code r=i-x.off;uint32_t hr=x.stride?uint32_t(r/x.stride):0,lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;
    unsigned am=__activemask(),lane=threadIdx.x&31u,leader=__ffs(am)-1;uint32_t key=(uint32_t(b)<<20)^hr,k0=__shfl_sync(am,key,leader);bool same=__all_sync(am,key==k0);Code base;
    if(same){Code z=0;if(int(lane)==leader)z=factor_main_high_base(x,hr);base=__shfl_sync(am,z,leader);}else base=factor_main_high_base(x,hr);
    uint32_t lar;if(D_F_FIX_LOW){uint32_t lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];lar=D_F_LOW_DENSE_PACKED_RANK[lc]>>L;}else lar=lr;
    return base+lar;
}
__device__ __forceinline__ Code factor_global_rank_block_index(Code i){
    constexpr int L=LOW_LUT_K,S=MAXW+2;int b=f_find_block(i);FBlock x=D_F_BLOCK_BLOCKS[b];Code r=i-x.off;uint32_t hr=x.stride?uint32_t(r/x.stride):0,lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;
    unsigned am=__activemask(),lane=threadIdx.x&31u,leader=__ffs(am)-1;uint32_t key=(uint32_t(b)<<20)^hr,k0=__shfl_sync(am,key,leader);bool same=__all_sync(am,key==k0);Code base;
    if(same){Code z=0;if(int(lane)==leader)z=factor_block_high_base(x,hr);base=__shfl_sync(am,z,leader);}else base=factor_block_high_base(x,hr);
    uint32_t lar;if(D_F_FIX_LOW){uint32_t lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];lar=D_F_LOW_DENSE_PACKED_RANK[lc]>>L;}else lar=lr;
    return base+lar;
}
