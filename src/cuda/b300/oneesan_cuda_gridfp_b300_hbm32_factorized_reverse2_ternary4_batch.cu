...
__constant__ uint8_t* D_TRIT4_PTR;
...
__device__ __forceinline__ uint32_t ternary_index_dev(uint32_t code,int len){uint32_t z=__ldg(D_TRIT4_PTR+(code&0xffu));if(len>4)z+=81u*__ldg(D_TRIT4_PTR+((code>>8)&0xffu));if(len>8)z+=6561u*__ldg(D_TRIT4_PTR+((code>>16)&0xffu));if(len>12)z+=531441u*__ldg(D_TRIT4_PTR+((code>>24)&0xffu));return z;}
...
int main(int argc,char**argv){
    ...
    std::array<uint8_t,256> trit4{};for(uint32_t c=0;c<256;++c){uint32_t z=0,m=1;bool ok=true;for(int k=0;k<4;++k){uint32_t v=(c>>(2*k))&3u;if(v>2){ok=false;break;}z+=v*m;m*=3u;}trit4[c]=ok?uint8_t(z):uint8_t(0);}
