...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,int g,int threads,Code mc,Code bc,int ng){
    auto t0=std::chrono::steady_clock::now();ck(cudaSetDevice(c.dev),"set worker");
    uint32_t mf,mo,bf,bo;window_masks(W,wp.p_hi,wp.p_lo,wp.fixed_pos,(uint32_t)g,mf,mo,bf,bo);auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);if(!ms.size&&!ds.size)return;
    bool use_mi=false,use_di=false;
    auto mi=make_peer_intervals(ms,mc,ng,use_mi);auto di=make_peer_intervals(ds,bc,ng,use_di);c.ensure(ms.size,ds.size,mi.size(),di.size());
    if(!mi.empty())ck(cudaMemcpy(c.dIM,mi.data(),mi.size()*sizeof(PeerInterval),cudaMemcpyHostToDevice),"copy main intervals");
    if(!di.empty())ck(cudaMemcpy(c.dID,di.data(),di.size()*sizeof(PeerInterval),cudaMemcpyHostToDevice),"copy block intervals");
    ck(cudaMemcpyToSymbol(D_MAIN_DP,ms.dp,sizeof(ms.dp)),"main dp");ck(cudaMemcpyToSymbol(D_BLOCK_DP,ds.dp,sizeof(ds.dp)),"block dp");ck(cudaMemcpyToSymbol(D_MAIN_FIXED,&mf,sizeof(mf)),"mf");ck(cudaMemcpyToSymbol(D_MAIN_OCC,&mo,sizeof(mo)),"mo");ck(cudaMemcpyToSymbol(D_BLOCK_FIXED,&bf,sizeof(bf)),"bf");ck(cudaMemcpyToSymbol(D_BLOCK_OCC,&bo,sizeof(bo)),"bo");int mw=W,bw=W-1;ck(cudaMemcpyToSymbol(D_MAIN_W,&mw,sizeof(mw)),"mw");ck(cudaMemcpyToSymbol(D_BLOCK_W,&bw,sizeof(bw)),"bw");
    int bm=int(std::min<Code>(65535,(ms.size+threads-1)/threads)),bd=int(std::min<Code>(65535,(ds.size+threads-1)/threads));
    if(ms.size){if(use_mi)interval_io_kernel<false,false><<<interval_blocks(mi.size(),threads),threads>>>(c.dA,c.dIM,mi.size());else gather_main_kernel<<<bm,threads>>>(c.dA,ms.size);}
    if(ds.size){if(use_di)interval_io_kernel<true,false><<<interval_blocks(di.size(),threads),threads>>>(c.dD,c.dID,di.size());else gather_block_kernel<<<bd,threads>>>(c.dD,ds.size);}
    ...
    if(ms.size){if(use_mi)interval_io_kernel<false,true><<<interval_blocks(mi.size(),threads),threads>>>(cur,c.dIM,mi.size());else scatter_main_kernel<<<bm,threads>>>(cur,ms.size);}
    if(ds.size){if(use_di)interval_io_kernel<true,true><<<interval_blocks(di.size(),threads),threads>>>(c.dD,c.dID,di.size());else scatter_block_kernel<<<bd,threads>>>(c.dD,ds.size);}
    ck(cudaGetLastError(),"interval scatter");ck(cudaDeviceSynchronize(),"group sync");c.groups++;c.active+=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
}
