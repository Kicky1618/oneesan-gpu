...
struct FactorRuntimeCfg {
    FBlock main_blocks[64]; FBlock block_blocks[32];
    uint32_t mask; int main_n,block_n,fix_low;
};
__constant__ FactorRuntimeCfg D_FCFG;
#define D_F_MAIN_BLOCKS D_FCFG.main_blocks
#define D_F_BLOCK_BLOCKS D_FCFG.block_blocks
#define D_F_MAIN_NBLOCKS D_FCFG.main_n
#define D_F_BLOCK_NBLOCKS D_FCFG.block_n
#define D_F_MASK D_FCFG.mask
#define D_F_FIX_LOW D_FCFG.fix_low
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    int fm=(int)fmb.size(),fd=(int)fdb.size(),fl=fixLow?1:0;FactorRuntimeCfg fcfg{};std::memcpy(fcfg.main_blocks,fmb.data(),fmb.size()*sizeof(FBlock));std::memcpy(fcfg.block_blocks,fdb.data(),fdb.size()*sizeof(FBlock));fcfg.mask=fmask;fcfg.main_n=fm;fcfg.block_n=fd;fcfg.fix_low=fl;ck(cudaMemcpyToSymbol(D_FCFG,&fcfg,sizeof(fcfg)),"factor runtime config");
    ...
    // Full pre-rank orbit/direct-I/O kernels do not consume the legacy group ranker constants.
