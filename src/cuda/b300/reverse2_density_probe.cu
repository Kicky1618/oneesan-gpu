...
__global__ void count_nonzero_kernel(const Count* a,Code n,unsigned long long* out){
    __shared__ unsigned long long sh[256]; unsigned tid=threadIdx.x; unsigned long long c=0;
    for(Code i=Code(blockIdx.x)*blockDim.x+tid;i<n;i+=Code(gridDim.x)*blockDim.x)c += (a[i]!=0);
    sh[tid]=c; __syncthreads();
    for(unsigned d=blockDim.x/2;d;d>>=1){if(tid<d)sh[tid]+=sh[tid+d];__syncthreads();}
    if(tid==0)atomicAdd(out,sh[0]);
}
static unsigned long long count_nonzero_dev(const Count* a,Code n){
    unsigned long long *d=nullptr,h=0;ck(cudaMalloc(&d,sizeof(h)),"nz malloc");ck(cudaMemset(d,0,sizeof(h)),"nz zero");
    int b=int(std::min<Code>(4096,(n+255)/256));count_nonzero_kernel<<<b,256>>>(a,n,d);ck(cudaDeviceSynchronize(),"nz sync");ck(cudaMemcpy(&h,d,sizeof(h),cudaMemcpyDeviceToHost),"nz copy");cudaFree(d);return h;
}
