// v0.25 experiment: v0.24 plus exact row-depth LOW closure task mapping.
// Host launch geometry remains v0.9-sized; the kernel grid-strides only the
// exact active HIGH-row x selected-LOW-column task set in early rows.
...
#define main oneesan_maskshard_v25_lowclosureexacttasks_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.25 exact LOW closure tasks require threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.25-highrowpack16-rowdepthexact-exacttasks-highclosureexact-lowclosureexacttasks-batch"
    ...
    return oneesan_maskshard_v25_lowclosureexacttasks_inner_main(argc, argv);
}
