...
__device__ __forceinline__ uint64_t plug_pos_mask(MateID m){return (uint64_t(m)|(uint64_t(m)>>1))&0x5555555555555555ULL;}
__global__ void reverse_block_group_kernel(const Count*in,Code n,Count*out,Code dn,int p){
    ...
    for(;i<dn;i+=stride){
        ...
        if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
            ...
            int s=1;uint64_t plugs=plug_pos_mask(u);
            uint64_t bits=plugs&((uint64_t(1)<<(2*(p-1)))-1ULL);
            while(bits&&s>0){int bit=63-__clzll(bits);bits&=(uint64_t(1)<<bit)-1ULL;int q=bit>>1;auto v=oneesan::gridfp::mget(u,q);
            ...
            s=1;bits=plugs&~((uint64_t(1)<<(2*(p+1)))-1ULL);
            while(bits&&s>0){int bit=__ffsll((long long)bits)-1;bits&=bits-1;int q=bit>>1;auto v=oneesan::gridfp::mget(u,q);
...
__device__ __forceinline__ Count reverse_eval_block_step(const Count*in,MateID b,int p){
    ...
    if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
        ...
        int s=1;uint64_t plugs=plug_pos_mask(u);uint64_t bits=plugs&((uint64_t(1)<<(2*(p-1)))-1ULL);
        while(bits&&s>0){int bit=63-__clzll(bits);bits&=(uint64_t(1)<<bit)-1ULL;int q=bit>>1;auto v=oneesan::gridfp::mget(u,q);
...
__global__ void reverse2_block_group_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<dn;i+=stride){
        ...
        if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
            ...
            int s=1;uint64_t plugs=plug_pos_mask(u);uint64_t bits=plugs&((uint64_t(1)<<(2*(q-1)))-1ULL);
            while(bits&&s>0){int bit=63-__clzll(bits);bits&=(uint64_t(1)<<bit)-1ULL;int r=bit>>1;auto v=oneesan::gridfp::mget(u,r);
                ...
                if(v==oneesan::gridfp::L)++s;else if(v==oneesan::gridfp::R)--s;}
            s=1;bits=plugs&~((uint64_t(1)<<(2*(q+1)))-1ULL);
            while(bits&&s>0){int bit=__ffsll((long long)bits)-1;bits&=bits-1;int r=bit>>1;auto v=oneesan::gridfp::mget(u,r);
