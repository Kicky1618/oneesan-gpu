...
__constant__ Code D_BOUND_NEXT_DP[MAXW+1][MAXW+2];
...
__global__ void init_after_three_rows_compactlut_kernel(Code total,int shard,int ng,const Count* pref,Code pn,const Count* suff,Code sn,int hlen,Count* bounded4){
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;;q+=st){Code brank=Code(shard)+q*Code(ng);if(brank>=total)break;Code r=brank,grank=0,rank4=0,pr=0,lr=0;int h=1,mid=0,idx=0;
#pragma unroll
        for(int pos=TARGET_W-1;pos>=0;--pos,++idx){int hb=h;MateValue z=N;Code c=D_BOUND_DP[pos][h];if(r<c)z=N;else{r-=c;if(h>0){c=D_BOUND_DP[pos][h-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}
            ...
            if(z>oneesan::gridfp::N) rank4 += D_BOUND_NEXT_DP[pos][h];
            if(z>oneesan::gridfp::R && h>0) rank4 += D_BOUND_NEXT_DP[pos][h-1];
            if(z==R){grank+=D_FULL_DP[pos][h];--h;}else if(z==L){grank+=D_FULL_DP[pos][h]+(h>0?D_FULL_DP[pos][h-1]:0);++h;}
...
#pragma unroll
        ...
        if(bounded4) bounded4[rank4]=ans; else global_store_main(grank,ans);
    }
}
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        if(directRow3){
            prefixK=3;
            Code bdp[MAXW+1][MAXW+2]{}; build_bounded_dp(3,bdp);
            Code total=bdp[W][1]; int cap=3;
            bool r4bounded=false; if(const char*e=std::getenv("GRIDFP_R4_BOUNDED")) r4bounded=std::atoi(e)!=0;
            if(r4bounded && ng!=1){std::cerr<<"row4 bounded prototype currently requires NGPU=1\n";return 32;}
            Code bdp4[MAXW+1][MAXW+2]{}; Code n4=0,dn4=0;
            Count *r4a=nullptr,*r4b=nullptr,*r4d=nullptr,*r4e=nullptr;
            if(r4bounded){
                build_bounded_dp(4,bdp4); n4=bdp4[W][1]; dn4=bdp4[W-1][1];
                ck(cudaSetDevice(0),"r4 set");
                ck(cudaMemcpyToSymbol(D_BOUND_NEXT_DP,bdp4,sizeof(bdp4)),"r4 next dp");
                ck(cudaMalloc(&r4a,size_t(n4)*sizeof(Count)),"r4 a");
                ck(cudaMalloc(&r4b,size_t(n4)*sizeof(Count)),"r4 b");
                ck(cudaMalloc(&r4d,size_t(dn4)*sizeof(Count)),"r4 d");
                ck(cudaMalloc(&r4e,size_t(dn4)*sizeof(Count)),"r4 e");
                ck(cudaMemset(r4a,0,size_t(n4)*sizeof(Count)),"r4 zero a");
                ck(cudaMemset(r4d,0,size_t(dn4)*sizeof(Count)),"r4 zero d");
            }
            auto powmod=[&](uint64_t x,uint64_t e){uint64_t r=1;while(e){if(e&1)r=(unsigned __int128)r*x%mod;x=(unsigned __int128)x*x%mod;e>>=1;}return r;};
            Count scale=Count(powmod(6,W)),inv=Count(powmod(scale,uint64_t(mod)-2));
            for(int d=0;d<ng;++d){
                ck(cudaSetDevice(d),"row3 direct set");
                ck(cudaMemcpyToSymbol(D_BOUND_DP,bdp,sizeof(bdp)),"row3 direct dp"); ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"row3 direct cap");
                ck(cudaMemcpyToSymbol(D_R3_OFF_N,oneesan::row3auto::OFF_N,sizeof(oneesan::row3auto::OFF_N)),"r3 offn");
                ck(cudaMemcpyToSymbol(D_R3_OFF_R,oneesan::row3auto::OFF_R,sizeof(oneesan::row3auto::OFF_R)),"r3 offr");
                ck(cudaMemcpyToSymbol(D_R3_OFF_L,oneesan::row3auto::OFF_L,sizeof(oneesan::row3auto::OFF_L)),"r3 offl");
                ck(cudaMemcpyToSymbol(D_R3_TR_N,oneesan::row3auto::TR_N,sizeof(oneesan::row3auto::TR_N)),"r3 trn");
                ck(cudaMemcpyToSymbol(D_R3_TR_R,oneesan::row3auto::TR_R,sizeof(oneesan::row3auto::TR_R)),"r3 trr");
                ck(cudaMemcpyToSymbol(D_R3_TR_L,oneesan::row3auto::TR_L,sizeof(oneesan::row3auto::TR_L)),"r3 trl");
                ck(cudaMemcpyToSymbol(D_R3_BETA,oneesan::row3auto::BETA,sizeof(oneesan::row3auto::BETA)),"r3 beta");
                uint8_t his[3][18*6]; int16_t hic[3][18*6];
                const uint8_t* hsp[3]={oneesan::row3warp::SRC_N,oneesan::row3warp::SRC_R,oneesan::row3warp::SRC_L};
                const int16_t* hcp[3]={oneesan::row3warp::CO_N,oneesan::row3warp::CO_R,oneesan::row3warp::CO_L};
                for(int aa=0;aa<3;++aa)for(int ii=0;ii<18*6;++ii){his[aa][ii]=hsp[aa][ii];hic[aa][ii]=hcp[aa][ii];}
                ck(cudaMemcpyToSymbol(D_R3I_SRC,his,sizeof(his)),"r3 incoming src"); ck(cudaMemcpyToSymbol(D_R3I_CO,hic,sizeof(hic)),"r3 incoming co"); ck(cudaMemcpyToSymbol(D_R3_UNSCALE,&inv,sizeof(inv)),"r3 inv");
                Code mine=(total+ng-1-d)/ng; int r3threads=128; if(const char*e=std::getenv("GRIDFP_R3_THREADS"))r3threads=std::atoi(e);
                int blocks=int(std::min<Code>(65535,(mine+r3threads-1)/r3threads));
                bool compact=false; if(const char*e=std::getenv("GRIDFP_R3_COMPACT_LUT"))compact=std::atoi(e)!=0;
                if(r4bounded && !compact){std::cerr<<"row4 bounded requires GRIDFP_R3_COMPACT_LUT=1\n";return 33;}
                if(compact){
                    int hlen=W/2,llen=W-hlen; Code any[MAXW+1][4]{}; build_r3_any_dp(3,any); Code off[5]{};
                    for(int hh=0;hh<4;++hh)off[hh+1]=off[hh]+bdp[llen][hh]; Code pn=any[hlen][1],sn=off[4]; Count sb[18]{};
                    for(int jj=0;jj<18;++jj)if(oneesan::row3auto::BETA[jj])sb[jj]=Count((uint64_t(oneesan::row3auto::BETA[jj])*inv)%mod);
                    ck(cudaMemcpyToSymbol(D_R3_ANY_DP,any,sizeof(any)),"r3 any dp"); ck(cudaMemcpyToSymbol(D_R3_SUFF_OFF,off,sizeof(off)),"r3 suff off"); ck(cudaMemcpyToSymbol(D_R3_SUFFIX_BASE,sb,sizeof(sb)),"r3 suffix base");
                    Count*pt=nullptr,*stb=nullptr; ck(cudaMalloc(&pt,size_t(18)*pn*sizeof(Count)),"r3 compact pref"); ck(cudaMalloc(&stb,size_t(18)*sn*sizeof(Count)),"r3 compact suff");
                    int pb=int(std::min<Code>(65535,(pn+r3threads-1)/r3threads)),sbk=int(std::min<Code>(65535,(sn+r3threads-1)/r3threads));
                    r3_build_prefix_compact_kernel<<<std::max(1,pb),r3threads,size_t(36)*r3threads*sizeof(Count)>>>(pn,hlen,pt);
                    r3_build_suffix_compact_kernel<<<std::max(1,sbk),r3threads,size_t(36)*r3threads*sizeof(Count)>>>(sn,llen,stb);
                    ck(cudaDeviceSynchronize(),"r3 compact build");
                    if(mine)init_after_three_rows_compactlut_kernel<<<std::max(1,blocks),r3threads>>>(total,d,ng,pt,pn,stb,sn,hlen,r4bounded?r4a:nullptr);
                    ck(cudaDeviceSynchronize(),"r3 compact init"); cudaFree(pt); cudaFree(stb);
                } else if(mine) init_after_three_rows_kernel<<<std::max(1,blocks),r3threads,size_t(36)*r3threads*sizeof(Count)>>>(total,d,ng);
            }
            for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"row3 direct sync set");ck(cudaDeviceSynchronize(),"row3 direct sync");}
            if(r4bounded){
                int cap4=4; ck(cudaSetDevice(0),"r4 run set"); ck(cudaMemcpyToSymbol(D_BOUND_DP,bdp4,sizeof(bdp4)),"r4 dp"); ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap4,sizeof(cap4)),"r4 cap");
                int bm=int(std::min<Code>(65535,(n4+threads-1)/threads)),bd=int(std::min<Code>(65535,(dn4+threads-1)/threads));
                Count*cur=r4a,*nxt=r4b,*dc=r4d,*de=r4e;
                for(int p=W-1;p>=1;--p){
                    if(p>1){ck(cudaMemset(de,0,size_t(dn4)*sizeof(Count)),"r4 clear d"); bounded_reverse_main_kernel<<<std::max(1,bm),threads>>>(cur,dc,n4,nxt,W,p); bounded_forward_block_kernel<<<std::max(1,bm),threads>>>(cur,n4,de,W,p);}
                    else {ck(cudaMemcpy(nxt,cur,size_t(n4)*sizeof(Count),cudaMemcpyDeviceToDevice),"r4 identity"); ck(cudaMemset(de,0,size_t(dn4)*sizeof(Count)),"r4 clear d p1"); bounded_main_kernel<<<std::max(1,bm),threads>>>(cur,n4,nxt,de,W,p); bounded_block_kernel<<<std::max(1,bd),threads>>>(dc,dn4,nxt,W,p);}
                    ck(cudaDeviceSynchronize(),"r4 step"); std::swap(cur,nxt); std::swap(dc,de);
                }
                int bs=int(std::min<Code>(65535,(n4+threads-1)/threads)); bounded_scatter_full_kernel<<<std::max(1,bs),threads>>>(cur,n4,W); ck(cudaDeviceSynchronize(),"r4 scatter full");
                cudaFree(r4a);cudaFree(r4b);cudaFree(r4d);cudaFree(r4e); prefixK=4;
                std::cerr<<"direct row3 + bounded row4 states="<<n4<<" block="<<dn4<<"\n";
            } else std::cerr<<"direct row3 states="<<total<<"\n";
        }
