...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;MateID*dMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0,gather_s=0,transition_s=0,scatter_s=0,orbit_s=0,reverse_s=0,orbit_gather_s=0,reverse_gather_s=0,orbit_scatter_s=0,reverse_scatter_s=0;uint64_t groups=0,orbit_groups=0,reverse_groups=0,mate_groups=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){
            if(ms.size)factor_main_block_orbit_kernel<<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,p);
            ck(cudaGetLastError(),"factor orbit");
            if(ms.size)factor_main_closure_inplace_kernel<<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,p);
            ck(cudaGetLastError(),"factor closure");
        }
    ...
    ck(cudaGetLastError(),"orbit scatter");ck(cudaDeviceSynchronize(),"orbit group sync");auto tScatter=std::chrono::steady_clock::now();c.groups++;double gg=std::chrono::duration<double>(tGather-t0).count(),tt=std::chrono::duration<double>(tTransition-tGather).count(),ss=std::chrono::duration<double>(tScatter-tTransition).count();c.gather_s+=gg;c.transition_s+=tt;c.scatter_s+=ss;c.active+=std::chrono::duration<double>(tScatter-t0).count();if(useOrbit){c.orbit_s+=tt;c.orbit_gather_s+=gg;c.orbit_scatter_s+=ss;c.orbit_groups++;}else{c.reverse_s+=tt;c.reverse_gather_s+=gg;c.reverse_scatter_s+=ss;c.reverse_groups++;}if(useMate)c.mate_groups++;
}
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0,os=0,rs=0,og=0,rg=0,osc=0,rsc=0;uint64_t on=0,rn=0,mn=0;size_t maxIntervals=0;for(auto&c:ctx){os+=c.orbit_s;rs+=c.reverse_s;og+=c.orbit_gather_s;rg+=c.reverse_gather_s;osc+=c.orbit_scatter_s;rsc+=c.reverse_scatter_s;on+=c.orbit_groups;rn+=c.reverse_groups;mn+=c.mate_groups;mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cerr<<"PROFILE orbit_transition_s="<<os<<" reverse_transition_s="<<rs<<" orbit_gather_s="<<og<<" reverse_gather_s="<<rg<<" orbit_scatter_s="<<osc<<" reverse_scatter_s="<<rsc<<" orbit_groups="<<on<<" reverse_groups="<<rn<<" mate_groups="<<mn<<"\n";std::cout<<"backend=gridfp-b300-hbm32-factorized-vmm-hybrid-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }
