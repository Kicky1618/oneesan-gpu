...
__constant__ int D_AUTH_UNIFIED;
...
__constant__ uint16_t* D_F_LOW_DENSE_PACKED_RANK;
...
__constant__ uint16_t* D_F_HIGH_PACKED_RANK;
...
struct SparseVmmU16 {
    CUdeviceptr base=0; size_t bytes=0,gran=0; std::vector<CUmemGenericAllocationHandle> handles; std::vector<size_t> offsets,sizes;
    uint16_t* ptr() const { return reinterpret_cast<uint16_t*>(base); }
    void destroy(){if(!base)return;for(size_t i=0;i<handles.size();++i){ckcu(cuMemUnmap(base+offsets[i],sizes[i]),"vmm16 unmap");ckcu(cuMemRelease(handles[i]),"vmm16 release");}ckcu(cuMemAddressFree(base,bytes),"vmm16 address free");base=0;}
};
static SparseVmmU16 make_sparse_vmm_u16(int dev,size_t virtualElems,const std::vector<uint32_t>&codes,const std::vector<uint16_t>&values,const char*label){
    if(codes.size()!=values.size())throw std::runtime_error("VMM16 sparse size mismatch");SparseVmmU16 z;ckcu(cuInit(0),"cuInit");CUdevice cdev;ckcu(cuDeviceGet(&cdev,dev),"cuDeviceGet");CUmemAllocationProp prop{};prop.type=CU_MEM_ALLOCATION_TYPE_PINNED;prop.location.type=CU_MEM_LOCATION_TYPE_DEVICE;prop.location.id=cdev;ckcu(cuMemGetAllocationGranularity(&z.gran,&prop,CU_MEM_ALLOC_GRANULARITY_MINIMUM),"vmm16 gran");size_t raw=virtualElems*2;z.bytes=(raw+z.gran-1)/z.gran*z.gran;ckcu(cuMemAddressReserve(&z.base,z.bytes,z.gran,0,0),"vmm16 reserve");size_t ep=z.gran/2;std::vector<std::pair<uint32_t,uint16_t>> kv;kv.reserve(codes.size());for(size_t i=0;i<codes.size();++i)kv.push_back({codes[i],values[i]});std::sort(kv.begin(),kv.end());
    std::vector<size_t> pages;size_t prev=~size_t(0);for(auto x:kv){size_t pg=x.first/ep;if(pg!=prev){pages.push_back(pg);prev=pg;}}CUmemAccessDesc ad{};ad.location.type=CU_MEM_LOCATION_TYPE_DEVICE;ad.location.id=cdev;ad.flags=CU_MEM_ACCESS_FLAGS_PROT_READWRITE;std::vector<uint16_t> page(ep,0xffffu);size_t mapped=0,ki=0,maxRun=0;
    for(size_t ri=0;ri<pages.size();){size_t rj=ri+1;while(rj<pages.size()&&pages[rj]==pages[rj-1]+1)++rj;size_t sp=pages[ri],rp=rj-ri,off=sp*z.gran,rb=rp*z.gran;maxRun=std::max(maxRun,rp);CUmemGenericAllocationHandle h;ckcu(cuMemCreate(&h,rb,&prop,0),"vmm16 create");ckcu(cuMemMap(z.base+off,rb,0,h,0),"vmm16 map");ckcu(cuMemSetAccess(z.base+off,rb,&ad,1),"vmm16 access");z.handles.push_back(h);z.offsets.push_back(off);z.sizes.push_back(rb);mapped+=rb;for(size_t qi=ri;qi<rj;++qi){size_t pg=pages[qi];std::fill(page.begin(),page.end(),0xffffu);while(ki<kv.size()&&size_t(kv[ki].first)/ep==pg){page[kv[ki].first%ep]=kv[ki].second;++ki;}auto ce=cudaMemcpy(reinterpret_cast<void*>(z.base+pg*z.gran),page.data(),z.gran,cudaMemcpyHostToDevice);if(ce!=cudaSuccess)throw std::runtime_error("vmm16 copy failed");}ri=rj;}
    std::cerr<<"factor "<<label<<" VMM16 virtual_mib="<<(z.bytes>>20)<<" mapped_mib="<<(mapped>>20)<<" pages="<<pages.size()<<" runs="<<z.handles.size()<<" max_run_pages="<<maxRun<<" gran_kib="<<(z.gran>>10)<<" sparse_entries="<<codes.size()<<"\n";return z;
}
...
static std::vector<uint32_t> build_high_owner_bounds(int ng){
    constexpr int H=HIGH_LUT_K,S=FactorTablesHost::STRIDE;const uint32_t NM=1u<<H;
    std::vector<uint64_t>w(NM,0),pref(NM+1,0);for(uint32_t m=0;m<NM;++m){uint64_t z=0;
        for(auto const&x:G_F_FULL_MAIN_BLOCKS)if(x.stride)z+=uint64_t(factor_count(G_FACTOR.high_mask_off,m,x.he))*x.stride;
        for(auto const&x:G_F_FULL_BLOCK_BLOCKS)if(x.stride)z+=uint64_t(factor_count(G_FACTOR.high_mask_off,m,x.he))*x.stride;w[m]=z;pref[m+1]=pref[m]+z;}
    std::vector<uint32_t>b(ng+1);b[0]=0;b[ng]=NM;for(int k=1;k<ng;++k){unsigned __int128 num=(unsigned __int128)pref[NM]*k;uint64_t target=uint64_t(num/ng);auto it=std::lower_bound(pref.begin(),pref.end(),target);uint32_t q=uint32_t(it-pref.begin());if(q>0&&q<NM&&target-pref[q-1]<pref[q]-target)--q;b[k]=q;}
    for(int k=1;k<=ng;++k)if(b[k]<b[k-1])throw std::runtime_error("high owner bounds not monotone");
    std::cerr<<"AUTHVMM high-mask bounds";for(auto x:b)std::cerr<<' '<<x;std::cerr<<"\n";return b;
}
static int high_owner_for_mask(uint32_t m,const std::vector<uint32_t>&b){auto it=std::upper_bound(b.begin()+1,b.end(),m);int o=int(it-b.begin())-1;return std::max(0,std::min<int>(int(b.size())-2,o));}
static std::vector<uint8_t> build_auth_page_owners(const std::vector<FBlock>&blocks,Code total,size_t gran,int ng,const std::vector<uint32_t>&bounds,const char*label){
    constexpr int H=HIGH_LUT_K,S=FactorTablesHost::STRIDE;const uint32_t NM=1u<<H;size_t ep=gran/sizeof(Count),np=(size_t(total)+ep-1)/ep;
    std::vector<std::array<uint32_t,MAXGPU>> vote(np);for(auto &a:vote)a.fill(0);
    for(auto const&x:blocks){if(!x.stride)continue;int h=x.he;for(uint32_t m=0;m<NM;++m){uint32_t hc=factor_count(G_FACTOR.high_mask_off,m,h);if(!hc)continue;uint32_t ar=G_FACTOR.high_occ_base[size_t(m)*S+h];Code a=x.off+Code(ar)*x.stride,b=a+Code(hc)*x.stride;int own=high_owner_for_mask(m,bounds);size_t p0=size_t(a)/ep,p1=size_t(b-1)/ep;for(size_t pg=p0;pg<=p1;++pg){Code lo=std::max<Code>(a,Code(pg)*ep),hi=std::min<Code>(b,Code(pg+1)*ep);uint64_t add=hi-lo;if(add>0xffffffffULL)throw std::runtime_error("AUTHVMM page vote overflow");vote[pg][own]+=uint32_t(add);}}}
    std::vector<uint8_t>owner(np,0);uint64_t mixed=0,remote=0,covered=0;std::array<uint64_t,MAXGPU> pages{};for(size_t pg=0;pg<np;++pg){uint64_t sum=0,best=0;int bo=0,nz=0;for(int d=0;d<ng;++d){sum+=vote[pg][d];if(vote[pg][d])++nz;if(vote[pg][d]>best){best=vote[pg][d];bo=d;}}if(!sum){bo=0;}else{covered+=sum;remote+=sum-best;if(nz>1)++mixed;}owner[pg]=uint8_t(bo);pages[bo]++;}
    std::cerr<<"AUTHVMM "<<label<<" pages="<<np<<" mixed="<<mixed<<" remote_pct="<<(covered?100.0*double(remote)/double(covered):0.0)<<" owner_pages";for(int d=0;d<ng;++d)std::cerr<<' '<<pages[d];std::cerr<<"\n";return owner;
}
struct DistributedVmmCountArray{
    struct Run{CUmemGenericAllocationHandle h=0;size_t off=0,bytes=0;int owner=0;};CUdeviceptr base=0;size_t bytes=0,gran=0;std::vector<Run>runs;
    Count* ptr()const{return reinterpret_cast<Count*>(base);}void zero(){for(auto const&r:runs){ck(cudaSetDevice(r.owner),"AUTHVMM zero set");ck(cudaMemset(reinterpret_cast<void*>(base+r.off),0,r.bytes),"AUTHVMM zero");}for(auto const&r:runs){ck(cudaSetDevice(r.owner),"AUTHVMM zero sync set");ck(cudaDeviceSynchronize(),"AUTHVMM zero sync");}}
    void destroy(){if(!base)return;for(auto const&r:runs){ckcu(cuMemUnmap(base+r.off,r.bytes),"AUTHVMM unmap");ckcu(cuMemRelease(r.h),"AUTHVMM release");}ckcu(cuMemAddressFree(base,bytes),"AUTHVMM address free");base=0;runs.clear();}
};
static DistributedVmmCountArray make_distributed_auth_vmm(Code elems,const std::vector<uint8_t>&owner,int ng,const char*label){
    DistributedVmmCountArray z;ckcu(cuInit(0),"AUTHVMM cuInit");CUdevice d0;ckcu(cuDeviceGet(&d0,0),"AUTHVMM dev0");CUmemAllocationProp p0{};p0.type=CU_MEM_ALLOCATION_TYPE_PINNED;p0.location.type=CU_MEM_LOCATION_TYPE_DEVICE;p0.location.id=d0;ckcu(cuMemGetAllocationGranularity(&z.gran,&p0,CU_MEM_ALLOC_GRANULARITY_MINIMUM),"AUTHVMM gran");size_t raw=size_t(elems)*sizeof(Count);z.bytes=(raw+z.gran-1)/z.gran*z.gran;if(owner.size()!=z.bytes/z.gran)throw std::runtime_error("AUTHVMM owner page count mismatch");ckcu(cuMemAddressReserve(&z.base,z.bytes,z.gran,0,0),"AUTHVMM reserve");
    std::vector<CUmemAccessDesc>ad(ng);for(int d=0;d<ng;++d){CUdevice cd;ckcu(cuDeviceGet(&cd,d),"AUTHVMM access dev");ad[d].location.type=CU_MEM_LOCATION_TYPE_DEVICE;ad[d].location.id=cd;ad[d].flags=CU_MEM_ACCESS_FLAGS_PROT_READWRITE;}
    std::array<uint64_t,MAXGPU> phys{};for(size_t a=0;a<owner.size();){size_t b=a+1;while(b<owner.size()&&owner[b]==owner[a])++b;int od=owner[a];CUdevice cd;ckcu(cuDeviceGet(&cd,od),"AUTHVMM owner dev");CUmemAllocationProp prop{};prop.type=CU_MEM_ALLOCATION_TYPE_PINNED;prop.location.type=CU_MEM_LOCATION_TYPE_DEVICE;prop.location.id=cd;size_t off=a*z.gran,rb=(b-a)*z.gran;CUmemGenericAllocationHandle h;ckcu(cuMemCreate(&h,rb,&prop,0),"AUTHVMM create");ckcu(cuMemMap(z.base+off,rb,0,h,0),"AUTHVMM map");ckcu(cuMemSetAccess(z.base+off,rb,ad.data(),ng),"AUTHVMM access");z.runs.push_back({h,off,rb,od});phys[od]+=rb;a=b;}
    std::cerr<<"AUTHVMM "<<label<<" virtual_gib="<<double(z.bytes)/(1ull<<30)<<" runs="<<z.runs.size()<<" physical_gib";for(int d=0;d<ng;++d)std::cerr<<' '<<double(phys[d])/(1ull<<30);std::cerr<<"\n";return z;
}
...
__device__ __forceinline__ Count global_load_main(Code g){if(D_AUTH_UNIFIED||D_NGPU==1)return D_MAIN_PTR[0][g];int o=int(g/D_MAIN_CHUNK);if(o>=D_NGPU)o=D_NGPU-1;return D_MAIN_PTR[o][g-Code(o)*D_MAIN_CHUNK];}
__device__ __forceinline__ Count global_load_block(Code g){if(D_AUTH_UNIFIED||D_NGPU==1)return D_BLOCK_PTR[0][g];int o=int(g/D_BLOCK_CHUNK);if(o>=D_NGPU)o=D_NGPU-1;return D_BLOCK_PTR[o][g-Code(o)*D_BLOCK_CHUNK];}
__device__ __forceinline__ void global_store_main(Code g,Count v){if(D_AUTH_UNIFIED||D_NGPU==1){D_MAIN_PTR[0][g]=v;return;}int o=int(g/D_MAIN_CHUNK);if(o>=D_NGPU)o=D_NGPU-1;D_MAIN_PTR[o][g-Code(o)*D_MAIN_CHUNK]=v;}
__device__ __forceinline__ void global_store_block(Code g,Count v){if(D_AUTH_UNIFIED||D_NGPU==1){D_BLOCK_PTR[0][g]=v;return;}int o=int(g/D_BLOCK_CHUNK);if(o>=D_NGPU)o=D_NGPU-1;D_BLOCK_PTR[o][g-Code(o)*D_BLOCK_CHUNK]=v;}
...
__device__ __forceinline__ uint32_t occ_code_dev(uint32_t code,int len){uint32_t z=0;for(int p=0;p<len;++p)if((code>>(2*p))&3u)z|=1u<<p;return z;}
__device__ __forceinline__ uint32_t low_all_rank16(uint32_t code,int h){constexpr int S=MAXW+2;return D_F_LOW_OCC_BASE[size_t(occ_code_dev(code,LOW_LUT_K))*S+h]+uint32_t(D_F_LOW_DENSE_PACKED_RANK[code]);}
__device__ __forceinline__ uint32_t high_all_rank16(uint32_t code,int h){constexpr int S=MAXW+2;return D_F_HIGH_OCC_BASE[size_t(occ_code_dev(code,HIGH_LUT_K))*S+h]+uint32_t(D_F_HIGH_PACKED_RANK[code]);}
__device__ __forceinline__ Code factor_rank_main(MateID m){constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H),cv=int(mget(m,L)),hs=he+(cv==int(::L)?1:cv==int(R)?-1:0);FBlock x=D_F_MAIN_BLOCKS[3*he+cv];uint32_t lr=D_F_FIX_LOW?uint32_t(D_F_LOW_DENSE_PACKED_RANK[lc]):low_all_rank16(lc,hs);uint32_t hr=D_F_FIX_LOW?high_all_rank16(hc,he):uint32_t(D_F_HIGH_PACKED_RANK[hc]);return x.off+Code(hr)*x.stride+lr;}
__device__ __forceinline__ Code factor_rank_block(MateID m){constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int h=seg_end_height(hc,H);FBlock x=D_F_BLOCK_BLOCKS[h];uint32_t lr=D_F_FIX_LOW?uint32_t(D_F_LOW_DENSE_PACKED_RANK[lc]):low_all_rank16(lc,h);uint32_t hr=D_F_FIX_LOW?high_all_rank16(hc,h):uint32_t(D_F_HIGH_PACKED_RANK[hc]);return x.off+Code(hr)*x.stride+lr;}
__device__ __forceinline__ Code factor_global_rank_main(MateID m){constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H),cv=int(mget(m,L)),hs=he+(cv==int(::L)?1:cv==int(R)?-1:0);FBlock x=D_F_FULL_MAIN_BLOCKS[3*he+cv];return x.off+Code(high_all_rank16(hc,he))*x.stride+low_all_rank16(lc,hs);}
__device__ __forceinline__ Code factor_global_rank_block(MateID m){constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int h=seg_end_height(hc,H);FBlock x=D_F_FULL_BLOCK_BLOCKS[h];return x.off+Code(high_all_rank16(hc,h))*x.stride+low_all_rank16(lc,h);}
...
__device__ __forceinline__ Code factor_global_rank_main_index(Code i){
    ...
    if(D_F_FIX_LOW){har=hr;lar=D_F_LOW_OCC_BASE[size_t(D_F_MASK)*S+x.hs]+lr;}
    else{har=D_F_HIGH_OCC_BASE[size_t(D_F_MASK)*S+x.he]+hr;lar=lr;}
...
__global__ void factor_lower_row_io_kernel(Count*buf,Code nrows){
    ...
    for(Code row=Code(blockIdx.y);row<nrows;row+=Code(gridDim.y)){
        if(threadIdx.x==0){
            ...
            if(bid<0){slen=0;}else{uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];uint32_t har=D_F_HIGH_OCC_BASE[size_t(D_F_MASK)*S+x.he]+uint32_t(D_F_HIGH_PACKED_RANK[hc]);FBlock gx=BLOCK?D_F_FULL_BLOCK_BLOCKS[x.he]:D_F_FULL_MAIN_BLOCKS[3*int(x.he)+int(x.c)];slocal=x.off+Code(hr)*x.stride;sglobal=gx.off+Code(har)*gx.stride;slen=x.stride;}
        }
        __syncthreads();
...
__global__ void factor_lower_flat_io_kernel(Count*buf,Code n){
    ...
    for(Code base=chunk0;base<n;base+=step){
        ...
        if(threadIdx.x==0){int bid=BLOCK?f_find_block(base):f_find_main(base);FBlock x=BLOCK?D_F_BLOCK_BLOCKS[bid]:D_F_MAIN_BLOCKS[bid];Code r=base-x.off;uint32_t hr=x.stride?uint32_t(r/x.stride):0;uint32_t lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;scontig=(x.stride&&Code(lr)+count<=x.stride);if(scontig){uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];uint32_t har=D_F_HIGH_OCC_BASE[size_t(D_F_MASK)*S+x.he]+uint32_t(D_F_HIGH_PACKED_RANK[hc]);FBlock gx=BLOCK?D_F_FULL_BLOCK_BLOCKS[x.he]:D_F_FULL_MAIN_BLOCKS[3*int(x.he)+int(x.c)];sg=gx.off+Code(har)*gx.stride+lr;}}
        __syncthreads();
...
#pragma unroll
        ...
        if(match<0)continue;uint32_t nhc=(hc&~(3u<<(2*match)))|(uint32_t(R)<<(2*match));uint32_t nhr=D_F_HIGH_PACKED_RANK[nhc];int nhe=int(x.he)-2;
...
#pragma unroll
            for(int z=0;z<H;++z){auto v=MateValue((hc>>(2*z))&3u);if(v==L){if(--s==0){match=z;break;}}else if(v==R)++s;}if(match<0)continue;uint32_t nhc=(hc&~(3u<<(2*match)))|(uint32_t(R)<<(2*match));uint32_t nhr=D_F_HIGH_PACKED_RANK[nhc];int nhe=int(x.he)-2;if(p==1){FBlock dx=D_F_MAIN_BLOCKS[3*nhe+int(x.c)];atomic_add_mod(mainv+dx.off+Code(nhr)*dx.stride+dlr,c);}else{FBlock bx=D_F_BLOCK_BLOCKS[nhe];atomic_add_mod(blockv+bx.off+Code(nhr)*bx.stride+dlr,c);}}
    }
}
...
#pragma unroll
            for(int z=LOWK-1;z>=0;--z){auto v=MateValue((code>>(2*z))&3u);if(v==R){if(--s==0){match=z;break;}}else if(v==oneesan::gridfp::L)++s;}if(match<0)continue;uint32_t nc=(code&~(3u<<(2*match)))|(uint32_t(::L)<<(2*match));uint32_t nlr=D_F_LOW_DENSE_PACKED_RANK[nc];int nh=int(x.hs)-2;FBlock bx=D_F_BLOCK_BLOCKS[nh];atomic_add_mod(blockv+bx.off+Code(bhr)*bx.stride+nlr,c);}}
}

__device__ __forceinline__ Count* global_main_ptr(Code g){if(D_AUTH_UNIFIED||D_NGPU==1)return D_MAIN_PTR[0]+g;int o=int(g/D_MAIN_CHUNK);if(o>=D_NGPU)o=D_NGPU-1;return D_MAIN_PTR[o]+g-Code(o)*D_MAIN_CHUNK;}
__device__ __forceinline__ Count* global_block_ptr(Code g){if(D_AUTH_UNIFIED||D_NGPU==1)return D_BLOCK_PTR[0]+g;int o=int(g/D_BLOCK_CHUNK);if(o>=D_NGPU)o=D_NGPU-1;return D_BLOCK_PTR[o]+g-Code(o)*D_BLOCK_CHUNK;}
...
__global__ void direct_prerank_closure_inv_combined_kernel(PreRankOrbitMap invMap,PreRankOrbitMap highMap,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){
        if(q<ni){
            ...
            Code zq=q-ni;int bid=prerank_find_block(zq,highMap);Code base=bid?highMap.end[bid-1]:0,loc=zq-base;uint32_t sc=highMap.sel_count[bid],hr=uint32_t(loc/sc),k=uint32_t(loc-Code(hr)*sc);unsigned long long rec=D_PR_HIGHRR_REC[highMap.sel_off[bid]+k];uint32_t lr=uint32_t(rec)&M,dlr=uint32_t(rec>>B)&M;int depth=int((rec>>(2*B))&63u);FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr],har=direct_high_all_rank(x.he,hr);FBlock gx=D_F_FULL_MAIN_BLOCKS[bid];Count c=global_load_main(gx.off+Code(har)*gx.stride+lr);if(!c)continue;int match=-1,ss=depth;
#pragma unroll
            for(int z=0;z<H;++z){auto v=MateValue((hc>>(2*z))&3u);if(v==L){if(--ss==0){match=z;break;}}else if(v==R)++ss;}if(match<0)continue;uint32_t nhc=(hc&~(3u<<(2*match)))|(uint32_t(R)<<(2*match));int nhe=int(x.he)-2;uint32_t nhar=D_F_HIGH_OCC_BASE[size_t(D_F_MASK)*S+nhe]+uint32_t(D_F_HIGH_PACKED_RANK[nhc]);
...
__global__ void direct_high_prerank_closure_inv_combined_kernel(HighPreRankMap invMap,HighPreRankMap crossMap){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){
        if(q<ni){
            ...
            Code zq=q-ni;int bid=high_pr_find_block(zq,crossMap);Code base=bid?crossMap.end[bid-1]:0,loc=zq-base;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lc=x.stride,k=uint32_t(loc/lc),lr=uint32_t(loc-Code(k)*lc);unsigned long long r=D_HP_CROSSLL_REC[crossMap.sel_off[bid]+k];uint32_t hr=uint32_t(r)&M,bhr=uint32_t(r>>B)&M;int depth=int((r>>(2*B))&63u);uint32_t code=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr],lar=D_F_LOW_OCC_BASE[size_t(D_F_MASK)*S+x.hs]+uint32_t(D_F_LOW_DENSE_PACKED_RANK[code]);FBlock gx=D_F_FULL_MAIN_BLOCKS[bid];Count c=global_load_main(gx.off+Code(hr)*gx.stride+lar);if(!c)continue;int match=-1,ss=depth;
#pragma unroll
            for(int z=LOWK-1;z>=0;--z){auto v=MateValue((code>>(2*z))&3u);if(v==R){if(--ss==0){match=z;break;}}else if(v==oneesan::gridfp::L)++ss;}if(match<0)continue;uint32_t nc=(code&~(3u<<(2*match)))|(uint32_t(::L)<<(2*match));int nh=int(x.hs)-2;uint32_t nlar=D_F_LOW_OCC_BASE[size_t(D_F_MASK)*S+nh]+uint32_t(D_F_LOW_DENSE_PACKED_RANK[nc]);FBlock bx=D_F_FULL_BLOCK_BLOCKS[nh];global_atomic_add_block(bx.off+Code(bhr)*bx.stride+nlar,c);
        }
    }
}
...
__device__ __forceinline__ Code orbit_rank_main_pair_from(Code ti,MateID pred,int p){
    ...
    if(p<L){
        ...
        uint32_t lp=D_F_LOW_DENSE_PACKED_RANK[lc],nlr=D_F_FIX_LOW?lp:low_all_rank16(lc,x.hs);
        return ti-Code(lr)+Code(nlr);
    }
    if(p>=L+2){
        constexpr uint32_t HM=(1u<<(2*H))-1u;uint32_t hc=uint32_t((pred>>(2*(L+1)))&HM);
        uint32_t hp=D_F_HIGH_PACKED_RANK[hc],nhr=D_F_FIX_LOW?high_all_rank16(hc,x.he):hp;
...
__device__ __forceinline__ Code orbit_rank_block_from_main(Code ti,MateID t,int p){
    ...
    if(D_F_FIX_LOW){
        ...
        uint32_t hp=D_F_HIGH_PACKED_RANK[hc],nhr=high_all_rank16(hc,h);FBlock bx=D_F_BLOCK_BLOCKS[h];
    ...
    uint32_t lp=D_F_LOW_DENSE_PACKED_RANK[lc],nlr=low_all_rank16(lc,the);
...
struct BatchIoDesc{Code buf,global,elems;uint32_t width,gstride;uint8_t block,pad[3];};
struct BatchIoTask{uint32_t desc,pad;Code begin;};
template<bool SCATTER> __global__ void batch_upper_io_kernel(Count*arena,const BatchIoDesc*ds,const BatchIoTask*ts,Code nt){
    Code qi=blockIdx.x;if(qi>=nt)return;BatchIoTask t=ts[qi];BatchIoDesc d=ds[t.desc];constexpr Code CH=65536;Code end=min(d.elems,t.begin+CH);
    for(Code i=t.begin+threadIdx.x;i<end;i+=blockDim.x){Code row=i/d.width,col=i-Code(row)*d.width,g=d.global+row*d.gstride+col;Count*bp=arena+d.buf+i;if constexpr(SCATTER){if(d.block)global_store_block(g,*bp);else global_store_main(g,*bp);}else{*bp=d.block?global_load_block(g):global_load_main(g);}}
}
...
static void process_upper_batched(DeviceCtx&c,int W,const PreparedWindow&pw,int threads,size_t target){
    if(pw.wp.p_hi<=LOW_LUT_K)throw std::runtime_error("upper batch called for lower window");ck(cudaSetDevice(c.dev),"upper batch set");constexpr size_t ALIGN=64;auto al=[](Code x){return (x+Code(ALIGN-1))&~Code(ALIGN-1);};
    size_t q0=0;while(q0<pw.groups.size()){
        struct Meta{size_t qi;Code mo,bo;};std::vector<Meta> meta;Code used=0;size_t q=q0;
        for(;q<pw.groups.size();++q){auto const&pg=pw.groups[q];Code need=al(pg.ms.size)+al(pg.ds.size);if(!meta.empty()&&uint64_t(used+need)*sizeof(Count)>target)break;if(uint64_t(need)*sizeof(Count)>target)throw std::runtime_error("upper batch single group exceeds target");meta.push_back({q,used,used+al(pg.ms.size)});used+=need;}
        size_t needBytes=size_t(used)*sizeof(Count);if(needBytes>c.capArena){if(c.arena)cudaFree(c.arena);c.capArena=needBytes;ck(cudaMalloc(&c.arena,c.capArena),"upper batch arena");}
        std::vector<BatchIoDesc> hd;std::vector<BatchIoTask> ht;hd.reserve(meta.size()*48);auto addDesc=[&](Code buf,Code glob,Code rows,uint32_t width,uint32_t gs,bool block){if(!rows||!width)return;uint32_t di=hd.size();Code elems=rows*Code(width);hd.push_back({buf,glob,elems,width,gs,uint8_t(block),{0,0,0}});for(Code b=0;b<elems;b+=65536)ht.push_back({di,0,b});};
        for(auto const&m:meta){auto const&pg=pw.groups[m.qi];uint32_t fmask=pg.mo&((1u<<LOW_LUT_K)-1u);auto fmb=make_factor_main_blocks(true,fmask),fdb=make_factor_block_blocks(true,fmask);constexpr int S=FactorTablesHost::STRIDE;
            for(auto const&x:fmb)if(x.stride&&x.end>x.off){FBlock gx=G_F_FULL_MAIN_BLOCKS[3*int(x.he)+int(x.c)];uint32_t lb=G_FACTOR.low_occ_base[size_t(fmask)*S+x.hs];addDesc(m.mo+x.off,gx.off+lb,(x.end-x.off)/x.stride,x.stride,gx.stride,false);}
            for(auto const&x:fdb)if(x.stride&&x.end>x.off){FBlock gx=G_F_FULL_BLOCK_BLOCKS[x.he];uint32_t lb=G_FACTOR.low_occ_base[size_t(fmask)*S+x.he];addDesc(m.bo+x.off,gx.off+lb,(x.end-x.off)/x.stride,x.stride,gx.stride,true);}}
        BatchIoDesc*dd=nullptr;BatchIoTask*dt=nullptr;ck(cudaMalloc(&dd,hd.size()*sizeof(BatchIoDesc)),"upper batch desc");ck(cudaMalloc(&dt,ht.size()*sizeof(BatchIoTask)),"upper batch task");ck(cudaMemcpy(dd,hd.data(),hd.size()*sizeof(BatchIoDesc),cudaMemcpyHostToDevice),"upper batch desc cp");ck(cudaMemcpy(dt,ht.data(),ht.size()*sizeof(BatchIoTask),cudaMemcpyHostToDevice),"upper batch task cp");auto t0=std::chrono::steady_clock::now();
        batch_upper_io_kernel<false><<<std::max<size_t>(1,ht.size()),threads,0,c.sMain>>>(reinterpret_cast<Count*>(c.arena),dd,dt,ht.size());ck(cudaGetLastError(),"upper batch gather");ck(cudaStreamSynchronize(c.sMain),"upper batch gather sync");auto tg=std::chrono::steady_clock::now();
        for(auto const&m:meta){auto const&pg=pw.groups[m.qi];uint32_t fmask=pg.mo&((1u<<LOW_LUT_K)-1u);auto fmb=make_factor_main_blocks(true,fmask),fdb=make_factor_block_blocks(true,fmask);GroupRuntimeCfg cfg{};std::memcpy(cfg.main_dp,pg.ms.dp,sizeof(pg.ms.dp));std::memcpy(cfg.block_dp,pg.ds.dp,sizeof(pg.ds.dp));std::memcpy(cfg.f_main,fmb.data(),fmb.size()*sizeof(FBlock));std::memcpy(cfg.f_block,fdb.data(),fdb.size()*sizeof(FBlock));cfg.main_fixed=pg.mf;cfg.main_occ=pg.mo;cfg.block_fixed=pg.bf;cfg.block_occ=pg.bo;cfg.f_mask=fmask;cfg.main_w=W;cfg.block_w=W-1;cfg.f_main_n=fmb.size();cfg.f_block_n=fdb.size();cfg.f_fix_low=1;ck(cudaMemcpyToSymbol(D_GRP,&cfg,sizeof(cfg)),"upper batch cfg");Count*cur=reinterpret_cast<Count*>(c.arena)+m.mo,*dcur=reinterpret_cast<Count*>(c.arena)+m.bo;
            for(int pp=pw.wp.p_hi;pp>=pw.wp.p_lo;--pp){auto im=make_high_prerank_map(fmb,pp,3),xm=make_high_prerank_map(fmb,pp,2);Code on=pg.ds.size,in=im.nblocks?im.end[im.nblocks-1]:0,xn=xm.nblocks?xm.end[xm.nblocks-1]:0;if(on){int nb=int(std::min<Code>(65535,(on+threads-1)/threads));factor_high_prerank_owner_blockorder_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,on,pp);}Code cn=in+xn;if(cn){int nb=int(std::min<Code>(65535,(cn+threads-1)/threads));factor_high_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,im,xm);}ck(cudaGetLastError(),"upper batch transition");}
            ck(cudaStreamSynchronize(c.sMain),"upper batch group transition sync");}
        auto tt=std::chrono::steady_clock::now();batch_upper_io_kernel<true><<<std::max<size_t>(1,ht.size()),threads,0,c.sMain>>>(reinterpret_cast<Count*>(c.arena),dd,dt,ht.size());ck(cudaGetLastError(),"upper batch scatter");ck(cudaStreamSynchronize(c.sMain),"upper batch scatter sync");auto ts=std::chrono::steady_clock::now();cudaFree(dd);cudaFree(dt);c.groups+=meta.size();c.gather_s+=std::chrono::duration<double>(tg-t0).count();c.transition_s+=std::chrono::duration<double>(tt-tg).count();c.scatter_s+=std::chrono::duration<double>(ts-tt).count();c.active+=std::chrono::duration<double>(ts-t0).count();q0=q;
    }
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    bool lowerFlatIo=!fixLow;if(const char*e=std::getenv("GRIDFP_FACTOR_FLAT_IO"))lowerFlatIo=!fixLow&&std::atoi(e)!=0;bool lowerRowIo=false;if(const char*e=std::getenv("GRIDFP_FACTOR_ROW_IO"))lowerRowIo=!fixLow&&std::atoi(e)!=0;
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        auto vmmMapped=[&](size_t denseElems,std::vector<uint32_t> const&codes)->uint64_t{uint64_t raw=uint64_t(denseElems)*2,elemsPer=vmmGran/2,np=(raw+vmmGran-1)/vmmGran;std::vector<uint8_t> used(np,0);for(uint32_t c:codes)used[uint64_t(c)/elemsPer]=1;uint64_t n=0;for(uint8_t x:used)n+=x!=0;return n*vmmGran;};
        ...
        uint64_t sparseBytes=0;uint64_t preRankBytes=uint64_t(G_PRERANK.owner_block_rec.size()+G_PRERANK.highrr_rec.size()+G_PRERANK.closure_inv_rec.size()+G_HPR.owner_block_rec.size()+G_HPR.crossll_rec.size()+G_HPR.closure_inv_rec.size())*8+uint64_t(G_PRERANK.closure_inv_src.size()+G_HPR.closure_inv_src.size())*4;uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+vb4(G_FACTOR.low_occ_base)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+vb4(G_FACTOR.high_occ_base)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+sparseBytes+preRankBytes;
        ...
        {auto fp=window_candidates(W,W-1,LOW_LUT_K+1);int nj=1<<int(fp.size());std::vector<uint64_t> sz;sz.reserve(nj);for(int g=0;g<nj;++g){uint32_t mf,mo,bf,bo;window_masks(W,W-1,LOW_LUT_K+1,fp,uint32_t(g),mf,mo,bf,bo);auto ms=make_spec(W,mf,mo),ds=make_spec(W-1,bf,bo);sz.push_back(uint64_t(ms.size+ds.size)*sizeof(Count));}std::sort(sz.begin(),sz.end(),std::greater<uint64_t>());auto batches=[&](uint64_t cap){uint64_t nb=0,used=0,maxn=0,cur=0;for(auto x:sz){if(used&&used+x>cap){++nb;maxn=std::max(maxn,cur);used=0;cur=0;}used+=x;++cur;}if(cur){++nb;maxn=std::max(maxn,cur);}return std::pair<uint64_t,uint64_t>(nb,maxn);};auto b5=batches(windowScratch[0]),b16=batches(16ull<<30);uint64_t sum=0;for(auto x:sz)sum+=x;std::cerr<<"PLAN upper_batch groups="<<sz.size()<<" total_gib="<<double(sum)/(1ull<<30)<<" max_mib="<<(sz.front()>>20)<<" median_kib="<<(sz[sz.size()/2]>>10)<<" cap4p8_batches="<<b5.first<<" max_groups="<<b5.second<<" cap16_batches="<<b16.first<<" max_groups="<<b16.second<<"\n";}
        bool scratchFits=minScratchBytes<=scratchLimit;uint64_t dpPeak=authMaxBytes+factorBytes+actualScratchBytes;
        auto planBounds=build_high_owner_bounds(png);auto planMO=build_auth_page_owners(G_F_FULL_MAIN_BLOCKS,pmain,vmmGran,png,planBounds,"main-plan-only"),planBO=build_auth_page_owners(G_F_FULL_BLOCK_BLOCKS,pblock,vmmGran,png,planBounds,"block-plan-only");std::array<uint64_t,MAXGPU>authVmmPhys{};for(auto o:planMO)authVmmPhys[o]+=vmmGran;for(auto o:planBO)authVmmPhys[o]+=vmmGran;uint64_t authVmmMax=0;for(int d=0;d<png;++d)authVmmMax=std::max(authVmmMax,authVmmPhys[d]);auto countRuns=[](std::vector<uint8_t>const&o){uint64_t r=0;for(size_t i=0;i<o.size();){++r;size_t j=i+1;while(j<o.size()&&o[j]==o[i])++j;i=j;}return r;};uint64_t authVmmRuns=countRuns(planMO)+countRuns(planBO),dpPeakVmm=authVmmMax+factorBytes+actualScratchBytes,row6PeakVmm=authVmmMax+factorBytes+row6Extra;std::cerr<<"PLAN authvmm runs="<<authVmmRuns<<" phys_gib";for(int d=0;d<png;++d)std::cerr<<' '<<double(authVmmPhys[d])/(1ull<<30);std::cerr<<" auth_max_gib="<<double(authVmmMax)/(1ull<<30)<<" dp_peak_gib="<<double(dpPeakVmm)/(1ull<<30)<<" row6_peak_gib="<<double(row6PeakVmm)/(1ull<<30)<<"\n";
        {std::array<unsigned long long,MAXGPU> lt{};unsigned long long sum=0;uint32_t NM=1u<<HIGH_LUT_K;for(uint32_t m=0;m<NM;++m){auto fmb=make_factor_main_blocks(false,m),fdb=make_factor_block_blocks(false,m);unsigned long long z=0;for(int pp=LOW_LUT_K;pp>=1;--pp){auto im=make_prerank_orbit_map(fmb,pp,3),sm=make_prerank_orbit_map(fmb,pp,2);Code in=im.nblocks?im.end[im.nblocks-1]:0,sn=sm.nblocks?sm.end[sm.nblocks-1]:0;z+=fdb.back().end+in+sn;}int o=high_owner_for_mask(m,planBounds);lt[o]+=z;sum+=z;}double avg=double(sum)/png,mx=0,mn=1e300;std::cerr<<"PLAN lower_tasks";for(int d=0;d<png;++d){std::cerr<<' '<<lt[d];mx=std::max(mx,double(lt[d]));mn=std::min(mn,double(lt[d]));}std::cerr<<" imbalance_max="<<(mx/avg)<<" min="<<(mn/avg)<<"\n";}
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        if(authVmm){authMainVmm.zero();authBlockVmm.zero();}
        for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"set residue reset");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"set modulus");if(!authVmm){if(ml[d])ck(cudaMemset(mp[d],0,size_t(ml[d])*sizeof(Count)),"zero main");if(bl[d])ck(cudaMemset(bp[d],0,size_t(bl[d])*sizeof(Count)),"zero block");ck(cudaDeviceSynchronize(),"zero sync");}ctx[d].active=0;ctx[d].gather_s=ctx[d].transition_s=ctx[d].scatter_s=0;ctx[d].groups=0;}
        ...
        int done_windows=0;
        for(int row=prefixK;row<W-1;++row){for(auto const&pw:schedule){int nj=(int)pw.groups.size();bool lower=(pw.wp.p_hi<=LOW_LUT_K),batchUpper=false;if(const char*e=std::getenv("GRIDFP_UPPER_BATCH_IO"))batchUpper=std::atoi(e)!=0;batchUpper=batchUpper&&ng==1&&!lower;bool ownerSched=authVmm&&lower;int dm=1;if(const char*e=std::getenv("GRIDFP_DIRECT_AUTH"))dm=std::atoi(e);ownerSched=ownerSched&&(dm==1||dm==3);if(batchUpper){process_upper_batched(ctx[0],W,pw,threads,target);}else{std::vector<std::thread>ths;ths.reserve(ng);if(ownerSched){for(int d=0;d<ng;++d)ths.emplace_back([&,d]{for(int q=0;q<nj;++q){auto const&pg=pw.groups[q];uint32_t hm=(pg.mo>>(LOW_LUT_K+1))&((1u<<HIGH_LUT_K)-1u);if(high_owner_for_mask(hm,authBounds)==d)process_group(ctx[d],W,pw.wp,pg,threads,target);}});}else{std::atomic<int>next{0};for(int d=0;d<ng;++d)ths.emplace_back([&,d]{for(;;){int q=next.fetch_add(1,std::memory_order_relaxed);if(q>=nj)break;process_group(ctx[d],W,pw.wp,pw.groups[q],threads,target);}});}for(auto&t:ths)t.join();}++done_windows;}std::cerr<<"mod "<<(ri+1)<<"/"<<mods.size()<<" p="<<mod<<" row "<<row+1<<"/"<<W<<"\n";}
        Count ans=0;
        for(int d=0;d<ng;++d){
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-occmajor-batchio-probe n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();if(authVmm){authMainVmm.destroy();authBlockVmm.destroy();}for(int d=0;d<ng;++d){cudaSetDevice(d);if(!authVmm&&mp[d])cudaFree(mp[d]);if(!authVmm&&bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);if(fLOB[d])cudaFree(fLOB[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);if(fHOB[d])cudaFree(fHOB[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prOB[d])cudaFree(prOB[d]);if(prCI[d])cudaFree(prCI[d]);if(prCIS[d])cudaFree(prCIS[d]);if(prH[d])cudaFree(prH[d]);if(hpOB[d])cudaFree(hpOB[d]);if(hpCI[d])cudaFree(hpCI[d]);if(hpCIS[d])cudaFree(hpCIS[d]);if(hpX[d])cudaFree(hpX[d]);}
}
