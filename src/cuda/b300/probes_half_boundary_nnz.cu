...
__global__ void probe_nnz_kernel(const Count*a,Code n,unsigned long long*out){extern __shared__ unsigned int zz_nnz[];unsigned int c=0;for(Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;i<n;i+=st)c+=(a[i]!=0);zz_nnz[threadIdx.x]=c;__syncthreads();for(int q=blockDim.x/2;q;q>>=1){if(threadIdx.x<q)zz_nnz[threadIdx.x]+=zz_nnz[threadIdx.x+q];__syncthreads();}if(threadIdx.x==0)atomicAdd(out,(unsigned long long)zz_nnz[0]);}
static unsigned long long probe_count_nnz(Count*a,Code n,int threads){if(!n)return 0;unsigned long long*d=nullptr;ck(cudaMalloc(&d,8),"nnz alloc");ck(cudaMemset(d,0,8),"nnz zero");int b=int(std::min<Code>(65535,(n+threads-1)/threads));probe_nnz_kernel<<<std::max(1,b),threads,threads*sizeof(unsigned int)>>>(a,n,d);unsigned long long z=0;ck(cudaMemcpy(&z,d,8,cudaMemcpyDeviceToHost),"nnz copy");cudaFree(d);return z;}
struct DeviceCtx{
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        int done_windows=0;
        int rowStop=halfMode?((n+1)/2):(W-1);for(int row=prefixK;row<rowStop;++row){for(auto const&pw:schedule){int nj=(int)pw.groups.size();std::atomic<int>next{0};std::vector<std::thread>ths;ths.reserve(ng);for(int d=0;d<ng;++d)ths.emplace_back([&,d]{for(;;){int q=next.fetch_add(1,std::memory_order_relaxed);if(q>=nj)break;process_group(ctx[d],W,pw.wp,pw.groups[q],threads,target);}});for(auto&t:ths)t.join();if(pw.wp.p_lo==LOW_LUT_K+1){unsigned long long z=0;for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"nnz set");z+=probe_count_nnz(bp[d],bl[d],threads);}std::cerr<<"BOUNDARY_NNZ row="<<(row+1)<<" nnz="<<z<<" total="<<blockN<<" frac="<<(double(z)/double(blockN))<<"\n";}++done_windows;}std::cerr<<"mod "<<(ri+1)<<"/"<<mods.size()<<" p="<<mod<<" row "<<row+1<<"/"<<W<<"\n";}
        Count ans=0;if(halfMode){
            ...
            ans=half_central_join_single_gpu(W,threads);
        }
