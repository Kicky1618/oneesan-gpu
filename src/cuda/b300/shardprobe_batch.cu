...
static HighPreRankTablesHost build_high_prerank_tables(){
    ...
    for(auto r:z.owner_block_rec)if(r==~0ULL)throw std::runtime_error("high blocked-order owner missing");double mib=(double(z.owner_block_rec.size()+z.crossll_rec.size()+z.closure_inv_rec.size())*8.0+double(z.closure_inv_src.size())*4.0)/(1<<20);std::cerr<<"high pre-rank owner="<<z.owner_block_rec.size()<<" closure_fast="<<z.closure_inv_src.size()<<" closure_inv="<<z.closure_inv_rec.size()<<" closure_crossll="<<z.crossll_rec.size()<<" device_mib="<<mib<<"\n";z.owner_rec.clear();z.owner_rec.shrink_to_fit();z.closure_rec.clear();z.closure_rec.shrink_to_fit();std::cerr<<"SHARDPROBE upper owner_tasks="<<shOwner<<" owner_cross="<<shOwnerCross<<" owner_cross_pct="<<(100.0*shOwnerCross/std::max<unsigned long long>(1,shOwner))<<" closure_tasks="<<shClosure<<" closure_cross="<<shClosureCross<<" closure_cross_pct="<<(100.0*shClosureCross/std::max<unsigned long long>(1,shClosure))<<" crossll_tasks="<<shCrossLL<<" crossll_cross="<<shCrossLLCross<<" crossll_cross_pct="<<(100.0*shCrossLLCross/std::max<unsigned long long>(1,shCrossLL))<<"\n";return z;
}
