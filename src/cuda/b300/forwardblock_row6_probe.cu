...
__device__ __forceinline__ void forward2_try_block(Count*out,MateID m,int q,Count c){
    auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,q);
    if(z.valid&&z.blocked)atomic_add_mod(out+factor_rank_block(z.mate),c);
}
__global__ void forward2_block_from_main_kernel(const Count*in,const MateID*mates,Code n,Count*out,int p){
    int q=p-1;Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=mates?mates[i]:factor_unrank_main(i);
        forward2_try_block(out,m,q,c);
        auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);
        if(z.valid&&!z.blocked)forward2_try_block(out,z.mate,q,c);
    }
}
__global__ void forward2_block_from_block_kernel(const Count*din,Code dn,Count*out,int p){
    int q=p-1;Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<dn;i+=stride){Count c=din[i];if(!c)continue;MateID b=factor_unrank_block(i);MateID m=oneesan::gridfp::blocked_exclude(b,p);forward2_try_block(out,m,q,c);}
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;){
        if(p>2 && p-1>=wp.p_lo){
            ...
            if(ds.size)ck(cudaMemsetAsync(dnext,0,size_t(ds.size)*sizeof(Count),c.sBlock),"fwd2 clear D");
            if(ms.size&&ds.size)forward2_block_from_main_kernel<<<bm,threads,0,c.sBlock>>>(cur,useMate?c.dMate:nullptr,ms.size,dnext,p);
            if(ds.size)forward2_block_from_block_kernel<<<bd,threads,0,c.sBlock>>>(dcur,ds.size,dnext,p);
            p-=2;
        }else{
            if(p>1){
