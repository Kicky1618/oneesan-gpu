...
__constant__ uint64_t* D_F_HIGH_HASH;
__constant__ uint32_t D_F_HIGH_HASH_MASK;
...
static FactorTablesHost build_factor_tables(){
    ...
    {
        size_t cap=1; while(cap < std::max<size_t>(16, f.high_all_codes.size()*2)) cap<<=1;
        f.high_hash.assign(cap,0);
        uint32_t mask=uint32_t(cap-1);
        for(uint32_t c: f.high_all_codes){
            uint32_t key=c+1u, val=f.high_packed_rank[c];
            uint32_t q=(uint32_t(uint64_t(key)*2654435761u)) & mask;
            while(f.high_hash[q]) q=(q+1)&mask;
            f.high_hash[q]=(uint64_t(key)<<32)|val;
        }
    }
    ...
    std::cerr<<"factor tables low_all="<<f.low_all_codes.size()<<" low_mask="<<f.low_mask_codes.size()<<" low_dense_mib="<<double(f.low_packed_rank.size()*4)/(1<<20)
             <<" high_all="<<f.high_all_codes.size()<<" high_mask="<<f.high_mask_codes.size()<<" high_dense_mib="<<double(f.high_packed_rank.size()*4)/(1<<20)<<" high_hash_mib="<<double(f.high_hash.size()*8)/(1<<20)<<"\n";
    return f;
}
...
__device__ __forceinline__ uint32_t high_packed_lookup(uint32_t code){
    uint32_t key=code+1u, q=(uint32_t(uint64_t(key)*2654435761u)) & D_F_HIGH_HASH_MASK;
#pragma unroll 1
    for(;;){uint64_t z=D_F_HIGH_HASH[q]; if(uint32_t(z>>32)==key)return uint32_t(z); q=(q+1)&D_F_HIGH_HASH_MASK;}
}
__device__ __forceinline__ Code factor_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);int cv=int(mget(m,L));int bid=3*he+cv;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lp=D_F_LOW_PACKED_RANK[lc],hp=high_packed_lookup(hc);uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
...
__device__ __forceinline__ Code factor_global_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);uint32_t hp=high_packed_lookup(hc),lp=D_F_LOW_PACKED_RANK[lc];uint32_t har=hp>>H,lar=lp>>L;Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[he]+har];MateValue c=mget(m,L);if(c>N)rank+=D_FULL_DP[L][he];if(c>R&&he>0)rank+=D_FULL_DP[L][he-1];return rank+lar;
}
__device__ __forceinline__ Code factor_global_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int he=seg_end_height(hc,H);uint32_t hp=high_packed_lookup(hc),lp=D_F_LOW_PACKED_RANK[lc];uint32_t har=hp>>H,lar=lp>>L;return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[he]+har]+lar;
}
...
static PreparedGroup prepare_group(int W,const WindowPlan&wp,int g,Code mc,Code bc,int ng){
    ...
    // Factorized local scratch order is not canonical group-rank order.
    // The legacy interval I/O map is therefore invalid here; always use
    // factor-aware per-state gather/scatter until factor intervals are used.
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;){
        ...
        ck(cudaGetLastError(),"reverse2 transition");
...
int main(int argc,char**argv){
    ...
    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);if(fLR[d])cudaFree(fLR[d]);if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);if(fHR[d])cudaFree(fHR[d]);if(fHH[d])cudaFree(fHH[d]);if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);}
}
