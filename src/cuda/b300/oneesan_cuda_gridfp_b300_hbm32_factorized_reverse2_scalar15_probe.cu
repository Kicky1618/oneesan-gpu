...
int main(int argc,char**argv){
    ...
    build_full_dp();
    // scalar15 probe forces prefixK=1; row6 CRT tables are unused.
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        int prefixK=1;if(const char*e=std::getenv("GRIDFP_BOUNDED_PREFIX_K")){if(std::atoi(e)!=1){std::cerr<<"scalar15 probe requires GRIDFP_BOUNDED_PREFIX_K=1\n";return 35;}}
