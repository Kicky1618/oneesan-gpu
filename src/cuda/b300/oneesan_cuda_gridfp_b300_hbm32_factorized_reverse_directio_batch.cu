...
__device__ __forceinline__ Code factor_direct_main(Code i,MateID* mout){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;
    int b=f_find_main(i);FBlock x=D_F_MAIN_BLOCKS[b];Code r=i-x.off;
    uint32_t hr=x.stride?uint32_t(r/x.stride):0,lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;
    uint32_t hc=0,lc=0,har=0,lar=0;
    if(D_F_FIX_LOW){
        har=hr;lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];lar=D_F_LOW_PACKED_RANK[lc]>>L;
        if(mout)hc=D_F_HIGH_ALL_CODES[D_F_HIGH_ALL_OFF[x.he]+hr];
    }else{
        hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];har=D_F_HIGH_PACKED_RANK[hc]>>H;lar=lr;
        if(mout)lc=D_F_LOW_ALL_CODES[D_F_LOW_ALL_OFF[x.hs]+lr];
    }
    Code g=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[x.he]+har];
    if(x.c>int(N))g+=D_FULL_DP[L][x.he];if(x.c>int(R)&&x.he>0)g+=D_FULL_DP[L][x.he-1];g+=lar;
    if(mout){if(D_F_FIX_LOW&&!hc)hc=D_F_HIGH_ALL_CODES[D_F_HIGH_ALL_OFF[x.he]+hr];if(!D_F_FIX_LOW&&!lc)lc=D_F_LOW_ALL_CODES[D_F_LOW_ALL_OFF[x.hs]+lr];*mout=MateID(lc)|(MateID(x.c)<<(2*L))|(MateID(hc)<<(2*(L+1)));}
    return g;
}
__device__ __forceinline__ Code factor_direct_block(Code i,MateID* mout){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;
    int b=f_find_block(i);FBlock x=D_F_BLOCK_BLOCKS[b];Code r=i-x.off;
    uint32_t hr=x.stride?uint32_t(r/x.stride):0,lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;
    uint32_t hc=0,lc=0,har=0,lar=0;
    if(D_F_FIX_LOW){har=hr;lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];lar=D_F_LOW_PACKED_RANK[lc]>>L;if(mout)hc=D_F_HIGH_ALL_CODES[D_F_HIGH_ALL_OFF[x.he]+hr];}
    else{hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];har=D_F_HIGH_PACKED_RANK[hc]>>H;lar=lr;if(mout)lc=D_F_LOW_ALL_CODES[D_F_LOW_ALL_OFF[x.hs]+lr];}
    Code g=D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[x.he]+har]+lar;
    if(mout){if(D_F_FIX_LOW&&!hc)hc=D_F_HIGH_ALL_CODES[D_F_HIGH_ALL_OFF[x.he]+hr];if(!D_F_FIX_LOW&&!lc)lc=D_F_LOW_ALL_CODES[D_F_LOW_ALL_OFF[x.hs]+lr];*mout=MateID(lc)|(MateID(hc)<<(2*L));}
    return g;
}
__global__ void gather_main_kernel(Count*out,MateID*mates,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){MateID m;Code g=factor_direct_main(i,mates?&m:nullptr);out[i]=global_load_main(g);if(mates)mates[i]=m;}}
__global__ void gather_block_kernel(Count*out,MateID*mates,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){MateID m;Code g=factor_direct_block(i,mates?&m:nullptr);out[i]=global_load_block(g);if(mates)mates[i]=m;}}
__global__ void scatter_main_kernel(const Count*in,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride)global_store_main(factor_direct_main(i,nullptr),in[i]);}
__global__ void scatter_block_kernel(const Count*in,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride)global_store_block(factor_direct_block(i,nullptr),in[i]);}
