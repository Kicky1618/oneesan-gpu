...
__constant__ uint32_t* D_F_LOW_PAGE_OFF;
__constant__ uint32_t* D_F_LOW_PAGE_RANK;
...
struct FactorTablesHost {
    ...
    std::vector<uint32_t> low_all_codes,low_mask_codes,low_mask_off,low_page_off,low_page_rank;
...
static FactorTablesHost build_factor_tables(){
    ...
    f.low_page_off.assign(size_t(1)<<(2*(L-Q)),0xffffffffu);
    auto low_slot=[&](uint32_t c)->uint32_t&{uint32_t pg=c>>(2*Q),o=f.low_page_off[pg];if(o==0xffffffffu){o=uint32_t(f.low_page_rank.size());f.low_page_off[pg]=o;f.low_page_rank.resize(size_t(o)+PS,0xffffffffu);}return f.low_page_rank[size_t(o)+(c&PM)];};
    f.high_packed_rank.assign(size_t(1)<<(2*H),0xffffffffu);
    for(int h=0;h<=MAXW;++h){f.low_all_off[h]=f.low_all_codes.size();for(uint32_t r=0;r<la[h].size();++r){auto c=la[h][r];f.low_all_codes.push_back(c);low_slot(c)=r<<L;} }
    f.low_all_off[MAXW+1]=f.low_all_codes.size();
    f.low_mask_off.resize(size_t(LM)*S);
    for(uint32_t m=0;m<LM;++m)for(int h=0;h<S;++h){size_t ix=size_t(m)*S+h;f.low_mask_off[ix]=f.low_mask_codes.size();if(h<=MAXW)for(uint32_t r=0;r<lm[ix].size();++r){auto c=lm[ix][r];f.low_mask_codes.push_back(c);auto &v=low_slot(c);v=(v&~((1u<<L)-1u))|r;}}
    ...
    std::cerr<<"factor tables low_all="<<f.low_all_codes.size()<<" low_mask="<<f.low_mask_codes.size()<<" low_page_trits="<<Q<<" low_page_off_mib="<<double(f.low_page_off.size()*4)/(1<<20)<<" low_page_rank_mib="<<double(f.low_page_rank.size()*4)/(1<<20)<<" high_all="<<f.high_all_codes.size()<<" high_mask="<<f.high_mask_codes.size()<<" high_dense_mib="<<double(f.high_packed_rank.size()*4)/(1<<20)<<"\n";
    return f;
}
...
__device__ __forceinline__ uint32_t factor_low_rank(uint32_t code){constexpr int Q=(LOW_PAGE_TRITS<LOW_LUT_K?LOW_PAGE_TRITS:LOW_LUT_K);constexpr uint32_t PM=(1u<<(2*Q))-1u;uint32_t o=__ldg(D_F_LOW_PAGE_OFF+(code>>(2*Q)));return __ldg(D_F_LOW_PAGE_RANK+o+(code&PM));}
__device__ __forceinline__ Code factor_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);int cv=int(mget(m,L));int bid=3*he+cv;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lp=factor_low_rank(lc),hp=D_F_HIGH_PACKED_RANK[hc];uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int h=seg_end_height(hc,H);FBlock x=D_F_BLOCK_BLOCKS[h];uint32_t lp=factor_low_rank(lc),hp=D_F_HIGH_PACKED_RANK[hc];uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_global_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[hc],lp=factor_low_rank(lc);uint32_t har=hp>>H,lar=lp>>L;Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[he]+har];MateValue c=mget(m,L);if(c>N)rank+=D_FULL_DP[L][he];if(c>R&&he>0)rank+=D_FULL_DP[L][he-1];return rank+lar;
}
__device__ __forceinline__ Code factor_global_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[hc],lp=factor_low_rank(lc);uint32_t har=hp>>H,lar=lp>>L;return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[he]+har]+lar;
}
...
// Re-rank a predecessor that differs from target only at (p,p-1).
// Reuse the unchanged factor-half rank encoded by target index.
__device__ __forceinline__ Code factor_rank_main_pair_from(Code ti,MateID pred,int p){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;
    constexpr uint32_t HM0=(1u<<(2*H))-1u;uint32_t hc0=uint32_t((pred>>(2*(L+1)))&HM0);int he0=seg_end_height(hc0,H);int cv0=int(mget(pred,L));FBlock x=D_F_MAIN_BLOCKS[3*he0+cv0];Code r=ti-x.off;
    ...
    if(p<L){
        ...
        uint32_t lp=factor_low_rank(lc),nlr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);
...
__device__ __forceinline__ Count reverse_eval_main_step_known(const Count*in,const Count*din,MateID t,Code ti,int p){
    Count acc=in[ti];MateID pred=0;bool have=false;
    switch(oneesan::gridfp::mpair(t,p)){
    case oneesan::gridfp::LR:pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::NN);have=true;break;
    case oneesan::gridfp::NR:pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::RN);have=true;break;
    case oneesan::gridfp::NL:pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::LN);have=true;break;
    default:break;}
    if(have){Count c=in[factor_rank_main_pair_from(ti,pred,p)];if(c)acc=add_mod_plain(acc,c);}
    if(oneesan::gridfp::mget(t,p)==oneesan::gridfp::N){MateID b=oneesan::gridfp::mshrink(t,p);Count c=din[factor_rank_block(b)];if(c)acc=add_mod_plain(acc,c);}
    return acc;
}
__device__ __forceinline__ Count reverse_eval_main_step(const Count*in,const Count*din,MateID t,int p){return reverse_eval_main_step_known(in,din,t,factor_rank_main(t),p);}
...
__global__ void reverse2_main_group_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<n;i+=stride){
        ...
        Count acc=reverse_eval_main_step_known(in,din,t,i,p);
        MateID pred=0; bool have=false;
        ...
        if(have){Code pi=factor_rank_main_pair_from(i,pred,q);Count c=reverse_eval_main_step_known(in,din,pred,pi,p);if(c)acc=add_mod_plain(acc,c);}
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+vb4(G_FACTOR.low_page_off)+vb4(G_FACTOR.low_page_rank)+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+vb4(G_FACTOR.high_packed_rank)+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base);
    ...
    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);if(fLPO[d])cudaFree(fLPO[d]);if(fLPR[d])cudaFree(fLPR[d]);if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);if(fHR[d])cudaFree(fHR[d]);if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);}
}
