...
__global__ void factor_prerank_owner_blockorder_2d_kernel(Count*mainv,Count*blockv,int p){
    constexpr uint32_t B=20,M=(1u<<B)-1u;Code lowTotal=D_F_LOW_ALL_OFF[MAXW+1];int bb=int(blockIdx.y);if(bb>=D_F_BLOCK_NBLOCKS)return;FBlock bx=D_F_BLOCK_BLOCKS[bb];
    for(Code q=bx.off+Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<bx.end;q+=st){
        Code loc=q-bx.off;uint32_t hr=uint32_t(loc/bx.stride),blr=uint32_t(loc-Code(hr)*bx.stride);
...
__global__ void factor_high_prerank_owner_blockorder_2d_kernel(Count*mainv,Count*blockv,int p){
    constexpr uint32_t B=20,M=(1u<<B)-1u;Code highTotal=D_F_HIGH_ALL_OFF[MAXW+1];int bb=int(blockIdx.y);if(bb>=D_F_BLOCK_NBLOCKS)return;FBlock bx=D_F_BLOCK_BLOCKS[bb];
    for(Code q=bx.off+Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<bx.end;q+=st){
        Code loc=q-bx.off;uint32_t bhr=uint32_t(loc/bx.stride),lr=uint32_t(loc-Code(bhr)*bx.stride);
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){auto launchN=[&](Code n,auto fn,const char*w){if(!n)return;int nb=int(std::min<Code>(65535,(n+threads-1)/threads));fn(nb);ck(cudaGetLastError(),w);};
            if(fixLow){auto im=make_high_prerank_map(fmb,p,3),xm=make_high_prerank_map(fmb,p,2);Code in=im.nblocks?im.end[im.nblocks-1]:0,xn=xm.nblocks?xm.end[xm.nblocks-1]:0;Code maxb=0;for(auto const&b:fdb)maxb=std::max(maxb,b.end-b.off);if(maxb){unsigned gx=unsigned(std::min<Code>(65535,(maxb+threads-1)/threads));factor_high_prerank_owner_blockorder_2d_kernel<<<dim3(std::max(1u,gx),unsigned(fdb.size())),threads,0,c.sMain>>>(cur,dcur,p);ck(cudaGetLastError(),"factor high prerank owner blockorder 2d");}launchN(in+xn,[&](int nb){factor_high_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,im,xm);},"factor high prerank inverse closure");}
            else{Code maxb=0;for(auto const&b:fdb)maxb=std::max(maxb,b.end-b.off);if(maxb){unsigned gx=unsigned(std::min<Code>(65535,(maxb+threads-1)/threads));factor_prerank_owner_blockorder_2d_kernel<<<dim3(std::max(1u,gx),unsigned(fdb.size())),threads,0,c.sMain>>>(cur,dcur,p);ck(cudaGetLastError(),"factor prerank owner blockorder 2d");}auto im=make_prerank_orbit_map(fmb,p,3),sm=make_prerank_orbit_map(fmb,p,2);Code in=im.nblocks?im.end[im.nblocks-1]:0,sn=sm.nblocks?sm.end[sm.nblocks-1]:0;launchN(in+sn,[&](int nb){factor_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,im,sm,p);},"factor prerank inverse closure");}
        }
...
int main(int argc,char**argv){
    ...
    SparseVmmU32 fLDv[MAXGPU]{},fHRv[MAXGPU]{};uint32_t *fLA[MAXGPU]{},*fLM[MAXGPU]{},*fLO[MAXGPU]{},*fLD[MAXGPU]{},*fHA[MAXGPU]{},*fHM[MAXGPU]{},*fHO[MAXGPU]{},*fHR[MAXGPU]{};unsigned long long *prOB[MAXGPU]{},*prCI[MAXGPU]{},*prH[MAXGPU]{},*hpOB[MAXGPU]{},*hpCI[MAXGPU]{},*hpX[MAXGPU]{};uint32_t *prCIS[MAXGPU]{},*hpCIS[MAXGPU]{};Code *fHMB[MAXGPU]{},*fHBB[MAXGPU]{};
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-fullprerank-orbit-ownerblock2d-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prOB[d])cudaFree(prOB[d]);if(prCI[d])cudaFree(prCI[d]);if(prCIS[d])cudaFree(prCIS[d]);if(prH[d])cudaFree(prH[d]);if(hpOB[d])cudaFree(hpOB[d]);if(hpCI[d])cudaFree(hpCI[d]);if(hpCIS[d])cudaFree(hpCIS[d]);if(hpX[d])cudaFree(hpX[d]);}
}
