...
__global__ void reverse_main_p1_kernel(const Count*in,const Count*din,const MateID*mates,
        Code n,Code dn,Count*out){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        constexpr int p=1; MateID t=mates?mates[i]:factor_unrank_main(i); Count acc=in[i];
        auto addp=[&](MateID m){Count c=in[factor_rank_main(m)];if(c)acc=add_mod_plain(acc,c);};
        switch(oneesan::gridfp::mpair(t,p)){
        case oneesan::gridfp::LR: addp(oneesan::gridfp::msetpair(t,p,oneesan::gridfp::NN)); break;
        case oneesan::gridfp::RN: addp(oneesan::gridfp::msetpair(t,p,oneesan::gridfp::NR)); break;
        case oneesan::gridfp::LN: addp(oneesan::gridfp::msetpair(t,p,oneesan::gridfp::NL)); break;
        case oneesan::gridfp::NR: addp(oneesan::gridfp::msetpair(t,p,oneesan::gridfp::RN)); break;
        case oneesan::gridfp::NL: addp(oneesan::gridfp::msetpair(t,p,oneesan::gridfp::LN)); break;
        case oneesan::gridfp::NN:{
            if(height_before_rank_pos<TARGET_W>(t,p)>0)addp(oneesan::gridfp::msetpair(t,p,oneesan::gridfp::RL));
            int s=1;
            for(int q=2;q<TARGET_W&&s>0;++q){
                auto v=oneesan::gridfp::mget(t,q);
                if(v==oneesan::gridfp::R&&s==1){MateID c=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::RR);c=oneesan::gridfp::mset(c,q,oneesan::gridfp::L);addp(c);}
...
__global__ void reverse_block_group_kernel(const Count*in,Code n,const MateID*mates,Count*out,Code dn,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<dn;i+=stride){
...
// Two already-composed updates, evaluated at an arbitrary main target.
__device__ __forceinline__ Count reverse_eval_main2(const Count*in,const Count*din,MateID t,int p){
    int q=p-1;
    Count acc=reverse_eval_main_step(in,din,t,p);
    MateID pred=0; bool have=false;
    switch(oneesan::gridfp::mpair(t,q)){
    ...
    if(have){Count c=reverse_eval_main_step(in,din,pred,p);if(c)acc=add_mod_plain(acc,c);}
    if(oneesan::gridfp::mget(t,q)==oneesan::gridfp::N){MateID b=oneesan::gridfp::mshrink(t,q);Count c=reverse_eval_block_step(in,b,p);if(c)acc=add_mod_plain(acc,c);}
    return acc;
}

__device__ __forceinline__ void reverse_eval_main2_add(Count&acc,const Count*in,const Count*din,MateID m,int p){
    Count c=reverse_eval_main2(in,din,m,p);if(c)acc=add_mod_plain(acc,c);
}

__device__ __forceinline__ void reverse_eval_main1_add(Count&acc,const Count*in,const Count*din,MateID m,int p){
    Count c=reverse_eval_main_step(in,din,m,p);if(c)acc=add_mod_plain(acc,c);
}

__device__ __forceinline__ Count reverse_eval_block2(const Count*in,const Count*din,MateID b,int p){
    int q=p-1; Count acc=0; auto low=oneesan::gridfp::mget(b,q-1);
    if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
        reverse_eval_main1_add(acc,in,din,oneesan::gridfp::minsert(b,q,oneesan::gridfp::N),p);
    }else if(low==oneesan::gridfp::N){
        MateID u=oneesan::gridfp::minsert(b,q-1,oneesan::gridfp::N);
        if(height_before_rank_pos<TARGET_W>(u,q)>0)reverse_eval_main1_add(acc,in,din,oneesan::gridfp::msetpair(u,q,oneesan::gridfp::RL),p);
        int s=1;
        for(int r=q-2;r>=0&&s>0;--r){
            auto v=oneesan::gridfp::mget(u,r);
            if(v==oneesan::gridfp::L&&s==1){MateID c=oneesan::gridfp::msetpair(u,q,oneesan::gridfp::LL);c=oneesan::gridfp::mset(c,r,oneesan::gridfp::R);reverse_eval_main1_add(acc,in,din,c,p);}
            if(v==oneesan::gridfp::L)++s;else if(v==oneesan::gridfp::R)--s;
        }
        s=1;
        for(int r=q+1;r<TARGET_W&&s>0;++r){
            auto v=oneesan::gridfp::mget(u,r);
            if(v==oneesan::gridfp::R&&s==1){MateID c=oneesan::gridfp::msetpair(u,q,oneesan::gridfp::RR);c=oneesan::gridfp::mset(c,r,oneesan::gridfp::L);reverse_eval_main1_add(acc,in,din,c,p);}
            if(v==oneesan::gridfp::L)--s;else if(v==oneesan::gridfp::R)++s;
        }
    }
    return acc;
...
__global__ void reverse3_main_group_kernel(const Count*in,const Count*din,const MateID*mates,Code n,Count*out,int p){
    int r=p-2; Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        MateID t=mates?mates[i]:factor_unrank_main(i); Count acc=reverse_eval_main2(in,din,t,p);
        MateID pred=0; bool have=false;
        switch(oneesan::gridfp::mpair(t,r)){
        case oneesan::gridfp::LR: pred=oneesan::gridfp::msetpair(t,r,oneesan::gridfp::NN);have=true;break;
        case oneesan::gridfp::NR: pred=oneesan::gridfp::msetpair(t,r,oneesan::gridfp::RN);have=true;break;
        case oneesan::gridfp::NL: pred=oneesan::gridfp::msetpair(t,r,oneesan::gridfp::LN);have=true;break;
        default: break;
        }
        if(have){Count c=reverse_eval_main2(in,din,pred,p);if(c)acc=add_mod_plain(acc,c);}
        if(oneesan::gridfp::mget(t,r)==oneesan::gridfp::N){MateID b=oneesan::gridfp::mshrink(t,r);Count c=reverse_eval_block2(in,din,b,p);if(c)acc=add_mod_plain(acc,c);}
        out[i]=acc;
    }
}

__global__ void reverse3_block_group_kernel(const Count*in,const Count*din,const MateID*mates,Count*out,Code dn,int p){
    int r=p-2; Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<dn;i+=stride){
        MateID b=mates?mates[i]:factor_unrank_block(i); Count acc=0; auto low=oneesan::gridfp::mget(b,r-1);
        if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
            reverse_eval_main2_add(acc,in,din,oneesan::gridfp::minsert(b,r,oneesan::gridfp::N),p);
        }else if(low==oneesan::gridfp::N){
            MateID u=oneesan::gridfp::minsert(b,r-1,oneesan::gridfp::N);
            if(height_before_rank_pos<TARGET_W>(u,r)>0)reverse_eval_main2_add(acc,in,din,oneesan::gridfp::msetpair(u,r,oneesan::gridfp::RL),p);
            int ss=1;
            for(int k=r-2;k>=0&&ss>0;--k){auto v=oneesan::gridfp::mget(u,k);if(v==oneesan::gridfp::L&&ss==1){MateID c=oneesan::gridfp::msetpair(u,r,oneesan::gridfp::LL);c=oneesan::gridfp::mset(c,k,oneesan::gridfp::R);reverse_eval_main2_add(acc,in,din,c,p);}if(v==oneesan::gridfp::L)++ss;else if(v==oneesan::gridfp::R)--ss;}
            ss=1;
            for(int k=r+1;k<TARGET_W&&ss>0;++k){auto v=oneesan::gridfp::mget(u,k);if(v==oneesan::gridfp::R&&ss==1){MateID c=oneesan::gridfp::msetpair(u,r,oneesan::gridfp::RR);c=oneesan::gridfp::mset(c,k,oneesan::gridfp::L);reverse_eval_main2_add(acc,in,din,c,p);}if(v==oneesan::gridfp::L)--ss;else if(v==oneesan::gridfp::R)++ss;}
        }
        out[i]=acc;
    }
}
...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;MateID*dMate=nullptr,*dBlockMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0;uint64_t groups=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    size_t countBytes=size_t(2*ms.size+2*ds.size)*sizeof(Count),mateBytes=size_t(ms.size)*sizeof(MateID),blockMateBytes=size_t(ds.size)*sizeof(MateID);bool useMate=!pg.use_mi&&(countBytes+mateBytes<=target);bool useBlockMate=!pg.use_di&&(countBytes+(useMate?mateBytes:0)+blockMateBytes<=target);
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;){
        if(p>3 && p-2>=wp.p_lo){
            if(ms.size)reverse3_main_group_kernel<<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,nxt,p);
            if(ds.size)reverse3_block_group_kernel<<<bd,threads,0,c.sBlock>>>(cur,dcur,useBlockMate?c.dBlockMate:nullptr,dnext,ds.size,p);
            p-=3;
            ...
            if(p>1){
                ...
                if(ds.size)reverse_block_group_kernel<<<bd,threads,0,c.sBlock>>>(cur,ms.size,useBlockMate?c.dBlockMate:nullptr,dnext,ds.size,p);
            }else{
                if(ms.size)reverse_main_p1_kernel<<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,ds.size,nxt);
                if(ds.size)ck(cudaMemsetAsync(dnext,0,size_t(ds.size)*sizeof(Count),c.sBlock),"p1 clear next D");
            }
            --p;
        }
        ck(cudaGetLastError(),"reverse3 transition");
