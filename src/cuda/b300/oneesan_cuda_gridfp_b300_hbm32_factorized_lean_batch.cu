...
// is required.
__device__ __forceinline__ uint32_t code_shrink_2bit(uint32_t code,int k){
    uint32_t lowmask=k?((1u<<(2*k))-1u):0u;
    return ((code&~lowmask)>>2)|(code&lowmask);
}
__device__ __forceinline__ Code factor_rank_block_codes(uint32_t lc,uint32_t hc){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;
    int h=seg_end_height(hc,H);FBlock x=D_F_BLOCK_BLOCKS[h];
    uint32_t lp=D_F_LOW_PACKED_RANK[lc],hp=D_F_HIGH_PACKED_RANK[hc];
    uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);
    uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));
    return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ int inverse_main_pair(int pair){
    using namespace oneesan::gridfp;
    switch(pair){case oneesan::gridfp::LR:return oneesan::gridfp::NN;case oneesan::gridfp::NR:return oneesan::gridfp::RN;case oneesan::gridfp::NL:return oneesan::gridfp::LN;default:return -1;}
}

// Lean theorem `includeToMain_preserves_delta` justifies keeping the current
// FBlock when the two-symbol inverse lies wholly inside LOW or HIGH.
__global__ void reverse_main_group_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<n;i+=stride){
        int bid=f_find_main(i);FBlock x=D_F_MAIN_BLOCKS[bid];Code rr=i-x.off;
        uint32_t hr=x.stride?uint32_t(rr/x.stride):0,lr=x.stride?uint32_t(rr-Code(hr)*x.stride):0;
        uint32_t hc,lc;
        if(D_F_FIX_LOW){hc=D_F_HIGH_ALL_CODES[D_F_HIGH_ALL_OFF[x.he]+hr];lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];}
        else{hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];lc=D_F_LOW_ALL_CODES[D_F_LOW_ALL_OFF[x.hs]+lr];}
        Count acc=in[i];
        ...
        if(p<L){
            int sh=2*(p-1);int pr=inverse_main_pair((lc>>sh)&15u);
            if(pr>=0){uint32_t plc=(lc&~(15u<<sh))|(uint32_t(pr)<<sh);uint32_t pk=D_F_LOW_PACKED_RANK[plc];uint32_t plr=D_F_FIX_LOW?(pk&((1u<<L)-1u)):(pk>>L);Count c=in[x.off+Code(hr)*x.stride+plr];if(c)acc=add_mod_plain(acc,c);}localDone=true;
        }else if(p>L+1){
            int pp=p-(L+1),sh=2*(pp-1);int pr=inverse_main_pair((hc>>sh)&15u);
            if(pr>=0){uint32_t phc=(hc&~(15u<<sh))|(uint32_t(pr)<<sh);uint32_t pk=D_F_HIGH_PACKED_RANK[phc];uint32_t phr=D_F_FIX_LOW?(pk>>H):(pk&((1u<<H)-1u));Count c=in[x.off+Code(phr)*x.stride+lr];if(c)acc=add_mod_plain(acc,c);}localDone=true;
        }
        MateID m=0;
        if(!localDone){
            m=MateID(lc)|(MateID(x.c)<<(2*L))|(MateID(hc)<<(2*(L+1)));
            MateID pred=0; bool have=false;
            switch(oneesan::gridfp::mpair(m,p)){
            case oneesan::gridfp::LR:pred=oneesan::gridfp::msetpair(m,p,oneesan::gridfp::NN);have=true;break;
            case oneesan::gridfp::NR:pred=oneesan::gridfp::msetpair(m,p,oneesan::gridfp::RN);have=true;break;
            case oneesan::gridfp::NL:pred=oneesan::gridfp::msetpair(m,p,oneesan::gridfp::LN);have=true;break;
            default:break;}
            if(have){Count c=in[factor_rank_main(pred)];if(c)acc=add_mod_plain(acc,c);}
        }
        int vp;
        if(p<L)vp=(lc>>(2*p))&3u;else if(p==L)vp=x.c;else vp=(hc>>(2*(p-L-1)))&3u;
        if(vp==oneesan::gridfp::N){
            uint32_t blc,bhc;
            if(p<L){uint32_t lowc=lc|(uint32_t(x.c)<<(2*L));blc=code_shrink_2bit(lowc,p);bhc=hc;}
            else if(p==L){blc=lc;bhc=hc;}
            else{uint32_t ch=uint32_t(x.c)|(hc<<2);blc=lc;bhc=code_shrink_2bit(ch,p-L);}
            Count c=din[factor_rank_block_codes(blc,bhc)];if(c)acc=add_mod_plain(acc,c);
        }
        out[i]=acc;
    }
}
...
__global__ void forward_block_only_kernel(const Count*in,const MateID*mates,Code n,Count*out_block,int p){
    ...
    for(;i<n;i+=stride){
        ...
        MateID m=mates?mates[i]:factor_unrank_main(i);auto pr=oneesan::gridfp::mpair(m,p);
        auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);
        if(z.valid&&z.blocked){Code j=factor_rank_block(z.mate);if(pr==oneesan::gridfp::NR||pr==oneesan::gridfp::NL)out_block[j]=c;else atomic_add_mod(out_block+j,c);}
    }
}
