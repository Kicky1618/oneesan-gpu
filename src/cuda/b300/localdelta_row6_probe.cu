...
// Local factor-rank update for a predecessor differing only at (p,p-1).
__device__ __forceinline__ Code factor_rank_main_pair_from(Code ti,MateID pred,int p){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;int bid=f_find_main(ti);FBlock x=D_F_MAIN_BLOCKS[bid];Code r=ti-x.off;
    uint32_t hr=x.stride?uint32_t(r/x.stride):0,lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;
    if(p<L){constexpr uint32_t LM=(1u<<(2*L))-1u;uint32_t lc=uint32_t(pred)&LM,lp=D_F_LOW_PACKED_RANK[ternary_index_dev(lc,L)];uint32_t nlr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);return ti-Code(lr)+Code(nlr);}
    if(p>=L+2){constexpr uint32_t HM=(1u<<(2*H))-1u;uint32_t hc=uint32_t((pred>>(2*(L+1)))&HM),hp=D_F_HIGH_PACKED_RANK[hc];uint32_t nhr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(nhr)*x.stride+lr;}
...
__device__ __forceinline__ Count reverse_eval_main_step_known(const Count*in,const Count*din,MateID t,Code ti,int p){
    ...
    switch(oneesan::gridfp::mpair(t,p)){case oneesan::gridfp::LR:pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::NN);have=true;break;case oneesan::gridfp::NR:pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::RN);have=true;break;case oneesan::gridfp::NL:pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::LN);have=true;break;default:break;}
    if(have){Count c=in[factor_rank_main_pair_from(ti,pred,p)];if(c)acc=add_mod_plain(acc,c);}if(oneesan::gridfp::mget(t,p)==oneesan::gridfp::N){MateID b=oneesan::gridfp::mshrink(t,p);Count c=din[factor_rank_block(b)];if(c)acc=add_mod_plain(acc,c);}return acc;
}
...
__device__ __forceinline__ void reverse2_add_main(Count&acc,const Count*in,const Count*din,MateID m,int p){
    Code mi=factor_rank_main(m);Count c=reverse_eval_main_step_known(in,din,m,mi,p);if(c)acc=add_mod_plain(acc,c);
}
