...
struct FullOrbitAddress {
    int owner = -1;
    Code offset = 0;
};

static FullOrbitAddress fullorbit_main_address_host(
    MateID m,
    ...
    if (hp == 0xffffffffu || lp == 0xffffffffu || bid >= layout.main_blocks.size()) {
        std::cerr << "fullorbit invalid host main address\n";
        std::exit(190);
    }
...
static std::vector<uint32_t> build_fullorbit_high_route(
    const StorageFactorHost& storage
) {
    constexpr int H = HIGH_LUT_K;
    ...
    for (int he = 0; he <= H + 1; ++he) {
        ...
        for (uint32_t r = 0; r < b - a; ++r) {
            ...
            if (packed == 0xffffffffu) {
                std::cerr << "fullorbit missing HIGH packed rank\n";
                std::exit(191);
            }
...
struct FullOrbitWorker {
    ...
    void ensure(Code main_n, Code block_n) {
        ...
        if (need > cap) {
            if (arena) ck(cudaFree(arena), "fullorbit worker free arena");
            cap = need;
            ck(cudaMalloc(&arena, cap), "fullorbit HIGH scratch");
        }
...
static std::vector<FullOrbitHighJob> build_fullorbit_high_jobs() {
    constexpr uint32_t NM = 1u << LOW_LUT_K;
    std::vector<FullOrbitHighJob> jobs;
    jobs.reserve(NM);
...
static void configure_fullorbit_high_group(uint32_t mask) {
    constexpr int L = LOW_LUT_K;
    ...
    if (mb.empty() || db.empty() || mb.back().end != ms.size || db.back().end != ds.size) {
        std::cerr << "fullorbit HIGH factor-size mismatch mask=" << mask << '\n';
        std::exit(192);
    }
    const int mn = int(mb.size());
    const int dn = int(db.size());
    const int fix_low = 1;
    ck(cudaMemcpyToSymbol(D_F_MAIN_BLOCKS, mb.data(), mb.size() * sizeof(FBlock)), "fullorbit high main blocks");
    ck(cudaMemcpyToSymbol(D_F_BLOCK_BLOCKS, db.data(), db.size() * sizeof(FBlock)), "fullorbit high block blocks");
    ck(cudaMemcpyToSymbol(D_F_MAIN_NBLOCKS, &mn, sizeof(mn)), "fullorbit high main n");
    ck(cudaMemcpyToSymbol(D_F_BLOCK_NBLOCKS, &dn, sizeof(dn)), "fullorbit high block n");
    ck(cudaMemcpyToSymbol(D_F_MASK, &mask, sizeof(mask)), "fullorbit high mask");
    ck(cudaMemcpyToSymbol(D_F_FIX_LOW, &fix_low, sizeof(fix_low)), "fullorbit high mode");
    ck(cudaMemcpyToSymbol(D_MAIN_FIXED, &mf, sizeof(mf)), "fullorbit high mf");
    ck(cudaMemcpyToSymbol(D_MAIN_OCC, &mo, sizeof(mo)), "fullorbit high mo");
    ck(cudaMemcpyToSymbol(D_BLOCK_FIXED, &bf, sizeof(bf)), "fullorbit high bf");
    ck(cudaMemcpyToSymbol(D_BLOCK_OCC, &bo, sizeof(bo)), "fullorbit high bo");
    ck(cudaMemcpyToSymbol(D_MAIN_DP, ms.dp, sizeof(ms.dp)), "fullorbit high main dp");
    ck(cudaMemcpyToSymbol(D_BLOCK_DP, ds.dp, sizeof(ds.dp)), "fullorbit high block dp");
}

static void process_fullorbit_high_job(
    FullOrbitWorker& w, const FullOrbitHighJob& job, int threads
) {
    ck(cudaSetDevice(w.dev), "fullorbit high set device");
    if (!job.main_n && !job.block_n) return;
    configure_fullorbit_high_group(job.low_mask);
    ...
    ck(cudaGetLastError(), "fullorbit high gather");
    ck(cudaDeviceSynchronize(), "fullorbit high gather sync");
    ...
    for (int p = TARGET_W - 1; p >= LOW_LUT_K + 1; --p) {
        ...
        ck(cudaGetLastError(), "fullorbit high orbit");
        ck(cudaDeviceSynchronize(), "fullorbit high orbit sync");
        w.high_orbit_s += ram_seconds_since(t);

        t = std::chrono::steady_clock::now();
        if (job.main_n)
            maskshard_main_highdesc_closure_inplace_kernel<<<bm, threads>>>(w.a, w.d, job.main_n, p);
        ck(cudaGetLastError(), "fullorbit high closure");
        ck(cudaDeviceSynchronize(), "fullorbit high closure sync");
    ...
    ck(cudaGetLastError(), "fullorbit high scatter");
    ck(cudaDeviceSynchronize(), "fullorbit high scatter sync");
    w.high_io_s += ram_seconds_since(t);
    ++w.high_groups;
}

static void process_fullorbit_low_group(
    FullOrbitWorker& w,
    const MaskShardLayout& shard,
    uint32_t mask,
    ...
    ck(cudaSetDevice(w.dev), "fullorbit low set device");
    ...
    for (int p = LOW_LUT_K; p >= 1; --p) {
        ...
        ck(cudaGetLastError(), "fullorbit low orbit");
        ck(cudaDeviceSynchronize(), "fullorbit low orbit sync");
        w.low_orbit_s += ram_seconds_since(t);

        t = std::chrono::steady_clock::now();
        if (main_n)
            maskshard_main_lowdesc_closure_inplace_kernel<<<bm, threads>>>(mainv, blockv, main_n, p);
        ck(cudaGetLastError(), "fullorbit low closure");
        ck(cudaDeviceSynchronize(), "fullorbit low closure sync");
...
int main(int argc, char** argv) {
    ...
    if (n + 1 != TARGET_W || n < 2 || TARGET_W > MAXW) {
        std::cerr << "fullorbit binary specialized for n=" << TARGET_W - 1 << '\n';
        return 1;
    }
    if constexpr (LOW_LUT_K + HIGH_LUT_K != TARGET_W - 1) {
        std::cerr << "fullorbit requires LOW+HIGH=W-1\n";
        return 1;
    }
    ...
    const double desc_build_s = ram_seconds_since(desc_t0);
    ...
    int visible = 0;
    ck(cudaGetDeviceCount(&visible), "fullorbit device count");
    const int ngpu = std::min({requested, visible, 8});
    if (ngpu < 1) return 2;
    int peers = 0;
    for (int a = 0; a < ngpu; ++a) for (int b = 0; b < ngpu; ++b) if (a != b) {
        int can = 0;
        ck(cudaDeviceCanAccessPeer(&can, a, b), "fullorbit can peer");
        if (!can) continue;
        ck(cudaSetDevice(a), "fullorbit peer set");
        cudaError_t e = cudaDeviceEnablePeerAccess(b, 0);
        if (e == cudaErrorPeerAccessAlreadyEnabled) cudaGetLastError();
        else ck(e, "fullorbit enable peer");
        ++peers;
    }
    if (ngpu > 1 && peers != ngpu * (ngpu - 1)) {
        std::cerr << "fullorbit requires full P2P: " << peers << '/' << ngpu * (ngpu - 1) << '\n';
        return 3;
    }

    MaskShardLayout shard = build_high_mask_shard_layout(storage, layout, ngpu);
    const auto high_route = build_fullorbit_high_route(storage);
    report_high_mask_shard_layout(shard);

    Count* mp[8]{};
    Count* bp[8]{};
    for (int d = 0; d < ngpu; ++d) {
        ck(cudaSetDevice(d), "fullorbit auth set");
        if (shard.gpu_main[d])
            ck(cudaMalloc(&mp[d], size_t(shard.gpu_main[d]) * sizeof(Count)), "fullorbit auth main");
        if (shard.gpu_block[d])
            ck(cudaMalloc(&bp[d], size_t(shard.gpu_block[d]) * sizeof(Count)), "fullorbit auth block");
        if (mp[d]) ck(cudaMemset(mp[d], 0, size_t(shard.gpu_main[d]) * sizeof(Count)), "fullorbit zero main");
        if (bp[d]) ck(cudaMemset(bp[d], 0, size_t(shard.gpu_block[d]) * sizeof(Count)), "fullorbit zero block");
    }
    ...
    std::vector<FullOrbitWorker> workers(ngpu);
    for (int d = 0; d < ngpu; ++d) {
        ck(cudaSetDevice(d), "fullorbit table set");
        ...
        ck(cudaMemcpyToSymbol(D_MOD, &mod, sizeof(mod)), "fullorbit modulus");
        workers[d].init(d);
    }

    const MateID init = MateID(R) << (2 * (TARGET_W - 1));
    const FullOrbitAddress ia = fullorbit_main_address_host(init, storage, layout, shard);
    const FullOrbitAddress oa = fullorbit_main_address_host(MateID(R), storage, layout, shard);
    const Count one = 1;
    ck(cudaSetDevice(ia.owner), "fullorbit init device");
    ck(cudaMemcpy(mp[ia.owner] + ia.offset, &one, sizeof(one), cudaMemcpyHostToDevice), "fullorbit init one");

    const auto high_jobs = build_fullorbit_high_jobs();
        ...
        for (int d = 0; d < ngpu; ++d) {
            ts.emplace_back([&, d] {
                for (;;) {
                    ...
                    process_fullorbit_high_job(workers[d], high_jobs[q], threads);
                }
            });
        }
        ...
        for (int d = 0; d < ngpu; ++d) {
            ts.emplace_back([&, d] {
                for (uint32_t mask : low_jobs[d])
                    process_fullorbit_low_group(workers[d], shard, mask, mp[d], bp[d], threads);
            });
        }
        for (auto& t : ts) t.join();
        std::cerr << "fullorbit row " << row + 1 << '/' << TARGET_W << '\n';
    }
    const double wall_s = ram_seconds_since(wall0);

    Count answer = 0;
    ck(cudaSetDevice(oa.owner), "fullorbit answer device");
    ck(cudaMemcpy(&answer, mp[oa.owner] + oa.offset, sizeof(answer), cudaMemcpyDeviceToHost), "fullorbit answer");
    ...
    std::cout << "backend=b300-factorized-maskshard-v0.4-fullorbit"
              << " n=" << n
              << " gpus=" << ngpu
              << " residue=" << answer
              << " modulus=" << mod
    ...
    for (int d = 0; d < ngpu; ++d) {
        ck(cudaSetDevice(d), "fullorbit cleanup set");
        workers[d].release();
        shard_dev[d].release();
