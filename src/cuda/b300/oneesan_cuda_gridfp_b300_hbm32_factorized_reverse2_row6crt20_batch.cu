...
#include "../../common/ternary_rank_index.hpp"
#ifndef GRIDFP_TERNARY_RANK_LAYOUT
#define GRIDFP_TERNARY_RANK_LAYOUT 0
#endif
#ifndef GRIDFP_TRIT_CHUNK
#define GRIDFP_TRIT_CHUNK 7
#endif
static_assert(GRIDFP_TRIT_CHUNK==4||GRIDFP_TRIT_CHUNK==7,"rank conversion chunks: 4 or 7");
#if GRIDFP_TERNARY_RANK_LAYOUT
#if defined(GRIDFP_TRIT_CONSTANT) && GRIDFP_TRIT_CONSTANT
static_assert(GRIDFP_TRIT_CHUNK==4,"constant conversion table supports chunk 4 only");
__constant__ uint16_t D_F_TRIT[1u<<(2*GRIDFP_TRIT_CHUNK)];
#else
__device__ uint16_t D_F_TRIT[1u<<(2*GRIDFP_TRIT_CHUNK)];
#endif
#endif
static uint32_t rank_table_index_host(uint32_t code){
#if GRIDFP_TERNARY_RANK_LAYOUT
    return oneesan::ternary_rank_index(code);
#else
    return code;
#endif
}
template<int Width> __device__ __forceinline__ uint32_t rank_table_index(uint32_t code){
#if GRIDFP_TERNARY_RANK_LAYOUT
    constexpr uint32_t mask=(1u<<(2*GRIDFP_TRIT_CHUNK))-1u;
    constexpr uint32_t radix=GRIDFP_TRIT_CHUNK==7?2187u:81u;
    uint32_t index=0,scale=1;
#pragma unroll
    for(int off=0;off<Width;off+=GRIDFP_TRIT_CHUNK){index+=D_F_TRIT[(code>>(2*off))&mask]*scale;scale*=radix;}
    return index;
#else
    return code;
#endif
}
#ifndef GRIDFP_SHARD_SEARCH
#define GRIDFP_SHARD_SEARCH 1
#endif

// Sparse traversal is experimental: slower in the RTX 3070 scan benchmark.
#ifndef GRIDFP_SPARSE_REVERSE
#define GRIDFP_SPARSE_REVERSE 0
#endif
#ifndef GRIDFP_REUSE_MAIN_RANK
#define GRIDFP_REUSE_MAIN_RANK 1
#endif
#ifndef FACTOR_RECIPROCAL_DIV
// Set to 0 for a same-source GPU timing comparison with runtime division.
#define FACTOR_RECIPROCAL_DIV 1
#endif
...
__constant__ uint64_t D_MOD_RECIPROCAL;
...
#ifndef GRIDFP_BATCH_FACTOR_CONFIG
#define GRIDFP_BATCH_FACTOR_CONFIG 1
#endif
#ifndef GRIDFP_LEGACY_GROUP_UPLOAD
#define GRIDFP_LEGACY_GROUP_UPLOAD 0
#endif
struct FBlock { Code off,end; uint32_t stride; uint8_t he,hs,c,pad; uint64_t reciprocal; };
struct FactorRuntimeCfg {
    FBlock main_blocks[64]{},block_blocks[32]{};
    int main_n=0,block_n=0;
    uint32_t mask=0;
    int fix_low=0;
};
// Experimental placement for divergent factor-block reads; constant is faster on sm_86.
#ifndef GRIDFP_GLOBAL_FACTOR_CONFIG
#define GRIDFP_GLOBAL_FACTOR_CONFIG 0
#endif
#if GRIDFP_BATCH_FACTOR_CONFIG
#if GRIDFP_GLOBAL_FACTOR_CONFIG
__device__ FactorRuntimeCfg D_FACTOR_CFG;
#else
__constant__ FactorRuntimeCfg D_FACTOR_CFG;
#endif
#define D_F_MAIN_BLOCKS D_FACTOR_CFG.main_blocks
#define D_F_BLOCK_BLOCKS D_FACTOR_CFG.block_blocks
#define D_F_MAIN_NBLOCKS D_FACTOR_CFG.main_n
#define D_F_BLOCK_NBLOCKS D_FACTOR_CFG.block_n
#define D_F_MASK D_FACTOR_CFG.mask
#define D_F_FIX_LOW D_FACTOR_CFG.fix_low
#else
...
// The host configuration must remain alive until the selected stream completes.
static void upload_factor_config(const FactorRuntimeCfg& f,cudaStream_t stream=nullptr){
#if GRIDFP_BATCH_FACTOR_CONFIG
    ck(cudaMemcpyToSymbolAsync(D_FACTOR_CFG,&f,sizeof(f),0,cudaMemcpyHostToDevice,stream),"factor config");
#else
    ck(cudaMemcpyToSymbolAsync(D_F_MAIN_BLOCKS,f.main_blocks,f.main_n*sizeof(FBlock),0,cudaMemcpyHostToDevice,stream),"factor main blocks");
    ck(cudaMemcpyToSymbolAsync(D_F_BLOCK_BLOCKS,f.block_blocks,f.block_n*sizeof(FBlock),0,cudaMemcpyHostToDevice,stream),"factor block blocks");
    ck(cudaMemcpyToSymbolAsync(D_F_MAIN_NBLOCKS,&f.main_n,sizeof(f.main_n),0,cudaMemcpyHostToDevice,stream),"factor main n");
    ck(cudaMemcpyToSymbolAsync(D_F_BLOCK_NBLOCKS,&f.block_n,sizeof(f.block_n),0,cudaMemcpyHostToDevice,stream),"factor block n");
    ck(cudaMemcpyToSymbolAsync(D_F_MASK,&f.mask,sizeof(f.mask),0,cudaMemcpyHostToDevice,stream),"factor mask");
    ck(cudaMemcpyToSymbolAsync(D_F_FIX_LOW,&f.fix_low,sizeof(f.fix_low),0,cudaMemcpyHostToDevice,stream),"factor mode");
#endif
}
static void upload_rank_index_table(){
#if GRIDFP_TERNARY_RANK_LAYOUT
    std::array<uint16_t,1u<<(2*GRIDFP_TRIT_CHUNK)> table{};
    for(uint32_t code=0;code<table.size();++code)table[code]=oneesan::ternary_rank_index(code);
    ck(cudaMemcpyToSymbol(D_F_TRIT,table.data(),sizeof(table)),"ternary rank conversion");
#endif
}
...
struct FactorTablesHost {
    ...
    std::vector<uint32_t> low_all_codes,low_mask_codes,low_mask_off,low_dense_packed_rank;
...
static FactorTablesHost build_factor_tables(){
    ...
    f.low_dense_packed_rank.assign(GRIDFP_TERNARY_RANK_LAYOUT?oneesan::ternary_rank_capacity(L):(size_t(1)<<(2*L)),0xffffffffu);
    f.high_packed_rank.assign(GRIDFP_TERNARY_RANK_LAYOUT?oneesan::ternary_rank_capacity(H):(size_t(1)<<(2*H)),0xffffffffu);
    for(int h=0;h<=MAXW;++h){f.low_all_off[h]=f.low_all_codes.size();for(uint32_t r=0;r<la[h].size();++r){auto c=la[h][r];f.low_all_codes.push_back(c);f.low_dense_packed_rank[rank_table_index_host(c)]=r<<L;} }
    f.low_all_off[MAXW+1]=f.low_all_codes.size();
    f.low_mask_off.resize(size_t(LM)*S);
    for(uint32_t m=0;m<LM;++m)for(int h=0;h<S;++h){size_t ix=size_t(m)*S+h;f.low_mask_off[ix]=f.low_mask_codes.size();if(h<=MAXW)for(uint32_t r=0;r<lm[ix].size();++r){auto c=lm[ix][r];f.low_mask_codes.push_back(c);f.low_dense_packed_rank[rank_table_index_host(c)]=(f.low_dense_packed_rank[rank_table_index_host(c)]&~((1u<<L)-1u))|r;}}
    for(int h=0;h<=MAXW;++h){f.high_all_off[h]=f.high_all_codes.size();for(uint32_t r=0;r<ha[h].size();++r){auto c=ha[h][r];f.high_all_codes.push_back(c);f.high_packed_rank[rank_table_index_host(c)]=r<<H;}}
    f.high_all_off[MAXW+1]=f.high_all_codes.size();
    f.high_mask_off.resize(size_t(HM)*S);
    for(uint32_t m=0;m<HM;++m)for(int h=0;h<S;++h){size_t ix=size_t(m)*S+h;f.high_mask_off[ix]=f.high_mask_codes.size();if(h<=MAXW)for(uint32_t r=0;r<hm[ix].size();++r){auto c=hm[ix][r];f.high_mask_codes.push_back(c);f.high_packed_rank[rank_table_index_host(c)]=(f.high_packed_rank[rank_table_index_host(c)]&~((1u<<H)-1u))|r;}}
    ...
    std::cerr<<"factor tables low_all="<<f.low_all_codes.size()<<" low_mask="<<f.low_mask_codes.size()<<" low_rank_mib="<<double(f.low_dense_packed_rank.size()*4)/(1<<20)<<" high_all="<<f.high_all_codes.size()<<" high_mask="<<f.high_mask_codes.size()<<" high_dense_mib="<<double(f.high_packed_rank.size()*4)/(1<<20)<<"\n";
    return f;
}
...
// Nonempty blocks have stride > 0; keep the empty-block behavior explicit.
__device__ __forceinline__ oneesan::Divmod64By32 factor_divmod(Code r,const FBlock& x){
    if(!x.stride)return {0,0};
#if FACTOR_RECIPROCAL_DIV
    return oneesan::invariant_divmod(r,x.stride,x.reciprocal);
#else
    Code q=r/x.stride;
    return {q,uint32_t(r-q*x.stride)};
#endif
}
__device__ __forceinline__ MateID factor_unrank_main(Code i){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;int b=f_find_main(i);FBlock x=D_F_MAIN_BLOCKS[b];Code r=i-x.off;auto qr=factor_divmod(r,x);uint32_t hr=uint32_t(qr.quotient),lr=qr.remainder;uint32_t hc,lc;
...
__device__ __forceinline__ MateID factor_unrank_block(Code i){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;int b=f_find_block(i);FBlock x=D_F_BLOCK_BLOCKS[b];Code r=i-x.off;auto qr=factor_divmod(r,x);uint32_t hr=uint32_t(qr.quotient),lr=qr.remainder;uint32_t hc,lc;
    if(D_F_FIX_LOW){hc=D_F_HIGH_ALL_CODES[D_F_HIGH_ALL_OFF[x.he]+hr];lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];}
    else{hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];lc=D_F_LOW_ALL_CODES[D_F_LOW_ALL_OFF[x.hs]+lr];}
...
__device__ __forceinline__ Code factor_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);int cv=int(mget(m,L));int bid=3*he+cv;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lp=D_F_LOW_DENSE_PACKED_RANK[rank_table_index<LOW_LUT_K>(lc)],hp=D_F_HIGH_PACKED_RANK[rank_table_index<HIGH_LUT_K>(hc)];uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
#ifndef GRIDFP_LOW_RANK_DELTA
#define GRIDFP_LOW_RANK_DELTA 1
#endif
// For an update wholly inside the unfixed low segment, the high segment,
// central symbol and factor block are unchanged. rank = base + low_rank.
__device__ __forceinline__ Code factor_predecessor_rank(MateID original,MateID pred,Code rank,int p){
#if GRIDFP_LOW_RANK_DELTA
    if(p<LOW_LUT_K&&!D_F_FIX_LOW){
        constexpr uint32_t mask=(1u<<(2*LOW_LUT_K))-1u;
        uint32_t oldLow=D_F_LOW_DENSE_PACKED_RANK[rank_table_index<LOW_LUT_K>(uint32_t(original)&mask)]>>LOW_LUT_K;
        uint32_t newLow=D_F_LOW_DENSE_PACKED_RANK[rank_table_index<LOW_LUT_K>(uint32_t(pred)&mask)]>>LOW_LUT_K;
        return rank-oldLow+newLow;
    }
#endif
    return factor_rank_main(pred);
}
__device__ __forceinline__ Code factor_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int h=seg_end_height(hc,H);FBlock x=D_F_BLOCK_BLOCKS[h];uint32_t lp=D_F_LOW_DENSE_PACKED_RANK[rank_table_index<LOW_LUT_K>(lc)],hp=D_F_HIGH_PACKED_RANK[rank_table_index<HIGH_LUT_K>(hc)];uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_global_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[rank_table_index<HIGH_LUT_K>(hc)],lp=D_F_LOW_DENSE_PACKED_RANK[rank_table_index<LOW_LUT_K>(lc)];uint32_t har=hp>>H,lar=lp>>L;Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[he]+har];MateValue c=mget(m,L);if(c>N)rank+=D_FULL_DP[L][he];if(c>R&&he>0)rank+=D_FULL_DP[L][he-1];return rank+lar;
}
__device__ __forceinline__ Code factor_global_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[rank_table_index<HIGH_LUT_K>(hc)],lp=D_F_LOW_DENSE_PACKED_RANK[rank_table_index<LOW_LUT_K>(lc)];uint32_t har=hp>>H,lar=lp>>L;return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[he]+har]+lar;
}
// Compose factor unranking with global ranking without materializing the free
// segment or ranking it again. The free-segment rank is already hr/lr.
#ifndef GRIDFP_DIRECT_GLOBAL_INDEX
#define GRIDFP_DIRECT_GLOBAL_INDEX 1
#endif
template<bool Block> __device__ __forceinline__ Code factor_global_index(Code i,MateID* mate=nullptr,uint32_t mask=0xffffffffu){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;
    FBlock x=Block?D_F_BLOCK_BLOCKS[f_find_block(i)]:D_F_MAIN_BLOCKS[f_find_main(i)];
    auto qr=factor_divmod(i-x.off,x);uint32_t hr=uint32_t(qr.quotient),lr=qr.remainder;
    uint32_t hc=0,lc=0;if(mask==0xffffffffu)mask=D_F_MASK;
    if(D_F_FIX_LOW){
        lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(mask)*S+x.hs]+lr];
        lr=D_F_LOW_DENSE_PACKED_RANK[rank_table_index<L>(lc)]>>L;
        if(mate)hc=D_F_HIGH_ALL_CODES[D_F_HIGH_ALL_OFF[x.he]+hr];
    }else{
        hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(mask)*S+x.he]+hr];
        hr=D_F_HIGH_PACKED_RANK[rank_table_index<H>(hc)]>>H;
        if(mate)lc=D_F_LOW_ALL_CODES[D_F_LOW_ALL_OFF[x.hs]+lr];
    }
    Code rank=(Block?D_F_HIGH_BLOCK_BASE:D_F_HIGH_MAIN_BASE)[D_F_HIGH_ALL_OFF[x.he]+hr]+lr;
    if constexpr(!Block){if(x.c>N)rank+=D_FULL_DP[L][x.he];if(x.c>R&&x.he>0)rank+=D_FULL_DP[L][x.he-1];}
    if(mate)*mate=MateID(lc)|(MateID(hc)<<(2*(L+(Block?0:1))))|(Block?0:MateID(x.c)<<(2*L));
    return rank;
}
...
template<bool Block> __device__ __forceinline__ Code factor_io_index(Code i,MateID* mate){
#if GRIDFP_DIRECT_GLOBAL_INDEX
    return factor_global_index<Block>(i,mate);
#else
    MateID m=Block?factor_unrank_block(i):factor_unrank_main(i);if(mate)*mate=m;
    return Block?factor_global_rank_block(m):factor_global_rank_main(m);
#endif
}
__global__ void gather_main_kernel(Count*out,MateID*mates,Code n,bool cache_rank=false){
    for(Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x;i<n;i+=Code(gridDim.x)*blockDim.x){
        MateID m=0;Code g=factor_io_index<false>(i,(mates&&!cache_rank)?&m:nullptr);
        out[i]=global_load_main(g);if(mates)mates[i]=cache_rank?g:m;
    }
}
__global__ void gather_block_kernel(Count*out,Code n,MateID*mates,bool cache_rank=false){
    for(Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x;i<n;i+=Code(gridDim.x)*blockDim.x){
        MateID m=0;Code g=factor_io_index<true>(i,(mates&&!cache_rank)?&m:nullptr);
        out[i]=global_load_block(g);if(mates)mates[i]=cache_rank?g:m;
    }
}
#ifndef GRIDFP_REUSE_SCATTER_MATE
#define GRIDFP_REUSE_SCATTER_MATE 0
#endif
__global__ void scatter_main_kernel(const Count*in,Code n,const MateID*mates,bool cache_rank=false){
    for(Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x;i<n;i+=Code(gridDim.x)*blockDim.x){
        Code g=cache_rank&&mates?mates[i]:((GRIDFP_REUSE_SCATTER_MATE&&mates)?factor_global_rank_main(mates[i]):factor_io_index<false>(i,nullptr));
        global_store_main(g,in[i]);
    }
}
__global__ void scatter_block_kernel(const Count*in,Code n,const MateID*mates,bool cache_rank=false){
    for(Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x;i<n;i+=Code(gridDim.x)*blockDim.x){
        Code g=mates?(cache_rank?mates[i]:factor_global_rank_block(mates[i])):factor_io_index<true>(i,nullptr);
        global_store_block(g,in[i]);
    }
}
...
__global__ void reverse_main_group_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<n;i+=stride){
        ...
        if(have){Count c=in[factor_predecessor_rank(t,pred,i,p)];if(c)acc=add_mod_plain(acc,c);}
...
__global__ void reverse_block_group_kernel(const Count*in,Code n,Count*out,Code dn,int p,const MateID*mates){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<dn;i+=stride){
        MateID b=mates?mates[i]:factor_unrank_block(i); Count acc=0;
        oneesan::gridfp::reverse_block_predecessors<GRIDFP_SPARSE_REVERSE>(b,TARGET_W,p,
            [&](oneesan::gridfp::MateID m){reverse_add_main(acc,in,m);});
        out[i]=acc;
    }
}
...
__device__ __forceinline__ Count reverse_eval_main_step_ranked(const Count*in,const Count*din,MateID t,int p,Code rank){
    Count acc=in[rank];
    MateID pred=0; bool have=false;
    switch(oneesan::gridfp::mpair(t,p)){
    case oneesan::gridfp::LR: pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::NN);have=true;break;
    ...
    if(have){Count c=in[factor_predecessor_rank(t,pred,rank,p)];if(c)acc=add_mod_plain(acc,c);}
    if(oneesan::gridfp::mget(t,p)==oneesan::gridfp::N){
        MateID b=oneesan::gridfp::mshrink(t,p);Count c=din[factor_rank_block(b)];if(c)acc=add_mod_plain(acc,c);
    }
    return acc;
...
__device__ __forceinline__ Count reverse_eval_main_step(const Count*in,const Count*din,MateID t,int p){
    return reverse_eval_main_step_ranked(in,din,t,p,factor_rank_main(t));
}
...
template<bool ReuseMainRank = GRIDFP_REUSE_MAIN_RANK>
__global__ void reverse2_main_group_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<n;i+=stride){
        ...
        Count acc;
        if constexpr(ReuseMainRank) acc=reverse_eval_main_step_ranked(in,din,t,p,i);
        else acc=reverse_eval_main_step(in,din,t,p);
        MateID pred=0; bool have=false;
        ...
        if(have){Count c=reverse_eval_main_step_ranked(in,din,pred,p,factor_predecessor_rank(t,pred,i,q));if(c)acc=add_mod_plain(acc,c);}
...
__global__ void reverse2_block_group_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<dn;i+=stride){
        MateID b=mates?mates[i]:factor_unrank_block(i); Count acc=0;
        oneesan::gridfp::reverse_block_predecessors<GRIDFP_SPARSE_REVERSE>(b,TARGET_W,q,
            [&](oneesan::gridfp::MateID m){reverse2_add_main(acc,in,din,m,p);});
        out[i]=acc;
    }
}


#include "factor_transfer_map.cuh"
...
struct DeviceCtx{
    // Kernel arguments depend on geometry and arena layout, not the occupancy mask
    // or modulus. Configuration symbols are uploaded before each graph launch.
    // I/O policy and stream handles are fixed for this device context lifetime.
    using GraphKey=std::tuple<Code,Code,int,int,int,bool,bool,bool,bool,bool,int>;
    std::map<GraphKey,cudaGraphExec_t> transition_graphs;
    bool use_graphs=true,graph_io=true,parallel_io=true;
    double group_graph_s=0;
    uint64_t graph_builds=0,graph_replays=0,graph_evictions=0;
    void clear_graphs(){if(!transition_graphs.empty())++graph_evictions;for(auto const& entry:transition_graphs)ck(cudaGraphExecDestroy(entry.second),"destroy transition graph");transition_graphs.clear();}
    using TransferKey=std::tuple<int,int,int,int,Code,Code>;
    TransferKey transfer_key{};bool transfer_key_valid=false,verify_transfer=false;
    size_t transfer_budget=512ull<<20,transfer_bytes=0,transfer_peak=0;
    uint64_t transfer_builds=0,transfer_hits=0,transfer_skips=0;
    double transfer_build_s=0;
    std::vector<factor_transfer::Step> transfer_steps;
    void clear_transfer(){
        if(!transfer_steps.empty())clear_graphs();
        for(auto& step:transfer_steps){if(step.offsets)cudaFree(step.offsets);if(step.columns)cudaFree(step.columns);}
        transfer_steps.clear();transfer_bytes=0;
    }
    int frontier_threads=256,frontier_blocks=0;
    Code single_stream_limit=0;
    bool cache_block_mates=true;
    bool pipeline_groups=true,cache_global_rank=true;
    bool profile_batch=false;
    uint64_t transfer_batches=0,transfer_batched_groups=0;
    // A window is a global dependency barrier across devices. Groups on one
    // device are serialized by sMain; every captured branch rejoins that stream.
    void drain_groups(){auto start=std::chrono::steady_clock::now();ck(cudaStreamSynchronize(sMain),"window main drain");ck(cudaStreamSynchronize(sBlock),"window block drain");double elapsed=std::chrono::duration<double>(std::chrono::steady_clock::now()-start).count();active+=elapsed;if(use_graphs&&graph_io)group_graph_s+=elapsed;}
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;MateID*dMate=nullptr,*dBlockMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0,gather_s=0,transition_s=0,scatter_s=0;uint64_t groups=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
    void init(int d,Count mod,Count**mp,Count**bp,Code mc,Code bc,int ng){dev=d;if(const char*e=std::getenv("GRIDFP_CACHE_GLOBAL_RANK"))cache_global_rank=std::atoi(e)!=0;if(const char*e=std::getenv("GRIDFP_PIPELINE_GROUPS"))pipeline_groups=std::atoi(e)!=0;if(const char*e=std::getenv("GRIDFP_TRANSFER_MAP_MIB"))transfer_budget=size_t(std::max(0,std::atoi(e)))<<20;if(const char*e=std::getenv("GRIDFP_VERIFY_TRANSFER_MAPS"))verify_transfer=std::atoi(e)!=0;if(const char*e=std::getenv("GRIDFP_FRONTIER_BLOCKS")){frontier_blocks=std::atoi(e);if(frontier_blocks<0||frontier_blocks>65535)throw std::runtime_error("GRIDFP_FRONTIER_BLOCKS must be in [0,65535]");}if(const char*e=std::getenv("GRIDFP_FRONTIER_THREADS")){frontier_threads=std::atoi(e);if(frontier_threads<32||frontier_threads>1024||(frontier_threads&(frontier_threads-1)))throw std::runtime_error("GRIDFP_FRONTIER_THREADS must be a power of two in [32,1024]");}if(const char*e=std::getenv("GRIDFP_PARALLEL_GROUP_IO"))parallel_io=std::atoi(e)!=0;if(const char*e=std::getenv("GRIDFP_TRANSITION_GRAPHS")){use_graphs=std::atoi(e)!=0;graph_io=std::atoi(e)>=2;}if(const char*e=std::getenv("GRIDFP_CACHE_BLOCK_MATES"))cache_block_mates=std::atoi(e)!=0;if(const char*e=std::getenv("GRIDFP_SINGLE_STREAM_MAX_STATES"))single_stream_limit=std::strtoull(e,nullptr,10);ck(cudaSetDevice(dev),"set init");ck(cudaMemcpyToSymbol(D_FULL_DP,H_DP,sizeof(H_DP)),"full dp");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"mod");ck(cudaMemcpyToSymbol(D_MAIN_PTR,mp,sizeof(Count*)*MAXGPU),"main ptrs");ck(cudaMemcpyToSymbol(D_BLOCK_PTR,bp,sizeof(Count*)*MAXGPU),"block ptrs");ck(cudaMemcpyToSymbol(D_MAIN_CHUNK,&mc,sizeof(mc)),"main chunk");ck(cudaMemcpyToSymbol(D_BLOCK_CHUNK,&bc,sizeof(bc)),"block chunk");ck(cudaMemcpyToSymbol(D_NGPU,&ng,sizeof(ng)),"ngpu");ck(cudaStreamCreateWithFlags(&sMain,cudaStreamNonBlocking),"stream main");ck(cudaStreamCreateWithFlags(&sBlock,cudaStreamNonBlocking),"stream block");ck(cudaEventCreateWithFlags(&copyDone,cudaEventDisableTiming),"event copy");ck(cudaEventCreateWithFlags(&clearDone,cudaEventDisableTiming),"event clear");ck(cudaEventCreateWithFlags(&mainDone,cudaEventDisableTiming),"event main");ck(cudaEventCreateWithFlags(&blockDone,cudaEventDisableTiming),"event block");}
    void ensure(Code m,Code b,bool useMate,size_t im,size_t id,bool useBlockMate=false){ck(cudaSetDevice(dev),"set ensure");auto al=[](size_t x){return(x+255)&~size_t(255);};size_t ab=al(size_t(m)*sizeof(Count)),db=al(size_t(b)*sizeof(Count)),mb=useMate?al(size_t(m)*sizeof(MateID)):0,need=2*ab+2*db+mb+(useBlockMate?al(size_t(b)*sizeof(MateID)):0);if(need>capArena){clear_graphs();if(arena)cudaFree(arena);capArena=need;ck(cudaMalloc(&arena,capArena),"scratch arena");}size_t off=0;dA=(Count*)(arena+off);off+=ab;dB=(Count*)(arena+off);off+=ab;dD=(Count*)(arena+off);off+=db;dE=(Count*)(arena+off);off+=db;dMate=useMate?(MateID*)(arena+off):nullptr;off+=mb;dBlockMate=useBlockMate?(MateID*)(arena+off):nullptr;if(im>capIM){if(dIM)cudaFree(dIM);capIM=im;ck(cudaMalloc(&dIM,capIM*sizeof(PeerInterval)),"interval main");}if(id>capID){if(dID)cudaFree(dID);capID=id;ck(cudaMalloc(&dID,capID*sizeof(PeerInterval)),"interval block");}maxIntervals=std::max({maxIntervals,im,id});}
    void destroy(){if(dev<0)return;cudaSetDevice(dev);clear_transfer();clear_graphs();if(arena)cudaFree(arena);if(dIM)cudaFree(dIM);if(dID)cudaFree(dID);if(copyDone)cudaEventDestroy(copyDone);if(clearDone)cudaEventDestroy(clearDone);if(mainDone)cudaEventDestroy(mainDone);if(blockDone)cudaEventDestroy(blockDone);if(sMain)cudaStreamDestroy(sMain);if(sBlock)cudaStreamDestroy(sBlock);}
};

struct PreparedGroup{
    FactorRuntimeCfg factor;
    int g=0;
...
static PreparedGroup prepare_group(int W,const WindowPlan&wp,int g,Code mc,Code bc,int ng){
    ...
    uint32_t mask=fixLow?(pg.mo&((1u<<LOW_LUT_K)-1u)):((pg.mo>>(LOW_LUT_K+1))&((1u<<HIGH_LUT_K)-1u));
    auto main=make_factor_main_blocks(fixLow,mask),block=make_factor_block_blocks(fixLow,mask);
    if(main.size()>64||block.size()>32||main.back().end!=pg.ms.size||block.back().end!=pg.ds.size)
        throw std::runtime_error("factor group size mismatch");
    std::copy(main.begin(),main.end(),pg.factor.main_blocks);
    std::copy(block.begin(),block.end(),pg.factor.block_blocks);
    pg.factor.main_n=main.size();pg.factor.block_n=block.size();
    pg.factor.mask=mask;pg.factor.fix_low=fixLow;
...
static void ensure_transfer(DeviceCtx& c,const PreparedGroup& pg,const WindowPlan& wp,size_t scratch_target){
    if(!c.transfer_budget)return;
    int occupied=__builtin_popcount(pg.factor.mask);
    DeviceCtx::TransferKey key{pg.factor.fix_low,occupied,wp.p_hi,wp.p_lo,pg.ms.size,pg.ds.size};
    if(c.transfer_key_valid&&key==c.transfer_key){if(!c.transfer_steps.empty())++c.transfer_hits;return;}
    c.clear_transfer();c.transfer_key=key;c.transfer_key_valid=true;
    int fixed_width=pg.factor.fix_low?LOW_LUT_K:HIGH_LUT_K;
    Code total=pg.ms.size+pg.ds.size;
    int stages=0;for(int p=wp.p_hi;p>=wp.p_lo;++stages){if(p>1&&p-1>=wp.p_lo)p-=2;else --p;}
    size_t free=0,capacity=0;ck(cudaMemGetInfo(&free,&capacity),"transfer memory");
    size_t reserve=(256ull<<20)+(scratch_target>c.capArena?scratch_target-c.capArena:0);
    size_t budget=std::min(c.transfer_budget,free>reserve?free-reserve:0);
    // Extreme occupancy masks occur only once, so compilation cannot amortize.
    // 6*W bounds two-step indegree, including the special p=1 boundary.
    // This admission test makes the uint32 prefix scan and columns exact.
    if(!stages||occupied==0||occupied==fixed_width||total>=0x7fffffffu||total>0xffffffffu/(6u*TARGET_W)||Code(stages)*(total+1)*4>budget){++c.transfer_skips;return;}
    auto start=std::chrono::steady_clock::now();
    int blocks=int(std::min<Code>(65535,(total+256)/256));
    bool fit=true;
    for(int p=wp.p_hi;p>=wp.p_lo;){
        factor_transfer::Step step;step.p=p;step.paired=p>1&&p-1>=wp.p_lo;
        size_t offset_bytes=size_t(total+1)*sizeof(uint32_t);
        if(c.transfer_bytes+offset_bytes>budget){fit=false;break;}
        ck(cudaMalloc(&step.offsets,offset_bytes),"transfer offsets");
        factor_transfer::counts_kernel<<<blocks,256,0,c.sMain>>>(pg.ms.size,total,p,step.paired,step.offsets);
        void* temporary=nullptr;size_t temporary_bytes=0;
        ck(cub::DeviceScan::ExclusiveSum(temporary,temporary_bytes,step.offsets,step.offsets,int(total+1),c.sMain),"transfer scan size");
        ck(cudaMalloc(&temporary,temporary_bytes),"transfer scan memory");
        ck(cub::DeviceScan::ExclusiveSum(temporary,temporary_bytes,step.offsets,step.offsets,int(total+1),c.sMain),"transfer scan");
        uint32_t edge_count=0;
        ck(cudaMemcpyAsync(&edge_count,step.offsets+total,sizeof(edge_count),cudaMemcpyDeviceToHost,c.sMain),"transfer edge count");
        ck(cudaStreamSynchronize(c.sMain),"transfer count sync");cudaFree(temporary);step.edges=edge_count;
        size_t bytes=offset_bytes+size_t(step.edges)*sizeof(uint32_t);
        if(c.transfer_bytes+bytes>budget){cudaFree(step.offsets);fit=false;break;}
        if(step.edges)ck(cudaMalloc(&step.columns,size_t(step.edges)*sizeof(uint32_t)),"transfer columns");
        factor_transfer::fill_kernel<<<blocks,256,0,c.sMain>>>(pg.ms.size,total,p,step.paired,step.offsets,step.columns);
        ck(cudaStreamSynchronize(c.sMain),"transfer fill sync");
        c.transfer_steps.push_back(step);c.transfer_bytes+=bytes;
        p-=step.paired?2:1;
    }
    if(!fit){c.clear_transfer();++c.transfer_skips;}
    else {++c.transfer_builds;c.transfer_peak=std::max(c.transfer_peak,c.transfer_bytes);}
    c.transfer_build_s+=std::chrono::duration<double>(std::chrono::steady_clock::now()-start).count();
}
static void verify_transfer(DeviceCtx& c,Code n,Code dn){
    if(!c.verify_transfer||c.transfer_steps.empty())return;
    Code *errors=nullptr,result=0;ck(cudaMalloc(&errors,sizeof(Code)),"transfer verify allocate");
    ck(cudaMemsetAsync(errors,0,sizeof(Code),c.sMain),"transfer verify clear");
    for(auto const& step:c.transfer_steps)factor_transfer::verify_kernel<<<256,256,0,c.sMain>>>(n,n+dn,step.p,step.paired,step.offsets,step.columns,errors);
    ck(cudaMemcpyAsync(&result,errors,sizeof(result),cudaMemcpyDeviceToHost,c.sMain),"transfer verify result");
    ck(cudaStreamSynchronize(c.sMain),"transfer verify sync");cudaFree(errors);
    if(result)throw std::runtime_error("shared transfer map differs from exact predecessor enumeration");
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    // Keep configuration upload ordered with nonblocking gather/graph execution.
    upload_factor_config(pg.factor,c.sMain);
    ...
    // The group index-to-state mapping is invariant across all cell updates.
    // Cache blocked targets only when the existing scratch budget permits it.
    bool useBlockMate=c.cache_block_mates&&!pg.use_di&&countBytes+(useMate?mateBytes:0)+size_t(ds.size)*sizeof(MateID)+6*255<=target;
    c.ensure(ms.size,ds.size,useMate,pg.mi.size(),pg.di.size(),useBlockMate);
    ensure_transfer(c,pg,wp,target);verify_transfer(c,ms.size,ds.size);
    bool cacheRank=c.cache_global_rank&&!c.transfer_steps.empty();
...
#if GRIDFP_LEGACY_GROUP_UPLOAD
    // Retained for A/B testing only. Factorized kernels never read these
    // legacy group DP tables/masks (rank_drop_n_t is not instantiated).
    ...
    if(c.frontier_blocks){bm=std::min(bm,c.frontier_blocks);bd=std::min(bd,c.frontier_blocks);}
    bool single=ms.size+ds.size<=c.single_stream_limit;
    cudaStream_t blockStream=single?c.sMain:c.sBlock;
    bool fullGraph=c.use_graphs&&c.graph_io&&!pg.use_mi&&!pg.use_di;
    bool parallelIO=fullGraph&&c.parallel_io&&!single;
    DeviceCtx::GraphKey fullKey{ms.size,ds.size,wp.p_hi,wp.p_lo,threads,useMate,useBlockMate,single,true,!c.transfer_steps.empty(),0};
    auto finish_full_graph=[&](){
        // Configuration, graph execution and the next configuration share sMain.
        if(!c.pipeline_groups)ck(cudaStreamSynchronize(c.sMain),"group graph sync");
        double elapsed=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
        ++c.groups;c.group_graph_s+=elapsed;c.active+=elapsed;
    };
    if(fullGraph){
        auto it=c.transition_graphs.find(fullKey);
        if(it!=c.transition_graphs.end()){
            ++c.graph_replays;
            ck(cudaGraphLaunch(it->second,c.sMain),"group graph launch");
            finish_full_graph();return;
        }
        ck(cudaStreamBeginCapture(c.sMain,cudaStreamCaptureModeThreadLocal),"group capture begin");
        if(parallelIO){
            ck(cudaEventRecord(c.mainDone,c.sMain),"I/O capture fork");
            ck(cudaStreamWaitEvent(blockStream,c.mainDone,0),"I/O capture fork wait");
        }
    }

    if(ms.size){if(pg.use_mi)interval_io_kernel<false,false><<<interval_blocks(pg.mi.size(),threads),threads>>>(c.dA,c.dIM,pg.mi.size());else gather_main_kernel<<<bm,threads,0,c.sMain>>>(c.dA,useMate?c.dMate:nullptr,ms.size,cacheRank);}
    if(ds.size){if(pg.use_di)interval_io_kernel<true,false><<<interval_blocks(pg.di.size(),threads),threads>>>(c.dD,c.dID,pg.di.size());else gather_block_kernel<<<bd,threads,0,(parallelIO?blockStream:c.sMain)>>>(c.dD,ds.size,c.dBlockMate,cacheRank);}
    if(parallelIO){
        ck(cudaEventRecord(c.mainDone,c.sMain),"gather main done");
        ck(cudaEventRecord(c.blockDone,blockStream),"gather block done");
        ck(cudaStreamWaitEvent(c.sMain,c.blockDone,0),"gather join main");
        ck(cudaStreamWaitEvent(blockStream,c.mainDone,0),"gather join block");
    }
    ck(cudaGetLastError(),"doubleD gather");if(!fullGraph)ck(cudaDeviceSynchronize(),"doubleD gather sync");auto tGather=std::chrono::steady_clock::now();

    Count*cur=c.dA,*nxt=c.dB,*dcur=c.dD,*dnext=c.dE;
    auto enqueue_transition=[&](){
        size_t transfer_stage=0;
        for(int p=wp.p_hi;p>=wp.p_lo;){
            if(!c.transfer_steps.empty()){
                auto const& step=c.transfer_steps[transfer_stage++];
                factor_transfer::apply_kernel<<<std::max(bm,bd),threads,0,c.sMain>>>(cur,dcur,nxt,dnext,ms.size,ms.size+ds.size,step.offsets,step.columns);
                p-=step.paired?2:1;
            }else if(p>2 && p-1>=wp.p_lo){
                if(ms.size)reverse2_main_group_kernel<><<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,nxt,p);
                if(ds.size)reverse2_block_group_kernel<<<bd,threads,0,blockStream>>>(cur,dcur,c.dBlockMate,dnext,ds.size,p);
                p-=2;
            }else{
                if(p>1){
                    if(ms.size)reverse_main_group_kernel<<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,ds.size,nxt,p);
                    if(ds.size)reverse_block_group_kernel<<<bd,threads,0,blockStream>>>(cur,ms.size,dnext,ds.size,p,c.dBlockMate);
                }else{
                    if(ms.size)ck(cudaMemcpyAsync(nxt,cur,size_t(ms.size)*sizeof(Count),cudaMemcpyDeviceToDevice,c.sMain),"p1 identity");
                    if(ds.size)ck(cudaMemsetAsync(dnext,0,size_t(ds.size)*sizeof(Count),blockStream),"p1 clear next D");
                    if(!single){
                        ck(cudaEventRecord(c.copyDone,c.sMain),"record p1 copy");ck(cudaEventRecord(c.clearDone,blockStream),"record p1 clear");
                        ck(cudaStreamWaitEvent(c.sMain,c.clearDone,0),"p1 main wait clear");ck(cudaStreamWaitEvent(blockStream,c.copyDone,0),"p1 block wait copy");
                    }
                    if(ms.size)main_group_kernel<<<bm,threads,0,c.sMain>>>(cur,(useMate&&!cacheRank)?c.dMate:nullptr,ms.size,nxt,dnext,p);
                    if(ds.size)blocked_group_kernel<<<bd,threads,0,blockStream>>>(dcur,ds.size,nxt,p);
                }
                --p;
            }
            ck(cudaGetLastError(),"reverse2 bounded transition");
            if(!single){
                ck(cudaEventRecord(c.mainDone,c.sMain),"record main");ck(cudaEventRecord(c.blockDone,blockStream),"record block");
                ck(cudaStreamWaitEvent(c.sMain,c.blockDone,0),"main wait block");ck(cudaStreamWaitEvent(blockStream,c.mainDone,0),"block wait main");
            }
            std::swap(cur,nxt);std::swap(dcur,dnext);
        }
    };
    if(c.use_graphs&&!fullGraph){
        DeviceCtx::GraphKey key{ms.size,ds.size,wp.p_hi,wp.p_lo,threads,useMate,useBlockMate,single,false,!c.transfer_steps.empty(),0};
        auto it=c.transition_graphs.find(key);
        if(it==c.transition_graphs.end()){
            ck(cudaStreamBeginCapture(c.sMain,cudaStreamCaptureModeThreadLocal),"transition capture begin");
            if(!single){
                ck(cudaEventRecord(c.mainDone,c.sMain),"capture fork");
                ck(cudaStreamWaitEvent(blockStream,c.mainDone,0),"capture fork wait");
            }
            enqueue_transition();
            cudaGraph_t graph=nullptr;
            ck(cudaStreamEndCapture(c.sMain,&graph),"transition capture end");
            cudaGraphExec_t exec=nullptr;
            ck(cudaGraphInstantiate(&exec,graph,0),"transition instantiate");
            ck(cudaGraphDestroy(graph),"transition definition destroy");
            it=c.transition_graphs.emplace(key,exec).first;++c.graph_builds;
        }else{
            ++c.graph_replays;
            // Replay does not execute the host-side ping-pong swaps.
            int steps=0;
            for(int p=wp.p_hi;p>=wp.p_lo;++steps){if(p>2&&p-1>=wp.p_lo)p-=2;else --p;}
            if(!c.transfer_steps.empty())steps=int(c.transfer_steps.size());
            if(steps&1){std::swap(cur,nxt);std::swap(dcur,dnext);}
        }
        ck(cudaGraphLaunch(it->second,c.sMain),"transition graph launch");
    }else {
        if(fullGraph&&!single&&!parallelIO){
            ck(cudaEventRecord(c.mainDone,c.sMain),"group capture fork");
            ck(cudaStreamWaitEvent(blockStream,c.mainDone,0),"group capture fork wait");
        }
        enqueue_transition();
    }
    if(!fullGraph){ck(cudaStreamSynchronize(c.sMain),"main sync");if(!single)ck(cudaStreamSynchronize(blockStream),"block sync");}auto tTransition=std::chrono::steady_clock::now();
    if(ms.size){if(pg.use_mi)interval_io_kernel<false,true><<<interval_blocks(pg.mi.size(),threads),threads>>>(cur,c.dIM,pg.mi.size());else scatter_main_kernel<<<bm,threads,0,c.sMain>>>(cur,ms.size,useMate?c.dMate:nullptr,cacheRank);}
    if(ds.size){if(pg.use_di)interval_io_kernel<true,true><<<interval_blocks(pg.di.size(),threads),threads>>>(dcur,c.dID,pg.di.size());else scatter_block_kernel<<<bd,threads,0,(parallelIO?blockStream:c.sMain)>>>(dcur,ds.size,c.dBlockMate,cacheRank);}
    if(fullGraph){
        if(parallelIO){
            ck(cudaEventRecord(c.blockDone,blockStream),"scatter block done");
            ck(cudaStreamWaitEvent(c.sMain,c.blockDone,0),"scatter join");
        }
        cudaGraph_t graph=nullptr;
        ck(cudaStreamEndCapture(c.sMain,&graph),"group capture end");
        cudaGraphExec_t exec=nullptr;
        ck(cudaGraphInstantiate(&exec,graph,0),"group instantiate");
        ck(cudaGraphDestroy(graph),"group definition destroy");
        c.transition_graphs.emplace(fullKey,exec);++c.graph_builds;
        ck(cudaGraphLaunch(exec,c.sMain),"group graph first launch");
        finish_full_graph();return;
    }
    ck(cudaGetLastError(),"doubleD scatter");ck(cudaDeviceSynchronize(),"group sync");auto tScatter=std::chrono::steady_clock::now();c.groups++;c.gather_s+=std::chrono::duration<double>(tGather-t0).count();c.transition_s+=std::chrono::duration<double>(tTransition-tGather).count();c.scatter_s+=std::chrono::duration<double>(tScatter-tTransition).count();c.active+=std::chrono::duration<double>(tScatter-t0).count();
}


#include "factor_transfer_batch.cuh"
...
static void init_direct_row6_lut_full_multi_gpu(int W,Count mod,int threads,Count**fullMain,const std::vector<Code>&mainLen,Code mainChunk,int ng){
    ...
    for(int dev=0;dev<ng;++dev)workers.emplace_back([&,dev]{ck(cudaSetDevice(dev),"r6 levelpack set");ck(cudaMemcpyToSymbol(D_BOUND_DP,dp,sizeof(dp)),"r6 levelpack dp");int cap=6;ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"r6 levelpack cap");ck(cudaMemcpyToSymbol(D_R6_GPU_MAT_OFF,moff,sizeof(moff)),"r6 levelpack mat off");Count*dmat=nullptr,*dbeta=nullptr;uint32_t*dpidx=nullptr,*dsidx=nullptr;ck(cudaMalloc(&dmat,mat.size()*4),"r6 levelpack mat");ck(cudaMalloc(&dbeta,beta.size()*4),"r6 levelpack beta");ck(cudaMalloc(&dpidx,pi.first.size()*4),"r6 levelpack pidx");ck(cudaMalloc(&dsidx,si.first.size()*4),"r6 levelpack sidx");ck(cudaMemcpy(dmat,mat.data(),mat.size()*4,cudaMemcpyHostToDevice),"r6 levelpack mat cp");ck(cudaMemcpy(dbeta,beta.data(),beta.size()*4,cudaMemcpyHostToDevice),"r6 levelpack beta cp");ck(cudaMemcpy(dpidx,pi.first.data(),pi.first.size()*4,cudaMemcpyHostToDevice),"r6 levelpack pidx cp");ck(cudaMemcpy(dsidx,si.first.data(),si.first.size()*4,cudaMemcpyHostToDevice),"r6 levelpack sidx cp");auto tb=std::chrono::steady_clock::now();
        auto build_pref=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*4),"r6 pref base");ck(cudaMemset(cur,0,size_t(182)*4),"r6 pref base zero");Count one=1;ck(cudaMemcpy(cur+initLocal,&one,4,cudaMemcpyHostToDevice),"r6 pref one");ck(cudaMalloc(&hp,1),"r6 pref h");int8_t h1=1;ck(cudaMemcpy(hp,&h1,1,cudaMemcpyHostToDevice),"r6 pref h1");for(int lev=0;lev<hi;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*4),"r6 pref next");ck(cudaMalloc(&hn,size_t(children)),"r6 pref hn");bool wide=true;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_WIDE"))wide=std::atoi(e)!=0;int wt=128;if(const char*e=std::getenv("GRIDFP_ROW6_WIDE_THREADS"))wt=std::max(32,std::min(256,std::atoi(e)));Code wideMin=243;if(const char*e=std::getenv("GRIDFP_ROW6_WIDE_MIN"))wideMin=std::max<Code>(1,std::strtoull(e,nullptr,10));if(wide&&children>=wideMin){int gb=int(std::min<Code>(children,2147483647));r6_prefix_level_wide_kernel<<<std::max(1,gb),wt>>>(parents,cur,hp,nxt,hn,dmat);}else{int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_prefix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);}ck(cudaGetLastError(),"r6 pref level launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}Count*packed=nullptr;ck(cudaMalloc(&packed,size_t(pi.second)*4),"r6 pref packed");int bl=int(std::min<Code>(65535,(parents+255)/256));r6_pack_level_kernel<<<std::max(1,bl),256>>>(parents,cur,hp,dpidx,packed);ck(cudaGetLastError(),"r6 pref pack launch");cudaFree(cur);cudaFree(hp);return packed;};
        auto build_suff=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*4),"r6 suff base");ck(cudaMemset(cur,0,size_t(182)*4),"r6 suff base zero");ck(cudaMemcpy(cur,dbeta,size_t(r6_block_size(0))*4,cudaMemcpyDeviceToDevice),"r6 suff beta");ck(cudaMalloc(&hp,1),"r6 suff h");int8_t h0=0;ck(cudaMemcpy(hp,&h0,1,cudaMemcpyHostToDevice),"r6 suff h0");for(int lev=0;lev<lo;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*4),"r6 suff next");ck(cudaMalloc(&hn,size_t(children)),"r6 suff hn");bool wide=true;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_WIDE"))wide=std::atoi(e)!=0;int wt=128;if(const char*e=std::getenv("GRIDFP_ROW6_WIDE_THREADS"))wt=std::max(32,std::min(256,std::atoi(e)));Code wideMin=243;if(const char*e=std::getenv("GRIDFP_ROW6_WIDE_MIN"))wideMin=std::max<Code>(1,std::strtoull(e,nullptr,10));if(wide&&children>=wideMin){int gb=int(std::min<Code>(children,2147483647));r6_suffix_level_wide_kernel<<<std::max(1,gb),wt>>>(parents,cur,hp,nxt,hn,dmat);}else{int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_suffix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);}ck(cudaGetLastError(),"r6 suff level launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}Count*packed=nullptr;ck(cudaMalloc(&packed,size_t(si.second)*4),"r6 suff packed");int bl=int(std::min<Code>(65535,(parents+255)/256));r6_pack_level_kernel<<<std::max(1,bl),256>>>(parents,cur,hp,dsidx,packed);ck(cudaGetLastError(),"r6 suff pack launch");cudaFree(cur);cudaFree(hp);return packed;};
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+vb4(G_FACTOR.low_dense_packed_rank)+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+vb4(G_FACTOR.high_packed_rank)+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+(GRIDFP_TERNARY_RANK_LAYOUT?(2ull<<(2*GRIDFP_TRIT_CHUNK)):0);
        Code pbch=(pblock+png-1)/png;uint64_t authMaxBytes=uint64_t(pch+pbch)*4;
        constexpr int r6S=182;constexpr int r6lo=TARGET_W/2,r6hi=TARGET_W-r6lo;auto p3=[](int k)->uint64_t{uint64_t z=1;while(k--)z*=3;return z;};
        ...
        uint64_t r6Base=matElems*4+uint64_t(r6_block_size(0))*4+(p3(r6hi)+p3(r6lo))*4;
        auto levelPeak=[&](int len)->uint64_t{if(len<=0)return uint64_t(r6S)*4+1;uint64_t a=p3(len-1),b=p3(len);return (a+b)*(uint64_t(r6S)*4+1);};
        auto packPeak=[&](int len,uint64_t packedVals)->uint64_t{uint64_t a=p3(len);return a*(uint64_t(r6S)*4+1)+packedVals*4;};
        uint64_t prefPacked=pvals*4,suffPacked=svals*4;
        uint64_t row6Extra=std::max({r6Base+levelPeak(r6hi),r6Base+packPeak(r6hi,pvals),r6Base+prefPacked+levelPeak(r6lo),r6Base+prefPacked+packPeak(r6lo,svals)});
        uint64_t optionalTransferBytes=512ull<<20;
        if(const char*e=std::getenv("GRIDFP_TRANSFER_MAP_MIB"))optionalTransferBytes=uint64_t(std::max(0,std::atoi(e)))<<20;
        uint64_t scratchBytes=uint64_t(std::max(1,target_mib))<<20;uint64_t row6Peak=authMaxBytes+factorBytes+row6Extra,dpPeak=authMaxBytes+factorBytes+scratchBytes;
        uint64_t minScratchBytes=0;{const int rr[2][2]={{W-1,LOW_LUT_K+1},{LOW_LUT_K,1}};for(auto const&r:rr){auto fp=window_candidates(W,r[0],r[1]);int nj=1<<int(fp.size());uint64_t mx=0;for(int g=0;g<nj;++g){uint32_t mf,mo,bf,bo;window_masks(W,r[0],r[1],fp,uint32_t(g),mf,mo,bf,bo);auto ms=make_spec(W,mf,mo);auto ds=make_spec(W-1,bf,bo);mx=std::max<uint64_t>(mx,uint64_t(2*ms.size+2*ds.size)*sizeof(Count));}minScratchBytes=std::max(minScratchBytes,mx);}}
        std::cerr<<"PLAN n="<<n<<" W="<<W<<" gpus="<<png<<" main="<<pmain<<" blocked="<<pblock<<" auth_gib="<<double(pmain+pblock)*4.0/(1ull<<30)<<" auth_per_gpu_gib="<<double(authMaxBytes)/(1ull<<30)<<" factor_gpu_gib="<<double(factorBytes)/(1ull<<30)<<" row6_extra_gib="<<double(row6Extra)/(1ull<<30)<<" row6_peak_gib="<<double(row6Peak)/(1ull<<30)<<" dp_peak_gib="<<double(dpPeak)/(1ull<<30)<<" optional_transfer_cap_mib="<<(optionalTransferBytes>>20)<<" dp_peak_with_transfer_cap_gib="<<double(dpPeak+optionalTransferBytes)/(1ull<<30)<<" min_scratch_gib="<<double(minScratchBytes)/(1ull<<30)<<" min_dp_peak_gib="<<double(authMaxBytes+factorBytes+minScratchBytes)/(1ull<<30)<<" row6_states="<<r6n<<" pref_mib="<<(pvals*4>>20)<<" suff_mib="<<(svals*4>>20)<<" task_chunk="<<tch<<"\n";
    ...
    uint32_t *fLA[MAXGPU]{},*fLM[MAXGPU]{},*fLO[MAXGPU]{},*fLD[MAXGPU]{},*fHA[MAXGPU]{},*fHM[MAXGPU]{},*fHO[MAXGPU]{},*fHR[MAXGPU]{};Code *fHMB[MAXGPU]{},*fHBB[MAXGPU]{};
    for(int d=0;d<ng;++d){cudaSetDevice(d);upload_rank_index_table();auto cp=[&](uint32_t**dst,const std::vector<uint32_t>&v,const char*w){if(v.empty())return;ck(cudaMalloc(dst,v.size()*sizeof(uint32_t)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),w);};cp(&fLA[d],G_FACTOR.low_all_codes,"f low all");cp(&fLM[d],G_FACTOR.low_mask_codes,"f low mask");cp(&fLO[d],G_FACTOR.low_mask_off,"f low off");cp(&fLD[d],G_FACTOR.low_dense_packed_rank,"f low dense rank");cp(&fHA[d],G_FACTOR.high_all_codes,"f high all");cp(&fHM[d],G_FACTOR.high_mask_codes,"f high mask");cp(&fHO[d],G_FACTOR.high_mask_off,"f high off");cp(&fHR[d],G_FACTOR.high_packed_rank,"f high rank");auto cpc=[&](Code**dst,const std::vector<Code>&v,const char*w){ck(cudaMalloc(dst,v.size()*sizeof(Code)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(Code),cudaMemcpyHostToDevice),w);};cpc(&fHMB[d],G_FACTOR.high_main_base,"f high main base");cpc(&fHBB[d],G_FACTOR.high_block_base,"f high block base");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_CODES,&fLA[d],sizeof(fLA[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_MASK_CODES,&fLM[d],sizeof(fLM[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_MASK_OFF,&fLO[d],sizeof(fLO[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_DENSE_PACKED_RANK,&fLD[d],sizeof(fLD[d])),"f dense ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_ALL_CODES,&fHA[d],sizeof(fHA[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MASK_CODES,&fHM[d],sizeof(fHM[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MASK_OFF,&fHO[d],sizeof(fHO[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_PACKED_RANK,&fHR[d],sizeof(fHR[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MAIN_BASE,&fHMB[d],sizeof(fHMB[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_BLOCK_BASE,&fHBB[d],sizeof(fHBB[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_OFF,G_FACTOR.low_all_off.data(),sizeof(uint32_t)*(MAXW+2)),"f low all off");ck(cudaMemcpyToSymbol(D_F_HIGH_ALL_OFF,G_FACTOR.high_all_off.data(),sizeof(uint32_t)*(MAXW+2)),"f high all off");}
    ...
    double prepare_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-prep0).count();std::cerr<<"prepared windows="<<schedule.size()<<" frontier_threads="<<ctx[0].frontier_threads<<" pipeline_groups="<<ctx[0].pipeline_groups<<" frontier_blocks="<<ctx[0].frontier_blocks<<" max_groups="<<maxgroups<<" prepare_s="<<prepare_s<<"\n";

    int batch_limit=32;
    if(const char* e=std::getenv("GRIDFP_TRANSFER_BATCH"))batch_limit=std::atoi(e);
    if(batch_limit<1||batch_limit>factor_transfer::MAX_BATCH)throw std::runtime_error("GRIDFP_TRANSFER_BATCH must be in [1,32]");
    if(!ctx[0].transfer_budget||!ctx[0].cache_global_rank)batch_limit=1;
    if(const char* e=std::getenv("GRIDFP_PROFILE_BATCH"))for(auto& c:ctx)c.profile_batch=std::atoi(e)!=0;
    std::vector<std::vector<PreparedTransferBatch>> batch_schedule;
    for(const auto& pw:schedule)batch_schedule.push_back(prepare_transfer_batches(pw,target,batch_limit));
    std::cerr<<"transfer_batch_limit="<<batch_limit<<" batches_per_sweep=";
    for(const auto& batches:batch_schedule)std::cerr<<batches.size()<<' ';
    std::cerr<<'\n';
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"set residue reset");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"set modulus");uint64_t reciprocal=oneesan::division_reciprocal(mod);ck(cudaMemcpyToSymbol(D_MOD_RECIPROCAL,&reciprocal,sizeof(reciprocal)),"set modulus reciprocal");if(ml[d])ck(cudaMemset(mp[d],0,size_t(ml[d])*sizeof(Count)),"zero main");if(bl[d])ck(cudaMemset(bp[d],0,size_t(bl[d])*sizeof(Count)),"zero block");ck(cudaDeviceSynchronize(),"zero sync");ctx[d].active=0;ctx[d].gather_s=ctx[d].transition_s=ctx[d].scatter_s=ctx[d].group_graph_s=0;ctx[d].groups=0;ctx[d].transfer_builds=ctx[d].transfer_hits=ctx[d].transfer_skips=0;ctx[d].transfer_build_s=0;ctx[d].transfer_peak=ctx[d].transfer_bytes;ctx[d].graph_builds=ctx[d].graph_replays=ctx[d].graph_evictions=0;}
        ...
        int done_windows=0;
        for(auto& c:ctx)c.transfer_batches=c.transfer_batched_groups=0;
        for(int row=prefixK;row<W-1;++row){
            for(size_t w=0;w<schedule.size();++w){
                const auto& pw=schedule[w];const auto& batches=batch_schedule[w];
                std::atomic<size_t> next{0};std::vector<std::thread> ths;ths.reserve(ng);
                for(int d=0;d<ng;++d)ths.emplace_back([&,d]{
                    for(;;){size_t q=next.fetch_add(1,std::memory_order_relaxed);if(q>=batches.size())break;
                        process_transfer_batch(ctx[d],W,pw,batches[q],threads,target);}
                    ck(cudaSetDevice(d),"window drain device");ctx[d].drain_groups();
                });
                for(auto& t:ths)t.join();++done_windows;
            }
            std::cerr<<"mod "<<(ri+1)<<"/"<<mods.size()<<" p="<<mod<<" row "<<row+1<<"/"<<W<<"\n";
        }
        uint64_t batch_runs=0,batched_groups=0;size_t scratch_peak=0;
        for(const auto& c:ctx){batch_runs+=c.transfer_batches;batched_groups+=c.transfer_batched_groups;scratch_peak=std::max(scratch_peak,c.capArena);}
        std::cerr<<"transfer_batches="<<batch_runs<<" transfer_batched_groups="<<batched_groups<<" scratch_peak_mib="<<double(scratch_peak)/(1<<20)<<'\n';
        Count ans=0;
        for(int d=0;d<ng;++d){
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0,ggs=0;uint64_t graphBuilds=0,graphReplays=0,graphEvictions=0,graphEntries=0,mapBuilds=0,mapHits=0,mapSkips=0;size_t mapPeak=0;double mapBuildTime=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;ggs+=c.group_graph_s;graphBuilds+=c.graph_builds;graphReplays+=c.graph_replays;graphEvictions+=c.graph_evictions;graphEntries+=c.transition_graphs.size();mapBuilds+=c.transfer_builds;mapHits+=c.transfer_hits;mapSkips+=c.transfer_skips;mapPeak=std::max(mapPeak,c.transfer_peak);mapBuildTime+=c.transfer_build_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" frontier_threads="<<ctx[0].frontier_threads<<" pipeline_groups="<<ctx[0].pipeline_groups<<" frontier_blocks="<<ctx[0].frontier_blocks<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" transfer_builds="<<mapBuilds<<" transfer_hits="<<mapHits<<" transfer_skips="<<mapSkips<<" transfer_peak_mib="<<double(mapPeak)/(1<<20)<<" transfer_build_s="<<mapBuildTime<<" graph_builds="<<graphBuilds<<" graph_replays="<<graphReplays<<" graph_evictions="<<graphEvictions<<" graph_cache_entries="<<graphEntries<<" group_graph_sum_s="<<ggs<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);cudaFree(fLD[d]);if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);if(fHR[d])cudaFree(fHR[d]);if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);}
}
