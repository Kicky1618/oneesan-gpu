...
int main(int argc,char**argv){
    ...
    bool halfMode=true;if(const char*e=std::getenv("GRIDFP_HALF_MEANDER"))halfMode=std::atoi(e)!=0;if(halfMode&&ng>1)std::cerr<<"half join fallback uses GPU0 with P2P reads from "<<ng<<" authoritative shards\n";
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        // Half mode frees the authoritative blocked shards before the central join.
        // Re-create them here for the next residue; at a row boundary they start at zero.
        ...
        if(ng!=1&&prefixK>1&&!(requestDirect5&&prefixK==5)){std::cerr<<"bounded prefix prototype currently requires NGPU=1 except direct row5\n";return 31;}
        if(prefixK==1){
        ...
        Count ans=0;if(halfMode){
            ...
            // No blocked state survives a completed row, and the half-height computation
            // stops at a row boundary.  Reclaim the very large blocked authoritative shards
            // before loading meander factors / join tiles.
            ...
            size_t jf=~size_t(0);for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"half postfree meminfo set");size_t f=0,t=0;ck(cudaMemGetInfo(&f,&t),"half postfree meminfo");jf=std::min(jf,f);}std::cerr<<"half join free_after_block_release_gib="<<double(jf)/(1ull<<30)<<"\n";
