...
__global__ void r6_prefix_level_kernel(Code parents,const Count*cur,const int8_t*hp,Count*nxt,int8_t*hn,const Count*mat){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<children;q+=st){
        Code p=q/3;int sym=int(q-p*3),h=hp[p];if(h<0){hn[q]=-1;continue;}int h2=h+r6_gpu_delta(sym);if(h2<0||h2>6){hn[q]=-1;continue;}hn[q]=int8_t(h2);
        int ns=r6_block_size(h),nd=r6_block_size(h2);const Count*M=mat+D_R6_GPU_MAT_OFF[sym][h];
        for(int d=0;d<nd;++d){Count acc=0;for(int a=0;a<ns;++a){Count u=cur[Code(a)*parents+p],c=M[a*nd+d];if(u&&c)acc=r3_add_mod(acc,r3_mul_mod(u,c));}nxt[Code(d)*children+q]=acc;}
    }
}
__global__ void r6_suffix_level_kernel(Code parents,const Count*cur,const int8_t*hp,Count*nxt,int8_t*hn,const Count*mat){
    Code children=parents*3;
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<children;q+=st){
        int sym=int(q/parents);Code p=q-Code(sym)*parents;int h=hp[p];if(h<0){hn[q]=-1;continue;}int hs=h-r6_gpu_delta(sym);if(hs<0||hs>6){hn[q]=-1;continue;}hn[q]=int8_t(hs);
        int ns=r6_block_size(hs),nd=r6_block_size(h);const Count*M=mat+D_R6_GPU_MAT_OFF[sym][hs];
        for(int a=0;a<ns;++a){Count acc=0;const Count*row=M+a*nd;for(int d=0;d<nd;++d){Count u=cur[Code(d)*parents+p],c=row[d];if(u&&c)acc=r3_add_mod(acc,r3_mul_mod(c,u));}nxt[Code(a)*children+q]=acc;}
    }
}
__global__ void r6_pack_level_kernel(Code codes,const Count*dense,const int8_t*height,const uint32_t*idx,Count*packed){
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<codes;q+=st){int h=height[q];if(h<0)continue;uint32_t o=idx[q];int n=r6_block_size(h);for(int j=0;j<n;++j)packed[Code(o)+j]=dense[Code(j)*codes+q];}
}
...
template<int LANES,int MIN_H=0> __global__ void bounded_fill_row6_full_group_range_kernel(Count*fullMain,Code blo,Code bhi,Code fullBase){
...
#pragma unroll
        for(int pos=TARGET_W-1;pos>=0;--pos){int dig=0;Code a=D_BOUND_DP[pos][h];if(r<a)dig=0;else{r-=a;if(h>0){a=D_BOUND_DP[pos][h-1];if(r<a){dig=1;grank+=D_FULL_DP[pos][h];--h;}else{r-=a;dig=2;grank+=D_FULL_DP[pos][h]+D_FULL_DP[pos][h-1];++h;}}else{dig=2;grank+=D_FULL_DP[pos][h];++h;}}if(pos>=LO){ch=ch*3u+dig;if(pos==LO)hs=h;}else cl=cl*3u+dig;}meta=uint64_t(ch)|(uint64_t(cl)<<24)|(uint64_t(hs)<<48);}
        meta=__shfl_sync(mask,meta,leader);grank=__shfl_sync(mask,grank,leader);uint32_t ch=uint32_t(meta&0xffffffu),cl=uint32_t((meta>>24)&0xffffffu);int hs=int((meta>>48)&0xffu);if(hs<MIN_H)continue;int dim=r6_block_size(hs);const Count*pv=D_R6_PREF+D_R6_PREF_IDX[ch];const Count*sv=D_R6_SUFF+D_R6_SUFF_IDX[cl];Count acc=0;for(int i=lane;i<dim;i+=LANES)acc=r3_add_mod(acc,r3_mul_mod(pv[i],sv[i]));for(int off=LANES/2;off;off>>=1)acc=r3_add_mod(acc,__shfl_down_sync(mask,acc,off,LANES));if(lane==0)fullMain[grank-fullBase]=acc;}
}
...
static PackedRow6Lut make_row6_prefix_lut_packed_levels(int len){PackedRow6Lut cur;std::vector<int8_t>h(1,1);cur.idx.assign(1,0);cur.data.assign(r6_block_size(1),0);cur.data[row6_layout().li[0]]=1;size_t cnt=1;for(int d=0;d<len;++d){auto lev0=std::chrono::steady_clock::now();size_t nc=cnt*3;PackedRow6Lut nxt;nxt.idx.assign(nc,0xffffffffu);std::vector<int8_t>nh(nc,-127);uint64_t total=0;for(size_t c=0;c<cnt;++c)if(h[c]>=0)for(int a=0;a<3;++a){int h2=h[c]+row6_delta(a);if(h2<0||h2>6)continue;size_t q=c*3+a;nh[q]=h2;nxt.idx[q]=uint32_t(total);total+=r6_block_size(h2);}if(total>0xffffffffULL)throw std::runtime_error("row6 packed level offset overflow");nxt.data.assign(total,0);auto fill0=std::chrono::steady_clock::now();row6_parallel_for(cnt,[&](size_t c){int hc=h[c];if(hc<0)return;const Count*in=cur.data.data()+cur.idx[c];for(int a=0;a<3;++a){size_t q=c*3+a;if(nh[q]<0)continue;row6_apply_row_compact(in,nxt.data.data()+nxt.idx[q],hc,a);}});auto lev1=std::chrono::steady_clock::now();if(std::getenv("GRIDFP_ROW6_PROFILE_LUT"))std::cerr<<"r6 pref level="<<d+1<<" codes="<<nc<<" data="<<total<<" setup_s="<<std::chrono::duration<double>(fill0-lev0).count()<<" fill_s="<<std::chrono::duration<double>(lev1-fill0).count()<<"\n";cur=std::move(nxt);h=std::move(nh);cnt=nc;}return cur;}
static PackedRow6Lut make_row6_suffix_lut_packed_levels(int len){using namespace oneesan::row6mod;PackedRow6Lut cur;std::vector<int8_t>h(1,0);cur.idx.assign(1,0);cur.data.assign(r6_block_size(0),0);auto const&lay=row6_layout();for(int i=0;i<DIM;++i)if(BETA[i])cur.data[lay.li[i]]=BETA[i];size_t cnt=1;for(int d=0;d<len;++d){auto lev0=std::chrono::steady_clock::now();size_t nc=cnt*3;PackedRow6Lut nxt;nxt.idx.assign(nc,0xffffffffu);std::vector<int8_t>nh(nc,-127);uint64_t total=0;for(int a=0;a<3;++a)for(size_t c=0;c<cnt;++c)if(h[c]>=0){int h2=h[c]-row6_delta(a);if(h2<0||h2>6)continue;size_t q=size_t(a)*cnt+c;nh[q]=h2;nxt.idx[q]=uint32_t(total);total+=r6_block_size(h2);}if(total>0xffffffffULL)throw std::runtime_error("row6 packed level offset overflow");nxt.data.assign(total,0);auto fill0=std::chrono::steady_clock::now();row6_parallel_for(cnt,[&](size_t c){int hc=h[c];if(hc<0)return;const Count*in=cur.data.data()+cur.idx[c];for(int a=0;a<3;++a){int h2=hc-row6_delta(a);if(h2<0||h2>6)continue;size_t q=size_t(a)*cnt+c;row6_apply_col_compact(in,nxt.data.data()+nxt.idx[q],hc,a);}});auto lev1=std::chrono::steady_clock::now();if(std::getenv("GRIDFP_ROW6_PROFILE_LUT"))std::cerr<<"r6 suff level="<<d+1<<" codes="<<nc<<" data="<<total<<" setup_s="<<std::chrono::duration<double>(fill0-lev0).count()<<" fill_s="<<std::chrono::duration<double>(lev1-fill0).count()<<"\n";cur=std::move(nxt);h=std::move(nh);cnt=nc;}return cur;}

static std::pair<PackedRow6Lut,PackedRow6Lut> make_row6_luts_gpu_roundtrip(int plen,int slen){
    using namespace oneesan::row6mod;
    auto all0=std::chrono::steady_clock::now();auto const&lay=row6_layout();
    std::vector<Count>mat,beta(r6_block_size(0));uint32_t moff[3][7]{};const uint32_t*offs[3]={OFF_N,OFF_R,OFF_L};const Tr*trs[3]={TR_N,TR_R,TR_L};
    for(int sym=0;sym<3;++sym)for(int h=0;h<=6;++h){int h2=h+row6_delta(sym);if(h2<0||h2>6)continue;int ns=r6_block_size(h),nd=r6_block_size(h2);moff[sym][h]=uint32_t(mat.size());mat.resize(mat.size()+size_t(ns)*nd,0);Count*M=mat.data()+moff[sym][h];for(int a=0;a<ns;++a){int gi=lay.bidx[h][a];for(uint32_t q=offs[sym][gi];q<offs[sym][gi+1];++q){int gd=trs[sym][q].dst;if(HEIGHT[gd]!=h2)throw std::runtime_error("row6 GPU roundtrip height grading");int d=lay.li[gd];M[size_t(a)*nd+d]=row6_add_host(M[size_t(a)*nd+d],trs[sym][q].coeff);}}}
    for(int i=0;i<DIM;++i)if(BETA[i]){if(HEIGHT[i]!=0)throw std::runtime_error("row6 GPU roundtrip beta height");beta[lay.li[i]]=row6_add_host(beta[lay.li[i]],BETA[i]);}
    auto make_idx=[&](int len,bool prefix){size_t codes=1;for(int i=0;i<len;++i)codes*=3;PackedRow6Lut z;z.idx.assign(codes,0xffffffffu);uint64_t total=0;for(size_t c=0;c<codes;++c){int h=prefix?row6_prefix_code_height(c,len):row6_suffix_code_height(c,len);if(h>=0){if(total>0xffffffffULL)throw std::runtime_error("row6 GPU roundtrip packed offset overflow");z.idx[c]=uint32_t(total);total+=r6_block_size(h);}}z.data.resize(total);return z;};
    PackedRow6Lut pref=make_idx(plen,true),suff=make_idx(slen,false);
    ck(cudaSetDevice(0),"r6 GPU roundtrip set");Count mod=MOD;ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"r6 GPU roundtrip mod");ck(cudaMemcpyToSymbol(D_R6_GPU_MAT_OFF,moff,sizeof(moff)),"r6 GPU roundtrip mat off");
    Count*dmat=nullptr,*dbeta=nullptr;uint32_t*dpidx=nullptr,*dsidx=nullptr;ck(cudaMalloc(&dmat,mat.size()*sizeof(Count)),"r6 GPU roundtrip mat");ck(cudaMalloc(&dbeta,beta.size()*sizeof(Count)),"r6 GPU roundtrip beta");ck(cudaMalloc(&dpidx,pref.idx.size()*sizeof(uint32_t)),"r6 GPU roundtrip pidx");ck(cudaMalloc(&dsidx,suff.idx.size()*sizeof(uint32_t)),"r6 GPU roundtrip sidx");ck(cudaMemcpy(dmat,mat.data(),mat.size()*sizeof(Count),cudaMemcpyHostToDevice),"r6 GPU roundtrip mat cp");ck(cudaMemcpy(dbeta,beta.data(),beta.size()*sizeof(Count),cudaMemcpyHostToDevice),"r6 GPU roundtrip beta cp");ck(cudaMemcpy(dpidx,pref.idx.data(),pref.idx.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"r6 GPU roundtrip pidx cp");ck(cudaMemcpy(dsidx,suff.idx.data(),suff.idx.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"r6 GPU roundtrip sidx cp");
    ...
    auto build_pref=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*sizeof(Count)),"r6 GPU roundtrip pref base");ck(cudaMemset(cur,0,size_t(182)*sizeof(Count)),"r6 GPU roundtrip pref zero");Count one=1;ck(cudaMemcpy(cur+lay.li[0],&one,sizeof(one),cudaMemcpyHostToDevice),"r6 GPU roundtrip pref one");ck(cudaMalloc(&hp,1),"r6 GPU roundtrip pref h");int8_t h1=1;ck(cudaMemcpy(hp,&h1,1,cudaMemcpyHostToDevice),"r6 GPU roundtrip pref h1");for(int lev=0;lev<plen;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*sizeof(Count)),"r6 GPU roundtrip pref next");ck(cudaMalloc(&hn,size_t(children)),"r6 GPU roundtrip pref hn");bool wide=true;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_WIDE"))wide=std::atoi(e)!=0;if(wide&&children>=wideMin){int gb=int(std::min<Code>(children,2147483647));r6_prefix_level_wide_kernel<<<std::max(1,gb),wt>>>(parents,cur,hp,nxt,hn,dmat);}else{int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_prefix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);}ck(cudaGetLastError(),"r6 GPU roundtrip pref launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}Count*packed=nullptr;ck(cudaMalloc(&packed,pref.data.size()*sizeof(Count)),"r6 GPU roundtrip pref packed");int bl=int(std::min<Code>(65535,(parents+255)/256));r6_pack_level_kernel<<<std::max(1,bl),256>>>(parents,cur,hp,dpidx,packed);ck(cudaGetLastError(),"r6 GPU roundtrip pref pack");cudaFree(cur);cudaFree(hp);return packed;};
    auto build_suff=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*sizeof(Count)),"r6 GPU roundtrip suff base");ck(cudaMemset(cur,0,size_t(182)*sizeof(Count)),"r6 GPU roundtrip suff zero");ck(cudaMemcpy(cur,dbeta,beta.size()*sizeof(Count),cudaMemcpyDeviceToDevice),"r6 GPU roundtrip suff beta");ck(cudaMalloc(&hp,1),"r6 GPU roundtrip suff h");int8_t h0=0;ck(cudaMemcpy(hp,&h0,1,cudaMemcpyHostToDevice),"r6 GPU roundtrip suff h0");for(int lev=0;lev<slen;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*sizeof(Count)),"r6 GPU roundtrip suff next");ck(cudaMalloc(&hn,size_t(children)),"r6 GPU roundtrip suff hn");bool wide=true;if(const char*e=std::getenv("GRIDFP_ROW6_LEVEL_WIDE"))wide=std::atoi(e)!=0;if(wide&&children>=wideMin){int gb=int(std::min<Code>(children,2147483647));r6_suffix_level_wide_kernel<<<std::max(1,gb),wt>>>(parents,cur,hp,nxt,hn,dmat);}else{int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_suffix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);}ck(cudaGetLastError(),"r6 GPU roundtrip suff launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}Count*packed=nullptr;ck(cudaMalloc(&packed,suff.data.size()*sizeof(Count)),"r6 GPU roundtrip suff packed");int bl=int(std::min<Code>(65535,(parents+255)/256));r6_pack_level_kernel<<<std::max(1,bl),256>>>(parents,cur,hp,dsidx,packed);ck(cudaGetLastError(),"r6 GPU roundtrip suff pack");cudaFree(cur);cudaFree(hp);return packed;};
    auto k0=std::chrono::steady_clock::now();Count*dpref=build_pref();Count*dsuff=build_suff();ck(cudaDeviceSynchronize(),"r6 GPU roundtrip tables");double kernel_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-k0).count();auto cp0=std::chrono::steady_clock::now();ck(cudaMemcpy(pref.data.data(),dpref,pref.data.size()*sizeof(Count),cudaMemcpyDeviceToHost),"r6 GPU roundtrip pref back");ck(cudaMemcpy(suff.data.data(),dsuff,suff.data.size()*sizeof(Count),cudaMemcpyDeviceToHost),"r6 GPU roundtrip suff back");double copy_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-cp0).count();cudaFree(dpref);cudaFree(dsuff);cudaFree(dmat);cudaFree(dbeta);cudaFree(dpidx);cudaFree(dsidx);std::cerr<<"row6 GPU-LUT roundtrip kernel_s="<<kernel_s<<" copy_s="<<copy_s<<" total_s="<<std::chrono::duration<double>(std::chrono::steady_clock::now()-all0).count()<<" pref_mib="<<(pref.data.size()*4>>20)<<" suff_mib="<<(suff.data.size()*4>>20)<<"\\n";return {std::move(pref),std::move(suff)};
}

static void row6_print_lut_stats(const PackedRow6Lut& pref,int plen,const PackedRow6Lut& suff,int slen){
    std::array<std::vector<uint32_t>,7> pcodes,scodes;
    uint64_t pnz[7]{},snz[7]{},pcnt[7]{},scnt[7]{};
    for(uint32_t c=0;c<pref.idx.size();++c){int h=row6_prefix_code_height(c,plen);if(h<0)continue;pcodes[h].push_back(c);++pcnt[h];const Count*v=pref.data.data()+pref.idx[c];for(int i=0;i<r6_block_size(h);++i)pnz[h]+=v[i]!=0;}
    for(uint32_t c=0;c<suff.idx.size();++c){int h=row6_suffix_code_height(c,slen);if(h<0)continue;scodes[h].push_back(c);++scnt[h];const Count*v=suff.data.data()+suff.idx[c];for(int i=0;i<r6_block_size(h);++i)snz[h]+=v[i]!=0;}
    uint64_t rng=0x9e3779b97f4a7c15ULL;
    auto next=[&](){rng^=rng>>12;rng^=rng<<25;rng^=rng>>27;return rng*2685821657736338717ULL;};
    std::cerr<<"row6 lut density\n";
    for(int h=0;h<=6;++h){
        if(!pcnt[h]||!scnt[h])continue;
        const int dim=r6_block_size(h);const int samples=20000;uint64_t inter=0;
        for(int q=0;q<samples;++q){uint32_t pc=pcodes[h][next()%pcodes[h].size()],sc=scodes[h][next()%scodes[h].size()];const Count*a=pref.data.data()+pref.idx[pc],*b=suff.data.data()+suff.idx[sc];for(int i=0;i<dim;++i)inter+=(a[i]!=0)&&(b[i]!=0);}
        std::cerr<<"  h="<<h<<" dim="<<dim<<" pvec="<<pcnt[h]<<" svec="<<scnt[h]
                 <<" p_nz_avg="<<double(pnz[h])/pcnt[h]<<" s_nz_avg="<<double(snz[h])/scnt[h]
                 <<" intersection_avg="<<double(inter)/samples<<"\n";
    }
}

__global__ void row6_gemm_update_kernel(const int32_t* cmat,int ldc,int m,int n,
    ...
    constexpr unsigned long long MOD=oneesan::row6mod::MOD;
    for(unsigned long long q=unsigned(blockIdx.x)*blockDim.x+threadIdx.x,st=unsigned(gridDim.x)*blockDim.x;
        q<static_cast<unsigned long long>(m)*n;q+=st){
        int r=int(q%m),c=int(q/m);
        long long de=static_cast<long long>(cmat[r+size_t(ldc)*c])+rowCorr[r]+colCorr[c]-static_cast<long long>(npairs)*16384LL*k;
        unsigned long long x=acc[q]*256ULL+static_cast<unsigned long long>(de);
        if(reduceNow)x%=MOD;
        acc[q]=x;
        if(writeNow)fullMain[pbase[r]+srank[c]]=Count(x);
    }
}

static void row6_cublas_check(cublasStatus_t st,const char*what){if(st!=CUBLAS_STATUS_SUCCESS)throw std::runtime_error(std::string("cuBLAS ")+what+" status="+std::to_string(int(st)));}

static void row6_gemm_benchmark(const PackedRow6Lut&pref,int plen,const PackedRow6Lut&suff,int slen){
    cublasHandle_t handle=nullptr;row6_cublas_check(cublasCreate(&handle),"create");
    auto all0=std::chrono::steady_clock::now();
    for(int h=0;h<=6;++h){
        std::vector<uint32_t> pc,sc;
        for(uint32_t c=0;c<pref.idx.size();++c)if(row6_prefix_code_height(c,plen)==h)pc.push_back(c);
        for(uint32_t c=0;c<suff.idx.size();++c)if(row6_suffix_code_height(c,slen)==h)sc.push_back(c);
        if(pc.empty()||sc.empty())continue;
        const int m=int(pc.size()),n=int(sc.size()),k=r6_block_size(h),kp=(k+15)&~15,mp=(m+15)&~15,nPad=(n+15)&~15,lda=kp,ldb=kp,ldc=mp;
        auto prep0=std::chrono::steady_clock::now();
        std::array<std::vector<int8_t>,4>A,B;std::array<std::vector<int32_t>,4>ars,bcs;
        for(int a=0;a<4;++a){A[a].assign(size_t(lda)*mp,0);B[a].assign(size_t(ldb)*nPad,0);ars[a].assign(m,0);bcs[a].assign(n,0);}
        for(int r=0;r<m;++r){const Count*v=pref.data.data()+pref.idx[pc[r]];for(int j=0;j<k;++j){Count x=v[j];for(int a=0;a<4;++a){int z=int((x>>(8*a))&255u);A[a][j+size_t(lda)*r]=int8_t(z-128);ars[a][r]+=z;}}}
        for(int r=0;r<n;++r){const Count*v=suff.data.data()+suff.idx[sc[r]];for(int j=0;j<k;++j){Count x=v[j];for(int a=0;a<4;++a){int z=int((x>>(8*a))&255u);B[a][j+size_t(ldb)*r]=int8_t(z-128);bcs[a][r]+=z;}}}
        double prep_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-prep0).count();
        std::array<int8_t*,4>dA{},dB{};for(int a=0;a<4;++a){ck(cudaMalloc(&dA[a],A[a].size()),"r6 gemm A");ck(cudaMalloc(&dB[a],B[a].size()),"r6 gemm B");ck(cudaMemcpy(dA[a],A[a].data(),A[a].size(),cudaMemcpyHostToDevice),"r6 gemm Acp");ck(cudaMemcpy(dB[a],B[a].data(),B[a].size(),cudaMemcpyHostToDevice),"r6 gemm Bcp");}
        int32_t*dC=nullptr,*dRow=nullptr,*dCol=nullptr;unsigned long long*dAcc=nullptr;size_t outN=size_t(m)*n;
        ck(cudaMalloc(&dC,size_t(ldc)*nPad*sizeof(int32_t)),"r6 gemm C");ck(cudaMalloc(&dAcc,outN*sizeof(unsigned long long)),"r6 gemm acc");ck(cudaMemset(dAcc,0,outN*sizeof(unsigned long long)),"r6 gemm acc zero");
        ck(cudaMalloc(&dRow,size_t(m)*sizeof(int32_t)),"r6 gemm rowcorr");ck(cudaMalloc(&dCol,size_t(n)*sizeof(int32_t)),"r6 gemm colcorr");
        auto gpu0=std::chrono::steady_clock::now();
        for(int e=6;e>=0;--e){
            int nPairs=0;bool first=true;int32_t alpha=1;
            for(int a=0;a<4;++a){int b=e-a;if(b<0||b>=4)continue;++nPairs;int32_t beta=first?0:1;row6_cublas_check(cublasGemmEx(handle,CUBLAS_OP_T,CUBLAS_OP_N,mp,nPad,kp,&alpha,dA[a],CUDA_R_8I,lda,dB[b],CUDA_R_8I,ldb,&beta,dC,CUDA_R_32I,ldc,CUBLAS_COMPUTE_32I,CUBLAS_GEMM_DEFAULT),"gemmEx");first=false;}
            std::vector<int32_t>rc(m),cc(n);for(int r=0;r<m;++r){long long z=0;for(int a=0;a<4;++a){int b=e-a;if(b>=0&&b<4)z+=ars[a][r];}rc[r]=int32_t(128*z);}for(int c=0;c<n;++c){long long z=0;for(int b=0;b<4;++b){int a=e-b;if(a>=0&&a<4)z+=bcs[b][c];}cc[c]=int32_t(128*z);}
            ck(cudaMemcpy(dRow,rc.data(),size_t(m)*sizeof(int32_t),cudaMemcpyHostToDevice),"r6 gemm rowcorr cp");ck(cudaMemcpy(dCol,cc.data(),size_t(n)*sizeof(int32_t),cudaMemcpyHostToDevice),"r6 gemm colcorr cp");
            if(h==0&&std::getenv("GRIDFP_ROW6_GEMM_DEBUG")){int dr=2893,dc=817;int32_t cv=0;ck(cudaMemcpy(&cv,dC+(size_t(dc)*ldc+dr),sizeof(cv),cudaMemcpyDeviceToHost),"r6 gemm debug c");long long gd=static_cast<long long>(cv)+rc[dr]+cc[dc]-static_cast<long long>(nPairs)*16384LL*k;const Count*pa=pref.data.data()+pref.idx[pc[dr]],*pb=suff.data.data()+suff.idx[sc[dc]];unsigned long long wd=0;for(int j=0;j<k;++j)for(int a=0;a<4;++a){int b=e-a;if(b>=0&&b<4)wd+=unsigned((pa[j]>>(8*a))&255u)*unsigned((pb[j]>>(8*b))&255u);}std::cerr<<"row6 digitcheck e="<<e<<" c="<<cv<<" gpuD="<<gd<<" cpuD="<<wd<<"\n";}
            int blocks=int(std::min<size_t>(65535,(outN+255)/256));row6_gemm_update_kernel<<<std::max(1,blocks),256>>>(dC,ldc,m,n,dRow,dCol,dAcc,nPairs,k,e==3||e==0,nullptr,nullptr,nullptr,0);
        }
        ck(cudaDeviceSynchronize(),"r6 gemm bench sync");double gpu_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-gpu0).count();
        bool ok=true;uint64_t rng=0x123456789abcdefULL+uint64_t(h);for(int q=0;q<8&&ok;++q){rng=rng*6364136223846793005ULL+1;int r=int(rng%m);rng=rng*6364136223846793005ULL+1;int c=int(rng%n);unsigned long long got=0;ck(cudaMemcpy(&got,dAcc+(size_t(c)*m+r),sizeof(got),cudaMemcpyDeviceToHost),"r6 gemm sample");const Count*a=pref.data.data()+pref.idx[pc[r]],*b=suff.data.data()+suff.idx[sc[c]];uint64_t want=0;for(int j=0;j<k;++j)want=(want+uint64_t(a[j])*b[j])%oneesan::row6mod::MOD;if(got!=want){std::cerr<<"row6 gemm mismatch h="<<h<<" r="<<r<<" c="<<c<<" got="<<got<<" want="<<want<<"\n";ok=false;}}
        std::cerr<<"row6 gemm h="<<h<<" m="<<m<<" n="<<n<<" k="<<k<<" prep_s="<<prep_s<<" gpu_s="<<gpu_s<<" exact="<<(ok?1:0)<<" out_mib="<<(outN*sizeof(uint32_t)>>20)<<"\n";
        for(int a=0;a<4;++a){cudaFree(dA[a]);cudaFree(dB[a]);}cudaFree(dC);cudaFree(dAcc);cudaFree(dRow);cudaFree(dCol);
        if(!ok)break;
    }
    cublasDestroy(handle);std::cerr<<"row6 gemm total_s="<<std::chrono::duration<double>(std::chrono::steady_clock::now()-all0).count()<<"\n";
}

static double row6_gemm_fill_all_heights(const PackedRow6Lut&pref,int plen,const PackedRow6Lut&suff,int slen,Count*fullMain){
    cublasHandle_t handle=nullptr;row6_cublas_check(cublasCreate(&handle),"create fill");auto all0=std::chrono::steady_clock::now();
    int tileMiB=192;if(const char*e=std::getenv("GRIDFP_ROW6_GEMM_TILE_MIB"))tileMiB=std::max(32,std::atoi(e));
    for(int h=0;h<=6;++h){
        std::vector<uint32_t>pc,sc;for(uint32_t c=0;c<pref.idx.size();++c)if(row6_prefix_code_height(c,plen)==h)pc.push_back(c);for(uint32_t c=0;c<suff.idx.size();++c)if(row6_suffix_code_height(c,slen)==h)sc.push_back(c);
        if(pc.empty()||sc.empty())continue;
        const int m=int(pc.size()),n=int(sc.size()),k=r6_block_size(h),kp=(k+15)&~15,mp=(m+15)&~15,nPad=(n+15)&~15,lda=kp,ldb=kp,ldc=mp;auto prep0=std::chrono::steady_clock::now();
        std::array<std::vector<int8_t>,4>A,B;std::array<std::vector<int32_t>,4>ars,bcs;for(int a=0;a<4;++a){A[a].assign(size_t(lda)*mp,0);B[a].assign(size_t(ldb)*nPad,0);ars[a].assign(m,0);bcs[a].assign(n,0);}
        for(int r=0;r<m;++r){const Count*v=pref.data.data()+pref.idx[pc[r]];for(int j=0;j<k;++j){Count x=v[j];for(int a=0;a<4;++a){int z=int((x>>(8*a))&255u);A[a][j+size_t(lda)*r]=int8_t(z-128);ars[a][r]+=z;}}}
        for(int r=0;r<n;++r){const Count*v=suff.data.data()+suff.idx[sc[r]];for(int j=0;j<k;++j){Count x=v[j];for(int a=0;a<4;++a){int z=int((x>>(8*a))&255u);B[a][j+size_t(ldb)*r]=int8_t(z-128);bcs[a][r]+=z;}}}
        ...
        std::vector<Code>pbase(m),srank(n);for(int r=0;r<m;++r)pbase[r]=row6_prefix_full_base(pc[r],plen,plen+slen);for(int c=0;c<n;++c)srank[c]=row6_suffix_full_rank(sc[c],slen,h);double prep_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-prep0).count();
        std::array<int8_t*,4>dA{},dB{};for(int a=0;a<4;++a){ck(cudaMalloc(&dA[a],A[a].size()),"r6 fill A");ck(cudaMalloc(&dB[a],B[a].size()),"r6 fill B");ck(cudaMemcpy(dA[a],A[a].data(),A[a].size(),cudaMemcpyHostToDevice),"r6 fill Acp");ck(cudaMemcpy(dB[a],B[a].data(),B[a].size(),cudaMemcpyHostToDevice),"r6 fill Bcp");}
        int32_t*dRow=nullptr;Code*dPB=nullptr;ck(cudaMalloc(&dRow,size_t(m)*sizeof(int32_t)),"r6 fill row");ck(cudaMalloc(&dPB,size_t(m)*sizeof(Code)),"r6 fill pbase");ck(cudaMemcpy(dPB,pbase.data(),size_t(m)*sizeof(Code),cudaMemcpyHostToDevice),"r6 fill pbase cp");
        const size_t budget=size_t(tileMiB)<<20;size_t maxCells=std::max<size_t>(size_t(m)*16,budget/12);int tileCols=int(std::min<size_t>(n,std::max<size_t>(16,maxCells/size_t(m))));if(tileCols<n)tileCols=std::max(16,(tileCols/16)*16);int maxTilePad=(std::min(tileCols,n)+15)&~15;
        int32_t*dC=nullptr,*dCol=nullptr;unsigned long long*dAcc=nullptr;Code*dSR=nullptr;ck(cudaMalloc(&dC,size_t(ldc)*maxTilePad*sizeof(int32_t)),"r6 fill tiled C");ck(cudaMalloc(&dAcc,size_t(m)*std::min(tileCols,n)*sizeof(unsigned long long)),"r6 fill tiled acc");ck(cudaMalloc(&dCol,size_t(std::min(tileCols,n))*sizeof(int32_t)),"r6 fill tiled col");ck(cudaMalloc(&dSR,size_t(std::min(tileCols,n))*sizeof(Code)),"r6 fill tiled srank");
        auto gpu0=std::chrono::steady_clock::now();int tiles=0;
        for(int c0=0;c0<n;c0+=tileCols){++tiles;int nt=std::min(tileCols,n-c0),ntPad=(nt+15)&~15;size_t outN=size_t(m)*nt;ck(cudaMemset(dAcc,0,outN*sizeof(unsigned long long)),"r6 fill tiled acc zero");ck(cudaMemcpy(dSR,srank.data()+c0,size_t(nt)*sizeof(Code),cudaMemcpyHostToDevice),"r6 fill tiled srank cp");
            for(int e=6;e>=0;--e){int nPairs=0;bool first=true;int32_t alpha=1;for(int a=0;a<4;++a){int b=e-a;if(b<0||b>=4)continue;++nPairs;int32_t beta=first?0:1;row6_cublas_check(cublasGemmEx(handle,CUBLAS_OP_T,CUBLAS_OP_N,mp,ntPad,kp,&alpha,dA[a],CUDA_R_8I,lda,dB[b]+size_t(ldb)*c0,CUDA_R_8I,ldb,&beta,dC,CUDA_R_32I,ldc,CUBLAS_COMPUTE_32I,CUBLAS_GEMM_DEFAULT),"fill tiled gemmEx");first=false;}
                ck(cudaMemcpy(dRow,rc[e].data(),size_t(m)*sizeof(int32_t),cudaMemcpyHostToDevice),"r6 fill tiled row cp");ck(cudaMemcpy(dCol,cc[e].data()+c0,size_t(nt)*sizeof(int32_t),cudaMemcpyHostToDevice),"r6 fill tiled col cp");int blocks=int(std::min<size_t>(65535,(outN+255)/256));row6_gemm_update_kernel<<<std::max(1,blocks),256>>>(dC,ldc,m,nt,dRow,dCol,dAcc,nPairs,k,e==3||e==0,dPB,dSR,fullMain,e==0);
            }
        }
        ck(cudaDeviceSynchronize(),"r6 gemm tiled fill sync");double gpu_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-gpu0).count();bool ok=true;for(int q=0;q<8&&ok;++q){int r=(q*7919+17)%m,c=(q*1543+11)%n;Count got=0;ck(cudaMemcpy(&got,fullMain+pbase[r]+srank[c],sizeof(got),cudaMemcpyDeviceToHost),"r6 tiled fill verify");const Count*a=pref.data.data()+pref.idx[pc[r]],*b=suff.data.data()+suff.idx[sc[c]];uint64_t want=0;for(int j=0;j<k;++j)want=(want+uint64_t(a[j])*b[j])%oneesan::row6mod::MOD;if(got!=want){ok=false;std::cerr<<"row6 tiled fill mismatch h="<<h<<" r="<<r<<" c="<<c<<" got="<<got<<" want="<<want<<"\n";}}
        std::cerr<<"row6 gemm tiled fill h="<<h<<" states="<<size_t(m)*n<<" m="<<m<<" n="<<n<<" k="<<k<<" tiles="<<tiles<<" tile_cols="<<tileCols<<" tile_mib="<<tileMiB<<" prep_s="<<prep_s<<" gpu_s="<<gpu_s<<" exact="<<(ok?1:0)<<"\n";
        for(int a=0;a<4;++a){cudaFree(dA[a]);cudaFree(dB[a]);}cudaFree(dC);cudaFree(dAcc);cudaFree(dRow);cudaFree(dCol);cudaFree(dPB);cudaFree(dSR);if(!ok)throw std::runtime_error("row6 tiled gemm fill verification failed");
    }
    cublasDestroy(handle);double sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-all0).count();std::cerr<<"row6 gemm tiled fill all total_s="<<sec<<"\n";return sec;
}
...
static void init_direct_row6_lut_full_multi_gpu(int W,Count mod,int threads,Count**fullMain,const std::vector<Code>&mainLen,Code mainChunk,int ng){
    if(mod!=oneesan::row6mod::MOD)throw std::runtime_error("row6 prototype requires modulus 1000000007");Code dp[MAXW+1][MAXW+2]{};build_bounded_dp(6,dp);Code n=dp[W][1];constexpr int lo=TARGET_W/2,hi=TARGET_W-lo;bool gemmInit=false;if(const char*e=std::getenv("GRIDFP_ROW6_GEMM_INIT"))gemmInit=std::atoi(e)!=0;bool gpuLut=gemmInit;if(const char*e=std::getenv("GRIDFP_ROW6_GPU_LUT_FOR_GEMM"))gpuLut=std::atoi(e)!=0;auto t0=std::chrono::steady_clock::now();PackedRow6Lut pref,suff;if(gpuLut){auto z=make_row6_luts_gpu_roundtrip(hi,lo);pref=std::move(z.first);suff=std::move(z.second);}else{std::thread tp([&]{pref=make_row6_prefix_lut_packed_levels(hi);});std::thread ts([&]{suff=make_row6_suffix_lut_packed_levels(lo);});tp.join();ts.join();}double host_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();if(const char*e=std::getenv("GRIDFP_ROW6_STATS");e&&std::atoi(e))row6_print_lut_stats(pref,hi,suff,lo);if(const char*e=std::getenv("GRIDFP_ROW6_GEMM_BENCH");e&&std::atoi(e))row6_gemm_benchmark(pref,hi,suff,lo);
    if(gemmInit){
        if(ng!=1)throw std::runtime_error("row6 GEMM initializer prototype currently requires NGPU=1");
        ck(cudaSetDevice(0),"r6 gemm init set");ck(cudaMemcpyToSymbol(D_BOUND_DP,dp,sizeof(dp)),"r6 gemm init dp");int cap=6;ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"r6 gemm init cap");
        auto gg0=std::chrono::steady_clock::now();double gemm_s=row6_gemm_fill_all_heights(pref,hi,suff,lo,fullMain[0]);
        double total_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-gg0).count();std::cerr<<"direct row6 gemm states="<<n<<" lut_host_s="<<host_s<<" gemm_s="<<gemm_s<<" init_total_s="<<total_s<<"\n";return;
    }
    std::vector<Code>cut(ng+1);cut[0]=0;cut[ng]=n;for(int d=1;d<ng;++d)cut[d]=row5_lower_bound_full_rank(n,Code(d)*mainChunk,W,dp);int lanes=8;if(const char*e=std::getenv("GRIDFP_ROW6_LANES"))lanes=std::atoi(e);std::vector<double>gpu_s(ng);std::vector<std::thread>workers;auto g0=std::chrono::steady_clock::now();
    for(int d=0;d<ng;++d)workers.emplace_back([&,d]{ck(cudaSetDevice(d),"row6 set");ck(cudaMemcpyToSymbol(D_BOUND_DP,dp,sizeof(dp)),"row6 dp");int cap=6;ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"row6 cap");Count*dpref=nullptr,*dsuff=nullptr;uint32_t*dpidx=nullptr,*dsidx=nullptr;ck(cudaMalloc(&dpref,pref.data.size()*sizeof(Count)),"r6 pref");ck(cudaMalloc(&dsuff,suff.data.size()*sizeof(Count)),"r6 suff");ck(cudaMalloc(&dpidx,pref.idx.size()*4),"r6 pidx");ck(cudaMalloc(&dsidx,suff.idx.size()*4),"r6 sidx");ck(cudaMemcpy(dpref,pref.data.data(),pref.data.size()*4,cudaMemcpyHostToDevice),"r6 pref cp");ck(cudaMemcpy(dsuff,suff.data.data(),suff.data.size()*4,cudaMemcpyHostToDevice),"r6 suff cp");ck(cudaMemcpy(dpidx,pref.idx.data(),pref.idx.size()*4,cudaMemcpyHostToDevice),"r6 pidx cp");ck(cudaMemcpy(dsidx,suff.idx.data(),suff.idx.size()*4,cudaMemcpyHostToDevice),"r6 sidx cp");ck(cudaMemcpyToSymbol(D_R6_PREF,&dpref,sizeof(dpref)),"r6 pref ptr");ck(cudaMemcpyToSymbol(D_R6_SUFF,&dsuff,sizeof(dsuff)),"r6 suff ptr");ck(cudaMemcpyToSymbol(D_R6_PREF_IDX,&dpidx,sizeof(dpidx)),"r6 pidx ptr");ck(cudaMemcpyToSymbol(D_R6_SUFF_IDX,&dsidx,sizeof(dsidx)),"r6 sidx ptr");Code cnt=cut[d+1]-cut[d],base=Code(d)*mainChunk;int b=lanes?65535:int(std::min<Code>(65535,(cnt+threads-1)/threads));auto q0=std::chrono::steady_clock::now();if(cnt){if(lanes==16)bounded_fill_row6_full_group_range_kernel<16><<<std::max(1,b),threads>>>(fullMain[d],cut[d],cut[d+1],base);else if(lanes==8)bounded_fill_row6_full_group_range_kernel<8><<<std::max(1,b),threads>>>(fullMain[d],cut[d],cut[d+1],base);else if(lanes==4)bounded_fill_row6_full_group_range_kernel<4><<<std::max(1,b),threads>>>(fullMain[d],cut[d],cut[d+1],base);else bounded_fill_row6_full_range_kernel<<<std::max(1,b),threads>>>(fullMain[d],cut[d],cut[d+1],base);}ck(cudaDeviceSynchronize(),"r6 init");gpu_s[d]=std::chrono::duration<double>(std::chrono::steady_clock::now()-q0).count();cudaFree(dpref);cudaFree(dsuff);cudaFree(dpidx);cudaFree(dsidx);});for(auto&t:workers)t.join();double gw=std::chrono::duration<double>(std::chrono::steady_clock::now()-g0).count(),gm=*std::max_element(gpu_s.begin(),gpu_s.end());std::cerr<<"direct row6 full states="<<n<<" lut_host_s="<<host_s<<" kernel_max_s="<<gm<<" gpu_phase_wall_s="<<gw<<" pref_mib="<<(pref.data.size()*4>>20)<<" suff_mib="<<(suff.data.size()*4>>20)<<" pref_idx_mib="<<(pref.idx.size()*4>>20)<<" suff_idx_mib="<<(suff.idx.size()*4>>20)<<"\n";
}
