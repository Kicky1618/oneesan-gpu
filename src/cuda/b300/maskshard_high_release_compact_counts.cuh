#pragma once

#include <cstddef>
#include <cstdint>
#include <iostream>
#include <vector>

#ifndef MASKSHARD_HIGH_RELEASE_COMPACT_COUNTS
#error "HIGH compact-count release header requires MASKSHARD_HIGH_RELEASE_COMPACT_COUNTS"
#endif
#ifndef MASKSHARD_HIGH_ROW_BATCH_ASYNC
#error "HIGH compact-count release requires row-batch plan caching"
#endif
#ifndef MASKSHARD_HIGH_ROW_PLAN_CLASS_CACHE
#error "HIGH compact-count release is only enabled after class plans are materialized"
#endif
#ifndef MASKSHARD_ROW_DEPTH_ORBIT_COMPACT
#error "HIGH compact-count release requires compact row-depth metadata"
#endif

// v0.69: v0.67 materializes every unsaturated HIGH compact plan needed at
// runtime into G_MS_HIGH_ROW_BATCH_PLAN_CACHE during setup. Once that setup hook
// returns, MaskShardRowDepthOrbitCompactCache::{low_count,high_count} are no
// longer consulted by the HIGH execution path: saturated rows use the cached
// GroupSpec size, and unsaturated rows use the materialized plan directly.
// Release only those host-only cumulative-count vectors. The compact rank
// permutations remain resident on host/device exactly as before.
static void maskshard_report_high_mask_shard_layout_release_compact_counts(
    const MaskShardLayout& s
) {
    // At this point the macro still names the v0.61+ row-batch setup hook.
    report_high_mask_shard_layout(s);

    auto& compact = maskshard_row_depth_orbit_compact_cache();
    const std::size_t low_bytes =
        compact.low_count.capacity() * sizeof(std::uint16_t);
    const std::size_t high_bytes =
        compact.high_count.capacity() * sizeof(std::uint32_t);
    const std::size_t released = low_bytes + high_bytes;

    std::vector<std::uint16_t>().swap(compact.low_count);
    std::vector<std::uint32_t>().swap(compact.high_count);

    std::cerr << "HIGH compact count tables released host_mib="
              << double(released) / double(1ULL << 20)
              << " low_capacity=0 high_capacity=0\n";
}

#ifdef report_high_mask_shard_layout
#undef report_high_mask_shard_layout
#endif
#define report_high_mask_shard_layout \
        maskshard_report_high_mask_shard_layout_release_compact_counts
