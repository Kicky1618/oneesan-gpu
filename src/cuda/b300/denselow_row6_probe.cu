...
struct FactorTablesHost {
    ...
    std::vector<uint32_t> low_all_codes,low_mask_codes,low_mask_off,low_packed_rank,low_dense_packed_rank;
...
static FactorTablesHost build_factor_tables(){
    ...
    f.low_dense_packed_rank.assign(size_t(1)<<(2*L),0xffffffffu);for(uint32_t c:f.low_all_codes)f.low_dense_packed_rank[c]=f.low_packed_rank[ternary_index_host(c,L)];
    ...
    std::cerr<<"factor tables low_all="<<f.low_all_codes.size()<<" low_mask="<<f.low_mask_codes.size()<<" low_dense_mib="<<double(f.low_packed_rank.size()*4)/(1<<20)
             <<" low_4ary_dense_mib="<<double(f.low_dense_packed_rank.size()*4)/(1<<20)<<" high_all="<<f.high_all_codes.size()<<" high_mask="<<f.high_mask_codes.size()<<" high_dense_mib="<<double(f.high_packed_rank.size()*4)/(1<<20)<<"\n";
    return f;
}
...
int main(int argc,char**argv){
    ...
    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);if(fLR[d])cudaFree(fLR[d]);if(fLD[d])cudaFree(fLD[d]);if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);if(fHR[d])cudaFree(fHR[d]);if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(fTrit[d])cudaFree(fTrit[d]);}
}
