...
__global__ void blocked_group_inplace_kernel(const Count*in,Code n,Count*out_main,int p){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID sm=unrank_group_t<TARGET_W-1>(i,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);MateID t=minsert(sm,p,N);Code j=rank_group_t<TARGET_W>(t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);out_main[j]=add_mod_plain(out_main[j],c);}}
__global__ void main_to_block_kernel(const Count*in,const MateID*mates,Code n,Count*out_block,int p){
    ...
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=mates?mates[i]:unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,p);MateID t;int q,s;
        switch(w){
        case NR: case NL:{Code j=rank_drop_n_t<TARGET_W>(i,m,p);atomic_add_mod(out_block+j,c);break;}
        case LL:{t=msetpair(m,p,NN);q=p-1;s=1;while(s){--q;auto v=mget(t,q);if(v==L)++s;else if(v==R)--s;}t=mset(t,q,L);t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);break;}
        case RR:{t=msetpair(m,p,NN);q=p;s=1;while(s){++q;auto v=mget(t,q);if(v==L)--s;else if(v==R)++s;}t=mset(t,q,R);t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);break;}
        case RL:{t=msetpair(m,p,NN);t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);break;}
        default:break;}
    }
}

__global__ void main_block_orbit_kernel(Count*mainv,Count*blockv,const MateID*mates,Code n,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        MateID m=mates?mates[i]:unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);
        MateValuePair w=mpair(m,p);
        if(w!=NN && w!=NR && w!=NL)continue;
        Count c=mainv[i];
        Code dj=rank_drop_n_t<TARGET_W>(i,m,p); // delete the N at position p
        Count d=blockv[dj];
        if(w==NN){
            MateID t=msetpair(m,p,LR);
            Code j=rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);
            mainv[j]=add_mod_plain(mainv[j],c); // NN -> LR is injective
            mainv[i]=add_mod_plain(c,d);        // identity + old blocked contribution
            blockv[dj]=0;
        }else{
            ...
            if(p==1){
                ...
                mainv[i]=all;
                // RN/LN identity remains unchanged in mainv[j].
                blockv[dj]=c; // NR/NL -> blocked, after consuming the old blocked value.
            }
        }
    }
}

__global__ void main_closure_inplace_kernel(Count*mainv,Count*blockv,const MateID*mates,Code n,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        Count c=mainv[i];if(!c)continue;
        MateID m=mates?mates[i]:unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);
        MateValuePair w=mpair(m,p);MateID t;int q,s;
        if(w==LL){
            t=msetpair(m,p,NN);q=p-1;s=1;while(s){--q;auto v=mget(t,q);if(v==L)++s;else if(v==R)--s;}t=mset(t,q,L);
            if(p==1){Code j=rank_known_t<TARGET_W>(i,m,t,p,q,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(mainv+j,c);}
            else{t=mshrink(t,p-1);Code j=rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_mod(blockv+j,c);}
        }else if(w==RR){
            t=msetpair(m,p,NN);q=p;s=1;while(s){++q;auto v=mget(t,q);if(v==L)--s;else if(v==R)++s;}t=mset(t,q,R);
            if(p==1){Code j=rank_known_t<TARGET_W>(i,m,t,q,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(mainv+j,c);}
            else{t=mshrink(t,p-1);Code j=rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_mod(blockv+j,c);}
        }else if(w==RL){
            t=msetpair(m,p,NN);
            if(p==1){Code j=rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(mainv+j,c);}
            else{t=mshrink(t,p-1);Code j=rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_mod(blockv+j,c);}
        }
    }
}

__global__ void main_p1_pairs_kernel(Count*io,const MateID*mates,Code n){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=io[i];MateID m=mates?mates[i]:unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,1);MateID t;Code j;
        if(w==NN){t=msetpair(m,1,LR);j=rank_known_t<TARGET_W>(i,m,t,1,0,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);io[j]=add_mod_plain(io[j],c);}
        else if(w==NR){t=msetpair(m,1,RN);j=rank_known_t<TARGET_W>(i,m,t,1,0,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);Count z=add_mod_plain(c,io[j]);io[i]=z;io[j]=z;}
    }
}
__global__ void main_p1_closure_kernel(Count*io,const MateID*mates,Code n){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=io[i];if(!c)continue;MateID m=mates?mates[i]:unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,1);MateID t;int q,s;Code j;
        if(w==RR){t=msetpair(m,1,NN);q=1;s=1;while(s){++q;auto v=mget(t,q);if(v==L)--s;else if(v==R)++s;}t=mset(t,q,R);j=rank_known_t<TARGET_W>(i,m,t,q,0,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(io+j,c);}
    }
}
__global__ void main_to_main_inplace_kernel(Count*io,const MateID*mates,Code n,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=io[i];if(!c)continue;MateID m=mates?mates[i]:unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,p);MateID t;Code j;
        switch(w){
        case NN:t=msetpair(m,p,LR);j=rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);io[j]=add_mod_plain(io[j],c);break;
        case RN:t=msetpair(m,p,NR);j=rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);io[j]=add_mod_plain(io[j],c);break;
        case LN:t=msetpair(m,p,NL);j=rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);io[j]=add_mod_plain(io[j],c);break;
        default:break;}
    }
}
__global__ void main_group_kernel(const Count*in,const MateID*mates,Code n,Count*out_main,Count*out_block,int p){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=mates?mates[i]:unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,p);switch(w){case NN:{MateID t=msetpair(m,p,LR);atomic_add_mod(out_main+rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}case NR:case NL:{if(p==1){MateID t=msetpair(m,p,w==NR?RN:LN);atomic_add_mod(out_main+rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);}else{Code j=rank_drop_n_t<TARGET_W>(i,m,p);atomic_add_mod(out_block+j,c);}break;}case RN:{MateID t=msetpair(m,p,NR);atomic_add_mod(out_main+rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}case LN:{MateID t=msetpair(m,p,NL);atomic_add_mod(out_main+rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}case LL:{MateID t=msetpair(m,p,NN);int q=p-1,s=1;while(s){--q;auto v=mget(t,q);if(v==L)++s;else if(v==R)--s;}t=mset(t,q,L);if(p==1)atomic_add_mod(out_main+rank_known_t<TARGET_W>(i,m,t,p,q,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}case RR:{MateID t=msetpair(m,p,NN);int q=p,s=1;while(s){++q;auto v=mget(t,q);if(v==L)--s;else if(v==R)++s;}t=mset(t,q,R);if(p==1)atomic_add_mod(out_main+rank_known_t<TARGET_W>(i,m,t,q,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}case RL:{MateID t=msetpair(m,p,NN);if(p==1)atomic_add_mod(out_main+rank_known_t<TARGET_W>(i,m,t,p,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}default:break;}}}
...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dD=nullptr;MateID*dMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0;uint64_t groups=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(ms.size)main_block_orbit_kernel<<<bm,threads>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,p);
        ck(cudaGetLastError(),"main/block orbit");ck(cudaDeviceSynchronize(),"main/block orbit sync");
        if(ms.size)main_closure_inplace_kernel<<<bm,threads>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,p);
        ck(cudaGetLastError(),"closure inplace");ck(cudaDeviceSynchronize(),"closure inplace sync");
    }
...
int main(int argc,char**argv){
    ...
    int threads=256,maxgroups=0;auto prep0=std::chrono::steady_clock::now();std::vector<PreparedWindow> schedule;
    {
        ...
        for(auto const& r:ranges){
            ...
            for(int g=0;g<nj;++g){auto pg=prepare_group(W,pw.wp,g,mc,bc,ng);size_t b=size_t(pg.ms.size+pg.ds.size)*sizeof(Count);mx=std::max(mx,b);pw.groups.push_back(std::move(pg));}
