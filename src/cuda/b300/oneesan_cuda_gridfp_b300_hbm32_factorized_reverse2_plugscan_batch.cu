...
__device__ __forceinline__ uint64_t plug_pos_mask(MateID m){
    return (uint64_t(m) | (uint64_t(m)>>1)) & 0x5555555555555555ULL;
}
...
__global__ void reverse_main_p1_kernel(const Count*in,const Count*din,const MateID*mates,
    ...
    for(;i<n;i+=stride){
        ...
        case oneesan::gridfp::NN:{
            ...
            int s=1;
            uint64_t bits=plug_pos_mask(t) & ~((uint64_t(1)<<(2*2))-1ULL);
            while(bits&&s>0){int bit=__ffsll((long long)bits)-1;bits&=bits-1;int q=bit>>1;
...
__global__ void reverse_block_group_kernel(const Count*in,Code n,const MateID*mates,Count*out,Code dn,int p){
    ...
    for(;i<dn;i+=stride){
        ...
        if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
            ...
            while(bits&&s>0){int bit=63-__clzll(bits);bits&=(uint64_t(1)<<bit)-1ULL;int q=bit>>1;
