...
__constant__ unsigned long long* D_F_LOW_RANK_HASH;
__constant__ uint32_t D_F_LOW_RANK_HASH_MASK;
...
__constant__ unsigned long long* D_F_HIGH_RANK_HASH;
__constant__ uint32_t D_F_HIGH_RANK_HASH_MASK;
...
struct FactorTablesHost {
    ...
    std::vector<unsigned long long> low_rank_hash,high_rank_hash;
...
static FactorTablesHost build_factor_tables(){
    ...
    auto build_hash=[&](const std::vector<uint32_t>&codes,const std::vector<uint32_t>&dense){size_t n=1;while(n<codes.size()*2)n<<=1;std::vector<unsigned long long> tab(n,0);size_t maxProbe=0;for(uint32_t code:codes){uint32_t val=dense[code];if(val==0xffffffffu)throw std::runtime_error("factor hash missing dense rank");uint32_t x=code*0x9e3779b1u;x^=x>>16;size_t i=x&(n-1),probe=0;while(tab[i]){i=(i+1)&(n-1);++probe;}tab[i]=(uint64_t(code+1)<<32)|val;maxProbe=std::max(maxProbe,probe);}std::cerr<<"factor hash entries="<<codes.size()<<" slots="<<n<<" mib="<<double(n*8)/(1<<20)<<" max_probe="<<maxProbe<<"\\n";return tab;};
    f.low_rank_hash=build_hash(f.low_all_codes,f.low_dense_packed_rank);f.high_rank_hash=build_hash(f.high_all_codes,f.high_packed_rank);
...
__device__ __forceinline__ uint32_t factor_hash_lookup(const unsigned long long*tab,uint32_t mask,uint32_t code){uint32_t x=code*0x9e3779b1u;x^=x>>16;uint32_t i=x&mask;for(int q=0;q<64;++q){unsigned long long z=__ldg(tab+i);uint32_t k=uint32_t(z>>32);if(k==code+1)return uint32_t(z);if(k==0)return 0xffffffffu;i=(i+1)&mask;}return 0xffffffffu;}
__device__ __forceinline__ Code factor_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);int cv=int(mget(m,L));int bid=3*he+cv;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lp=factor_hash_lookup(D_F_LOW_RANK_HASH,D_F_LOW_RANK_HASH_MASK,lc),hp=factor_hash_lookup(D_F_HIGH_RANK_HASH,D_F_HIGH_RANK_HASH_MASK,hc);uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int h=seg_end_height(hc,H);FBlock x=D_F_BLOCK_BLOCKS[h];uint32_t lp=factor_hash_lookup(D_F_LOW_RANK_HASH,D_F_LOW_RANK_HASH_MASK,lc),hp=factor_hash_lookup(D_F_HIGH_RANK_HASH,D_F_HIGH_RANK_HASH_MASK,hc);uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_global_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);uint32_t hp=factor_hash_lookup(D_F_HIGH_RANK_HASH,D_F_HIGH_RANK_HASH_MASK,hc),lp=factor_hash_lookup(D_F_LOW_RANK_HASH,D_F_LOW_RANK_HASH_MASK,lc);uint32_t har=hp>>H,lar=lp>>L;Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[he]+har];MateValue c=mget(m,L);if(c>N)rank+=D_FULL_DP[L][he];if(c>R&&he>0)rank+=D_FULL_DP[L][he-1];return rank+lar;
}
__device__ __forceinline__ Code factor_global_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int he=seg_end_height(hc,H);uint32_t hp=factor_hash_lookup(D_F_HIGH_RANK_HASH,D_F_HIGH_RANK_HASH_MASK,hc),lp=factor_hash_lookup(D_F_LOW_RANK_HASH,D_F_LOW_RANK_HASH_MASK,lc);uint32_t har=hp>>H,lar=lp>>L;return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[he]+har]+lar;
}
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+vb8(G_FACTOR.low_rank_hash)+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+vb8(G_FACTOR.high_rank_hash)+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base);
    ...
    uint32_t *fLA[MAXGPU]{},*fLM[MAXGPU]{},*fLO[MAXGPU]{},*fLD[MAXGPU]{},*fHA[MAXGPU]{},*fHM[MAXGPU]{},*fHO[MAXGPU]{},*fHR[MAXGPU]{};unsigned long long*fLH[MAXGPU]{},*fHH[MAXGPU]{};Code *fHMB[MAXGPU]{},*fHBB[MAXGPU]{};
    for(int d=0;d<ng;++d){cudaSetDevice(d);auto cp=[&](uint32_t**dst,const std::vector<uint32_t>&v,const char*w){if(v.empty())return;ck(cudaMalloc(dst,v.size()*sizeof(uint32_t)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),w);};cp(&fLA[d],G_FACTOR.low_all_codes,"f low all");cp(&fLM[d],G_FACTOR.low_mask_codes,"f low mask");cp(&fLO[d],G_FACTOR.low_mask_off,"f low off");cp(&fHA[d],G_FACTOR.high_all_codes,"f high all");cp(&fHM[d],G_FACTOR.high_mask_codes,"f high mask");cp(&fHO[d],G_FACTOR.high_mask_off,"f high off");auto cph=[&](unsigned long long**dst,const std::vector<unsigned long long>&v,const char*w){ck(cudaMalloc(dst,v.size()*sizeof(unsigned long long)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(unsigned long long),cudaMemcpyHostToDevice),w);};cph(&fLH[d],G_FACTOR.low_rank_hash,"f low rank hash");cph(&fHH[d],G_FACTOR.high_rank_hash,"f high rank hash");uint32_t lhm=uint32_t(G_FACTOR.low_rank_hash.size()-1),hhm=uint32_t(G_FACTOR.high_rank_hash.size()-1);ck(cudaMemcpyToSymbol(D_F_LOW_RANK_HASH,&fLH[d],sizeof(fLH[d])),"f low hash ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_RANK_HASH,&fHH[d],sizeof(fHH[d])),"f high hash ptr");ck(cudaMemcpyToSymbol(D_F_LOW_RANK_HASH_MASK,&lhm,sizeof(lhm)),"f low hash mask");ck(cudaMemcpyToSymbol(D_F_HIGH_RANK_HASH_MASK,&hhm,sizeof(hhm)),"f high hash mask");auto cpc=[&](Code**dst,const std::vector<Code>&v,const char*w){ck(cudaMalloc(dst,v.size()*sizeof(Code)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(Code),cudaMemcpyHostToDevice),w);};cpc(&fHMB[d],G_FACTOR.high_main_base,"f high main base");cpc(&fHBB[d],G_FACTOR.high_block_base,"f high block base");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_CODES,&fLA[d],sizeof(fLA[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_MASK_CODES,&fLM[d],sizeof(fLM[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_MASK_OFF,&fLO[d],sizeof(fLO[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_ALL_CODES,&fHA[d],sizeof(fHA[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MASK_CODES,&fHM[d],sizeof(fHM[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MASK_OFF,&fHO[d],sizeof(fHO[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MAIN_BASE,&fHMB[d],sizeof(fHMB[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_BLOCK_BASE,&fHBB[d],sizeof(fHBB[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_OFF,G_FACTOR.low_all_off.data(),sizeof(uint32_t)*(MAXW+2)),"f low all off");ck(cudaMemcpyToSymbol(D_F_HIGH_ALL_OFF,G_FACTOR.high_all_off.data(),sizeof(uint32_t)*(MAXW+2)),"f high all off");}
