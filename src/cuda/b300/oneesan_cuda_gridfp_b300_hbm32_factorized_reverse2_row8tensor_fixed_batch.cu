...
#include "row7_tensor_init.cuh"
#include "row8_runtime_bounded.cuh"
#include "row8_tensor_init_fixed.cuh"
...
int main(int argc,char**argv){
    ...
    bool configuredDirect7=(configuredPrefixK==7);if(const char*e=std::getenv("GRIDFP_DIRECT_ROW7_TENSOR"))configuredDirect7=std::atoi(e)!=0;
    ...
    bool row7Pipeline=configuredDirect7&&configuredPrefixK==7&&mods.size()>1;
    double row8PrebuildS=0;bool row8Prebuild=configuredDirect8&&configuredPrefixK==8&&mods.size()>1&&ng>1;if(const char*e=std::getenv("GRIDFP_ROW8_PREBUILD_CACHE"))row8Prebuild=std::atoi(e)!=0;if(row8Prebuild){bool validate=false;if(const char*e=std::getenv("GRIDFP_ROW8_VALIDATE_BUILD"))validate=std::atoi(e)!=0;row8PrebuildS=oneesan::row8runtime::prebuild_mod_caches(mods,ng,validate);}
    const oneesan::row7runtime::ExactData* row7Exact=nullptr;std::future<oneesan::row7runtime::ModData> row7Future;
    if(row7Pipeline){auto const&ex=oneesan::row7tensor::exact_table(oneesan::row7tensor::exact_table_path());row7Exact=&ex;Count p0=mods[0];row7Future=std::async(std::launch::async,[row7Exact,p0]{return oneesan::row7runtime::load_or_build_mod(*row7Exact,p0);});std::cerr<<"row7 mod pipeline enabled residues="<<mods.size()<<"\n";}
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        bool requestDirect7=(prefixK==7);if(const char*e=std::getenv("GRIDFP_DIRECT_ROW7_TENSOR"))requestDirect7=std::atoi(e)!=0;
        bool requestDirect8=(prefixK==8);if(const char*e=std::getenv("GRIDFP_DIRECT_ROW8_TENSOR"))requestDirect8=std::atoi(e)!=0;
        if(ng!=1&&prefixK>1&&!((requestDirect5&&prefixK==5)||(requestDirect6&&prefixK==6)||(requestDirect7&&prefixK==7)||(requestDirect8&&prefixK==8))){std::cerr<<"bounded prefix prototype currently requires NGPU=1 except direct row5/6/7/8\n";return 31;}
        if(prefixK==1){
            ...
            }else if(direct7&&prefixK==7){
                if(row7Pipeline){auto ready0=std::chrono::steady_clock::now();auto md=row7Future.get();double wait_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-ready0).count();if(ri+1<mods.size()){Count pn=mods[ri+1];row7Future=std::async(std::launch::async,[row7Exact,pn]{return oneesan::row7runtime::load_or_build_mod(*row7Exact,pn);});}std::cerr<<"row7 pipeline residue="<<ri+1<<"/"<<mods.size()<<" mod="<<mod<<" table_wait_s="<<wait_s<<" table_build_s="<<md.build_s<<"\n";oneesan::row7tensor::init_multi_gpu_prebuilt(W,mod,mp,ml,mc,ng,md);}
                else oneesan::row7tensor::init_multi_gpu(W,mod,mp,ml,mc,ng);
