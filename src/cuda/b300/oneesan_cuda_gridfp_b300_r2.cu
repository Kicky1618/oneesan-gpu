...
__constant__ Count D_MOD0, D_MOD1;
...
static WindowPlan plan_window(int W,int p_hi,int p_lo,size_t target_bytes,int max_group_bits=16){
    ...
    for(int k=0;k<=klim;++k){
        ...
        for(uint64_t g=0;g<ng;++g){
            ...
            size_t b=size_t(2*ms.size+ds.size)*sizeof(Count2);
...
__device__ __forceinline__ void atomic_add_mod_lane(Count* p,Count v,Count mod){
    if(!v)return;
    Count old=atomicCAS(p,0ULL,0ULL);
    for(;;){
...
__device__ __forceinline__ void atomic_add_pair(Count2* p,Count2 v){
    atomic_add_mod_lane(&p->x,v.x,D_MOD0);
    atomic_add_mod_lane(&p->y,v.y,D_MOD1);
}

__global__ void blocked_group_kernel(const Count2* in,Code n,Count2* out_main,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x, stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        Count2 c=in[i]; if(!(c.x|c.y))continue;
        MateID sm=unrank_group(i,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);
        MateID t=minsert(sm,p,N); Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);
        atomic_add_pair(out_main+j,c);
    }
}

__global__ void main_group_kernel(const Count2* in,Code n,Count2* out_main,Count2* out_block,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x, stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        Count2 c=in[i];if(!(c.x|c.y))continue;
        MateID m=unrank_group(i,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,p);
        switch(w){
        case NN:{MateID t=msetpair(m,p,LR);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_pair(out_main+j,c);break;}
        case NR:case NL:{if(p==1){MateID t=msetpair(m,p,w==NR?RN:LN);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_pair(out_main+j,c);}else{MateID t=mshrink(m,p);Code j=rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_pair(out_block+j,c);}break;}
        case RN:{MateID t=msetpair(m,p,NR);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_pair(out_main+j,c);break;}
        case LN:{MateID t=msetpair(m,p,NL);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_pair(out_main+j,c);break;}
        case LL:{MateID t=msetpair(m,p,NN);int q=p-1,s=1;while(s){--q;auto v=mget(t,q);if(v==L)++s;else if(v==R)--s;}t=mset(t,q,L);if(p==1){Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_pair(out_main+j,c);}else{t=mshrink(t,p-1);Code j=rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_pair(out_block+j,c);}break;}
        case RR:{MateID t=msetpair(m,p,NN);int q=p,s=1;while(s){++q;auto v=mget(t,q);if(v==L)--s;else if(v==R)++s;}t=mset(t,q,R);if(p==1){Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_pair(out_main+j,c);}else{t=mshrink(t,p-1);Code j=rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_pair(out_block+j,c);}break;}
        case RL:{MateID t=msetpair(m,p,NN);if(p==1){Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_pair(out_main+j,c);}else{t=mshrink(t,p-1);Code j=rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_pair(out_block+j,c);}break;}
        default:break;
        }
    }
}
...
struct MappedCounts {
    int fd=-1; Count2* p=nullptr; size_t n=0, bytes=0; std::string path;
    void open_file(const std::string& fn,size_t count){
        path=fn;n=count;bytes=n*sizeof(Count2);
    ...
    Count2& operator[](size_t i){return p[i];}
};

static void gather_raw(const Count2* global,const std::vector<Interval>& iv,std::vector<Count2>& local){
    for(auto const& x:iv)std::memcpy(local.data()+x.local,global+x.global,size_t(x.len)*sizeof(Count2));
}
static void scatter_raw(Count2* global,const std::vector<Interval>& iv,const std::vector<Count2>& local){
    for(auto const& x:iv)std::memcpy(global+x.global,local.data()+x.local,size_t(x.len)*sizeof(Count2));
}

struct DeviceCtx {
    int dev = -1;
    Count2 *dA=nullptr, *dB=nullptr, *dD=nullptr;
    Code capM=0, capD=0;
    std::vector<Count2> hM, hD;
    ...
    void init(int device, Count mod0, Count mod1) {
        dev=device;
        ck(cudaSetDevice(dev), "cudaSetDevice init");
        ck(cudaMemcpyToSymbol(D_FULL_DP,H_DP,sizeof(H_DP)),"full dp");
        ck(cudaMemcpyToSymbol(D_MOD0,&mod0,sizeof(mod0)),"mod0");
        ck(cudaMemcpyToSymbol(D_MOD1,&mod1,sizeof(mod1)),"mod1");
    }
    void ensure(Code main_cap, Code block_cap) {
        ...
        if(main_cap>capM){
            ...
            capM=main_cap;
            ck(cudaMalloc(&dA,size_t(capM)*sizeof(Count2)),"dA");
            ck(cudaMalloc(&dB,size_t(capM)*sizeof(Count2)),"dB");
            hM.resize(capM);
        }
        if(block_cap>capD){
            if(dD)cudaFree(dD);
            capD=block_cap;
            ck(cudaMalloc(&dD,size_t(capD)*sizeof(Count2)),"dD");
            hD.resize(capD);
        }
    }
    void destroy(){
...
static void process_group(DeviceCtx& c,
    ...
    c.transferred += double(ms.size+ds.size)*sizeof(Count2);
    if(ms.size)ck(cudaMemcpy(c.dA,c.hM.data(),size_t(ms.size)*sizeof(Count2),cudaMemcpyHostToDevice),"H2D main");
    if(ds.size)ck(cudaMemcpy(c.dD,c.hD.data(),size_t(ds.size)*sizeof(Count2),cudaMemcpyHostToDevice),"H2D block");
    ...
    Count2* cur=c.dA; Count2* nxt=c.dB;
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(ms.size)ck(cudaMemcpy(nxt,cur,size_t(ms.size)*sizeof(Count2),cudaMemcpyDeviceToDevice),"identity");
        if(ds.size)blocked_group_kernel<<<bd,threads>>>(c.dD,ds.size,nxt,p);
        if(ds.size)ck(cudaMemset(c.dD,0,size_t(ds.size)*sizeof(Count2)),"clear new block");
    ...
    if(ms.size)ck(cudaMemcpy(c.hM.data(),cur,size_t(ms.size)*sizeof(Count2),cudaMemcpyDeviceToHost),"D2H main");
    if(ds.size)ck(cudaMemcpy(c.hD.data(),c.dD,size_t(ds.size)*sizeof(Count2),cudaMemcpyDeviceToHost),"D2H block");
    c.transferred += double(ms.size+ds.size)*sizeof(Count2);
...
int main(int argc,char**argv){
    ...
    Count mod0=argc>2?std::strtoull(argv[2],nullptr,10):2305843009213693951ULL;
    Count mod1=argc>3?std::strtoull(argv[3],nullptr,10):2305843009213693921ULL;
    int target_mib=argc>4?std::atoi(argv[4]):256;      // per GPU
    int max_window=argc>5?std::atoi(argv[5]):(n+1-1);
    int requested_gpus=argc>6?std::atoi(argv[6]):0;   // 0 = all visible GPUs
    int W=n+1;
    ...
    Count2 ans=mainv[rank_full(MateID(R),W)];
    ...
    std::cout<<"backend=gridfp-multigpu-mmap n="<<n<<" residue0="<<ans.x<<" residue1="<<ans.y<<" modulus0="<<mod0<<" modulus1="<<mod1
             <<" gpus="<<ngpu<<" peer_links="<<peer_links<<" main_states="<<mainN<<" blocked_states="<<blockN
             <<" external_bytes="<<size_t(mainN+blockN)*sizeof(Count2)
             <<" max_group_main="<<maxGM<<" max_group_blocked="<<maxGD
             <<" max_vram_per_gpu_bytes="<<size_t(2*maxGM+maxGD)*sizeof(Count2)
