...
__constant__ Count D_MOD; // scalar-only helpers; packed probe forces prefixK=1
__constant__ uint16_t D_RNS_P0, D_RNS_P1;
__constant__ uint32_t D_RNS_DPACK;
...
__device__ __forceinline__ Count rns_add_mod(Count a,Count b){
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 900
    uint32_t s,t,r,d=D_RNS_DPACK;
    asm("add.u16x2 %0, %1, %2;" : "=r"(s) : "r"(a), "r"(b));
    asm("add.u16x2 %0, %1, %2;" : "=r"(t) : "r"(s), "r"(d));
    asm("min.u16x2 %0, %1, %2;" : "=r"(r) : "r"(s), "r"(t));
    return r;
#else
    uint32_t x=(a&0xffffu)+(b&0xffffu),y=(a>>16)+(b>>16),p0=D_RNS_P0,p1=D_RNS_P1;
    if(x>=p0)x-=p0;if(y>=p1)y-=p1;return x|(y<<16);
#endif
}
static inline Count host_rns_add(Count a,Count b,uint32_t p0,uint32_t p1){uint32_t x=(a&0xffffu)+(b&0xffffu),y=(a>>16)+(b>>16);if(x>=p0)x-=p0;if(y>=p1)y-=p1;return x|(y<<16);}
static inline Count host_rns_pack(uint32_t a,uint32_t b){return (a&0xffffu)|((b&0xffffu)<<16);}
...
#pragma unroll
        ...
        global_store_main(factor_global_rank_main(m),Count(0x00010001u));
    }
}
...
__global__ void sum_first_row_states_kernel(int shard,int ng,Count* out){
    ...
    for(Code mask=Code(shard)+tid*Code(ng);mask<total;mask+=stride*Code(ng)){
        MateID m=MateID(oneesan::gridfp::R)<<(2*(TARGET_W-1));
#pragma unroll
        for(int p=TARGET_W-1;p>=1;--p){if((mask>>(p-1))&1ULL){auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);m=z.mate;}}
        acc=rns_add_mod(acc,global_load_main(factor_global_rank_main(m)));
    }
    sh[threadIdx.x]=acc;__syncthreads();
    for(unsigned q=blockDim.x/2;q;q>>=1){if(threadIdx.x<q)sh[threadIdx.x]=rns_add_mod(sh[threadIdx.x],sh[threadIdx.x+q]);__syncthreads();}
    if(threadIdx.x==0)atomic_add_mod(out,sh[0]);
}
...
__device__ __forceinline__ void atomic_add_mod(Count*p,Count v){if(!v)return;Count old=*p;for(;;){Count neu=rns_add_mod(old,v);Count seen=atomicCAS((unsigned int*)p,(unsigned int)old,(unsigned int)neu);if(seen==old)return;old=seen;}}
...
__global__ void bounded_reverse_main_kernel(const Count*in,const Count*din,Code n,Count*out,int width,int p){
    for(Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;i<n;i+=st){
        ...
        if(have){Count c=in[bounded_rank(pred,width,D_BOUND_DP,D_BOUND_CAP)];if(c)acc=rns_add_mod(acc,c);}
        if(oneesan::gridfp::mget(t,p)==oneesan::gridfp::N){MateID b=oneesan::gridfp::mshrink(t,p);Count c=din[bounded_rank(b,width-1,D_BOUND_DP,D_BOUND_CAP)];if(c)acc=rns_add_mod(acc,c);}
        out[i]=acc;
    }
}
...
__device__ __forceinline__ Count add_mod_plain(Count a,Count b){return rns_add_mod(a,b);}
...
int main(int argc,char**argv){
    ...
    uint32_t rns_p0=argc>5?(uint32_t)std::strtoul(argv[5],nullptr,10):32749u;
    uint32_t rns_p1=argc>6?(uint32_t)std::strtoul(argv[6],nullptr,10):32719u;
    if(rns_p0<3||rns_p1<3||rns_p0>=32768u||rns_p1>=32768u){std::cerr<<"RNS15 probe requires 3 <= p0,p1 < 32768\n";return 1;}
    std::vector<Count> mods{rns_p0};
    int W=n+1;
    if(n<2||W>MAXW){std::cerr<<"n=2..27\n";return 1;}
    if(W!=TARGET_W){std::cerr<<"specialized for width "<<TARGET_W<<" (n="<<(TARGET_W-1)<<")\n";return 1;}
    build_full_dp();
    // RNS15x2 probe forces prefixK=1; row6 CRT tables are unused.
    ...
    uint16_t hp0=(uint16_t)rns_p0,hp1=(uint16_t)rns_p1;uint32_t hdpack=(uint32_t)(65536u-rns_p0)|((uint32_t)(65536u-rns_p1)<<16);
    for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"rns const set");ck(cudaMemcpyToSymbol(D_RNS_P0,&hp0,sizeof(hp0)),"rns p0");ck(cudaMemcpyToSymbol(D_RNS_P1,&hp1,sizeof(hp1)),"rns p1");ck(cudaMemcpyToSymbol(D_RNS_DPACK,&hdpack,sizeof(hdpack)),"rns dpack");}
    ...
    MateID init=MateID(R)<<(2*(W-1));Code ig=rank_full(init,W);int io=int(ig/mc);Code fg=rank_full(MateID(R),W);int fo=int(fg/mc);Count one=host_rns_pack(1,1);
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        int prefixK=1;if(const char*e=std::getenv("GRIDFP_BOUNDED_PREFIX_K")){if(std::atoi(e)!=1){std::cerr<<"RNS15x2 probe requires GRIDFP_BOUNDED_PREFIX_K=1\n";return 35;}}
        ...
        for(int d=0;d<ng;++d){
            ...
            if(mine)sum_first_row_states_kernel<<<std::max(1,blocks),threads,threads*sizeof(Count)>>>(d,ng,da);
            Count x=0;ck(cudaMemcpy(&x,da,sizeof(x),cudaMemcpyDeviceToHost),"final sum copy");cudaFree(da);ans=host_rns_add(ans,x,rns_p0,rns_p1);
        }
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-rns15x2-probe n="<<n<<" residue0="<<(ans&0xffffu)<<" modulus0="<<rns_p0<<" residue1="<<(ans>>16)<<" modulus1="<<rns_p1<<" packed="<<ans<<" residue_index="<<ri<<" residues_total=2 gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }
