...
__global__ void reverse2_block_group_kernel_base(const Count*in,const Count*din,const MateID*mates,
...
template<bool HEAVY>
__global__ void reverse2_block_split_kernel(const Count*in,const Count*din,Count*out,Code dn,int p){
    int q=p-1;
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<dn;i+=stride){
        MateID b=factor_unrank_block(i); auto low=oneesan::gridfp::mget(b,q-1);
        if constexpr(HEAVY){ if(low!=oneesan::gridfp::N) continue; }
        else { if(low==oneesan::gridfp::N) continue; }
        Count acc=0;
        if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
            reverse2_add_main(acc,in,din,oneesan::gridfp::minsert(b,q,oneesan::gridfp::N),p);
        }else if(low==oneesan::gridfp::N){
            MateID u=oneesan::gridfp::minsert(b,q-1,oneesan::gridfp::N);
            if(height_before_rank_pos<TARGET_W>(u,q)>0) reverse2_add_main(acc,in,din,oneesan::gridfp::msetpair(u,q,oneesan::gridfp::RL),p);
            int ss=1;
            for(int r=q-2;r>=0&&ss>0;--r){auto v=oneesan::gridfp::mget(u,r);if(v==oneesan::gridfp::L&&ss==1){MateID c=oneesan::gridfp::msetpair(u,q,oneesan::gridfp::LL);c=oneesan::gridfp::mset(c,r,oneesan::gridfp::R);reverse2_add_main(acc,in,din,c,p);}if(v==oneesan::gridfp::L)++ss;else if(v==oneesan::gridfp::R)--ss;}
            ss=1;
            for(int r=q+1;r<TARGET_W&&ss>0;++r){auto v=oneesan::gridfp::mget(u,r);if(v==oneesan::gridfp::R&&ss==1){MateID c=oneesan::gridfp::msetpair(u,q,oneesan::gridfp::RR);c=oneesan::gridfp::mset(c,r,oneesan::gridfp::L);reverse2_add_main(acc,in,din,c,p);}if(v==oneesan::gridfp::L)--ss;else if(v==oneesan::gridfp::R)++ss;}
        }
        out[i]=acc;
    }
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;){
        if(p>2 && p-1>=wp.p_lo){
            ...
            if(ds.size){bool split=false;if(const char*e=std::getenv("GRIDFP_SPLIT_BLOCK"))split=std::atoi(e)!=0;if(split){reverse2_block_split_kernel<false><<<bd,threads,0,c.sBlock>>>(cur,dcur,dnext,ds.size,p);reverse2_block_split_kernel<true><<<bd,threads,0,c.sBlock>>>(cur,dcur,dnext,ds.size,p);}else reverse2_block_group_kernel_base<<<bd,threads,0,c.sBlock>>>(cur,dcur,nullptr,dnext,ds.size,p);}
            p-=2;
        }else{
            if(p>1){
