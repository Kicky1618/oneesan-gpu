...
__global__ void init_after_three_rows_kernel(Code total,int shard,int ng){
    ...
    for(Code q=tid;;q+=st){
        ...
        Code r=brank,grank=0;int h=1;R3Vec v{};v.x0=1;
#pragma unroll
        for(int pos=TARGET_W-1;pos>=0;--pos){
            MateValue z=N;Code a=D_BOUND_DP[pos][h];
            if(r<a)z=N;else{r-=a;if(h>0){a=D_BOUND_DP[pos][h-1];if(r<a)z=R;else{r-=a;z=L;}}else z=L;}
            if(z==R){grank+=D_FULL_DP[pos][h];--h;}
            else if(z==L){grank+=D_FULL_DP[pos][h]+(h>0?D_FULL_DP[pos][h-1]:0);++h;}
            v=r3_scalar_step(v,z);
        }
        Count ans=pm_mul(r3_scalar_finish(v),D_R3_UNSCALE);
