...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    int blockThreads=threads;if(const char*e=std::getenv("GRIDFP_BLOCK_THREADS")){int v=std::atoi(e);if(v==64||v==128||v==256||v==512)blockThreads=v;}
    int bm=int(std::min<Code>(65535,(ms.size+threads-1)/threads)),bd=int(std::min<Code>(65535,(ds.size+threads-1)/threads)),bdt=int(std::min<Code>(65535,(ds.size+blockThreads-1)/blockThreads));
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;){
        if(p>2 && p-1>=wp.p_lo){
            ...
            if(ds.size)reverse2_block_group_kernel<<<bdt,blockThreads,0,c.sBlock>>>(cur,dcur,nullptr,dnext,ds.size,p);
            p-=2;
        }else{
            if(p>1){
                if(ms.size)reverse_main_group_kernel<<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,ds.size,nxt,p);
                if(ds.size)reverse_block_group_kernel<<<bdt,blockThreads,0,c.sBlock>>>(cur,ms.size,dnext,ds.size,p);
            }else{
                ...
                if(ds.size)blocked_group_kernel<<<bdt,blockThreads,0,c.sBlock>>>(dcur,ds.size,nxt,p);
            }
            --p;
        }
