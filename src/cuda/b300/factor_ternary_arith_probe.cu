...
static FactorTablesHost build_factor_tables(){
    ...
    f.low_dense_packed_rank.assign(pow3_host(L),0xffffffffu);
    f.high_packed_rank.assign(pow3_host(H),0xffffffffu);
    for(int h=0;h<=MAXW;++h){f.low_all_off[h]=f.low_all_codes.size();for(uint32_t r=0;r<la[h].size();++r){auto c=la[h][r];f.low_all_codes.push_back(c);f.low_dense_packed_rank[ternary_index_host(c,L)]=r<<L;} }
    f.low_all_off[MAXW+1]=f.low_all_codes.size();
    f.low_mask_off.resize(size_t(LM)*S);
    for(uint32_t m=0;m<LM;++m)for(int h=0;h<S;++h){size_t ix=size_t(m)*S+h;f.low_mask_off[ix]=f.low_mask_codes.size();if(h<=MAXW)for(uint32_t r=0;r<lm[ix].size();++r){auto c=lm[ix][r];f.low_mask_codes.push_back(c);auto ti=ternary_index_host(c,L);f.low_dense_packed_rank[ti]=(f.low_dense_packed_rank[ti]&~((1u<<L)-1u))|r;}}
    for(int h=0;h<=MAXW;++h){f.high_all_off[h]=f.high_all_codes.size();for(uint32_t r=0;r<ha[h].size();++r){auto c=ha[h][r];f.high_all_codes.push_back(c);f.high_packed_rank[ternary_index_host(c,H)]=r<<H;}}
    f.high_all_off[MAXW+1]=f.high_all_codes.size();
    f.high_mask_off.resize(size_t(HM)*S);
    for(uint32_t m=0;m<HM;++m)for(int h=0;h<S;++h){size_t ix=size_t(m)*S+h;f.high_mask_off[ix]=f.high_mask_codes.size();if(h<=MAXW)for(uint32_t r=0;r<hm[ix].size();++r){auto c=hm[ix][r];f.high_mask_codes.push_back(c);auto ti=ternary_index_host(c,H);f.high_packed_rank[ti]=(f.high_packed_rank[ti]&~((1u<<H)-1u))|r;}}
    ...
    std::cerr<<"factor tables low_all="<<f.low_all_codes.size()<<" low_mask="<<f.low_mask_codes.size()<<" low_ternary_mib="<<double(f.low_dense_packed_rank.size()*4)/(1<<20)<<" high_all="<<f.high_all_codes.size()<<" high_mask="<<f.high_mask_codes.size()<<" high_ternary_mib="<<double(f.high_packed_rank.size()*4)/(1<<20)<<"\n";
    return f;
}
...
__device__ __forceinline__ uint32_t ternary_index_dev(uint32_t code,int len){uint32_t z=0,m=1;
#pragma unroll
    for(int i=0;i<7;++i){if(2*i>=len)break;uint32_t v=(code>>(4*i))&15u;uint32_t a=v&3u,b=(v>>2)&3u;z+=(a+3u*b)*m;m*=9u;}return z;}
__device__ __forceinline__ Code factor_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);int cv=int(mget(m,L));int bid=3*he+cv;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lp=D_F_LOW_DENSE_PACKED_RANK[ternary_index_dev(lc,L)],hp=D_F_HIGH_PACKED_RANK[ternary_index_dev(hc,H)];uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int h=seg_end_height(hc,H);FBlock x=D_F_BLOCK_BLOCKS[h];uint32_t lp=D_F_LOW_DENSE_PACKED_RANK[ternary_index_dev(lc,L)],hp=D_F_HIGH_PACKED_RANK[ternary_index_dev(hc,H)];uint32_t lr=D_F_FIX_LOW?(lp&((1u<<L)-1u)):(lp>>L);uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_global_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[ternary_index_dev(hc,H)],lp=D_F_LOW_DENSE_PACKED_RANK[ternary_index_dev(lc,L)];uint32_t har=hp>>H,lar=lp>>L;Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[he]+har];MateValue c=mget(m,L);if(c>N)rank+=D_FULL_DP[L][he];if(c>R&&he>0)rank+=D_FULL_DP[L][he-1];return rank+lar;
}
__device__ __forceinline__ Code factor_global_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[ternary_index_dev(hc,H)],lp=D_F_LOW_DENSE_PACKED_RANK[ternary_index_dev(lc,L)];uint32_t har=hp>>H,lar=lp>>L;return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[he]+har]+lar;
}
...
int main(int argc,char**argv){
    ...
    uint32_t *fLA[MAXGPU]{},*fLM[MAXGPU]{},*fLO[MAXGPU]{},*fLD[MAXGPU]{},*fHA[MAXGPU]{},*fHM[MAXGPU]{},*fHO[MAXGPU]{},*fHR[MAXGPU]{};uint16_t* fTrit[MAXGPU]{};Code *fHMB[MAXGPU]{},*fHBB[MAXGPU]{};
