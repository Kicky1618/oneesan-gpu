// v0.26 experiment: v0.25 threshold-29 HIGH packing plus exact row-depth LOW
// closure task mapping. HIGH behavior is unchanged from v0.25; only LOW closure
// enumerates the exact active HIGH-row x selected-LOW-column task set early.
...
#define main oneesan_maskshard_v26_lowclosureexacttasks_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.26 exact LOW closure tasks require threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.26-highrowpack29-rowdepthexact-exacttasks-highclosureexact-lowclosureexacttasks-batch"
    ...
    return oneesan_maskshard_v26_lowclosureexacttasks_inner_main(argc, argv);
}
