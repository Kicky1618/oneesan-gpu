// v0.22 experiment: v0.21 plus exact row-depth HIGH closure task mapping.
// Early rows enumerate only active selected HIGH rows x active LOW ranks; full
// depth falls back to original closure-row and physical LOW-rank order.
...
#define main oneesan_maskshard_v22_highclosureexacttasks_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.22 exact HIGH closure tasks require threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.22-highrowpack16-rowdepthexact-exacttasks-highclosureexacttasks-batch"
    ...
    return oneesan_maskshard_v22_highclosureexacttasks_inner_main(argc, argv);
}
