// v0.24 experiment: v0.23 exact row-depth HIGH closure with a less conservative
// row-packing threshold.  The task map and exact host launch table already use
// the compile-time threshold consistently; only the v0.11-era policy changes
// from dense LOW stride < 16 to dense LOW stride < 29.
...
#define main oneesan_maskshard_v24_highclosurethreshold29_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.24 threshold-29 HIGH closure requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.24-highrowpack29-rowdepthexact-exacttasks-highclosureexacttasks-exactlaunch-batch"
    ...
    return oneesan_maskshard_v24_highclosurethreshold29_inner_main(argc, argv);
}
