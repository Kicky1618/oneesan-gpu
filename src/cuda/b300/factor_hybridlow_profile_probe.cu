...
#ifndef LOW_PAGE_TRITS
#define LOW_PAGE_TRITS 4
#endif
...
__constant__ uint16_t* D_F_LOW_LOCAL_RANK;
__constant__ uint32_t* D_F_LOW_ALL_PAGE_OFF;
__constant__ uint32_t* D_F_LOW_ALL_PAGE_RANK;
...
struct FactorTablesHost {
    ...
    std::vector<uint32_t> low_all_codes,low_mask_codes,low_mask_off,low_all_page_off,low_all_page_rank;
    std::vector<uint16_t> low_local_rank;
...
static FactorTablesHost build_factor_tables(){
    ...
    constexpr int Q=(LOW_PAGE_TRITS<L?LOW_PAGE_TRITS:L);static_assert(Q>=1);constexpr uint32_t PS=1u<<(2*Q),PM=PS-1u;
    f.low_all_page_off.assign(size_t(1)<<(2*(L-Q)),0xffffffffu);
    auto all_slot=[&](uint32_t c)->uint32_t&{uint32_t pg=c>>(2*Q),o=f.low_all_page_off[pg];if(o==0xffffffffu){o=uint32_t(f.low_all_page_rank.size());f.low_all_page_off[pg]=o;f.low_all_page_rank.resize(size_t(o)+PS,0xffffffffu);}return f.low_all_page_rank[size_t(o)+(c&PM)];};
    f.low_local_rank.assign(size_t(1)<<(2*L),0xffffu);
    f.high_packed_rank.assign(size_t(1)<<(2*H),0xffffffffu);
    for(int h=0;h<=MAXW;++h){f.low_all_off[h]=f.low_all_codes.size();for(uint32_t r=0;r<la[h].size();++r){auto c=la[h][r];f.low_all_codes.push_back(c);all_slot(c)=r;} }
    f.low_all_off[MAXW+1]=f.low_all_codes.size();
    f.low_mask_off.resize(size_t(LM)*S);
    for(uint32_t m=0;m<LM;++m)for(int h=0;h<S;++h){size_t ix=size_t(m)*S+h;f.low_mask_off[ix]=f.low_mask_codes.size();if(h<=MAXW)for(uint32_t r=0;r<lm[ix].size();++r){if(r>0xffffu)throw std::runtime_error("low local rank exceeds u16");auto c=lm[ix][r];f.low_mask_codes.push_back(c);f.low_local_rank[c]=uint16_t(r);}}
    ...
    std::cerr<<"factor tables low_all="<<f.low_all_codes.size()<<" low_mask="<<f.low_mask_codes.size()<<" low_local_u16_mib="<<double(f.low_local_rank.size()*2)/(1<<20)<<" low_all_page_off_mib="<<double(f.low_all_page_off.size()*4)/(1<<20)<<" low_all_page_rank_mib="<<double(f.low_all_page_rank.size()*4)/(1<<20)<<" high_all="<<f.high_all_codes.size()<<" high_mask="<<f.high_mask_codes.size()<<" high_dense_mib="<<double(f.high_packed_rank.size()*4)/(1<<20)<<"\n";
    return f;
}
...
__device__ __forceinline__ uint32_t factor_low_all_rank(uint32_t code){constexpr int Q=(LOW_PAGE_TRITS<LOW_LUT_K?LOW_PAGE_TRITS:LOW_LUT_K);constexpr uint32_t PM=(1u<<(2*Q))-1u;uint32_t o=__ldg(D_F_LOW_ALL_PAGE_OFF+(code>>(2*Q)));return __ldg(D_F_LOW_ALL_PAGE_RANK+o+(code&PM));}
__device__ __forceinline__ uint32_t factor_low_local_rank(uint32_t code){return uint32_t(__ldg(D_F_LOW_LOCAL_RANK+code));}
__device__ __forceinline__ Code factor_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);int cv=int(mget(m,L));int bid=3*he+cv;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lr=D_F_FIX_LOW?factor_low_local_rank(lc):factor_low_all_rank(lc),hp=D_F_HIGH_PACKED_RANK[hc];uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int h=seg_end_height(hc,H);FBlock x=D_F_BLOCK_BLOCKS[h];uint32_t lr=D_F_FIX_LOW?factor_low_local_rank(lc):factor_low_all_rank(lc),hp=D_F_HIGH_PACKED_RANK[hc];uint32_t hr=D_F_FIX_LOW?(hp>>H):(hp&((1u<<H)-1u));return x.off+Code(hr)*x.stride+lr;
}
__device__ __forceinline__ Code factor_global_rank_main(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*(L+1)))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[hc],lar=factor_low_all_rank(lc);uint32_t har=hp>>H;Code rank=D_F_HIGH_MAIN_BASE[D_F_HIGH_ALL_OFF[he]+har];MateValue c=mget(m,L);if(c>N)rank+=D_FULL_DP[L][he];if(c>R&&he>0)rank+=D_FULL_DP[L][he-1];return rank+lar;
}
__device__ __forceinline__ Code factor_global_rank_block(MateID m){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u,HM=(1u<<(2*H))-1u;uint32_t lc=uint32_t(m)&LM,hc=uint32_t((m>>(2*L))&HM);int he=seg_end_height(hc,H);uint32_t hp=D_F_HIGH_PACKED_RANK[hc],lar=factor_low_all_rank(lc);uint32_t har=hp>>H;return D_F_HIGH_BLOCK_BASE[D_F_HIGH_ALL_OFF[he]+har]+lar;
}
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+uint64_t(G_FACTOR.low_local_rank.size())*2+vb4(G_FACTOR.low_all_page_off)+vb4(G_FACTOR.low_all_page_rank)+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+vb4(G_FACTOR.high_packed_rank)+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base);
    ...
    uint32_t *fLA[MAXGPU]{},*fLM[MAXGPU]{},*fLO[MAXGPU]{},*fLAPO[MAXGPU]{},*fLAPR[MAXGPU]{},*fHA[MAXGPU]{},*fHM[MAXGPU]{},*fHO[MAXGPU]{},*fHR[MAXGPU]{};uint16_t* fLLR[MAXGPU]{};Code *fHMB[MAXGPU]{},*fHBB[MAXGPU]{};
    for(int d=0;d<ng;++d){cudaSetDevice(d);auto cp=[&](uint32_t**dst,const std::vector<uint32_t>&v,const char*w){if(v.empty())return;ck(cudaMalloc(dst,v.size()*sizeof(uint32_t)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),w);};auto cp16=[&](uint16_t**dst,const std::vector<uint16_t>&v,const char*w){if(v.empty())return;ck(cudaMalloc(dst,v.size()*sizeof(uint16_t)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(uint16_t),cudaMemcpyHostToDevice),w);};cp(&fLA[d],G_FACTOR.low_all_codes,"f low all");cp(&fLM[d],G_FACTOR.low_mask_codes,"f low mask");cp(&fLO[d],G_FACTOR.low_mask_off,"f low off");cp16(&fLLR[d],G_FACTOR.low_local_rank,"f low local rank");cp(&fLAPO[d],G_FACTOR.low_all_page_off,"f low all page off");cp(&fLAPR[d],G_FACTOR.low_all_page_rank,"f low all page rank");cp(&fHA[d],G_FACTOR.high_all_codes,"f high all");cp(&fHM[d],G_FACTOR.high_mask_codes,"f high mask");cp(&fHO[d],G_FACTOR.high_mask_off,"f high off");cp(&fHR[d],G_FACTOR.high_packed_rank,"f high rank");auto cpc=[&](Code**dst,const std::vector<Code>&v,const char*w){ck(cudaMalloc(dst,v.size()*sizeof(Code)),w);ck(cudaMemcpy(*dst,v.data(),v.size()*sizeof(Code),cudaMemcpyHostToDevice),w);};cpc(&fHMB[d],G_FACTOR.high_main_base,"f high main base");cpc(&fHBB[d],G_FACTOR.high_block_base,"f high block base");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_CODES,&fLA[d],sizeof(fLA[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_MASK_CODES,&fLM[d],sizeof(fLM[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_MASK_OFF,&fLO[d],sizeof(fLO[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_LOCAL_RANK,&fLLR[d],sizeof(fLLR[d])),"f low local ptr");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_PAGE_OFF,&fLAPO[d],sizeof(fLAPO[d])),"f low all page off ptr");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_PAGE_RANK,&fLAPR[d],sizeof(fLAPR[d])),"f low all page rank ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_ALL_CODES,&fHA[d],sizeof(fHA[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MASK_CODES,&fHM[d],sizeof(fHM[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MASK_OFF,&fHO[d],sizeof(fHO[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_PACKED_RANK,&fHR[d],sizeof(fHR[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_MAIN_BASE,&fHMB[d],sizeof(fHMB[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_HIGH_BLOCK_BASE,&fHBB[d],sizeof(fHBB[d])),"f ptr");ck(cudaMemcpyToSymbol(D_F_LOW_ALL_OFF,G_FACTOR.low_all_off.data(),sizeof(uint32_t)*(MAXW+2)),"f low all off");ck(cudaMemcpyToSymbol(D_F_HIGH_ALL_OFF,G_FACTOR.high_all_off.data(),sizeof(uint32_t)*(MAXW+2)),"f high all off");}
