// v0.27 experiment: v0.26 exact LOW closure task mapping plus exact host launch
// sizing from the same row-depth active HIGH-row x selected-LOW-column product.
// HIGH behavior is unchanged from v0.25/v0.26.
...
#define main oneesan_maskshard_v27_lowclosureexactlaunch_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.27 exact LOW closure launch requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.27-lowclosureexactlaunch-batch"
        ...
        << " low_closure_row_depth_compact_launch=1"
    ...
    return oneesan_maskshard_v27_lowclosureexactlaunch_inner_main(argc, argv);
}
