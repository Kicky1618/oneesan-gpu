...
// Rank mshrink(t,p) in the block factorization by reusing the unchanged
// half-rank already encoded in the main target index ti.
__device__ __forceinline__ Code factor_rank_block_from_main(Code ti,MateID t,int p){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;
    ...
    if(D_F_FIX_LOW){
        // High-side window: p>L.  LOW is unchanged; recompute only HIGH after shrink.
    ...
    // Low-side window: p<=L.  HIGH is unchanged.  At the central N, LOW also stays unchanged.
    ...
    uint32_t lp=factor_low_rank(lc),nlr=lp>>L;
...
__device__ __forceinline__ Count reverse_eval_main_step(const Count*in,const Count*din,MateID t,int p){
    Code ti=factor_rank_main(t);Count acc=in[ti];
    MateID pred=0; bool have=false;
    switch(oneesan::gridfp::mpair(t,p)){
    case oneesan::gridfp::LR: pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::NN);have=true;break;
    case oneesan::gridfp::NR: pred=oneesan::gridfp::msetpair(t,p,oneesan::gridfp::RN);have=true;break;
    ...
    if(have){Count c=in[factor_rank_main(pred)];if(c)acc=add_mod_plain(acc,c);}
    if(oneesan::gridfp::mget(t,p)==oneesan::gridfp::N){
        Count c=din[factor_rank_block_from_main(ti,t,p)];if(c)acc=add_mod_plain(acc,c);
    }
    return acc;
}
