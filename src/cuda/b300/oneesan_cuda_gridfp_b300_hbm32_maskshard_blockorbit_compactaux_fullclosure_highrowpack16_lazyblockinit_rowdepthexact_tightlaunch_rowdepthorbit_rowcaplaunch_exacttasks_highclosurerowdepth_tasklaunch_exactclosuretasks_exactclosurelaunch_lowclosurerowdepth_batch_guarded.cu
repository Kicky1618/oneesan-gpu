// v0.24 experiment: v0.23 plus exact row-depth pruning inside LOW closure.
// Task mapping and host launch geometry stay v0.9; only structurally-zero source
// bodies are rejected before mainv loads. Saturated rows use the v0.23 path.
...
#define main oneesan_maskshard_v24_lowclosurerowdepth_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.24 LOW closure row-depth requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.24-highrowpack16-rowdepthexact-exacttasks-highclosureexact-lowclosurerowdepth-batch"
    ...
    return oneesan_maskshard_v24_lowclosurerowdepth_inner_main(argc, argv);
}
