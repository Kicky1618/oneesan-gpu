...
__constant__ uint32_t* D_PR_OWNER_CLOSURE_META;
__constant__ uint32_t* D_PR_OWNER_CLOSURE_SRC;
...
__constant__ uint32_t* D_HP_OWNER_CLOSURE_META;
__constant__ uint32_t* D_HP_OWNER_CLOSURE_SRC;
...
struct PreRankOrbitTablesHost {
    ...
    std::vector<uint32_t> owner_closure_meta,owner_closure_src;
...
static PreRankOrbitTablesHost build_prerank_orbit_tables(){
    ...
    for(int p=1;p<=L;++p)for(int hs=0;hs<=L+1;++hs)for(int c=0;c<3;++c){
        ...
        if(p==1){ constexpr uint32_t B=20,M=(1u<<B)-1u; std::vector<std::pair<uint32_t,uint32_t>> a; a.reserve(cs.count);
    ...
    // Fuse p>1 fast-closure contributions into the unique owner of each blocked state.
    z.owner_closure_meta.assign(z.owner_rec.size(),0u);
    { constexpr uint32_t B=20,M=(1u<<B)-1u,RB=18,RM=(1u<<RB)-1u;
      for(int p=2;p<=L;++p)for(int he=0;he<=HIGH_LUT_K+1;++he){
        uint32_t nb=G_FACTOR.low_all_off[he+1]-G_FACTOR.low_all_off[he];
        std::vector<std::vector<uint32_t>> srcs(nb);std::vector<uint8_t> seen(nb,0);
        for(int c=0;c<3;++c){int dc=(c==int(::L)?1:c==int(::R)?-1:0),hs=he+dc;if(hs<0||hs>L+1)continue;auto sel=z.closure_sel[sparse_key(p,hs,c)];for(uint32_t k=0;k<sel.count;++k){auto r=z.closure_rec[sel.off+k];uint32_t src=uint32_t(r)&M,dst=uint32_t(r>>B)&M;if(src>RM||dst>=nb)throw std::runtime_error("low owner-fused closure rank overflow");srcs[dst].push_back(src|(uint32_t(c)<<RB));}}
        for(int c=0;c<3;++c){int dc=(c==int(::L)?1:c==int(::R)?-1:0),hs=he+dc;if(hs<0||hs>L+1)continue;auto sel=z.owner_sel[sparse_key(p,hs,c)];for(uint32_t k=0;k<sel.count;++k){uint32_t ri=sel.off+k;auto r=z.owner_rec[ri];uint32_t dst=uint32_t(r>>(2*B))&M;if(dst>=nb||seen[dst])throw std::runtime_error("low owner-fused destination not unique");seen[dst]=1;auto const&v=srcs[dst];if(v.size()>15)throw std::runtime_error("low owner-fused indegree overflow");uint32_t off=uint32_t(z.owner_closure_src.size());if(off>0x0fffffffu)throw std::runtime_error("low owner-fused source offset overflow");z.owner_closure_src.insert(z.owner_closure_src.end(),v.begin(),v.end());z.owner_closure_meta[ri]=(off<<4)|uint32_t(v.size());}}
        for(uint8_t x:seen)if(!x)throw std::runtime_error("low owner-fused missing blocked owner");
      }
    }
    double mib=(double(z.owner_rec.size()+z.highrr_rec.size()+z.closure_inv_rec.size())*8.0+double(z.closure_inv_src.size()+z.owner_closure_meta.size()+z.owner_closure_src.size())*4.0)/(1<<20);
    std::cerr<<"pre-rank orbit owner="<<z.owner_rec.size()<<" closure_fast="<<z.closure_inv_src.size()<<" closure_inv="<<z.closure_inv_rec.size()<<" closure_highrr="<<z.highrr_rec.size()<<" device_mib="<<mib<<"\n";
    z.closure_rec.clear();z.closure_rec.shrink_to_fit();
    return z;
}
...
static HighPreRankTablesHost build_high_prerank_tables(){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t HM=(1u<<(2*H))-1u;HighPreRankTablesHost z;size_t keys=size_t(MAXW+1)*(MAXW+2)*3u;z.owner_sel.assign(keys,{0,0});z.closure_sel.assign(keys,{0,0});z.crossll_sel.assign(keys,{0,0});z.closure_inv_sel.assign(keys,{0,0});
    for(int p=L+1;p<=L+H;++p)for(int he=0;he<=H+1;++he)for(int c=0;c<3;++c){auto key=high_pr_key(p,he,c);SparseSel os{uint32_t(z.owner_rec.size()),0},cs{uint32_t(z.closure_rec.size()),0},xs{uint32_t(z.crossll_rec.size()),0};int dc=(c==int(::L)?1:c==int(::R)?-1:0),hs=he+dc;if(hs<0||hs>L+1){z.owner_sel[key]=os;z.closure_sel[key]=cs;z.crossll_sel[key]=xs;z.closure_inv_sel[key]={uint32_t(z.closure_inv_rec.size()),0};continue;}uint32_t beg=G_FACTOR.high_all_off[he],end=G_FACTOR.high_all_off[he+1];
        ...
        SparseSel is{uint32_t(z.closure_inv_rec.size()),0};
        is.count=0;z.owner_sel[key]=os;z.closure_sel[key]=cs;z.crossll_sel[key]=xs;z.closure_inv_sel[key]=is;
    }
    z.owner_closure_meta.assign(z.owner_rec.size(),0u);
    { constexpr uint32_t B=20,M=(1u<<B)-1u,RB=18,RM=(1u<<RB)-1u;
      for(int p=L+1;p<=L+H;++p)for(int hs=0;hs<=L+1;++hs){
        uint32_t nb=G_FACTOR.high_all_off[hs+1]-G_FACTOR.high_all_off[hs];
        std::vector<std::vector<uint32_t>> srcs(nb);std::vector<uint8_t> seen(nb,0);
        for(int c=0;c<3;++c){int dc=(c==int(::L)?1:c==int(::R)?-1:0),he=hs-dc;if(he<0||he>H+1)continue;auto sel=z.closure_sel[high_pr_key(p,he,c)];for(uint32_t k=0;k<sel.count;++k){auto r=z.closure_rec[sel.off+k];uint32_t src=uint32_t(r)&M,dst=uint32_t(r>>B)&M;if(src>RM||dst>=nb)throw std::runtime_error("high owner-fused closure rank overflow");srcs[dst].push_back(src|(uint32_t(c)<<RB));}}
        for(int c=0;c<3;++c){int dc=(c==int(::L)?1:c==int(::R)?-1:0),he=hs-dc;if(he<0||he>H+1)continue;auto sel=z.owner_sel[high_pr_key(p,he,c)];for(uint32_t k=0;k<sel.count;++k){uint32_t ri=sel.off+k;auto r=z.owner_rec[ri];uint32_t dst=uint32_t(r>>(2*B))&M;if(dst>=nb||seen[dst])throw std::runtime_error("high owner-fused destination not unique");seen[dst]=1;auto const&v=srcs[dst];if(v.size()>15)throw std::runtime_error("high owner-fused indegree overflow");uint32_t off=uint32_t(z.owner_closure_src.size());if(off>0x0fffffffu)throw std::runtime_error("high owner-fused source offset overflow");z.owner_closure_src.insert(z.owner_closure_src.end(),v.begin(),v.end());z.owner_closure_meta[ri]=(off<<4)|uint32_t(v.size());}}
        for(uint8_t x:seen)if(!x)throw std::runtime_error("high owner-fused missing blocked owner");
      }
    }
    double mib=(double(z.owner_rec.size()+z.crossll_rec.size()+z.closure_inv_rec.size())*8.0+double(z.closure_inv_src.size()+z.owner_closure_meta.size()+z.owner_closure_src.size())*4.0)/(1<<20);std::cerr<<"high pre-rank owner="<<z.owner_rec.size()<<" closure_fast="<<z.closure_inv_src.size()<<" closure_inv="<<z.closure_inv_rec.size()<<" closure_crossll="<<z.crossll_rec.size()<<" device_mib="<<mib<<"\n";z.closure_rec.clear();z.closure_rec.shrink_to_fit();return z;
}
...
__global__ void factor_prerank_owner_kernel(Count*mainv,Count*blockv,PreRankOrbitMap map,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){
        int bid=prerank_find_block(q,map);Code base=bid?map.end[bid-1]:0,loc=q-base;uint32_t sc=map.sel_count[bid],hr=uint32_t(loc/sc),k=uint32_t(loc-Code(hr)*sc),ri=map.sel_off[bid]+k;unsigned long long r=D_PR_OWNER_REC[ri];uint32_t lr=uint32_t(r)&M,mlr=uint32_t(r>>B)&M,blr=uint32_t(r>>(2*B))&M,ty=uint32_t(r>>(3*B))&3u;FBlock x=D_F_MAIN_BLOCKS[bid],bx=D_F_BLOCK_BLOCKS[x.he],dx=x;if(p==LOW_LUT_K){int dc=(ty==1?int(R):int(::L));dx=D_F_MAIN_BLOCKS[3*x.he+dc];}Code i=x.off+Code(hr)*x.stride+lr,j=dx.off+Code(hr)*dx.stride+mlr,dj=bx.off+Code(hr)*bx.stride+blr;Count c=mainv[i],d=blockv[dj],ca=0;uint32_t meta=D_PR_OWNER_CLOSURE_META[ri],co=meta>>4,cn=meta&15u;for(uint32_t z=0;z<cn;++z){uint32_t sr=D_PR_OWNER_CLOSURE_SRC[co+z],cc=sr>>18,sl=sr&((1u<<18)-1u);FBlock sx=D_F_MAIN_BLOCKS[3*x.he+int(cc)];Count v=mainv[sx.off+Code(hr)*sx.stride+sl];if(v)ca=add_mod_plain(ca,v);}
        if(ty==0){mainv[j]=add_mod_plain(mainv[j],c);mainv[i]=add_mod_plain(c,d);blockv[dj]=ca;}
        else{Count cc=mainv[j],pair=add_mod_plain(c,cc);mainv[i]=add_mod_plain(pair,d);if(p==1){mainv[j]=pair;blockv[dj]=0;}else blockv[dj]=add_mod_plain(c,ca);}
    }
}
...
__global__ void factor_high_prerank_owner_kernel(Count*mainv,Count*blockv,HighPreRankMap map){constexpr uint32_t B=20,M=(1u<<B)-1u;Code total=map.nblocks?map.end[map.nblocks-1]:0;for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){int bid=high_pr_find_block(q,map);Code base=bid?map.end[bid-1]:0,loc=q-base;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lc=x.stride,k=uint32_t(loc/lc),lr=uint32_t(loc-Code(k)*lc),ri=map.sel_off[bid]+k;unsigned long long r=D_HP_OWNER_REC[ri];uint32_t hr=uint32_t(r)&M,mhr=uint32_t(r>>B)&M,bhr=uint32_t(r>>(2*B))&M,ty=uint32_t(r>>(3*B))&3u,tc=uint32_t(r>>(3*B+2))&3u;int td=(tc==uint32_t(::L)?1:tc==uint32_t(R)?-1:0),the=int(x.hs)-td;FBlock dx=D_F_MAIN_BLOCKS[3*the+int(tc)],bx=D_F_BLOCK_BLOCKS[x.hs];Code i=x.off+Code(hr)*x.stride+lr,j=dx.off+Code(mhr)*dx.stride+lr,dj=bx.off+Code(bhr)*bx.stride+lr;Count c=mainv[i],d=blockv[dj],ca=0;uint32_t meta=D_HP_OWNER_CLOSURE_META[ri],co=meta>>4,cn=meta&15u;for(uint32_t z=0;z<cn;++z){uint32_t sr=D_HP_OWNER_CLOSURE_SRC[co+z],cc=sr>>18,sh=sr&((1u<<18)-1u);int sdc=(cc==uint32_t(::L)?1:cc==uint32_t(R)?-1:0),she=int(x.hs)-sdc;FBlock sx=D_F_MAIN_BLOCKS[3*she+int(cc)];Count v=mainv[sx.off+Code(sh)*sx.stride+lr];if(v)ca=add_mod_plain(ca,v);}if(ty==0){mainv[j]=add_mod_plain(mainv[j],c);mainv[i]=add_mod_plain(c,d);blockv[dj]=ca;}else{Count cc=mainv[j];mainv[i]=add_mod_plain(add_mod_plain(c,cc),d);blockv[dj]=add_mod_plain(c,ca);}}}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){auto launchN=[&](Code n,auto fn,const char*w){if(!n)return;int nb=int(std::min<Code>(65535,(n+threads-1)/threads));fn(nb);ck(cudaGetLastError(),w);};
            if(fixLow){auto om=make_high_prerank_map(fmb,p,0),xm=make_high_prerank_map(fmb,p,2);Code on=om.nblocks?om.end[om.nblocks-1]:0,xn=xm.nblocks?xm.end[xm.nblocks-1]:0;launchN(on,[&](int nb){factor_high_prerank_owner_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,om);},"factor high prerank owner+fastclosure");if(xn){HighPreRankMap empty{};launchN(xn,[&](int nb){factor_high_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,empty,xm);},"factor high prerank cross closure");}}
            else{auto om=make_prerank_orbit_map(fmb,p,0);Code on=om.nblocks?om.end[om.nblocks-1]:0;launchN(on,[&](int nb){factor_prerank_owner_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,om,p);},"factor prerank owner+fastclosure");auto sm=make_prerank_orbit_map(fmb,p,2);Code sn=sm.nblocks?sm.end[sm.nblocks-1]:0;if(p==1){auto im=make_prerank_orbit_map(fmb,p,3);Code in=im.nblocks?im.end[im.nblocks-1]:0;launchN(in+sn,[&](int nb){factor_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,im,sm,p);},"factor prerank p1 closure");}else launchN(sn,[&](int nb){factor_prerank_highrr_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,sm,p);},"factor prerank cross closure");}
        }
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t sparseBytes=0;uint64_t preRankBytes=uint64_t(G_PRERANK.owner_rec.size()+G_PRERANK.highrr_rec.size()+G_PRERANK.closure_inv_rec.size()+G_HPR.owner_rec.size()+G_HPR.crossll_rec.size()+G_HPR.closure_inv_rec.size())*8+uint64_t(G_PRERANK.closure_inv_src.size()+G_HPR.closure_inv_src.size()+G_PRERANK.owner_closure_meta.size()+G_PRERANK.owner_closure_src.size()+G_HPR.owner_closure_meta.size()+G_HPR.owner_closure_src.size())*4;uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+sparseBytes+preRankBytes;
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-fullprerank-orbit-ownerfused-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prO[d])cudaFree(prO[d]);if(prOCM[d])cudaFree(prOCM[d]);if(prOCS[d])cudaFree(prOCS[d]);if(prCI[d])cudaFree(prCI[d]);if(prCIS[d])cudaFree(prCIS[d]);if(prH[d])cudaFree(prH[d]);if(hpO[d])cudaFree(hpO[d]);if(hpOCM[d])cudaFree(hpOCM[d]);if(hpOCS[d])cudaFree(hpOCS[d]);if(hpCI[d])cudaFree(hpCI[d]);if(hpCIS[d])cudaFree(hpCIS[d]);if(hpX[d])cudaFree(hpX[d]);}
}
