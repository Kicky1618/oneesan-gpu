...
static void adaptive_refine_group(int W,const WindowPlan&wp,const PreparedGroup&pg,size_t target,Code mc,Code bc,int ng,std::vector<PreparedGroup>&out){
    size_t cb=size_t(2*pg.ms.size+2*pg.ds.size)*sizeof(Count);
    size_t mb=size_t(pg.ms.size)*sizeof(MateID);
    if(pg.use_mi || cb+mb<=target){out.push_back(pg);return;}
    auto cand=window_candidates(W,wp.p_hi,wp.p_lo);
    int q=-1;for(int x:cand)if(((pg.mf>>x)&1u)==0){q=x;break;}
    if(q<0){out.push_back(pg);return;}
    ...
    for(int one=0;one<2;++one){
        ...
        adaptive_refine_group(W,wp,ch,target,mc,bc,ng,out);
    }
}
...
int main(int argc,char**argv){
    ...
    for(int hi=W-1;hi>=1;){WindowPlan wp;bool found=false;for(int lo=std::max(1,hi-max_window+1);lo<=hi;++lo){auto t=plan_window(W,hi,lo,target);if(t.max_bytes&&t.max_bytes<=target){wp=std::move(t);found=true;break;}}if(!found){std::cerr<<"cannot fit window hi="<<hi<<"\n";return 4;}int k=wp.fixed_pos.size();int nj=1<<k;PreparedWindow pw;pw.wp=wp;pw.groups.reserve(nj+16);for(int g=0;g<nj;++g){auto pg=prepare_group(W,pw.wp,g,mc,bc,ng);adaptive_refine_group(W,pw.wp,pg,target,mc,bc,ng,pw.groups);}maxgroups=std::max(maxgroups,(int)pw.groups.size());std::sort(pw.groups.begin(),pw.groups.end(),[](auto const&a,auto const&b){return a.work>b.work;});schedule.push_back(std::move(pw));hi=wp.p_lo-1;}
