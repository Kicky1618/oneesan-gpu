...
struct GroupRuntimeCfg {
    ...
    uint8_t f_main_hint[256],f_block_hint[256];
    uint8_t f_main_shift,f_block_shift;
};
...
__device__ __forceinline__ int f_find_main_io(Code i){uint32_t k=uint32_t(i>>D_GRP.f_main_shift);if(k>255)k=255;int b=D_GRP.f_main_hint[k];while(b+1<D_F_MAIN_NBLOCKS&&i>=D_F_MAIN_BLOCKS[b].end)++b;return b;}
__device__ __forceinline__ int f_find_block_io(Code i){uint32_t k=uint32_t(i>>D_GRP.f_block_shift);if(k>255)k=255;int b=D_GRP.f_block_hint[k];while(b+1<D_F_BLOCK_NBLOCKS&&i>=D_F_BLOCK_BLOCKS[b].end)++b;return b;}
...
__device__ __forceinline__ Code factor_global_rank_main_index(Code i){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;int b=f_find_main_io(i);FBlock x=D_F_MAIN_BLOCKS[b];Code r=i-x.off;uint32_t hr=x.stride?uint32_t(r/x.stride):0,lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;uint32_t har,lar;
...
__device__ __forceinline__ Code factor_global_rank_block_index(Code i){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K,S=MAXW+2;int b=f_find_block_io(i);FBlock x=D_F_BLOCK_BLOCKS[b];Code r=i-x.off;uint32_t hr=x.stride?uint32_t(r/x.stride):0,lr=x.stride?uint32_t(r-Code(hr)*x.stride):0;uint32_t har,lar;
    if(D_F_FIX_LOW){har=hr;uint32_t lc=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];lar=D_F_LOW_DENSE_PACKED_RANK[lc]>>L;}
    else{uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];har=D_F_HIGH_PACKED_RANK[hc]>>H;lar=lr;}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    auto fill_hint=[](Code n,auto const& blocks,int nb,uint8_t* hint,uint8_t& shift){unsigned bw=0;Code z=n?n-1:0;while(z>>bw)++bw;shift=uint8_t(bw>8?bw-8:0);for(unsigned k=0;k<256;++k){Code q=Code(k)<<shift;int b=0;while(b+1<nb&&q>=blocks[b].end)++b;hint[k]=uint8_t(b);}};
    fill_hint(ms.size,fmb,fm,cfg.f_main_hint,cfg.f_main_shift);fill_hint(ds.size,fdb,fd,cfg.f_block_hint,cfg.f_block_shift);
