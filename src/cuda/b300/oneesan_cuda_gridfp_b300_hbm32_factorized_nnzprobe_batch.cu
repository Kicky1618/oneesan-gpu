...
__global__ void count_nnz_kernel(const Count* a, Code n, unsigned long long* out){
    unsigned long long local=0;
    for(Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;i<n;i+=stride) local += (a[i]!=0);
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        for(int row=0;row<W;++row){
            ...
            unsigned long long nnzm=0,nnzb=0;
            for(int d=0;d<ng;++d){
                ck(cudaSetDevice(d),"nnz set"); unsigned long long* dc=nullptr; ck(cudaMalloc(&dc,sizeof(*dc)),"nnz malloc");
                if(ml[d]){ck(cudaMemset(dc,0,sizeof(*dc)),"nnz zero");count_nnz_kernel<<<256,256>>>(mp[d],ml[d],dc);unsigned long long x=0;ck(cudaMemcpy(&x,dc,sizeof(x),cudaMemcpyDeviceToHost),"nnz main");nnzm+=x;}
                if(bl[d]){ck(cudaMemset(dc,0,sizeof(*dc)),"nnz zero");count_nnz_kernel<<<256,256>>>(bp[d],bl[d],dc);unsigned long long x=0;ck(cudaMemcpy(&x,dc,sizeof(x),cudaMemcpyDeviceToHost),"nnz block");nnzb+=x;}
                cudaFree(dc);
            }
            std::cerr<<"NNZ row="<<row+1<<" main="<<nnzm<<"/"<<mainN<<" ("<<(100.0*nnzm/mainN)<<"%) block="<<nnzb<<"/"<<blockN<<" ("<<(100.0*nnzb/blockN)<<"%)\n";
