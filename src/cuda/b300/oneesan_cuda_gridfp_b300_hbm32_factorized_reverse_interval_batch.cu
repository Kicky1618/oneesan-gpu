...
static Code factor_main_row_base_host(uint32_t hc,int he,int cv){
...
static Code factor_block_row_base_host(uint32_t hc,int he){
    constexpr int H=HIGH_LUT_K;
    uint32_t har=G_FACTOR.high_packed_rank[hc]>>H;
    return G_FACTOR.high_block_base[G_FACTOR.high_all_off[he]+har];
}
static void factor_add_peer_interval(std::vector<PeerInterval>&out,Code g,Code l,Code len,Code chunk,int ng){
    while(len){
        int owner=int(g/chunk);if(owner>=ng)owner=ng-1;
        Code shard0=Code(owner)*chunk;
        Code shardEnd=(owner+1<ng)?shard0+chunk:~Code(0);
        Code take=len;if(shardEnd!=~Code(0)&&g+take>shardEnd)take=shardEnd-g;
        Code remote=g-shard0;
        if(!out.empty()&&out.back().owner==(uint32_t)owner&&out.back().remote+out.back().len==remote&&out.back().local+out.back().len==l)
            out.back().len+=take;
        else out.push_back({remote,l,take,(uint32_t)owner,0});
        g+=take;l+=take;len-=take;
    }
}
static std::vector<PeerInterval> make_factor_peer_intervals_main(uint32_t mask,Code chunk,int ng){
    constexpr int H=HIGH_LUT_K,S=FactorTablesHost::STRIDE;
    std::vector<PeerInterval> out;auto bs=make_factor_main_blocks(false,mask);
    // HIGH occupancy is fixed, LOW is free.  Each fixed-HIGH row is exactly
    // one canonical interval over all LOW ranks.
    for(auto const&x:bs){if(!x.stride||x.end==x.off)continue;
        ...
        for(uint32_t hr=0;hr<hc;++hr){uint32_t hcode=G_FACTOR.high_mask_codes[hb+hr];
            Code g=factor_main_row_base_host(hcode,x.he,x.c),l=x.off+Code(hr)*x.stride;
            factor_add_peer_interval(out,g,l,x.stride,chunk,ng);
        }
    }
    return out;
}
static std::vector<PeerInterval> make_factor_peer_intervals_block(uint32_t mask,Code chunk,int ng){
    constexpr int S=FactorTablesHost::STRIDE;
    std::vector<PeerInterval> out;auto bs=make_factor_block_blocks(false,mask);
    for(auto const&x:bs){if(!x.stride||x.end==x.off)continue;
        uint32_t hb=G_FACTOR.high_mask_off[size_t(mask)*S+x.he];
        uint32_t hc=uint32_t((x.end-x.off)/x.stride);
        for(uint32_t hr=0;hr<hc;++hr){uint32_t hcode=G_FACTOR.high_mask_codes[hb+hr];
            Code g=factor_block_row_base_host(hcode,x.he),l=x.off+Code(hr)*x.stride;
            factor_add_peer_interval(out,g,l,x.stride,chunk,ng);
        }
    }
    return out;
...
__global__ void fill_main_mates_kernel(MateID*out,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride)out[i]=factor_unrank_main(i);}
__global__ void fill_block_mates_kernel(MateID*out,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride)out[i]=factor_unrank_block(i);}
...
static PreparedGroup prepare_group(int W,const WindowPlan&wp,int g,Code mc,Code bc,int ng){
    ...
    if(!fixLow){uint32_t fmask=(pg.mo>>(LOW_LUT_K+1))&((1u<<HIGH_LUT_K)-1u);pg.mi=make_factor_peer_intervals_main(fmask,mc,ng);pg.di=make_factor_peer_intervals_block(fmask,bc,ng);pg.use_mi=true;pg.use_di=true;}
    else{pg.mi=make_peer_intervals(pg.ms,mc,ng,pg.use_mi);pg.di=make_peer_intervals(pg.ds,bc,ng,pg.use_di);}
    return pg;
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    size_t countBytes=size_t(2*ms.size+2*ds.size)*sizeof(Count),mateBytes=size_t(ms.size)*sizeof(MateID),blockMateBytes=size_t(ds.size)*sizeof(MateID);bool useMate=(countBytes+mateBytes<=target);bool useBlockMate=(countBytes+(useMate?mateBytes:0)+blockMateBytes<=target);
    ...
    if(ms.size){if(pg.use_mi){interval_io_kernel<false,false><<<interval_blocks(pg.mi.size(),threads),threads>>>(c.dA,c.dIM,pg.mi.size());if(useMate)fill_main_mates_kernel<<<bm,threads>>>(c.dMate,ms.size);}else gather_main_kernel<<<bm,threads>>>(c.dA,useMate?c.dMate:nullptr,ms.size);}
    if(ds.size){if(pg.use_di){interval_io_kernel<true,false><<<interval_blocks(pg.di.size(),threads),threads>>>(c.dD,c.dID,pg.di.size());if(useBlockMate)fill_block_mates_kernel<<<bd,threads>>>(c.dBlockMate,ds.size);}else gather_block_kernel<<<bd,threads>>>(c.dD,useBlockMate?c.dBlockMate:nullptr,ds.size);}
