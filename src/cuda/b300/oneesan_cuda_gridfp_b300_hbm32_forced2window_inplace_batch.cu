...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dD=nullptr,*dE=nullptr;MateID*dMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0;uint64_t groups=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    size_t countBytes=size_t(ms.size+2*ds.size)*sizeof(Count),mateBytes=size_t(ms.size)*sizeof(MateID);bool useMate=!pg.use_mi&&(countBytes+mateBytes<=target);
    ...
    Count*cur=c.dA,*dcur=c.dD,*dnext=c.dE;
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(p>1){
            if(ds.size)ck(cudaMemset(dnext,0,size_t(ds.size)*sizeof(Count)),"clear next D inplace");
            if(ms.size)main_to_block_kernel<<<bm,threads>>>(cur,useMate?c.dMate:nullptr,ms.size,dnext,p);
            ck(cudaGetLastError(),"main to block inplace");ck(cudaDeviceSynchronize(),"main to block sync");
            if(ms.size)main_to_main_inplace_kernel<<<bm,threads>>>(cur,useMate?c.dMate:nullptr,ms.size,p);
            ck(cudaGetLastError(),"main to main inplace");ck(cudaDeviceSynchronize(),"main to main sync");
            if(ds.size)blocked_group_inplace_kernel<<<bd,threads>>>(dcur,ds.size,cur,p);
            ck(cudaGetLastError(),"block to main inplace");ck(cudaDeviceSynchronize(),"block to main sync");
            std::swap(dcur,dnext);
        }else{
            if(ms.size)main_p1_pairs_kernel<<<bm,threads>>>(cur,useMate?c.dMate:nullptr,ms.size);
            ck(cudaGetLastError(),"p1 pairs");ck(cudaDeviceSynchronize(),"p1 pairs sync");
            if(ms.size)main_p1_closure_kernel<<<bm,threads>>>(cur,useMate?c.dMate:nullptr,ms.size);
            ck(cudaGetLastError(),"p1 closure");ck(cudaDeviceSynchronize(),"p1 closure sync");
            if(ds.size)blocked_group_inplace_kernel<<<bd,threads>>>(dcur,ds.size,cur,p);
            ck(cudaGetLastError(),"p1 blocked");ck(cudaDeviceSynchronize(),"p1 blocked sync");
            if(ds.size)ck(cudaMemset(dcur,0,size_t(ds.size)*sizeof(Count)),"p1 clear blocked");
        }
    }
...
int main(int argc,char**argv){
    ...
    int threads=256,maxgroups=0;auto prep0=std::chrono::steady_clock::now();std::vector<PreparedWindow> schedule;
    {
        ...
        for(auto const& r:ranges){
            ...
            for(int g=0;g<nj;++g){auto pg=prepare_group(W,pw.wp,g,mc,bc,ng);size_t b=size_t(pg.ms.size+2*pg.ds.size)*sizeof(Count);mx=std::max(mx,b);pw.groups.push_back(std::move(pg));}
