// v0.21 experiment: v0.20 plus host HIGH closure grid sizing from the actual
// v0.11 hybrid row-pack warp-task count. Kernel mapping and row-depth predicate
// are unchanged; only the number of launched CTAs changes.
...
#define main oneesan_maskshard_v21_highclosuretasklaunch_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.21 HIGH closure task launch requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.21-highrowpack16-rowdepthexact-exacttasks-highclosurerowdepth-tasklaunch-batch"
        ...
        << " high_closure_row_depth=1 high_closure_task_launch=1"
    ...
    return oneesan_maskshard_v21_highclosuretasklaunch_inner_main(argc, argv);
}
