...
static double row6_gemm_fill_all_heights(const PackedRow6Lut&pref,int plen,const PackedRow6Lut&suff,int slen,Count*fullMain){
    ...
    int tileMiB=384;if(const char*e=std::getenv("GRIDFP_ROW6_GEMM_TILE_MIB"))tileMiB=std::max(32,std::atoi(e));
    for(int h=0;h<=6;++h){
...
static void init_direct_row6_lut_full_multi_gpu(int W,Count mod,int threads,Count**fullMain,const std::vector<Code>&mainLen,Code mainChunk,int ng){
    if(mod!=oneesan::row6mod::MOD)throw std::runtime_error("row6 prototype requires modulus 1000000007");Code dp[MAXW+1][MAXW+2]{};build_bounded_dp(6,dp);Code n=dp[W][1];constexpr int lo=TARGET_W/2,hi=TARGET_W-lo;auto t0=std::chrono::steady_clock::now();PackedRow6Lut pref,suff;std::thread tp([&]{pref=make_row6_prefix_lut_packed_levels(hi);});std::thread ts([&]{suff=make_row6_suffix_lut_packed_levels(lo);});tp.join();ts.join();double host_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();if(const char*e=std::getenv("GRIDFP_ROW6_STATS");e&&std::atoi(e))row6_print_lut_stats(pref,hi,suff,lo);if(const char*e=std::getenv("GRIDFP_ROW6_GEMM_BENCH");e&&std::atoi(e))row6_gemm_benchmark(pref,hi,suff,lo);
    bool gemmInit=false;if(const char*e=std::getenv("GRIDFP_ROW6_GEMM_INIT"))gemmInit=std::atoi(e)!=0;
    if(gemmInit){
