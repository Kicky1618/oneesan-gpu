...
static void init_direct_row6_lut_full_multi_gpu(int W,Count mod,int threads,Count**fullMain,const std::vector<Code>&mainLen,Code mainChunk,int ng){
    ...
    for(int dev=0;dev<ng;++dev)workers.emplace_back([&,dev]{ck(cudaSetDevice(dev),"r6 level set");ck(cudaMemcpyToSymbol(D_BOUND_DP,dp,sizeof(dp)),"r6 level dp");int cap=6;ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"r6 level cap");ck(cudaMemcpyToSymbol(D_R6_GPU_MAT_OFF,moff,sizeof(moff)),"r6 level mat off");Count*dmat=nullptr,*dbeta=nullptr;ck(cudaMalloc(&dmat,mat.size()*4),"r6 level mat");ck(cudaMalloc(&dbeta,beta.size()*4),"r6 level beta");ck(cudaMemcpy(dmat,mat.data(),mat.size()*4,cudaMemcpyHostToDevice),"r6 level mat cp");ck(cudaMemcpy(dbeta,beta.data(),beta.size()*4,cudaMemcpyHostToDevice),"r6 level beta cp");auto tb=std::chrono::steady_clock::now();
        auto build_pref=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*4),"r6 pref base");ck(cudaMemset(cur,0,size_t(182)*4),"r6 pref base zero");Count one=1;ck(cudaMemcpy(cur+initLocal,&one,4,cudaMemcpyHostToDevice),"r6 pref one");ck(cudaMalloc(&hp,1),"r6 pref h");int8_t h1=1;ck(cudaMemcpy(hp,&h1,1,cudaMemcpyHostToDevice),"r6 pref h1");for(int lev=0;lev<hi;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*4),"r6 pref next");ck(cudaMalloc(&hn,size_t(children)),"r6 pref hn");int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_prefix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);ck(cudaGetLastError(),"r6 pref level launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}cudaFree(hp);return std::pair<Count*,Code>(cur,parents);};
        auto build_suff=[&](){Code parents=1;Count*cur=nullptr;int8_t*hp=nullptr;ck(cudaMalloc(&cur,size_t(182)*4),"r6 suff base");ck(cudaMemset(cur,0,size_t(182)*4),"r6 suff base zero");ck(cudaMemcpy(cur,dbeta,size_t(r6_block_size(0))*4,cudaMemcpyDeviceToDevice),"r6 suff beta");ck(cudaMalloc(&hp,1),"r6 suff h");int8_t h0=0;ck(cudaMemcpy(hp,&h0,1,cudaMemcpyHostToDevice),"r6 suff h0");for(int lev=0;lev<lo;++lev){Code children=parents*3;Count*nxt=nullptr;int8_t*hn=nullptr;ck(cudaMalloc(&nxt,size_t(182)*children*4),"r6 suff next");ck(cudaMalloc(&hn,size_t(children)),"r6 suff hn");int bl=int(std::min<Code>(65535,(children+rt-1)/rt));r6_suffix_level_kernel<<<std::max(1,bl),rt>>>(parents,cur,hp,nxt,hn,dmat);ck(cudaGetLastError(),"r6 suff level launch");cudaFree(cur);cudaFree(hp);cur=nxt;hp=hn;parents=children;}cudaFree(hp);return std::pair<Count*,Code>(cur,parents);};
        auto P=build_pref(),S=build_suff();ck(cudaDeviceSynchronize(),"r6 level tables");table_s[dev]=std::chrono::duration<double>(std::chrono::steady_clock::now()-tb).count();Code cnt=cut[dev+1]-cut[dev],base=Code(dev)*mainChunk;int blocks=lanes?65535:int(std::min<Code>(65535,(cnt+threads-1)/threads));auto ib=std::chrono::steady_clock::now();if(cnt){if(lanes==16)bounded_fill_row6_dense_group_range_kernel<16><<<std::max(1,blocks),threads>>>(fullMain[dev],cut[dev],cut[dev+1],base,P.first,P.second,S.first,S.second);else if(lanes==8)bounded_fill_row6_dense_group_range_kernel<8><<<std::max(1,blocks),threads>>>(fullMain[dev],cut[dev],cut[dev+1],base,P.first,P.second,S.first,S.second);else if(lanes==4)bounded_fill_row6_dense_group_range_kernel<4><<<std::max(1,blocks),threads>>>(fullMain[dev],cut[dev],cut[dev+1],base,P.first,P.second,S.first,S.second);else bounded_fill_row6_dense_range_kernel<<<std::max(1,blocks),threads>>>(fullMain[dev],cut[dev],cut[dev+1],base,P.first,P.second,S.first,S.second);}ck(cudaDeviceSynchronize(),"r6 level init");init_s[dev]=std::chrono::duration<double>(std::chrono::steady_clock::now()-ib).count();cudaFree(P.first);cudaFree(S.first);cudaFree(dmat);cudaFree(dbeta);});for(auto&t:workers)t.join();double gw=std::chrono::duration<double>(std::chrono::steady_clock::now()-g0).count(),tm=*std::max_element(table_s.begin(),table_s.end()),im=*std::max_element(init_s.begin(),init_s.end());Code pc=1,sc=1;for(int i=0;i<hi;++i)pc*=3;for(int i=0;i<lo;++i)sc*=3;std::cerr<<"direct row6 GPU-level states="<<n<<" lut_host_s="<<host_s<<" table_kernel_max_s="<<tm<<" init_kernel_max_s="<<im<<" gpu_phase_wall_s="<<gw<<" pref_mib="<<(size_t(182)*pc*4>>20)<<" suff_mib="<<(size_t(182)*sc*4>>20)<<" mat_kib="<<(mat.size()*4>>10)<<" level_threads="<<rt<<"\n";
}
