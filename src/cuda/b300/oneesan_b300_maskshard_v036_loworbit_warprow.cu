// v0.36 experiment: v0.35 synchronization plus warp-row LOW orbit mapping.
// One warp processes one active HIGH row x up to 32 active LOW ranks; state set
// and state-sized host grid are unchanged.
...
#define main oneesan_maskshard_v36_loworbitwarprow_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.36 LOW orbit warp-row requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.36-loworbit-warprow-batch"
        << " low_orbit_warp_row_tasks=1 low_group_sync=1"
        << " low_orbit_row_depth_compact_launch=1 guarded_hbm=1\n";
    const int rc = oneesan_maskshard_v36_loworbitwarprow_inner_main(argc, argv);
    ...
    if (rc == 0 && skipped != executed * skips_per_group) {
        std::cerr << "v0.36 LOW group sync count mismatch\n";
        return 4;
    }
    return rc;
}
