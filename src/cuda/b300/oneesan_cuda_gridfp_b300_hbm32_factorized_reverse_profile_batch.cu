...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    ck(cudaGetLastError(),"doubleD gather");ck(cudaDeviceSynchronize(),"doubleD gather sync");c.gather_s+=std::chrono::duration<double>(std::chrono::steady_clock::now()-tg0).count();auto tt0=std::chrono::steady_clock::now();
    ...
    ck(cudaStreamSynchronize(c.sMain),"main sync");ck(cudaStreamSynchronize(c.sBlock),"block sync");c.transition_s+=std::chrono::duration<double>(std::chrono::steady_clock::now()-tt0).count();auto ts0=std::chrono::steady_clock::now();
    ...
    ck(cudaGetLastError(),"doubleD scatter");ck(cudaDeviceSynchronize(),"group sync");c.scatter_s+=std::chrono::duration<double>(std::chrono::steady_clock::now()-ts0).count();c.groups++;c.active+=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
}
