...
struct PreparedGroup{
    ...
    Code work=0;
    mutable cudaGraphExec_t transition_graph[MAXGPU]{};
};
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        ...
        if(pg.transition_graph[c.dev]){
            ck(cudaGraphLaunch(pg.transition_graph[c.dev],c.sMain),"transition graph launch");
        }else{
            cudaGraph_t graph=nullptr;ck(cudaStreamBeginCapture(c.sMain,cudaStreamCaptureModeThreadLocal),"transition graph begin capture");enqueue_transition();ck(cudaStreamEndCapture(c.sMain,&graph),"transition graph end capture");ck(cudaGraphInstantiate(&pg.transition_graph[c.dev],graph,0),"transition graph instantiate");ck(cudaGraphDestroy(graph),"transition graph destroy");ck(cudaGraphLaunch(pg.transition_graph[c.dev],c.sMain),"transition graph first launch");
        }
...
int main(int argc,char**argv){
    ...
    Code graphMaxMain=0,graphMaxBlock=0;for(auto const&pw:schedule)for(auto const&pg:pw.groups){graphMaxMain=std::max(graphMaxMain,pg.ms.size);graphMaxBlock=std::max(graphMaxBlock,pg.ds.size);}for(auto&c:ctx)c.ensure(graphMaxMain,graphMaxBlock,false,0,0,true);
