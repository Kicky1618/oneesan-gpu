...
__global__ void r6_build_prefix_packed_kernel(Code codes,int len,Count*out,const uint32_t*idx,const Count*mat,int initLocal){
    extern __shared__ Count sh_r6gpu[];int tid=threadIdx.x,bd=blockDim.x;Count*A=sh_r6gpu,*B=sh_r6gpu+182*bd;
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<codes;q+=st){uint32_t oo=idx[q];if(oo==0xffffffffu)continue;
        for(int j=0;j<182;++j)A[j*bd+tid]=0;A[initLocal*bd+tid]=1;Count*v=A,*w=B;Code x=q,p3=1;for(int i=1;i<len;++i)p3*=3;int h=1;
        for(int step=0;step<len;++step){int sym=int(x/p3);x%=p3;if(p3>1)p3/=3;int h2=h+r6_gpu_delta(sym),ns=r6_block_size(h),nd=r6_block_size(h2);const Count*M=mat+D_R6_GPU_MAT_OFF[sym][h];
            for(int d=0;d<nd;++d){Count acc=0;for(int a=0;a<ns;++a){Count u=v[a*bd+tid],c=M[a*nd+d];if(u&&c)acc=r3_add_mod(acc,r3_mul_mod(u,c));}w[d*bd+tid]=acc;}Count*t=v;v=w;w=t;h=h2;}
        int n=r6_block_size(h);for(int j=0;j<n;++j)out[Code(oo)+j]=v[j*bd+tid];}
}
__global__ void r6_build_suffix_packed_kernel(Code codes,int len,Count*out,const uint32_t*idx,const Count*mat,const Count*beta){
    extern __shared__ Count sh_r6gpu[];int tid=threadIdx.x,bd=blockDim.x;Count*A=sh_r6gpu,*B=sh_r6gpu+182*bd;
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<codes;q+=st){uint32_t oo=idx[q];if(oo==0xffffffffu)continue;
        int n0=r6_block_size(0);for(int j=0;j<n0;++j)A[j*bd+tid]=beta[j];for(int j=n0;j<182;++j)A[j*bd+tid]=0;Count*v=A,*w=B;Code x=q;int h=0;
        for(int step=0;step<len;++step){int sym=int(x%3);x/=3;int hsrc=h-r6_gpu_delta(sym),ns=r6_block_size(hsrc),nd=r6_block_size(h);const Count*M=mat+D_R6_GPU_MAT_OFF[sym][hsrc];
            for(int a=0;a<ns;++a){Count acc=0;const Count*row=M+a*nd;for(int d=0;d<nd;++d){Count u=v[d*bd+tid],c=row[d];if(u&&c)acc=r3_add_mod(acc,r3_mul_mod(c,u));}w[a*bd+tid]=acc;}Count*t=v;v=w;w=t;h=hsrc;}
        int n=r6_block_size(h);for(int j=0;j<n;++j)out[Code(oo)+j]=v[j*bd+tid];}
}
...
static void init_direct_row6_lut_full_multi_gpu(int W,Count mod,int threads,Count**fullMain,const std::vector<Code>&mainLen,Code mainChunk,int ng){
    ...
    if(mod!=MOD)throw std::runtime_error("row6 GPU-LUT prototype requires modulus 1000000007");
    Code dp[MAXW+1][MAXW+2]{};build_bounded_dp(6,dp);Code n=dp[W][1];constexpr int lo=TARGET_W/2,hi=TARGET_W-lo;
    auto t0=std::chrono::steady_clock::now();auto const&lay=row6_layout();
    std::vector<Count> mat,beta(r6_block_size(0));uint32_t moff[3][7]{};
    const uint32_t* offs[3]={OFF_N,OFF_R,OFF_L};const Tr* trs[3]={TR_N,TR_R,TR_L};
    for(int sym=0;sym<3;++sym)for(int h=0;h<=6;++h){int h2=h+row6_delta(sym);if(h2<0||h2>6){moff[sym][h]=0;continue;}int ns=r6_block_size(h),nd=r6_block_size(h2);moff[sym][h]=uint32_t(mat.size());mat.resize(mat.size()+size_t(ns)*nd,0);Count*M=mat.data()+moff[sym][h];
        for(int a=0;a<ns;++a){int gi=lay.bidx[h][a];for(uint32_t k=offs[sym][gi];k<offs[sym][gi+1];++k){int gd=trs[sym][k].dst;if(HEIGHT[gd]!=h2)throw std::runtime_error("row6 transition violates height grading");int d=lay.li[gd];M[size_t(a)*nd+d]=row6_add_host(M[size_t(a)*nd+d],trs[sym][k].coeff);}}}
    for(int i=0;i<DIM;++i)if(BETA[i]){if(HEIGHT[i]!=0)throw std::runtime_error("row6 beta height");beta[lay.li[i]]=row6_add_host(beta[lay.li[i]],BETA[i]);}
    int initLocal=lay.li[0];
    auto make_idx=[&](int len,bool prefix){size_t codes=1;for(int i=0;i<len;++i)codes*=3;std::vector<uint32_t> idx(codes,0xffffffffu);uint64_t total=0;for(size_t c=0;c<codes;++c){int h=prefix?row6_prefix_code_height(c,len):row6_suffix_code_height(c,len);if(h>=0){if(total>0xffffffffULL)throw std::runtime_error("row6 GPU packed offset overflow");idx[c]=uint32_t(total);total+=r6_block_size(h);}}if(total>0xffffffffULL)throw std::runtime_error("row6 GPU packed LUT exceeds uint32 offset");return std::pair<std::vector<uint32_t>,uint64_t>(std::move(idx),total);};
    std::pair<std::vector<uint32_t>,uint64_t> pi,si;std::thread tp([&]{pi=make_idx(hi,true);});std::thread ts([&]{si=make_idx(lo,false);});tp.join();ts.join();
    double host_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
    std::vector<Code>cut(ng+1);cut[0]=0;cut[ng]=n;for(int d=1;d<ng;++d)cut[d]=row5_lower_bound_full_rank(n,Code(d)*mainChunk,W,dp);int lanes=8;if(const char*e=std::getenv("GRIDFP_ROW6_LANES"))lanes=std::atoi(e);int rt=16;if(const char*e=std::getenv("GRIDFP_ROW6_LUT_THREADS"))rt=std::max(1,std::atoi(e));
    std::vector<double>gpu_s(ng),table_s(ng);std::vector<std::thread>workers;auto g0=std::chrono::steady_clock::now();
    for(int dev=0;dev<ng;++dev)workers.emplace_back([&,dev]{ck(cudaSetDevice(dev),"row6 gpu lut set");ck(cudaMemcpyToSymbol(D_BOUND_DP,dp,sizeof(dp)),"row6 dp");int cap=6;ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"row6 cap");ck(cudaMemcpyToSymbol(D_R6_GPU_MAT_OFF,moff,sizeof(moff)),"row6 matrix offsets");
        Count *dmat=nullptr,*dbeta=nullptr,*dpref=nullptr,*dsuff=nullptr;uint32_t *dpidx=nullptr,*dsidx=nullptr;ck(cudaMalloc(&dmat,mat.size()*sizeof(Count)),"r6 mat");ck(cudaMalloc(&dbeta,beta.size()*sizeof(Count)),"r6 beta");ck(cudaMalloc(&dpidx,pi.first.size()*sizeof(uint32_t)),"r6 pidx");ck(cudaMalloc(&dsidx,si.first.size()*sizeof(uint32_t)),"r6 sidx");ck(cudaMalloc(&dpref,size_t(pi.second)*sizeof(Count)),"r6 pref");ck(cudaMalloc(&dsuff,size_t(si.second)*sizeof(Count)),"r6 suff");
        ck(cudaMemcpy(dmat,mat.data(),mat.size()*sizeof(Count),cudaMemcpyHostToDevice),"r6 mat cp");ck(cudaMemcpy(dbeta,beta.data(),beta.size()*sizeof(Count),cudaMemcpyHostToDevice),"r6 beta cp");ck(cudaMemcpy(dpidx,pi.first.data(),pi.first.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"r6 pidx cp");ck(cudaMemcpy(dsidx,si.first.data(),si.first.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"r6 sidx cp");
        auto tb=std::chrono::steady_clock::now();int pb=int(std::min<size_t>(65535,(pi.first.size()+rt-1)/rt)),sb=int(std::min<size_t>(65535,(si.first.size()+rt-1)/rt));size_t sh=size_t(2)*182*rt*sizeof(Count);r6_build_prefix_packed_kernel<<<std::max(1,pb),rt,sh>>>(pi.first.size(),hi,dpref,dpidx,dmat,initLocal);r6_build_suffix_packed_kernel<<<std::max(1,sb),rt,sh>>>(si.first.size(),lo,dsuff,dsidx,dmat,dbeta);ck(cudaDeviceSynchronize(),"r6 gpu tables");table_s[dev]=std::chrono::duration<double>(std::chrono::steady_clock::now()-tb).count();
        ck(cudaMemcpyToSymbol(D_R6_PREF,&dpref,sizeof(dpref)),"r6 pref ptr");ck(cudaMemcpyToSymbol(D_R6_SUFF,&dsuff,sizeof(dsuff)),"r6 suff ptr");ck(cudaMemcpyToSymbol(D_R6_PREF_IDX,&dpidx,sizeof(dpidx)),"r6 pidx ptr");ck(cudaMemcpyToSymbol(D_R6_SUFF_IDX,&dsidx,sizeof(dsidx)),"r6 sidx ptr");Code cnt=cut[dev+1]-cut[dev],base=Code(dev)*mainChunk;int blocks=lanes?65535:int(std::min<Code>(65535,(cnt+threads-1)/threads));auto q0=std::chrono::steady_clock::now();if(cnt){if(lanes==16)bounded_fill_row6_full_group_range_kernel<16><<<std::max(1,blocks),threads>>>(fullMain[dev],cut[dev],cut[dev+1],base);else if(lanes==8)bounded_fill_row6_full_group_range_kernel<8><<<std::max(1,blocks),threads>>>(fullMain[dev],cut[dev],cut[dev+1],base);else if(lanes==4)bounded_fill_row6_full_group_range_kernel<4><<<std::max(1,blocks),threads>>>(fullMain[dev],cut[dev],cut[dev+1],base);else bounded_fill_row6_full_range_kernel<<<std::max(1,blocks),threads>>>(fullMain[dev],cut[dev],cut[dev+1],base);}ck(cudaDeviceSynchronize(),"r6 init");gpu_s[dev]=std::chrono::duration<double>(std::chrono::steady_clock::now()-q0).count();cudaFree(dmat);cudaFree(dbeta);cudaFree(dpref);cudaFree(dsuff);cudaFree(dpidx);cudaFree(dsidx);});
    for(auto&t:workers)t.join();double gw=std::chrono::duration<double>(std::chrono::steady_clock::now()-g0).count(),gm=*std::max_element(gpu_s.begin(),gpu_s.end()),tm=*std::max_element(table_s.begin(),table_s.end());
    std::cerr<<"direct row6 GPU-LUT states="<<n<<" lut_host_s="<<host_s<<" table_kernel_max_s="<<tm<<" init_kernel_max_s="<<gm<<" gpu_phase_wall_s="<<gw<<" pref_mib="<<(pi.second*4>>20)<<" suff_mib="<<(si.second*4>>20)<<" pidx_mib="<<(pi.first.size()*4>>20)<<" sidx_mib="<<(si.first.size()*4>>20)<<" mat_kib="<<(mat.size()*4>>10)<<" lut_threads="<<rt<<"\n";
}
