// v0.32 experiment: v0.31 exact LOW orbit launch plus warp-shared compact
// FBlock decode.  The task set and host launch geometry are unchanged; only the
// unsaturated compact-task -> FBlock decode path is replaced.
...
#define main oneesan_maskshard_v32_loworbitwarpdecode_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.32 warp LOW orbit decode requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.32-loworbit-warpdecode-batch"
    ...
    return oneesan_maskshard_v32_loworbitwarpdecode_inner_main(argc, argv);
}
