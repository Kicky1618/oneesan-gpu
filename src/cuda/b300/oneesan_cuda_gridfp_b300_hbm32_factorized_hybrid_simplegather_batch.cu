...
__global__ void reverse_block_simple_kernel(const Count*in,Code n,Count*out,Code dn,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<dn;i+=stride){
        ...
        if(low==oneesan::gridfp::R||low==oneesan::gridfp::L){
            MateID c=oneesan::gridfp::minsert(b,p,oneesan::gridfp::N);Count v=in[factor_rank_main(c)];if(v)acc=v;
        }else if(low==oneesan::gridfp::N){
            MateID u=oneesan::gridfp::minsert(b,p-1,oneesan::gridfp::N);
            if(height_before_rank_pos<TARGET_W>(u,p)>0){MateID c=oneesan::gridfp::msetpair(u,p,oneesan::gridfp::RL);Count v=in[factor_rank_main(c)];if(v)acc=v;}
        }
        out[i]=acc;
    }
}
__global__ void forward_block_closure_only_kernel(const Count*in,const MateID*mates,Code n,Count*out,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(p>1){
            ...
            if(ds.size)reverse_block_simple_kernel<<<bd,threads,0,c.sBlock>>>(cur,ms.size,dnext,ds.size,p);
            if(ms.size&&ds.size)forward_block_closure_only_kernel<<<bm,threads,0,c.sBlock>>>(cur,useMate?c.dMate:nullptr,ms.size,dnext,p);
        }else{
