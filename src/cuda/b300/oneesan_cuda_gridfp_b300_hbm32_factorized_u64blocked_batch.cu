...
#pragma unroll
    for(int j=0;j<18;++j)w[j*bd+tid]=0;
...
#pragma unroll
    for(int i=0;i<18;++i){Count x=v[i*bd+tid];if(!x)continue;int e=off[i+1];for(int t=off[i];t<e;++t){auto q=tr[t];Count y=pm_mul_small_signed(x,q.coeff);Count* p=&w[int(q.dst)*bd+tid];*p=r3_add(*p,y);}}
}
...
#pragma unroll
        ...
        Count ans=0;
#pragma unroll
        for(int j=0;j<18;++j)if(D_R3_BETA[j])ans=r3_add(ans,pm_mul_small_signed(v[j*bd+tid],D_R3_BETA[j]));
        ans=pm_mul(ans,D_R3_UNSCALE);global_store_main(grank,ans);
    }
}
...
__global__ void forward_block_u64_kernel(const Count*in,const MateID*mates,Code n,unsigned long long*out_block,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        Count c=in[i];if(!c)continue;MateID m=mates?mates[i]:factor_unrank_main(i);auto pr=oneesan::gridfp::mpair(m,p);auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);
        if(z.valid&&z.blocked){Code j=factor_rank_block(z.mate);if(pr==oneesan::gridfp::NR||pr==oneesan::gridfp::NL)out_block[j]=c;else atomicAdd(out_block+j,(unsigned long long)c);}
    }
}
__device__ __forceinline__ Count reduce_u64_mod(unsigned long long x){
    unsigned long long mod=D_MOD,delta=0x100000000ULL-mod;
    if(delta>0&&delta<(1ULL<<20)){x=(unsigned long long)(uint32_t)x+(x>>32)*delta;if(x>=mod)x-=mod;return Count(x);}
    return Count(x%mod);
}
__global__ void reduce_block_u64_kernel(const unsigned long long*in,Count*out,Code n){for(Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;i<n;i+=st)out[i]=reduce_u64_mod(in[i]);}
...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;unsigned long long*dAcc=nullptr;Code capAcc=0;MateID*dMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0;uint64_t groups=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(p>1){
            ...
            if(ds.size)ck(cudaMemsetAsync(c.dAcc,0,size_t(ds.size)*sizeof(unsigned long long),c.sBlock),"clear u64 blocked");
            if(ms.size&&ds.size)forward_block_u64_kernel<<<bm,threads,0,c.sBlock>>>(cur,useMate?c.dMate:nullptr,ms.size,c.dAcc,p);
            if(ds.size)reduce_block_u64_kernel<<<bd,threads,0,c.sBlock>>>(c.dAcc,dnext,ds.size);
        }else{
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        if(directRow3){prefixK=3;Code bdp[MAXW+1][MAXW+2]{};build_bounded_dp(3,bdp);Code total=bdp[W][1];int cap=3;auto powmod=[&](uint64_t a,uint64_t e){uint64_t r=1;while(e){if(e&1)r=(unsigned __int128)r*a%mod;a=(unsigned __int128)a*a%mod;e>>=1;}return r;};Count scale=Count(powmod(6,W)),inv=Count(powmod(scale,uint64_t(mod)-2));for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"row3 direct set");ck(cudaMemcpyToSymbol(D_BOUND_DP,bdp,sizeof(bdp)),"row3 direct dp");ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"row3 direct cap");ck(cudaMemcpyToSymbol(D_R3_OFF_N,oneesan::row3auto::OFF_N,sizeof(oneesan::row3auto::OFF_N)),"r3 offn");ck(cudaMemcpyToSymbol(D_R3_OFF_R,oneesan::row3auto::OFF_R,sizeof(oneesan::row3auto::OFF_R)),"r3 offr");ck(cudaMemcpyToSymbol(D_R3_OFF_L,oneesan::row3auto::OFF_L,sizeof(oneesan::row3auto::OFF_L)),"r3 offl");ck(cudaMemcpyToSymbol(D_R3_TR_N,oneesan::row3auto::TR_N,sizeof(oneesan::row3auto::TR_N)),"r3 trn");ck(cudaMemcpyToSymbol(D_R3_TR_R,oneesan::row3auto::TR_R,sizeof(oneesan::row3auto::TR_R)),"r3 trr");ck(cudaMemcpyToSymbol(D_R3_TR_L,oneesan::row3auto::TR_L,sizeof(oneesan::row3auto::TR_L)),"r3 trl");ck(cudaMemcpyToSymbol(D_R3_BETA,oneesan::row3auto::BETA,sizeof(oneesan::row3auto::BETA)),"r3 beta");ck(cudaMemcpyToSymbol(D_R3_UNSCALE,&inv,sizeof(inv)),"r3 inv");Code mine=(total+ng-1-d)/ng;int r3threads=128;if(const char*e=std::getenv("GRIDFP_R3_THREADS"))r3threads=std::atoi(e);int blocks=int(std::min<Code>(65535,(mine+r3threads-1)/r3threads));if(mine)init_after_three_rows_kernel<<<std::max(1,blocks),r3threads,size_t(36)*r3threads*sizeof(Count)>>>(total,d,ng);}for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"row3 direct sync set");ck(cudaDeviceSynchronize(),"row3 direct sync");}std::cerr<<"direct row3 states="<<total<<"\n";}
