...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;MateID*dMate=nullptr,*dBlockMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0,gather=0,transition=0,scatter=0;uint64_t groups=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    c.gather+=std::chrono::duration<double>(std::chrono::steady_clock::now()-tg0).count();
    ...
    c.transition+=std::chrono::duration<double>(std::chrono::steady_clock::now()-tt0).count();
    ...
    ck(cudaGetLastError(),"doubleD scatter");ck(cudaDeviceSynchronize(),"group sync");c.scatter+=std::chrono::duration<double>(std::chrono::steady_clock::now()-ts0).count();c.groups++;c.active+=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
}
