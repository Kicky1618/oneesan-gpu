...
#error "compact LOW mask-batch kernels require compact dynamic config"
#endif

struct MaskShardLowBatchOrbitSharedCompact {
...
struct MaskShardLowBatchClosureSharedCompact {
    FBlock main_blocks[64];
    FBlock block_blocks[32];
...
__device__ __forceinline__ int maskshard_low_batch_find_prefix_compact(
...
__global__ void maskshard_low_maskbatch_orbit_kernel(
    ...
    __shared__ MaskShardLowBatchOrbitSharedCompact s;
    ...
    for (; warp_task < total; warp_task += warp_step) {
        ...
        if (lane == 0) {
            dbid = maskshard_low_batch_find_prefix_compact(
...
__global__ void maskshard_low_maskbatch_closure_kernel(
    ...
    __shared__ MaskShardLowBatchClosureSharedCompact s;
    ...
    for (int i = int(threadIdx.x); i < 65; i += int(blockDim.x)) {
        ...
        const std::uint32_t a = D_MS_LOW_CLOSURE_BLOCK_OFF[
            std::size_t(pi) * 65 + std::size_t(i)];
        s.begin[i] = a;
        if (i < main_nb) {
            const std::uint32_t z = D_MS_LOW_CLOSURE_BLOCK_OFF[
                std::size_t(pi) * 65 + std::size_t(i + 1)];
            s.selected[i] = saturated
                ? z - a
                : D_MS_LOW_CLOSURE_COMPACT_ACTIVE_COUNT[
                    (std::size_t(pi) * 65 + std::size_t(i)) * CAP_STRIDE
                    + std::size_t(cap)];
        } else {
            s.selected[i] = 0;
        }
    ...
    for (; task < total; task += task_step) {
        ...
        if (lane == 0) {
            bid = maskshard_low_batch_find_prefix_compact(s.prefix, task, main_nb);
