...
__global__ void r3_prefix_expand_kernel(const Count* parent,Code np,Count* child,Code nc){
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<nc;q+=st){
        int sym=int(q&3ULL);Code par=q>>2;
#pragma unroll
        for(int j=0;j<18;++j){
            if(sym==3){child[Code(j)*nc+q]=0;continue;}
            int64_t acc=0;
#pragma unroll
            for(int k=0;k<6;++k){int u=j*6+k;uint8_t si=D_R3I_SRC[sym][u];if(si==255)break;acc+=int64_t(parent[Code(si)*np+par])*int64_t(D_R3I_CO[sym][u]);}
            child[Code(j)*nc+q]=pm_reduce_i64(acc);
        }
    }
}
__global__ void r3_suffix_expand_kernel(const Count* parent,Code np,Count* child,Code nc){
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<nc;q+=st){
        int sym=int(q&3ULL);Code par=q>>2;
        const uint16_t* off=sym==0?D_R3_OFF_N:(sym==1?D_R3_OFF_R:D_R3_OFF_L);
        const oneesan::row3auto::Tr* tr=sym==0?D_R3_TR_N:(sym==1?D_R3_TR_R:D_R3_TR_L);
#pragma unroll
        for(int i=0;i<18;++i){
            if(sym==3){child[Code(i)*nc+q]=0;continue;}
            int64_t acc=0;int e=off[i+1];
            for(int t=off[i];t<e;++t){auto z=tr[t];acc+=int64_t(parent[Code(z.dst)*np+par])*int64_t(z.coeff);}
            child[Code(i)*nc+q]=pm_reduce_i64(acc);
        }
    }
}
__global__ void init_after_three_rows_halflut_kernel(Code total,int shard,int ng,const Count* pref,Code pn,const Count* suff,Code sn,int hlen){
    Code tid=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;
    int llen=TARGET_W-hlen;
    for(Code q=tid;;q+=st){Code brank=Code(shard)+q*Code(ng);if(brank>=total)break;Code r=brank,grank=0,hcode=0,lrev=0;int h=1,idx=0;
#pragma unroll
        for(int pos=TARGET_W-1;pos>=0;--pos,++idx){MateValue z=N;Code c=D_BOUND_DP[pos][h];if(r<c)z=N;else{r-=c;if(h>0){c=D_BOUND_DP[pos][h-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}if(z==R){grank+=D_FULL_DP[pos][h];--h;}else if(z==L){grank+=D_FULL_DP[pos][h]+(h>0?D_FULL_DP[pos][h-1]:0);++h;}if(idx<hlen)hcode=(hcode<<2)|Code(z);else lrev|=Code(z)<<(2*(idx-hlen));}
        Count ans=0;
#pragma unroll
        for(int j=0;j<18;++j){Count a=pref[Code(j)*pn+hcode],b=suff[Code(j)*sn+lrev];if(a&&b)ans=r3_add(ans,pm_mul(a,b));}
...
static Count* build_r3_prefix_lut(int len,int threads){
    Count*cur=nullptr;ck(cudaMalloc(&cur,18*sizeof(Count)),"r3 pref base");Count base[18]{};base[0]=1;for(int j=0;j<18;++j)ck(cudaMemcpy(cur+Code(j),base+j,sizeof(Count),cudaMemcpyHostToDevice),"r3 pref base copy");Code np=1;
    for(int d=1;d<=len;++d){Code nc=np*4;Count*nxt=nullptr;ck(cudaMalloc(&nxt,size_t(18)*nc*sizeof(Count)),"r3 pref level");int blocks=int(std::min<Code>(65535,(nc+threads-1)/threads));r3_prefix_expand_kernel<<<std::max(1,blocks),threads>>>(cur,np,nxt,nc);ck(cudaDeviceSynchronize(),"r3 pref expand");cudaFree(cur);cur=nxt;np=nc;}return cur;
}
