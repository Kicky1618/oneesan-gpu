...
#pragma unroll
        ...
        const Count*pv=D_R5_PREF+(size_t)ch*STRIDE;const Count*sv=D_R5_SUFF+(size_t)cl*STRIDE;Count acc=0;int dim=r5_block_size(hs);
#pragma unroll 4
        for(int i=0;i<dim;++i)acc=r3_add_mod(acc,r3_mul_mod(pv[i],sv[i]));
        fullMain[grank]=acc;
    }
...
#pragma unroll
        ...
        meta=__shfl_sync(0xffffffffu,meta,0);grank=__shfl_sync(0xffffffffu,grank,0);uint32_t ch=uint32_t(meta&0xffffffu),cl=uint32_t((meta>>24)&0xffffffu);int hs=int((meta>>48)&0xffu),dim=r5_block_size(hs);const Count*pv=D_R5_PREF+(size_t)ch*STRIDE;const Count*sv=D_R5_SUFF+(size_t)cl*STRIDE;Count acc=0;for(int i=lane;i<dim;i+=32)acc=r3_add_mod(acc,r3_mul_mod(pv[i],sv[i]));
...
#pragma unroll
            for(int pos=TARGET_W-1;pos>=0;--pos){int dig=0;Code a=D_BOUND_DP[pos][h];if(r<a)dig=0;else{r-=a;if(h>0){a=D_BOUND_DP[pos][h-1];if(r<a){dig=1;grank+=D_FULL_DP[pos][h];--h;}else{r-=a;dig=2;grank+=D_FULL_DP[pos][h]+D_FULL_DP[pos][h-1];++h;}}else{dig=2;grank+=D_FULL_DP[pos][h];++h;}}if(pos>=LO){ch=ch*3u+dig;if(pos==LO)hs=h;}else cl=cl*3u+dig;}meta=uint64_t(ch)|(uint64_t(cl)<<24)|(uint64_t(hs)<<48);}
        meta=__shfl_sync(mask,meta,leader);grank=__shfl_sync(mask,grank,leader);uint32_t ch=uint32_t(meta&0xffffffu),cl=uint32_t((meta>>24)&0xffffffu);int hs=int((meta>>48)&0xffu),dim=r5_block_size(hs);const Count*pv=D_R5_PREF+(size_t)ch*STRIDE;const Count*sv=D_R5_SUFF+(size_t)cl*STRIDE;Count acc=0;for(int i=lane;i<dim;i+=8)acc=r3_add_mod(acc,r3_mul_mod(pv[i],sv[i]));
...
#pragma unroll
        for(int pos=TARGET_W-1;pos>=0;--pos){int dig=0;Code a=D_BOUND_DP[pos][h];if(r<a)dig=0;else{r-=a;if(h>0){a=D_BOUND_DP[pos][h-1];if(r<a){dig=1;grank+=D_FULL_DP[pos][h];--h;}else{r-=a;dig=2;grank+=D_FULL_DP[pos][h]+D_FULL_DP[pos][h-1];++h;}}else{dig=2;grank+=D_FULL_DP[pos][h];++h;}}if(pos>=LO){ch=ch*3u+dig;if(pos==LO)hs=h;}else cl=cl*3u+dig;}meta=uint64_t(ch)|(uint64_t(cl)<<24)|(uint64_t(hs)<<48);}
        meta=__shfl_sync(mask,meta,leader);grank=__shfl_sync(mask,grank,leader);uint32_t ch=uint32_t(meta&0xffffffu),cl=uint32_t((meta>>24)&0xffffffu);int hs=int((meta>>48)&0xffu),dim=r5_block_size(hs);const Count*pv=D_R5_PREF+(size_t)ch*STRIDE;const Count*sv=D_R5_SUFF+(size_t)cl*STRIDE;Count acc=0;for(int i=lane;i<dim;i+=LANES)acc=r3_add_mod(acc,r3_mul_mod(pv[i],sv[i]));for(int off=LANES/2;off;off>>=1)acc=r3_add_mod(acc,__shfl_down_sync(mask,acc,off,LANES));if(lane==0)fullMain[grank]=acc;
    }
}
...
static inline Count row5_mul_mod_fast_host(Count a,Count b){
    // row5 prototype modulus is p = 2^32 - 5.
    uint64_t z=uint64_t(a)*b;
    uint64_t t=uint32_t(z)+(z>>32)*5ULL;
    t=uint32_t(t)+(t>>32)*5ULL;
    constexpr uint64_t P=oneesan::row5mod::MOD;
    if(t>=P)t-=P;if(t>=P)t-=P;
    return Count(t);
}
static inline Count row5_add_mod_fast_host(Count a,Count b){
    constexpr Count P=oneesan::row5mod::MOD;
...
static void row5_apply_row_compact(const Count*in,Count*out,int h,int sym){
    using namespace oneesan::row5mod;int h2=h+row5_delta(sym);if(h<0||h>5||h2<0||h2>5)return;
    const uint32_t*off=sym==0?OFF_N:sym==1?OFF_R:OFF_L;const Tr*tr=sym==0?TR_N:sym==1?TR_R:TR_L;const auto&li=row5_local_index();
    for(int q=0;q<R5_BSZ[h];++q){Count v=in[q];if(!v)continue;int i=R5_BIDX[h][q];for(uint32_t k=off[i];k<off[i+1];++k){int d=li[tr[k].dst];Count z=row5_mul_mod_fast_host(v,tr[k].coeff);out[d]=row5_add_mod_fast_host(out[d],z);}}
}
static void row5_apply_col_compact(const Count*in,Count*out,int hdst,int sym){
    using namespace oneesan::row5mod;int hsrc=hdst-row5_delta(sym);if(hdst<0||hdst>5||hsrc<0||hsrc>5)return;
    const uint32_t*off=sym==0?OFF_N:sym==1?OFF_R:OFF_L;const Tr*tr=sym==0?TR_N:sym==1?TR_R:TR_L;const auto&li=row5_local_index();
    for(int q=0;q<R5_BSZ[hsrc];++q){int i=R5_BIDX[hsrc][q];Count acc=0;for(uint32_t k=off[i];k<off[i+1];++k){Count v=in[li[tr[k].dst]];if(v)acc=row5_add_mod_fast_host(acc,row5_mul_mod_fast_host(tr[k].coeff,v));}out[q]=acc;}
}
...
static std::vector<Count> make_row5_suffix_lut_compact_direct(int len){
    using namespace oneesan::row5mod;constexpr int S=61;std::vector<Count>cur(S);const auto&li=row5_local_index();for(int i=0;i<DIM;++i)if(BETA[i])cur[li[i]]=BETA[i];size_t cnt=1;
...
static void init_direct_row5_lut_full_single_gpu(int W,Count mod,int threads,Count*fullMain){
    using namespace oneesan::row5mod;if(mod!=MOD){std::cerr<<"row5 LUT prototype only supports modulus "<<MOD<<"\
";std::exit(34);}cudaSetDevice(0);Code dp[MAXW+1][MAXW+2]{};build_bounded_dp(5,dp);Code n=dp[W][1];ck(cudaMemcpyToSymbol(D_BOUND_DP,dp,sizeof(dp)),"row5 lut dp");int cap=5;ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"row5 lut cap");constexpr int lo=TARGET_W/2,hi=TARGET_W-lo;auto t0=std::chrono::steady_clock::now();std::vector<Count>pref,suff;std::thread tp([&]{pref=make_row5_prefix_lut_compact_direct(hi);});std::thread ts([&]{suff=make_row5_suffix_lut_compact_direct(lo);});tp.join();ts.join();double host_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();Count *dpref=nullptr,*dsuff=nullptr;ck(cudaMalloc(&dpref,pref.size()*sizeof(Count)),"row5 pref");ck(cudaMalloc(&dsuff,suff.size()*sizeof(Count)),"row5 suff");ck(cudaMemcpy(dpref,pref.data(),pref.size()*sizeof(Count),cudaMemcpyHostToDevice),"row5 pref copy");ck(cudaMemcpy(dsuff,suff.data(),suff.size()*sizeof(Count),cudaMemcpyHostToDevice),"row5 suff copy");ck(cudaMemcpyToSymbol(D_R5_PREF,&dpref,sizeof(dpref)),"row5 pref ptr");ck(cudaMemcpyToSymbol(D_R5_SUFF,&dsuff,sizeof(dsuff)),"row5 suff ptr");int lanes=0;if(const char*e=std::getenv("GRIDFP_ROW5_LANES"))lanes=std::atoi(e);else if(const char*e=std::getenv("GRIDFP_ROW5_WARP"))lanes=std::atoi(e)?32:0;int b=lanes?65535:int(std::min<Code>(65535,(n+threads-1)/threads));auto g0=std::chrono::steady_clock::now();if(lanes==32)bounded_fill_row5_full_warp_kernel<<<std::max(1,b),threads>>>(fullMain,n,W);else if(lanes==16)bounded_fill_row5_full_group_kernel<16><<<std::max(1,b),threads>>>(fullMain,n,W);else if(lanes==8)bounded_fill_row5_full_group_kernel<8><<<std::max(1,b),threads>>>(fullMain,n,W);else if(lanes==4)bounded_fill_row5_full_group_kernel<4><<<std::max(1,b),threads>>>(fullMain,n,W);else bounded_fill_row5_full_kernel<<<std::max(1,b),threads>>>(fullMain,n,W);ck(cudaDeviceSynchronize(),"row5 lut init");double gpu_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-g0).count();cudaFree(dpref);cudaFree(dsuff);std::cerr<<"direct row5 full states="<<n<<" lut_host_s="<<host_s<<" kernel_s="<<gpu_s<<" pref_mib="<<(pref.size()*sizeof(Count)>>20)<<" suff_mib="<<(suff.size()*sizeof(Count)>>20)<<"\
";
}
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        if(ng!=1&&prefixK>1){std::cerr<<"bounded prefix prototype currently requires NGPU=1\n";return 31;}
        if(prefixK==1){
            ...
            bool direct5=(prefixK==5);if(const char*e=std::getenv("GRIDFP_DIRECT_ROW5_LUT"))direct5=std::atoi(e)!=0;
            ...
            if(direct5&&prefixK==5&&ng==1){
                init_direct_row5_lut_full_single_gpu(W,mod,threads,mp[0]);
