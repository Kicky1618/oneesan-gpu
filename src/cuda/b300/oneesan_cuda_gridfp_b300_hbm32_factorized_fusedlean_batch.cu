...
__global__ void fused_reverse_main_forward_block_kernel(const Count*in,const Count*din,const MateID*mates,
        Code n,Code dn,Count*out_main,Count*out_block,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        MateID m=mates?mates[i]:factor_unrank_main(i);Count src=in[i];Count acc=src;
        MateID pred=0;bool have=false;
        ...
        default:break;}
        if(have){Count c=in[factor_rank_main(pred)];if(c)acc=add_mod_plain(acc,c);}
        if(oneesan::gridfp::mget(m,p)==oneesan::gridfp::N){MateID b=oneesan::gridfp::mshrink(m,p);Count c=din[factor_rank_block(b)];if(c)acc=add_mod_plain(acc,c);}
        out_main[i]=acc;
        if(src&&dn){auto pr=oneesan::gridfp::mpair(m,p);auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);if(z.valid&&z.blocked){Code j=factor_rank_block(z.mate);if(pr==oneesan::gridfp::NR||pr==oneesan::gridfp::NL)out_block[j]=src;else atomic_add_mod(out_block+j,src);}}
    }
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(p>1){
            if(ds.size)ck(cudaMemsetAsync(dnext,0,size_t(ds.size)*sizeof(Count),c.sMain),"clear fused blocked");
            if(ms.size)fused_reverse_main_forward_block_kernel<<<bm,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,ms.size,ds.size,nxt,dnext,p);
        }else{
