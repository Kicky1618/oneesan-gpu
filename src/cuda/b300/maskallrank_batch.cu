...
__device__ __forceinline__ Code factor_global_rank_main_index(Code i){
    ...
    if(D_F_FIX_LOW){har=hr;lar=D_F_LOW_MASK_ALL_RANK[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];}
    else{har=D_F_HIGH_MASK_ALL_RANK[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];lar=lr;}
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t sparseBytes=0;uint64_t preRankBytes=uint64_t(G_PRERANK.owner_block_rec.size()+G_PRERANK.highrr_rec.size()+G_PRERANK.closure_inv_rec.size()+G_HPR.owner_block_rec.size()+G_HPR.crossll_rec.size()+G_HPR.closure_inv_rec.size())*8+uint64_t(G_PRERANK.closure_inv_src.size()+G_HPR.closure_inv_src.size())*4;uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+vb4(G_FACTOR.low_mask_all_rank)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+vb4(G_FACTOR.high_mask_all_rank)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+sparseBytes+preRankBytes;
    ...
    SparseVmmU32 fLDv[MAXGPU]{},fHRv[MAXGPU]{};uint32_t *fLA[MAXGPU]{},*fLM[MAXGPU]{},*fLO[MAXGPU]{},*fLAR[MAXGPU]{},*fLD[MAXGPU]{},*fHA[MAXGPU]{},*fHM[MAXGPU]{},*fHO[MAXGPU]{},*fHAR[MAXGPU]{},*fHR[MAXGPU]{};unsigned long long *prOB[MAXGPU]{},*prCI[MAXGPU]{},*prH[MAXGPU]{},*hpOB[MAXGPU]{},*hpCI[MAXGPU]{},*hpX[MAXGPU]{};uint32_t *prCIS[MAXGPU]{},*hpCIS[MAXGPU]{};Code *fHMB[MAXGPU]{},*fHBB[MAXGPU]{};
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-fullprerank-orbit-maskallrank-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);if(fLAR[d])cudaFree(fLAR[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);if(fHAR[d])cudaFree(fHAR[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prOB[d])cudaFree(prOB[d]);if(prCI[d])cudaFree(prCI[d]);if(prCIS[d])cudaFree(prCIS[d]);if(prH[d])cudaFree(prH[d]);if(hpOB[d])cudaFree(hpOB[d]);if(hpCI[d])cudaFree(hpCI[d]);if(hpCIS[d])cudaFree(hpCIS[d]);if(hpX[d])cudaFree(hpX[d]);}
}
