...
template<int LANES> __global__ void row6_cartesian_group_kernel(Count*fullMain,Code fullBase,const uint32_t*pc,const Code*pr,Code np,const uint32_t*sc,const Code*sr,Code ns,int dim){
    static_assert(LANES==1||LANES==2||LANES==4||LANES==8||LANES==16);int lane=threadIdx.x&(LANES-1),warpLane=threadIdx.x&31,leader=warpLane&~(LANES-1);unsigned mask=((1u<<LANES)-1u)<<leader;Code total=np*ns,group=(Code(blockIdx.x)*blockDim.x+threadIdx.x)/LANES,step=(Code(gridDim.x)*blockDim.x)/LANES;
