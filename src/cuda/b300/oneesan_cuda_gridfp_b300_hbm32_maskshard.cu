...
struct MaskShardAddress {
    int owner = -1;
    Code offset = 0;
};

static MaskShardAddress maskshard_main_address_host(
    MateID m,
    ...
    if (hp == 0xffffffffu || lp == 0xffffffffu || bid >= layout.main_blocks.size()) {
        std::cerr << "maskshard invalid host main address\n";
        std::exit(140);
    }
...
static std::vector<uint32_t> build_maskshard_high_route(
    const StorageFactorHost& storage
) {
    constexpr int H = HIGH_LUT_K;
    ...
    for (int he = 0; he <= H + 1; ++he) {
        ...
        for (uint32_t r = 0; r < b - a; ++r) {
            ...
            if (packed == 0xffffffffu) {
                std::cerr << "maskshard missing HIGH packed rank\n";
                std::exit(141);
            }
...
struct MaskShardWorker {
    ...
    void ensure_bytes(size_t need) {
        if (need <= cap) return;
        if (arena) ck(cudaFree(arena), "maskshard worker free arena");
        cap = need;
        ck(cudaMalloc(&arena, cap), "maskshard worker arena");
    }
...
static std::vector<HighJob> build_high_jobs() {
    constexpr uint32_t NM = 1u << LOW_LUT_K;
    std::vector<HighJob> jobs;
    jobs.reserve(NM);
    ...
    std::sort(jobs.begin(), jobs.end(), [](const HighJob& x, const HighJob& y) {
        return x.work > y.work;
    });
    return jobs;
}

static void maskshard_configure_high_group(uint32_t mask) {
    constexpr int L = LOW_LUT_K;
    ...
    if (mb.empty() || db.empty() || mb.back().end != ms.size || db.back().end != ds.size) {
        std::cerr << "maskshard high factor-size mismatch mask=" << mask << '\n';
        std::exit(142);
    }
    ...
    ck(cudaMemcpyToSymbol(D_F_MAIN_BLOCKS, mb.data(), mb.size() * sizeof(FBlock)),
       "maskshard high main blocks");
    ck(cudaMemcpyToSymbol(D_F_BLOCK_BLOCKS, db.data(), db.size() * sizeof(FBlock)),
       "maskshard high block blocks");
    ck(cudaMemcpyToSymbol(D_F_MAIN_NBLOCKS, &mn, sizeof(mn)), "maskshard high main nblocks");
    ck(cudaMemcpyToSymbol(D_F_BLOCK_NBLOCKS, &dn, sizeof(dn)), "maskshard high block nblocks");
    ck(cudaMemcpyToSymbol(D_F_MASK, &mask, sizeof(mask)), "maskshard high low-mask");
    ck(cudaMemcpyToSymbol(D_F_FIX_LOW, &fix_low, sizeof(fix_low)), "maskshard high mode");
    ck(cudaMemcpyToSymbol(D_MAIN_FIXED, &mf, sizeof(mf)), "maskshard high main fixed");
    ck(cudaMemcpyToSymbol(D_MAIN_OCC, &mo, sizeof(mo)), "maskshard high main occ");
    ck(cudaMemcpyToSymbol(D_BLOCK_FIXED, &bf, sizeof(bf)), "maskshard high block fixed");
    ck(cudaMemcpyToSymbol(D_BLOCK_OCC, &bo, sizeof(bo)), "maskshard high block occ");
    ck(cudaMemcpyToSymbol(D_MAIN_DP, ms.dp, sizeof(ms.dp)), "maskshard high main dp");
    ck(cudaMemcpyToSymbol(D_BLOCK_DP, ds.dp, sizeof(ds.dp)), "maskshard high block dp");
}

static void process_high_job(MaskShardWorker& w, const HighJob& job, int threads) {
    ck(cudaSetDevice(w.dev), "maskshard high set device");
    if (!job.main_n && !job.block_n) return;
    maskshard_configure_high_group(job.low_mask);
    ...
    ck(cudaGetLastError(), "maskshard high gather");
    ck(cudaDeviceSynchronize(), "maskshard high gather sync");
    ...
    for (int p = TARGET_W - 1; p >= LOW_LUT_K + 1; --p) {
        ...
        if (job.main_n)
            ck(cudaMemcpy(nxt, cur, size_t(job.main_n) * sizeof(Count), cudaMemcpyDeviceToDevice),
               "maskshard high identity");
        if (job.block_n)
            ck(cudaMemset(dnext, 0, size_t(job.block_n) * sizeof(Count)),
               "maskshard high clear blocked");
        w.high_identity_s += ram_seconds_since(t);

        t = std::chrono::steady_clock::now();
        if (job.main_n)
            main_group_kernel<<<bm, threads>>>(cur, nullptr, job.main_n, nxt, dnext, p);
        if (job.block_n)
            blocked_group_kernel<<<bd, threads>>>(dcur, job.block_n, nxt, p);
        ck(cudaGetLastError(), "maskshard high transition");
        ck(cudaDeviceSynchronize(), "maskshard high transition sync");
    ...
    ck(cudaGetLastError(), "maskshard high scatter");
    ck(cudaDeviceSynchronize(), "maskshard high scatter sync");
    w.high_io_s += ram_seconds_since(t);
    ++w.high_groups;
}
...
int main(int argc, char** argv) {
    ...
    if (n + 1 != TARGET_W || n < 2 || TARGET_W > MAXW) {
        std::cerr << "maskshard binary specialized for n=" << TARGET_W - 1 << '\n';
        return 1;
    }
    ...
    for (int a = 0; a < ngpu; ++a) {
        for (int b = 0; b < ngpu; ++b) if (a != b) {
            ...
            if (!can) continue;
            ck(cudaSetDevice(a), "maskshard peer set");
    ...
    if (ngpu > 1 && peers != ngpu * (ngpu - 1)) {
        std::cerr << "maskshard requires full P2P: " << peers << '/' << ngpu * (ngpu - 1) << '\n';
        return 3;
    }

    MaskShardLayout shard = build_high_mask_shard_layout(storage, layout, ngpu);
    const auto high_route = build_maskshard_high_route(storage);
    report_high_mask_shard_layout(shard);

    Count* mp[8]{};
    Count* bp[8]{};
    for (int d = 0; d < ngpu; ++d) {
        ck(cudaSetDevice(d), "maskshard auth set");
        if (shard.gpu_main[d])
            ck(cudaMalloc(&mp[d], size_t(shard.gpu_main[d]) * sizeof(Count)), "maskshard auth main");
        if (shard.gpu_block[d])
            ck(cudaMalloc(&bp[d], size_t(shard.gpu_block[d]) * sizeof(Count)), "maskshard auth block");
        if (mp[d]) ck(cudaMemset(mp[d], 0, size_t(shard.gpu_main[d]) * sizeof(Count)), "maskshard zero main");
        if (bp[d]) ck(cudaMemset(bp[d], 0, size_t(shard.gpu_block[d]) * sizeof(Count)), "maskshard zero block");
    }

    std::vector<StorageDeviceTables> factor_dev(ngpu);
    std::vector<MaskShardDeviceMeta> shard_dev(ngpu);
    std::vector<MaskShardWorker> workers(ngpu);
    for (int d = 0; d < ngpu; ++d) {
        ck(cudaSetDevice(d), "maskshard table set");
    ...
    const MaskShardAddress ia = maskshard_main_address_host(init, storage, layout, shard);
    const MaskShardAddress oa = maskshard_main_address_host(MateID(R), storage, layout, shard);
    const Count one = 1;
    ck(cudaSetDevice(ia.owner), "maskshard init device");
    ck(cudaMemcpy(mp[ia.owner] + ia.offset, &one, sizeof(one), cudaMemcpyHostToDevice),
       "maskshard init one");

    const auto high_jobs = build_high_jobs();
    ...
    for (int row = 0; row < TARGET_W; ++row) {
        // HIGH-active phase. Dynamic scheduling keeps compute balanced. Because
        // HIGH/LOW mask correlation is tiny, locality-aware scheduling changes
        // peer volume by only ~0.02%, so this baseline favors load balance.
        ...
        for (int d = 0; d < ngpu; ++d) {
            ts.emplace_back([&, d] {
                for (;;) {
                    ...
                    process_high_job(workers[d], high_jobs[q], threads);
                }
            });
        }
        for (auto& t : ts) t.join();

        // LOW-active phase. Every HIGH occupancy mask is closed, so each group
        // executes wholly on its owner GPU with zero P2P group I/O.
        ts.clear();
        for (int d = 0; d < ngpu; ++d) {
            ts.emplace_back([&, d] {
                ck(cudaSetDevice(d), "maskshard low worker set");
        ...
        for (auto& t : ts) t.join();
        std::cerr << "maskshard row " << row + 1 << '/' << TARGET_W << '\n';
    }
    const double wall_s = ram_seconds_since(wall0);

    Count answer = 0;
    ck(cudaSetDevice(oa.owner), "maskshard answer device");
    ...
    std::cout << "backend=b300-factorized-maskshard-v0.1"
              << " n=" << n
              << " gpus=" << ngpu
              << " residue=" << answer
              << " modulus=" << mod
    ...
    for (int d = 0; d < ngpu; ++d) {
        ck(cudaSetDevice(d), "maskshard cleanup set");
        workers[d].release();
        shard_dev[d].release();
