...
__global__ void factor_orbit_window_coop_kernel(Count*mainv,Count*blockv,const MateID*mates,Code n,int p_hi,int p_lo){
    cg::grid_group grid=cg::this_grid();
    Code tid=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(int p=p_hi;p>=p_lo;--p){
        for(Code i=tid;i<n;i+=stride){
            MateID m=mates?mates[i]:factor_unrank_main(i);
            auto w=oneesan::gridfp::mpair(m,p);
            if(w!=oneesan::gridfp::NN && w!=oneesan::gridfp::NR && w!=oneesan::gridfp::NL)continue;
            Count c=mainv[i];
            MateID b=oneesan::gridfp::mshrink(m,p);Code dj=factor_rank_block(b);Count d=blockv[dj];
            if(w==oneesan::gridfp::NN){
                MateID t=oneesan::gridfp::msetpair(m,p,oneesan::gridfp::LR);Code j=factor_rank_main(t);
                mainv[j]=add_mod_plain(mainv[j],c);mainv[i]=add_mod_plain(c,d);blockv[dj]=0;
            }else{
                MateID t=oneesan::gridfp::msetpair(m,p,w==oneesan::gridfp::NR?oneesan::gridfp::RN:oneesan::gridfp::LN);Code j=factor_rank_main(t);
                Count cc=mainv[j],all=add_mod_plain(add_mod_plain(c,cc),d);
                if(p==1){mainv[i]=all;mainv[j]=add_mod_plain(c,cc);blockv[dj]=0;}
                else{mainv[i]=all;blockv[dj]=c;}
            }
        }
        grid.sync();
        for(Code i=tid;i<n;i+=stride){
            Count c=mainv[i];if(!c)continue;
            MateID m=mates?mates[i]:factor_unrank_main(i);auto w=oneesan::gridfp::mpair(m,p);
            if(w!=oneesan::gridfp::LL && w!=oneesan::gridfp::RR && w!=oneesan::gridfp::RL)continue;
            auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);if(!z.valid)continue;
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        if(ms.size){
            int coop=0;ck(cudaDeviceGetAttribute(&coop,cudaDevAttrCooperativeLaunch,c.dev),"coop attr");if(!coop)throw std::runtime_error("cooperative launch unsupported");
            int ct=256;if(const char*e=std::getenv("GRIDFP_ORBIT_COOP_THREADS"))ct=std::max(64,std::min(1024,std::atoi(e)));
            int perSm=0,sm=0;ck(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&perSm,factor_orbit_window_coop_kernel,ct,0),"coop occupancy");ck(cudaDeviceGetAttribute(&sm,cudaDevAttrMultiProcessorCount,c.dev),"coop sm");
            int cb=std::max(1,perSm*sm);Count*ma=cur;Count*bl=dcur;const MateID*mt=useMate?c.dMate:nullptr;Code nn=ms.size;int ph=wp.p_hi,pl=wp.p_lo;void*args[]={&ma,&bl,&mt,&nn,&ph,&pl};
            ck(cudaLaunchCooperativeKernel((void*)factor_orbit_window_coop_kernel,cb,ct,args,0,c.sMain),"orbit cooperative launch");
        }
        ck(cudaStreamSynchronize(c.sMain),"hybrid orbit cooperative sync");
    }else{
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base);
