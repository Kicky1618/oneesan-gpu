...
// Experimental half-height DP + sparse connected-meander central join.
// Factor binaries are currently coefficients modulo 1,000,000,007 only.
struct HalfFacEntry { uint32_t dst, coeff; };
struct HalfFactorHost {
    uint32_t n=0; uint64_t rn=0,en=0;
    std::vector<Count> diag; std::vector<uint64_t> ro,eo;
    std::vector<HalfFacEntry> Rv,Ev;
};
...
static HalfFactorHost half_load_factor(int m,Count targetMod){
    ...
    std::string fn=dir+"/meander_factor_m"+std::to_string(m)+".bin";std::ifstream f(fn,std::ios::binary);
    if(!f){std::cerr<<"cannot open meander factor "<<fn<<"\n";std::exit(61);}uint64_t magic=0;uint32_t ver=0;HalfFactorHost z;
    f.read((char*)&magic,8);f.read((char*)&ver,4);f.read((char*)&z.n,4);f.read((char*)&z.rn,8);f.read((char*)&z.en,8);
    if(magic!=0x4d45414e44455246ULL||ver!=1){std::cerr<<"bad meander factor header "<<fn<<"\n";std::exit(62);}
    z.diag.resize(z.n);z.ro.resize(z.n+1);z.eo.resize(z.n+1);z.Rv.resize(z.rn);z.Ev.resize(z.en);
    f.read((char*)z.diag.data(),size_t(z.n)*sizeof(Count));f.read((char*)z.ro.data(),size_t(z.n+1)*sizeof(uint64_t));f.read((char*)z.eo.data(),size_t(z.n+1)*sizeof(uint64_t));
    f.read((char*)z.Rv.data(),size_t(z.rn)*sizeof(HalfFacEntry));f.read((char*)z.Ev.data(),size_t(z.en)*sizeof(HalfFacEntry));
    if(!f){std::cerr<<"truncated meander factor "<<fn<<"\n";std::exit(63);}
    if(targetMod!=1000000007u){
        std::unordered_map<uint32_t,uint32_t> cv;int maxscale=0;
        auto conv=[&](uint32_t x)->uint32_t{auto it=cv.find(x);if(it!=cv.end())return it->second;int besta=0,bestd=1,bests=INT_MAX;for(int d=1;d<=10000;++d){long long a=(unsigned long long)x*d%1000000007ULL;if(a>500000003LL)a-=1000000007LL;if(std::gcd((long long)std::llabs(a),(long long)d)!=1)continue;int sc=std::max<int>(std::llabs(a),d);if(sc<bests){bests=sc;besta=int(a);bestd=d;}}if(bests==INT_MAX||bests>10000){std::cerr<<"cannot rational-reconstruct factor coefficient "<<x<<"\n";std::exit(68);}maxscale=std::max(maxscale,bests);uint32_t den=uint32_t(bestd%targetMod);uint32_t inv=half_pow_host(den,uint64_t(targetMod)-2,targetMod);uint64_t aa=besta<0?uint64_t(targetMod-uint32_t((-int64_t(besta))%targetMod)):uint64_t(uint32_t(besta)%targetMod);uint32_t y=uint32_t(aa*inv%targetMod);cv.emplace(x,y);return y;};
        for(auto&x:z.diag)x=conv(x);for(auto&x:z.Rv)x.coeff=conv(x.coeff);for(auto&x:z.Ev)x.coeff=conv(x.coeff);
        std::cerr<<"half factor rational map m="<<m<<" coeff_types="<<cv.size()<<" maxscale="<<maxscale<<"\n";
    }
    return z;
}
...
__global__ void half_factor_r(int n,const uint64_t*off,const HalfFacEntry*e,const Count*y,Count*z,int B){int lane=threadIdx.x&31;Code wg=(Code(blockIdx.x)*blockDim.x+threadIdx.x)>>5,step=(Code(gridDim.x)*blockDim.x)>>5;int groups=(B+31)>>5;for(;wg<Code(n)*groups;wg+=step){int i=int(wg/groups),g=int(wg-Code(i)*groups),b=g*32+lane;Count acc=0;for(uint64_t q=off[i];q<off[i+1];++q){uint32_t d=0,c=0;if(lane==0){d=e[q].dst;c=e[q].coeff;}d=__shfl_sync(0xffffffffu,d,0);c=__shfl_sync(0xffffffffu,c,0);if(b<B)acc=half_add(acc,half_mul(c,y[size_t(d)*B+b]));}if(b<B)z[size_t(i)*B+b]=acc;}}
__global__ void half_factor_e(int n,const uint64_t*off,const HalfFacEntry*e,const Count*diag,const Count*z,Count*out,int B){int lane=threadIdx.x&31;Code wg=(Code(blockIdx.x)*blockDim.x+threadIdx.x)>>5,step=(Code(gridDim.x)*blockDim.x)>>5;int groups=(B+31)>>5;for(;wg<Code(n)*groups;wg+=step){int i=int(wg/groups),g=int(wg-Code(i)*groups),b=g*32+lane;Count acc=0;if(b<B)acc=half_mul(diag[i],z[size_t(i)*B+b]);for(uint64_t q=off[i];q<off[i+1];++q){uint32_t d=0,c=0;if(lane==0){d=e[q].dst;c=e[q].coeff;}d=__shfl_sync(0xffffffffu,d,0);c=__shfl_sync(0xffffffffu,c,0);if(b<B)acc=half_add(acc,half_mul(c,z[size_t(d)*B+b]));}if(b<B)out[size_t(i)*B+b]=acc;}}
...
static Count half_central_join_single_gpu(int W,int threads){
    ...
    for(int m=1;2*m-1<=W;++m){int k=2*m-1;auto F=half_load_factor(m,mod);std::vector<uint64_t> pats;half_pat_rec(0,k,1,0,pats);if(pats.size()!=F.n){std::cerr<<"half factor dim mismatch m="<<m<<" pats="<<pats.size()<<" fac="<<F.n<<"\n";std::exit(66);}std::vector<uint64_t> rp(F.n);for(uint32_t i=0;i<F.n;++i)rp[i]=half_reflect_pat(pats[i],k);auto masks=half_masks(W,k);allmasks+=masks.size();coeffops+=(F.rn+F.en+F.n)*masks.size();
        uint64_t *dpat=nullptr,*drpat=nullptr,*dro=nullptr,*deo=nullptr;HalfFacEntry *dR=nullptr,*dE=nullptr;Count *ddiag=nullptr;uint32_t *dm=nullptr,*drm=nullptr;ck(cudaMalloc(&dpat,F.n*8),"half pat");ck(cudaMalloc(&drpat,F.n*8),"half rpat");ck(cudaMalloc(&dro,(F.n+1)*8),"half ro");ck(cudaMalloc(&deo,(F.n+1)*8),"half eo");ck(cudaMalloc(&dR,F.rn*sizeof(HalfFacEntry)),"half R");ck(cudaMalloc(&dE,F.en*sizeof(HalfFacEntry)),"half E");ck(cudaMalloc(&ddiag,F.n*4),"half diag");ck(cudaMalloc(&dm,size_t(tile)*4),"half masks");ck(cudaMalloc(&drm,size_t(tile)*4),"half rmasks");ck(cudaMemcpy(dpat,pats.data(),F.n*8,cudaMemcpyHostToDevice),"half pat copy");ck(cudaMemcpy(drpat,rp.data(),F.n*8,cudaMemcpyHostToDevice),"half rpat copy");ck(cudaMemcpy(dro,F.ro.data(),(F.n+1)*8,cudaMemcpyHostToDevice),"half ro copy");ck(cudaMemcpy(deo,F.eo.data(),(F.n+1)*8,cudaMemcpyHostToDevice),"half eo copy");ck(cudaMemcpy(dR,F.Rv.data(),F.rn*sizeof(HalfFacEntry),cudaMemcpyHostToDevice),"half R copy");ck(cudaMemcpy(dE,F.Ev.data(),F.en*sizeof(HalfFacEntry),cudaMemcpyHostToDevice),"half E copy");ck(cudaMemcpy(ddiag,F.diag.data(),F.n*4,cudaMemcpyHostToDevice),"half diag copy");
    ...
    Count ans=0;ck(cudaMemcpy(&ans,ansd,sizeof(ans),cudaMemcpyDeviceToHost),"half ans copy");cudaFree(ansd);std::cerr<<"half join total masks="<<allmasks<<" coeffops="<<coeffops<<" gather_s="<<gather_s<<" factor_s="<<factor_s<<" dot_s="<<dot_s<<"\n";return ans;
}
