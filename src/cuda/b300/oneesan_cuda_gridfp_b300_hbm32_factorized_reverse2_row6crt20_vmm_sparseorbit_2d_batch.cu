...
static SparseOrbitTablesHost build_sparse_orbit_tables(){
    ...
    for(int p=1;p<=L;++p)for(int hs=0;hs<=L+1;++hs){
        if(p<L){auto q=build_one(p,hs,0);for(int c=1;c<3;++c){z.owner_sel[sparse_key(p,hs,c)]=q.first;z.closure_sel[sparse_key(p,hs,c)]=q.second;}}
        else for(int c=0;c<3;++c)build_one(p,hs,c);
    }
...
static SparseOrbitMap make_sparse_orbit_map(const std::vector<FBlock>&fb,int p,bool closure){
    SparseOrbitMap m{};
    for(int b=0;b<(int)fb.size();++b){auto const&x=fb[b];SparseSel q=(closure?G_SPARSE.closure_sel:G_SPARSE.owner_sel)[sparse_key(p,x.hs,x.c)];
        Code hc=(x.stride&&x.end>x.off)?((x.end-x.off)/x.stride):0,work=hc*Code(q.count);if(!work)continue;
        int a=m.nactive++;m.bid[a]=uint8_t(b);m.sel_off[a]=q.off;m.sel_count[a]=q.count;m.total+=work;m.max_work=std::max(m.max_work,work);
    }
    return m;
}
...
__global__ void factor_sparse_orbit_2d_kernel(Count*mainv,Count*blockv,const MateID*mates,SparseOrbitMap map,int p){
    int a=int(blockIdx.y);if(a>=map.nactive)return;int bid=int(map.bid[a]);FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t sc=map.sel_count[a];
    Code hc=x.stride?((x.end-x.off)/x.stride):0,work=hc*Code(sc),st=Code(gridDim.x)*blockDim.x;
    for(Code loc=Code(blockIdx.x)*blockDim.x+threadIdx.x;loc<work;loc+=st){
        uint32_t hr=uint32_t(loc/sc),k=uint32_t(loc-Code(hr)*sc);uint32_t lr=(CLOSURE?D_SP_CLOSURE_LR:D_SP_OWNER_LR)[map.sel_off[a]+k];
        Code i=x.off+Code(hr)*x.stride+lr;Count c=mainv[i];MateID m=mates?mates[i]:sparse_unrank_main_known(bid,hr,lr);
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){
            auto om=make_sparse_orbit_map(fmb,p,false),cm=make_sparse_orbit_map(fmb,p,true);
            auto launch2d=[&](auto tag,SparseOrbitMap const&m,const char*what){if(!m.total)return;int gx=std::max(1,65535/std::max(1,m.nactive));gx=int(std::min<Code>(gx,(m.max_work+threads-1)/threads));dim3 gd(gx,m.nactive);if constexpr(decltype(tag)::value)factor_sparse_orbit_2d_kernel<true><<<gd,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,m,p);else factor_sparse_orbit_2d_kernel<false><<<gd,threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,m,p);ck(cudaGetLastError(),what);};
            launch2d(std::false_type{},om,"factor sparse orbit2d");
            launch2d(std::true_type{},cm,"factor sparse closure2d");
        }
...
int main(int argc,char**argv){
    ...
    G_SPARSE=build_sparse_orbit_tables();
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t sparseBytes=vb4(G_SPARSE.owner_lr)+vb4(G_SPARSE.closure_lr);uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+sparseBytes;
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-vmm-sparseorbit-2d-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(spO[d])cudaFree(spO[d]);if(spC[d])cudaFree(spC[d]);}
}
