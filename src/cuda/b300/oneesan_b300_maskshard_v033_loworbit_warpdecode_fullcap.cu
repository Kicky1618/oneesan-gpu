// v0.33 experiment: v0.32 warp-shared compact decode extended to the full-cap
// physical BLOCKED path. Task mapping and host launch geometry are unchanged.
...
#define main oneesan_maskshard_v33_loworbitwarpdecodefullcap_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.33 full-cap warp LOW orbit decode requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.33-loworbit-warpdecode-fullcap-batch"
        ...
        << " low_orbit_row_depth_compact_launch=1 low_orbit_warp_decode=1"
        << " low_orbit_warp_decode_fullcap=1"
    ...
    return oneesan_maskshard_v33_loworbitwarpdecodefullcap_inner_main(argc, argv);
}
