...
struct PreRankOrbitTablesHost {
    std::vector<unsigned long long> owner_rec,closure_rec;
    std::vector<unsigned long long> highrr_rec;
    std::vector<SparseSel> owner_sel,closure_sel,highrr_sel;
};
...
static PreRankOrbitTablesHost build_prerank_orbit_tables(){
    ...
    z.owner_sel.assign(keys,{0,0});z.closure_sel.assign(keys,{0,0});z.highrr_sel.assign(keys,{0,0});
    ...
    for(int p=1;p<=L;++p)for(int hs=0;hs<=L+1;++hs)for(int c=0;c<3;++c){
        ...
        if(he<0||he>HIGH_LUT_K+1){z.owner_sel[key]=os;z.closure_sel[key]=cs;z.highrr_sel[key]=ss;continue;}
        uint32_t beg=G_FACTOR.low_all_off[hs],end=G_FACTOR.low_all_off[hs+1];
        for(uint32_t a=beg;a<end;++a){
            uint32_t lr=a-beg,lc=G_FACTOR.low_all_codes[a];
            oneesan::gridfp::MateID m=oneesan::gridfp::MateID(lc)|(oneesan::gridfp::MateID(c)<<(2*L));
        ...
        z.owner_sel[key]=os;z.closure_sel[key]=cs;z.highrr_sel[key]=ss;
    }
    double mib=double(z.owner_rec.size()+z.closure_rec.size()+z.highrr_rec.size())*8.0/(1<<20);
    std::cerr<<"pre-rank orbit owner="<<z.owner_rec.size()<<" closure_fast="<<z.closure_rec.size()<<" closure_highrr="<<z.highrr_rec.size()<<" mib="<<mib<<"\n";
    return z;
}

struct HighPreRankTablesHost {
    std::vector<unsigned long long> owner_rec,closure_rec,crossll_rec;
    std::vector<SparseSel> owner_sel,closure_sel,crossll_sel;
};
...
static HighPreRankTablesHost build_high_prerank_tables(){
    constexpr int L=LOW_LUT_K,H=HIGH_LUT_K;constexpr uint32_t HM=(1u<<(2*H))-1u;HighPreRankTablesHost z;size_t keys=size_t(MAXW+1)*(MAXW+2)*3u;z.owner_sel.assign(keys,{0,0});z.closure_sel.assign(keys,{0,0});z.crossll_sel.assign(keys,{0,0});
    for(int p=L+1;p<=L+H;++p)for(int he=0;he<=H+1;++he)for(int c=0;c<3;++c){auto key=high_pr_key(p,he,c);SparseSel os{uint32_t(z.owner_rec.size()),0},cs{uint32_t(z.closure_rec.size()),0},xs{uint32_t(z.crossll_rec.size()),0};int dc=(c==int(::L)?1:c==int(::R)?-1:0),hs=he+dc;if(hs<0||hs>L+1){z.owner_sel[key]=os;z.closure_sel[key]=cs;z.crossll_sel[key]=xs;continue;}uint32_t beg=G_FACTOR.high_all_off[he],end=G_FACTOR.high_all_off[he+1];
        ...
        os.count=uint32_t(z.owner_rec.size())-os.off;cs.count=uint32_t(z.closure_rec.size())-cs.off;xs.count=uint32_t(z.crossll_rec.size())-xs.off;z.owner_sel[key]=os;z.closure_sel[key]=cs;z.crossll_sel[key]=xs;
    }
    double mib=double(z.owner_rec.size()+z.closure_rec.size()+z.crossll_rec.size())*8.0/(1<<20);std::cerr<<"high pre-rank owner="<<z.owner_rec.size()<<" closure_fast="<<z.closure_rec.size()<<" closure_crossll="<<z.crossll_rec.size()<<" mib="<<mib<<"\n";return z;
}
...
static PreRankOrbitMap make_prerank_orbit_map(const std::vector<FBlock>&fb,int p,int kind){
    ...
    for(int b=0;b<m.nblocks;++b){auto const&x=fb[b];auto key=sparse_key(p,x.hs,x.c);SparseSel q=kind==0?G_PRERANK.owner_sel[key]:(kind==1?G_PRERANK.closure_sel[key]:G_PRERANK.highrr_sel[key]);Code hc=(x.stride&&x.end>x.off)?((x.end-x.off)/x.stride):0;sum+=hc*Code(q.count);m.end[b]=sum;m.sel_off[b]=q.off;m.sel_count[b]=q.count;}
    return m;
}
...
__global__ void factor_prerank_closure_combined_kernel(Count*mainv,Count*blockv,PreRankOrbitMap fastMap,PreRankOrbitMap highMap,int p){
    constexpr uint32_t B=20,M=(1u<<B)-1u;constexpr int H=HIGH_LUT_K,S=MAXW+2;constexpr uint32_t HM=(1u<<H)-1u;
    Code nf=fastMap.nblocks?fastMap.end[fastMap.nblocks-1]:0,nh=highMap.nblocks?highMap.end[highMap.nblocks-1]:0,total=nf+nh;
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){
        if(q<nf){int bid=prerank_find_block(q,fastMap);Code base=bid?fastMap.end[bid-1]:0,loc=q-base;uint32_t sc=fastMap.sel_count[bid],hr=uint32_t(loc/sc),k=uint32_t(loc-Code(hr)*sc);unsigned long long r=D_PR_CLOSURE_REC[fastMap.sel_off[bid]+k];uint32_t lr=uint32_t(r)&M,dlr=uint32_t(r>>B)&M,dc=uint32_t(r>>(2*B))&3u;FBlock x=D_F_MAIN_BLOCKS[bid];Count c=mainv[x.off+Code(hr)*x.stride+lr];if(!c)continue;if(p==1){FBlock dx=D_F_MAIN_BLOCKS[3*x.he+int(dc)];atomic_add_mod(mainv+dx.off+Code(hr)*dx.stride+dlr,c);}else{FBlock bx=D_F_BLOCK_BLOCKS[x.he];atomic_add_mod(blockv+bx.off+Code(hr)*bx.stride+dlr,c);}}
        else{Code zq=q-nf;int bid=prerank_find_block(zq,highMap);Code base=bid?highMap.end[bid-1]:0,loc=zq-base;uint32_t sc=highMap.sel_count[bid],hr=uint32_t(loc/sc),k=uint32_t(loc-Code(hr)*sc);unsigned long long rec=D_PR_HIGHRR_REC[highMap.sel_off[bid]+k];uint32_t lr=uint32_t(rec)&M,dlr=uint32_t(rec>>B)&M;int depth=int((rec>>(2*B))&63u);FBlock x=D_F_MAIN_BLOCKS[bid];Code i=x.off+Code(hr)*x.stride+lr;Count c=mainv[i];if(!c)continue;uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];int match=-1,s=depth;
#pragma unroll
...
static HighPreRankMap make_high_prerank_map(const std::vector<FBlock>&fb,int p,int kind){HighPreRankMap m{};m.nblocks=int(fb.size());Code sum=0;for(int b=0;b<m.nblocks;++b){auto const&x=fb[b];auto key=high_pr_key(p,x.he,x.c);SparseSel q=kind==0?G_HPR.owner_sel[key]:(kind==1?G_HPR.closure_sel[key]:G_HPR.crossll_sel[key]);Code lc=x.stride;sum+=lc*Code(q.count);m.end[b]=sum;m.sel_off[b]=q.off;m.sel_count[b]=q.count;}return m;}
...
__global__ void factor_high_prerank_closure_combined_kernel(Count*mainv,Count*blockv,HighPreRankMap fastMap,HighPreRankMap crossMap){constexpr uint32_t B=20,M=(1u<<B)-1u;constexpr int L=LOW_LUT_K,S=MAXW+2;constexpr uint32_t LM=(1u<<L)-1u;Code nf=fastMap.nblocks?fastMap.end[fastMap.nblocks-1]:0,nx=crossMap.nblocks?crossMap.end[crossMap.nblocks-1]:0,total=nf+nx;for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){if(q<nf){int bid=high_pr_find_block(q,fastMap);Code base=bid?fastMap.end[bid-1]:0,loc=q-base;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lc=x.stride,k=uint32_t(loc/lc),lr=uint32_t(loc-Code(k)*lc);unsigned long long r=D_HP_CLOSURE_REC[fastMap.sel_off[bid]+k];uint32_t hr=uint32_t(r)&M,bhr=uint32_t(r>>B)&M;Count c=mainv[x.off+Code(hr)*x.stride+lr];if(c){FBlock bx=D_F_BLOCK_BLOCKS[x.hs];atomic_add_mod(blockv+bx.off+Code(bhr)*bx.stride+lr,c);}}
        else{Code zq=q-nf;int bid=high_pr_find_block(zq,crossMap);Code base=bid?crossMap.end[bid-1]:0,loc=zq-base;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lc=x.stride,k=uint32_t(loc/lc),lr=uint32_t(loc-Code(k)*lc);unsigned long long r=D_HP_CROSSLL_REC[crossMap.sel_off[bid]+k];uint32_t hr=uint32_t(r)&M,bhr=uint32_t(r>>B)&M;int depth=int((r>>(2*B))&63u);Count c=mainv[x.off+Code(hr)*x.stride+lr];if(!c)continue;uint32_t code=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];int match=-1,s=depth;
#pragma unroll
            for(int z=L-1;z>=0;--z){auto v=MateValue((code>>(2*z))&3u);if(v==R){if(--s==0){match=z;break;}}else if(v==oneesan::gridfp::L)++s;}if(match<0)continue;uint32_t nc=(code&~(3u<<(2*match)))|(uint32_t(::L)<<(2*match));uint32_t nlr=D_F_LOW_DENSE_PACKED_RANK[nc]&LM;int nh=int(x.hs)-2;FBlock bx=D_F_BLOCK_BLOCKS[nh];atomic_add_mod(blockv+bx.off+Code(bhr)*bx.stride+nlr,c);}}
}


#ifdef GRIDFP_VALIDATE_HPR
__global__ void validate_high_owner_kernel(HighPreRankMap map,int p,unsigned long long*bad){constexpr uint32_t B=20,M=(1u<<B)-1u;Code total=map.nblocks?map.end[map.nblocks-1]:0;for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){int bid=high_pr_find_block(q,map);Code base=bid?map.end[bid-1]:0,loc=q-base;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lc=x.stride,k=uint32_t(loc/lc),lr=uint32_t(loc-Code(k)*lc);auto r=D_HP_OWNER_REC[map.sel_off[bid]+k];uint32_t hr=uint32_t(r)&M,mhr=uint32_t(r>>B)&M,bhr=uint32_t(r>>(2*B))&M,ty=uint32_t(r>>(3*B))&3u,tc=uint32_t(r>>(3*B+2))&3u;Code i=x.off+Code(hr)*x.stride+lr;MateID m=factor_unrank_main(i);auto w=oneesan::gridfp::mpair(m,p);uint32_t ety=(w==oneesan::gridfp::NN?0u:w==oneesan::gridfp::NR?1u:2u);MateID t=oneesan::gridfp::msetpair(m,p,w==oneesan::gridfp::NN?oneesan::gridfp::LR:w==oneesan::gridfp::NR?oneesan::gridfp::RN:oneesan::gridfp::LN);Code ej=factor_rank_main(t),ed=factor_rank_block(oneesan::gridfp::mshrink(m,p));int td=(tc==uint32_t(::L)?1:tc==uint32_t(R)?-1:0),the=int(x.hs)-td;FBlock dx=D_F_MAIN_BLOCKS[3*the+int(tc)],bx=D_F_BLOCK_BLOCKS[x.hs];Code j=dx.off+Code(mhr)*dx.stride+lr,dj=bx.off+Code(bhr)*bx.stride+lr;if(ety!=ty||j!=ej||dj!=ed){auto z=atomicAdd(bad,1ULL);if(z<8)printf("HOWNER p=%d bid=%d i=%llu w=%u ty=%u/%u j=%llu/%llu d=%llu/%llu xhe=%u xhs=%u tc=%u hr=%u lr=%u mhr=%u bhr=%u\\n",p,bid,(unsigned long long)i,(unsigned)w,ty,ety,(unsigned long long)j,(unsigned long long)ej,(unsigned long long)dj,(unsigned long long)ed,x.he,x.hs,tc,hr,lr,mhr,bhr);}}}
__global__ void validate_high_fast_kernel(HighPreRankMap map,int p,unsigned long long*bad){constexpr uint32_t B=20,M=(1u<<B)-1u;Code total=map.nblocks?map.end[map.nblocks-1]:0;for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){int bid=high_pr_find_block(q,map);Code base=bid?map.end[bid-1]:0,loc=q-base;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lc=x.stride,k=uint32_t(loc/lc),lr=uint32_t(loc-Code(k)*lc);auto r=D_HP_CLOSURE_REC[map.sel_off[bid]+k];uint32_t hr=uint32_t(r)&M,bhr=uint32_t(r>>B)&M;Code i=x.off+Code(hr)*x.stride+lr;MateID m=factor_unrank_main(i);auto iz=oneesan::gridfp::include_horizontal(m,TARGET_W,p);Code ed=(iz.valid&&iz.blocked)?factor_rank_block(iz.mate):~Code(0);FBlock bx=D_F_BLOCK_BLOCKS[x.hs];Code dj=bx.off+Code(bhr)*bx.stride+lr;if(!iz.valid||!iz.blocked||dj!=ed){auto z=atomicAdd(bad,1ULL);if(z<8)printf("HFAST p=%d bid=%d i=%llu valid=%d blk=%d d=%llu/%llu xhe=%u xhs=%u hr=%u lr=%u bhr=%u\\n",p,bid,(unsigned long long)i,int(iz.valid),int(iz.blocked),(unsigned long long)dj,(unsigned long long)ed,x.he,x.hs,hr,lr,bhr);}}}
__global__ void validate_high_cross_kernel(HighPreRankMap map,int p,unsigned long long*bad){constexpr uint32_t B=20,M=(1u<<B)-1u;constexpr int L=LOW_LUT_K,S=MAXW+2;constexpr uint32_t LM=(1u<<L)-1u;Code total=map.nblocks?map.end[map.nblocks-1]:0;for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){int bid=high_pr_find_block(q,map);Code base=bid?map.end[bid-1]:0,loc=q-base;FBlock x=D_F_MAIN_BLOCKS[bid];uint32_t lc=x.stride,k=uint32_t(loc/lc),lr=uint32_t(loc-Code(k)*lc);auto r=D_HP_CROSSLL_REC[map.sel_off[bid]+k];uint32_t hr=uint32_t(r)&M,bhr=uint32_t(r>>B)&M;int depth=int((r>>(2*B))&63u);Code i=x.off+Code(hr)*x.stride+lr;MateID m=factor_unrank_main(i);auto iz=oneesan::gridfp::include_horizontal(m,TARGET_W,p);Code ed=(iz.valid&&iz.blocked)?factor_rank_block(iz.mate):~Code(0);uint32_t code=D_F_LOW_MASK_CODES[D_F_LOW_MASK_OFF[size_t(D_F_MASK)*S+x.hs]+lr];int match=-1,ss=depth;for(int zz=L-1;zz>=0;--zz){auto v=MateValue((code>>(2*zz))&3u);if(v==R){if(--ss==0){match=zz;break;}}else if(v==oneesan::gridfp::L)++ss;}Code dj=~Code(0);if(match>=0){uint32_t nc=(code&~(3u<<(2*match)))|(uint32_t(::L)<<(2*match));uint32_t nlr=D_F_LOW_DENSE_PACKED_RANK[nc]&LM;int nh=int(x.hs)-2;FBlock bx=D_F_BLOCK_BLOCKS[nh];dj=bx.off+Code(bhr)*bx.stride+nlr;}if(!iz.valid||!iz.blocked||dj!=ed){auto z=atomicAdd(bad,1ULL);if(z<8){uint32_t shc=D_F_HIGH_ALL_CODES[D_F_HIGH_ALL_OFF[x.he]+hr];uint32_t ehc=iz.valid?uint32_t((iz.mate>>(2*LOW_LUT_K))&((1u<<(2*HIGH_LUT_K))-1u)):0;uint32_t ep=iz.valid?D_F_HIGH_PACKED_RANK[ehc]:0;uint32_t elc=iz.valid?uint32_t(iz.mate)&((1u<<(2*LOW_LUT_K))-1u):0;uint32_t cnc=0;if(match>=0)cnc=(code&~(3u<<(2*match)))|(uint32_t(::L)<<(2*match));printf("HCROSS p=%d bid=%d i=%llu valid=%d blk=%d match=%d depth=%d d=%llu/%llu xhe=%u xhs=%u hr=%u lr=%u bhr=%u shc=%x ehc=%x ehr=%u srcLC=%x gotLC=%x expLC=%x gotLR=%u expLR=%u recdiff=%lld\\n",p,bid,(unsigned long long)i,int(iz.valid),int(iz.blocked),match,depth,(unsigned long long)dj,(unsigned long long)ed,x.he,x.hs,hr,lr,bhr,shc,ehc,ep>>HIGH_LUT_K,code,cnc,elc,match>=0?(D_F_LOW_DENSE_PACKED_RANK[cnc]&LM):0,D_F_LOW_DENSE_PACKED_RANK[elc]&LM,(long long)dj-(long long)ed);}}}}
#endif
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){auto launchN=[&](Code n,auto fn,const char*w){if(!n)return;int nb=int(std::min<Code>(65535,(n+threads-1)/threads));fn(nb);ck(cudaGetLastError(),w);};
            if(fixLow){auto om=make_high_prerank_map(fmb,p,0),cm=make_high_prerank_map(fmb,p,1),xm=make_high_prerank_map(fmb,p,2);Code on=om.nblocks?om.end[om.nblocks-1]:0,cn=cm.nblocks?cm.end[cm.nblocks-1]:0,xn=xm.nblocks?xm.end[xm.nblocks-1]:0;
#ifdef GRIDFP_VALIDATE_HPR
                {unsigned long long *dbad=nullptr,z=0;ck(cudaMalloc(&dbad,sizeof(z)),"val bad");ck(cudaMemset(dbad,0,sizeof(z)),"val clear");int vb=std::max(1,int(std::min<Code>(65535,(std::max({on,cn,xn})+threads-1)/threads)));if(on)validate_high_owner_kernel<<<vb,threads>>>(om,p,dbad);if(cn)validate_high_fast_kernel<<<vb,threads>>>(cm,p,dbad);if(xn)validate_high_cross_kernel<<<vb,threads>>>(xm,p,dbad);ck(cudaDeviceSynchronize(),"validate HPR");ck(cudaMemcpy(&z,dbad,sizeof(z),cudaMemcpyDeviceToHost),"val copy");cudaFree(dbad);if(z){std::cerr<<"HPR VALIDATION FAIL p="<<p<<" bad="<<z<<" group="<<pg.g<<"\n";std::exit(91);}}
#endif
                launchN(on,[&](int nb){factor_high_prerank_owner_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,om);},"factor high prerank owner");launchN(cn+xn,[&](int nb){factor_high_prerank_closure_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,cm,xm);},"factor high prerank closure");}
            else{auto om=make_prerank_orbit_map(fmb,p,0);Code on=om.nblocks?om.end[om.nblocks-1]:0;launchN(on,[&](int nb){factor_prerank_owner_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,om,p);},"factor prerank owner");auto cm=make_prerank_orbit_map(fmb,p,1),sm=make_prerank_orbit_map(fmb,p,2);Code cn=cm.nblocks?cm.end[cm.nblocks-1]:0,sn=sm.nblocks?sm.end[sm.nblocks-1]:0;launchN(cn+sn,[&](int nb){factor_prerank_closure_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,cm,sm,p);},"factor prerank fused closure");}
        }
    ...
    ck(cudaGetLastError(),"orbit scatter");ck(cudaDeviceSynchronize(),"orbit group sync");auto tScatter=std::chrono::steady_clock::now();c.groups++;c.gather_s+=std::chrono::duration<double>(tGather-t0).count();c.transition_s+=std::chrono::duration<double>(tTransition-tGather).count();c.scatter_s+=std::chrono::duration<double>(tScatter-tTransition).count();c.active+=std::chrono::duration<double>(tScatter-t0).count();
}
...
int main(int argc,char**argv){
    ...
    if(const char*e=std::getenv("GRIDFP_PLAN_ONLY");e&&std::atoi(e)!=0){
        ...
        uint64_t sparseBytes=0;uint64_t preRankBytes=uint64_t(G_PRERANK.owner_rec.size()+G_PRERANK.closure_rec.size()+G_PRERANK.highrr_rec.size()+G_HPR.owner_rec.size()+G_HPR.closure_rec.size()+G_HPR.crossll_rec.size())*8;uint64_t factorBytes=vb4(G_FACTOR.low_all_codes)+vb4(G_FACTOR.low_mask_codes)+vb4(G_FACTOR.low_mask_off)+lowVmmBytes+vb4(G_FACTOR.high_all_codes)+vb4(G_FACTOR.high_mask_codes)+vb4(G_FACTOR.high_mask_off)+highVmmBytes+vb8(G_FACTOR.high_main_base)+vb8(G_FACTOR.high_block_base)+sparseBytes+preRankBytes;
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-fullprerank-orbit-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prO[d])cudaFree(prO[d]);if(prC[d])cudaFree(prC[d]);if(prH[d])cudaFree(prH[d]);if(hpO[d])cudaFree(hpO[d]);if(hpC[d])cudaFree(hpC[d]);if(hpX[d])cudaFree(hpX[d]);}
}
