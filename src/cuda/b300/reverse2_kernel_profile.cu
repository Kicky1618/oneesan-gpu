...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;MateID*dMate=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0,gather_s=0,transition_s=0,scatter_s=0;
    double r2_main_ms=0,r2_block_ms=0,r1_main_ms=0,r1_block_ms=0;uint64_t groups=0;
    cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
    cudaEvent_t pm0=nullptr,pm1=nullptr,pb0=nullptr,pb1=nullptr;
    void init(int d,Count mod,Count**mp,Count**bp,Code mc,Code bc,int ng){dev=d;ck(cudaSetDevice(dev),"set init");ck(cudaMemcpyToSymbol(D_FULL_DP,H_DP,sizeof(H_DP)),"full dp");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"mod");ck(cudaMemcpyToSymbol(D_MAIN_PTR,mp,sizeof(Count*)*MAXGPU),"main ptrs");ck(cudaMemcpyToSymbol(D_BLOCK_PTR,bp,sizeof(Count*)*MAXGPU),"block ptrs");ck(cudaMemcpyToSymbol(D_MAIN_CHUNK,&mc,sizeof(mc)),"main chunk");ck(cudaMemcpyToSymbol(D_BLOCK_CHUNK,&bc,sizeof(bc)),"block chunk");ck(cudaMemcpyToSymbol(D_NGPU,&ng,sizeof(ng)),"ngpu");ck(cudaStreamCreateWithFlags(&sMain,cudaStreamNonBlocking),"stream main");ck(cudaStreamCreateWithFlags(&sBlock,cudaStreamNonBlocking),"stream block");ck(cudaEventCreateWithFlags(&copyDone,cudaEventDisableTiming),"event copy");ck(cudaEventCreateWithFlags(&clearDone,cudaEventDisableTiming),"event clear");ck(cudaEventCreateWithFlags(&mainDone,cudaEventDisableTiming),"event main");ck(cudaEventCreateWithFlags(&blockDone,cudaEventDisableTiming),"event block");
        ck(cudaEventCreate(&pm0),"pm0");ck(cudaEventCreate(&pm1),"pm1");ck(cudaEventCreate(&pb0),"pb0");ck(cudaEventCreate(&pb1),"pb1");}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;){
        bool wasR2=(p>2 && p-1>=wp.p_lo); bool wasR1=(!wasR2 && p>1);
        if(wasR2){
            if(ms.size)ck(cudaEventRecord(c.pm0,c.sMain),"r2m0"); if(ds.size)ck(cudaEventRecord(c.pb0,c.sBlock),"r2b0");
            ...
            if(ms.size)ck(cudaEventRecord(c.pm1,c.sMain),"r2m1"); if(ds.size)ck(cudaEventRecord(c.pb1,c.sBlock),"r2b1");
            p-=2;
        }else{
            if(p>1){
                if(ms.size)ck(cudaEventRecord(c.pm0,c.sMain),"r1m0"); if(ds.size)ck(cudaEventRecord(c.pb0,c.sBlock),"r1b0");
                ...
                if(ms.size)ck(cudaEventRecord(c.pm1,c.sMain),"r1m1"); if(ds.size)ck(cudaEventRecord(c.pb1,c.sBlock),"r1b1");
            }else{
        ...
        if(wasR2||wasR1){ck(cudaStreamSynchronize(c.sMain),"prof msync");ck(cudaStreamSynchronize(c.sBlock),"prof bsync");float mms=0,bms=0;if(ms.size)ck(cudaEventElapsedTime(&mms,c.pm0,c.pm1),"prof melapsed");if(ds.size)ck(cudaEventElapsedTime(&bms,c.pb0,c.pb1),"prof belapsed");if(wasR2){c.r2_main_ms+=mms;c.r2_block_ms+=bms;}else{c.r1_main_ms+=mms;c.r1_block_ms+=bms;}}
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"set residue reset");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"set modulus");if(ml[d])ck(cudaMemset(mp[d],0,size_t(ml[d])*sizeof(Count)),"zero main");if(bl[d])ck(cudaMemset(bp[d],0,size_t(bl[d])*sizeof(Count)),"zero block");ck(cudaDeviceSynchronize(),"zero sync");ctx[d].active=0;ctx[d].gather_s=ctx[d].transition_s=ctx[d].scatter_s=0;ctx[d].r2_main_ms=ctx[d].r2_block_ms=ctx[d].r1_main_ms=ctx[d].r1_block_ms=0;ctx[d].groups=0;}
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0,r2m=0,r2b=0,r1m=0,r1b=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;r2m+=c.r2_main_ms;r2b+=c.r2_block_ms;r1m+=c.r1_main_ms;r1b+=c.r1_block_ms;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" r2_main_ms="<<r2m<<" r2_block_ms="<<r2b<<" r1_main_ms="<<r1m<<" r1_block_ms="<<r1b<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }
