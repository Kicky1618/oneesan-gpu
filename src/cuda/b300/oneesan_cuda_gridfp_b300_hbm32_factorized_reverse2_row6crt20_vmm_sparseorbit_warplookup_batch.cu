...
__global__ void factor_sparse_orbit_kernel(Count*mainv,Count*blockv,const MateID*mates,SparseOrbitMap map,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){
        unsigned mask=__activemask();int lane=threadIdx.x&31,bid=0;if(lane==0)bid=sparse_find_block(q,map);bid=__shfl_sync(mask,bid,0);
        if(q>=map.end[bid])bid=sparse_find_block(q,map);Code base=bid?map.end[bid-1]:0,loc=q-base;uint32_t sc=map.sel_count[bid];
