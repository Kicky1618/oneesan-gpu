...
static PreRankOrbitTablesHost build_prerank_orbit_tables(){
    ...
    double mib=(double(z.owner_rec.size()+z.highrr_rec.size()+z.closure_inv_rec.size())*8.0+double(z.closure_inv_src.size())*4.0)/(1<<20);
...
static HighPreRankTablesHost build_high_prerank_tables(){
    ...
    double mib=(double(z.owner_rec.size()+z.crossll_rec.size()+z.closure_inv_rec.size())*8.0+double(z.closure_inv_src.size())*4.0)/(1<<20);std::cerr<<"high pre-rank owner="<<z.owner_rec.size()<<" closure_fast="<<z.closure_inv_src.size()<<" closure_inv="<<z.closure_inv_rec.size()<<" closure_crossll="<<z.crossll_rec.size()<<" device_mib="<<mib<<"\n";z.closure_rec.clear();z.closure_rec.shrink_to_fit();return z;
}
...
static void build_cross_match_words(){
    ...
    std::cerr<<"cross match LUT low_mib="<<double(G_CROSS_LOW_MATCH.size()*8)/(1<<20)<<" high_mib="<<double(G_CROSS_HIGH_MATCH.size()*8)/(1<<20)<<"\n";
}
...
__global__ void factor_prerank_highrr_kernel(Count*mainv,Count*blockv,PreRankOrbitMap map,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){
        ...
        uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];uint32_t shp=D_F_HIGH_PACKED_RANK[hc],har=shp>>H;unsigned long long mw=D_CROSS_HIGH_MATCH[D_F_HIGH_ALL_OFF[x.he]+har];int match=(depth>=1&&depth<=H)?int((mw>>(4*(depth-1)))&15u)-1:-1;if(match<0)continue;uint32_t nhc=(hc&~(3u<<(2*match)))|(uint32_t(R)<<(2*match));uint32_t hp=D_F_HIGH_PACKED_RANK[nhc],nhr=hp&HM;int nhe=int(x.he)-2;
...
__global__ void factor_prerank_closure_inv_combined_kernel(Count*mainv,Count*blockv,PreRankOrbitMap invMap,PreRankOrbitMap highMap,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){
        ...
        else{Code zq=q-ni;int bid=prerank_find_block(zq,highMap);Code base=bid?highMap.end[bid-1]:0,loc=zq-base;uint32_t sc=highMap.sel_count[bid],hr=uint32_t(loc/sc),k=uint32_t(loc-Code(hr)*sc);unsigned long long rec=D_PR_HIGHRR_REC[highMap.sel_off[bid]+k];uint32_t lr=uint32_t(rec)&M,dlr=uint32_t(rec>>B)&M;int depth=int((rec>>(2*B))&63u);FBlock x=D_F_MAIN_BLOCKS[bid];Code i=x.off+Code(hr)*x.stride+lr;Count c=mainv[i];if(!c)continue;uint32_t hc=D_F_HIGH_MASK_CODES[D_F_HIGH_MASK_OFF[size_t(D_F_MASK)*S+x.he]+hr];uint32_t shp=D_F_HIGH_PACKED_RANK[hc],har=shp>>H;unsigned long long mw=D_CROSS_HIGH_MATCH[D_F_HIGH_ALL_OFF[x.he]+har];int match=(depth>=1&&depth<=H)?int((mw>>(4*(depth-1)))&15u)-1:-1;if(match<0)continue;uint32_t nhc=(hc&~(3u<<(2*match)))|(uint32_t(R)<<(2*match));uint32_t hp=D_F_HIGH_PACKED_RANK[nhc],nhr=hp&HM;int nhe=int(x.he)-2;if(p==1){FBlock dx=D_F_MAIN_BLOCKS[3*nhe+int(x.c)];atomic_add_mod(mainv+dx.off+Code(nhr)*dx.stride+dlr,c);}else{FBlock bx=D_F_BLOCK_BLOCKS[nhe];atomic_add_mod(blockv+bx.off+Code(nhr)*bx.stride+dlr,c);}}
    }
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){auto launchN=[&](Code n,auto fn,const char*w){if(!n)return;int nb=int(std::min<Code>(65535,(n+threads-1)/threads));fn(nb);ck(cudaGetLastError(),w);};
            if(fixLow){auto om=make_high_prerank_map(fmb,p,0),im=make_high_prerank_map(fmb,p,3),xm=make_high_prerank_map(fmb,p,2);Code on=om.nblocks?om.end[om.nblocks-1]:0,in=im.nblocks?im.end[im.nblocks-1]:0,xn=xm.nblocks?xm.end[xm.nblocks-1]:0;launchN(on,[&](int nb){factor_high_prerank_owner_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,om);},"factor high prerank owner");launchN(in+xn,[&](int nb){factor_high_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,im,xm);},"factor high prerank inverse closure");}
            else{auto om=make_prerank_orbit_map(fmb,p,0);Code on=om.nblocks?om.end[om.nblocks-1]:0;launchN(on,[&](int nb){factor_prerank_owner_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,om,p);},"factor prerank owner");auto im=make_prerank_orbit_map(fmb,p,3),sm=make_prerank_orbit_map(fmb,p,2);Code in=im.nblocks?im.end[im.nblocks-1]:0,sn=sm.nblocks?sm.end[sm.nblocks-1]:0;launchN(in+sn,[&](int nb){factor_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,im,sm,p);},"factor prerank inverse closure");}
        }
...
int main(int argc,char**argv){
    ...
    G_PRERANK=build_prerank_orbit_tables();G_HPR=build_high_prerank_tables();build_cross_match_words();
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-fullprerank-orbit-matchlut-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prO[d])cudaFree(prO[d]);if(prCI[d])cudaFree(prCI[d]);if(prCIS[d])cudaFree(prCIS[d]);if(prH[d])cudaFree(prH[d]);if(hpO[d])cudaFree(hpO[d]);if(hpCI[d])cudaFree(hpCI[d]);if(hpCIS[d])cudaFree(hpCIS[d]);if(hpX[d])cudaFree(hpX[d]);if(cmL[d])cudaFree(cmL[d]);if(cmH[d])cudaFree(cmH[d]);}
}
