...
int main(int argc,char**argv){
    ...
    ReverseLowDescHost rlow=build_reverse_low_descriptors(storage,layout);ReverseHighDescHost rhigh=build_reverse_high_descriptors(storage,layout);ReverseOrbitHost rlo=build_reverse_orbit(storage,layout,true),rhi=build_reverse_orbit(storage,layout,false);SnakeReverseHost reverse=bsn_build_reverse(storage,layout,owner,rlow,rhigh,rlo,rhi);BucketReverseOrbitClosureAttachHost rattach=build_bucket_reverse_orbit_closure_attach(layout,reverse.atomic,reverse.fused);
