...
int main(int argc,char**argv){
    ...
    ReverseLowDescHost rlow=build_reverse_low_descriptors(storage,layout);ReverseHighDescHost rhigh=build_reverse_high_descriptors(storage,layout);ReverseOrbitHost rlo=build_reverse_orbit(storage,layout,true),rhi=build_reverse_orbit(storage,layout,false);ReverseBucketAtomicHost reverse=build_reverse_bucket_atomic(storage,layout,owner,rlow,rhigh,rlo,rhi);
    ...
    std::array<Count*,NG>base{};std::array<std::array<Count*,NG>,NG>slots{};std::array<BucketFusedDeviceTables,NG>ft;std::array<ReverseBucketAtomicDeviceTables,NG>rt;
    ...
    for(int row=0;row<W;++row){
        ...
        else{auto t=std::chrono::steady_clock::now();for(int g=0;g<NG;++g){ck(cudaSetDevice(g),"snake rlow set");bucket_launch_reverse_low_atomic(layout,threads,gx,gy);}bucket_sync_devices(NG);rl+=bsn_since(t);t=std::chrono::steady_clock::now();tx.transpose(tplan);ts+=bsn_since(t);t=std::chrono::steady_clock::now();for(int g=0;g<NG;++g){ck(cudaSetDevice(g),"snake rhigh set");bucket_launch_reverse_high_atomic(layout,threads,gx,gy);}bucket_sync_devices(NG);rh+=bsn_since(t);}
    ...
    std::cout<<"backend=gridfp-b300-bucket-snake-atomic-v0.1 n="<<n<<" residue="<<answer<<" modulus="<<mod<<" forward_high_s="<<fh<<" forward_low_s="<<fl<<" reverse_low_s="<<rl<<" reverse_high_s="<<rh<<" transpose_s="<<ts<<" wall_s="<<wall<<" transposes="<<tx.transposes<<" peer_gib="<<tx.peer_gib<<" final_layout="<<(final_lmajor?"L-major":"H-major")<<" reverse_closure_atomic=1 forward_closure_atomic=0\n";
