...
static Count half_central_join_single_gpu(int W,int threads){
    ...
    int tile=2048;if(const char*e=std::getenv("GRIDFP_HALF_MASK_TILE"))tile=std::max(32,std::atoi(e));double gather_s=0,factor_s=0,dot_s=0;uint64_t allmasks=0,coeffops=0;
...
int main(int argc,char**argv){
    ...
    bool halfMode=true;if(const char*e=std::getenv("GRIDFP_HALF_MEANDER"))halfMode=std::atoi(e)!=0;if(halfMode&&ng!=1){std::cerr<<"half prototype currently NGPU=1 only\n";return 67;}
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        Count ans=0;if(halfMode)ans=half_central_join_single_gpu(W,threads);
