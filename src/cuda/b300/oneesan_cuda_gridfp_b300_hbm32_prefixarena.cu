...
__global__ void gather_main_kernel(Count*out,uint32_t*prefix,int cutoff,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){Code g;MateID m=unrank_group_global_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP,g);out[i]=global_load_main(g);if(prefix)prefix[i]=(uint32_t)(m>>(2*cutoff));}}
...
__global__ void main_group_kernel(const Count*in,const uint32_t*prefix,int cutoff,Code n,Count*out_main,Count*out_block,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        Count c=in[i];if(!c)continue;
        MateID m;MateValuePair w;
        if(prefix){
            m=MateID(prefix[i])<<(2*cutoff);w=mpair(m,p);
            if(w==NN){MateID t=msetpair(m,p,LR);atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);continue;}
            if(w==RN){MateID t=msetpair(m,p,NR);atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);continue;}
            if(w==LN){MateID t=msetpair(m,p,NL);atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);continue;}
            m=unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);w=mpair(m,p);
        }else{m=unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);w=mpair(m,p);}
        switch(w){
        case NN:{MateID t=msetpair(m,p,LR);atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}
        case NR:case NL:{if(p==1){MateID t=msetpair(m,p,w==NR?RN:LN);atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);}else{MateID t=mshrink(m,p);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}
        case RN:{MateID t=msetpair(m,p,NR);atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}
        case LN:{MateID t=msetpair(m,p,NL);atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}
        case LL:{MateID t=msetpair(m,p,NN);int q=p-1,dep=1;while(dep){--q;auto v=mget(t,q);if(v==L)++dep;else if(v==R)--dep;}t=mset(t,q,L);if(p==1)atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}
        case RR:{MateID t=msetpair(m,p,NN);int q=p,dep=1;while(dep){++q;auto v=mget(t,q);if(v==L)--dep;else if(v==R)++dep;}t=mset(t,q,R);if(p==1)atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}
        case RL:{MateID t=msetpair(m,p,NN);if(p==1)atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}
        default:break;
        }
    }
}
...
struct DeviceCtx{
    int dev=-1;
    uint8_t*arena=nullptr;size_t capArena=0;
    Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;
    uint32_t*dPrefix=nullptr;
    PeerInterval*dIM=nullptr,*dID=nullptr;
    size_t capIM=0,capID=0,maxIntervals=0;
    double active=0;uint64_t groups=0;
    cudaStream_t sMain=nullptr,sBlock=nullptr;
    cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
    void init(int d,Count mod,Count**mp,Count**bp,Code mc,Code bc,int ng){
        dev=d;ck(cudaSetDevice(dev),"set init");
        ck(cudaMemcpyToSymbol(D_FULL_DP,H_DP,sizeof(H_DP)),"full dp");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"mod");
        ck(cudaMemcpyToSymbol(D_MAIN_PTR,mp,sizeof(Count*)*MAXGPU),"main ptrs");ck(cudaMemcpyToSymbol(D_BLOCK_PTR,bp,sizeof(Count*)*MAXGPU),"block ptrs");
        ck(cudaMemcpyToSymbol(D_MAIN_CHUNK,&mc,sizeof(mc)),"main chunk");ck(cudaMemcpyToSymbol(D_BLOCK_CHUNK,&bc,sizeof(bc)),"block chunk");ck(cudaMemcpyToSymbol(D_NGPU,&ng,sizeof(ng)),"ngpu");
        ck(cudaStreamCreateWithFlags(&sMain,cudaStreamNonBlocking),"stream main");ck(cudaStreamCreateWithFlags(&sBlock,cudaStreamNonBlocking),"stream block");
        ck(cudaEventCreateWithFlags(&copyDone,cudaEventDisableTiming),"event copy");ck(cudaEventCreateWithFlags(&clearDone,cudaEventDisableTiming),"event clear");
        ck(cudaEventCreateWithFlags(&mainDone,cudaEventDisableTiming),"event main");ck(cudaEventCreateWithFlags(&blockDone,cudaEventDisableTiming),"event block");
    }
    void ensure(Code m,Code b,bool usePrefix,size_t im,size_t id){
        ck(cudaSetDevice(dev),"set ensure");
        auto al=[](size_t x){return (x+255)&~size_t(255);};
        size_t aBytes=al(size_t(m)*sizeof(Count)),dBytes=al(size_t(b)*sizeof(Count));
        size_t pBytes=usePrefix?al(size_t(m)*sizeof(uint32_t)):0;
        size_t need=2*aBytes+2*dBytes+pBytes;
        if(need>capArena){if(arena)cudaFree(arena);capArena=need;ck(cudaMalloc(&arena,capArena),"scratch arena");}
        size_t off=0;dA=(Count*)(arena+off);off+=aBytes;dB=(Count*)(arena+off);off+=aBytes;dD=(Count*)(arena+off);off+=dBytes;dE=(Count*)(arena+off);off+=dBytes;
        dPrefix=usePrefix?(uint32_t*)(arena+off):nullptr;
        if(im>capIM){if(dIM)cudaFree(dIM);capIM=im;ck(cudaMalloc(&dIM,capIM*sizeof(PeerInterval)),"interval main");}
        if(id>capID){if(dID)cudaFree(dID);capID=id;ck(cudaMalloc(&dID,capID*sizeof(PeerInterval)),"interval block");}
        maxIntervals=std::max({maxIntervals,im,id});
    }
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads){
    ...
    int cutoff=wp.p_lo-1;bool usePrefix=(wp.p_hi==W-1)&&!pg.use_mi&&cutoff>=0&&(W-cutoff)<=16;
    c.ensure(ms.size,ds.size,usePrefix,pg.mi.size(),pg.di.size());
    ...
    if(ms.size){if(pg.use_mi)interval_io_kernel<false,false><<<interval_blocks(pg.mi.size(),threads),threads>>>(c.dA,c.dIM,pg.mi.size());else gather_main_kernel<<<bm,threads>>>(c.dA,usePrefix?c.dPrefix:nullptr,cutoff,ms.size);}
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        ...
        if(ms.size)main_group_kernel<<<bm,threads,0,c.sMain>>>(cur,usePrefix?c.dPrefix:nullptr,cutoff,ms.size,nxt,dnext,p);
...
int main(int argc,char**argv){int n=argc>1?std::atoi(argv[1]):16;unsigned long long raw_mod=argc>2?std::strtoull(argv[2],nullptr,10):4294967291ULL;if(raw_mod<2||raw_mod>0xffffffffULL){std::cerr<<"HBM32 modulus must be in [2, 4294967295], got "<<raw_mod<<"\n";return 1;}Count mod=(Count)raw_mod;int target_mib=argc>3?std::atoi(argv[3]):16384;int max_window=argc>4?std::atoi(argv[4]):14;int requested=argc>5?std::atoi(argv[5]):0;int W=n+1;if(n<2||W>MAXW){std::cerr<<"n=2..27\n";return 1;}if(W!=TARGET_W){std::cerr<<"specialized for width "<<TARGET_W<<" (n="<<(TARGET_W-1)<<")\n";return 1;}build_full_dp();int visible=0;ck(cudaGetDeviceCount(&visible),"count");int ng=requested<=0?visible:std::min(requested,visible);if(ng<1||ng>MAXGPU){std::cerr<<"need 1..8 GPUs\n";return 2;}
    ...
    int threads=256,maxgroups=0;auto prep0=std::chrono::steady_clock::now();
    std::vector<PreparedWindow> schedule;
    for(int hi=W-1;hi>=1;){
        WindowPlan wp;bool found=false;
        for(int lo=std::max(1,hi-max_window+1);lo<=hi;++lo){auto t=plan_window(W,hi,lo,target);if(t.max_bytes&&t.max_bytes<=target){wp=std::move(t);found=true;break;}}
        if(!found){std::cerr<<"cannot fit window hi="<<hi<<"\n";return 4;}
        int k=wp.fixed_pos.size();int nj=1<<k;maxgroups=std::max(maxgroups,nj);
        PreparedWindow pw;pw.wp=wp;pw.groups.reserve(nj);
        for(int g=0;g<nj;++g)pw.groups.push_back(prepare_group(W,pw.wp,g,mc,bc,ng));
    ...
    for(int row=0;row<W;++row){
        for(auto const&pw:schedule){
            ...
            for(int d=0;d<ng;++d)ths.emplace_back([&,d]{for(;;){int q=next.fetch_add(1,std::memory_order_relaxed);if(q>=nj)break;process_group(ctx[d],W,pw.wp,pw.groups[q],threads);}});
            for(auto&t:ths)t.join();++done_windows;
        }
        std::cerr<<"row "<<row+1<<"/"<<W<<" windows="<<done_windows<<"\n";
    }
    int total_windows=done_windows;
    double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();Code fg=rank_full(MateID(R),W);int fo=int(fg/mc);Count ans=0;cudaSetDevice(fo);ck(cudaMemcpy(&ans,mp[fo]+(fg-Code(fo)*mc),sizeof(ans),cudaMemcpyDeviceToHost),"answer");double mx=0,sum=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;maxIntervals=std::max(maxIntervals,c.maxIntervals);std::cerr<<"gpu "<<c.dev<<" groups="<<c.groups<<" active_s="<<c.active<<" max_intervals="<<c.maxIntervals<<"\n";}std::cout<<"backend=gridfp-b300-hbm32-prefixarena n="<<n<<" residue="<<ans<<" modulus="<<mod<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" auth_gib="<<double(mainN+blockN)*sizeof(Count)/(1ull<<30)<<" auth_per_gpu_gib="<<double(mainN+blockN)*sizeof(Count)/ng/(1ull<<30)<<" scratch_target_mib="<<effective_target_mib<<" scratch_requested_mib="<<target_mib<<" reserve_mib="<<reserve_mib<<" windows="<<total_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<"\n";
