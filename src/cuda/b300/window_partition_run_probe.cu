...
int main(int argc,char**argv){
    ...
    {
        int windowParts=2;if(const char*e=std::getenv("GRIDFP_WINDOW_PARTS"))windowParts=std::max(2,std::min(4,std::atoi(e)));std::vector<std::pair<int,int>> ranges;if(windowParts==2)ranges={{W-1,LOW_LUT_K+1},{LOW_LUT_K,1}};else{int pHi=W-1;for(int z=0;z<windowParts;++z){int remain=pHi,left=windowParts-z,len=(remain+left-1)/left,pLo=std::max(1,pHi-len+1);ranges.push_back({pHi,pLo});pHi=pLo-1;}}
        for(auto const& r:ranges){
            int hi=r.first,lo=r.second;WindowPlan wp;wp.p_hi=hi;wp.p_lo=lo;wp.fixed_pos=window_candidates(W,hi,lo);
