...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target,bool sourceMode){
    ...
    if(sourceMode){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){
            if(ms.size)ck(cudaMemcpyAsync(nxt,cur,size_t(ms.size)*sizeof(Count),cudaMemcpyDeviceToDevice,c.sMain),"hybrid identity");
            if(ds.size)ck(cudaMemsetAsync(dnext,0,size_t(ds.size)*sizeof(Count),c.sBlock),"hybrid clear D");
            ...
            ck(cudaGetLastError(),"sparse source transition");
...
int main(int argc,char**argv){
    ...
    int sparseRows=0;if(const char*e=std::getenv("GRIDFP_SOURCE_ROWS"))sparseRows=std::max(0,std::atoi(e));std::cerr<<"source_rows="<<sparseRows<<"\n";
