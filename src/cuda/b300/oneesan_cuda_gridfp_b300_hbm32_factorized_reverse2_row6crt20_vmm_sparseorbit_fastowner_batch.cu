...
__global__ void factor_sparse_orbit_kernel(Count*mainv,Count*blockv,const MateID*mates,SparseOrbitMap map,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<total;q+=st){
        ...
        if constexpr(CLOSURE){
            ...
            constexpr int L=LOW_LUT_K;constexpr uint32_t LM=(1u<<(2*L))-1u;auto w=oneesan::gridfp::mpair(m,p);MateID b=oneesan::gridfp::mshrink(m,p);Code dj;
            if(p<L){uint32_t blc=uint32_t(b)&LM,blp=D_F_LOW_DENSE_PACKED_RANK[blc],blr=blp>>L;FBlock bx=D_F_BLOCK_BLOCKS[x.he];dj=bx.off+Code(hr)*bx.stride+blr;}else dj=factor_rank_block(b);
            Count d=blockv[dj];
            if(w==oneesan::gridfp::NN){MateID t=oneesan::gridfp::msetpair(m,p,oneesan::gridfp::LR);Code j;if(p<L){uint32_t tlc=uint32_t(t)&LM,tlp=D_F_LOW_DENSE_PACKED_RANK[tlc],tlr=tlp>>L;j=x.off+Code(hr)*x.stride+tlr;}else j=factor_rank_main(t);mainv[j]=add_mod_plain(mainv[j],c);mainv[i]=add_mod_plain(c,d);blockv[dj]=0;}
            else {MateID t=oneesan::gridfp::msetpair(m,p,w==oneesan::gridfp::NR?oneesan::gridfp::RN:oneesan::gridfp::LN);Code j;if(p<L){uint32_t tlc=uint32_t(t)&LM,tlp=D_F_LOW_DENSE_PACKED_RANK[tlc],tlr=tlp>>L;j=x.off+Code(hr)*x.stride+tlr;}else j=factor_rank_main(t);Count cc=mainv[j];Count all=add_mod_plain(add_mod_plain(c,cc),d);if(p==1){mainv[i]=all;mainv[j]=add_mod_plain(c,cc);blockv[dj]=0;}else{mainv[i]=all;blockv[dj]=c;}}
        }
    }
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    if(useOrbit){
        for(int p=wp.p_hi;p>=wp.p_lo;--p){
            ...
            Code on=om.nblocks?om.end[om.nblocks-1]:0,cn=cm.nblocks?cm.end[cm.nblocks-1]:0;
            int ob=int(std::min<Code>(65535,(on+threads-1)/threads)),cb=int(std::min<Code>(65535,(cn+threads-1)/threads));
            if(on)factor_sparse_orbit_kernel<false><<<std::max(1,ob),threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,om,p);
            ck(cudaGetLastError(),"factor sparse orbit");
            if(cn)factor_sparse_orbit_kernel<true><<<std::max(1,cb),threads,0,c.sMain>>>(cur,dcur,useMate?c.dMate:nullptr,cm,p);
            ck(cudaGetLastError(),"factor sparse closure");
        }
