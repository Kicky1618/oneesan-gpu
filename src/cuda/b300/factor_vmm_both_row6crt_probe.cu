...
struct SparseVmmU32 {
    CUdeviceptr base=0; size_t bytes=0,gran=0; std::vector<CUmemGenericAllocationHandle> handles; std::vector<size_t> offsets;
    uint32_t* ptr() const { return reinterpret_cast<uint32_t*>(base); }
    void destroy(){if(!base)return;for(size_t i=0;i<handles.size();++i){ckcu(cuMemUnmap(base+offsets[i],gran),"vmm unmap");ckcu(cuMemRelease(handles[i]),"vmm release");}ckcu(cuMemAddressFree(base,bytes),"vmm address free");base=0;handles.clear();offsets.clear();}
};
static SparseVmmU32 make_sparse_vmm_u32(int dev,const std::vector<uint32_t>&dense,const std::vector<uint32_t>&codes){
    ...
    ckcu(cuMemGetAllocationGranularity(&z.gran,&prop,CU_MEM_ALLOC_GRANULARITY_MINIMUM),"vmm granularity");size_t raw=dense.size()*sizeof(uint32_t);z.bytes=(raw+z.gran-1)/z.gran*z.gran;ckcu(cuMemAddressReserve(&z.base,z.bytes,z.gran,0,0),"vmm reserve");
    size_t elemsPer=z.gran/sizeof(uint32_t),np=(z.bytes/z.gran);std::vector<uint8_t>used(np,0);for(uint32_t c:codes)used[size_t(c)/elemsPer]=1;
    ...
    size_t mapped=0;for(size_t pg=0;pg<np;++pg)if(used[pg]){CUmemGenericAllocationHandle h;ckcu(cuMemCreate(&h,z.gran,&prop,0),"vmm create");size_t off=pg*z.gran;ckcu(cuMemMap(z.base+off,z.gran,0,h,0),"vmm map");ckcu(cuMemSetAccess(z.base+off,z.gran,&ad,1),"vmm access");size_t copy=std::min(z.gran,raw-off);do{auto ce=cudaMemcpy(reinterpret_cast<void*>(z.base+off),reinterpret_cast<const char*>(dense.data())+off,copy,cudaMemcpyHostToDevice);if(ce!=cudaSuccess){std::cerr<<"vmm copy: "<<cudaGetErrorString(ce)<<"\n";std::exit(1);}}while(0);z.handles.push_back(h);z.offsets.push_back(off);mapped+=z.gran;}
    std::cerr<<"factor low VMM virtual_mib="<<(z.bytes>>20)<<" mapped_mib="<<(mapped>>20)<<" pages="<<z.handles.size()<<" gran_kib="<<(z.gran>>10)<<"\n";return z;
}
...
int main(int argc,char**argv){
    ...
    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);}
}
