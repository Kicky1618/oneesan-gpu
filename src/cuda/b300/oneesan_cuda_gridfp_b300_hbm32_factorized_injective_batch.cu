...
__device__ __forceinline__ Count add_mod_plain_unique(Count a,Count b){Count mod=D_MOD;return a>=mod-b?a-(mod-b):a+b;}
__device__ __forceinline__ void plain_add_mod_unique(Count* p,Count v){if(v)*p=add_mod_plain_unique(*p,v);}

// p>1 source-scatter decomposition.  Formal proof in ReverseMainCore.lean
// establishes injectivity of the main→main branch; blocked→main is forced-N
// insertion.  NR/NL/RL→blocked have disjoint target markers R/L/N, so only
// LL/RR closure needs atomics.
__global__ void main_to_main_unique_kernel(const Count*in,const MateID*mates,Code n,Count*out,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=mates?mates[i]:factor_unrank_main(i);auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);if(z.valid&&!z.blocked)plain_add_mod_unique(out+factor_rank_main(z.mate),c);}
}
__global__ void blocked_to_main_unique_kernel(const Count*in,Code n,Count*out,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=factor_unrank_block(i);plain_add_mod_unique(out+factor_rank_main(oneesan::gridfp::blocked_exclude(m,p)),c);}
}
__global__ void main_to_block_simple_unique_kernel(const Count*in,const MateID*mates,Code n,Count*out,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=mates?mates[i]:factor_unrank_main(i);auto pr=oneesan::gridfp::mpair(m,p);if(!(pr==oneesan::gridfp::NR||pr==oneesan::gridfp::NL||pr==oneesan::gridfp::RL))continue;auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);if(z.valid&&z.blocked)out[factor_rank_block(z.mate)]=c;}
}
__global__ void main_to_block_closure_atomic_kernel(const Count*in,const MateID*mates,Code n,Count*out,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=mates?mates[i]:factor_unrank_main(i);auto pr=oneesan::gridfp::mpair(m,p);if(!(pr==oneesan::gridfp::LL||pr==oneesan::gridfp::RR))continue;auto z=oneesan::gridfp::include_horizontal(m,TARGET_W,p);if(z.valid&&z.blocked)atomic_add_mod(out+factor_rank_block(z.mate),c);}
}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(p>1){
            if(ms.size)ck(cudaMemcpyAsync(nxt,cur,size_t(ms.size)*sizeof(Count),cudaMemcpyDeviceToDevice,c.sMain),"identity unique");
            if(ds.size)ck(cudaMemsetAsync(dnext,0,size_t(ds.size)*sizeof(Count),c.sBlock),"clear unique block");
            if(ms.size)main_to_main_unique_kernel<<<bm,threads,0,c.sMain>>>(cur,useMate?c.dMate:nullptr,ms.size,nxt,p);
            if(ds.size)blocked_to_main_unique_kernel<<<bd,threads,0,c.sMain>>>(dcur,ds.size,nxt,p);
            if(ms.size&&ds.size)main_to_block_simple_unique_kernel<<<bm,threads,0,c.sBlock>>>(cur,useMate?c.dMate:nullptr,ms.size,dnext,p);
            if(ms.size&&ds.size)main_to_block_closure_atomic_kernel<<<bm,threads,0,c.sBlock>>>(cur,useMate?c.dMate:nullptr,ms.size,dnext,p);
        }else{
