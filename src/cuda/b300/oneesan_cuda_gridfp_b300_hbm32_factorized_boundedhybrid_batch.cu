...
#pragma unroll
        ...
        const Count*pv=D_R5_PREF+(size_t)ch*STRIDE;const Count*sv=D_R5_SUFF+(size_t)cl*STRIDE;Count acc=0,dim=r5_block_size(hs);for(int i=0;i<dim;++i)acc=r3_add_mod(acc,r3_mul_mod(pv[i],sv[i]));fullMain[grank-fullBase]=acc;}
}
...
#pragma unroll
        for(int pos=TARGET_W-1;pos>=0;--pos){int dig=0;Code a=D_BOUND_DP[pos][h];if(r<a)dig=0;else{r-=a;if(h>0){a=D_BOUND_DP[pos][h-1];if(r<a){dig=1;grank+=D_FULL_DP[pos][h];--h;}else{r-=a;dig=2;grank+=D_FULL_DP[pos][h]+D_FULL_DP[pos][h-1];++h;}}else{dig=2;grank+=D_FULL_DP[pos][h];++h;}}if(pos>=LO){ch=ch*3u+dig;if(pos==LO)hs=h;}else cl=cl*3u+dig;}meta=uint64_t(ch)|(uint64_t(cl)<<24)|(uint64_t(hs)<<48);}
        meta=__shfl_sync(mask,meta,leader);grank=__shfl_sync(mask,grank,leader);uint32_t ch=uint32_t(meta&0xffffffu),cl=uint32_t((meta>>24)&0xffffffu);int hs=int((meta>>48)&0xffu),dim=r5_block_size(hs);const Count*pv=D_R5_PREF+(size_t)ch*STRIDE;const Count*sv=D_R5_SUFF+(size_t)cl*STRIDE;Count acc=0;for(int i=lane;i<dim;i+=LANES)acc=r3_add_mod(acc,r3_mul_mod(pv[i],sv[i]));for(int off=LANES/2;off;off>>=1)acc=r3_add_mod(acc,__shfl_down_sync(mask,acc,off,LANES));if(lane==0)fullMain[grank-fullBase]=acc;}
}
...
static void init_direct_row5_lut_full_multi_gpu(int W,Count mod,int threads,Count**fullMain,const std::vector<Code>&mainLen,Code mainChunk,int ng){
    ...
    auto t0=std::chrono::steady_clock::now();std::vector<Count>pref,suff;std::thread tp([&]{pref=make_row5_prefix_lut_compact_direct(hi);});std::thread ts([&]{suff=make_row5_suffix_lut_compact_direct(lo);});tp.join();ts.join();double host_s=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
    std::vector<Code> cut(ng+1);cut[0]=0;cut[ng]=n;for(int d=1;d<ng;++d)cut[d]=row5_lower_bound_full_rank(n,Code(d)*mainChunk,W,dp);
    if(cut.front()!=0||cut.back()!=n){std::cerr<<"row5 shard cut endpoints invalid\n";std::exit(35);}
    Code mn=~Code(0),mx=0,fullN=H_DP[W][1];
    for(int d=0;d<ng;++d){
        if(cut[d]>cut[d+1]){std::cerr<<"row5 shard cuts not monotone at gpu="<<d<<"\n";std::exit(35);}
        Code z=cut[d+1]-cut[d],base=Code(d)*mainChunk,end=std::min<Code>(fullN,base+mainChunk);
        if(end<base||mainLen[d]!=end-base){std::cerr<<"row5 shard owner span mismatch gpu="<<d<<" base="<<base<<" end="<<end<<" local_cap="<<mainLen[d]<<"\n";std::exit(35);}
        Code first=z?row5_bounded_to_full_rank_host(cut[d],W,dp):base;
        Code last=z?row5_bounded_to_full_rank_host(cut[d+1]-1,W,dp):base;
        if(z&&(first<base||first>=end||last<first||last>=end)){std::cerr<<"row5 shard rank outside owner gpu="<<d<<" first="<<first<<" last="<<last<<" owner=["<<base<<","<<end<<")\n";std::exit(35);}
        if(cut[d]&&row5_bounded_to_full_rank_host(cut[d]-1,W,dp)>=base){std::cerr<<"row5 shard lower-bound predecessor failed gpu="<<d<<"\n";std::exit(35);}
        if(cut[d+1]<n&&row5_bounded_to_full_rank_host(cut[d+1],W,dp)<end){std::cerr<<"row5 shard upper-bound successor failed gpu="<<d<<"\n";std::exit(35);}
        mn=std::min(mn,z);mx=std::max(mx,z);
        std::cerr<<"row5 shard gpu="<<d<<" bounded=["<<cut[d]<<","<<cut[d+1]<<") states="<<z<<" full=["<<first<<","<<last<<"] owner=["<<base<<","<<end<<") local_cap="<<mainLen[d]<<"\n";
    }
    int lanes=8;if(const char*e=std::getenv("GRIDFP_ROW5_LANES"))lanes=std::atoi(e);else if(const char*e=std::getenv("GRIDFP_ROW5_WARP"))lanes=std::atoi(e)?32:0;
    std::vector<double> gpu_s(ng);std::vector<std::thread> workers;workers.reserve(ng);auto g0=std::chrono::steady_clock::now();
    for(int d=0;d<ng;++d)workers.emplace_back([&,d]{ck(cudaSetDevice(d),"row5 multi set");ck(cudaMemcpyToSymbol(D_BOUND_DP,dp,sizeof(dp)),"row5 lut dp");int cap=5;ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"row5 lut cap");Count *dpref=nullptr,*dsuff=nullptr;ck(cudaMalloc(&dpref,pref.size()*sizeof(Count)),"row5 pref");ck(cudaMalloc(&dsuff,suff.size()*sizeof(Count)),"row5 suff");ck(cudaMemcpy(dpref,pref.data(),pref.size()*sizeof(Count),cudaMemcpyHostToDevice),"row5 pref copy");ck(cudaMemcpy(dsuff,suff.data(),suff.size()*sizeof(Count),cudaMemcpyHostToDevice),"row5 suff copy");ck(cudaMemcpyToSymbol(D_R5_PREF,&dpref,sizeof(dpref)),"row5 pref ptr");ck(cudaMemcpyToSymbol(D_R5_SUFF,&dsuff,sizeof(dsuff)),"row5 suff ptr");Code cnt=cut[d+1]-cut[d],base=Code(d)*mainChunk;int b=lanes?65535:int(std::min<Code>(65535,(cnt+threads-1)/threads));auto q0=std::chrono::steady_clock::now();if(cnt){if(lanes==32&&ng==1)bounded_fill_row5_full_warp_kernel<<<std::max(1,b),threads>>>(fullMain[d],n,W);else if(lanes==16)bounded_fill_row5_full_group_range_kernel<16><<<std::max(1,b),threads>>>(fullMain[d],cut[d],cut[d+1],base);else if(lanes==8)bounded_fill_row5_full_group_range_kernel<8><<<std::max(1,b),threads>>>(fullMain[d],cut[d],cut[d+1],base);else if(lanes==4)bounded_fill_row5_full_group_range_kernel<4><<<std::max(1,b),threads>>>(fullMain[d],cut[d],cut[d+1],base);else bounded_fill_row5_full_range_kernel<<<std::max(1,b),threads>>>(fullMain[d],cut[d],cut[d+1],base);}ck(cudaDeviceSynchronize(),"row5 lut init");gpu_s[d]=std::chrono::duration<double>(std::chrono::steady_clock::now()-q0).count();cudaFree(dpref);cudaFree(dsuff);});
    ...
    std::cerr<<"direct row5 full states="<<n<<" lut_host_s="<<host_s<<" kernel_max_s="<<gmax<<" gpu_phase_wall_s="<<gpu_wall<<" pref_mib="<<(pref.size()*sizeof(Count)>>20)<<" suff_mib="<<(suff.size()*sizeof(Count)>>20)<<" shard_min="<<mn<<" shard_max="<<mx<<"\n";
}
