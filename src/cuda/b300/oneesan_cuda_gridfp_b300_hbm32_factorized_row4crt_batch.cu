...
#include "row4_automaton_rational.hpp"
...
#pragma unroll
    for(int j=0;j<oneesan::row4rat::DIM;++j){uint64_t acc=0;Code base=(Code(sym)*oneesan::row4rat::DIM+j)*oneesan::row4rat::K;
#pragma unroll
        for(int k=0;k<oneesan::row4rat::K;++k){uint8_t i=src[base+k];if(i==255)break;Count x=v[int(i)*bd+tid];if(x)acc+=r4_fold_product(x,co[base+k]);}
...
#pragma unroll
    for(int i=0;i<oneesan::row4rat::DIM;++i){uint64_t acc=0;Code base=(Code(sym)*oneesan::row4rat::DIM+i)*oneesan::row4rat::K;
#pragma unroll
        for(int k=0;k<oneesan::row4rat::K;++k){uint8_t j=src[base+k];if(j==255)break;Count x=v[int(j)*bd+tid];if(x)acc+=r4_fold_product(co[base+k],x);}
...
__global__ void r4_build_prefix_kernel(Code total,int len,Count* out,const uint8_t* csrc,const Count* cco){
    extern __shared__ Count sh4[];int tid=threadIdx.x,bd=blockDim.x;Count*A=sh4,*B=sh4+oneesan::row4rat::DIM*bd;
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<total;q+=st){
#pragma unroll
        for(int j=0;j<oneesan::row4rat::DIM;++j)A[j*bd+tid]=0;A[tid]=1;Count*v=A,*w=B;Code r=q;int h=1;
...
#pragma unroll
        for(int j=0;j<oneesan::row4rat::DIM;++j)out[Code(j)*total+q]=v[j*bd+tid];
    }
}
__global__ void r4_build_suffix_kernel(Code total,int len,Count* out,const uint8_t* src,const Count* co,const Count* beta){
    ...
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<total;q+=st){
        ...
        for(int step=0;step<len;++step){int rem=len-step-1;MateValue z=N;Code c=D_BOUND_DP[rem][curh];if(r<c)z=N;else{r-=c;if(curh>0){c=D_BOUND_DP[rem][curh-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}code=(code<<2)|uint32_t(z);if(z==R)--curh;else if(z==L)++curh;}
#pragma unroll
        for(int j=0;j<oneesan::row4rat::DIM;++j)A[j*bd+tid]=beta[j];Count*v=A,*w=B;
...
#pragma unroll
        for(int j=0;j<oneesan::row4rat::DIM;++j){Count a=pref[Code(j)*pn+pr],b=suff[Code(j)*sn+si];if(a&&b)ans=r3_add(ans,pm_mul(a,b));}
        global_store_main(grank,ans);
    }
}
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        bool directRow4=false;if(const char*e=std::getenv("GRIDFP_DIRECT_ROW4"))directRow4=std::atoi(e)!=0;bool directRow3=false;if(const char*e=std::getenv("GRIDFP_DIRECT_ROW3"))directRow3=std::atoi(e)!=0;bool directRow2=true;if(const char*e=std::getenv("GRIDFP_DIRECT_ROW2"))directRow2=std::atoi(e)!=0;
        ...
        if(directRow4){
            ...
            std::vector<uint8_t> hsrc(3*oneesan::row4rat::DIM*oneesan::row4rat::K),hcsrc(hsrc.size());
            std::vector<Count> hco(hsrc.size()),hcco(hsrc.size()),hbeta(oneesan::row4rat::DIM);
            const uint8_t* ss[3]={oneesan::row4rat::RSRC_N,oneesan::row4rat::RSRC_R,oneesan::row4rat::RSRC_L};const uint8_t* cs[3]={oneesan::row4rat::CSRC_N,oneesan::row4rat::CSRC_R,oneesan::row4rat::CSRC_L};
            const oneesan::row4rat::Rat* cc[3]={oneesan::row4rat::RCO_N,oneesan::row4rat::RCO_R,oneesan::row4rat::RCO_L};const oneesan::row4rat::Rat* ccc[3]={oneesan::row4rat::CCO_N,oneesan::row4rat::CCO_R,oneesan::row4rat::CCO_L};
            auto hpw=[&](uint64_t x,uint64_t e){uint64_t r=1;while(e){if(e&1)r=(unsigned __int128)r*x%mod;x=(unsigned __int128)x*x%mod;e>>=1;}return r;};
            auto ratmod=[&](oneesan::row4rat::Rat z)->Count{if(z.num==0)return 0;uint64_t a=uint64_t(z.num<0?-z.num:z.num)%mod;uint64_t inv=hpw(z.den%mod,uint64_t(mod)-2);uint64_t v=(unsigned __int128)a*inv%mod;if(z.num<0&&v)v=mod-v;return Count(v);};
            size_t oneMat=size_t(oneesan::row4rat::DIM)*oneesan::row4rat::K;for(int a=0;a<3;++a)for(size_t q=0;q<oneMat;++q){hsrc[a*oneMat+q]=ss[a][q];hcsrc[a*oneMat+q]=cs[a][q];hco[a*oneMat+q]=ratmod(cc[a][q]);hcco[a*oneMat+q]=ratmod(ccc[a][q]);}for(int q=0;q<oneesan::row4rat::DIM;++q){int64_t b=oneesan::row4rat::BETA[q];uint64_t v=uint64_t(b<0?-b:b)%mod;hbeta[q]=Count(b<0&&v?mod-v:v);}
            for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"r4 direct set");ck(cudaMemcpyToSymbol(D_BOUND_DP,bdp,sizeof(bdp)),"r4 dp");ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"r4 cap");ck(cudaMemcpyToSymbol(D_R4_ANY_DP,any,sizeof(any)),"r4 any");ck(cudaMemcpyToSymbol(D_R4_SUFF_OFF,off,sizeof(off)),"r4 off");
                ...
                ck(cudaMalloc(&pt,size_t(oneesan::row4rat::DIM)*pn*sizeof(Count)),"r4 pref");ck(cudaMalloc(&stb,size_t(oneesan::row4rat::DIM)*sn*sizeof(Count)),"r4 suff");int rt=64;int pb=int(std::min<Code>(65535,(pn+rt-1)/rt)),sbk=int(std::min<Code>(65535,(sn+rt-1)/rt));size_t sh=size_t(2)*oneesan::row4rat::DIM*rt*sizeof(Count);r4_build_prefix_kernel<<<std::max(1,pb),rt,sh>>>(pn,hlen,pt,dcsrc,dcco);r4_build_suffix_kernel<<<std::max(1,sbk),rt,sh>>>(sn,llen,stb,dsrc,dco,dbeta);ck(cudaDeviceSynchronize(),"r4 tables");Code mine=(total+ng-1-d)/ng;int blocks=int(std::min<Code>(65535,(mine+threads-1)/threads));if(mine)init_after_four_rows_compactlut_kernel<<<std::max(1,blocks),threads>>>(total,d,ng,pt,pn,stb,sn,hlen);ck(cudaDeviceSynchronize(),"r4 init");cudaFree(pt);cudaFree(stb);cudaFree(dsrc);cudaFree(dcsrc);cudaFree(dco);cudaFree(dcco);cudaFree(dbeta);
            }
