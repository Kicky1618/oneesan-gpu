...
#include "row4_automaton_mod.hpp"
#include "row5_automaton_mod.hpp"
...
__constant__ Code D_R3_ANY_DP[MAXW+1][4];
__constant__ Code D_R4_ANY_DP[MAXW+1][5];
__constant__ Code D_R4_SUFF_OFF[6];
__constant__ Code D_R5_ANY_DP[MAXW+1][6];
__constant__ Code D_R5_SUFF_OFF[7];
__device__ __constant__ int D_R5_BSZ[6]={51,61,40,18,5,1};
__device__ __constant__ int D_R5_BIDX[6][61]={
...
__constant__ Code D_R3_SUFF_OFF[5];
__constant__ Count D_R3_SUFFIX_BASE[18];
__constant__ uint8_t D_R3I_SRC[3][18*6];
__constant__ int16_t D_R3I_CO[3][18*6];
...
__device__ __forceinline__ Count pm_reduce_i64(int64_t x){
    if(x>=0)return pm_reduce_u64(uint64_t(x));
    Count r=pm_reduce_u64(uint64_t(-x));return r?D_MOD-r:0;
}
__device__ __forceinline__ void r3_step_shared(Count* v,Count* w,int tid,int bd,MateValue z){
    int sym=(z==N?0:(z==R?1:2));
#pragma unroll
    for(int j=0;j<18;++j){
        int64_t acc=0;
#pragma unroll
        for(int k=0;k<6;++k){int q=j*6+k;uint8_t si=D_R3I_SRC[sym][q];if(si==255)break;acc += int64_t(v[int(si)*bd+tid])*int64_t(D_R3I_CO[sym][q]);}
        w[j*bd+tid]=pm_reduce_i64(acc);
    }
}

__device__ __forceinline__ void r3_col_step_shared(Count* v,Count* w,int tid,int bd,MateValue z){
...
#pragma unroll
    for(int i=0;i<18;++i){int64_t acc=0;int e=off[i+1];for(int t=off[i];t<e;++t){auto q=tr[t];acc += int64_t(v[int(q.dst)*bd+tid])*int64_t(q.coeff);}w[i*bd+tid]=pm_reduce_i64(acc);}
}
__global__ void r3_build_prefix_compact_kernel(Code total,int len,Count* out){
    extern __shared__ Count r3comp[];int tid=threadIdx.x,bd=blockDim.x;Count*A=r3comp,*B=r3comp+18*bd;
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<total;q+=st){
#pragma unroll
        for(int j=0;j<18;++j)A[j*bd+tid]=0;A[tid]=1;Count*v=A,*w=B;Code r=q;int h=1;
        for(int step=0;step<len;++step){int rem=len-step-1;MateValue z=N;Code c=D_R3_ANY_DP[rem][h];if(r<c)z=N;else{r-=c;if(h>0){c=D_R3_ANY_DP[rem][h-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}if(z==R)--h;else if(z==L)++h;r3_step_shared(v,w,tid,bd,z);Count*t=v;v=w;w=t;}
#pragma unroll
        for(int j=0;j<18;++j)out[Code(j)*total+q]=v[j*bd+tid];
    }
}
__global__ void r3_build_suffix_compact_kernel(Code total,int len,Count* out){
    extern __shared__ Count r3comp[];int tid=threadIdx.x,bd=blockDim.x;Count*A=r3comp,*B=r3comp+18*bd;
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<total;q+=st){
        int h=0;while(h<3&&q>=D_R3_SUFF_OFF[h+1])++h;Code r=q-D_R3_SUFF_OFF[h];uint32_t code=0;int curh=h;
        for(int step=0;step<len;++step){int rem=len-step-1;MateValue z=N;Code c=D_BOUND_DP[rem][curh];if(r<c)z=N;else{r-=c;if(curh>0){c=D_BOUND_DP[rem][curh-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}code=(code<<2)|uint32_t(z);if(z==R)--curh;else if(z==L)++curh;}
#pragma unroll
        for(int j=0;j<18;++j)A[j*bd+tid]=D_R3_SUFFIX_BASE[j];Count*v=A,*w=B;
        for(int step=0;step<len;++step){MateValue z=MateValue(code&3u);code>>=2;r3_col_step_shared(v,w,tid,bd,z);Count*t=v;v=w;w=t;}
#pragma unroll
        for(int j=0;j<18;++j)out[Code(j)*total+q]=v[j*bd+tid];
    }
}
__global__ void init_after_three_rows_compactlut_kernel(Code total,int shard,int ng,const Count* pref,Code pn,const Count* suff,Code sn,int hlen){
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;;q+=st){Code brank=Code(shard)+q*Code(ng);if(brank>=total)break;Code r=brank,grank=0,pr=0,lr=0;int h=1,mid=0,idx=0;
#pragma unroll
        for(int pos=TARGET_W-1;pos>=0;--pos,++idx){int hb=h;MateValue z=N;Code c=D_BOUND_DP[pos][h];if(r<c)z=N;else{r-=c;if(h>0){c=D_BOUND_DP[pos][h-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}
            if(idx<hlen){int rem=hlen-idx-1;if(z>N)pr+=D_R3_ANY_DP[rem][hb];if(z>R&&hb>0)pr+=D_R3_ANY_DP[rem][hb-1];}
            if(z==R){grank+=D_FULL_DP[pos][h];--h;}else if(z==L){grank+=D_FULL_DP[pos][h]+(h>0?D_FULL_DP[pos][h-1]:0);++h;}
            if(idx==hlen-1){mid=h;lr=r;}
        }
        Code si=D_R3_SUFF_OFF[mid]+lr;Count ans=0;
#pragma unroll
        for(int j=0;j<18;++j){Count a=pref[Code(j)*pn+pr],b=suff[Code(j)*sn+si];if(a&&b)ans=r3_add(ans,pm_mul(a,b));}
        global_store_main(grank,ans);
    }
}
__global__ void init_after_three_rows_kernel(Code total,int shard,int ng){
    extern __shared__ Count r3sh[];int tid=threadIdx.x,bd=blockDim.x;Count* A=r3sh;Count* B=r3sh+18*bd;
    Code gid=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;
    for(Code q=gid;;q+=st){
        Code brank=Code(shard)+q*Code(ng);if(brank>=total)break;
#pragma unroll
        for(int j=0;j<18;++j)A[j*bd+tid]=0;
        A[tid]=1;Count* v=A;Count* w=B;
        Code r=brank,grank=0;int h=1;
#pragma unroll
        for(int pos=TARGET_W-1;pos>=0;--pos){MateValue z=N;Code c=D_BOUND_DP[pos][h];if(r<c)z=N;else{r-=c;if(h>0){c=D_BOUND_DP[pos][h-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}if(z==R){grank+=D_FULL_DP[pos][h];--h;}else if(z==L){grank+=D_FULL_DP[pos][h]+(h>0?D_FULL_DP[pos][h-1]:0);++h;}r3_step_shared(v,w,tid,bd,z);Count* tmp=v;v=w;w=tmp;}
        int64_t acc=0;
#pragma unroll
        for(int j=0;j<18;++j)acc += int64_t(v[j*bd+tid])*int64_t(D_R3_BETA[j]);
        Count ans=pm_mul(pm_reduce_i64(acc),D_R3_UNSCALE);global_store_main(grank,ans);
    }
}


__device__ __forceinline__ uint64_t r4_fold_product(Count a,Count b){
    uint64_t x=uint64_t(a)*uint64_t(b),delta=0x100000000ULL-D_MOD;
    return uint64_t(uint32_t(x))+(x>>32)*delta;
}
__device__ __forceinline__ void r4_in_step(Count* v,Count* w,int tid,int bd,int sym,const uint8_t* src,const Count* co){
#pragma unroll
    for(int j=0;j<oneesan::row4mod::DIM;++j){uint64_t acc=0;Code base=(Code(sym)*oneesan::row4mod::DIM+j)*oneesan::row4mod::K;
#pragma unroll
        for(int k=0;k<oneesan::row4mod::K;++k){uint8_t i=src[base+k];if(i==255)break;Count x=v[int(i)*bd+tid];if(x)acc+=r4_fold_product(x,co[base+k]);}
        w[j*bd+tid]=pm_reduce_u64(acc);
    }
}
__device__ __forceinline__ void r4_row_step(Count* v,Count* w,int tid,int bd,int sym,const uint8_t* src,const Count* co){
#pragma unroll
    for(int i=0;i<oneesan::row4mod::DIM;++i){uint64_t acc=0;Code base=(Code(sym)*oneesan::row4mod::DIM+i)*oneesan::row4mod::K;
#pragma unroll
        for(int k=0;k<oneesan::row4mod::K;++k){uint8_t j=src[base+k];if(j==255)break;Count x=v[int(j)*bd+tid];if(x)acc+=r4_fold_product(co[base+k],x);}
        w[i*bd+tid]=pm_reduce_u64(acc);
    }
}
__global__ void r4_build_prefix_kernel(Code total,int len,Count* out,const uint8_t* csrc,const Count* cco){
    extern __shared__ Count sh4[];int tid=threadIdx.x,bd=blockDim.x;Count*A=sh4,*B=sh4+oneesan::row4mod::DIM*bd;
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<total;q+=st){
#pragma unroll
        for(int j=0;j<oneesan::row4mod::DIM;++j)A[j*bd+tid]=0;A[tid]=1;Count*v=A,*w=B;Code r=q;int h=1;
        for(int step=0;step<len;++step){int rem=len-step-1;MateValue z=N;Code c=D_R4_ANY_DP[rem][h];if(r<c)z=N;else{r-=c;if(h>0){c=D_R4_ANY_DP[rem][h-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}if(z==R)--h;else if(z==L)++h;int sym=(z==N?0:(z==R?1:2));r4_in_step(v,w,tid,bd,sym,csrc,cco);Count*t=v;v=w;w=t;}
#pragma unroll
        for(int j=0;j<oneesan::row4mod::DIM;++j)out[Code(j)*total+q]=v[j*bd+tid];
    }
}
__global__ void r4_build_suffix_kernel(Code total,int len,Count* out,const uint8_t* src,const Count* co,const Count* beta){
    extern __shared__ Count sh4[];int tid=threadIdx.x,bd=blockDim.x;Count*A=sh4,*B=sh4+oneesan::row4mod::DIM*bd;
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<total;q+=st){
        int h=0;while(h<4&&q>=D_R4_SUFF_OFF[h+1])++h;Code r=q-D_R4_SUFF_OFF[h];uint32_t code=0;int curh=h;
        for(int step=0;step<len;++step){int rem=len-step-1;MateValue z=N;Code c=D_BOUND_DP[rem][curh];if(r<c)z=N;else{r-=c;if(curh>0){c=D_BOUND_DP[rem][curh-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}code=(code<<2)|uint32_t(z);if(z==R)--curh;else if(z==L)++curh;}
#pragma unroll
        for(int j=0;j<oneesan::row4mod::DIM;++j)A[j*bd+tid]=beta[j];Count*v=A,*w=B;
        for(int step=0;step<len;++step){MateValue z=MateValue(code&3u);code>>=2;int sym=(z==N?0:(z==R?1:2));r4_row_step(v,w,tid,bd,sym,src,co);Count*t=v;v=w;w=t;}
#pragma unroll
        for(int j=0;j<oneesan::row4mod::DIM;++j)out[Code(j)*total+q]=v[j*bd+tid];
    }
}
__global__ void init_after_four_rows_compactlut_kernel(Code total,int shard,int ng,const Count* pref,Code pn,const Count* suff,Code sn,int hlen){
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;;q+=st){Code brank=Code(shard)+q*Code(ng);if(brank>=total)break;Code r=brank,grank=0,pr=0,lr=0;int h=1,mid=0,idx=0;
#pragma unroll
        for(int pos=TARGET_W-1;pos>=0;--pos,++idx){int hb=h;MateValue z=N;Code c=D_BOUND_DP[pos][h];if(r<c)z=N;else{r-=c;if(h>0){c=D_BOUND_DP[pos][h-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}
            if(idx<hlen){int rem=hlen-idx-1;if(z>N)pr+=D_R4_ANY_DP[rem][hb];if(z>R&&hb>0)pr+=D_R4_ANY_DP[rem][hb-1];}
            if(z==R){grank+=D_FULL_DP[pos][h];--h;}else if(z==L){grank+=D_FULL_DP[pos][h]+(h>0?D_FULL_DP[pos][h-1]:0);++h;}
            if(idx==hlen-1){mid=h;lr=r;}
        }
        Code si=D_R4_SUFF_OFF[mid]+lr;Count ans=0;
#pragma unroll
        for(int j=0;j<oneesan::row4mod::DIM;++j){Count a=pref[Code(j)*pn+pr],b=suff[Code(j)*sn+si];if(a&&b)ans=r3_add(ans,pm_mul(a,b));}
        global_store_main(grank,ans);
    }
}

__device__ __forceinline__ uint64_t r5_fold_product(Count a,Count b){uint64_t x=uint64_t(a)*uint64_t(b),delta=0x100000000ULL-D_MOD;return uint64_t(uint32_t(x))+(x>>32)*delta;}
__device__ __forceinline__ void r5_in_step(Count* v,Count* w,int tid,int bd,int sym,const uint8_t* src,const Count* co){
#pragma unroll 1
    for(int j=0;j<oneesan::row5mod::DIM;++j){uint64_t acc=0;Code base=(Code(sym)*oneesan::row5mod::DIM+j)*oneesan::row5mod::K;
#pragma unroll 1
        for(int k=0;k<oneesan::row5mod::K;++k){uint8_t i=src[base+k];if(i==255)break;Count x=v[int(i)*bd+tid];if(x)acc+=r5_fold_product(x,co[base+k]);}
        w[j*bd+tid]=pm_reduce_u64(acc);
    }
}
__device__ __forceinline__ void r5_row_step(Count* v,Count* w,int tid,int bd,int sym,const uint8_t* src,const Count* co){
#pragma unroll 1
    for(int i=0;i<oneesan::row5mod::DIM;++i){uint64_t acc=0;Code base=(Code(sym)*oneesan::row5mod::DIM+i)*oneesan::row5mod::K;
#pragma unroll 1
        for(int k=0;k<oneesan::row5mod::K;++k){uint8_t j=src[base+k];if(j==255)break;Count x=v[int(j)*bd+tid];if(x)acc+=r5_fold_product(co[base+k],x);}
        w[i*bd+tid]=pm_reduce_u64(acc);
    }
}
__global__ void r5_build_prefix_kernel(Code total,int len,Count* out,const uint8_t* csrc,const Count* cco){
    extern __shared__ Count sh5[];int tid=threadIdx.x,bd=blockDim.x;Count*A=sh5,*B=sh5+oneesan::row5mod::DIM*bd;
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<total;q+=st){
        for(int j=0;j<oneesan::row5mod::DIM;++j)A[j*bd+tid]=0;A[tid]=1;Count*v=A,*w=B;Code r=q;int h=1;
        for(int step=0;step<len;++step){int rem=len-step-1;MateValue z=N;Code c=D_R5_ANY_DP[rem][h];if(r<c)z=N;else{r-=c;if(h>0){c=D_R5_ANY_DP[rem][h-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}if(z==R)--h;else if(z==L)++h;int sym=(z==N?0:(z==R?1:2));r5_in_step(v,w,tid,bd,sym,csrc,cco);Count*t=v;v=w;w=t;}
        for(int j=0;j<D_R5_BSZ[h];++j){int gj=D_R5_BIDX[h][j];out[q*Code(61)+j]=v[gj*bd+tid];}
    }
}
__global__ void r5_build_suffix_kernel(Code total,int len,Count* out,const uint8_t* src,const Count* co,const Count* beta){
    extern __shared__ Count sh5[];int tid=threadIdx.x,bd=blockDim.x;Count*A=sh5,*B=sh5+oneesan::row5mod::DIM*bd;
    for(Code q=Code(blockIdx.x)*bd+tid,st=Code(gridDim.x)*bd;q<total;q+=st){
        int h=0;while(h<5&&q>=D_R5_SUFF_OFF[h+1])++h;Code r=q-D_R5_SUFF_OFF[h];uint32_t code=0;int curh=h;
        for(int step=0;step<len;++step){int rem=len-step-1;MateValue z=N;Code c=D_BOUND_DP[rem][curh];if(r<c)z=N;else{r-=c;if(curh>0){c=D_BOUND_DP[rem][curh-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}code=(code<<2)|uint32_t(z);if(z==R)--curh;else if(z==L)++curh;}
        for(int j=0;j<oneesan::row5mod::DIM;++j)A[j*bd+tid]=beta[j];Count*v=A,*w=B;
        for(int step=0;step<len;++step){MateValue z=MateValue(code&3u);code>>=2;int sym=(z==N?0:(z==R?1:2));r5_row_step(v,w,tid,bd,sym,src,co);Count*t=v;v=w;w=t;}
        for(int j=0;j<D_R5_BSZ[h];++j){int gj=D_R5_BIDX[h][j];out[q*Code(61)+j]=v[gj*bd+tid];}
    }
}
template<int LANES>
__global__ void init_after_five_rows_compactlut_lane_kernel(Code total,int shard,int ng,const Count* pref,Code pn,const Count* suff,Code sn,int hlen){
    static_assert(LANES==4||LANES==8||LANES==16);
    int lane=threadIdx.x&(LANES-1), warpLane=threadIdx.x&31, leader=warpLane&~(LANES-1);
    unsigned mask=((1u<<LANES)-1u)<<leader;
    Code group=(Code(blockIdx.x)*blockDim.x+threadIdx.x)/LANES;
    Code step=(Code(gridDim.x)*blockDim.x)/LANES;
    for(Code q=group;;q+=step){
        Code brank=Code(shard)+q*Code(ng); if(brank>=total)break;
        Code grank=0,pr=0,si=0; int mid=0;
        if(lane==0){
            Code r=brank,lr=0;int h=1,idx=0;
            for(int pos=TARGET_W-1;pos>=0;--pos,++idx){
                int hb=h;MateValue z=N;Code c=D_BOUND_DP[pos][h];
                if(r<c)z=N;else{r-=c;if(h>0){c=D_BOUND_DP[pos][h-1];if(r<c)z=R;else{r-=c;z=L;}}else z=L;}
                if(idx<hlen){int rem=hlen-idx-1;if(z>N)pr+=D_R5_ANY_DP[rem][hb];if(z>R&&hb>0)pr+=D_R5_ANY_DP[rem][hb-1];}
                if(z==R){grank+=D_FULL_DP[pos][h];--h;}else if(z==L){grank+=D_FULL_DP[pos][h]+(h>0?D_FULL_DP[pos][h-1]:0);++h;}
                if(idx==hlen-1){mid=h;lr=r;}
            }
            si=D_R5_SUFF_OFF[mid]+lr;
        }
        grank=(Code)__shfl_sync(mask,(unsigned long long)grank,leader);
        pr=(Code)__shfl_sync(mask,(unsigned long long)pr,leader);
        si=(Code)__shfl_sync(mask,(unsigned long long)si,leader);
        mid=__shfl_sync(mask,mid,leader);
        Count acc=0;const Count*pv=pref+pr*Code(61);const Count*sv=suff+si*Code(61);
        for(int j=lane;j<D_R5_BSZ[mid];j+=LANES){Count a=pv[j],b=sv[j];if(a&&b)acc=r3_add(acc,pm_mul(a,b));}
        for(int off=LANES/2;off;off>>=1)acc=r3_add(acc,__shfl_down_sync(mask,acc,off,LANES));
        if(lane==0)global_store_main(grank,acc);
    }
}
...
static void build_r3_any_dp(int cap,Code out[MAXW+1][4]){std::memset(out,0,sizeof(Code)*(MAXW+1)*4);for(int h=0;h<=cap;++h)out[0][h]=1;for(int n=1;n<=MAXW;++n)for(int h=0;h<=cap;++h){Code x=out[n-1][h];if(h>0)x+=out[n-1][h-1];if(h<cap)x+=out[n-1][h+1];out[n][h]=x;}}
static void build_r4_any_dp(int cap,Code out[MAXW+1][5]){std::memset(out,0,sizeof(Code)*(MAXW+1)*5);for(int h=0;h<=cap;++h)out[0][h]=1;for(int n=1;n<=MAXW;++n)for(int h=0;h<=cap;++h){Code x=out[n-1][h];if(h>0)x+=out[n-1][h-1];if(h<cap)x+=out[n-1][h+1];out[n][h]=x;}}
static void build_r5_any_dp(int cap,Code out[MAXW+1][6]){std::memset(out,0,sizeof(Code)*(MAXW+1)*6);for(int h=0;h<=cap;++h)out[0][h]=1;for(int n=1;n<=MAXW;++n)for(int h=0;h<=cap;++h){Code x=out[n-1][h];if(h>0)x+=out[n-1][h-1];if(h<cap)x+=out[n-1][h+1];out[n][h]=x;}}
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        bool directRow5=false;if(const char*e=std::getenv("GRIDFP_DIRECT_ROW5"))directRow5=std::atoi(e)!=0;bool directRow4=false;if(const char*e=std::getenv("GRIDFP_DIRECT_ROW4"))directRow4=std::atoi(e)!=0;bool directRow3=false;if(const char*e=std::getenv("GRIDFP_DIRECT_ROW3"))directRow3=std::atoi(e)!=0;bool directRow2=true;if(const char*e=std::getenv("GRIDFP_DIRECT_ROW2"))directRow2=std::atoi(e)!=0;
        int prefixK=1;if(const char*e=std::getenv("GRIDFP_BOUNDED_PREFIX_K"))prefixK=std::max(1,std::min(W-2,std::atoi(e)));
        if(directRow5){
            if(mod!=oneesan::row5mod::MOD){std::cerr<<"row5 prototype requires modulus "<<oneesan::row5mod::MOD<<"\n";return 51;}
            prefixK=5;Code bdp[MAXW+1][MAXW+2]{};build_bounded_dp(5,bdp);Code total=bdp[W][1];int cap=5;int hlen=W/2,llen=W-hlen;Code any[MAXW+1][6]{};build_r5_any_dp(5,any);Code off[7]{};for(int hh=0;hh<6;++hh)off[hh+1]=off[hh]+bdp[llen][hh];Code pn=any[hlen][1],sn=off[6];
            std::vector<uint8_t> hsrc(3*oneesan::row5mod::DIM*oneesan::row5mod::K),hcsrc(hsrc.size());std::vector<Count> hco(hsrc.size()),hcco(hsrc.size()),hbeta(oneesan::row5mod::DIM);
            const uint8_t* ss[3]={oneesan::row5mod::SRC_N,oneesan::row5mod::SRC_R,oneesan::row5mod::SRC_L};const uint8_t* cs[3]={oneesan::row5mod::CSRC_N,oneesan::row5mod::CSRC_R,oneesan::row5mod::CSRC_L};const uint32_t* cc[3]={oneesan::row5mod::CO_N,oneesan::row5mod::CO_R,oneesan::row5mod::CO_L};const uint32_t* ccc[3]={oneesan::row5mod::CCO_N,oneesan::row5mod::CCO_R,oneesan::row5mod::CCO_L};size_t oneMat=size_t(oneesan::row5mod::DIM)*oneesan::row5mod::K;for(int a=0;a<3;++a){std::copy(ss[a],ss[a]+oneMat,hsrc.begin()+a*oneMat);std::copy(cs[a],cs[a]+oneMat,hcsrc.begin()+a*oneMat);std::copy(cc[a],cc[a]+oneMat,hco.begin()+a*oneMat);std::copy(ccc[a],ccc[a]+oneMat,hcco.begin()+a*oneMat);}std::copy(oneesan::row5mod::BETA,oneesan::row5mod::BETA+oneesan::row5mod::DIM,hbeta.begin());
            for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"r5 set");ck(cudaMemcpyToSymbol(D_BOUND_DP,bdp,sizeof(bdp)),"r5 dp");ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"r5 cap");ck(cudaMemcpyToSymbol(D_R5_ANY_DP,any,sizeof(any)),"r5 any");ck(cudaMemcpyToSymbol(D_R5_SUFF_OFF,off,sizeof(off)),"r5 off");uint8_t*dsrc=nullptr,*dcsrc=nullptr;Count*dco=nullptr,*dcco=nullptr,*dbeta=nullptr,*pt=nullptr,*stb=nullptr;size_t mb=hsrc.size();ck(cudaMalloc(&dsrc,mb),"r5 src");ck(cudaMalloc(&dcsrc,mb),"r5 csrc");ck(cudaMalloc(&dco,mb*sizeof(Count)),"r5 co");ck(cudaMalloc(&dcco,mb*sizeof(Count)),"r5 cco");ck(cudaMalloc(&dbeta,hbeta.size()*sizeof(Count)),"r5 beta");ck(cudaMemcpy(dsrc,hsrc.data(),mb,cudaMemcpyHostToDevice),"r5 src cp");ck(cudaMemcpy(dcsrc,hcsrc.data(),mb,cudaMemcpyHostToDevice),"r5 csrc cp");ck(cudaMemcpy(dco,hco.data(),mb*sizeof(Count),cudaMemcpyHostToDevice),"r5 co cp");ck(cudaMemcpy(dcco,hcco.data(),mb*sizeof(Count),cudaMemcpyHostToDevice),"r5 cco cp");ck(cudaMemcpy(dbeta,hbeta.data(),hbeta.size()*sizeof(Count),cudaMemcpyHostToDevice),"r5 beta cp");ck(cudaMalloc(&pt,size_t(61)*pn*sizeof(Count)),"r5 pref");ck(cudaMalloc(&stb,size_t(61)*sn*sizeof(Count)),"r5 suff");int rt=32;int pb=int(std::min<Code>(65535,(pn+rt-1)/rt)),sbk=int(std::min<Code>(65535,(sn+rt-1)/rt));size_t sh=size_t(2)*oneesan::row5mod::DIM*rt*sizeof(Count);r5_build_prefix_kernel<<<std::max(1,pb),rt,sh>>>(pn,hlen,pt,dcsrc,dcco);r5_build_suffix_kernel<<<std::max(1,sbk),rt,sh>>>(sn,llen,stb,dsrc,dco,dbeta);ck(cudaDeviceSynchronize(),"r5 tables");Code mine=(total+ng-1-d)/ng;int blocks=int(std::min<Code>(65535,(mine*Code(4)+threads-1)/threads));if(mine)init_after_five_rows_compactlut_lane_kernel<4><<<std::max(1,blocks),threads>>>(total,d,ng,pt,pn,stb,sn,hlen);ck(cudaDeviceSynchronize(),"r5 init");cudaFree(pt);cudaFree(stb);cudaFree(dsrc);cudaFree(dcsrc);cudaFree(dco);cudaFree(dcco);cudaFree(dbeta);}
            std::cerr<<"direct row5 states="<<total<<" pref="<<pn<<" suff="<<sn<<"\n";
        }
        else         if(directRow4){
            if(mod!=oneesan::row4mod::MOD){std::cerr<<"row4 prototype requires modulus "<<oneesan::row4mod::MOD<<"\n";return 41;}
            prefixK=4;Code bdp[MAXW+1][MAXW+2]{};build_bounded_dp(4,bdp);Code total=bdp[W][1];int cap=4;
            int hlen=W/2,llen=W-hlen;Code any[MAXW+1][5]{};build_r4_any_dp(4,any);Code off[6]{};for(int hh=0;hh<5;++hh)off[hh+1]=off[hh]+bdp[llen][hh];Code pn=any[hlen][1],sn=off[5];
            std::vector<uint8_t> hsrc(3*oneesan::row4mod::DIM*oneesan::row4mod::K),hcsrc(hsrc.size());
            std::vector<Count> hco(hsrc.size()),hcco(hsrc.size()),hbeta(oneesan::row4mod::DIM);
            const uint8_t* ss[3]={oneesan::row4mod::SRC_N,oneesan::row4mod::SRC_R,oneesan::row4mod::SRC_L};const uint8_t* cs[3]={oneesan::row4mod::CSRC_N,oneesan::row4mod::CSRC_R,oneesan::row4mod::CSRC_L};
            const uint32_t* cc[3]={oneesan::row4mod::CO_N,oneesan::row4mod::CO_R,oneesan::row4mod::CO_L};const uint32_t* ccc[3]={oneesan::row4mod::CCO_N,oneesan::row4mod::CCO_R,oneesan::row4mod::CCO_L};
            size_t oneMat=size_t(oneesan::row4mod::DIM)*oneesan::row4mod::K;for(int a=0;a<3;++a){std::copy(ss[a],ss[a]+oneMat,hsrc.begin()+a*oneMat);std::copy(cs[a],cs[a]+oneMat,hcsrc.begin()+a*oneMat);std::copy(cc[a],cc[a]+oneMat,hco.begin()+a*oneMat);std::copy(ccc[a],ccc[a]+oneMat,hcco.begin()+a*oneMat);}std::copy(oneesan::row4mod::BETA,oneesan::row4mod::BETA+oneesan::row4mod::DIM,hbeta.begin());
            for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"r4 direct set");ck(cudaMemcpyToSymbol(D_BOUND_DP,bdp,sizeof(bdp)),"r4 dp");ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"r4 cap");ck(cudaMemcpyToSymbol(D_R4_ANY_DP,any,sizeof(any)),"r4 any");ck(cudaMemcpyToSymbol(D_R4_SUFF_OFF,off,sizeof(off)),"r4 off");
                uint8_t *dsrc=nullptr,*dcsrc=nullptr;Count *dco=nullptr,*dcco=nullptr,*dbeta=nullptr,*pt=nullptr,*stb=nullptr;size_t mb=hsrc.size();ck(cudaMalloc(&dsrc,mb),"r4 src");ck(cudaMalloc(&dcsrc,mb),"r4 csrc");ck(cudaMalloc(&dco,mb*sizeof(Count)),"r4 co");ck(cudaMalloc(&dcco,mb*sizeof(Count)),"r4 cco");ck(cudaMalloc(&dbeta,hbeta.size()*sizeof(Count)),"r4 beta");ck(cudaMemcpy(dsrc,hsrc.data(),mb,cudaMemcpyHostToDevice),"r4 src cp");ck(cudaMemcpy(dcsrc,hcsrc.data(),mb,cudaMemcpyHostToDevice),"r4 csrc cp");ck(cudaMemcpy(dco,hco.data(),mb*sizeof(Count),cudaMemcpyHostToDevice),"r4 co cp");ck(cudaMemcpy(dcco,hcco.data(),mb*sizeof(Count),cudaMemcpyHostToDevice),"r4 cco cp");ck(cudaMemcpy(dbeta,hbeta.data(),hbeta.size()*sizeof(Count),cudaMemcpyHostToDevice),"r4 beta cp");
                ck(cudaMalloc(&pt,size_t(oneesan::row4mod::DIM)*pn*sizeof(Count)),"r4 pref");ck(cudaMalloc(&stb,size_t(oneesan::row4mod::DIM)*sn*sizeof(Count)),"r4 suff");int rt=64;int pb=int(std::min<Code>(65535,(pn+rt-1)/rt)),sbk=int(std::min<Code>(65535,(sn+rt-1)/rt));size_t sh=size_t(2)*oneesan::row4mod::DIM*rt*sizeof(Count);r4_build_prefix_kernel<<<std::max(1,pb),rt,sh>>>(pn,hlen,pt,dcsrc,dcco);r4_build_suffix_kernel<<<std::max(1,sbk),rt,sh>>>(sn,llen,stb,dsrc,dco,dbeta);ck(cudaDeviceSynchronize(),"r4 tables");Code mine=(total+ng-1-d)/ng;int blocks=int(std::min<Code>(65535,(mine+threads-1)/threads));if(mine)init_after_four_rows_compactlut_kernel<<<std::max(1,blocks),threads>>>(total,d,ng,pt,pn,stb,sn,hlen);ck(cudaDeviceSynchronize(),"r4 init");cudaFree(pt);cudaFree(stb);cudaFree(dsrc);cudaFree(dcsrc);cudaFree(dco);cudaFree(dcco);cudaFree(dbeta);
            }
            std::cerr<<"direct row4 states="<<total<<" pref="<<pn<<" suff="<<sn<<"\n";
        }
        else         if(directRow3){prefixK=3;Code bdp[MAXW+1][MAXW+2]{};build_bounded_dp(3,bdp);Code total=bdp[W][1];int cap=3;auto powmod=[&](uint64_t a,uint64_t e){uint64_t r=1;while(e){if(e&1)r=(unsigned __int128)r*a%mod;a=(unsigned __int128)a*a%mod;e>>=1;}return r;};Count scale=Count(powmod(6,W)),inv=Count(powmod(scale,uint64_t(mod)-2));for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"row3 direct set");ck(cudaMemcpyToSymbol(D_BOUND_DP,bdp,sizeof(bdp)),"row3 direct dp");ck(cudaMemcpyToSymbol(D_BOUND_CAP,&cap,sizeof(cap)),"row3 direct cap");ck(cudaMemcpyToSymbol(D_R3_OFF_N,oneesan::row3auto::OFF_N,sizeof(oneesan::row3auto::OFF_N)),"r3 offn");ck(cudaMemcpyToSymbol(D_R3_OFF_R,oneesan::row3auto::OFF_R,sizeof(oneesan::row3auto::OFF_R)),"r3 offr");ck(cudaMemcpyToSymbol(D_R3_OFF_L,oneesan::row3auto::OFF_L,sizeof(oneesan::row3auto::OFF_L)),"r3 offl");ck(cudaMemcpyToSymbol(D_R3_TR_N,oneesan::row3auto::TR_N,sizeof(oneesan::row3auto::TR_N)),"r3 trn");ck(cudaMemcpyToSymbol(D_R3_TR_R,oneesan::row3auto::TR_R,sizeof(oneesan::row3auto::TR_R)),"r3 trr");ck(cudaMemcpyToSymbol(D_R3_TR_L,oneesan::row3auto::TR_L,sizeof(oneesan::row3auto::TR_L)),"r3 trl");ck(cudaMemcpyToSymbol(D_R3_BETA,oneesan::row3auto::BETA,sizeof(oneesan::row3auto::BETA)),"r3 beta");uint8_t his[3][18*6];int16_t hic[3][18*6];const uint8_t* hsp[3]={oneesan::row3warp::SRC_N,oneesan::row3warp::SRC_R,oneesan::row3warp::SRC_L};const int16_t* hcp[3]={oneesan::row3warp::CO_N,oneesan::row3warp::CO_R,oneesan::row3warp::CO_L};for(int aa=0;aa<3;++aa)for(int ii=0;ii<18*6;++ii){his[aa][ii]=hsp[aa][ii];hic[aa][ii]=hcp[aa][ii];}ck(cudaMemcpyToSymbol(D_R3I_SRC,his,sizeof(his)),"r3 incoming src");ck(cudaMemcpyToSymbol(D_R3I_CO,hic,sizeof(hic)),"r3 incoming co");ck(cudaMemcpyToSymbol(D_R3_UNSCALE,&inv,sizeof(inv)),"r3 inv");Code mine=(total+ng-1-d)/ng;int r3threads=128;if(const char*e=std::getenv("GRIDFP_R3_THREADS"))r3threads=std::atoi(e);int blocks=int(std::min<Code>(65535,(mine+r3threads-1)/r3threads));bool compact=false;if(const char*e=std::getenv("GRIDFP_R3_COMPACT_LUT"))compact=std::atoi(e)!=0;if(compact){int hlen=W/2,llen=W-hlen;Code any[MAXW+1][4]{};build_r3_any_dp(3,any);Code off[5]{};for(int hh=0;hh<4;++hh)off[hh+1]=off[hh]+bdp[llen][hh];Code pn=any[hlen][1],sn=off[4];Count sb[18]{};for(int jj=0;jj<18;++jj)if(oneesan::row3auto::BETA[jj])sb[jj]=Count((uint64_t(oneesan::row3auto::BETA[jj])*inv)%mod);ck(cudaMemcpyToSymbol(D_R3_ANY_DP,any,sizeof(any)),"r3 any dp");ck(cudaMemcpyToSymbol(D_R3_SUFF_OFF,off,sizeof(off)),"r3 suff off");ck(cudaMemcpyToSymbol(D_R3_SUFFIX_BASE,sb,sizeof(sb)),"r3 suffix base");Count*pt=nullptr,*stb=nullptr;ck(cudaMalloc(&pt,size_t(18)*pn*sizeof(Count)),"r3 compact pref");ck(cudaMalloc(&stb,size_t(18)*sn*sizeof(Count)),"r3 compact suff");int pb=int(std::min<Code>(65535,(pn+r3threads-1)/r3threads)),sbk=int(std::min<Code>(65535,(sn+r3threads-1)/r3threads));r3_build_prefix_compact_kernel<<<std::max(1,pb),r3threads,size_t(36)*r3threads*sizeof(Count)>>>(pn,hlen,pt);r3_build_suffix_compact_kernel<<<std::max(1,sbk),r3threads,size_t(36)*r3threads*sizeof(Count)>>>(sn,llen,stb);ck(cudaDeviceSynchronize(),"r3 compact build");if(mine)init_after_three_rows_compactlut_kernel<<<std::max(1,blocks),r3threads>>>(total,d,ng,pt,pn,stb,sn,hlen);ck(cudaDeviceSynchronize(),"r3 compact init");cudaFree(pt);cudaFree(stb);}else if(mine)init_after_three_rows_kernel<<<std::max(1,blocks),r3threads,size_t(36)*r3threads*sizeof(Count)>>>(total,d,ng);}for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"row3 direct sync set");ck(cudaDeviceSynchronize(),"row3 direct sync");}std::cerr<<"direct row3 states="<<total<<"\n";}
