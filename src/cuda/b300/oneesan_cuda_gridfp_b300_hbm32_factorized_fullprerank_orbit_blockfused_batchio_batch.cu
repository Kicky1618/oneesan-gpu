...
struct FactorTablesHost {
    ...
    std::vector<uint32_t> low_all_codes,low_packed_values,low_mask_codes,low_mask_off;
    std::vector<uint32_t> high_all_codes,high_packed_values,high_mask_codes,high_mask_off;
...
template<bool SCATTER> __global__ void batch_factor_io_kernel(Count*arena,const BatchFactorIoCfg*cfgs,const BatchFactorIoTask*tasks,Code nt){
    Code ti=blockIdx.x;if(ti>=nt)return;auto t=tasks[ti];const BatchFactorIoCfg*c=cfgs+t.cfg;constexpr Code CH=65536;Code n=t.block?c->block_n:c->main_n,end=min(n,t.begin+CH),bo=t.block?c->block_off:c->main_off;
    for(Code i=t.begin+threadIdx.x;i<end;i+=blockDim.x){Code g=t.block?batch_factor_global_block(i,c):batch_factor_global_main(i,c);Count*p=arena+bo+i;if constexpr(SCATTER){if(t.block)global_store_block(g,*p);else global_store_main(g,*p);}else *p=t.block?global_load_block(g):global_load_main(g);}
}
...
static void process_window_batched_io(DeviceCtx&c,int W,const PreparedWindow&pw,int threads,size_t target){
    ck(cudaSetDevice(c.dev),"batchio set");constexpr Code ALIGN=64;auto al=[](Code x){return (x+ALIGN-1)&~Code(ALIGN-1);};
    size_t q0=0;while(q0<pw.groups.size()){
        struct Meta{size_t qi;Code mo,bo;GroupRuntimeCfg runtime;BatchFactorIoCfg io;std::vector<FBlock> fmb,fdb;};
        std::vector<Meta> meta;Code used=0;size_t q=q0;
        for(;q<pw.groups.size();++q){auto const&pg=pw.groups[q];if(!pg.ms.size&&!pg.ds.size)continue;Code need=al(pg.ms.size)+al(pg.ds.size);if(!meta.empty()&&uint64_t(used+need)*sizeof(Count)>target)break;if(uint64_t(need)*sizeof(Count)>target)throw std::runtime_error("batchio single group exceeds target");
            bool fixLow=pw.wp.p_hi>LOW_LUT_K;uint32_t fmask=fixLow?(pg.mo&((1u<<LOW_LUT_K)-1u)):((pg.mo>>(LOW_LUT_K+1))&((1u<<HIGH_LUT_K)-1u));auto fmb=make_factor_main_blocks(fixLow,fmask),fdb=make_factor_block_blocks(fixLow,fmask);if(fmb.back().end!=pg.ms.size||fdb.back().end!=pg.ds.size)throw std::runtime_error("batchio factor size mismatch");
            Meta m{};m.qi=q;m.mo=used;m.bo=used+al(pg.ms.size);m.fmb=fmb;m.fdb=fdb;std::memcpy(m.runtime.main_dp,pg.ms.dp,sizeof(pg.ms.dp));std::memcpy(m.runtime.block_dp,pg.ds.dp,sizeof(pg.ds.dp));std::memcpy(m.runtime.f_main,fmb.data(),fmb.size()*sizeof(FBlock));std::memcpy(m.runtime.f_block,fdb.data(),fdb.size()*sizeof(FBlock));m.runtime.main_fixed=pg.mf;m.runtime.main_occ=pg.mo;m.runtime.block_fixed=pg.bf;m.runtime.block_occ=pg.bo;m.runtime.f_mask=fmask;m.runtime.main_w=W;m.runtime.block_w=W-1;m.runtime.f_main_n=fmb.size();m.runtime.f_block_n=fdb.size();m.runtime.f_fix_low=fixLow?1:0;
            std::memcpy(m.io.mainb,fmb.data(),fmb.size()*sizeof(FBlock));std::memcpy(m.io.blockb,fdb.data(),fdb.size()*sizeof(FBlock));m.io.main_n=pg.ms.size;m.io.block_n=pg.ds.size;m.io.main_off=m.mo;m.io.block_off=m.bo;m.io.mask=fmask;m.io.main_nb=fmb.size();m.io.block_nb=fdb.size();m.io.fix_low=fixLow?1:0;meta.push_back(std::move(m));used+=need;}
        if(meta.empty()){q0=q+1;continue;}size_t needBytes=size_t(used)*sizeof(Count);if(needBytes>c.capArena){if(c.arena)cudaFree(c.arena);c.capArena=needBytes;ck(cudaMalloc(&c.arena,c.capArena),"batchio arena");}
        std::vector<BatchFactorIoCfg> hc;std::vector<BatchFactorIoTask> ht;hc.reserve(meta.size());for(uint32_t gi=0;gi<meta.size();++gi){hc.push_back(meta[gi].io);for(Code b=0;b<meta[gi].io.main_n;b+=65536)ht.push_back({gi,0,b});for(Code b=0;b<meta[gi].io.block_n;b+=65536)ht.push_back({gi,1,b});}
        BatchFactorIoCfg*dcfg=nullptr;BatchFactorIoTask*dt=nullptr;ck(cudaMalloc(&dcfg,hc.size()*sizeof(BatchFactorIoCfg)),"batchio cfg alloc");ck(cudaMalloc(&dt,ht.size()*sizeof(BatchFactorIoTask)),"batchio task alloc");ck(cudaMemcpy(dcfg,hc.data(),hc.size()*sizeof(BatchFactorIoCfg),cudaMemcpyHostToDevice),"batchio cfg copy");ck(cudaMemcpy(dt,ht.data(),ht.size()*sizeof(BatchFactorIoTask),cudaMemcpyHostToDevice),"batchio task copy");auto t0=std::chrono::steady_clock::now();
        batch_factor_io_kernel<false><<<std::max<size_t>(1,ht.size()),threads,0,c.sMain>>>(reinterpret_cast<Count*>(c.arena),dcfg,dt,ht.size());ck(cudaGetLastError(),"batchio gather");ck(cudaStreamSynchronize(c.sMain),"batchio gather sync");auto tg=std::chrono::steady_clock::now();
        for(auto&m:meta){auto const&pg=pw.groups[m.qi];ck(cudaMemcpyToSymbolAsync(D_GRP,&m.runtime,sizeof(m.runtime),0,cudaMemcpyHostToDevice,c.sMain),"batchio runtime cfg async");Count*cur=reinterpret_cast<Count*>(c.arena)+m.mo,*dcur=reinterpret_cast<Count*>(c.arena)+m.bo;bool fixLow=m.runtime.f_fix_low!=0;
            for(int p=pw.wp.p_hi;p>=pw.wp.p_lo;--p){auto launchN=[&](Code n,auto fn,const char*w){if(!n)return;int nb=int(std::min<Code>(65535,(n+threads-1)/threads));fn(nb);ck(cudaGetLastError(),w);};if(fixLow){auto xm=make_high_prerank_map(m.fmb,p,2);Code on=pg.ds.size,xn=xm.nblocks?xm.end[xm.nblocks-1]:0;launchN(on,[&](int nb){factor_high_prerank_owner_blockorder_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,on,p);},"batchio high owner");if(xn){HighPreRankMap empty{};launchN(xn,[&](int nb){factor_high_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,empty,xm);},"batchio high cross");}}else{Code on=pg.ds.size;launchN(on,[&](int nb){factor_prerank_owner_blockorder_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,on,p);},"batchio low owner");auto sm=make_prerank_orbit_map(m.fmb,p,2);Code sn=sm.nblocks?sm.end[sm.nblocks-1]:0;if(p==1){auto im=make_prerank_orbit_map(m.fmb,p,3);Code in=im.nblocks?im.end[im.nblocks-1]:0;launchN(in+sn,[&](int nb){factor_prerank_closure_inv_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,im,sm,p);},"batchio p1 closure");}else launchN(sn,[&](int nb){factor_prerank_highrr_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,sm,p);},"batchio cross");}}
            }
        ck(cudaStreamSynchronize(c.sMain),"batchio transition batch sync");auto tt=std::chrono::steady_clock::now();batch_factor_io_kernel<true><<<std::max<size_t>(1,ht.size()),threads,0,c.sMain>>>(reinterpret_cast<Count*>(c.arena),dcfg,dt,ht.size());ck(cudaGetLastError(),"batchio scatter");ck(cudaStreamSynchronize(c.sMain),"batchio scatter sync");auto ts=std::chrono::steady_clock::now();cudaFree(dcfg);cudaFree(dt);c.groups+=meta.size();c.gather_s+=std::chrono::duration<double>(tg-t0).count();c.transition_s+=std::chrono::duration<double>(tt-tg).count();c.scatter_s+=std::chrono::duration<double>(ts-tt).count();c.active+=std::chrono::duration<double>(ts-t0).count();q0=q;
    }
}
...
int main(int argc,char**argv){
    ...
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        ...
        int done_windows=0;
        for(int row=prefixK;row<W-1;++row){for(auto const&pw:schedule){if(ng==1)process_window_batched_io(ctx[0],W,pw,threads,target);else{int nj=(int)pw.groups.size();std::atomic<int>next{0};std::vector<std::thread>ths;ths.reserve(ng);for(int d=0;d<ng;++d)ths.emplace_back([&,d]{for(;;){int q=next.fetch_add(1,std::memory_order_relaxed);if(q>=nj)break;process_group(ctx[d],W,pw.wp,pw.groups[q],threads,target);}});for(auto&t:ths)t.join();}++done_windows;}std::cerr<<"mod "<<(ri+1)<<"/"<<mods.size()<<" p="<<mod<<" row "<<row+1<<"/"<<W<<"\n";}
        Count ans=0;
        for(int d=0;d<ng;++d){
        ...
        double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();double mx=0,sum=0,gs=0,ts=0,ss=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;gs+=c.gather_s;ts+=c.transition_s;ss+=c.scatter_s;maxIntervals=std::max(maxIntervals,c.maxIntervals);}std::cout<<"backend=gridfp-b300-hbm32-factorized-fullprerank-orbit-blockfused-batchio-batch n="<<n<<" residue="<<ans<<" modulus="<<mod<<" residue_index="<<ri<<" residues_total="<<mods.size()<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" scratch_target_mib="<<effective_target_mib<<" windows="<<done_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" gather_sum_s="<<gs<<" transition_sum_s="<<ts<<" scatter_sum_s="<<ss<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<std::endl;
    }

    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(fLA[d])cudaFree(fLA[d]);if(fLM[d])cudaFree(fLM[d]);if(fLO[d])cudaFree(fLO[d]);fLDv[d].destroy();if(fHA[d])cudaFree(fHA[d]);if(fHM[d])cudaFree(fHM[d]);if(fHO[d])cudaFree(fHO[d]);fHRv[d].destroy();if(fHMB[d])cudaFree(fHMB[d]);if(fHBB[d])cudaFree(fHBB[d]);if(prOB[d])cudaFree(prOB[d]);if(prBCM[d])cudaFree(prBCM[d]);if(prBCS[d])cudaFree(prBCS[d]);if(prCI[d])cudaFree(prCI[d]);if(prCIS[d])cudaFree(prCIS[d]);if(prH[d])cudaFree(prH[d]);if(hpOB[d])cudaFree(hpOB[d]);if(hpBCM[d])cudaFree(hpBCM[d]);if(hpBCS[d])cudaFree(hpBCS[d]);if(hpCI[d])cudaFree(hpCI[d]);if(hpCIS[d])cudaFree(hpCIS[d]);if(hpX[d])cudaFree(hpX[d]);}
}
