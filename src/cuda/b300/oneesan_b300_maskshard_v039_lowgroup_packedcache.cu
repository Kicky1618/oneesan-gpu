// v0.39 experiment: v0.38 packed LOW group constant config plus a setup-time
// host cache of all fixed HIGH-mask base FBlock layouts.  Per-row execution no
// longer rebuilds factor block vectors for every mask.
...
#define main oneesan_maskshard_v39_lowgrouppackedcache_inner_main
...
int main(int argc, char** argv) {
    ...
    if (threads < 32 || threads > 1024 || (threads & 31)) {
        std::cerr << "v0.39 packed LOW group cache requires threads to be a multiple of 32 in [32,1024]\n";
        return 1;
    }
    std::cerr
        << "backend_alias=b300-factorized-maskshard-v0.39-lowgroup-packedcache-batch"
        << " low_group_packed_config=1 low_group_packed_cache=1"
        << " low_orbit_warp_row_tasks=1 low_orbit_warp_row_u32=1"
        << " low_group_sync=1 guarded_hbm=1\n";
    const int rc = oneesan_maskshard_v39_lowgrouppackedcache_inner_main(argc, argv);
    ...
    if (rc == 0 && skipped != executed * skips_per_group) {
        std::cerr << "v0.39 LOW group sync count mismatch skipped=" << skipped
                  << " executed=" << executed << '\n';
        return 4;
    }
    return rc;
