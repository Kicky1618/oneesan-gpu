...
// Half-height DP + packed sparse connected-meander central join.
// .qfac stores each sparse entry as [10-bit rational-coefficient id | 22-bit Catalan dst].
static constexpr uint32_t HALF_DST_BITS=22, HALF_DST_MASK=(1u<<HALF_DST_BITS)-1u;
__constant__ Count D_HALF_COEFF[1024];
struct HalfFactorHost {
    uint32_t n=0,rn=0,en=0;
    std::vector<Count> coeff;
    std::vector<uint16_t> diag;
    std::vector<uint32_t> ro,eo,Rv,Ev;
};
static uint32_t half_pow_host(uint32_t a,uint64_t e,uint32_t mod){uint64_t r=1,x=a;while(e){if(e&1)r=r*x%mod;x=x*x%mod;e>>=1;}return uint32_t(r);}
static HalfFactorHost half_load_factor(int m,Count targetMod){
    const char* root=std::getenv("GRIDFP_MEANDER_DIR");std::string dir=root?root:"work/formal-probes";
    std::string fn=dir+"/meander_factor_m"+std::to_string(m)+".qfac";std::ifstream f(fn,std::ios::binary);
    if(!f){std::cerr<<"cannot open packed meander factor "<<fn<<"\n";std::exit(61);}uint64_t magic=0;uint32_t ver=0,nc=0,db=0;HalfFactorHost z;
    f.read((char*)&magic,8);f.read((char*)&ver,4);f.read((char*)&z.n,4);f.read((char*)&z.rn,4);f.read((char*)&z.en,4);f.read((char*)&nc,4);f.read((char*)&db,4);
    if(magic!=0x514641434d45414eULL||ver!=1||db!=HALF_DST_BITS){std::cerr<<"bad packed meander factor header "<<fn<<"\n";std::exit(62);}
    z.coeff.resize(nc);
    int maxscale=0;
    for(uint32_t i=0;i<nc;++i){int32_t num=0;uint32_t den=0;f.read((char*)&num,4);f.read((char*)&den,4);if(!den||den%targetMod==0){std::cerr<<"bad meander rational denominator m="<<m<<" den="<<den<<" mod="<<targetMod<<"\n";std::exit(68);}maxscale=std::max<int64_t>(maxscale,std::max<int64_t>(std::llabs((long long)num),int64_t(den)));uint32_t inv=half_pow_host(den%targetMod,uint64_t(targetMod)-2,targetMod);uint64_t aa=num<0?uint64_t(targetMod-uint32_t((-int64_t(num))%targetMod)):uint64_t(uint32_t(num)%targetMod);z.coeff[i]=uint32_t(aa*inv%targetMod);}
    z.diag.resize(z.n);f.read((char*)z.diag.data(),size_t(z.n)*sizeof(uint16_t));
    auto pos=uint64_t(f.tellg());if(pos&3)f.seekg(4-(pos&3),std::ios::cur);
    z.ro.resize(size_t(z.n)+1);z.eo.resize(size_t(z.n)+1);z.Rv.resize(z.rn);z.Ev.resize(z.en);
    f.read((char*)z.ro.data(),size_t(z.n+1)*sizeof(uint32_t));f.read((char*)z.eo.data(),size_t(z.n+1)*sizeof(uint32_t));
    f.read((char*)z.Rv.data(),size_t(z.rn)*sizeof(uint32_t));f.read((char*)z.Ev.data(),size_t(z.en)*sizeof(uint32_t));
    if(!f){std::cerr<<"truncated packed meander factor "<<fn<<"\n";std::exit(63);}
    std::cerr<<"half qfactor m="<<m<<" coeff_types="<<nc<<" maxscale="<<maxscale<<" bytes="<<(size_t(z.rn+z.en)*4+size_t(z.n)*2+size_t(2)*(z.n+1)*4)<<"\n";
    return z;
}
static void half_pat_rec(int p,int k,int h,uint64_t code,std::vector<uint64_t>&out){
    if(p==k){if(h==0)out.push_back(code);return;}
    if(h>0)half_pat_rec(p+1,k,h-1,code|(uint64_t(R)<<(2*p)),out);
    half_pat_rec(p+1,k,h+1,code|(uint64_t(::L)<<(2*p)),out);
}
static uint64_t half_reflect_pat(uint64_t code,int k){
    std::vector<int> mate(k,-2),st(1,-1);int defect=-1;
    for(int q=0;q<k;++q){int v=int((code>>(2*q))&3u);if(v==int(::L))st.push_back(q);else if(v==int(R)){int a=st.back();st.pop_back();if(a<0)defect=q;else mate[q]=a,mate[a]=q;}}
    if(defect<0){std::cerr<<"half pattern has no defect\n";std::exit(64);}std::vector<int> rm(k,-2);int rd=k-1-defect;
    for(int q=0;q<k;++q)if(q!=defect)rm[k-1-q]=k-1-mate[q];
    uint64_t out=0;for(int q=0;q<k;++q){int v=(q==rd||rm[q]<q)?int(R):int(::L);out|=uint64_t(v)<<(2*q);}return out;
}
static std::vector<uint32_t> half_masks(int W,int k){
    std::vector<uint32_t> v;if(k==0){v.push_back(0);return v;}uint64_t x=(1ull<<k)-1,limit=1ull<<W;
    while(x<limit){v.push_back((uint32_t)x);uint64_t c=x&-x,r=x+c;if(r>=limit)break;x=(((r^x)>>2)/c)|r;}return v;
}
static uint32_t half_reflect_mask(uint32_t x,int W){uint32_t y=0;for(int p=0;p<W;++p)if(x&(1u<<p))y|=1u<<(W-1-p);return y;}
__device__ __forceinline__ MateID half_make_mate(uint32_t mask,uint64_t pat,int k){MateID m=0;int q=0;
#pragma unroll
    for(int p=TARGET_W-1;p>=0;--p)if(mask&(1u<<p)){MateValue v=MateValue((pat>>(2*q))&3u);m|=MateID(v)<<(2*p);++q;}return m;}
__global__ void half_gather_xy(const uint32_t*masks,const uint32_t*rmasks,int B,const uint64_t*pat,const uint64_t*rpat,int dim,int k,Count*x,Count*y){
    size_t n=size_t(dim)*B;for(size_t z=size_t(blockIdx.x)*blockDim.x+threadIdx.x,st=size_t(gridDim.x)*blockDim.x;z<n;z+=st){int i=int(z/B),b=int(z-size_t(i)*B);MateID a=half_make_mate(masks[b],pat[i],k),c=half_make_mate(rmasks[b],rpat[i],k);x[z]=global_load_main(factor_global_rank_main(a));y[z]=global_load_main(factor_global_rank_main(c));}}
__device__ __forceinline__ Count half_add(Count a,Count b){return r3_add_mod(a,b);}
__device__ __forceinline__ Count half_mul(Count a,Count b){return r3_mul_mod(a,b);}
__global__ void half_factor_r(int n,const uint32_t*off,const uint32_t*e,const Count*y,Count*z,int B){int lane=threadIdx.x&31;Code wg=(Code(blockIdx.x)*blockDim.x+threadIdx.x)>>5,step=(Code(gridDim.x)*blockDim.x)>>5;int groups=(B+31)>>5;for(;wg<Code(n)*groups;wg+=step){int i=int(wg/groups),g=int(wg-Code(i)*groups),b=g*32+lane;Count acc=0;for(uint32_t q=off[i];q<off[i+1];++q){uint32_t v=0;if(lane==0)v=e[q];v=__shfl_sync(0xffffffffu,v,0);if(b<B){uint32_t d=v&HALF_DST_MASK,c=v>>HALF_DST_BITS;acc=half_add(acc,half_mul(D_HALF_COEFF[c],y[size_t(d)*B+b]));}}if(b<B)z[size_t(i)*B+b]=acc;}}
__global__ void half_factor_e(int n,const uint32_t*off,const uint32_t*e,const uint16_t*diag,const Count*z,Count*out,int B){int lane=threadIdx.x&31;Code wg=(Code(blockIdx.x)*blockDim.x+threadIdx.x)>>5,step=(Code(gridDim.x)*blockDim.x)>>5;int groups=(B+31)>>5;for(;wg<Code(n)*groups;wg+=step){int i=int(wg/groups),g=int(wg-Code(i)*groups),b=g*32+lane;Count acc=0;if(b<B)acc=half_mul(D_HALF_COEFF[diag[i]],z[size_t(i)*B+b]);for(uint32_t q=off[i];q<off[i+1];++q){uint32_t v=0;if(lane==0)v=e[q];v=__shfl_sync(0xffffffffu,v,0);if(b<B){uint32_t d=v&HALF_DST_MASK,c=v>>HALF_DST_BITS;acc=half_add(acc,half_mul(D_HALF_COEFF[c],z[size_t(d)*B+b]));}}if(b<B)out[size_t(i)*B+b]=acc;}}
__global__ void half_factor_r_small(int n,const uint32_t*off,const uint32_t*e,const Count*y,Count*z,int B){
    int lane=threadIdx.x&31;Code w=(Code(blockIdx.x)*blockDim.x+threadIdx.x)>>5,step=(Code(gridDim.x)*blockDim.x)>>5,total=Code(n)*B;
    for(;w<total;w+=step){int i=int(w/B),b=int(w-Code(i)*B);Count acc=0;for(uint32_t q=off[i]+lane;q<off[i+1];q+=32){uint32_t v=e[q],d=v&HALF_DST_MASK,c=v>>HALF_DST_BITS;acc=half_add(acc,half_mul(D_HALF_COEFF[c],y[size_t(d)*B+b]));}for(int sh=16;sh;sh>>=1)acc=half_add(acc,__shfl_down_sync(0xffffffffu,acc,sh));if(lane==0)z[size_t(i)*B+b]=acc;}
}
__global__ void half_factor_e_small(int n,const uint32_t*off,const uint32_t*e,const uint16_t*diag,const Count*z,Count*out,int B){
    int lane=threadIdx.x&31;Code w=(Code(blockIdx.x)*blockDim.x+threadIdx.x)>>5,step=(Code(gridDim.x)*blockDim.x)>>5,total=Code(n)*B;
    for(;w<total;w+=step){int i=int(w/B),b=int(w-Code(i)*B);Count acc=0;for(uint32_t q=off[i]+lane;q<off[i+1];q+=32){uint32_t v=e[q],d=v&HALF_DST_MASK,c=v>>HALF_DST_BITS;acc=half_add(acc,half_mul(D_HALF_COEFF[c],z[size_t(d)*B+b]));}for(int sh=16;sh;sh>>=1)acc=half_add(acc,__shfl_down_sync(0xffffffffu,acc,sh));if(lane==0){acc=half_add(acc,half_mul(D_HALF_COEFF[diag[i]],z[size_t(i)*B+b]));out[size_t(i)*B+b]=acc;}}
}
__global__ void half_dot_kernel(const Count*x,const Count*y,size_t n,Count*out){extern __shared__ Count halfsh[];Count a=0;for(size_t i=size_t(blockIdx.x)*blockDim.x+threadIdx.x,st=size_t(gridDim.x)*blockDim.x;i<n;i+=st)a=half_add(a,half_mul(x[i],y[i]));halfsh[threadIdx.x]=a;__syncthreads();for(int s=blockDim.x/2;s;s>>=1){if(threadIdx.x<s)halfsh[threadIdx.x]=half_add(halfsh[threadIdx.x],halfsh[threadIdx.x+s]);__syncthreads();}if(threadIdx.x==0)atomic_add_mod(out,halfsh[0]);}
static Count half_central_join_single_gpu(int W,int threads){
    ck(cudaSetDevice(0),"half join set");Count mod=0;ck(cudaMemcpyFromSymbol(&mod,D_MOD,sizeof(mod)),"half read mod");Count*ansd=nullptr;ck(cudaMalloc(&ansd,sizeof(Count)),"half ans");ck(cudaMemset(ansd,0,sizeof(Count)),"half ans zero");
    int requestedTile=8192;if(const char*e=std::getenv("GRIDFP_HALF_MASK_TILE"))requestedTile=std::max(32,std::atoi(e));double gather_s=0,factor_s=0,dot_s=0;uint64_t allmasks=0,coeffops=0;
    for(int m=1;2*m-1<=W;++m){int k=2*m-1;auto F=half_load_factor(m,mod);std::vector<uint64_t> pats;half_pat_rec(0,k,1,0,pats);if(pats.size()!=F.n){std::cerr<<"half factor dim mismatch m="<<m<<" pats="<<pats.size()<<" fac="<<F.n<<"\n";std::exit(66);}std::vector<uint64_t> rp(F.n);for(uint32_t i=0;i<F.n;++i)rp[i]=half_reflect_pat(pats[i],k);auto masks=half_masks(W,k);bool orbitHalf=(W%2==0);if(orbitHalf){std::vector<uint32_t> reps;reps.reserve(masks.size()/2+1);for(uint32_t q:masks){uint32_t rq=half_reflect_mask(q,W);if(q<rq)reps.push_back(q);else if(q==rq){std::cerr<<"unexpected self-reflect odd-occupancy mask W="<<W<<" k="<<k<<"\n";std::exit(69);}}masks.swap(reps);}allmasks+=masks.size();coeffops+=(F.rn+F.en+F.n)*masks.size();
        uint64_t *dpat=nullptr,*drpat=nullptr;uint32_t *dro=nullptr,*deo=nullptr,*dR=nullptr,*dE=nullptr;uint16_t *ddiag=nullptr;uint32_t *dm=nullptr,*drm=nullptr;ck(cudaMalloc(&dpat,F.n*8),"half pat");ck(cudaMalloc(&drpat,F.n*8),"half rpat");ck(cudaMalloc(&dro,(F.n+1)*4),"half ro");ck(cudaMalloc(&deo,(F.n+1)*4),"half eo");ck(cudaMalloc(&dR,size_t(F.rn)*4),"half R");ck(cudaMalloc(&dE,size_t(F.en)*4),"half E");ck(cudaMalloc(&ddiag,F.n*2),"half diag");size_t hf=0,ht=0;ck(cudaMemGetInfo(&hf,&ht),"half join meminfo");size_t reserveJoin=512ull<<20;size_t bytesPerMask=size_t(F.n)*sizeof(Count)*4;size_t maxTile=bytesPerMask?((hf>reserveJoin?hf-reserveJoin:0)/bytesPerMask):size_t(requestedTile);int tile=int(std::min<size_t>(requestedTile,std::max<size_t>(1,maxTile)));if(tile>=32)tile=(tile/32)*32;tile=std::max(1,tile);tile=std::min<int>(tile,std::max<size_t>(1,masks.size()));std::cerr<<"half join tile m="<<m<<" requested="<<requestedTile<<" selected="<<tile<<" free_mib="<<(hf>>20)<<" bytes_per_mask="<<bytesPerMask<<"\n";ck(cudaMalloc(&dm,size_t(tile)*4),"half masks");ck(cudaMalloc(&drm,size_t(tile)*4),"half rmasks");ck(cudaMemcpy(dpat,pats.data(),F.n*8,cudaMemcpyHostToDevice),"half pat copy");ck(cudaMemcpy(drpat,rp.data(),F.n*8,cudaMemcpyHostToDevice),"half rpat copy");ck(cudaMemcpy(dro,F.ro.data(),(F.n+1)*4,cudaMemcpyHostToDevice),"half ro copy");ck(cudaMemcpy(deo,F.eo.data(),(F.n+1)*4,cudaMemcpyHostToDevice),"half eo copy");ck(cudaMemcpy(dR,F.Rv.data(),size_t(F.rn)*4,cudaMemcpyHostToDevice),"half R copy");ck(cudaMemcpy(dE,F.Ev.data(),size_t(F.en)*4,cudaMemcpyHostToDevice),"half E copy");ck(cudaMemcpy(ddiag,F.diag.data(),F.n*2,cudaMemcpyHostToDevice),"half diag copy");ck(cudaMemcpyToSymbol(D_HALF_COEFF,F.coeff.data(),F.coeff.size()*4),"half coeff const");
        size_t cap=size_t(F.n)*std::min<int>(tile,masks.size());Count *dx=nullptr,*dy=nullptr,*dz=nullptr,*do_=nullptr;ck(cudaMalloc(&dx,cap*4),"half x");ck(cudaMalloc(&dy,cap*4),"half y");ck(cudaMalloc(&dz,cap*4),"half z");ck(cudaMalloc(&do_,cap*4),"half out");
        for(size_t base=0;base<masks.size();base+=tile){int B=int(std::min<size_t>(tile,masks.size()-base));std::vector<uint32_t> rms(B);for(int b=0;b<B;++b)rms[b]=half_reflect_mask(masks[base+b],W);ck(cudaMemcpy(dm,masks.data()+base,B*4,cudaMemcpyHostToDevice),"half mask copy");ck(cudaMemcpy(drm,rms.data(),B*4,cudaMemcpyHostToDevice),"half rmask copy");cudaEvent_t a,b,c,d;cudaEventCreate(&a);cudaEventCreate(&b);cudaEventCreate(&c);cudaEventCreate(&d);cudaEventRecord(a);size_t ne=size_t(F.n)*B;int gb=int(std::min<size_t>(65535,(ne+threads-1)/threads));half_gather_xy<<<std::max(1,gb),threads>>>(dm,drm,B,dpat,drpat,F.n,k,dx,dy);cudaEventRecord(b);int groups=(B+31)/32;Code warps=Code(F.n)*groups;int fb=int(std::min<Code>(65535,(warps*32+threads-1)/threads));half_factor_r<<<std::max(1,fb),threads>>>(F.n,dro,dR,dy,dz,B);half_factor_e<<<std::max(1,fb),threads>>>(F.n,deo,dE,ddiag,dz,do_,B);cudaEventRecord(c);int db=int(std::min<size_t>(4096,(ne+threads-1)/threads));half_dot_kernel<<<std::max(1,db),threads,threads*sizeof(Count)>>>(dx,do_,ne,ansd);cudaEventRecord(d);ck(cudaEventSynchronize(d),"half tile sync");float t1=0,t2=0,t3=0;cudaEventElapsedTime(&t1,a,b);cudaEventElapsedTime(&t2,b,c);cudaEventElapsedTime(&t3,c,d);gather_s+=t1/1000.;factor_s+=t2/1000.;dot_s+=t3/1000.;cudaEventDestroy(a);cudaEventDestroy(b);cudaEventDestroy(c);cudaEventDestroy(d);}
        cudaFree(dx);cudaFree(dy);cudaFree(dz);cudaFree(do_);cudaFree(dpat);cudaFree(drpat);cudaFree(dro);cudaFree(deo);cudaFree(dR);cudaFree(dE);cudaFree(ddiag);cudaFree(dm);cudaFree(drm);std::cerr<<"half join m="<<m<<" k="<<k<<" dim="<<F.n<<" masks="<<masks.size()<<" factor_coeff="<<(F.rn+F.en+F.n)<<"\n";
    }
    Count ans=0;ck(cudaMemcpy(&ans,ansd,sizeof(ans),cudaMemcpyDeviceToHost),"half ans copy");cudaFree(ansd);if(W%2==0)ans=ans>=mod-ans?ans-(mod-ans):ans+ans;std::cerr<<"half join total masks="<<allmasks<<" coeffops="<<coeffops<<" orbit_half="<<(W%2==0)<<" gather_s="<<gather_s<<" factor_s="<<factor_s<<" dot_s="<<dot_s<<"\n";return ans;
}

struct HalfJoinDeviceStat { Count ans=0; double gather_s=0,factor_s=0,dot_s=0; };

struct HalfMappedFactor {
    int fd=-1; size_t bytes=0; const uint8_t*base=nullptr;
    uint32_t n=0,rn=0,en=0,nc=0;
    const int32_t*num=nullptr; const uint32_t*den=nullptr; // interleaved in file; accessed via base below
    const uint8_t*ratbase=nullptr; const uint16_t*diag=nullptr; const uint32_t*ro=nullptr,*eo=nullptr,*Rv=nullptr,*Ev=nullptr;
};
static std::array<std::shared_ptr<HalfMappedFactor>,16> G_HALF_MMAP{};
static const HalfMappedFactor& half_map_factor(int m){
    if(G_HALF_MMAP[m])return *G_HALF_MMAP[m];
    const char*root=std::getenv("GRIDFP_MEANDER_DIR");std::string dir=root?root:"work/formal-probes";std::string fn=dir+"/meander_factor_m"+std::to_string(m)+".qfac";
    int fd=open(fn.c_str(),O_RDONLY);if(fd<0){std::cerr<<"cannot mmap meander factor "<<fn<<"\n";std::exit(61);}struct stat sb{};if(fstat(fd,&sb)){std::cerr<<"fstat failed "<<fn<<"\n";std::exit(61);}size_t sz=sb.st_size;auto*b=(const uint8_t*)mmap(nullptr,sz,PROT_READ,MAP_PRIVATE,fd,0);if(b==MAP_FAILED){std::cerr<<"mmap failed "<<fn<<"\n";std::exit(61);}const uint8_t*q=b;
    auto rd32=[&](){uint32_t x;memcpy(&x,q,4);q+=4;return x;};auto rd64=[&](){uint64_t x;memcpy(&x,q,8);q+=8;return x;};uint64_t magic=rd64();uint32_t ver=rd32();auto z=std::make_shared<HalfMappedFactor>();z->fd=fd;z->bytes=sz;z->base=b;z->n=rd32();z->rn=rd32();z->en=rd32();z->nc=rd32();uint32_t db=rd32();if(magic!=0x514641434d45414eULL||ver!=1||db!=HALF_DST_BITS||z->nc>1024){std::cerr<<"bad mmap qfac "<<fn<<"\n";std::exit(62);}z->ratbase=q;q+=size_t(z->nc)*8;z->diag=(const uint16_t*)q;q+=size_t(z->n)*2;while((q-b)&3)++q;z->ro=(const uint32_t*)q;q+=size_t(z->n+1)*4;z->eo=(const uint32_t*)q;q+=size_t(z->n+1)*4;z->Rv=(const uint32_t*)q;q+=size_t(z->rn)*4;z->Ev=(const uint32_t*)q;q+=size_t(z->en)*4;if(size_t(q-b)!=sz){std::cerr<<"qfac mmap size mismatch "<<fn<<" parsed="<<(q-b)<<" size="<<sz<<"\n";std::exit(63);}madvise((void*)b,sz,MADV_SEQUENTIAL);G_HALF_MMAP[m]=z;std::cerr<<"half mmap factor m="<<m<<" n="<<z->n<<" types="<<z->nc<<" mib="<<double(sz)/(1<<20)<<"\n";return *z;
}
static std::vector<Count> half_mapped_coeff(const HalfMappedFactor&F,Count mod){
    std::vector<Count> c(F.nc);for(uint32_t i=0;i<F.nc;++i){int32_t num;uint32_t den;memcpy(&num,F.ratbase+size_t(i)*8,4);memcpy(&den,F.ratbase+size_t(i)*8+4,4);if(!den||den%mod==0){std::cerr<<"bad mmap coeff denominator\n";std::exit(68);}uint32_t inv=half_pow_host(den%mod,uint64_t(mod)-2,mod);uint64_t aa=num<0?uint64_t(mod-uint32_t((-int64_t(num))%mod)):uint64_t(uint32_t(num)%mod);c[i]=uint32_t(aa*inv%mod);}return c;
}
struct HalfGeomCache {int W=0,k=0;std::vector<uint64_t>pats,rp;std::vector<uint32_t>masks;};
static std::array<std::shared_ptr<HalfGeomCache>,16> G_HALF_GEOM{};
static const HalfGeomCache& half_geom(int W,int m){
    if(G_HALF_GEOM[m]&&G_HALF_GEOM[m]->W==W)return *G_HALF_GEOM[m];int k=2*m-1;auto g=std::make_shared<HalfGeomCache>();g->W=W;g->k=k;half_pat_rec(0,k,1,0,g->pats);g->rp.resize(g->pats.size());
    // Heap-free reflection: same semantics as half_reflect_pat.
    for(size_t ii=0;ii<g->pats.size();++ii){uint64_t code=g->pats[ii];int mate[32],st[33],rm[32],sp=1,def=-1;std::fill(mate,mate+k,-2);std::fill(rm,rm+k,-2);st[0]=-1;for(int q=0;q<k;++q){int v=int((code>>(2*q))&3u);if(v==int(::L))st[sp++]=q;else{int a=st[--sp];if(a<0)def=q;else mate[q]=a,mate[a]=q;}}int rd=k-1-def;for(int q=0;q<k;++q)if(q!=def)rm[k-1-q]=k-1-mate[q];uint64_t out=0;for(int q=0;q<k;++q){int v=(q==rd||rm[q]<q)?int(R):int(::L);out|=uint64_t(v)<<(2*q);}g->rp[ii]=out;}
    g->masks=half_masks(W,k);if(W%2==0){std::vector<uint32_t>r;r.reserve(g->masks.size()/2+1);for(uint32_t q:g->masks){uint32_t rq=half_reflect_mask(q,W);if(q<rq)r.push_back(q);else if(q==rq){std::cerr<<"unexpected self-reflect mask cache\n";std::exit(69);}}g->masks.swap(r);}G_HALF_GEOM[m]=g;std::cerr<<"half geom cache m="<<m<<" dim="<<g->pats.size()<<" masks="<<g->masks.size()<<"\n";return *g;
}

static HalfJoinDeviceStat half_join_one_m_device_mmap(int dev,int W,int threads,const HalfMappedFactor&F,const std::vector<Count>&coeff,
        const HalfGeomCache&G,const std::vector<uint32_t>&masks,int requestedTile){
    HalfJoinDeviceStat st;if(masks.empty())return st;ck(cudaSetDevice(dev),"half mmap set");Count*ansd=nullptr;ck(cudaMalloc(&ansd,4),"half mmap ans");ck(cudaMemset(ansd,0,4),"half mmap ans zero");
    uint64_t*dpat=nullptr,*drpat=nullptr;uint32_t*dro=nullptr,*deo=nullptr,*dR=nullptr,*dE=nullptr,*dm=nullptr,*drm=nullptr;uint16_t*ddiag=nullptr;
    ck(cudaMalloc(&dpat,size_t(F.n)*8),"hm pat");ck(cudaMalloc(&drpat,size_t(F.n)*8),"hm rpat");ck(cudaMalloc(&dro,size_t(F.n+1)*4),"hm ro");ck(cudaMalloc(&deo,size_t(F.n+1)*4),"hm eo");ck(cudaMalloc(&dR,size_t(F.rn)*4),"hm R");ck(cudaMalloc(&dE,size_t(F.en)*4),"hm E");ck(cudaMalloc(&ddiag,size_t(F.n)*2),"hm diag");
    size_t hf=0,ht=0;ck(cudaMemGetInfo(&hf,&ht),"hm meminfo");size_t bpm=size_t(F.n)*16,reserve=512ull<<20,maxTile=bpm?((hf>reserve?hf-reserve:0)/bpm):requestedTile;int tile=int(std::min<size_t>(requestedTile,std::max<size_t>(1,maxTile)));if(tile>=32)tile=(tile/32)*32;tile=std::max(1,tile);tile=std::min<int>(tile,masks.size());ck(cudaMalloc(&dm,size_t(tile)*4),"hm mask");ck(cudaMalloc(&drm,size_t(tile)*4),"hm rmask");
    ck(cudaMemcpy(dpat,G.pats.data(),size_t(F.n)*8,cudaMemcpyHostToDevice),"hm pat cp");ck(cudaMemcpy(drpat,G.rp.data(),size_t(F.n)*8,cudaMemcpyHostToDevice),"hm rp cp");ck(cudaMemcpy(dro,F.ro,size_t(F.n+1)*4,cudaMemcpyHostToDevice),"hm ro cp");ck(cudaMemcpy(deo,F.eo,size_t(F.n+1)*4,cudaMemcpyHostToDevice),"hm eo cp");ck(cudaMemcpy(dR,F.Rv,size_t(F.rn)*4,cudaMemcpyHostToDevice),"hm R cp");ck(cudaMemcpy(dE,F.Ev,size_t(F.en)*4,cudaMemcpyHostToDevice),"hm E cp");ck(cudaMemcpy(ddiag,F.diag,size_t(F.n)*2,cudaMemcpyHostToDevice),"hm diag cp");ck(cudaMemcpyToSymbol(D_HALF_COEFF,coeff.data(),coeff.size()*4),"hm coeff");
    size_t cap=size_t(F.n)*tile;Count*dx=nullptr,*dy=nullptr,*dz=nullptr,*doo=nullptr;ck(cudaMalloc(&dx,cap*4),"hm x");ck(cudaMalloc(&dy,cap*4),"hm y");ck(cudaMalloc(&dz,cap*4),"hm z");ck(cudaMalloc(&doo,cap*4),"hm o");
    for(size_t base=0;base<masks.size();base+=tile){int B=int(std::min<size_t>(tile,masks.size()-base));std::vector<uint32_t>rms(B);for(int b=0;b<B;++b)rms[b]=half_reflect_mask(masks[base+b],W);ck(cudaMemcpy(dm,masks.data()+base,size_t(B)*4,cudaMemcpyHostToDevice),"hm masks cp");ck(cudaMemcpy(drm,rms.data(),size_t(B)*4,cudaMemcpyHostToDevice),"hm rm cp");cudaEvent_t a,b,c,d;cudaEventCreate(&a);cudaEventCreate(&b);cudaEventCreate(&c);cudaEventCreate(&d);cudaEventRecord(a);size_t ne=size_t(F.n)*B;int gb=int(std::min<size_t>(65535,(ne+threads-1)/threads));half_gather_xy<<<std::max(1,gb),threads>>>(dm,drm,B,dpat,drpat,F.n,G.k,dx,dy);cudaEventRecord(b);if(B<=12){Code warps=Code(F.n)*B;int fb=int(std::min<Code>(65535,(warps*32+threads-1)/threads));half_factor_r_small<<<std::max(1,fb),threads>>>(F.n,dro,dR,dy,dz,B);half_factor_e_small<<<std::max(1,fb),threads>>>(F.n,deo,dE,ddiag,dz,doo,B);}else{int groups=(B+31)/32;Code warps=Code(F.n)*groups;int fb=int(std::min<Code>(65535,(warps*32+threads-1)/threads));half_factor_r<<<std::max(1,fb),threads>>>(F.n,dro,dR,dy,dz,B);half_factor_e<<<std::max(1,fb),threads>>>(F.n,deo,dE,ddiag,dz,doo,B);}cudaEventRecord(c);int db=int(std::min<size_t>(4096,(ne+threads-1)/threads));half_dot_kernel<<<std::max(1,db),threads,threads*4>>>(dx,doo,ne,ansd);cudaEventRecord(d);ck(cudaEventSynchronize(d),"hm sync");float t1,t2,t3;cudaEventElapsedTime(&t1,a,b);cudaEventElapsedTime(&t2,b,c);cudaEventElapsedTime(&t3,c,d);st.gather_s+=t1/1000.;st.factor_s+=t2/1000.;st.dot_s+=t3/1000.;cudaEventDestroy(a);cudaEventDestroy(b);cudaEventDestroy(c);cudaEventDestroy(d);}
    ck(cudaMemcpy(&st.ans,ansd,4,cudaMemcpyDeviceToHost),"hm ans cp");cudaFree(dx);cudaFree(dy);cudaFree(dz);cudaFree(doo);cudaFree(dpat);cudaFree(drpat);cudaFree(dro);cudaFree(deo);cudaFree(dR);cudaFree(dE);cudaFree(ddiag);cudaFree(dm);cudaFree(drm);cudaFree(ansd);return st;
}
static Count half_central_join_multi_gpu_mmap(int W,int threads,int ng,Count mod){
    int req=8192;if(const char*e=getenv("GRIDFP_HALF_MASK_TILE"))req=std::max(32,atoi(e));Count ans=0;double gs=0,fs=0,ds=0;uint64_t nm=0,ops=0;
    for(int m=1;2*m-1<=W;++m){auto const&F=half_map_factor(m);auto coeff=half_mapped_coeff(F,mod);auto const&G=half_geom(W,m);if(G.pats.size()!=F.n){std::cerr<<"mmap geom dim mismatch\n";exit(66);}std::vector<std::vector<uint32_t>>part(ng);for(size_t i=0;i<G.masks.size();++i)part[i%ng].push_back(G.masks[i]);std::vector<HalfJoinDeviceStat>st(ng);std::vector<std::thread>th;for(int d=0;d<ng;++d)th.emplace_back([&,d]{st[d]=half_join_one_m_device_mmap(d,W,threads,F,coeff,G,part[d],req);});for(auto&t:th)t.join();double gm=0,fm=0,dm=0;for(int d=0;d<ng;++d){Count x=st[d].ans;ans=ans>=mod-x?ans-(mod-x):ans+x;gm=std::max(gm,st[d].gather_s);fm=std::max(fm,st[d].factor_s);dm=std::max(dm,st[d].dot_s);}gs+=gm;fs+=fm;ds+=dm;nm+=G.masks.size();ops+=uint64_t(F.rn+F.en+F.n)*G.masks.size();std::cerr<<"half mmap mg m="<<m<<" masks="<<G.masks.size()<<" gpus="<<ng<<" crit="<<gm+fm+dm<<"\n";}
    if(W%2==0)ans=ans>=mod-ans?ans-(mod-ans):ans+ans;std::cerr<<"half mmap mg total masks="<<nm<<" ops="<<ops<<" gpus="<<ng<<" gather="<<gs<<" factor="<<fs<<" dot="<<ds<<"\n";return ans;
}




static HalfJoinDeviceStat half_join_one_m_device(int dev,int W,int threads,const HalfFactorHost&F,
        const std::vector<uint64_t>&pats,const std::vector<uint64_t>&rp,int k,
        const std::vector<uint32_t>&masks,int requestedTile){
    HalfJoinDeviceStat st; if(masks.empty()) return st;
    ck(cudaSetDevice(dev),"half mg set");
    Count*ansd=nullptr;ck(cudaMalloc(&ansd,sizeof(Count)),"half mg ans");ck(cudaMemset(ansd,0,sizeof(Count)),"half mg ans zero");
    uint64_t *dpat=nullptr,*drpat=nullptr;uint32_t *dro=nullptr,*deo=nullptr,*dR=nullptr,*dE=nullptr;uint16_t *ddiag=nullptr;uint32_t *dm=nullptr,*drm=nullptr;
    ck(cudaMalloc(&dpat,size_t(F.n)*8),"half mg pat");ck(cudaMalloc(&drpat,size_t(F.n)*8),"half mg rpat");
    ck(cudaMalloc(&dro,size_t(F.n+1)*4),"half mg ro");ck(cudaMalloc(&deo,size_t(F.n+1)*4),"half mg eo");
    ck(cudaMalloc(&dR,size_t(F.rn)*4),"half mg R");ck(cudaMalloc(&dE,size_t(F.en)*4),"half mg E");ck(cudaMalloc(&ddiag,size_t(F.n)*2),"half mg diag");
    size_t hf=0,ht=0;ck(cudaMemGetInfo(&hf,&ht),"half mg meminfo");size_t reserveJoin=512ull<<20;
    size_t bytesPerMask=size_t(F.n)*sizeof(Count)*4;size_t maxTile=bytesPerMask?((hf>reserveJoin?hf-reserveJoin:0)/bytesPerMask):size_t(requestedTile);
    int tile=int(std::min<size_t>(requestedTile,std::max<size_t>(1,maxTile)));if(tile>=32)tile=(tile/32)*32;tile=std::max(1,tile);tile=std::min<int>(tile,masks.size());
    ck(cudaMalloc(&dm,size_t(tile)*4),"half mg masks");ck(cudaMalloc(&drm,size_t(tile)*4),"half mg rmasks");
    ck(cudaMemcpy(dpat,pats.data(),size_t(F.n)*8,cudaMemcpyHostToDevice),"half mg pat copy");
    ck(cudaMemcpy(drpat,rp.data(),size_t(F.n)*8,cudaMemcpyHostToDevice),"half mg rpat copy");
    ck(cudaMemcpy(dro,F.ro.data(),size_t(F.n+1)*4,cudaMemcpyHostToDevice),"half mg ro copy");
    ck(cudaMemcpy(deo,F.eo.data(),size_t(F.n+1)*4,cudaMemcpyHostToDevice),"half mg eo copy");
    ck(cudaMemcpy(dR,F.Rv.data(),size_t(F.rn)*4,cudaMemcpyHostToDevice),"half mg R copy");
    ck(cudaMemcpy(dE,F.Ev.data(),size_t(F.en)*4,cudaMemcpyHostToDevice),"half mg E copy");
    ck(cudaMemcpy(ddiag,F.diag.data(),size_t(F.n)*2,cudaMemcpyHostToDevice),"half mg diag copy");
    ck(cudaMemcpyToSymbol(D_HALF_COEFF,F.coeff.data(),F.coeff.size()*sizeof(Count)),"half mg coeff const");
    size_t cap=size_t(F.n)*tile;Count *dx=nullptr,*dy=nullptr,*dz=nullptr,*do_=nullptr;
    ck(cudaMalloc(&dx,cap*4),"half mg x");ck(cudaMalloc(&dy,cap*4),"half mg y");ck(cudaMalloc(&dz,cap*4),"half mg z");ck(cudaMalloc(&do_,cap*4),"half mg out");
    for(size_t base=0;base<masks.size();base+=tile){
        int B=int(std::min<size_t>(tile,masks.size()-base));std::vector<uint32_t> rms(B);for(int b=0;b<B;++b)rms[b]=half_reflect_mask(masks[base+b],W);
        ck(cudaMemcpy(dm,masks.data()+base,size_t(B)*4,cudaMemcpyHostToDevice),"half mg mask copy");ck(cudaMemcpy(drm,rms.data(),size_t(B)*4,cudaMemcpyHostToDevice),"half mg rmask copy");
        cudaEvent_t a,b,c,d;cudaEventCreate(&a);cudaEventCreate(&b);cudaEventCreate(&c);cudaEventCreate(&d);cudaEventRecord(a);
        size_t ne=size_t(F.n)*B;int gb=int(std::min<size_t>(65535,(ne+threads-1)/threads));half_gather_xy<<<std::max(1,gb),threads>>>(dm,drm,B,dpat,drpat,F.n,k,dx,dy);cudaEventRecord(b);
        int groups=(B+31)/32;Code warps=Code(F.n)*groups;int fb=int(std::min<Code>(65535,(warps*32+threads-1)/threads));
        half_factor_r<<<std::max(1,fb),threads>>>(F.n,dro,dR,dy,dz,B);half_factor_e<<<std::max(1,fb),threads>>>(F.n,deo,dE,ddiag,dz,do_,B);cudaEventRecord(c);
        int db=int(std::min<size_t>(4096,(ne+threads-1)/threads));half_dot_kernel<<<std::max(1,db),threads,threads*sizeof(Count)>>>(dx,do_,ne,ansd);cudaEventRecord(d);ck(cudaEventSynchronize(d),"half mg tile sync");
        float t1=0,t2=0,t3=0;cudaEventElapsedTime(&t1,a,b);cudaEventElapsedTime(&t2,b,c);cudaEventElapsedTime(&t3,c,d);st.gather_s+=t1/1000.;st.factor_s+=t2/1000.;st.dot_s+=t3/1000.;
        cudaEventDestroy(a);cudaEventDestroy(b);cudaEventDestroy(c);cudaEventDestroy(d);
    }
    ck(cudaMemcpy(&st.ans,ansd,sizeof(Count),cudaMemcpyDeviceToHost),"half mg ans copy");
    cudaFree(dx);cudaFree(dy);cudaFree(dz);cudaFree(do_);cudaFree(dpat);cudaFree(drpat);cudaFree(dro);cudaFree(deo);cudaFree(dR);cudaFree(dE);cudaFree(ddiag);cudaFree(dm);cudaFree(drm);cudaFree(ansd);
    return st;
}

static Count half_central_join_multi_gpu(int W,int threads,int ng,Count mod){
    int requestedTile=8192;if(const char*e=std::getenv("GRIDFP_HALF_MASK_TILE"))requestedTile=std::max(32,std::atoi(e));
    Count ans=0;double gatherMax=0,factorMax=0,dotMax=0;uint64_t allmasks=0,coeffops=0;
    for(int m=1;2*m-1<=W;++m){
        int k=2*m-1;auto F=half_load_factor(m,mod);std::vector<uint64_t> pats;half_pat_rec(0,k,1,0,pats);if(pats.size()!=F.n){std::cerr<<"half mg factor dim mismatch m="<<m<<"\n";std::exit(66);}std::vector<uint64_t>rp(F.n);for(uint32_t i=0;i<F.n;++i)rp[i]=half_reflect_pat(pats[i],k);
        auto masks=half_masks(W,k);bool orbitHalf=(W%2==0);if(orbitHalf){std::vector<uint32_t>reps;reps.reserve(masks.size()/2+1);for(uint32_t q:masks){uint32_t rq=half_reflect_mask(q,W);if(q<rq)reps.push_back(q);else if(q==rq){std::cerr<<"unexpected self-reflect odd-occupancy mask W="<<W<<" k="<<k<<"\n";std::exit(69);}}masks.swap(reps);}
        allmasks+=masks.size();coeffops+=uint64_t(F.rn+F.en+F.n)*masks.size();
        std::vector<std::vector<uint32_t>> part(ng);for(size_t i=0;i<masks.size();++i)part[i%ng].push_back(masks[i]);
        std::vector<HalfJoinDeviceStat> stats(ng);std::vector<std::thread> th;th.reserve(ng);
        for(int d=0;d<ng;++d)th.emplace_back([&,d]{stats[d]=half_join_one_m_device(d,W,threads,F,pats,rp,k,part[d],requestedTile);});for(auto&t:th)t.join();
        double gm=0,fm=0,dmx=0;for(int d=0;d<ng;++d){Count x=stats[d].ans;ans=(ans>=mod-x)?ans-(mod-x):ans+x;gm=std::max(gm,stats[d].gather_s);fm=std::max(fm,stats[d].factor_s);dmx=std::max(dmx,stats[d].dot_s);}gatherMax+=gm;factorMax+=fm;dotMax+=dmx;
        std::cerr<<"half mg join m="<<m<<" k="<<k<<" dim="<<F.n<<" masks="<<masks.size()<<" gpus="<<ng<<" factor_coeff="<<(uint64_t(F.rn)+F.en+F.n)<<" max_gather_s="<<gm<<" max_factor_s="<<fm<<" max_dot_s="<<dmx<<"\n";
    }
    if(W%2==0)ans=ans>=mod-ans?ans-(mod-ans):ans+ans;
    std::cerr<<"half mg join total masks="<<allmasks<<" coeffops="<<coeffops<<" gpus="<<ng<<" orbit_half="<<(W%2==0)<<" gather_crit_s="<<gatherMax<<" factor_crit_s="<<factorMax<<" dot_crit_s="<<dotMax<<"\n";
    return ans;
}
...
struct DeviceCtx{
    ...
    void release_scratch(){if(dev<0)return;cudaSetDevice(dev);if(arena){cudaFree(arena);arena=nullptr;capArena=0;}if(dIM){cudaFree(dIM);dIM=nullptr;capIM=0;}if(dID){cudaFree(dID);dID=nullptr;capID=0;}dA=dB=dD=dE=nullptr;dMate=nullptr;}
    void destroy(){if(dev<0)return;release_scratch();cudaSetDevice(dev);if(copyDone)cudaEventDestroy(copyDone);if(clearDone)cudaEventDestroy(clearDone);if(mainDone)cudaEventDestroy(mainDone);if(blockDone)cudaEventDestroy(blockDone);if(sMain)cudaStreamDestroy(sMain);if(sBlock)cudaStreamDestroy(sBlock);}
};
...
int main(int argc,char**argv){
    ...
    bool halfMode=true;if(const char*e=std::getenv("GRIDFP_HALF_MEANDER"))halfMode=std::atoi(e)!=0;
    for(size_t ri=0;ri<mods.size();++ri){Count mod=mods[ri];
        for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"set residue reset");if(bl[d]&&!bp[d])ck(cudaMalloc(&bp[d],size_t(bl[d])*sizeof(Count)),"realloc auth block");}
        for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"refresh block ptrs");ck(cudaMemcpyToSymbol(D_BLOCK_PTR,bp,sizeof(Count*)*MAXGPU),"refresh block ptrs");}
        ...
        int prefixK=halfMode?6:5;if(const char*e=std::getenv("GRIDFP_BOUNDED_PREFIX_K"))prefixK=std::max(1,std::min(W-2,std::atoi(e)));
        ...
        int done_windows=0;
        int rowStop=halfMode?((n+1)/2):(W-1);for(int row=prefixK;row<rowStop;++row){for(auto const&pw:schedule){int nj=(int)pw.groups.size();std::atomic<int>next{0};std::vector<std::thread>ths;ths.reserve(ng);for(int d=0;d<ng;++d)ths.emplace_back([&,d]{for(;;){int q=next.fetch_add(1,std::memory_order_relaxed);if(q>=nj)break;process_group(ctx[d],W,pw.wp,pw.groups[q],threads,target);}});for(auto&t:ths)t.join();++done_windows;}std::cerr<<"mod "<<(ri+1)<<"/"<<mods.size()<<" p="<<mod<<" row "<<row+1<<"/"<<W<<"\n";}
        Count ans=0;if(halfMode){
            for(auto&c:ctx)c.release_scratch();
            for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"half free blocked set");if(bp[d]){ck(cudaFree(bp[d]),"half free blocked");bp[d]=nullptr;}}
            for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"half null block ptrs");ck(cudaMemcpyToSymbol(D_BLOCK_PTR,bp,sizeof(Count*)*MAXGPU),"half null block ptrs");}
            ans=half_central_join_multi_gpu_mmap(W,threads,ng,mod);
        }
        if(!halfMode)for(int d=0;d<ng;++d){
