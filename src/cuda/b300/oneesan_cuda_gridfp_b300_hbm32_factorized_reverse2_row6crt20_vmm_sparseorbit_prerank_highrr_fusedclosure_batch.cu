        ...
        for(int p=wp.p_hi;p>=wp.p_lo;--p){
            {
                ...
                auto cm=make_prerank_orbit_map(fmb,p,1),sm=make_prerank_orbit_map(fmb,p,2);Code cn=cm.nblocks?cm.end[cm.nblocks-1]:0,sn=sm.nblocks?sm.end[sm.nblocks-1]:0;launchN(cn+sn,[&](int nb){factor_prerank_closure_combined_kernel<<<std::max(1,nb),threads,0,c.sMain>>>(cur,dcur,cm,sm,p);},"factor prerank fused closure");
            }
        }
