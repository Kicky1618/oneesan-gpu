...
__global__ void half_factor_r(int n,const uint32_t*off,const uint32_t*e,const Count*cf,const Count*y,Count*z,int B){int lane=threadIdx.x&31;Code wg=(Code(blockIdx.x)*blockDim.x+threadIdx.x)>>5,step=(Code(gridDim.x)*blockDim.x)>>5;int groups=(B+31)>>5;for(;wg<Code(n)*groups;wg+=step){int i=int(wg/groups),g=int(wg-Code(i)*groups),b=g*32+lane;Count acc=0;for(uint32_t q=off[i];q<off[i+1];++q){uint32_t v=0;if(lane==0)v=e[q];v=__shfl_sync(0xffffffffu,v,0);if(b<B){uint32_t d=v&HALF_DST_MASK,c=v>>HALF_DST_BITS;acc=half_add(acc,half_mul(cf[c],y[size_t(d)*B+b]));}}if(b<B)z[size_t(i)*B+b]=acc;}}
__global__ void half_factor_e(int n,const uint32_t*off,const uint32_t*e,const uint16_t*diag,const Count*cf,const Count*z,Count*out,int B){int lane=threadIdx.x&31;Code wg=(Code(blockIdx.x)*blockDim.x+threadIdx.x)>>5,step=(Code(gridDim.x)*blockDim.x)>>5;int groups=(B+31)>>5;for(;wg<Code(n)*groups;wg+=step){int i=int(wg/groups),g=int(wg-Code(i)*groups),b=g*32+lane;Count acc=0;if(b<B)acc=half_mul(cf[diag[i]],z[size_t(i)*B+b]);for(uint32_t q=off[i];q<off[i+1];++q){uint32_t v=0;if(lane==0)v=e[q];v=__shfl_sync(0xffffffffu,v,0);if(b<B){uint32_t d=v&HALF_DST_MASK,c=v>>HALF_DST_BITS;acc=half_add(acc,half_mul(cf[c],z[size_t(d)*B+b]));}}if(b<B)out[size_t(i)*B+b]=acc;}}
...
static Count half_central_join_single_gpu(int W,int threads){
    ...
    for(int m=1;2*m-1<=W;++m){int k=2*m-1;auto F=half_load_factor(m,mod);std::vector<uint64_t> pats;half_pat_rec(0,k,1,0,pats);if(pats.size()!=F.n){std::cerr<<"half factor dim mismatch m="<<m<<" pats="<<pats.size()<<" fac="<<F.n<<"\n";std::exit(66);}std::vector<uint64_t> rp(F.n);for(uint32_t i=0;i<F.n;++i)rp[i]=half_reflect_pat(pats[i],k);auto masks=half_masks(W,k);allmasks+=masks.size();coeffops+=(F.rn+F.en+F.n)*masks.size();
        uint64_t *dpat=nullptr,*drpat=nullptr;uint32_t *dro=nullptr,*deo=nullptr,*dR=nullptr,*dE=nullptr;uint16_t *ddiag=nullptr;Count *dcf=nullptr;uint32_t *dm=nullptr,*drm=nullptr;ck(cudaMalloc(&dpat,F.n*8),"half pat");ck(cudaMalloc(&drpat,F.n*8),"half rpat");ck(cudaMalloc(&dro,(F.n+1)*4),"half ro");ck(cudaMalloc(&deo,(F.n+1)*4),"half eo");ck(cudaMalloc(&dR,size_t(F.rn)*4),"half R");ck(cudaMalloc(&dE,size_t(F.en)*4),"half E");ck(cudaMalloc(&ddiag,F.n*2),"half diag");ck(cudaMalloc(&dcf,F.coeff.size()*4),"half coeff");ck(cudaMalloc(&dm,size_t(tile)*4),"half masks");ck(cudaMalloc(&drm,size_t(tile)*4),"half rmasks");ck(cudaMemcpy(dpat,pats.data(),F.n*8,cudaMemcpyHostToDevice),"half pat copy");ck(cudaMemcpy(drpat,rp.data(),F.n*8,cudaMemcpyHostToDevice),"half rpat copy");ck(cudaMemcpy(dro,F.ro.data(),(F.n+1)*4,cudaMemcpyHostToDevice),"half ro copy");ck(cudaMemcpy(deo,F.eo.data(),(F.n+1)*4,cudaMemcpyHostToDevice),"half eo copy");ck(cudaMemcpy(dR,F.Rv.data(),size_t(F.rn)*4,cudaMemcpyHostToDevice),"half R copy");ck(cudaMemcpy(dE,F.Ev.data(),size_t(F.en)*4,cudaMemcpyHostToDevice),"half E copy");ck(cudaMemcpy(ddiag,F.diag.data(),F.n*2,cudaMemcpyHostToDevice),"half diag copy");ck(cudaMemcpy(dcf,F.coeff.data(),F.coeff.size()*4,cudaMemcpyHostToDevice),"half coeff copy");
