...
struct DeviceCtx{
    ...
    double active=0,setup_s=0,gather_s=0,trans_s=0,scatter_s=0;uint64_t groups=0;
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads){
    ...
    int bm=int(std::min<Code>(65535,(ms.size+threads-1)/threads)),bd=int(std::min<Code>(65535,(ds.size+threads-1)/threads));auto tsetup=std::chrono::steady_clock::now();c.setup_s+=std::chrono::duration<double>(tsetup-t0).count();
    ...
    ck(cudaGetLastError(),"doubleD gather");ck(cudaDeviceSynchronize(),"doubleD gather sync");auto tgather=std::chrono::steady_clock::now();c.gather_s+=std::chrono::duration<double>(tgather-tsetup).count();
    ...
    ck(cudaStreamSynchronize(c.sMain),"main sync");ck(cudaStreamSynchronize(c.sBlock),"block sync");auto ttrans=std::chrono::steady_clock::now();c.trans_s+=std::chrono::duration<double>(ttrans-tgather).count();
    ...
    ck(cudaGetLastError(),"doubleD scatter");ck(cudaDeviceSynchronize(),"group sync");auto tscatter=std::chrono::steady_clock::now();c.scatter_s+=std::chrono::duration<double>(tscatter-ttrans).count();c.groups++;c.active+=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
}
