...
int main(int argc,char**argv){
    ...
    for(int d=0;d<ng;++d){
        ...
        verify_all<<<256,256>>>(abase,elems,err);verify_all<<<256,256>>>(bbase,elems_b,err);ck(cudaGetLastError(),"verify launch");ck(cudaDeviceSynchronize(),"verify sync");
    ...
    for(int d=0;d<ng;++d){
        Count* mp=abase+Code(d)*mc;Count* bp=bbase+Code(d)*bc;
        if(mp!=abase+Code(d)*mc||bp!=bbase+Code(d)*bc)return 4;
    }
    ...
    const size_t common_gran=std::lcm(a.granularity,b.granularity);
    if(max_combined-min_combined>common_gran){std::fprintf(stderr,"combined physical imbalance too large\n");return 5;}
    if(total_errors){std::fprintf(stderr,"VMM helper verification errors=%llu\n",total_errors);return 6;}

    std::printf("gridfp-b300-vmm-storage-helper-microprobe OK gpus=%d elems_a=%llu elems_b=%llu gran_a=%zu gran_b=%zu padding_a=%zu padding_b=%zu combined_imbalance=%zu direct_base_index=1 logical_shard_views=1 all_gpu_read=OK exact=OK\n",ng,(unsigned long long)elems,(unsigned long long)elems_b,a.granularity,b.granularity,a.mapped_bytes-a.logical_bytes,b.mapped_bytes-b.logical_bytes,max_combined-min_combined);
