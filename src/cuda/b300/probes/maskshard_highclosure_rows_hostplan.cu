...
int main(int argc, char** argv) {
    ...
    std::cout << "maskshard-highclosure-rows-hostplan OK n=" << n
              << " rows=" << total
              << " mib=" << double(total * sizeof(uint32_t)) / double(1ULL << 20)
              << '\n';
    return 0;
}
