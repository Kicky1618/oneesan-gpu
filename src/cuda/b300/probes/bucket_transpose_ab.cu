...
static void fill_all(
    const BucketTransposePlan&p,const std::array<Count*,8>&base,bool transposed
){
    for(int g=0;g<p.ngpu;++g){
        btc(cudaSetDevice(g),"bt fill set device");
        auto*b=reinterpret_cast<uint8_t*>(base[g]);
...
static bool verify_all(
    ...
    for(int g=0;g<p.ngpu;++g){
        ...
        for(int s=0;s<p.ngpu;++s){
            uint64_t bytes=p.slot[g][s].capacity_bytes;if(!bytes)continue;
            uint32_t want=transposed?uint32_t(s*16+g):uint32_t(g*16+s),first=0,last=0;
            uint8_t*ptr=b+p.slot[g][s].off_bytes;
...
static bool run_mode(
    const char*name,const BucketTransposePlan&p,const std::array<Count*,8>&base,
    size_t chunk_bytes,int repeats
){
    fill_all(p,base,false);
    Ctx ctx;ctx.init(p,base,chunk_bytes);
    double total_s=0.0;
    for(int r=0;r<repeats;++r){
        auto t=std::chrono::steady_clock::now();ctx.transpose(p);double dt=secs(t);total_s+=dt;
        bool transposed=(r&1)==0;
        if(!verify_all(p,base,transposed)){ctx.release();return false;}
    }
    double avg=total_s/repeats;
    double peer_per=ctx.peer_gib/repeats;
    std::cout<<std::setprecision(12)
             <<"transpose_mode="<<name
             <<" ngpu="<<p.ngpu
             <<" repeats="<<repeats
             <<" avg_s="<<avg
             <<" peer_gib_per_transpose="<<peer_per
             <<" local_gib_per_transpose="<<(ctx.local_gib/repeats)
             <<" peer_gib_s="<<(peer_per/avg)
             <<" chunk_mib="<<(chunk_bytes>>20)<<'\n';
...
int main(int argc,char**argv){
    ...
    std::string mode=argc>5?argv[5]:"both";
    if(ngpu<2||ngpu>8||(ngpu&1)||!slot_mib||!chunk_mib||repeats<1){std::cerr<<"usage: [ngpu even 2..8] [slot_mib] [chunk_mib] [repeats] [sync|events|both]\n";return 2;}
    int visible=0;btc(cudaGetDeviceCount(&visible),"bt device count");if(visible<ngpu){std::cerr<<"need "<<ngpu<<" GPUs, visible="<<visible<<'\n';return 3;}
    uint64_t slot_bytes=uint64_t(slot_mib)<<20;size_t chunk_bytes=chunk_mib<<20;
    BucketTransposePlan plan=make_plan(ngpu,slot_bytes);
    std::array<Count*,8>base{};
    for(int g=0;g<ngpu;++g){btc(cudaSetDevice(g),"bt alloc set");btc(cudaMalloc(&base[g],size_t(plan.gpu_bytes[g])),"bt base alloc");}

    bool ok=true;
    if(mode=="sync"||mode=="both")ok=run_mode<BucketTransposeCtx>("sync",plan,base,chunk_bytes,repeats)&&ok;
    if(mode=="events"||mode=="both")ok=run_mode<BucketTransposeEventCtx>("events",plan,base,chunk_bytes,repeats)&&ok;
    if(mode!="sync"&&mode!="events"&&mode!="both"){std::cerr<<"invalid mode\n";ok=false;}
