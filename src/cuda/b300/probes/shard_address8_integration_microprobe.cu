...
__global__ void shard_load_once_kernel(
    const Code* global_index,
    Count* out,
    int n
) {
    const int tid = int(blockIdx.x * blockDim.x + threadIdx.x);
    if (tid < n) out[tid] = global_load_main(global_index[tid]);
}
...
float run_once(Count* out,int n,int blocks,int threads,int iters,Code total,Code stride,Code step){
    ...
    ck(cudaGetLastError(),"shard perf launch");ck(cudaEventRecord(b),"record b");ck(cudaEventSynchronize(b),"sync b");
    float ms=0;ck(cudaEventElapsedTime(&ms,a,b),"elapsed");cudaEventDestroy(a);cudaEventDestroy(b);return ms;
}

float median(std::vector<float> x){std::sort(x.begin(),x.end());size_t n=x.size();return n&1?x[n/2]:0.5f*(x[n/2-1]+x[n/2]);}
...
int main(int argc,char**argv){
    ...
    if(blocks<1||threads<1||threads>1024||iters<1||repeats<1||chunk<2||chunk>(~Code(0)>>3))return 2;
    ...
    std::printf("gridfp-b300-shard-address8-integration-microprobe OK fast=%d blocks=%d threads=%d iters=%d repeats=%d chunk=%llu loads=%.0f median_ms=%.6f Gload_s=%.6f checksum=%llu exact=OK\n",int(B300_FAST_SHARD_ADDRESS8),blocks,threads,iters,repeats,(unsigned long long)chunk,ops,ms,ops/ms/1.0e6,(unsigned long long)checksum);
    cudaFree(d_index);cudaFree(d_out);cudaFree(d_data);return 0;
}
