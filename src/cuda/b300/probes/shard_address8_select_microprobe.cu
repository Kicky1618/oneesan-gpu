...
int main(int argc, char** argv) {
    ...
    for (Code i = 0; i < total; ++i)
        h_data[size_t(i)] = Count((i * 2654435761ULL + 17ULL) & 0xffffffffu);
    ...
    ck(cudaMemcpy(d_data, h_data.data(), size_t(total) * sizeof(Count),
    ...
    std::vector<Count> h_once(size_t(n));
    ck(cudaMemcpy(h_once.data(), d_out, size_t(n) * sizeof(Count),
                  cudaMemcpyDeviceToHost), "copy once");
    for (int i = 0; i < n; ++i) {
        if (h_once[size_t(i)] != h_data[size_t(h_index[size_t(i)])]) {
    ...
    std::vector<Count> h_out(size_t(n));
    ck(cudaMemcpy(h_out.data(), d_out, size_t(n) * sizeof(Count),
