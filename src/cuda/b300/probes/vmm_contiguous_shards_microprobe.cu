...
int main(int argc,char**argv){
    ...
    ck(cudaSetDevice(srcdev),"set src");const int n=blocks*threads;Code *didx=nullptr,*dout=nullptr;Count *dold=nullptr,*dnew=nullptr;ck(cudaMalloc(&didx,static_cast<size_t>(n)*sizeof(Code)),"idx alloc");ck(cudaMalloc(&dout,static_cast<size_t>(n)*sizeof(Code)),"out alloc");ck(cudaMalloc(&dold,static_cast<size_t>(n)*sizeof(Count)),"old alloc");ck(cudaMalloc(&dnew,static_cast<size_t>(n)*sizeof(Count)),"new alloc");
    auto hidx=std::vector<Code>(static_cast<size_t>(n));const Code stride=std::max<Code>(1,total/Code(n)),step=std::max<Code>(1,chunk/7+1);for(int i=0;i<n;++i)hidx[static_cast<size_t>(i)]=(Code(i)*stride)%total;ck(cudaMemcpy(didx,hidx.data(),static_cast<size_t>(n)*sizeof(Code),cudaMemcpyHostToDevice),"idx copy");exact_kernel<<<blocks,threads>>>(didx,dold,dnew,n);ck(cudaGetLastError(),"exact launch");ck(cudaDeviceSynchronize(),"exact sync");
    auto hold=std::vector<Count>(static_cast<size_t>(n));auto hnew=std::vector<Count>(static_cast<size_t>(n));ck(cudaMemcpy(hold.data(),dold,static_cast<size_t>(n)*sizeof(Count),cudaMemcpyDeviceToHost),"old copy");ck(cudaMemcpy(hnew.data(),dnew,static_cast<size_t>(n)*sizeof(Count),cudaMemcpyDeviceToHost),"new copy");for(int i=0;i<n;++i){Count ex=value_for(hidx[static_cast<size_t>(i)]);if(hold[static_cast<size_t>(i)]!=ex||hnew[static_cast<size_t>(i)]!=ex){std::fprintf(stderr,"exact mismatch i=%d\n",i);return 4;}}
    ...
    cudaFree(dnew);cudaFree(dold);cudaFree(dout);cudaFree(didx);for(int d=0;d<ng;++d){ck(cudaSetDevice(d),"set final sync");ck(cudaDeviceSynchronize(),"final sync");}for(int d=0;d<ng;++d)cuck(cuMemUnmap(va+CUdeviceptr(static_cast<size_t>(d)*chunk_bytes),chunk_bytes),"unmap");for(auto h:handles)cuck(cuMemRelease(h),"release");cuck(cuMemAddressFree(va,total_bytes),"address free");return 0;
}
