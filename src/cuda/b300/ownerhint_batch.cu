...
struct GroupRuntimeCfg {
    ...
    uint8_t f_block_hint[256];
    uint8_t f_block_shift;
};
...
__device__ __forceinline__ int f_find_block_hint(Code i){uint32_t k=uint32_t(i>>D_GRP.f_block_shift);if(k>255)k=255;int b=D_GRP.f_block_hint[k];while(b+1<D_F_BLOCK_NBLOCKS && i>=D_F_BLOCK_BLOCKS[b].end)++b;return b;}
...
__global__ void factor_prerank_owner_blockorder_kernel(Count*mainv,Count*blockv,Code n,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<n;q+=st){
        int bb=f_find_block_hint(q);FBlock bx=D_F_BLOCK_BLOCKS[bb];Code loc=q-bx.off;uint32_t hr=uint32_t(loc/bx.stride),blr=uint32_t(loc-Code(hr)*bx.stride);
...
__global__ void factor_high_prerank_owner_blockorder_kernel(Count*mainv,Count*blockv,Code n,int p){
    ...
    for(Code q=Code(blockIdx.x)*blockDim.x+threadIdx.x,st=Code(gridDim.x)*blockDim.x;q<n;q+=st){
        int bb=f_find_block_hint(q);FBlock bx=D_F_BLOCK_BLOCKS[bb];Code loc=q-bx.off;uint32_t bhr=uint32_t(loc/bx.stride),lr=uint32_t(loc-Code(bhr)*bx.stride);
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    { unsigned bw=0; Code z=ds.size?ds.size-1:0; while(z>>bw)++bw; cfg.f_block_shift=uint8_t(bw>8?bw-8:0); for(unsigned k=0;k<256;++k){Code q=Code(k)<<cfg.f_block_shift;int b=0;while(b+1<fd&&q>=fdb[b].end)++b;cfg.f_block_hint[k]=uint8_t(b);} }
