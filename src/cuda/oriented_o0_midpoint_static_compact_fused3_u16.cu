...
__constant__ std::uint8_t D_F3_STATE[108], D_F3_OFF[10];
__constant__ std::uint16_t D_F3_FROW[109], D_F3_RROW[109];
__constant__ std::uint8_t D_F3_FCOL[500], D_F3_RCOL[500];
__constant__ std::uint16_t D_F3_FVAL[500], D_F3_RVAL[500];
...
__global__ void fused3_left(std::uint16_t* v,int p,bool rev,bool bottom,std::uint64_t threads_total,
                            std::uint32_t powp,std::uint32_t powp3,std::uint32_t right_count,
                            const std::uint32_t* rL,const std::uint32_t* rR,const std::int8_t* qL,const std::int8_t* qR,
                            const std::int8_t* qRest){
    std::uint64_t t=std::uint64_t(blockIdx.x)*blockDim.x+threadIdx.x;if(t>=threads_total)return;
    std::uint32_t rest=t/right_count, rr=t%right_count;int T=1-int(qRest[rest])-int(qR[rr]);if(T<-4||T>4)return;
    int sec=T+4, off=D_F3_OFF[sec], end=D_F3_OFF[sec+1], dim=end-off;
    std::uint32_t low=rest%powp, high=rest/powp, x[26];
#pragma unroll 1
    for(int j=0;j<dim;++j){int code=D_F3_STATE[off+j],k=code&3,qraw=code>>2;std::uint32_t lr=low+std::uint32_t(qraw)*powp+high*powp3;auto id=state_index(k,lr,rr,rL,rR,qL,qR);x[j]=v[id];}
#pragma unroll 1
    for(int j=0;j<dim;++j){int row=off+j, code=D_F3_STATE[row],k=code&3,qraw=code>>2;std::uint32_t sum=0;int eb=rev?D_F3_RROW[row]:D_F3_FROW[row],ee=rev?D_F3_RROW[row+1]:D_F3_FROW[row+1];for(int e=eb;e<ee;++e){int c=rev?D_F3_RCOL[e]:D_F3_FCOL[e];std::uint32_t w=rev?D_F3_RVAL[e]:D_F3_FVAL[e];sum=add2(sum,mulm(w,x[c]));}std::uint32_t lr=low+std::uint32_t(qraw)*powp+high*powp3;auto id=state_index(k,lr,rr,rL,rR,qL,qR);v[id]=(bottom&&qraw!=0)?0:static_cast<std::uint16_t>(sum);}
}

__global__ void fused3_right(std::uint16_t* v,int p,bool rev,bool bottom,std::uint64_t threads_total,
                             std::uint32_t powp,std::uint32_t powp3,std::uint32_t rest_count,
                             const std::uint32_t* rL,const std::uint32_t* rR,const std::int8_t* qL,const std::int8_t* qR,
                             const std::int8_t* qRest){
    std::uint64_t t=std::uint64_t(blockIdx.x)*blockDim.x+threadIdx.x;if(t>=threads_total)return;
    std::uint32_t lr=t/rest_count, rest=t%rest_count;int T=1-int(qL[lr])-int(qRest[rest]);if(T<-4||T>4)return;
    int sec=T+4, off=D_F3_OFF[sec], end=D_F3_OFF[sec+1], dim=end-off;
    std::uint32_t low=rest%powp, high=rest/powp, x[26];
#pragma unroll 1
    for(int j=0;j<dim;++j){int code=D_F3_STATE[off+j],k=code&3,qraw=code>>2;std::uint32_t rr=low+std::uint32_t(qraw)*powp+high*powp3;auto id=state_index(k,lr,rr,rL,rR,qL,qR);x[j]=v[id];}
#pragma unroll 1
    for(int j=0;j<dim;++j){int row=off+j, code=D_F3_STATE[row],k=code&3,qraw=code>>2;std::uint32_t sum=0;int eb=rev?D_F3_RROW[row]:D_F3_FROW[row],ee=rev?D_F3_RROW[row+1]:D_F3_FROW[row+1];for(int e=eb;e<ee;++e){int c=rev?D_F3_RCOL[e]:D_F3_FCOL[e];std::uint32_t w=rev?D_F3_RVAL[e]:D_F3_FVAL[e];sum=add2(sum,mulm(w,x[c]));}std::uint32_t rr=low+std::uint32_t(qraw)*powp+high*powp3;auto id=state_index(k,lr,rr,rL,rR,qL,qR);v[id]=(bottom&&qraw!=0)?0:static_cast<std::uint16_t>(sum);}
}
...
void upload_layout(const Layout& z,DevTables& d){CUDA_CHECK(cudaMalloc(&d.rL,z.left.rank.size()*4));CUDA_CHECK(cudaMalloc(&d.rR,z.right.rank.size()*4));CUDA_CHECK(cudaMalloc(&d.qL,z.left.charge.size()));CUDA_CHECK(cudaMalloc(&d.qR,z.right.charge.size()));CUDA_CHECK(cudaMemcpy(d.rL,z.left.rank.data(),z.left.rank.size()*4,cudaMemcpyHostToDevice));CUDA_CHECK(cudaMemcpy(d.rR,z.right.rank.data(),z.right.rank.size()*4,cudaMemcpyHostToDevice));CUDA_CHECK(cudaMemcpy(d.qL,z.left.charge.data(),z.left.charge.size(),cudaMemcpyHostToDevice));CUDA_CHECK(cudaMemcpy(d.qR,z.right.charge.data(),z.right.charge.size(),cudaMemcpyHostToDevice));CUDA_CHECK(cudaMemcpyToSymbol(D_BASE,z.base.data(),sizeof(z.base)));CUDA_CHECK(cudaMemcpyToSymbol(D_OFF,z.off.data(),sizeof(z.off)));CUDA_CHECK(cudaMemcpyToSymbol(D_CNT_R,z.right.count.data(),sizeof(z.right.count)));CUDA_CHECK(cudaMemcpyToSymbol(D_L,&z.L,sizeof(int)));CUDA_CHECK(cudaMemcpyToSymbol(D_R,&z.R,sizeof(int)));CUDA_CHECK(cudaMemcpyToSymbol(D_F3_STATE,o0f3::STATE,sizeof(o0f3::STATE)));CUDA_CHECK(cudaMemcpyToSymbol(D_F3_OFF,o0f3::SECTOR_OFF,sizeof(o0f3::SECTOR_OFF)));CUDA_CHECK(cudaMemcpyToSymbol(D_F3_FROW,o0f3::F_ROW,sizeof(o0f3::F_ROW)));CUDA_CHECK(cudaMemcpyToSymbol(D_F3_FCOL,o0f3::F_COL,sizeof(o0f3::F_COL)));CUDA_CHECK(cudaMemcpyToSymbol(D_F3_FVAL,o0f3::F_VAL,sizeof(o0f3::F_VAL)));CUDA_CHECK(cudaMemcpyToSymbol(D_F3_RROW,o0f3::R_ROW,sizeof(o0f3::R_ROW)));CUDA_CHECK(cudaMemcpyToSymbol(D_F3_RCOL,o0f3::R_COL,sizeof(o0f3::R_COL)));CUDA_CHECK(cudaMemcpyToSymbol(D_F3_RVAL,o0f3::R_VAL,sizeof(o0f3::R_VAL)));}
...
Result solve(int n){if(n<3||!(n&1)||n>19)throw std::runtime_error("odd n 3..19");int m=n-1;auto z=make_layout(m);DevTables d;upload_layout(z,d);std::uint16_t* v=nullptr;CUDA_CHECK(cudaMalloc(&v,z.total*2));CUDA_CHECK(cudaMemset(v,0,z.total*2));constexpr int TPB=256;auto blocks=[](std::uint64_t n){return unsigned((n+TPB-1)/TPB);};
    ...
    // Rest-charge tables for one-site and three-site fibers.
    ...
    HalfTable qLm3,qRm3;std::int8_t *dqLm3=nullptr,*dqRm3=nullptr;if(z.L>=3){qLm3=make_half(z.L-3);CUDA_CHECK(cudaMalloc(&dqLm3,qLm3.charge.size()));CUDA_CHECK(cudaMemcpy(dqLm3,qLm3.charge.data(),qLm3.charge.size(),cudaMemcpyHostToDevice));}if(z.R>=3){qRm3=make_half(z.R-3);CUDA_CHECK(cudaMalloc(&dqRm3,qRm3.charge.size()));CUDA_CHECK(cudaMemcpy(dqRm3,qRm3.charge.data(),qRm3.charge.size(),cudaMemcpyHostToDevice));}
    ...
    auto fuse3=[&](int p,bool rev,bool bottom=false){if(p<z.L){std::uint32_t pp=pow3i(p),pp3=pp*27,right=z.right.n,rest=pow3i(z.L-3);std::uint64_t nt=std::uint64_t(rest)*right;fused3_left<<<blocks(nt),TPB>>>(v,p,rev,bottom,nt,pp,pp3,right,d.rL,d.rR,d.qL,d.qR,dqLm3);}else{int hp=p-z.L;std::uint32_t pp=pow3i(hp),pp3=pp*27,rest=pow3i(z.R-3);std::uint64_t nt=std::uint64_t(z.left.n)*rest;fused3_right<<<blocks(nt),TPB>>>(v,hp,rev,bottom,nt,pp,pp3,rest,d.rL,d.rR,d.qL,d.qR,dqRm3);}CUDA_CHECK(cudaGetLastError());};
    ...
    auto forward_range=[&](int lo,int hi,bool bottom=false){for(int p=lo;p<=hi;){int bh=(p<z.L)?std::min(hi,z.L-1):hi;if(bh-p+1>=3){fuse3(p,false,bottom);p+=3;}else{local(p,false,bottom);++p;}}};
    auto reverse_range=[&](int lo,int hi){for(int p=hi;p>=lo;){int bl=(p>=z.L)?std::max(lo,z.L):lo;if(p-bl+1>=3){fuse3(p-2,true,false);p-=3;}else{local(p,true,false);--p;}}};
    forward_range(1,m-1); dump("top");
    bool forward=true;for(int y=1;y<n-1;++y){if(forward){turn(false);forward=false;reverse_range(0,m-1);}else{turn(true);forward=true;forward_range(0,m-1);}}
    dump("interior");if(forward)throw std::runtime_error("orientation");turn(true);forward_range(0,m-2,true);dump("bottom");
    ...
    cudaEventDestroy(a);cudaEventDestroy(b);cudaFree(dqLm1);cudaFree(dqRm1);if(dqLm3)cudaFree(dqLm3);if(dqRm3)cudaFree(dqRm3);cudaFree(v);free_tables(d);return{ans,z.total,ms};}
}
