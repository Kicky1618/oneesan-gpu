...
int main(int argc, char** argv) {
    ...
    for (int ei=0; ei<(int)edges.size(); ++ei) {
        ...
        auto set_table_capacity = [&](size_t need_slots) {
            size_t need = next_pow2(std::max<size_t>(1u << 16, need_slots));
            const bool too_small = allocated_cap < need;
            const bool too_large = need >= (1u << 20) && allocated_cap > need * 2;
            if (!too_small && !too_large) return;
            if (table_index) cudaFree(table_index);
            allocated_cap = need;
            ck(cudaMalloc(&table_index, allocated_cap*sizeof(uint32_t)), "malloc table_index");
        };
        // About 65% target occupancy against the predicted next layer.
        set_table_capacity((wanted_out * 3 + 1) / 2);
        size_t cap = allocated_cap;
        ...
        for (int attempt = 0; attempt < 4; ++attempt) {
            ...
            if (attempt == 3) { std::cerr << "hash/output overflow after retries\n"; return 2; }
            if (rn > spare_cap) {
                // Dense output overflow: grow toward the observed demand.
                ...
                // Hash table saturated before dense output did. Double slots.
                set_table_capacity(cap * 2);
                cap = allocated_cap;
                peak_table = std::max(peak_table, cap);
            }
        }
