...
int main(int argc, char** argv) {
    ...
    for (int ei=0; ei<(int)edges.size(); ++ei) {
        ...
        size_t cap = next_pow2(std::max<size_t>(1u << 16, in_n * 8));
        ...
        }
        {
            ...
            peak_alloc_bytes = std::max(peak_alloc_bytes, table_bytes + state_bytes + sizeof(uint32_t));
        }
    ...
    std::cout << "backend=h100-key192 n=" << n << " residues=";
