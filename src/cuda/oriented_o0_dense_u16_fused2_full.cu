...
__device__ __forceinline__ V9 keep_down0(V9 a) {
    a.v1=a.v2=a.v4=a.v5=a.v7=a.v8=0;
    return a;
}
__device__ __forceinline__ V9 keep_right0(V9 a) {
    a.v3=a.v4=a.v5=a.v6=a.v7=a.v8=0;
    return a;
}
...
#define ST(k,x) out[base + std::uint64_t{k} * stride] = static_cast<std::uint16_t>(x)
    V9 m0 = gate9({LD(0),LD(1),LD(2),LD(3),LD(4),LD(5),LD(6),LD(7),LD(8)});
    V9 m1 = gate9({LD(9),LD(10),LD(11),LD(12),LD(13),LD(14),LD(15),LD(16),LD(17)});
    V9 m2 = gate9({LD(18),LD(19),LD(20),LD(21),LD(22),LD(23),LD(24),LD(25),LD(26)});
    if (mode == 1) { m0=keep_down0(m0); m1=keep_down0(m1); m2=keep_down0(m2); }

    // Second gate is on digits 1,2 at fixed digit 0.
    V9 o0 = gate9({m0.v0,m0.v3,m0.v6,m1.v0,m1.v3,m1.v6,m2.v0,m2.v3,m2.v6});
    V9 o1 = gate9({m0.v1,m0.v4,m0.v7,m1.v1,m1.v4,m1.v7,m2.v1,m2.v4,m2.v7});
    V9 o2 = gate9({m0.v2,m0.v5,m0.v8,m1.v2,m1.v5,m1.v8,m2.v2,m2.v5,m2.v8});
    if (mode == 1) { o0=keep_down0(o0); o1=keep_down0(o1); o2=keep_down0(o2); }
    else if (mode == 2) { o0=keep_right0(o0); o1=keep_right0(o1); o2=keep_right0(o2); }
    ...
    for (unsigned y = 0; y < n; ++y) {
        ...
        if (y + 1 < n) {
            while (x + 2 < n) {
                ...
                local_gate_fused2_thread<<<blocks_for(groups), threads>>>(a, b, groups, stride, 0);
                CUDA_CHECK(cudaGetLastError());
                std::swap(a, b);
                x += 2;
            }
            // If two vertices remain, fuse the final interior vertex with the
            // right boundary vertex; the second gate is projected to right=0.
            if (x + 1 < n) {
                const std::uint64_t stride = pow3(x);
                const std::uint64_t groups = total / 27;
                local_gate_fused2_thread<<<blocks_for(groups), threads>>>(a, b, groups, stride, 2);
                CUDA_CHECK(cudaGetLastError());
                std::swap(a, b);
                x += 2;
            }
        } else {
            // Bottom row: all non-target vertices are projected to down=0.
            while (x + 2 < n) {
                const std::uint64_t stride = pow3(x);
                const std::uint64_t groups = total / 27;
                local_gate_fused2_thread<<<blocks_for(groups), threads>>>(a, b, groups, stride, 1);
                CUDA_CHECK(cudaGetLastError());
                std::swap(a, b);
                x += 2;
            }
