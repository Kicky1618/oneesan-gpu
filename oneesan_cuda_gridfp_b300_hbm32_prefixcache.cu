...
struct DeviceCtx{
    ...
    Code capM=0,capD=0,capPrefix=0;
    ...
    void ensure(Code m,Code b,size_t im,size_t id){
        ck(cudaSetDevice(dev),"set ensure");
        if(m>capM){if(dA){cudaFree(dA);cudaFree(dB);}capM=m;ck(cudaMalloc(&dA,size_t(m)*sizeof(Count)),"scratch A");ck(cudaMalloc(&dB,size_t(m)*sizeof(Count)),"scratch B");}if(m>capPrefix){if(dPrefix)cudaFree(dPrefix);capPrefix=m;ck(cudaMalloc(&dPrefix,size_t(m)*sizeof(uint32_t)),"prefix cache");}
        if(b>capD){if(dD){cudaFree(dD);cudaFree(dE);}capD=b;ck(cudaMalloc(&dD,size_t(b)*sizeof(Count)),"scratch D");ck(cudaMalloc(&dE,size_t(b)*sizeof(Count)),"scratch E");}
    ...
    void destroy(){if(dev<0)return;cudaSetDevice(dev);if(dA){cudaFree(dA);cudaFree(dB);}if(dD){cudaFree(dD);cudaFree(dE);}if(dIM)cudaFree(dIM);if(dID)cudaFree(dID);if(dPrefix)cudaFree(dPrefix);if(copyDone)cudaEventDestroy(copyDone);if(clearDone)cudaEventDestroy(clearDone);if(mainDone)cudaEventDestroy(mainDone);if(blockDone)cudaEventDestroy(blockDone);if(sMain)cudaStreamDestroy(sMain);if(sBlock)cudaStreamDestroy(sBlock);}
};
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads){
    ...
    c.ensure(ms.size,ds.size,pg.mi.size(),pg.di.size());
    ...
    int bm=int(std::min<Code>(65535,(ms.size+threads-1)/threads)),bd=int(std::min<Code>(65535,(ds.size+threads-1)/threads));int cutoff=wp.p_lo-1;bool usePrefix=!pg.use_mi && cutoff>=0 && (W-cutoff)<=16;
