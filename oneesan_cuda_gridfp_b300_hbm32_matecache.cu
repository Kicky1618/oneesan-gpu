...
__global__ void build_main_mates_kernel(MateID*out,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride)out[i]=unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);}
__global__ void build_block_mates_kernel(MateID*out,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride)out[i]=unrank_group_t<TARGET_W-1>(i,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);}
...
struct DeviceCtx{
    ...
    MateID*dMateM=nullptr,*dMateD=nullptr;
    Code capM=0,capD=0,capMateM=0,capMateD=0;
    ...
    void ensure(Code m,Code b,size_t im,size_t id){
        ...
        if(m>capM){if(dA){cudaFree(dA);cudaFree(dB);}capM=m;ck(cudaMalloc(&dA,size_t(m)*sizeof(Count)),"scratch A");ck(cudaMalloc(&dB,size_t(m)*sizeof(Count)),"scratch B");}
        if(b>capD){if(dD){cudaFree(dD);cudaFree(dE);}capD=b;ck(cudaMalloc(&dD,size_t(b)*sizeof(Count)),"scratch D");ck(cudaMalloc(&dE,size_t(b)*sizeof(Count)),"scratch E");}if(m>capMateM){if(dMateM)cudaFree(dMateM);capMateM=m;ck(cudaMalloc(&dMateM,size_t(m)*sizeof(MateID)),"mate main");}if(b>capMateD){if(dMateD)cudaFree(dMateD);capMateD=b;ck(cudaMalloc(&dMateD,size_t(b)*sizeof(MateID)),"mate block");}
    ...
    void destroy(){if(dev<0)return;cudaSetDevice(dev);if(dA){cudaFree(dA);cudaFree(dB);}if(dD){cudaFree(dD);cudaFree(dE);}if(dIM)cudaFree(dIM);if(dID)cudaFree(dID);if(dMateM)cudaFree(dMateM);if(dMateD)cudaFree(dMateD);if(copyDone)cudaEventDestroy(copyDone);if(clearDone)cudaEventDestroy(clearDone);if(mainDone)cudaEventDestroy(mainDone);if(blockDone)cudaEventDestroy(blockDone);if(sMain)cudaStreamDestroy(sMain);if(sBlock)cudaStreamDestroy(sBlock);}
};
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads){
    ...
    if(ms.size)build_main_mates_kernel<<<bm,threads>>>(c.dMateM,ms.size);if(ds.size)build_block_mates_kernel<<<bd,threads>>>(c.dMateD,ds.size);ck(cudaGetLastError(),"mate build");ck(cudaDeviceSynchronize(),"mate build sync");
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        ...
        if(ms.size)main_group_kernel<<<bm,threads,0,c.sMain>>>(cur,c.dMateM,ms.size,nxt,dnext,p);
        if(ds.size)blocked_group_kernel<<<bd,threads,0,c.sBlock>>>(dcur,c.dMateD,ds.size,nxt,p);
