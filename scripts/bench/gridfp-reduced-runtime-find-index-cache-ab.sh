...
W="${W:-10}"; NGPU="${NGPU:-2}"; BLOCKS="${BLOCKS:-256}"; MOD="${MOD:-4294967291}"; REPEATS="${REPEATS:-7}"; WARMUP="${WARMUP:-1}"; ARCH="${ARCH:-native}"; PTXAS_VERBOSE="${PTXAS_VERBOSE:-1}"; BUCKETS="${BUCKETS:-64}"
...
if [[ "$BUCKETS" != 16 && "$BUCKETS" != 32 && "$BUCKETS" != 64 ]]; then echo "BUCKETS must be 16, 32, or 64" >&2; exit 2; fi
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/gridfp_reduced_runtime_find_index_cache_ab_w${W}_g${NGPU}_b${BLOCKS}_h${BUCKETS}}"; RESULT="${RESULT:-${PREFIX}.tsv}"; SUMMARY="${SUMMARY:-${PREFIX}_summary.tsv}"; LOGDIR="${LOGDIR:-${PREFIX}_logs}"; mkdir -p "$(dirname "$RESULT")" "$LOGDIR"
...
for ((r=1;r<=REPEATS;++r)); do if ((r&1)); then order=(0 1); else order=(1 0); fi; for cache in "${order[@]}"; do [[ "$cache" == 0 ]] && bin="$BIN0" || bin="$BIN1"; echo "=== runtime index-cache=$cache buckets=$BUCKETS run $r/$REPEATS ===" >&2; run_one "$cache" "$bin" "$r" run; done; done
cat "$RESULT"
python3 - "$RESULT" "$SUMMARY" "$BUCKETS" <<'PY'
import csv,statistics,sys
src,dst,buckets=sys.argv[1:]; rows=list(csv.DictReader(open(src),delimiter='\t')); out=[]
for mode in ('0','1'):
...
q={r['index_cache']:r for r in out}; old=float(q['0']['wall_ms_median']); new=float(q['1']['wall_ms_median']); b=int(buckets); shared=64*b; total=16384+shared
...
print(f'runtime_find_index_cache_buckets={b}')
print(f'runtime_find_index_cache_latest_index_bytes_per_set={b}')
print(f'runtime_find_index_cache_extra_shared_bytes_per_block={shared}')
print(f'runtime_find_index_cache_total_known_shared_bytes={total}')
print(f'runtime_find_index_cache_net_vs_unpacked_baseline={total-24064}')
print('runtime_find_index_cache_component_clear_bytes=0')
print('runtime_find_index_cache_lane0_occupancy_masks=2x64bit')
print('runtime_find_index_cache_collision_state=packed_high_bit')
print('runtime_find_index_cache_extra_collision_registers=0')
print('runtime_find_index_cache_unique_bucket_mismatch=exact_miss_no_scan')
...
echo "gridfp-reduced-runtime-find-index-cache-ab OK W=$W ngpu=$NGPU blocks=$BLOCKS buckets=$BUCKETS repeats=$REPEATS result=$RESULT" >&2
