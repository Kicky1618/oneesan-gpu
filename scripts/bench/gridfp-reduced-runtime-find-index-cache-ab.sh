...
STORAGE="${STORAGE:-64}"; WAYS="${WAYS:-1}"
...
if [[ "$STORAGE" != 16 && "$STORAGE" != 32 && "$STORAGE" != 64 ]]; then echo "STORAGE must be 16, 32, or 64" >&2; exit 2; fi
if [[ "$WAYS" != 1 && "$WAYS" != 2 && "$WAYS" != 4 ]]; then echo "WAYS must be 1, 2, or 4" >&2; exit 2; fi
if (( STORAGE % WAYS != 0 )); then echo "STORAGE must divide evenly by WAYS" >&2; exit 2; fi
HASH_BUCKETS=$((STORAGE / WAYS)); if [[ "$HASH_BUCKETS" != 16 && "$HASH_BUCKETS" != 32 && "$HASH_BUCKETS" != 64 ]]; then echo "hash buckets must be 16, 32, or 64" >&2; exit 2; fi
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/gridfp_reduced_runtime_find_index_cache_ab_w${W}_g${NGPU}_b${BLOCKS}_s${STORAGE}w${WAYS}}"; RESULT="${RESULT:-${PREFIX}.tsv}"; SUMMARY="${SUMMARY:-${PREFIX}_summary.tsv}"; LOGDIR="${LOGDIR:-${PREFIX}_logs}"; mkdir -p "$(dirname "$RESULT")" "$LOGDIR"
...
for ((r=1;r<=REPEATS;++r)); do if ((r&1)); then order=(0 1); else order=(1 0); fi; for cache in "${order[@]}"; do [[ "$cache" == 0 ]] && bin="$BIN0" || bin="$BIN1"; echo "=== runtime index-cache=$cache storage=$STORAGE ways=$WAYS run $r/$REPEATS ===" >&2; run_one "$cache" "$bin" "$r" run; done; done
cat "$RESULT"
python3 - "$RESULT" "$SUMMARY" "$STORAGE" "$WAYS" <<'PY'
import csv,statistics,sys
src,dst,storage_s,ways_s=sys.argv[1:]; rows=list(csv.DictReader(open(src),delimiter='\t')); out=[]
for mode in ('0','1'):
...
q={r['index_cache']:r for r in out}; old=float(q['0']['wall_ms_median']); new=float(q['1']['wall_ms_median']); storage=int(storage_s); ways=int(ways_s); shared=64*storage; total=16384+shared
...
print('runtime_find_index_cache_hash=xor_shift_7_14'); print(f'runtime_find_index_cache_storage_bytes_per_set={storage}'); print(f'runtime_find_index_cache_ways={ways}'); print(f'runtime_find_index_cache_hash_buckets={storage//ways}'); print(f'runtime_find_index_cache_extra_shared_bytes_per_block={shared}'); print(f'runtime_find_index_cache_total_known_shared_bytes={total}'); print(f'runtime_find_index_cache_net_vs_unpacked_baseline={total-24064}'); print('runtime_find_index_cache_component_clear_bytes=0'); print('runtime_find_index_cache_lane0_occupancy_masks=2x64bit'); print('runtime_find_index_cache_overflow_state=packed_high_bit'); print('runtime_find_index_cache_extra_overflow_registers=0'); print('runtime_find_index_cache_nonoverflow_mismatch=exact_miss_no_scan'); print('runtime_find_index_cache_overflow_fallback=exact_recent_first'); print('runtime_find_index_cache_false_negative=0'); print('runtime_find_index_cache_default=0_pending_gpu_ab'); print(f'summary={dst}')
PY
...
echo "gridfp-reduced-runtime-find-index-cache-ab OK W=$W ngpu=$NGPU blocks=$BLOCKS storage=$STORAGE ways=$WAYS repeats=$REPEATS result=$RESULT" >&2
