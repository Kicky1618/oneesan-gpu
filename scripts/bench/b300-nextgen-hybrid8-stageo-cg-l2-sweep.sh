...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_nextgen_hybrid8_stageo_cg_l2_row${ROWS}_g${NGPU}}"; LOGDIR="${LOGDIR:-${PREFIX}_logs}"; RESULT="${RESULT:-${PREFIX}.tsv}"; RESOURCE="${RESOURCE:-${PREFIX}_ptxas.tsv}"; WINNER_ENV="${WINNER_ENV:-${PREFIX}_winner.env}"
BUILDER="$ONEESAN_ROOT/scripts/build/b300-forced-nextgen-hybrid8-stageo-cg-l2-policy.sh"; PARSER="$ONEESAN_ROOT/scripts/bench/parse-ptxas-resources.py"
...
norm_bytes(){ local raw="$1" out=() v old seen; for v in $raw; do case "$v" in 0|64|128|256) ;; *) echo "bad Stage-O L2 bytes=$v" >&2; exit 2;; esac; seen=0; for old in "${out[@]}"; do [[ "$old" == "$v" ]] && seen=1; done; ((seen)) || out+=("$v"); done; ((${#out[@]})) || exit 2; printf '%s' "${out[*]}"; }
...
# Stage F fixes all recurrence-level knobs including the Stage-N shared CG L2 baseline.
...
PAIR_POLICY="$B300_STAGEN_PAIR_POLICY"; BLOCK_POLICY="$B300_STAGEN_BLOCK_POLICY"; MATE_POLICY="$B300_STAGEN_MATE_LOAD_POLICY"; CONTROL_BIN="$B300_STAGEN_FINAL_BIN"; CONTROL_THREADS="$B300_STAGEN_FINAL_THREADS"
for p in "$PAIR_POLICY" "$BLOCK_POLICY" "$MATE_POLICY"; do case "$p" in default|cg|cs) ;; *) exit 3;; esac; done
[[ "$PAIR_POLICY" == cg || "$BLOCK_POLICY" == cg ]] || { echo 'Stage O not applicable: selected Stage N has no CG Count axis' >&2; exit 4; }
SW="$B300_STAGEN_SELF_WIDTH"; SD="$B300_STAGEN_SELF_DISTANCE"; SE="$B300_STAGEN_SELF_EVICT"; SG="$B300_STAGEN_SELF_GUARD"; MW="$B300_STAGEN_MATE_WIDTH"; MD="$B300_STAGEN_MATE_DISTANCE"; ME="$B300_STAGEN_MATE_EVICT"; MG="$B300_STAGEN_MATE_GUARD"
# shellcheck disable=SC1090
source "$STAGEN_PREPARE_ENV"
[[ "${B300_STAGEN_PREPARED:-0}" == 1 && "${B300_STAGEN_PREPARED_MOD:-}" == "$MOD" && "${B300_STAGEN_PREPARED_NGPU:-0}" == "$NGPU" ]] || { echo 'Stage-O/Stage-N prepare mismatch' >&2; exit 3; }
[[ "$B300_STAGEN_PREPARED_BIN" == "$CONTROL_BIN" && "$B300_STAGEN_PREPARED_PAIR_POLICY" == "$PAIR_POLICY" && "$B300_STAGEN_PREPARED_BLOCK_POLICY" == "$BLOCK_POLICY" && "$B300_STAGEN_PREPARED_MATE_LOAD_POLICY" == "$MATE_POLICY" ]] || { echo 'Stage-O Stage-N prepared/winner drift' >&2; exit 3; }
[[ -s "${B300_STAGEN_PREPARED_MANIFEST:-}" ]] || exit 3; sha256sum -c "$B300_STAGEN_PREPARED_MANIFEST" >/dev/null || { echo 'Stage-N manifest mismatch before Stage O' >&2; exit 3; }
[[ -x "$CONTROL_BIN" ]] || exit 3

# Non-CG axes have exactly one legal L2 value: zero. CG axes must include the
# inherited Stage-F baseline so the exact Stage-N control remains in the search domain.
if [[ "$PAIR_POLICY" != cg ]]; then PAIR_L2_LIST=0; else case " $PAIR_L2_LIST " in *" $BASE_L2 "*) ;; *) echo 'PAIR_L2_LIST omits inherited baseline' >&2; exit 2;; esac; fi
if [[ "$BLOCK_POLICY" != cg ]]; then BLOCK_L2_LIST=0; else case " $BLOCK_L2_LIST " in *" $BASE_L2 "*) ;; *) echo 'BLOCK_L2_LIST omits inherited baseline' >&2; exit 2;; esac; fi
BASE_PAIR_L2=0; [[ "$PAIR_POLICY" == cg ]] && BASE_PAIR_L2="$BASE_L2"; BASE_BLOCK_L2=0; [[ "$BLOCK_POLICY" == cg ]] && BASE_BLOCK_L2="$BASE_L2"

BINS="$LOGDIR/binaries.tsv"; printf 'label\tpair_l2\tblock_l2\tbinary\tbuild_err\n' >"$BINS"; printf 'control\t%s\t%s\t%s\t-\n' "$BASE_PAIR_L2" "$BASE_BLOCK_L2" "$CONTROL_BIN" >>"$BINS"
for pl2 in $PAIR_L2_LIST; do for bl2 in $BLOCK_L2_LIST; do
  [[ "$pl2" == "$BASE_PAIR_L2" && "$bl2" == "$BASE_BLOCK_L2" ]] && continue
  label="p${pl2}_b${bl2}"; bin="$ONEESAN_BUILD_DIR/b300_stageo_${label}_pp${PAIR_POLICY}_bp${BLOCK_POLICY}_mp${MATE_POLICY}_n27"; err="$LOGDIR/${label}.build.err"; bout="$LOGDIR/${label}.build.out"
  N=27 ARCH="$ARCH" OUT="$bin" MATE_LOAD_POLICY="$MATE_POLICY" PAIR_LOAD_POLICY="$PAIR_POLICY" BLOCK_LOAD_POLICY="$BLOCK_POLICY" BASE_CG_L2_BYTES="$BASE_L2" PAIR_CG_L2_BYTES="$pl2" BLOCK_CG_L2_BYTES="$bl2" HIGH_DROP_CHUNK="$H" HYBRID_THRESHOLD="$T" SELF_WIDTH="$SW" SELF_DISTANCE="$SD" MATE_WIDTH="$MW" MATE_DISTANCE="$MD" SELF_EVICT="$SE" MATE_EVICT="$ME" SELF_GUARD="$SG" MATE_GUARD="$MG" RANDOM_CG="$CG" RANDOM_CG_L2_FETCH_BYTES="$BASE_L2" PREFETCH_L2="$PRE" DUALMASK="$DUAL" CLOSURE_BATCH="$BATCH" MAXRREGCOUNT="$CAP" PTXAS_VERBOSE=1 BUILD_ERR="$err" bash "$BUILDER" >"$bout" 2>"$LOGDIR/${label}.driver.err"
  [[ -x "$bin" ]] || exit 3; grep -Fq "base_cg_l2_bytes=$BASE_L2 pair_cg_l2_bytes=$pl2 block_cg_l2_bytes=$bl2" "$bout" || exit 3; grep -Fq 'stage_o_scope=cg_l2_fetch_size_only' "$bout" || exit 3
...
while IFS=$'\t' read -r label pl2 bl2 bin err; do [[ "$label" == label ]] && continue; for th in "${threads[@]}"; do for ((r=1;r<=REPEATS;++r)); do echo "=== Stage O label=$label pair_l2=$pl2 block_l2=$bl2 threads=$th rows=$ROWS policies=$PAIR_POLICY/$BLOCK_POLICY ===" >&2; run_one "$label" "$pl2" "$bl2" "$bin" "$th" "$r"; done; done; done <"$BINS"

python3 - "$RESULT" "$RESOURCE" "$BINS" "$WINNER_ENV" "$ROWS" "$NGPU" "$PAIR_POLICY" "$BLOCK_POLICY" "$MATE_POLICY" "$BASE_L2" "$BASE_PAIR_L2" "$BASE_BLOCK_L2" "$CONTROL_THREADS" <<'PY'
import csv,math,statistics,sys,shlex
result,resource,bins_path,winner,rows_arg,ngpu,pair_policy,block_policy,mate_policy,base_l2,base_pair,base_block,control_src_threads=sys.argv[1:]
...
for x in sorted(agg,key=rank):
    b=bins[x[1]]; print(f'STAGE_O label={x[1]} pair_l2={b["pair_l2"]} block_l2={b["block_l2"]} threads={x[2]} wall_s={x[0]:.9f} high_s={x[3]:.9f} regs={x[4]} spill_store={x[5]} spill_load={x[6]} spill_free={int(x[7])}',file=sys.stderr)
...
vals={'B300_STAGEO_ROWS':rows_arg,'B300_STAGEO_NGPU':ngpu,'B300_STAGEO_RESIDUE':next(iter(res)),'B300_STAGEO_PAIR_POLICY':pair_policy,'B300_STAGEO_BLOCK_POLICY':block_policy,'B300_STAGEO_MATE_LOAD_POLICY':mate_policy,'B300_STAGEO_BASE_CG_L2_BYTES':base_l2,'B300_STAGEO_BASE_PAIR_L2_BYTES':base_pair,'B300_STAGEO_BASE_BLOCK_L2_BYTES':base_block,'B300_STAGEO_CONTROL_BIN':bins['control']['binary'],'B300_STAGEO_CONTROL_THREADS':base[2],'B300_STAGEO_CONTROL_SOURCE_THREADS':control_src_threads,'B300_STAGEO_CONTROL_WALL_S':f'{base[0]:.9f}','B300_STAGEO_CONTROL_HIGH_S':f'{base[3]:.9f}','B300_STAGEO_CONTROL_SPILL_FREE':1,'B300_STAGEO_PAIR_L2_BYTES':bb['pair_l2'],'B300_STAGEO_BLOCK_L2_BYTES':bb['block_l2'],'B300_STAGEO_BIN':bb['binary'],'B300_STAGEO_THREADS':best[2],'B300_STAGEO_WALL_S':f'{best[0]:.9f}','B300_STAGEO_HIGH_S':f'{best[3]:.9f}','B300_STAGEO_SPILL_FREE':int(best[7]),'B300_STAGEO_SPEEDUP':f'{speed:.9f}','B300_STAGEO_BEST_ENABLED':enabled}
with open(winner,'w') as f:
    for k,v in vals.items(): f.write(f'{k}={q(v)}\n')
print(f'b300_stageo_exact_match=1 b300_stageo_ngpu={ngpu} pair_policy={pair_policy} block_policy={block_policy} base_l2={base_l2} best_pair_l2={bb["pair_l2"]} best_block_l2={bb["block_l2"]} speedup={speed:.9f} residue={next(iter(res))}')
PY
cat "$WINNER_ENV"
