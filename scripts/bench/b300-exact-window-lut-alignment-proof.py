#!/usr/bin/env python3
MAXW=28;W=28;TARGET=65536<<20
H=[[0]*(MAXW+2) for _ in range(MAXW+1)]
for h in range(MAXW+2):H[0][h]=int(h==0)
for w in range(1,MAXW+1):
    for h in range(MAXW+1):
        H[w][h]=H[w-1][h]+(H[w-1][h-1] if h else 0)+(H[w-1][h+1] if h<MAXW+1 else 0)
...
def plan(hi,lo,target):
    c=cand(hi,lo)
    for k in range(min(len(c),20)+1):
        fp=c[:k];mx=mm=md=0
        for g in range(1<<k):
            mf,mo,bf,bo=masks(hi,lo,fp,g);m=size(W,mf,mo);d=size(W-1,bf,bo);b=8*(m+d)
            if b>mx:mx,mm,md=b,m,d
            if mx>target and k<min(len(c),20):break
        if mx<=target or k==min(len(c),20):return k,mx,mm,md,fp
def schedule(maxwin):
    hi=W-1;out=[]
    while hi>=1:
...
wide=schedule(27);aligned=schedule(14)
assert [(x[0],x[1],x[2]) for x in wide]==[(27,11,10),(10,1,10)],wide
assert [(x[0],x[1],x[2]) for x in aligned]==[(27,14,13),(13,1,13)],aligned
# One representative group is enough: fixed masks do not depend on occupancy bits.
...
# Main+block MateID caches add exactly another 8*(m+d), equal to count scratch.
with_both_mates=2*count_max
assert with_both_mates<TARGET
print('b300-exact-window-lut-alignment-proof OK '
      f'wide_windows=27:11,10:1 wide_fixed_bits=10 lut13_fastpath=0 '
      f'aligned_windows=27:14,13:1 aligned_fixed_bits=13 lut13_fastpath=1 '
      f'aligned_count_scratch_gib={count_max/(1<<30):.9f} '
      f'aligned_with_main_block_mates_gib={with_both_mates/(1<<30):.9f} target_gib={TARGET/(1<<30):.1f}')
