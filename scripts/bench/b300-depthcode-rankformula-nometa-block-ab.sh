...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_nometa_block_ab_n${N}_${TRANSPOSE_MODE}}"
...
build_one(){
  local block="$1" bin="$2" label="block${block}"
  N="$N" ARCH="$ARCH" OUT="$bin" RANKFORMULA_NOMETA_BLOCK="$block" \
...
run_one(){
  local block="$1" bin="$2" rep="$3" label="block${block}"
...
for block in 4 8; do
  ...
  if [[ "$RUN_SELFTEST" == 1 ]]; then
    RANKFORMULA_NOMETA_BLOCK="$block" RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" PM_ACCUM="$PM_ACCUM" \
  ...
  bin="$ONEESAN_BUILD_DIR/ab_rankformula_nometa_abstract_${label}_n${N}"
  build_one "$block" "$bin"
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
    if q['block4'][k] and q['block8'][k]: print(f'block8_{k}_speedup={q["block4"][k]/q["block8"][k]:.6f}x')
ld=pathlib.Path(logdir)
for mode in ('block4','block8'):
    p=ld/f'{mode}_r1.err'; vals=[]; steps=[]; loads=[]
    if p.exists():
        for line in p.read_text(errors='replace').splitlines():
            if not line.startswith('p10dc_low_rankformula_nometa4 fixed_owner='): continue
            d=dict(re.findall(r'([A-Za-z0-9_]+)=([^ ]+)',line))
            vals.append(int(d['total_bytes'])); steps.append(float(d['avg_locator_steps'])); loads.append(float(d['avg_group_loads_model']))
    if vals:
        print(f'{mode}_aux_bytes_all_gpus={sum(vals)}')
        print(f'{mode}_avg_locator_steps_mean_gpu={sum(steps)/len(steps):.9f}')
        print(f'{mode}_avg_group_loads_model_mean_gpu={sum(loads)/len(loads):.9f}')
if run_ptxas=='1':
...
print('block4_aux_model_bytes_all_owners=1158104')
print('block8_aux_model_bytes_all_owners=857642')
print('block8_aux_model_reduction=25.944302%')
print('block4_avg_group_loads_model=1.086816311')
print('block8_avg_group_loads_model=1.202523968')
PY

echo "b300-depthcode-rankformula-nometa-block-ab OK n=$N repeats=$REPEATS result=$RESULT" >&2
