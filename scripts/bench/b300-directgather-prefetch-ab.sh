...
COL_ILP="${COL_ILP:-2}";THREADS="${BUCKET_THREADS:-256}";GX="${BUCKET_GRID_X:-32}";GY="${BUCKET_GRID_Y:-8}";SAMPLE_INTERVAL="${SAMPLE_INTERVAL:-0.25}"
if [[ -z "${EXPECT+x}" ]];then [[ "$N" == 21 && "$MOD" == 4294967291 ]]&&EXPECT=998035516||{ echo EXPECT required >&2;exit 2;};fi
command -v nvcc >/dev/null&&command -v nvidia-smi >/dev/null||exit 2
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_prefetch_n${N}}";LOGDIR="${LOGDIR:-${PREFIX}_logs}";TSV="${TSV:-${PREFIX}.tsv}";mkdir -p "$LOGDIR"
...
for pf in 0 1;do mode=$([[ "$pf" == 1 ]]&&echo prefetch||echo baseline);bin="$ONEESAN_BUILD_DIR/b300_prefetch_${mode}_n${N}";N="$N" ARCH="$ARCH" OUT="$bin" COL_ILP="$COL_ILP" DEPTHMAJOR=1 PREFETCH_NEXT="$pf" FORCE7=0 MLP_WINDOW4=0 PTXAS_VERBOSE=1 bash "$ONEESAN_ROOT/scripts/build/b300-directgather-colilp-fast.sh">"$LOGDIR/$mode.build.out" 2>"$LOGDIR/$mode.build.err";for((r=1;r<=REPEATS;++r));do so="$LOGDIR/${mode}_r${r}.out";se="$LOGDIR/${mode}_r${r}.err";u="$LOGDIR/${mode}_r${r}.util";BUCKET_THREADS="$THREADS" BUCKET_GRID_X="$GX" BUCKET_GRID_Y="$GY" "$bin" "$N" "$TARGET_MIB" "$MAX_WINDOW" "$NGPU" "$MOD">"$so" 2>"$se"&pid=$!;sample "$pid" "$u"&sp=$!;set +e;wait "$pid";rc=$?;set -e;wait "$sp"||true;((rc==0))||exit "$rc";line="$(grep '^residue=' "$so"|tail -n1)";detail="$(grep 'snake_onepass_graph_batch modulus=' "$se"|tail -n1)";res="$(field residue "$line")";[[ "$res" == "$EXPECT" ]]||exit 4;wall="$(field wall_s "$line")";fh="$(field forward_high_s "$detail")";rh="$(field reverse_high_s "$detail")";high="$(python3 -c 'import sys;print(float(sys.argv[1])+float(sys.argv[2]))' "$fh" "$rh")";read -r gu mu mm < <(awk '{sg+=$1;sm+=$2;if($3>mm)mm=$3;n++}END{if(n)printf "%.6f %.6f %d\n",sg/n,sm/n,mm;else print "NA NA NA"}' "$u");printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$mode" "$r" "$res" "$wall" "$high" "$gu" "$mu" "$mm">>"$TSV";done;done
cat "$TSV"
python3 - "$TSV" <<'PY'
import csv,statistics,sys
r=list(csv.DictReader(open(sys.argv[1]),delimiter='\t'))
def med(m,k):
 x=[float(z[k]) for z in r if z['mode']==m and z[k]!='NA'];return statistics.median(x)
for m in ('baseline','prefetch'):print(f'{m} wall={med(m,"wall_s"):.6f} high={med(m,"high_s"):.6f} memavg={med(m,"avg_memctrl_util_pct"):.2f}% memmax={med(m,"max_memctrl_util_pct"):.2f}%')
print(f'prefetch_high_speedup={med("baseline","high_s")/med("prefetch","high_s"):.6f}x')
PY
