...
ILP="${ILP:-2}";THREADS="${THREADS:-256}";HIGH_DROP_CHUNK="${HIGH_DROP_CHUNK:-0}";ARCH="${ARCH:-native}"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_low_main_recurrence_row${ROWS}_ilp${ILP}}"
...
build_one(){
  ...
  echo "=== build $mode ILP=$ILP low_main_recurrence=$rec ===" >&2
  N="$N" ARCH="$ARCH" OUT="$bin" MAIN_PULL_ILP="$ILP" LOW_MAIN_RECURRENCE="$rec" HIGH_DROP_CHUNK="$HIGH_DROP_CHUNK" \
  ...
  grep -Fq "low_main_recurrence=$rec" "${PREFIX}.${mode}.build.out"
  if [[ "$rec" == 1 ]]; then grep -Fq 'low_main_state_bits=60 extra_state_bytes=0' "${PREFIX}.${mode}.build.out";fi
}
BASE_BIN="$ONEESAN_BUILD_DIR/b300_batch_n27_lowmainrec0_ilp${ILP}"
CAND_BIN="$ONEESAN_BUILD_DIR/b300_batch_n27_lowmainrec1_ilp${ILP}"
...
[[ "$BR" == "$CR" ]]||{ echo "low-main recurrence residue mismatch base=$BR cand=$CR" >&2;exit 6; }
python3 - "$BW" "$CW" "$BM" "$CM" "$BBUSY" "$CBUSY" "$BSM" "$CSM" <<'PY'
import sys
bw,cw,bm,cm,bb,cb,bs,cs=map(float,sys.argv[1:])
print('b300_low_main_recurrence_exact_intermediate_match=1')
print(f'b300_low_main_recurrence_wall_baseline_s={bw:.9f}')
print(f'b300_low_main_recurrence_wall_candidate_s={cw:.9f}')
print(f'b300_low_main_recurrence_wall_speedup={bw/cw:.9f}x')
print(f'b300_low_main_recurrence_memctl_all_baseline_avg_pct={bm:.3f}')
print(f'b300_low_main_recurrence_memctl_all_candidate_avg_pct={cm:.3f}')
print(f'b300_low_main_recurrence_memctl_busy_baseline_avg_pct={bb:.3f}')
print(f'b300_low_main_recurrence_memctl_busy_candidate_avg_pct={cb:.3f}')
print(f'b300_low_main_recurrence_memctl_busy_delta_pp={cb-bb:.3f}')
print(f'b300_low_main_recurrence_sm_baseline_avg_pct={bs:.3f}')
print(f'b300_low_main_recurrence_sm_candidate_avg_pct={cs:.3f}')
PY
printf 'b300_low_main_recurrence_ilp=%s\n' "$ILP"
printf 'b300_low_main_recurrence_threads=%s\n' "$THREADS"
printf 'b300_low_main_recurrence_high_drop_chunk=%s\n' "$HIGH_DROP_CHUNK"
printf 'b300_low_main_recurrence_rows=%s\n' "$ROWS"
printf 'b300_low_main_recurrence_residue=%s\n' "$BR"
printf 'b300_low_main_recurrence_note=8B/state/p Mate streaming store is deliberate; adopt only on wall-time win\n'
