...
[[ -f "$GEN" && -f "$BUILDER" && -f "$PROOF" && -f "$SWEEP" && -f "$STAGED" && -f "$PROMOTE" ]] || exit 2
python3 -m py_compile "$GEN"; for f in "$BUILDER" "$PROOF" "$SWEEP" "$STAGED" "$PROMOTE"; do bash -n "$f"; done; bash "$PROOF"
for s in \
  ...
  'b300_mainrec_stages_ilp2_pair_block_cg_l2=1'; do grep -Fq "$s" "$GEN" || { echo "Stage-S generator marker missing: $s" >&2; exit 3; }; done
for s in \
  ...
  'stage_s_scope=ilp2_cg_l2_hint_only ilp8_exact_upstream=1 stage_r_policy_preserved=1'; do grep -Fq "$s" "$BUILDER" || { echo "Stage-S builder marker missing: $s" >&2; exit 3; }; done
for s in \
  ...
  'b300_stages_exact_match=1'; do grep -Fq "$s" "$SWEEP" || { echo "Stage-S sweep marker missing: $s" >&2; exit 3; }; done
for s in \
  ...
  'FATAL Stage-S/Stage-R residue mismatch'; do grep -Fq "$s" "$STAGED" || { echo "Stage-S staged marker missing: $s" >&2; exit 3; }; done
for s in \
  'PREPARE_ONLY="${PREPARE_ONLY:-0}"' \
  'b300-nextgen-hybrid8-stages-ilp2-cg-l2-staged-calibrate.sh' \
  'Stage S retained exact Stage-R zero-hint tuple' \
  'Stage-S control is not exact prepared Stage-R binary' \
  'sha256sum -c "$B300_STAGER_PREPARED_MANIFEST"' \
  'B300_STAGES_PREPARED=1' \
  'B300_STAGES_PREPARED_CONTROL_BIN=' \
  'B300_STAGES_PREPARED_MANIFEST=' \
  'PREPARE_ONLY=1 for screening' \
  'b300x8-race-external-forced-profiled-once.sh'; do grep -Fq "$s" "$PROMOTE" || { echo "Stage-S promotion marker missing: $s" >&2; exit 3; }; done
python3 - "$GEN" "$BUILDER" "$SWEEP" "$STAGED" "$PROMOTE" <<'PY'
from pathlib import Path
import sys
g,b,w,t,p=(Path(x).read_text() for x in sys.argv[1:])
...
# Grand integration must use PREPARE_ONLY. The standalone full-prime path remains isolated after it.
prep=p.find('if [[ "$PREPARE_ONLY" == 1 ]]'); full=p.find('b300x8-race-external-forced-profiled-once.sh')
if prep<0 or full<0 or prep>=full: raise SystemExit('Stage-S promotion prepare/full-prime ordering broken')
if p.count('b300x8-race-external-forced-profiled-once.sh') != 1: raise SystemExit('Stage-S promotion complete-prime call count drift')
print('stages_promotion_contract_structure=OK')
PY
echo 'b300_stages_preflight=OK stage_s=ilp2_pair_block_cg_l2_split upstream=R exact_prepared_control=1 low_l2=0,64,128,256 noncg_zero_locked=1 high_provenance=Q,O,P,N stage_r_policy_preserved=1 high_state_locked=1 ilp8_byte_locked=1 residue_gate=1 spill_gate=1 staged_rows=1,4,8 selected_tuple_lock=1 prepare_only=1 standalone_prime_isolated=1 gpu_work=0'
