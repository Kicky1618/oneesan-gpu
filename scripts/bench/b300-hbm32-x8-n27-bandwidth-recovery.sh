...
GPUS=8
ARCH="${ARCH:-native}"
OUT="${OUT:-$ONEESAN_BUILD_DIR/oneesan_cuda_gridfp_b300_hbm32_n27_x8_bandwidth_recovery}"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_hbm32_n27_x8_bandwidth_recovery}"
...
# candidate deliberately trades HBM traffic for less integer work:
...
echo "=== build B300 n=27 bandwidth-recovery candidate ===" >&2
...
grep -Fq 'main_mate_cache=1 main_pull=1' "$BUILD_OUT" || {
  echo "bandwidth-recovery build did not report MAIN_MATE_CACHE=1 MAIN_PULL=1" >&2
  cat "$BUILD_OUT" >&2
  exit 3
}
...
echo "scratch_mib=$SCRATCH_MIB max_window=$MAX_WINDOW gpus=$GPUS telemetry=$TELEMETRY" >&2
set +e
...
with open(p,newline='') as f:
    for r in csv.reader(f):
        ...
        try:
            ...
            sm=float(r[2].strip())
            mem=float(r[3].strip())
            power=float(r[4].strip())
        except ValueError:
            continue
        rows[gpu].append((sm,mem,power))
if not rows:
    print('b300_bandwidth_recovery_telemetry_samples=0')
    raise SystemExit(0)
all_mem=[]; all_sm=[]
for gpu in sorted(rows):
    xs=rows[gpu]
    sm=[x[0] for x in xs]; mem=[x[1] for x in xs]; power=[x[2] for x in xs]
