...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_nextgen_hybrid8_mate_evict_stagek_staged}"; FINAL_ENV="${FINAL_ENV:-${PREFIX}_winner.env}"
...
run_stage(){
  ...
  STAGE_F_ENV="$STAGE_F_ENV" STAGEJ_PREPARE_ENV="$STAGEJ_PREPARE_ENV" ARCH="$ARCH" MOD="$MOD" ROWS="$rows" TARGET_MIB="$TARGET_MIB" MAX_WINDOW="$MAX_WINDOW" \
...
cat "$FINAL_ENV"
echo "b300-nextgen-hybrid8-mate-evict-staged-calibrate OK validated=$VALIDATED self=w${SW}d${SD}/$SE mate=w${MW}d${MD} mate_evict=$FINAL_EV speedup=$FINAL_SPEED final_rows=$B300_STAGEK_ROWS" >&2
