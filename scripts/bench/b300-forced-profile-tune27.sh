...
# profile|ILP|high-drop-chunk|low-main-recurrence|high-main-recurrence|unified-main-recurrence
SPECS=(
...
printf 'profile\tbinary\tstatus\tpartial_residue\twall_s\tmc_avg_pct\tmc_max_pct\thigh_rec_groups\thigh_rec_fallback_groups\n' >"$RESULT"
run_one(){
 ...
 if [[ -z "$line" ]] || ! grep -Fq " rows=$ROWS calibration=1 " <<<"$line"; then printf '%s\t%s\tfailed:no_row_result\tNA\tNA\tNA\tNA\t0\t0\n' "$mode" "$bin" >>"$RESULT"; return 0; fi
 local hg hf; hg="$(field high_rec_groups "$line")"; hf="$(field high_rec_fallback_groups "$line")"; [[ -n "$hg" ]]||hg=0; [[ -n "$hf" ]]||hf=0
 case "$mode" in forced_highrec*|forced_mainrec*) if ! [[ "$hg" =~ ^[0-9]+$ ]] || ((hg==0)); then printf '%s\t%s\tfailed:no_rec_coverage\tNA\tNA\tNA\tNA\t%s\t%s\n' "$mode" "$bin" "$hg" "$hf" >>"$RESULT"; return 0; fi;; esac
 ...
 printf '%s\t%s\tok\t%s\t%s\t%s\t%s\t%s\t%s\n' "$mode" "$bin" "$(field residue "$line")" "$(field wall_s "$line")" "$avg" "$mx" "$hg" "$hf" >>"$RESULT"
}
...
for x in sorted(ok,key=lambda z:float(z['wall_s'])):
 print('FORCED_PROFILE',x['profile'],'wall_s='+x['wall_s'],'mc_avg='+x['mc_avg_pct'],'mc_max='+x['mc_max_pct'],'high_rec='+x['high_rec_groups'],'fallback='+x['high_rec_fallback_groups'])
...
with open(out,'w') as f:
 ...
 f.write(f'FORCED_HIGH_REC_GROUPS={b["high_rec_groups"]}\n')
 f.write(f'FORCED_HIGH_REC_FALLBACK_GROUPS={b["high_rec_fallback_groups"]}\n')
