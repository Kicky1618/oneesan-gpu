...
python3 - "$RESULT" "$RESOURCE" "$BINS" "$WINNER_ENV" "$ROWS" "$NGPU" "$RESOLVED" "$STAGEQ_UPSTREAM" "$BASE_PAIR" "$BASE_BLOCK" "$PAIR_POLICY" "$BLOCK_POLICY" "$Q_PAIR_L2" "$Q_BLOCK_L2" "$CONTROL_THREADS" "$UP_ROWS" "$UP_RES" "$UP_MANIFEST" <<'PY'
import csv,math,statistics,sys,shlex
result,resource,bins_path,winner,rows_arg,ngpu,upstream,q_up,base_pair,base_block,high_pair,high_block,qpl2,qbl2,control_src_threads,up_rows,up_res,up_manifest=sys.argv[1:]
...
vals={'B300_STAGER_ROWS':rows_arg,'B300_STAGER_NGPU':ngpu,'B300_STAGER_RESIDUE':next(iter(res)),'B300_STAGER_UPSTREAM_KIND':upstream,'B300_STAGER_STAGEQ_UPSTREAM_KIND':q_up,'B300_STAGER_HIGH_PAIR_POLICY':high_pair,'B300_STAGER_HIGH_BLOCK_POLICY':high_block,'B300_STAGER_HIGH_PAIR_L2_BYTES':qpl2,'B300_STAGER_HIGH_BLOCK_L2_BYTES':qbl2,'B300_STAGER_UPSTREAM_PAIR_POLICY':base_pair,'B300_STAGER_UPSTREAM_BLOCK_POLICY':base_block,'B300_STAGER_CONTROL_BIN':bins['control']['binary'],'B300_STAGER_CONTROL_THREADS':base[2],'B300_STAGER_CONTROL_SOURCE_THREADS':control_src_threads,'B300_STAGER_CONTROL_WALL_S':f'{base[0]:.9f}','B300_STAGER_CONTROL_HIGH_S':f'{base[3]:.9f}','B300_STAGER_CONTROL_SPILL_FREE':1,'B300_STAGER_PAIR_POLICY':bb['pair_policy'],'B300_STAGER_BLOCK_POLICY':bb['block_policy'],'B300_STAGER_BIN':bb['binary'],'B300_STAGER_THREADS':best[2],'B300_STAGER_WALL_S':f'{best[0]:.9f}','B300_STAGER_HIGH_S':f'{best[3]:.9f}','B300_STAGER_SPILL_FREE':int(best[7]),'B300_STAGER_SPEEDUP':f'{speed:.9f}','B300_STAGER_BEST_ENABLED':enabled,'B300_STAGER_UPSTREAM_ROWS':up_rows,'B300_STAGER_UPSTREAM_RESIDUE':up_res,'B300_STAGER_UPSTREAM_MANIFEST':up_manifest}
with open(winner,'w') as f:
    for k,v in vals.items(): f.write(f'{k}={q(v)}\n')
print(f'b300_stager_exact_match=1 b300_stager_ngpu={ngpu} upstream={upstream} base_pair={base_pair} base_block={base_block} best_pair={bb["pair_policy"]} best_block={bb["block_policy"]} speedup={speed:.9f} residue={next(iter(res))}')
PY
cat "$WINNER_ENV"
