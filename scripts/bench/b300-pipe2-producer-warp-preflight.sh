...
PIPE2_PRODUCER_PATCH_ONLY=1 ORBITCTA_FLAT=1 ORBITCTA_FLAT_DYNAMIC=1 ORBITCTA_FLAT_DYNAMIC_PIPE2=1 \
  ORBITCTA_FLAT_CHUNK=1 ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP=0 bash "$build" >"${TMPDIR:-/tmp}/oneesan-producer-patch-basic.$$.out"
grep -q 'b300_pipe2_producer_warp_patch=OK' "${TMPDIR:-/tmp}/oneesan-producer-patch-basic.$$.out"
PIPE2_PRODUCER_PATCH_ONLY=1 PRODUCER_PRECTX_WARPCOOP=1 QUAD_MLP=1 ORBITCTA_COL_ILP=4 \
  ...
  bash "$build" >"${TMPDIR:-/tmp}/oneesan-producer-patch-prectx.$$.out"
grep -q 'producer_prectx_warpcoop=1 prectx_flat_bid=1 prectx_flat_bid_fused=0' "${TMPDIR:-/tmp}/oneesan-producer-patch-prectx.$$.out"
rm -f "${TMPDIR:-/tmp}/oneesan-producer-patch-basic.$$.out" "${TMPDIR:-/tmp}/oneesan-producer-patch-prectx.$$.out"
...
for s in \
 ...
 '_dpw${ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP}_ppw${ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_PRECTX_WARPCOOP}' \
 'b300-directgather-orbitcta-pipe2-producer-warp.sh'; do
  grep -Fq "$s" "$run" || { echo "missing n27 producer marker: $s" >&2; exit 3; }
done
grep -Fq '_dpw${ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP}_ppw${ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_PRECTX_WARPCOOP}' "$staged" || { echo 'staged producer fingerprint mismatch' >&2; exit 3; }

echo 'b300_pipe2_producer_warp_preflight=OK producer_profile=OK producer_prectx=OK n27=OK gpu_work=0 actions_triggered=0'
