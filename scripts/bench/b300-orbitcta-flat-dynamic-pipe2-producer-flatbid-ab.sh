...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_pipe2_producer_flatbid_ab_n${N}_t${THREADS}_b${BATCH}}"
...
(( THREADS >= 64 && THREADS % 32 == 0 )) || { echo 'producer flat-bid A/B requires BUCKET_THREADS>=64 and multiple of 32' >&2; exit 2; }
...
[[ "$CPASYNC_PAIR" == 1 ]] || { echo 'producer flat-bid A/B fixes CPASYNC_PAIR=1' >&2; exit 2; }
...
PIPE2_PRODUCER_PATCH_ONLY=1 PRODUCER_PRECTX_WARPCOOP=1 QUAD_MLP=1 ORBITCTA_COL_ILP=4 \
  ...
  bash "$ONEESAN_ROOT/scripts/build/b300-directgather-orbitcta-pipe2-producer-warp.sh" >"$LOGDIR/producer-flatbid-patch.out"
grep -q 'producer_prectx_warpcoop=1 prectx_flat_bid=1 prectx_flat_bid_fused=0' "$LOGDIR/producer-flatbid-patch.out" || {
  echo 'producer flat-bid patch marker missing' >&2; exit 5;
}
...
COMMON=(N="$N" ARCH="$ARCH" PM_ACCUM="$PM_ACCUM" DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$SPARSE64" DIRECTGATHER_SORT_RANKS=0
  ...
  PRECTX_FORWARD=1 PRECTX_REVERSE=1 PRECTX_COMPACT=1 PRECTX_FLAT_BID_FUSED=0 PRECTX_WARPCOOP=0
  PRODUCER_PRECTX_WARPCOOP=1 PTXAS_VERBOSE=1)

printf 'backend\tkernel\tregisters\tstack_bytes\tspill_store_bytes\tspill_load_bytes\tsmem_bytes\tcmem0_bytes\n' >"$RESOURCE"
build_one(){
  local flatbid="$1" name="$2" bin="$ONEESAN_BUILD_DIR/b300_pipe2_producer_flatbid_${name}_t${THREADS}_b${BATCH}_n${N}"
  env "${COMMON[@]}" PRECTX_FLAT_BID="$flatbid" OUT="$bin" \
  ...
  grep -q 'pipe2_producer_prectx_warpcoop=1' "$LOGDIR/${name}.build.err" || { echo "$name producer prectx marker missing" >&2; exit 6; }
  grep -q "prectx_flat_bid=$flatbid" "$LOGDIR/${name}.build.err" || { echo "$name flat-bid marker mismatch" >&2; exit 6; }
  grep -q 'prectx_flat_bid_fused=0' "$LOGDIR/${name}.build.err" || { echo "$name fused marker mismatch" >&2; exit 6; }
  python3 "$PARSER" "$LOGDIR/${name}.build.err" --label "$name" >>"$RESOURCE" || true
  printf '%s' "$bin"
}

binary_bin="$(build_one 0 binary)"
cached_bin="$(build_one 1 cached)"
...
for ((r=1;r<=REPEATS;++r)); do run_one binary "$binary_bin" "$r"; done
for ((r=1;r<=REPEATS;++r)); do run_one cached "$cached_bin" "$r"; done
...
print(f'producer_flatbid_wall_speedup={m("binary","wall_s")/m("cached","wall_s"):.6f}x')
print(f'producer_flatbid_high_speedup={m("binary","high_s")/m("cached","high_s"):.6f}x')
print(f'producer_flatbid_memctrl_delta={m("cached","avg_memctrl_pct")-m("binary","avg_memctrl_pct"):.6f}pp')
print('cached_bid_storage=compact_prectx_pad_u8 bytes_added=0 fused_descriptor_load=0')
PY

echo "pipe2 producer cached-flat-bid A/B OK result=$RESULT resources=$RESOURCE" >&2
