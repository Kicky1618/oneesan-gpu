...
for k in main_load block_load main_store block_store; do
  ...
  if grep -Fq 'D_MAIN_PTR' "$body" || grep -Fq 'D_BLOCK_PTR' "$body"; then
    echo "$k VMM direct path still references dynamic shard pointer array" >&2
    exit 6
  fi
...
echo "b300-vmm-production-ptx-proof OK arch=$ARCH owner_div64=0 owner_mul64=0 owner_compare64=0 dynamic_shard_ptr_index=0 direct_global_index=1"
