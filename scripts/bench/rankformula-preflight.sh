...
echo '=== rankformula exact CUDA: fused13 off ===' >&2
RANKDELTA8_FUSED13=0 ARCH="$ARCH" W="$W" DECODE_LOAD="$DECODE_LOAD" \
...
echo '=== rankformula exact CUDA: fused13 on ===' >&2
RANKDELTA8_FUSED13=1 ARCH="$ARCH" W="$W" DECODE_LOAD="$DECODE_LOAD" \
  RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" \
  bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankformula-cross5-selftest.sh"
...
if [[ "$RUN_BUILD_SMOKE" == 1 ]]; then
  ...
  N="$N" ARCH="$ARCH" HIGH_CTX=warpstriped_delta_direct_affine_rankformula_cross5 \
    ...
    RANKDELTA8_FUSED13=1 TRANSPOSE_MODE=pipeline PTXAS_VERBOSE=1 \
...
echo "rankformula-preflight OK w=$W n=$N arch=$ARCH lut=$RANKSTREAM_LUT_LOAD exact_formula_w28=1 fused13_0_1=1 build_smoke=$RUN_BUILD_SMOKE" >&2
