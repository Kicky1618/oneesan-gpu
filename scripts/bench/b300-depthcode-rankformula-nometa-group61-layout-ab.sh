...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_nometa_group61_layout_ab_n${N}_${TRANSPOSE_MODE}}"
...
for spec in 'group59 0' 'group61 1'; do
  read -r label g61 <<<"$spec"
  if [[ "$RUN_SELFTEST" == 1 ]]; then RANKFORMULA_NOMETA_BLOCK=16 RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP=1 RANKFORMULA_NOMETA_COOP_UNROLL=0 RANKFORMULA_NOMETA_GROUP56=0 RANKFORMULA_NOMETA_GROUP61="$g61" RANKFORMULA_ABSTRACT_SELECT8=1 RANKFORMULA_ABSTRACT_DEPTH4=1 RANKFORMULA_ABSTRACT_SRCPACK10=1 RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" PM_ACCUM="$PM_ACCUM" bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankformula-nometa4-abstract-block-selftest.sh" >"$LOGDIR/$label.selftest.out" 2>"$LOGDIR/$label.selftest.err"; fi
  bin="$ONEESAN_BUILD_DIR/ab_rankformula_nometa_b16_${label}_n${N}"; build_one "$g61" "$label" "$bin"
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
 if q['group59'][k] and q['group61'][k]:print(f'group61_{k}_speedup={q["group59"][k]/q["group61"][k]:.6f}x')
if run_ptxas=='1':
...
print('block=16 coopgroup=1 coop_unroll=0 depth4=1 srcpack10=1')
print('group59_pack_bits=59 self_index_bits=14')
print('group61_pack_bits=61 self_index_bits=0 direct_end_compare=1')
print('group61_source_base_packed=1 closure_direct_source_base=0')
print('group59_abstract_off_device_bytes=480')
print('group61_abstract_off_device_bytes=480')
print('semantic_delta=locator_layout_only')
PY
echo "b300-depthcode-rankformula-nometa-group61-layout-ab OK n=$N repeats=$REPEATS block=16 result=$RESULT" >&2
