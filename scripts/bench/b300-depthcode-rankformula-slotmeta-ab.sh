...
for x in PM_ACCUM TERNARY_KEY4 RUN_SELFTEST RUN_PTXAS; do
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_slotmeta_ab_n${N}_${RANKSTREAM_LUT_LOAD}_${TRANSPOSE_MODE}}"
...
build_one(){
  local label="$1" slotmeta="$2" bin="$3"
  local raw=1
  [[ "$slotmeta" == 1 ]] && raw=0
  N="$N" OUT="$bin" HIGH_CTX=warpstriped_delta_direct_affine_rankformula_cross5 \
    ...
    RANKFORMULA_SPARSE_BASE=1 RANKFORMULA_RAWCODE="$raw" RANKFORMULA_INLINE_CROSS=1 \
    RANKFORMULA_BASE_DELTA=1 RANKFORMULA_SLOTMETA="$slotmeta" \
...
for spec in 'raw2bit 0' 'slotmeta 1'; do
  read -r label slotmeta <<<"$spec"
  if [[ "$RUN_SELFTEST" == 1 ]]; then
    if [[ "$slotmeta" == 1 ]]; then
      ...
      RANKFORMULA_SPARSE_BASE=1 RANKFORMULA_RAWCODE=1 RANKFORMULA_INLINE_CROSS=1 RANKFORMULA_BASE_DELTA=1 \
        RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" PM_ACCUM="$PM_ACCUM" \
        bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankformula-cross5-selftest.sh" \
        >"$LOGDIR/$label.selftest.out" 2>"$LOGDIR/$label.selftest.err"
    fi
  fi
  bin="$ONEESAN_BUILD_DIR/ab_rankformula_slotmeta_${label}_n${N}"
  build_one "$label" "$slotmeta" "$bin"
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
    if q['raw2bit'][k] and q['slotmeta'][k]: print(f'rankformula_slotmeta_{k}_speedup={q["raw2bit"][k]/q["slotmeta"][k]:.6f}x')
...
for mode in ('raw2bit','slotmeta'):
    ...
    for line in text.splitlines():
        if not line.startswith('p10dc_low_rankformula fixed_owner='): continue
        d=dict(re.findall(r'([A-Za-z0-9_]+)=([^ ]+)',line))
        if d: vals.append(d)
    if vals:
        print(f'{mode}_mask_index_bytes_all_gpus={sum(int(v.get("mask_slot_bytes",0)) for v in vals)}')
        print(f'{mode}_base_bytes_all_gpus={sum(int(v.get("base_bytes",0)) for v in vals)}')
        print(f'{mode}_meta_bytes_all_gpus={sum(int(v.get("meta_bytes",0)) for v in vals)}')
        print(f'{mode}_max_meta_bits={max(int(v.get("max_meta_bits",0)) for v in vals)}')
        print(f'{mode}_mask_table_mode={vals[0].get("mask_table_mode","NA")}')
if run_ptxas=='1':
...
print('slotmeta_broadword_support=0')
print('slotmeta_direct_mask_slot=0')
print('slotmeta_cross_lut_loads=0')
PY

echo "b300-depthcode-rankformula-slotmeta-ab OK n=$N repeats=$REPEATS result=$RESULT" >&2
