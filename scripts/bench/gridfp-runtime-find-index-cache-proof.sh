...
for buckets in 16 32 64; do
  grep -Fq "buckets=$buckets components=100000 exact_queries=3200000" <<<"$out"
done
grep -Fq 'bucket_counts=16,32,64 max_pairs=20 total_exact_queries=9600000' <<<"$out"
grep -Fq 'collision_state=packed_high_bit extra_collision_registers=0' <<<"$out"
grep -Fq 'stale_bytes_clear_required=0 false_negative=0 exact=1' <<<"$out"
echo 'gridfp-runtime-find-index-cache-proof OK exact=1 collision_state=packed_high_bit' >&2
