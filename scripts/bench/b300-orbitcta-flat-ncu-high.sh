...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_orbitcta_flat_ncu_n${N}_wi${SPARSE64_WARP_INDEX}_dyn${DYNAMIC}_b${DYNAMIC_BATCH}}"
mkdir -p "$(dirname "$PREFIX")"

for c in ncu nvcc nvidia-smi; do command -v "$c" >/dev/null || { echo "$c required" >&2; exit 2; }; done
for x in PM_ACCUM SPARSE64 SPARSE64_WARP_INDEX CPASYNC_PAIR DYNAMIC DYNAMIC_FUSE_PREP PRECTX_FLAT_BID PRECTX_FLAT_BID_FUSED; do
...
if [[ "$DYNAMIC" == 0 ]]; then
  [[ "$DYNAMIC_BATCH" == 1 && "$DYNAMIC_FUSE_PREP" == 0 && "$DYNAMIC_ADAPTIVE_WAVES" == 0 ]] || { echo 'static flat requires dynamic extras disabled' >&2; exit 2; }
fi
...
ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES="$DYNAMIC_ADAPTIVE_WAVES" ORBITCTA_COL_ILP="$COL_ILP" \
...
echo "config: flat=1 dynamic=$DYNAMIC batch=$DYNAMIC_BATCH adaptive_waves=$DYNAMIC_ADAPTIVE_WAVES fuse_lease_prep=$DYNAMIC_FUSE_PREP sparse64=$SPARSE64 warp_index=$SPARSE64_WARP_INDEX prectx_flat_bid=$PRECTX_FLAT_BID prectx_fused=$PRECTX_FLAT_BID_FUSED ilp=$COL_ILP cpasync_pair=$CPASYNC_PAIR" >&2
