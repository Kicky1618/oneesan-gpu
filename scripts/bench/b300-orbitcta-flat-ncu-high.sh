...
QUAD_MLP="${QUAD_MLP:-0}"; COL_ILP="${ORBITCTA_COL_ILP:-$([[ "${QUAD_MLP:-0}" == 1 ]] && echo 4 || echo 2)}"; CPASYNC_PAIR="${CPASYNC_PAIR:-0}"
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_orbitcta_flat_ncu_n${N}_q${QUAD_MLP}_wi${SPARSE64_WARP_INDEX}_dyn${DYNAMIC}_b${DYNAMIC_BATCH}_p2${DYNAMIC_PIPE2}}"
mkdir -p "$(dirname "$PREFIX")"

for c in ncu nvcc nvidia-smi; do command -v "$c" >/dev/null || { echo "$c required" >&2; exit 2; }; done
for x in PM_ACCUM SPARSE64 SPARSE64_WARP_INDEX QUAD_MLP CPASYNC_PAIR DYNAMIC DYNAMIC_FUSE_PREP DYNAMIC_PIPE2 PRECTX_FLAT_BID PRECTX_FLAT_BID_FUSED; do
...
if [[ "$QUAD_MLP" == 1 ]]; then
  [[ "$DYNAMIC" == 1 ]] || { echo 'this profiler uses QUAD_MLP only with dynamic flat' >&2; exit 2; }
  [[ "$COL_ILP" == 4 ]] || { echo 'QUAD_MLP requires ORBITCTA_COL_ILP=4' >&2; exit 2; }
fi
if [[ "$DYNAMIC" == 0 ]]; then
...
grep -q "quad_mlp=$QUAD_MLP" "${PREFIX}.build.err" || { echo 'build quad marker mismatch' >&2; exit 4; }
...
# compact-prectx, dynamic/quad and pipe2 variants while excluding queue reset.
...
echo "config: flat=1 dynamic=$DYNAMIC batch=$DYNAMIC_BATCH adaptive_waves=$DYNAMIC_ADAPTIVE_WAVES fuse_lease_prep=$DYNAMIC_FUSE_PREP pipe2=$DYNAMIC_PIPE2 quad=$QUAD_MLP sparse64=$SPARSE64 warp_index=$SPARSE64_WARP_INDEX prectx_flat_bid=$PRECTX_FLAT_BID prectx_fused=$PRECTX_FLAT_BID_FUSED ilp=$COL_ILP cpasync_pair=$CPASYNC_PAIR" >&2
