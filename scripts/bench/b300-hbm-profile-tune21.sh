...
COL_ILP="${COL_ILP:-2}"; PM_ACCUM="${PM_ACCUM:-1}"; REPEATS="${REPEATS:-1}"
SAMPLE_INTERVAL="${SAMPLE_INTERVAL:-0.15}"; RUN_CPASYNC="${RUN_CPASYNC:-1}"; RUN_PRECTX="${RUN_PRECTX:-1}"
...
for x in PM_ACCUM RUN_CPASYNC RUN_PRECTX; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0 or 1" >&2; exit 2; }; done
case "$COL_ILP" in 2|4) ;; *) echo 'COL_ILP must be 2 or 4 for pair MLP tuner' >&2; exit 2;; esac
...
profiles=(
  'warp:warp_dense_reg:0:0:0:0:0'
  'warp:warp_sparse_reg:1:0:0:0:0'
  'warp:warp_sparse_sorted_reg:1:1:0:0:0'
  'orbit:orbit_dense_reg:0:0:0:0:0'
  'orbit:orbit_sparse_reg:1:0:0:0:0'
)
if [[ "$RUN_CPASYNC" == 1 ]]; then
  profiles+=(
    'warp:warp_sparse_cpa:1:0:1:0:0'
    'warp:warp_sparse_sorted_cpa:1:1:1:0:0'
    'orbit:orbit_sparse_cpa:1:0:1:0:0'
  )
fi
if [[ "$RUN_PRECTX" == 1 ]]; then
  profiles+=(
    'warp:warp_sparse_prectx:1:0:0:1:1'
    'orbit:orbit_sparse_prectx:1:0:0:1:1'
  )
fi

printf 'profile\tfamily\tsparse64\tsorted\tcpasync\tpre_fwd\tpre_rev\trepeat\tresidue\twall_s\tforward_high_s\treverse_high_s\thigh_s\tprectx_mib_per_gpu\tavg_gpu_util_pct\tavg_memctrl_util_pct\tmax_memctrl_util_pct\n' >"$RESULT"
printf 'backend\tkernel\tregisters\tstack_bytes\tspill_store_bytes\tspill_load_bytes\tsmem_bytes\tcmem0_bytes\n' >"$PTXAS"
PARSER="$ONEESAN_ROOT/scripts/bench/parse-ptxas-resources.py"

for spec in "${profiles[@]}"; do
  IFS=: read -r family name sparse sorted cpa pf pr <<<"$spec"
  bin="$ONEESAN_BUILD_DIR/b300_tune21_${name}"
  if [[ "$family" == warp ]]; then
    N=21 ARCH="$ARCH" OUT="$bin" COL_ILP="$COL_ILP" PM_ACCUM="$PM_ACCUM" DEPTHMAJOR=1 PAIR_MLP=1 MLP_WINDOW4=1 \
      DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$sparse" SORTED="$sorted" CPASYNC_PAIR="$cpa" PRECTX_FORWARD="$pf" PRECTX_REVERSE="$pr" \
      FORCE7=0 PREFETCH_NEXT=0 PTXAS_VERBOSE=1 TRANSPOSE_MODE=pipeline \
      bash "$ONEESAN_ROOT/scripts/build/b300-directgather-colilp-fast.sh" >"$LOGDIR/$name.build.out" 2>"$LOGDIR/$name.build.err"
  else
    [[ "$sorted" == 0 ]] || { echo 'orbit sorted mode is not wired yet' >&2; exit 6; }
    N=21 ARCH="$ARCH" OUT="$bin" DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$sparse" ORBITCTA_COL_ILP="$COL_ILP" PAIR_MLP=1 CPASYNC_PAIR="$cpa" \
...
PY
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
      "$name" "$family" "$sparse" "$sorted" "$cpa" "$pf" "$pr" "$r" "$residue" "$(field wall_s "$line")" "$fh" "$rh" "$high" "$premib" "$ag" "$am" "$mm" >>"$RESULT"
  done
done
...
for z in sorted(profiles.values(),key=lambda z:z['wall']):
 print(f"PROFILE {z['profile']} family={z['family']} wall_s={z['wall']:.6f} high_s={z['high']:.6f} mc_avg={z['mc']:.3f} sparse={z['sparse64']} sorted={z['sorted']} cpa={z['cpasync']} pf={z['pre_fwd']} pr={z['pre_rev']}")
