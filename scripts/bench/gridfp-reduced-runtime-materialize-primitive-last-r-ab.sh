...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 ||
  ...
  echo "primitive final-R runtime A/B supports even W=8..10" >&2
  exit 2
fi
if [[ "$MOD" != 4294967291 ]]; then
  echo "primitive final-R A/B requires MOD=4294967291" >&2
  exit 2
fi
...
build_one() {
  local fast="$1" bin="$2"
  local inject="-DRP_FAST_MATERIALIZE_PRIMITIVE_LAST_R=$fast"
...
run_one() {
  ...
  [[ -n "$line" ]] || {
    echo "materialize_primitive_last_r=$fast $phase$rep missing runtime result" >&2
    cat "$out" >&2 || true; cat "$err" >&2 || true; exit 3
  }
  grep -Fq ' exact=OK' <<<"$line" || {
    echo "materialize_primitive_last_r=$fast $phase$rep failed exactness" >&2
    echo "$line" >&2; exit 4
  }
  if [[ "$phase" == run ]]; then
    local wall
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for fast in "${order[@]}"; do
    ...
    echo "=== runtime materialize-primitive-last-r=$fast run $r/$REPEATS ===" >&2
...
for mode in ('0', '1'):
    xs = [float(r['wall_ms']) for r in rows
          if r['materialize_primitive_last_r'] == mode]
    if not xs:
        raise SystemExit(f'missing materialize_primitive_last_r={mode}')
    out.append({
        'materialize_primitive_last_r': mode,
        'repeats': len(xs),
...
q = {r['materialize_primitive_last_r']: r for r in out}
old = float(q['0']['wall_ms_median'])
new = float(q['1']['wall_ms_median'])
print(f'runtime_materialize_primitive_last_r_wall_speedup={old/new:.6f}x')
print(f'runtime_materialize_primitive_last_r_wall_delta_pct={(new/old-1)*100:.4f}%')
print('runtime_materialize_primitive_last_r_saved_primitive_table_loads_per_materialize=1')
print('runtime_materialize_primitive_last_r_saved_rank_branches_per_materialize=1')
print('runtime_materialize_primitive_last_r_exact=1')
print('runtime_materialize_primitive_last_r_extra_shared_bytes=0')
print('runtime_materialize_primitive_last_r_extra_constant_bytes=0')
print(f'summary={dst}')
PY
...
if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  ...
  echo '--- ptxas forced final R primitive materialize ---' >&2
