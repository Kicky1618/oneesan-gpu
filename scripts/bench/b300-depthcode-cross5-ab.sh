...
REPEATS="${REPEATS:-3}"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_depthcode_cross5_ab_n${N}_${TRANSPOSE_MODE}_${DEPTHCODE_DECODE_LOAD}_pm${PM_ACCUM}_t${BUCKET_THREADS}_gx${BUCKET_GRID_X}_gy${BUCKET_GRID_Y}}"
...
for ctx in warpstriped_delta warpstriped_delta_cross5; do
...
out=[]
for ctx in ('warpstriped_delta','warpstriped_delta_cross5'):
...
base='warpstriped_delta'; opt='warpstriped_delta_cross5'
for m in ('wall_s','forward_high_s','reverse_high_s'):
    if q[base][m]!='NA' and q[opt][m]!='NA':
        print(f'cross5_{m}_speedup={float(q[base][m])/float(q[opt][m]):.6f}x')
if q[base]['forward_high_s']!='NA' and q[base]['reverse_high_s']!='NA' and q[opt]['forward_high_s']!='NA' and q[opt]['reverse_high_s']!='NA':
    b=float(q[base]['forward_high_s'])+float(q[base]['reverse_high_s'])
    o=float(q[opt]['forward_high_s'])+float(q[opt]['reverse_high_s'])
    print(f'cross5_total_high_speedup={b/o:.6f}x')
print(f'summary={dst}')
PY

echo "depthcode-cross5-ab OK n=$N repeats=$REPEATS threads=$BUCKET_THREADS gx=$BUCKET_GRID_X gy=$BUCKET_GRID_Y decode_load=$DEPTHCODE_DECODE_LOAD transpose=$TRANSPOSE_MODE result=$RESULT" >&2
