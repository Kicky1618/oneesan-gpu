...
for cfg in "${configs[@]}";do workers="${cfg%%:*}";domain="${cfg#*:}";echo "augmented n=$N workers=$workers domain=$domain" >&2;line="$($bin "$N" "$workers" "$domain" "$MAX_RUN" "$MAX_SWAP" 2> >(tee /dev/stderr)|tail -n1)";[[ "$(field "$line" objective)" == exact-neutral-augmented-v5.36-plan ]]||exit 4;[[ "$(field "$line" limits_clear)" == 1 ]]||exit 4;[[ "$(field "$line" workspace_audit_ok)" == 1 ]]||exit 4
printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$workers" "$domain" "$(field "$line" baseline_pages_2m)" "$(field "$line" baseline_pages_4k)" "$(field "$line" baseline_transitions)" "$(field "$line" baseline_max_worker_cells)" "$(field "$line" augmented_pages_2m)" "$(field "$line" augmented_pages_4k)" "$(field "$line" augmented_transitions)" "$(field "$line" augmented_max_worker_cells)" "$(field "$line" pages_2m_delta)" "$(field "$line" pages_4k_delta)" "$(field "$line" transition_delta)" "$(field "$line" augmented_rounds)" "$(field "$line" exact_schedule_changes)" "$(field "$line" exact_primary_improvements)" "$(field "$line" exact_profile_improvements)" "$(field "$line" neutral_moves)" "$(field "$line" neutral_candidates)" "$(field "$line" augmented_build_s)" "$(field "$line" workspace_audited_jobs)" "$(field "$line" workspace_audited_cells)" "$(field "$line" workspace_mib)" "$(field "$line" workspace_audit_s)" "$(field "$line" workspace_build_s)" "$line" >>"$out";done
python3 - "$out" <<'PY'
import csv,sys
for r in csv.DictReader(open(sys.argv[1],newline=''),delimiter='\t'):
 ...
 if int(r['workspace_audited_jobs']) <= 0 or int(r['workspace_audited_cells']) <= 0:raise SystemExit('workspace audit unexpectedly empty')
 ...
 print(f"comparison workers={r['workers']} domain_size={r['domain_size']} classification={cls} rounds={rounds} neutral_moves={moves} exact_primary_improvements={r['exact_primary_improvements']} pages_2m_delta={a[0]-b[0]} pages_4k_delta={a[1]-b[1]} transition_delta={a[2]-b[2]} max_worker_delta={am-bm} workspace_jobs={r['workspace_audited_jobs']} workspace_cells={r['workspace_audited_cells']} workspace_mib={float(r['workspace_mib']):.6f} workspace_audit_s={float(r['workspace_audit_s']):.9f} build_s={float(r['augmented_build_s']):.9f}")
PY
