...
printf 'repeat\tbackend\tresidue\twall_s\tpack_s\th2d_s\tkernel_s\td2h_s\tunpack_s\traw\n' >"$out"
...
run_one() {
  local repeat="$1" kind="$2" line backend residue wall pack h2d kernel d2h unpack
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$repeat" "$backend" "$residue" "$wall" "$pack" "$h2d" "$kernel" "$d2h" "$unpack" "$line" >>"$out"
  printf '%s\n' "$residue"
}

for ((r = 1; r <= REPEATS; ++r)); do
  echo "repeat $r/$REPEATS: baseline" >&2
  base_residue="$(run_one "$r" baseline)"
  echo "repeat $r/$REPEATS: factorized" >&2
  fact_residue="$(run_one "$r" factorized)"
...
base_mean="$(awk -F '\t' 'NR>1 && $2=="gridfp-ramstream32-v1" {s+=$4; n++} END {if(n) printf "%.12f", s/n}' "$out")"
fact_mean="$(awk -F '\t' 'NR>1 && $2 ~ /^gridfp-ramstream32-factorized-/ {s+=$4; n++} END {if(n) printf "%.12f", s/n}' "$out")"
