...
trap 'rm -rf "$tmp"' EXIT
root="$tmp/root"
mkdir -p "$root/scripts/run" "$root/scripts/lib" "$root/work" "$root/build" "$root/bin"
...
cat >"$root/scripts/run/b300x8-nextgen-hybrid8-prefetch-guard-stagel-staged-fullprime-race.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
B300_STAGEL_PREPARED_LABEL=stagel_guard_pp
B300_STAGEL_PREPARED_THREADS=512
B300_STAGEL_PREPARED_CONTROL_BIN=$ONEESAN_ROOT/bin/stagel-control
B300_STAGEL_PREPARED_CONTROL_LABEL=stagel_guard_bb
