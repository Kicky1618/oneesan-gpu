...
# label bytepack align block64
CONFIGS=(
  "compact32 0 0 0"
  "bytepack32 1 0 0"
  "compact32_align 0 1 0"
  "compact64 0 0 1"
  "compact64_align 0 1 1"
)
...
build_one(){
  local label="$1" bytepack="$2" align="$3" block64="$4" bin="$5"
...
for spec in "${CONFIGS[@]}"; do
  read -r label bytepack align block64 <<<"$spec"
  ...
  echo "=== build $label ===" >&2
  build_one "$label" "$bytepack" "$align" "$block64" "$bin"
...
layouts=('compact32','bytepack32','compact32_align','compact64','compact64_align')
...
for layout in layouts:
    ...
    if vals:
        print(f'{layout}_runtime_metadata_mib_total={sum(v[3] for v in vals)/(1<<20):.6f}')
        print(f'{layout}_blocks_total={sum(v[1] for v in vals)}')
