...
if (( W > 12 || LOW_LUT_K <= 0 || HIGH_LUT_K <= 0 || LOW_LUT_K + HIGH_LUT_K + 1 != W )); then
  echo "rankformula slotrow32 selftest requires valid W<=12 split" >&2; exit 2
fi
...
BIN="${BIN:-$ONEESAN_BUILD_DIR/pattern10_depthcode_rankformula_slotrow32_selftest_w${W}_pm${PM_ACCUM}_${DECODE_LOAD}_ranklut${RANKSTREAM_LUT_LOAD}}"
mkdir -p "$(dirname "$BIN")"
TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  ...
  -DP10DC_RANKFORMULA_SLOTMETA=1 -DP10DC_RANKFORMULA_SLOTROW32=1 \
  "$SRC" -o "$BIN"
out="$($BIN)"
printf '%s\n' "$out"
...
if grep -Fq "OK W=$W" <<<"$out"; then
  ...
  grep -Fq 'sparse_base=1 base_delta=1 base_values_per_lookup=1 rawcode=0 slotmeta=1 slotrow32=1' <<<"$out"
  grep -Fq 'chunkinfo_elided=1 broadword_support_elided=1 direct_mask_slot_elided=1 support_delta_loads=1' <<<"$out"
  grep -Fq 'inline_cross=1 cross_lut_elided=1' <<<"$out"
  grep -Fq 'cross_runtime_div=0 cross_runtime_mod=0' <<<"$out"
fi
echo "pattern10-depthcode-rankformula-slotrow32-selftest OK W=$W pm=$PM_ACCUM lut=$RANKSTREAM_LUT_LOAD" >&2
