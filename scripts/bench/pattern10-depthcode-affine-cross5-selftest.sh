...
if (( W > 12 || LOW_LUT_K <= 0 || HIGH_LUT_K <= 0 || LOW_LUT_K + HIGH_LUT_K + 1 != W )); then echo "affine CROSS5 selftest requires valid W<=12 split" >&2; exit 2; fi
...
SRC="$ONEESAN_ROOT/src/cuda/gridfp/probes/ramstream32_bucket_orbit_closure_pattern10_depthcode_affine_cross5_selftest.cu"
BIN="${BIN:-$ONEESAN_BUILD_DIR/pattern10_depthcode_affine_cross5_selftest_w${W}_pm${PM_ACCUM}_${DECODE_LOAD}}"
mkdir -p "$(dirname "$BIN")"
