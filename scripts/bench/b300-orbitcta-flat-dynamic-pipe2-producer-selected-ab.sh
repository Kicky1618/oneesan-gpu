...
(( THREADS >= 64 && THREADS % 32 == 0 )) || { echo 'producer sweep requires BUCKET_THREADS>=64 and multiple of 32' >&2; exit 2; }
...
declare -A BIN
BIN[standard]="$ONEESAN_BUILD_DIR/b300_pipe2_selected_standard_t${THREADS}_b${BATCH}_q${QUAD_MLP}_n${N}"
env "${COMMON[@]}" OUT="${BIN[standard]}" bash "$ONEESAN_ROOT/scripts/build/b300-directgather-orbitcta.sh" >"$LOGDIR/standard.build.out" 2>"$LOGDIR/standard.build.err"
...
for w in 0 1 2 3 4; do
  name="w${w}"; BIN[$name]="$ONEESAN_BUILD_DIR/b300_pipe2_selected_producer_w${w}_t${THREADS}_b${BATCH}_q${QUAD_MLP}_n${N}"
...
printf 'variant\tworker_weight\trepeat\tresidue\twall_s\tforward_high_s\treverse_high_s\thigh_s\tavg_gpu_pct\tavg_memctrl_pct\tpeak_gpu_pct\tpeak_memctrl_pct\tforward_regs\treverse_regs\tforward_blocks_per_sm\treverse_blocks_per_sm\n' >"$RESULT"
run_one(){
 local name="$1" weight="$2" bin="$3" rep="$4" so="$LOGDIR/${name}_r${rep}.out" se="$LOGDIR/${name}_r${rep}.err" util="$LOGDIR/${name}_r${rep}.util"
...
PY
 ...
 printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$name" "$weight" "$rep" "$residue" "$(field wall_s "$line")" "$fh" "$rh" "$high" "$ag" "$am" "$pg" "$pm" "$(field forward_regs "$occ")" "$(field reverse_regs "$occ")" "$(field forward_blocks_per_sm "$occ")" "$(field reverse_blocks_per_sm "$occ")" >>"$RESULT"
}
for ((r=1;r<=REPEATS;++r)); do run_one standard -1 "${BIN[standard]}" "$r"; done
for w in 0 1 2 3 4; do for ((r=1;r<=REPEATS;++r)); do run_one "w${w}" "$w" "${BIN[w${w}]}" "$r"; done; done
cat "$RESULT"
python3 - "$RESULT" "$WINNER_ENV" <<'PY'
import csv,statistics,sys
src,out=sys.argv[1:]; rows=list(csv.DictReader(open(src),delimiter='\t'))
variants=['standard','w0','w1','w2','w3','w4']
...
for v in variants: print(f'{v}_wall_median={m(v,"wall_s"):.9f} {v}_high_median={m(v,"high_s"):.9f}')
