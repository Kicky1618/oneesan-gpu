...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_abstract_select8_ab_n${N}_${TRANSPOSE_MODE}}"
...
build_one(){
  local sel="$1" label="$2" bin="$3"
  N="$N" ARCH="$ARCH" OUT="$bin" RANKFORMULA_NOMETA_BLOCK=16 \
    RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP=1 RANKFORMULA_NOMETA_COOP_UNROLL=0 \
    RANKFORMULA_ABSTRACT_SELECT8="$sel" DEPTHCODE_DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" \
...
for spec in 'walk 0' 'select8 1'; do
  read -r label sel <<<"$spec"
  if [[ "$RUN_SELFTEST" == 1 ]]; then
    RANKFORMULA_NOMETA_BLOCK=16 RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP=1 \
      RANKFORMULA_NOMETA_COOP_UNROLL=0 RANKFORMULA_ABSTRACT_SELECT8="$sel" \
  ...
  bin="$ONEESAN_BUILD_DIR/ab_rankformula_abstract_b16_${label}_n${N}"
  build_one "$sel" "$label" "$bin"
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
    if q['walk'][k] and q['select8'][k]: print(f'select8_{k}_speedup={q["walk"][k]/q["select8"][k]:.6f}x')
if run_ptxas=='1':
...
print('block=16 coopgroup=1 coop_unroll=0')
print('walk_abstract_lut_bytes=94206')
print('select8_abstract_lut_bytes=171866')
print('select8_table_bytes=91780')
print('select8_depth14_15_fast_zero=1')
print('semantic_delta=abstract_closure_selection_only')
PY

echo "b300-depthcode-rankformula-abstract-select8-ab OK n=$N repeats=$REPEATS block=16 result=$RESULT" >&2
