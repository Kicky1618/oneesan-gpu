...
grep -Fq 'staged group meta exceeds HBM reserve:' "$OUT"
grep -Fq 'increase GRIDFP_VRAM_RESERVE_MIB' "$OUT"
grep -Fq 'cap_mib=' "$OUT"
grep -Fq 'reserve_mib=' "$OUT"
...
grep -Fq 'calibration=' "$OUT"
grep -Fq 'staged_group_meta.clear();staged_group_meta.shrink_to_fit();' "$OUT"
for stale in 'DeviceGroupMeta hmeta{}' 'cudaMemcpyToSymbol(D_GROUP_META,&hmeta' 'for(int row=0;row<W;++row)'; do
  if grep -Fq "$stale" "$OUT"; then echo "stagedmeta source still contains stale per-group/row artifact: $stale" >&2; exit 3; fi
done

echo "b300-vmm-basearg-stagedmeta-production-generate-proof OK group_meta_bytes=13936 staged_h2d_once=1 per_group_meta_copy=D2D_sync host_meta_rebuild_per_group=0 staged_device_copy=1 default_stage_cap_mib=512 stage_within_hbm_reserve=1 row_limit_env=B300_ROW_LIMIT row_limit_default_full=1 expected_default_groups=16384 expected_default_mib_per_gpu=217.75 expected_old_meta_h2d_gib=5.9541015625 expected_staged_total_h2d_gib=1.701171875 h2d_reduction=3.5x"
