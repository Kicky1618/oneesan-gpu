...
N="${N:-27}";MOD="${MOD:-4294967291}";NGPU="${NGPU:-8}";TARGET_MIB="${TARGET_MIB:-65536}";PLAN_TARGET_MIB="${GRIDFP_PLAN_TARGET_MIB:-16384}";MAX_WINDOW="${MAX_WINDOW:-14}";ROWS="${ROWS:-1}";THREADS_LIST="${THREADS_LIST:-64 128 256 512}"
...
for t in $THREADS_LIST;do
  ...
  GRIDFP_THREADS="$t" GRIDFP_PLAN_TARGET_MIB="$PLAN_TARGET_MIB" B300_ROW_LIMIT="$ROWS" GRIDFP_VRAM_RESERVE_MIB="${GRIDFP_VRAM_RESERVE_MIB:-8192}" "$BIN" "$N" "$MOD" "$TARGET_MIB" "$MAX_WINDOW" "$NGPU" >"$out" 2>"$err"
