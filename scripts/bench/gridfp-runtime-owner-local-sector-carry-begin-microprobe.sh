...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/gridfp_runtime_owner_local_sector_carry_begin_microprobe_b${BLOCKS}_t${THREADS}_i${ITERS}}"
...
BIN0="$ONEESAN_BUILD_DIR/gridfp_runtime_owner_local_sector_carry_begin_microprobe0"
BIN1="$ONEESAN_BUILD_DIR/gridfp_runtime_owner_local_sector_carry_begin_microprobe1"
PTXAS_FLAGS=()
...
build_one() {
  ...
  TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
    ...
    -DRP_RUNTIME_OWNER_LOCAL_SECTOR_CARRY_BEGIN="$carry" \
...
run_one() {
  ...
  local out="$LOGDIR/carry${carry}_run${rep}.out"
  local err="$LOGDIR/carry${carry}_run${rep}.err"
  ...
  grep -Fq ' w28_tree=0 ' <<<"$line" || { echo "unexpected tree result: $line" >&2; exit 4; }
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for carry in "${order[@]}"; do
    ...
    echo "=== owner-local-sector carry=$carry run $r/$REPEATS ===" >&2
    run_one "$carry" "$bin" "$r"
  done
done
cat "$RESULT"
...
for mode in ('0', '1'):
    ...
    if not rs:
        raise SystemExit(f'missing carry={mode}')
    cs = {r['checksum'] for r in rs}
    if len(cs) != 1:
        raise SystemExit(f'nondeterministic checksum carry={mode}: {sorted(cs)}')
...
if checksums['0'] != checksums['1']:
    raise SystemExit(f'checksum mismatch old={checksums["0"]} carry={checksums["1"]}')
with open(dst, 'w', newline='') as f:
    w = csv.DictWriter(f, fieldnames=out[0].keys(), delimiter='\t')
    w.writeheader(); w.writerows(out)
q = {r['carry']: r for r in out}
old = float(q['0']['ns_per_call_median'])
new = float(q['1']['ns_per_call_median'])
print(f'owner_local_sector_carry_begin_microprobe_speedup={old/new:.6f}x')
print(f'owner_local_sector_carry_begin_microprobe_delta_pct={(new/old-1)*100:.4f}%')
print('owner_local_sector_carry_begin_old=parity_binary_plus_begin_reload')
print('owner_local_sector_carry_begin_new=parity_binary_with_begin_invariant')
print('owner_local_sector_carry_begin_W28_max_table_loads_old=4')
print('owner_local_sector_carry_begin_W28_max_table_loads_new=3')
print('owner_local_sector_carry_begin_extra_constant_bytes=0')
print('owner_local_sector_carry_begin_extra_shared_bytes=0')
print(f'checksum={checksums["0"]}')
print(f'summary={dst}')
PY

if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  echo '--- ptxas parity binary + reload ---' >&2
  grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/carry0.build.err" >&2 || true
  echo '--- ptxas parity binary + begin carry ---' >&2
  grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/carry1.build.err" >&2 || true
fi

echo "gridfp-runtime-owner-local-sector-carry-begin-microprobe OK repeats=$REPEATS result=$RESULT" >&2
