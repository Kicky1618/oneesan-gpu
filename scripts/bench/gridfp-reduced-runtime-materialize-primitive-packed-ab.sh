...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 ||
  ...
  echo "packed primitive materialize runtime A/B supports even W=8..10" >&2
  exit 2
fi
if [[ "$MOD" != 4294967291 ]]; then
  echo "packed primitive materialize A/B requires MOD=4294967291" >&2
  exit 2
fi
...
build_one() {
  ...
  local inject="-DRP_FAST_MATERIALIZE_PRIMITIVE_LAST_R=1 -DRP_FAST_MATERIALIZE_PRIMITIVE_PACKED=$packed"
...
run_one() {
  ...
  [[ -n "$line" ]] || {
    echo "materialize_primitive_packed=$packed $phase$rep missing runtime result" >&2
    cat "$out" >&2 || true; cat "$err" >&2 || true; exit 3
  }
  grep -Fq ' exact=OK' <<<"$line" || {
    echo "materialize_primitive_packed=$packed $phase$rep failed exactness" >&2
    echo "$line" >&2; exit 4
  }
  if [[ "$phase" == run ]]; then
    local wall
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for packed in "${order[@]}"; do
    ...
    echo "=== runtime materialize-primitive-packed=$packed run $r/$REPEATS ===" >&2
...
print(f'runtime_materialize_primitive_packed_wall_speedup={old/new:.6f}x')
print(f'runtime_materialize_primitive_packed_wall_delta_pct={(new/old-1)*100:.4f}%')
print('runtime_materialize_primitive_packed_baseline_last_r=1')
print('runtime_materialize_primitive_packed_old_threshold_table_bytes=6960')
print('runtime_materialize_primitive_packed_new_materialize_table_bytes=416')
print('runtime_materialize_primitive_packed_new_threshold_load_bits=32')
print('runtime_materialize_primitive_packed_exact=1')
print(f'summary={dst}')
PY

if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  echo '--- ptxas full primitive threshold table ---' >&2
  grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/packed0.build.err" >&2 || true
  echo '--- ptxas packed materialize threshold table ---' >&2
