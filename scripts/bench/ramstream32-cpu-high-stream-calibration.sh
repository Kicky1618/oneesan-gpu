...
if [[ -z "$MANIFEST" || ! -f "$MANIFEST" ]]; then echo "MANIFEST is required and must exist" >&2; exit 2; fi
...
if [[ "$RUN_VALIDATION" != 0 && "$RUN_VALIDATION" != 1 ]]; then echo "RUN_VALIDATION must be 0 or 1" >&2; exit 2; fi
...
if ((${#sample_rows[@]} < 5)); then echo "manifest must contain at least five calibration samples" >&2; exit 2; fi
...
run_policy() {
  ...
  line="$(CPU_HIGH_MODE=direct CPU_HIGH_MAX_MIB=0 CPU_HIGH_GROUPS_FILE="$path" CPU_HIGH_WORKERS="$CPU_HIGH_WORKERS" CPU_HIGH_OVERLAP="$CPU_HIGH_OVERLAP" CPU_HIGH_CPU_LIST="$CPU_HIGH_CPU_LIST" CPU_LOW_CPU_LIST="$CPU_LOW_CPU_LIST" CPU_LOW_SCHEDULE="$CPU_LOW_SCHEDULE" CPU_LOW_DOMAIN_SIZE="$CPU_LOW_DOMAIN_SIZE" CPU_LOW_DOMAIN_REFINE="$CPU_LOW_DOMAIN_REFINE" CPU_LOW_DOMAIN_PAGE_TIEBREAK="$CPU_LOW_DOMAIN_PAGE_TIEBREAK" "$bin" "$N" "$MODULUS" "$GPU_TARGET_MIB" "$CPU_WORKERS" | tail -n1)"
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$repeat" "$order" "$role" "$sample" "$group" "$groups_file" "$residue" "$(field "$line" wall_s)" "$(field "$line" cpu_high_wall_s)" "$(field "$line" cpu_high_kernel_sum_s)" "$(field "$line" cpu_high_groups)" "$(field "$line" h2d_s)" "$(field "$line" gpu_kernel_s)" "$(field "$line" d2h_s)" "$(field "$line" cpu_low_wall_s)" "$(field "$line" cpu_high_selection_hash)" "$line" >>"$out"
  printf '%s\n' "$residue"
}

reference_residue=""
for ((r=1; r<=REPEATS; ++r)); do
  if ((r % 2 == 1)); then order=forward; indices=(); for ((i=0; i<${#sample_rows[@]}; ++i)); do indices+=("$i"); done; else order=reverse; indices=(); for ((i=${#sample_rows[@]}-1; i>=0; --i)); do indices+=("$i"); done; fi
  echo "repeat $r/$REPEATS low_schedule=$CPU_LOW_SCHEDULE low_domain_size=${CPU_LOW_DOMAIN_SIZE:-none} low_domain_refine=$CPU_LOW_DOMAIN_REFINE low_domain_page=$CPU_LOW_DOMAIN_PAGE_TIEBREAK ($order)" >&2
  for i in "${indices[@]}"; do
    IFS=$'\t' read -r sample groups_file group <<<"${sample_rows[i]}"; echo "  sample=$sample group=$group" >&2
    residue="$(run_policy "$r" "$order" sample "$sample" "$group" "$groups_file")"
    if [[ -z "$reference_residue" ]]; then reference_residue="$residue"; fi
    [[ "$residue" == "$reference_residue" ]] || { echo "residue mismatch sample=$sample got=$residue reference=$reference_residue" >&2; exit 5; }
    if [[ -n "$EXPECTED_RESIDUE" && "$residue" != "$EXPECTED_RESIDUE" ]]; then echo "unexpected residue sample=$sample got=$residue expected=$EXPECTED_RESIDUE" >&2; exit 6; fi
  done
done
...
if [[ "$RUN_VALIDATION" == 1 ]]; then
  ...
  if [[ -f "$validation" ]]; then
    echo "validation-all" >&2; residue="$(run_policy 0 validation validation 0 -1 "$validation")"
    [[ "$residue" == "$reference_residue" ]] || { echo "validation residue mismatch got=$residue reference=$reference_residue" >&2; exit 5; }
  fi
fi

awk -F '\t' 'NR==1 || $3!="sample" {next} { n[$4]++; wall[$4]+=$8; high[$4]+=$9; groups[$4]+=$11 } END { for (s in n) printf("summary sample=%s runs=%d mean_wall_s=%.9f mean_cpu_high_wall_s=%.9f mean_cpu_high_groups=%.3f\n", s,n[s],wall[s]/n[s],high[s]/n[s],groups[s]/n[s]); }' "$out" | sort -t= -k2,2n
