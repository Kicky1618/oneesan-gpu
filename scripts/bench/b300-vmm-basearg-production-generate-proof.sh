...
for stale in D_MAIN_VBASE D_BLOCK_VBASE 'cudaMemcpyToSymbol(D_MAIN_VBASE' 'cudaMemcpyToSymbol(D_BLOCK_VBASE' B300_FAST_SHARD_ADDRESS8 ShardAddress8 D_MAIN_PTR D_BLOCK_PTR D_MAIN_CHUNK D_BLOCK_CHUNK D_NGPU D_MAIN_W D_BLOCK_W; do
  if grep -Fq "$stale" "$OUT"; then echo "basearg generated source still contains $stale" >&2; exit 3; fi
done

echo "b300-vmm-basearg-production-generate-proof OK vmm_base_source=kernel_param vmm_base_symbols=0 vmm_base_symbol_copies=0 logical_shard_chunks=0 logical_shard_views=0 legacy_shard_address_scaffolding=0 compact_interval_bytes=24 direct_global_index=1"
