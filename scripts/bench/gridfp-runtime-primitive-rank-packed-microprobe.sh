...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/gridfp_runtime_primitive_rank_packed_microprobe_b${BLOCKS}_t${THREADS}_i${ITERS}}"
...
bash "$ONEESAN_ROOT/scripts/bench/gridfp-materialize-primitive-packed-proof.sh" \
  >"$LOGDIR/materialize-packed-proof.out" 2>"$LOGDIR/materialize-packed-proof.err"
bash "$ONEESAN_ROOT/scripts/bench/gridfp-runtime-primitive-rank-packed-proof.sh" \
  >"$LOGDIR/rank-packed-proof.out" 2>"$LOGDIR/rank-packed-proof.err"

SRC="$ONEESAN_ROOT/src/cuda/gridfp/gridfp_runtime_primitive_rank_packed_microprobe.cu"
BIN0="$ONEESAN_BUILD_DIR/gridfp_runtime_primitive_rank_packed_microprobe0"
BIN1="$ONEESAN_BUILD_DIR/gridfp_runtime_primitive_rank_packed_microprobe1"
PTXAS_FLAGS=()
...
run_one() {
  ...
  local line
  line="$(grep '^gridfp-runtime-primitive-rank-packed-microprobe ' "$out" | tail -n1 || true)"
  [[ -n "$line" ]] || { cat "$out" >&2 || true; cat "$err" >&2 || true; exit 3; }
  grep -Fq " packed=$packed " <<<"$line" || exit 4
  grep -Fq ' shared_table=materialize_primitive_thresholds ' <<<"$line" || exit 4
  grep -Fq ' avg_threshold_loads_per_call=6.5 ' <<<"$line" || exit 4
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for packed in "${order[@]}"; do
    ...
    echo "=== primitive rank packed=$packed run $r/$REPEATS ===" >&2
...
print(f'primitive_rank_packed_microprobe_speedup={old/new:.6f}x')
print(f'primitive_rank_packed_microprobe_delta_pct={(new/old-1)*100:.4f}%')
print('primitive_rank_packed_old_threshold_load_bits=64')
print('primitive_rank_packed_new_threshold_load_bits=32')
print('primitive_rank_packed_shared_table_bytes=416')
print('primitive_rank_packed_avg_threshold_loads_per_call=6.5')
print('primitive_rank_packed_legacy_64bit_symbol_retained=1')
print('primitive_rank_packed_exact=1')
print(f'checksum={cs["0"]}')
print(f'summary={dst}')
PY

if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  echo '--- ptxas rank full 64-bit thresholds ---' >&2
  grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/packed0.build.err" >&2 || true
  echo '--- ptxas rank shared 32-bit packed thresholds ---' >&2
  grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/packed1.build.err" >&2 || true
fi

echo "gridfp-runtime-primitive-rank-packed-microprobe OK repeats=$REPEATS result=$RESULT" >&2
