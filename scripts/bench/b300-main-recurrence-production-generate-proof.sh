...
S1="$TMP/01_main_mate.cu";S2="$TMP/02_main_pull.cu";S3="$TMP/03_block_pull.cu";S4="$TMP/04_block_mate.cu";S5="$TMP/05_low_cache.cu";S6="$TMP/06_low_chunk.cu";S7="$TMP/07_low_block.cu";S8="$TMP/08_ilp2.cu";S9="$TMP/09_main_recurrence.cu"
...
if helper<0 or gather<0 or helper>=gather:
    raise SystemExit(f'helper ordering invalid helper={helper} gather={gather}')
if s.count('b300_pack_main_transition_cache(m)') != 2:
    raise SystemExit('expected exactly two production unified packing calls')
# p>1 cached path must use ILP2 and must not call main_to_block after full pull.
start=s.find('if(p>1){')
...
# Recurrent cached path must not contain the generic high rank walk in prepare.
prep=s.find('__device__ __forceinline__ void b300_main_pull_prepare')
prep_end=s.find('__global__ void main_pull_kernel_ilp2',prep)
q=s[prep:prep_end]
...
print('b300-main-recurrence-production-generate-proof OK helper_before_gather=1 packing_calls=2 ilp2=1 low_recurrence=1 high_recurrence=1 stale_main_to_block=0 exact_structure=1')
PY

echo "b300-main-recurrence-production-generate-proof OK source=$S9" >&2
