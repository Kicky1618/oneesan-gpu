...
printf 'src_gpu\tgpus\tchunk_elems\tchunk_bytes\tgranularity\ttotal_mib\told_ms\tvmm_ms\tspeedup\told_Gload_s\tvmm_Gload_s\n' >"$RESULT"
...
for src in "${SOURCES[@]}"; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$src" "$(field gpus)" "$(field chunk_elems)" "$(field chunk_bytes)" "$(field granularity)" "$(field total_mib)" "$(field old_ms)" "$(field vmm_ms)" "$(field speedup)" "$(field old_Gload_s)" "$(field vmm_Gload_s)" >>"$RESULT"
done
cat "$RESULT"
python3 - "$RESULT" <<'PY'
import csv,statistics,sys
r=list(csv.DictReader(open(sys.argv[1]),delimiter='\t')); sp=[float(x['speedup']) for x in r]
print(f'b300_vmm_sources={len(r)}')
print(f'b300_vmm_speedup_median={statistics.median(sp):.6f}x')
print(f'b300_vmm_speedup_min={min(sp):.6f}x')
print(f'b300_vmm_speedup_max={max(sp):.6f}x')
print('b300_vmm_owner_ops=0 b300_vmm_dynamic_ptr_index=0')
PY
