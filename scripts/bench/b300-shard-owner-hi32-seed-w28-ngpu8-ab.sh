...
ARCH="$PTX_ARCH" bash "$ONEESAN_ROOT/scripts/bench/b300-shard-owner-hi32-seed-w28-ngpu8-ptx-proof.sh"

SRC="$ONEESAN_ROOT/src/cuda/b300/probes/shard_owner_hi32_seed_w28_ngpu8_microprobe.cu"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_shard_owner_hi32_seed_w28_ngpu8_ab}"
...
run_one(){
  ...
  line="$(grep '^gridfp-b300-shard-owner-hi32-seed-w28-ngpu8-microprobe OK ' "$out" | tail -n1 || true)"
...
for ((r=1;r<=RUNS;++r)); do
  ...
  for mode in "${order[@]}"; do
    echo "=== B300 W28x8 hi32-seed mode=$mode run $r/$RUNS ===" >&2
    run_one "$mode" "$r"
  done
done
cat "$RESULT"
...
for mode in ('0','1','2'):
    ...
    if not xs or len(cs)!=1: raise SystemExit(f'invalid mode={mode}')
    checks[mode]=next(iter(cs))
    out.append({'mode':mode,'runs':len(xs),'median_ms':f'{statistics.median(xs):.9f}',
                'min_ms':f'{min(xs):.9f}','max_ms':f'{max(xs):.9f}',
                'median_Gaddr_s':f'{statistics.median(rates):.9f}','checksum':checks[mode]})
if len(set(checks.values()))!=1: raise SystemExit(f'checksum mismatch {checks}')
with open(dst,'w',newline='') as f:
    w=csv.DictWriter(f,fieldnames=out[0].keys(),delimiter='\t'); w.writeheader(); w.writerows(out)
q={r['mode']:r for r in out}; base=float(q['0']['median_ms'])
for mode,name in [('1','hi32_mul_seed'),('2','hi32_shiftadd_seed')]:
...
best=min(out,key=lambda r:float(r['median_ms']))
print(f'b300_shard_owner_hi32_seed_best_mode={best["mode"]}')
print(f'b300_shard_owner_hi32_seed_best_median_ms={best["median_ms"]}')
print('mode0=three_compare_subtract_stages')
print('mode1=hi32_mul365_seed_plus_single_correction')
print('mode2=hi32_shiftadd365_seed_plus_single_correction')
print('seed_correction_max=1 device_div64=0 device_mul64=0 base_table=0')
...
if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  for mode in 0 1 2; do
    echo "--- ptxas hi32-seed mode=$mode ---" >&2
    grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/mode_${mode}.build.err" >&2 || true
  done
fi

echo "b300-shard-owner-hi32-seed-w28-ngpu8-ab OK runs=$RUNS blocks=$BLOCKS threads=$THREADS iters=$ITERS result=$RESULT" >&2
