...
echo '=== CUDA reference matrix: PM 0/1 x global/ldg x all HIGH contexts ===' >&2
ARCH="$ARCH" W=10 bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-selftest-matrix.sh"

if [[ "$RUN_PTXAS" == 1 ]]; then
  echo '=== ptxas resource comparison ===' >&2
...
echo "b300-depthcode-warpstriped-preflight OK n=$N arch=$ARCH run_ptxas=$RUN_PTXAS run_b300_ab=$RUN_B300_AB decode_load=$DEPTHCODE_DECODE_LOAD ternary_delta_proved=1" >&2
