...
echo '=== rankchunk32 warp block-base sharing proof ===' >&2
...
for pm in 0 1; do
  for load in global ldg; do
    ...
    for ranklut in constant ldg; do
      PM_ACCUM="$pm" DECODE_LOAD="$load" RANKSTREAM_LUT_LOAD="$ranklut" ARCH="$ARCH" W=10 \
...
echo "b300-depthcode-warpstriped-preflight OK n=$N arch=$ARCH run_ptxas=$RUN_PTXAS run_delta_ab=$RUN_DELTA_AB run_cross5_ab=$RUN_CROSS5_AB run_direct_ab=$RUN_DIRECT_AB run_affine_ab=$RUN_AFFINE_AB run_prekey_ab=$RUN_PREKEY_AB run_rank16_ab=$RUN_RANK16_AB run_rank_backends_ab=$RUN_RANK_BACKENDS_AB run_b300_ab=$RUN_B300_AB ternary_delta_proved=1 cross5_cuda=1 low_rank_owner_proved=1 rankstream32_pack_proved=1 rankstream32_warpbase_proved=1 rankchunk32_warpbase_proved=1 direct_resolve_cuda=1 affine_rows_cuda=1 prekey_cuda=1 rank16_cuda=1 rankstream_cuda=1 rankstream32_cuda=1 rankchunk32_cuda=1" >&2
