...
echo '=== rankchunk32 bytepack prefix proof ===' >&2
bash "$ONEESAN_ROOT/scripts/bench/rankchunk32-bytepack-proof.sh"
echo '=== rankchunk32 align32 one-block warp proof ===' >&2
...
echo '=== rankchunk32 bytepack basepair64 proof ===' >&2
bash "$ONEESAN_ROOT/scripts/bench/rankchunk32-basepair64-proof.sh"
echo '=== rankchunk32 basepair64 unaligned warp routing proof ===' >&2
...
echo '=== rankchunk32 basepair64 warp CUDA: packed + aligned ===' >&2
RUN_HOST_PROOF=0 ARCH="$ARCH" bash "$ONEESAN_ROOT/scripts/bench/rankchunk32-basepair64-warp-cuda-selftest.sh"
...
for pm in 0 1; do
  for load in global ldg; do
    ...
    for ranklut in constant ldg ldg256; do
      PM_ACCUM="$pm" DECODE_LOAD="$load" RANKSTREAM_LUT_LOAD="$ranklut" RUN_LAYOUT_PROOF=0 ARCH="$ARCH" W=10 \
...
PM_ACCUM=0 DECODE_LOAD=ldg RANKSTREAM_LUT_LOAD=ldg RANKCHUNK32_BYTEPACK=1 RUN_LAYOUT_PROOF=0 ARCH="$ARCH" W=10 \
  bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankchunk32-cross5-selftest.sh"
echo '=== rankchunk32 align32 full HIGH smoke ===' >&2
PM_ACCUM=0 DECODE_LOAD=ldg RANKSTREAM_LUT_LOAD=ldg RANKCHUNK32_ALIGN32=1 RUN_LAYOUT_PROOF=0 ARCH="$ARCH" W=10 \
  bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankchunk32-cross5-selftest.sh"
...
PM_ACCUM=0 DECODE_LOAD=ldg RANKSTREAM_LUT_LOAD=ldg RANKCHUNK32_BLOCK64=1 RANKCHUNK32_ALIGN32=1 RUN_LAYOUT_PROOF=0 ARCH="$ARCH" W=10 \
  bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankchunk32-cross5-selftest.sh"
echo '=== rankchunk32 bytepack basepair64 full HIGH smoke ===' >&2
PM_ACCUM=0 DECODE_LOAD=ldg RANKSTREAM_LUT_LOAD=ldg RANKCHUNK32_FUSED16=0 RUN_LAYOUT_PROOF=0 ARCH="$ARCH" W=10 \
