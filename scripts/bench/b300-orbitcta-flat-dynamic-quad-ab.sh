...
SPARSE64="${DIRECTGATHER_SPARSE64:-1}"; SPARSE64_WARP_INDEX="${SPARSE64_WARP_INDEX:-0}"; CPASYNC_PAIR="${CPASYNC_PAIR:-0}"; PM_ACCUM="${PM_ACCUM:-1}"
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_dynamic_quad_ab_n${N}_b${BATCH}_aw${ADAPTIVE_WAVES}_p${PIPE2}_wi${SPARSE64_WARP_INDEX}}"
...
for x in SPARSE64 SPARSE64_WARP_INDEX CPASYNC_PAIR PM_ACCUM FUSE_PREP PIPE2 PRECTX_FORWARD PRECTX_REVERSE PRECTX_COMPACT PRECTX_FLAT_BID PRECTX_FLAT_BID_FUSED; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0/1" >&2; exit 2; }; done
...
[[ "$SPARSE64_WARP_INDEX" == 0 || "$SPARSE64" == 1 ]] || { echo 'SPARSE64_WARP_INDEX requires sparse64' >&2; exit 2; }
...
for quad in 0 1; do
  ...
  grep -q "sparse64=$SPARSE64 sparse64_warp_index=$SPARSE64_WARP_INDEX" "$LOGDIR/quad${quad}.build.err" || { echo "sparse build marker mismatch quad=$quad" >&2; exit 6; }
...
printf 'quad\twarp_index\trepeat\tresidue\twall_s\tforward_high_s\treverse_high_s\thigh_s\tavg_gpu_util_pct\tavg_memctrl_util_pct\tmax_memctrl_util_pct\tforward_regs\treverse_regs\tforward_blocks_per_sm\treverse_blocks_per_sm\tdynamic_smem_bytes\tforward_flat_blocks\treverse_flat_blocks\tscheduler_mode\n' >"$RESULT"
for quad in 0 1; do
  bin="$ONEESAN_BUILD_DIR/b300_dynamic_quad${quad}_b${BATCH}_aw${ADAPTIVE_WAVES}_p${PIPE2}_wi${SPARSE64_WARP_INDEX}_n${N}"
...
PY
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$quad" "$SPARSE64_WARP_INDEX" "$rep" "$residue" "$(field wall_s "$line")" "$fh" "$rh" "$high" "$ag" "$am" "$mm" "$(field forward_regs "$occ")" "$(field reverse_regs "$occ")" "$(field forward_blocks_per_sm "$occ")" "$(field reverse_blocks_per_sm "$occ")" "$(field dynamic_smem_bytes "$occ")" "$(field forward_flat_blocks "$grid")" "$(field reverse_flat_blocks "$grid")" "$(field scheduler_mode "$grid")" >>"$RESULT"
  done
done
cat "$RESULT"
python3 - "$RESULT" "$WINNER_ENV" "$SPARSE64_WARP_INDEX" <<'PY'
import csv,statistics,sys
src,winner,wi=sys.argv[1:]; rows=list(csv.DictReader(open(src),delimiter='\t')); z={}
for q in ('0','1'):
...
print(f"DYNAMIC_QUAD warp_index={wi} wall_speedup={z['0']['wall_s']/z['1']['wall_s']:.6f} high_speedup={z['0']['high_s']/z['1']['high_s']:.6f} regs0={z['0']['fr']}/{z['0']['rr']} regs1={z['1']['fr']}/{z['1']['rr']} active0={z['0']['fb']}/{z['0']['rb']} active1={z['1']['fb']}/{z['1']['rb']} smem0={z['0']['smem']} smem1={z['1']['smem']} winner={w}")
with open(winner,'w') as o:
 o.write(f'QUAD_MLP={w}\nORBITCTA_COL_ILP=4\nSPARSE64_WARP_INDEX={wi}\n')
