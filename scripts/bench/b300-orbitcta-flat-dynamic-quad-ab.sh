...
FUSE_PREP="${ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP:-1}"; PIPE2="${ORBITCTA_FLAT_DYNAMIC_PIPE2:-0}"
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_dynamic_quad_ab_n${N}_b${BATCH}}"
...
for x in SPARSE64 CPASYNC_PAIR PM_ACCUM FUSE_PREP PIPE2 PRECTX_FLAT_BID PRECTX_FLAT_BID_FUSED; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0/1" >&2; exit 2; }; done
...
if (( ADAPTIVE_WAVES != 0 )); then (( BATCH > 1 )) || { echo 'adaptive waves require BATCH>1' >&2; exit 2; }; fi
if [[ "$PIPE2" == 1 ]]; then [[ "$FUSE_PREP" == 0 ]] || { echo 'PIPE2 requires FUSE_PREP=0' >&2; exit 2; }; fi
...
# SPARSE64_WARP_INDEX is deliberately fixed at zero here: the existing pair
# helper has that experiment, while the quad helper is benchmarked separately.
...
for quad in 0 1; do
  bin="$ONEESAN_BUILD_DIR/b300_dynamic_quad${quad}_n${N}"
