...
for f in "$DEVICE" "$GROUP" "$CHOOSE_PRE" "$PRIMITIVE_PRE" "$CODEC_PRE" "$ABENV"; do
...
# see the proxy macro. Keep this small, known uncovered set explicit.
choose_tokens="$(grep -o 'RP_CHOOSE\[' "$DEVICE" | wc -l | tr -d ' ')"
primitive_tokens="$(grep -o 'RP_PRIMITIVE\[' "$DEVICE" | wc -l | tr -d ' ')"
[[ "$choose_tokens" == 3 ]] || {
  echo "unexpected RP_CHOOSE token count in device.cuh: $choose_tokens (expected declaration + 2 rank reads)" >&2
  grep -n 'RP_CHOOSE\[' "$DEVICE" >&2 || true
  exit 3
}
[[ "$primitive_tokens" == 2 ]] || {
  echo "unexpected RP_PRIMITIVE token count in device.cuh: $primitive_tokens (expected declaration + 1 rank read)" >&2
...
# Verify each proxy has the intended include-before-redirect structure.
...
echo "gridfp-codec-table-proxy-coverage-proof OK" \
  "device_choose_tokens=$choose_tokens" \
  "device_primitive_tokens=$primitive_tokens" \
  ...
  "proxy_physical_replacement=0 exact=1"
