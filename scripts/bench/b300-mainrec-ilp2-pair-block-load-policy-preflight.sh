...
cat <<'EOF'
 ...
 mates[i7]=b300_high_state_advance;
}
EOF
} >"$src"

# Compose the full Count-load chain N -> O -> Q. Stage R must read Q's current
# ILP8 L2 values, then change only the two-lane ILP2 stream.
...
[[ "$(grep -o 'b300_mainrec_stager_ilp2_load_cs(in+pj' "$tmp/r.cu" | wc -l)" == 2 ]] || exit 3
set +e
...
set -e
((rc!=0)) && grep -Fq 'Stage R requires Stage-N' "$tmp/non.err" || exit 3

echo 'b300-mainrec-ilp2-pair-block-load-policy-preflight OK stage=R low_pair_policies=default,cg,cs low_block_policies=default,cg,cs ilp2_lanes=2 stageq_high_l2_provenance=1 ilp8_byte_identical=1 self_unchanged=1 mate_unchanged=1 double_transform_rejected=1 gpu_work=0'
