...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 || BLOCKS < 1 || REPEATS < 1 || WARMUP < 0 )); then echo "compact primitive runtime A/B supports even W=8..10" >&2; exit 2; fi
[[ "$MOD" == 4294967291 ]] || { echo "compact primitive runtime A/B requires MOD=4294967291" >&2; exit 2; }
...
ARCH="$ARCH" LOG="$LOGDIR/nvcc-prepend-smoke.log" OUT="$ONEESAN_BUILD_DIR/gridfp_primitive_sym_nvcc_prepend_smoke_should_not_exist" \
...
build_one() {
  ...
  grep -Fq "nvcc_prepend=$inject)" "$LOGDIR/fast${fast}.build.out" || { echo "primitive_sym_u32=$fast build did not report expected prepend flags" >&2; exit 6; }
}
...
run_one() {
  ...
  [[ -n "$line" ]] || { echo "primitive_sym_u32=$fast $phase$rep missing runtime result" >&2; cat "$out" >&2 || true; cat "$err" >&2 || true; exit 3; }
  grep -Fq ' exact=OK' <<<"$line" || { echo "primitive_sym_u32=$fast $phase$rep failed exactness" >&2; echo "$line" >&2; exit 4; }
...
for ((r=1;r<=REPEATS;++r)); do if ((r&1)); then order=(0 1); else order=(1 0); fi; for fast in "${order[@]}"; do [[ "$fast" == 0 ]] && bin="$BIN0" || bin="$BIN1"; echo "=== runtime primitive-sym-u32=$fast run $r/$REPEATS ===" >&2; run_one "$fast" "$bin" "$r" run; done; done
cat "$RESULT"
...
q={r['primitive_sym_u32']:r for r in out}; old=float(q['0']['wall_ms_median']); new=float(q['1']['wall_ms_median'])
print(f'runtime_primitive_sym_u32_wall_speedup={old/new:.6f}x'); print(f'runtime_primitive_sym_u32_wall_delta_pct={(new/old-1)*100:.4f}%')
print('runtime_primitive_sym_u32_old_table_bytes=6960'); print('runtime_primitive_sym_u32_new_table_bytes=900'); print('runtime_primitive_sym_u32_saved_bytes=6060')
print('runtime_primitive_sym_u32_turn_direct_compress_inverse=1'); print('runtime_primitive_sym_u32_signature_filter=0'); print('runtime_primitive_sym_u32_index_cache=0'); print('runtime_primitive_sym_u32_exact=1'); print(f'summary={dst}')
PY
if [[ "$PTXAS_VERBOSE" == 1 ]]; then echo '--- ptxas primitive baseline ---' >&2; grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/fast0.build.err" >&2 || true; echo '--- ptxas primitive compact u32 proxy ---' >&2; grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/fast1.build.err" >&2 || true; fi
