...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 || BLOCKS < 1 || REPEATS < 1 || WARMUP < 0 )); then echo "primitive table runtime A/B supports even W=8..10" >&2; exit 2; fi
[[ "$MOD" == 4294967291 ]] || { echo "primitive table runtime A/B requires MOD=4294967291" >&2; exit 2; }
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/gridfp_reduced_runtime_primitive_u32_ab_w${W}_g${NGPU}_b${BLOCKS}}"
...
ARCH="$ARCH" LOG="$LOGDIR/nvcc-prepend-smoke.log" OUT="$ONEESAN_BUILD_DIR/gridfp_primitive_u32_nvcc_prepend_smoke_should_not_exist" \
...
build_one() {
  ...
  grep -Fq "nvcc_prepend=$inject)" "$LOGDIR/mode${mode}.build.out" || { echo "primitive table mode=$mode build did not report expected prepend flags" >&2; exit 6; }
}

BINS=()
for mode in 0 1 2; do BINS[$mode]="$ONEESAN_BUILD_DIR/gridfp_reduced_runtime_primitive_u32_m${mode}_w${W}_g${NGPU}_b${BLOCKS}"; build_one "$mode" "${BINS[$mode]}"; done
...
run_one() {
  ...
  [[ -n "$line" ]] || { echo "primitive table mode=$mode $phase$rep missing runtime result" >&2; cat "$out" >&2 || true; cat "$err" >&2 || true; exit 3; }
  grep -Fq ' exact=OK' <<<"$line" || { echo "primitive table mode=$mode $phase$rep failed exactness" >&2; echo "$line" >&2; exit 4; }
  if [[ "$phase" == run ]]; then local wall="$(sed -nE 's/.* wall_ms=([^[:space:]]+).*/\1/p' <<<"$line")"; [[ -n "$wall" ]] || exit 5; printf '%s\t%s\t%s\n' "$mode" "$rep" "$wall" >>"$RESULT"; fi
}
for ((r=1;r<=WARMUP;++r)); do for mode in 0 1 2; do run_one "$mode" "$r" warmup; done; done
printf 'mode\trepeat\twall_ms\n' >"$RESULT"
for ((r=1;r<=REPEATS;++r)); do
  case $(((r-1)%3)) in 0) order=(0 1 2);; 1) order=(1 2 0);; *) order=(2 0 1);; esac
  for mode in "${order[@]}"; do echo "=== runtime primitive table mode=$mode run $r/$REPEATS ===" >&2; run_one "$mode" "$r" run; done
done
cat "$RESULT"
