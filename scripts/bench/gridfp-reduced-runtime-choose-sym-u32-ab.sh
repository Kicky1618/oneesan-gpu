...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 ||
  ...
  echo "symmetric choose runtime A/B supports even W=8..10" >&2
  exit 2
fi
if [[ "$MOD" != 4294967291 ]]; then
  echo "symmetric choose runtime A/B requires MOD=4294967291" >&2
  exit 2
fi
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/gridfp_reduced_runtime_choose_sym_u32_ab_w${W}_g${NGPU}_b${BLOCKS}}"
...
build_one() {
  local fast="$1" bin="$2"
  local inject="-include $PREINCLUDE -DRP_RUNTIME_CHOOSE_SYM_U32_PREINCLUDE=$fast -DRP_RUNTIME_OWNER_PREFIX_CARRY_BEGIN=0 -DRP_RUNTIME_OWNER_LOCAL_SECTOR_CARRY_BEGIN=0 -DRP_RUNTIME_OWNER_LOCAL_SECTOR_COMPACT=0 -DRP_FAST_COMPONENT_SUPPORT_ADJACENT_MARKS=0 -DRP_FAST_MATERIALIZE_PRIMITIVE_LAST_R=0 -DRP_FAST_MATERIALIZE_PRIMITIVE_PACKED=0"
  ...
  grep -Fq "nvcc_prepend=$inject)" "$LOGDIR/fast${fast}.build.out" || {
    echo "choose_sym_u32=$fast build did not report expected prepend flags" >&2
    cat "$LOGDIR/fast${fast}.build.out" >&2 || true
    exit 6
  }
}

BIN0="$ONEESAN_BUILD_DIR/gridfp_reduced_runtime_choose_sym_u32_0_w${W}_g${NGPU}_b${BLOCKS}"
BIN1="$ONEESAN_BUILD_DIR/gridfp_reduced_runtime_choose_sym_u32_1_w${W}_g${NGPU}_b${BLOCKS}"
build_one 0 "$BIN0"
build_one 1 "$BIN1"
...
run_one() {
  ...
  [[ -n "$line" ]] || {
    echo "choose_sym_u32=$fast $phase$rep missing runtime result" >&2
    cat "$out" >&2 || true; cat "$err" >&2 || true; exit 3
  }
  grep -Fq ' exact=OK' <<<"$line" || {
    echo "choose_sym_u32=$fast $phase$rep failed exactness" >&2
    echo "$line" >&2; exit 4
  }
  if [[ "$phase" == run ]]; then
    local wall
...
out=[]
for mode in ('0','1'):
    xs=[float(r['wall_ms']) for r in rows if r['choose_sym_u32']==mode]
    if not xs: raise SystemExit(f'missing choose_sym_u32={mode}')
    out.append({'choose_sym_u32':mode,'repeats':len(xs),'wall_ms_median':f'{statistics.median(xs):.9f}','wall_ms_min':f'{min(xs):.9f}','wall_ms_max':f'{max(xs):.9f}'})
with open(dst,'w',newline='') as f:
    w=csv.DictWriter(f,fieldnames=out[0].keys(),delimiter='\t'); w.writeheader(); w.writerows(out)
q={r['choose_sym_u32']:r for r in out}
old=float(q['0']['wall_ms_median']); new=float(q['1']['wall_ms_median'])
print(f'runtime_choose_sym_u32_wall_speedup={old/new:.6f}x')
print(f'runtime_choose_sym_u32_wall_delta_pct={(new/old-1)*100:.4f}%')
print('runtime_choose_sym_u32_old_table_bytes=6728')
print('runtime_choose_sym_u32_new_table_bytes=900')
print('runtime_choose_sym_u32_saved_bytes=5828')
print('runtime_choose_sym_u32_nvcc_prepend_smoke=1')
print('runtime_choose_sym_u32_signature_filter=0')
print('runtime_choose_sym_u32_index_cache=0')
print('runtime_choose_sym_u32_exact=1')
print(f'summary={dst}')
PY

if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  echo '--- ptxas choose baseline ---' >&2
  grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/fast0.build.err" >&2 || true
  echo '--- ptxas choose symmetric u32 proxy ---' >&2
