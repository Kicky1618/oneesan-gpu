...
BINS=()
for mode in 0 1 2; do BINS[$mode]="$ONEESAN_BUILD_DIR/gridfp_reduced_runtime_choose_u32_m${mode}_w${W}_g${NGPU}_b${BLOCKS}"; build_one "$mode" "${BINS[$mode]}"; done
...
for ((r=1;r<=REPEATS;++r)); do case $(((r-1)%3)) in 0) order=(0 1 2);; 1) order=(1 2 0);; *) order=(2 0 1);; esac; for mode in "${order[@]}"; do echo "=== runtime choose table mode=$mode run $r/$REPEATS ===" >&2; run_one "$mode" "$r" run; done; done
cat "$RESULT"
python3 - "$RESULT" "$SUMMARY" <<'PY'
import csv,statistics,sys
src,dst=sys.argv[1:]; rows=list(csv.DictReader(open(src),delimiter='\t')); names={'0':'full_u64','1':'sym_u32','2':'tri_u32'}; out=[]
...
for mode in ('1','2'):
 new=float(q[mode]['wall_ms_median']); name=names[mode]; print(f'runtime_choose_{name}_wall_speedup={old/new:.6f}x'); print(f'runtime_choose_{name}_wall_delta_pct={(new/old-1)*100:.4f}%')
print('runtime_choose_mode0_bytes=6728'); print('runtime_choose_mode1_sym_bytes=900'); print('runtime_choose_mode2_tri_bytes=1740'); print('runtime_choose_sym_saved_bytes=5828'); print('runtime_choose_tri_saved_bytes=4988')
print('runtime_choose_turn_direct_compress_inverse=1'); print('runtime_choose_signature_filter=0'); print('runtime_choose_index_cache=0'); print('runtime_choose_exact=1'); print(f'summary={dst}')
PY
if [[ "$PTXAS_VERBOSE" == 1 ]]; then for mode in 0 1 2; do echo "--- ptxas choose table mode=$mode ---" >&2; grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/mode${mode}.build.err" >&2 || true; done; fi
