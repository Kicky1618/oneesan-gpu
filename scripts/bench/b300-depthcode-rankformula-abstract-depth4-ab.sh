...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_abstract_depth4_ab_n${N}_${TRANSPOSE_MODE}}"
...
for spec in 'select8 0' 'depth4 1'; do
  read -r label depth4 <<<"$spec"
  if [[ "$RUN_SELFTEST" == 1 ]]; then RANKFORMULA_NOMETA_BLOCK=16 RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP=1 RANKFORMULA_NOMETA_COOP_UNROLL=0 RANKFORMULA_ABSTRACT_SELECT8=1 RANKFORMULA_ABSTRACT_DEPTH4="$depth4" RANKFORMULA_ABSTRACT_SRCPACK10=1 RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" PM_ACCUM="$PM_ACCUM" bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankformula-nometa4-abstract-block-selftest.sh" >"$LOGDIR/$label.selftest.out" 2>"$LOGDIR/$label.selftest.err"; fi
  bin="$ONEESAN_BUILD_DIR/ab_rankformula_abstract_b16_${label}_n${N}"; build_one "$depth4" "$label" "$bin"
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
 if q['select8'][k] and q['depth4'][k]:print(f'depth4_{k}_speedup={q["select8"][k]/q["depth4"][k]:.6f}x')
if run_ptxas=='1':
...
print('block=16 coopgroup=1 coop_unroll=0 srcpack10=1'); print('select8_abstract_lut_bytes=149598'); print('depth4_abstract_lut_bytes=86058'); print('selection_table_bytes_select8=91780'); print('selection_table_bytes_depth4=28240'); print('semantic_delta=selection_encoding_only')
PY
echo "b300-depthcode-rankformula-abstract-depth4-ab OK n=$N repeats=$REPEATS block=16 result=$RESULT" >&2
