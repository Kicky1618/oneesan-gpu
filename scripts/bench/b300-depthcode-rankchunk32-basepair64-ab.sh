...
RANKCHUNK32_ONESHFL="${RANKCHUNK32_ONESHFL:-1}"; RANKCHUNK32_FUSED16="${RANKCHUNK32_FUSED16:-0}"; RANKCHUNK32_ALIGN32="${RANKCHUNK32_ALIGN32:-1}"; PM_ACCUM="${PM_ACCUM:-0}"; TERNARY_KEY4="${TERNARY_KEY4:-1}"
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_depthcode_rankchunk32_basepair64_ab_n${N}_${TRANSPOSE_MODE}_${DEPTHCODE_DECODE_LOAD}_${RANKSTREAM_LUT_LOAD}_align${RANKCHUNK32_ALIGN32}_fused${RANKCHUNK32_FUSED16}_pm${PM_ACCUM}_t${BUCKET_THREADS}_gx${BUCKET_GRID_X}_gy${BUCKET_GRID_Y}}"
...
if [[ "$RUN_SELFTEST" == 1 ]]; then
  RUN_LAYOUT_PROOF=0 RANKCHUNK32_BYTEPACK=1 RANKCHUNK32_ALIGN32="$RANKCHUNK32_ALIGN32" RANKCHUNK32_BLOCK64=0 \
...
build_one(){
  ...
  N="$N" OUT="$bin" HIGH_CTX="$highctx" DEPTHCODE_DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" \
    ...
    RANKCHUNK32_BYTEPACK=1 RANKCHUNK32_ALIGN32="$RANKCHUNK32_ALIGN32" RANKCHUNK32_BLOCK64=0 \
...
for mode in baseline basepair64; do
  bin="$ONEESAN_BUILD_DIR/ab_depthcode_rankchunk32_${mode}_align${RANKCHUNK32_ALIGN32}_${RANKSTREAM_LUT_LOAD}_${DEPTHCODE_DECODE_LOAD}_${TRANSPOSE_MODE}_n${N}"
  echo "=== build $mode align=$RANKCHUNK32_ALIGN32 ===" >&2; build_one "$mode" "$bin"
  if [[ "$RUN_PTXAS" == 1 ]]; then python3 "$PARSER" "$LOGDIR/${mode}.build.err" --label "$mode" >>"$RESOURCE"; fi
  for ((r=1;r<=REPEATS;++r)); do echo "=== run $mode align=$RANKCHUNK32_ALIGN32 $r/$REPEATS ===" >&2; run_one "$mode" "$bin" "$r"; done
done
cat "$RESULT"

python3 - "$RESULT" "$SUMMARY" "$RESOURCE" "$RUN_PTXAS" "$LOGDIR" "$RANKCHUNK32_ALIGN32" <<'PY'
import csv,pathlib,re,statistics,sys
src,dst,resource,run_ptxas,logdir,align=sys.argv[1:]
...
ld=pathlib.Path(logdir); p=ld/'basepair64_r1.err'
if p.exists():
 text=p.read_text(errors='replace')
 vals=[tuple(map(int,x)) for x in re.findall(r'p10dc_low_rankchunk32_basepair64 fixed_owner=(\d+) base32_entries=(\d+) pair_entries=(\d+) pair_bytes=(\d+)',text)]
 if vals:
  print(f'basepair64_pair_entries_total={sum(v[2] for v in vals)}'); print(f'basepair64_pair_bytes_total={sum(v[3] for v in vals)}')
 parent=[tuple(map(int,x)) for x in re.findall(r'p10dc_low_rankchunk32 fixed_owner=\d+ codes=(\d+) blocks=(\d+) l_ranks=(\d+) bytes=(\d+) meta_entries=(\d+) padding=(\d+)',text)]
 if parent: print(f'basepair64_padding_entries_total={sum(v[5] for v in parent)}')
...
print('basepair64_base_bits=22'); print('basepair64_delta_bits=8')
print(f'basepair64_height_align={32 if align=="1" else 1}')
print(f'basepair64_block_base_loads_per_warp_max={1 if align=="1" else 2}')
print('basepair64_byte_aligned_chunks=1')
print(f'summary={dst}')
PY

echo "b300-depthcode-rankchunk32-basepair64-ab OK n=$N repeats=$REPEATS lut=$RANKSTREAM_LUT_LOAD decode_load=$DEPTHCODE_DECODE_LOAD align=$RANKCHUNK32_ALIGN32 fused16=$RANKCHUNK32_FUSED16 transpose=$TRANSPOSE_MODE result=$RESULT resource=$RESOURCE" >&2