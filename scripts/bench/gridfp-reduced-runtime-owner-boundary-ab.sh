...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/gridfp_reduced_runtime_owner_boundary_ab_w${W}_g${NGPU}_b${BLOCKS}}"
...
build_one() {
  ...
  MODE=two-row-runtime-multigpu \
    ...
    >"$LOGDIR/boundary${boundary}.build.out" 2>"$LOGDIR/boundary${boundary}.build.err"
}

BIN0="$ONEESAN_BUILD_DIR/gridfp_reduced_runtime_owner_boundary0_w${W}_g${NGPU}_b${BLOCKS}"
BIN1="$ONEESAN_BUILD_DIR/gridfp_reduced_runtime_owner_boundary1_w${W}_g${NGPU}_b${BLOCKS}"
build_one 0 "$BIN0"; build_one 1 "$BIN1"

run_one() {
  local boundary="$1" bin="$2" rep="$3" phase="$4"
  local out="$LOGDIR/boundary${boundary}_${phase}${rep}.out" err="$LOGDIR/boundary${boundary}_${phase}${rep}.err"
  "$bin" "$W" "$NGPU" "$BLOCKS" "$MOD" >"$out" 2>"$err"
  local line; line="$(grep '^gridfp-reduced-two-row-runtime-multigpu ' "$out" | tail -n1 || true)"
  [[ -n "$line" ]] || { echo "owner_boundary=$boundary $phase$rep missing runtime result" >&2; cat "$out" >&2 || true; cat "$err" >&2 || true; exit 3; }
  grep -Fq ' exact=OK' <<<"$line" || { echo "owner_boundary=$boundary $phase$rep failed exactness" >&2; echo "$line" >&2; exit 4; }
  if [[ "$phase" == run ]]; then
    local wall; wall="$(sed -nE 's/.* wall_ms=([^[:space:]]+).*/\1/p' <<<"$line")"
    [[ -n "$wall" ]] || exit 5
    printf '%s\t%s\t%s\n' "$boundary" "$rep" "$wall" >>"$RESULT"
  fi
}
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for boundary in "${order[@]}"; do
    [[ "$boundary" == 0 ]] && bin="$BIN0" || bin="$BIN1"
    echo "=== runtime owner-from-boundaries=$boundary run $r/$REPEATS ===" >&2
    run_one "$boundary" "$bin" "$r" run
  done
done
...
print(f'runtime_owner_boundary_wall_speedup={old/new:.6f}x')
print(f'runtime_owner_boundary_wall_delta_pct={(new/old-1)*100:.4f}%')
print('runtime_owner_boundary_old=weighted_midpoint_recompute')
print('runtime_owner_boundary_new=owner_begin_boundary_scan')
print('runtime_owner_boundary_max_global_boundary_loads=15')
print('runtime_owner_boundary_dynamic_owner_divisions=0')
print(f'summary={dst}')
PY

if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  echo '--- ptxas weighted owner ---' >&2; grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/boundary0.build.err" >&2 || true
  echo '--- ptxas boundary owner ---' >&2; grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/boundary1.build.err" >&2 || true
fi
