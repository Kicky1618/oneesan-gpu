...
for chunk in $CHUNKS; do
  ...
  for pool in $POOLS; do
    ...
    for ((r=1;r<=REPEATS;++r)); do
      ...
      if [[ "$pool" == auto ]]; then
        env -u BUCKET_ORBITCTA_FLAT_BLOCKS_PER_SM \
        ...
        BUCKET_THREADS="$THREADS" BUCKET_ORBITCTA_FLAT_BLOCKS_PER_SM="$pool" \
...
for (c,p),g in by.items():
 ...
 out.append((h,w,c,p,mc))
for h,w,c,p,mc in sorted(out): print(f'chunk={c}',f'pool={p}',f'high_s={h:.6f}',f'wall_s={w:.6f}',f'mc_avg_pct={mc:.3f}')
b=min(out)
print('BEST_ORBITCTA_QUAD',f'chunk={b[2]}',f'pool={b[3]}',f'high_s={b[0]:.6f}',f'wall_s={b[1]:.6f}',f'mc_avg_pct={b[4]:.3f}')
PY

echo "b300-orbitcta-flat-quad-sweep OK result=$RESULT resources=$RESOURCE" >&2
