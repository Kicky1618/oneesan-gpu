...
grep -Fq 'routing_cases=210 accumulation_cases=400' <<<"$out"
grep -Fq 'subgroup_width=8 max_pairs=20 max_edge_terms=3' <<<"$out"
grep -Fq 'max_destination_slot=2' <<<"$out"
grep -Fq 'cache_bytes_per_subgroup=140 cache_bytes_per_block=4480' <<<"$out"
grep -Fq 'routing_exact=1 accumulation_exact=1' <<<"$out"

echo 'gridfp-runtime-edgecache-proof OK routing_cases=210 accumulation_cases=400 cache_bytes_per_block=4480' >&2
