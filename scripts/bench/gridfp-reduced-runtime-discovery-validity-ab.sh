...
for proof in discovery-validity discovery-shrink-validity discovery-llrr discovery-roundtrip; do bash "$ONEESAN_ROOT/scripts/bench/gridfp-runtime-${proof}-proof.sh" >"$LOGDIR/${proof}-proof.out" 2>"$LOGDIR/${proof}-proof.err"; done
...
print(f'runtime_discovery_validity_wall_speedup={old/new:.6f}x'); print(f'runtime_discovery_validity_wall_delta_pct={(new/old-1)*100:.4f}%'); print('runtime_discovery_full_validity_scans=0'); print('runtime_discovery_structural_include_rechecks=0'); print('runtime_discovery_rl_validity=two_popcounts'); print('runtime_discovery_ll_rr_validity=structural'); print('runtime_discovery_compressed_blocked_validity=structural'); print(f'summary={dst}')
PY
if [[ "$PTXAS_VERBOSE" == 1 ]]; then echo '--- ptxas full discovery validity ---' >&2; grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/fast0.build.err" >&2 || true; echo '--- ptxas structural discovery ---' >&2; grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/fast1.build.err" >&2 || true; fi
