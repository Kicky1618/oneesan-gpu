...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_abstract_srcpack10_ab_n${N}_${TRANSPOSE_MODE}}"
...
build_one(){ local pack="$1" label="$2" bin="$3"; N="$N" ARCH="$ARCH" OUT="$bin" RANKFORMULA_NOMETA_BLOCK=16 RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP=1 RANKFORMULA_NOMETA_COOP_UNROLL=0 RANKFORMULA_ABSTRACT_SELECT8=1 RANKFORMULA_ABSTRACT_SRCPACK10="$pack" DEPTHCODE_DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" TRANSPOSE_MODE="$TRANSPOSE_MODE" PM_ACCUM="$PM_ACCUM" TERNARY_KEY4="$TERNARY_KEY4" PTXAS_VERBOSE="$RUN_PTXAS" bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-depthcode-rankformula-nometa4-abstract.sh" >"$LOGDIR/$label.build.out" 2>"$LOGDIR/$label.build.err"; }
...
for spec in 'roff16 0' 'srcpack10 1'; do
  read -r label pack <<<"$spec"
  if [[ "$RUN_SELFTEST" == 1 ]]; then RANKFORMULA_NOMETA_BLOCK=16 RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP=1 RANKFORMULA_NOMETA_COOP_UNROLL=0 RANKFORMULA_ABSTRACT_SELECT8=1 RANKFORMULA_ABSTRACT_SRCPACK10="$pack" RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" PM_ACCUM="$PM_ACCUM" bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankformula-nometa4-abstract-block-selftest.sh" >"$LOGDIR/$label.selftest.out" 2>"$LOGDIR/$label.selftest.err"; fi
  bin="$ONEESAN_BUILD_DIR/ab_rankformula_abstract_b16_${label}_n${N}"; build_one "$pack" "$label" "$bin"
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
 if q['roff16'][k] and q['srcpack10'][k]:print(f'srcpack10_{k}_speedup={q["roff16"][k]/q["srcpack10"][k]:.6f}x')
if run_ptxas=='1':
...
print('block=16 coopgroup=1 coop_unroll=0 select8=1'); print('roff16_abstract_lut_bytes=171866'); print('srcpack10_abstract_lut_bytes=149598'); print('srcpack10_source_bytes=57338'); print('semantic_delta=source_rank_storage_only')
PY
echo "b300-depthcode-rankformula-abstract-srcpack10-ab OK n=$N repeats=$REPEATS block=16 result=$RESULT" >&2
