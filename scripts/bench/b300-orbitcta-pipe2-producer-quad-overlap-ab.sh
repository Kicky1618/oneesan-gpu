...
(( THREADS >= 64 && THREADS % 32 == 0 )) || { echo 'producer quad-overlap A/B requires warp-aligned threads>=64' >&2; exit 2; }
...
overlap_bin="$(build_one 1 overlap)"
...
PY
  ...
  local ag am pg pm; read -r ag am pg pm < <(awk '{sg+=$1;sm+=$2;if($3>pg)pg=$3;if($4>pm)pm=$4;n++}END{if(n)printf "%.6f %.6f %d %d\n",sg/n,sm/n,pg,pm;else print "NA NA NA NA"}' "$util")
...
for ((r=1;r<=REPEATS;++r)); do run_one overlap "$overlap_bin" "$r"; done
cat "$RESULT"
python3 - "$RESULT" <<'PY'
...
assert len({x['residue'] for x in r})==1
print(f'producer_quad_overlap_wall_speedup={m("base","wall_s")/m("overlap","wall_s"):.6f}x')
