...
MIN_STAGED_SPEEDUP="${MIN_STAGED_SPEEDUP:-1.02}";MICRO_ITERS="${MICRO_ITERS:-4096}";MICRO_REPEATS="${MICRO_REPEATS:-9}";STAGED_SLOTS="${STAGED_SLOTS:-1024}"
...
STAGED_SPEEDUP="$(python3 - "$MICRO_RESULT" <<'PY'
import csv,statistics,sys
r=list(csv.DictReader(open(sys.argv[1]),delimiter='\t'))
print(f"{statistics.median(float(x['staged_speedup_vs_packed']) for x in r):.9f}")
PY
)"
echo "b300_group_meta_staged_median_speedup_vs_packed=${STAGED_SPEEDUP}x" >&2
set +e
python3 - "$STAGED_SPEEDUP" "$MIN_STAGED_SPEEDUP" <<'PY'
import sys
s,t=map(float,sys.argv[1:]);raise SystemExit(0 if s>=t else 9)
PY
gate_rc=$?;set -e
case "$gate_rc" in 0);;9) echo "b300-vmm-stagedmeta-production-ab SKIP_FULL micro_staged_speedup=${STAGED_SPEEDUP}x threshold=${MIN_STAGED_SPEEDUP}x";exit 0;;*)exit "$gate_rc";;esac
...
printf 'mode\trun\tresidue\twall_s\tprepare_s\ttotal_s\tactive_max_s\tactive_sum_s\tmax_intervals\tstaged_groups\tstaged_mib_per_gpu\tstaged_total_h2d_gib\n' >"$RESULT"
run_one(){
...
PY
  ...
  if [[ "$mode" == stage ]];then
    ...
    groups="$(mfield groups)";mib="$(mfield mib_per_gpu)";h2d="$(mfield total_h2d_gib)";[[ -n "$groups"&&-n "$mib"&&-n "$h2d" ]]||exit 6
    if [[ "$TARGET_MIB" == 16384 && "$MAX_WINDOW" == 14 ]];then [[ "$groups" == 16384 ]]||{ echo "unexpected default staged group count $groups" >&2;exit 7; };fi
  fi
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$mode" "$run" "$residue" "$wall" "$prep" "$total" "$active" "$sum" "$ints" "$groups" "$mib" "$h2d" >>"$RESULT"
}
for((r=1;r<=RUNS;++r));do if((r&1));then order=(pack stage);else order=(stage pack);fi;for m in "${order[@]}";do run_one "$m" "$r";done;done
cat "$RESULT"
python3 - "$RESULT" "$SUMMARY" "$STAGED_SPEEDUP" <<'PY'
import csv,statistics,sys
...
for mode in ('pack','stage'):
 ...
 if mode=='stage':
  for k in ('staged_groups','staged_mib_per_gpu','staged_total_h2d_gib'):
   vals={x[k] for x in r};
...
print(f'b300_stagedmeta_micro_speedup_vs_packed={float(micro):.6f}x')
...
print(f'summary={dst}')
PY
echo "b300-vmm-stagedmeta-production-ab OK runs=$RUNS micro_gate=${STAGED_SPEEDUP}x threshold=${MIN_STAGED_SPEEDUP}x result=$RESULT" >&2
