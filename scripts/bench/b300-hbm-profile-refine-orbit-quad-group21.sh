...
PROFILE_OUT="${PROFILE_OUT:-$ONEESAN_ROOT/work/b300_hbm_profile_group21.env}"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_hbm_profile_orbit_quad_group21}"
...
GROUP_VALUES="${GROUP_VALUES:-1 2 4}"
...
for x in ORBIT_SPARSE64 ORBIT_CPASYNC_PAIR ORBIT_PRECTX_FORWARD ORBIT_PRECTX_REVERSE ORBIT_PRECTX_COMPACT ORBIT_PRECTX_WARPCOOP ORBITCTA_FLAT ORBIT_QUAD_MLP ORBIT_QUAD_OVERLAP_LOCAL ORBIT_QUAD_SPARSE_DESC_MLP ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0; do
...
case "$ORBIT_QUAD_CPASYNC_GROUP_COLS" in 1|2|4) ;; *) echo 'bad existing ORBIT_QUAD_CPASYNC_GROUP_COLS' >&2; exit 2;; esac
[[ "$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES" == 0 ]] || { echo 'group refine must run before prefetch refinement (prefetch=0)' >&2; exit 2; }
[[ "$ORBIT_QUAD_CPASYNC_GROUP_COLS" == 1 ]] || { echo 'group refine input must be untuned (group_cols=1)' >&2; exit 2; }
[[ "$ORBIT_QUAD_MLP" == 1 && "$ORBIT_QUAD_OVERLAP_LOCAL" == 1 ]] || { echo 'group refine requires selected quad overlap-local path' >&2; exit 2; }
[[ "$ORBIT_CPASYNC_PAIR" == 1 && "$ORBIT_COL_ILP" == 4 ]] || { echo 'group refine requires cp.async quad ILP4' >&2; exit 2; }
[[ "$ORBITCTA_FLAT" == 1 ]] && (( ORBITCTA_FLAT_CHUNK > 1 )) || { echo 'group refine requires chunked flat orbit CTA' >&2; exit 2; }
[[ "$ORBITCTA_FLAT_BLOCKS_PER_SM" =~ ^[0-9]+$ ]] || { echo 'bad ORBITCTA_FLAT_BLOCKS_PER_SM' >&2; exit 2; }
[[ "$ORBIT_QUAD_LOCAL_DIRECT_MAX" == 0 ]] || { echo 'group refine expects QOL direct local path' >&2; exit 2; }
[[ "$ORBIT_QUAD_SPARSE_DESC_MLP" == 0 || "$ORBIT_SPARSE64" == 1 ]] || { echo 'qsd1 requires sparse64' >&2; exit 2; }
if [[ "$ORBIT_PRECTX_WARPCOOP" == 1 ]]; then
  [[ "$ORBIT_PRECTX_COMPACT" == 1 && "$ORBIT_PRECTX_FORWARD" == 1 && "$ORBIT_PRECTX_REVERSE" == 1 ]] || { echo 'warpcoop group refine requires compact forward+reverse prectx' >&2; exit 2; }
fi
for gc in $GROUP_VALUES; do case "$gc" in 1|2|4) ;; *) echo "bad GROUP_VALUES entry=$gc" >&2; exit 2;; esac; done
...
printf 'group_cols\trepeat\tresidue\twall_s\tforward_high_s\treverse_high_s\thigh_s\tavg_gpu_util_pct\tavg_memctrl_util_pct\tmax_memctrl_util_pct\n' >"$RESULT"
...
for gc in $GROUP_VALUES; do
  bin="$ONEESAN_BUILD_DIR/b300_orbit_quad_group${gc}_wc${ORBIT_PRECTX_WARPCOOP}_qsd${ORBIT_QUAD_SPARSE_DESC_MLP}_ql0${ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0}_n21"
  N=21 ARCH="$ARCH" OUT="$bin" PM_ACCUM="$PM_ACCUM" DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$ORBIT_SPARSE64" DIRECTGATHER_SORT_RANKS=0 \
    ...
    QUAD_CPASYNC_PREFETCH_BYTES=0 QUAD_CPASYNC_GROUP_COLS="$gc" \
    ...
    bash "$ONEESAN_ROOT/scripts/build/b300-directgather-orbitcta.sh" >"$LOGDIR/gc${gc}.build.out" 2>"$LOGDIR/gc${gc}.build.err"
  grep -q "quad_cpasync_group_cols=$gc" "$LOGDIR/gc${gc}.build.err" || { echo "group build marker mismatch gc=$gc" >&2; exit 6; }
  python3 "$ONEESAN_ROOT/scripts/bench/parse-ptxas-resources.py" "$LOGDIR/gc${gc}.build.err" --label "gc${gc}" >>"$RESOURCE" || true

  for ((r=1;r<=REPEATS;++r)); do
    so="$LOGDIR/gc${gc}_r${r}.out"; se="$LOGDIR/gc${gc}_r${r}.err"; util="$LOGDIR/gc${gc}_r${r}.util"
    ...
    (( rc == 0 )) || { echo "group=$gc repeat=$r failed rc=$rc" >&2; exit "$rc"; }
    line="$(grep '^residue=' "$so" | tail -n1 || true)"; [[ -n "$line" ]] || { echo "group=$gc missing residue" >&2; exit 3; }
    residue="$(field residue "$line")"; [[ "$residue" == "$EXPECT" ]] || { echo "group=$gc residue=$residue expected=$EXPECT" >&2; exit 4; }
    ...
    [[ -n "$fh" && -n "$rh" ]] || { echo "group=$gc missing HIGH timing" >&2; exit 7; }
...
PY
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$gc" "$r" "$residue" "$(field wall_s "$line")" "$fh" "$rh" "$high" "$ag" "$am" "$mm" >>"$RESULT"
  done
done
...
for r in rows: by.setdefault(int(r['group_cols']),[]).append(r)
if not by: raise SystemExit('no group rows')
...
for line in open(profile_in):
 ...
 kv[k]=v.strip('"')
kv['ORBIT_QUAD_CPASYNC_GROUP_COLS']=str(winner)
if 'ORBIT_QUAD_CPASYNC_GROUP_COLS' not in order: order.append('ORBIT_QUAD_CPASYNC_GROUP_COLS')
if winner!=1: kv['ORBIT_PROFILE']=kv.get('ORBIT_PROFILE','orbit')+f'_gc{winner}'
with open(profile_out,'w') as f:
 f.write('# generated by b300-hbm-profile-refine-orbit-quad-group21.sh\n')
 for k in order:
  v=kv[k]
  if k=='CANDIDATES': f.write(f'{k}="{v}"\n')
  else: f.write(f'{k}={v}\n')
for k in sorted(stats): print(f'GROUP_{k}',f"wall_s={stats[k]['wall']:.6f}",f"high_s={stats[k]['high']:.6f}",f"mc_avg_pct={stats[k]['mc']:.3f}")
print('BEST_QUAD_CPASYNC_GROUP_COLS='+str(winner),f'profile_file={profile_out}')
PY
cat "$PROFILE_OUT"
echo "orbit quad cpasync group refine OK input=$PROFILE_IN output=$PROFILE_OUT result=$RESULT resources=$RESOURCE" >&2
