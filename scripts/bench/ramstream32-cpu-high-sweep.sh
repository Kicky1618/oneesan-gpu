...
if ((${#thresholds[@]} == 0)); then echo "THRESHOLDS must contain at least one value" >&2; exit 2; fi
for x in "${thresholds[@]}"; do [[ "$x" =~ ^([0-9]+([.][0-9]*)?|[.][0-9]+)$ ]] || { echo "invalid threshold: $x" >&2; exit 2; }; done
...
cost_plan_path=""; cost_plan_log=""; policy_out=""; policy_log=""; numa_analysis_out=""
...
analysis_field() {
  ...
  awk -v o="$CPU_HIGH_OVERLAP" -v p="$prefix" -v n="$name" '$1==p && $2=="mode=direct" && $3==("overlap=" o) { for (i=4; i<=NF; ++i) if ($i ~ ("^" n "=")) { sub("^" n "=", "", $i); print $i; exit } }' "$analysis_out"
}

if [[ "$CPU_HIGH_MODE" == direct && "$COST_PLAN" != none && -n "$COST_PLAN" ]]; then
  if [[ "$COST_PLAN" == auto ]]; then
    if [[ "$BUILD" != 0 ]]; then N="$N" ARCH="$ARCH" bash scripts/build/gridfp-ramstream32-cpu-high-cost-plan.sh; fi
    cost_bin="$ROOT/build/ramstream32_cpu_high_cost_plan_n${N}"
    [[ -x "$cost_bin" ]] || { echo "missing executable: $cost_bin" >&2; exit 3; }
    cost_plan_path="$OUT_DIR/cpu-high-cost-plan-n${N}-${ts}.tsv"; cost_plan_log="$OUT_DIR/cpu-high-cost-plan-n${N}-${ts}.log"
    "$cost_bin" "$N" >"$cost_plan_path" 2>"$cost_plan_log"
  else
    cost_plan_path="$COST_PLAN"; [[ -f "$cost_plan_path" ]] || { echo "missing COST_PLAN: $cost_plan_path" >&2; exit 3; }
  fi
fi
...
EOF
if [[ -n "$cost_plan_path" ]]; then echo "cost_plan_sha256=$(file_sha256 "$cost_plan_path")" >>"$meta"; fi
...
run_one() {
  local repeat="$1" order="$2" threshold="$3" line residue got_schedule got_domain
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for threshold in "${run_thresholds[@]}"; do
    ...
    if [[ "$residue" != "$reference_residue" ]]; then echo "residue mismatch threshold=$threshold got=$residue reference=$reference_residue" >&2; exit 5; fi
    if [[ -n "$EXPECTED_RESIDUE" && "$residue" != "$EXPECTED_RESIDUE" ]]; then echo "unexpected residue threshold=$threshold got=$residue expected=$EXPECTED_RESIDUE" >&2; exit 6; fi
  done
done
...
if [[ "$ANALYZE" != 0 ]]; then
  ...
  if [[ "$CPU_HIGH_MODE" == direct && "$GENERATE_POLICY" != 0 && -n "$cost_plan_path" ]]; then
    pcie_rate="$(analysis_field affine_calibration pcie_gib_s)"; pcie_copy_overhead="$(analysis_field affine_calibration pcie_copy_overhead_us)"; cpu_rate="$(analysis_field affine_calibration cpu_gcell_s)"; cpu_group_overhead="$(analysis_field affine_calibration cpu_group_overhead_us)"; gpu_rate="$(analysis_field affine_calibration gpu_gstate_s)"; gpu_group_overhead="$(analysis_field affine_calibration gpu_group_overhead_us)"
    [[ -n "$pcie_rate" ]] || pcie_rate="$(analysis_field calibration pcie_gib_s)"; [[ -n "$cpu_rate" ]] || cpu_rate="$(analysis_field calibration cpu_gcell_s)"; [[ -n "$gpu_rate" ]] || gpu_rate="$(analysis_field calibration gpu_gstate_s)"
    if [[ -n "$pcie_rate" && -n "$cpu_rate" ]]; then
      policy_out="$OUT_DIR/cpu-high-cost-policy-overlap${CPU_HIGH_OVERLAP}-n${N}-${ts}.groups"; policy_log="$OUT_DIR/cpu-high-cost-policy-overlap${CPU_HIGH_OVERLAP}-n${N}-${ts}.log"
      planner_args=(scripts/tools/plan_cpu_high_groups.py "$cost_plan_path" --pcie-gib-s "$pcie_rate" --cpu-gcell-s "$cpu_rate" --gpu-target-mib "$GPU_TARGET_MIB")
      if [[ -n "$pcie_copy_overhead" ]]; then planner_args+=(--pcie-copy-overhead-us "$pcie_copy_overhead"); fi; if [[ -n "$gpu_rate" ]]; then planner_args+=(--gpu-gstate-s "$gpu_rate"); fi; if [[ -n "$cpu_group_overhead" ]]; then planner_args+=(--group-overhead-us "$cpu_group_overhead"); fi; if [[ -n "$gpu_group_overhead" ]]; then planner_args+=(--gpu-group-overhead-us "$gpu_group_overhead"); fi; if [[ "$CPU_HIGH_OVERLAP" == 1 ]]; then planner_args+=(--overlap); fi
      python3 "${planner_args[@]}" >"$policy_out" 2>"$policy_log"; numa_analysis_out="$OUT_DIR/cpu-high-cost-policy-overlap${CPU_HIGH_OVERLAP}-n${N}-${ts}.numa.txt"; python3 scripts/tools/analyze_cpu_high_numa.py "$cost_plan_path" --groups-file "$policy_out" >"$numa_analysis_out"; echo "policy=$policy_out"; echo "policy_log=$policy_log"; echo "numa_analysis=$numa_analysis_out"
    else echo "cost-model policy not generated: calibration rates unavailable" >&2; fi
  fi
fi

echo "results=$out"; echo "metadata=$meta"; if [[ -n "$cost_plan_path" ]]; then echo "cost_plan=$cost_plan_path"; fi; if [[ -n "$cost_plan_log" ]]; then echo "cost_plan_log=$cost_plan_log"; fi; if [[ "$ANALYZE" != 0 ]]; then echo "analysis=$analysis_out"; fi
