...
OUT_DIR="${OUT_DIR:-$ROOT/work/bench_ramstream32_cpu_high_policy_ab}"

if [[ -z "$GROUPS_FILE" ]]; then
  echo "GROUPS_FILE is required" >&2
  exit 2
fi
if [[ ! -f "$GROUPS_FILE" ]]; then
  echo "missing GROUPS_FILE: $GROUPS_FILE" >&2
  exit 2
fi
if [[ ! "$THRESHOLD_MIB" =~ ^([0-9]+([.][0-9]*)?|[.][0-9]+)$ ]]; then
  echo "THRESHOLD_MIB must be non-negative" >&2
  exit 2
fi
...
out="$OUT_DIR/policy-ab-n${N}-${ts}.tsv"
meta="$OUT_DIR/policy-ab-n${N}-${ts}.meta"
...
run_one() {
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$(field "$line" cpu_high_wall_s)" "$(field "$line" cpu_low_wall_s)" \
    "$(field "$line" pcie_removed_tib_per_residue)" \
    "$(field "$line" cpu_high_groups)" "$(field "$line" cpu_high_selection_hash)" \
    "$line" >>"$out"
  printf '%s\n' "$residue"
}
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for variant in "${variants[@]}"; do
    ...
    if [[ "$residue" != "$reference_residue" ]]; then
      echo "residue mismatch variant=$variant got=$residue reference=$reference_residue" >&2
      exit 5
    fi
    if [[ -n "$EXPECTED_RESIDUE" && "$residue" != "$EXPECTED_RESIDUE" ]]; then
      echo "unexpected residue variant=$variant got=$residue expected=$EXPECTED_RESIDUE" >&2
      exit 6
    fi
  done
done
...
awk -F '\t' '
  ...
  {
    n[$3]++;
    wall[$3]+=$5; dma[$3]+=$6+$8; gpu[$3]+=$7;
    high[$3]+=$9; low[$3]+=$10; removed[$3]+=$11;
  }
  END {
    for (v in n)
      printf("summary variant=%s runs=%d mean_wall_s=%.9f mean_dma_s=%.9f mean_gpu_kernel_s=%.9f mean_cpu_high_wall_s=%.9f mean_cpu_low_wall_s=%.9f mean_pcie_removed_tib=%.9f\n",
             v,n[v],wall[v]/n[v],dma[v]/n[v],gpu[v]/n[v],high[v]/n[v],low[v]/n[v],removed[v]/n[v]);
    if (n["threshold"] && n["policy"]) {
      tw=wall["threshold"]/n["threshold"];
      pw=wall["policy"]/n["policy"];
      printf("comparison threshold_mean_wall_s=%.9f policy_mean_wall_s=%.9f policy_speedup=%.9fx policy_saved_s=%.9f\n",
             tw,pw,tw/pw,tw-pw);
    }
  }
' "$out" | sort
