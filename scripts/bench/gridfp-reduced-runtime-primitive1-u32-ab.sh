...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 ||
  ...
  echo "primitive1 u32 runtime A/B supports even W=8..10" >&2
  exit 2
fi
if [[ "$MOD" != 4294967291 ]]; then
  echo "primitive1 u32 runtime A/B requires MOD=4294967291" >&2
  exit 2
fi
...
OUT="$ONEESAN_BUILD_DIR/gridfp_primitive1_nvcc_prepend_smoke_should_not_exist" \
...
bash "$ONEESAN_ROOT/scripts/bench/gridfp-primitive1-u32-table-proof.sh" \
  >"$LOGDIR/primitive1-proof.out" 2>"$LOGDIR/primitive1-proof.err"
...
build_one() {
  ...
  grep -Fq "nvcc_prepend=$inject)" "$LOGDIR/fast${fast}.build.out" || {
    echo "primitive1_u32=$fast build did not report expected prepend flags" >&2
...
run_one() {
  ...
  [[ -n "$line" ]] || {
    echo "primitive1_u32=$fast $phase$rep missing runtime result" >&2
    cat "$out" >&2 || true; cat "$err" >&2 || true; exit 3
  }
  grep -Fq ' exact=OK' <<<"$line" || {
    echo "primitive1_u32=$fast $phase$rep failed exactness" >&2
    echo "$line" >&2; exit 4
  }
  if [[ "$phase" == run ]]; then
    local wall
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for fast in "${order[@]}"; do
    ...
    echo "=== runtime primitive1-u32=$fast run $r/$REPEATS ===" >&2
...
print(f'runtime_primitive1_u32_wall_speedup={old/new:.6f}x')
print(f'runtime_primitive1_u32_wall_delta_pct={(new/old-1)*100:.4f}%')
print('runtime_primitive1_u32_table_bytes=56')
print('runtime_primitive1_u32_fallback_table=RP_PRIMITIVE_PREINCLUDE_ORIG')
print('runtime_primitive1_u32_nvcc_prepend_smoke=1')
print('runtime_primitive1_u32_signature_filter=0')
print('runtime_primitive1_u32_index_cache=0')
print('runtime_primitive1_u32_exact=1')
print(f'summary={dst}')
PY

if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  echo '--- ptxas primitive1 baseline ---' >&2
  grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/fast0.build.err" >&2 || true
  echo '--- ptxas primitive1 u32 proxy ---' >&2
