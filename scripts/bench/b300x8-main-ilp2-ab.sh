...
LOGDIR="${LOGDIR:-$ONEESAN_ROOT/work/b300x8_main_ilp2_ab_n${N}}"
mkdir -p "$LOGDIR"

printf 'main_ilp2\twall_s\tactive_max_s\tactive_sum_s\n'
for mode in 0 1; do
  log="$LOGDIR/ilp2_${mode}.log"
  echo "=== B300 x8 main-pull ILP2=$mode one-row ===" >&2
  ROWS=1 GRIDFP_THREADS="$THREADS" TARGET_MIB="$TARGET_MIB" GRIDFP_PLAN_TARGET_MIB="$PLAN_MIB" MAX_WINDOW="$MAX_WINDOW" \
  MAIN_PULL_ILP2="$mode" HEIGHT_CACHE=0 RANK_DELTA_CACHE=0 REBUILD=1 \
  "$ONEESAN_ROOT/scripts/run/b300x8.sh" "$N" "$MOD" 2>&1 | tee "$log"
  line="$(grep '^backend=gridfp-b300-hbm32-fullmate-dropN ' "$log" | tail -n1 || true)"
  [[ -n "$line" ]] || { echo "missing backend result main_ilp2=$mode" >&2; exit 3; }
