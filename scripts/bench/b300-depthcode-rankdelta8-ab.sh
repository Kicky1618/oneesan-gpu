...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_depthcode_rankdelta8_ab_n${N}_${TRANSPOSE_MODE}_${DEPTHCODE_DECODE_LOAD}_${RANKSTREAM_LUT_LOAD}_fused${RANKCHUNK32_FUSED16}_align${RANKDELTA8_ALIGN32}_pm${PM_ACCUM}_t${BUCKET_THREADS}_gx${BUCKET_GRID_X}_gy${BUCKET_GRID_Y}}"
...
for x in RANKCHUNK32_FUSED16 RANKDELTA8_ALIGN32 PM_ACCUM TERNARY_KEY4 RUN_SELFTEST RUN_PTXAS; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0 or 1" >&2; exit 2; }; done
...
if [[ "$RUN_SELFTEST" == 1 ]]; then
  RUN_LAYOUT_PROOF=0 RANKCHUNK32_BYTEPACK=0 RANKCHUNK32_BLOCK64=0 RANKCHUNK32_ALIGN32="$RANKDELTA8_ALIGN32" \
    RANKCHUNK32_FUSED16="$RANKCHUNK32_FUSED16" RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" \
  ...
  RANKDELTA8_ALIGN32="$RANKDELTA8_ALIGN32" RANKCHUNK32_FUSED16="$RANKCHUNK32_FUSED16" \
...
build_one(){
  ...
  if [[ "$mode" == rankchunk32 ]]; then highctx=warpstriped_delta_direct_affine_rankchunk32_cross5
  else highctx=warpstriped_delta_direct_affine_rankdelta8_cross5; fi
  N="$N" OUT="$bin" HIGH_CTX="$highctx" DEPTHCODE_DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" \
    RANKCHUNK32_FUSED16="$RANKCHUNK32_FUSED16" RANKCHUNK32_BYTEPACK=0 RANKCHUNK32_BLOCK64=0 \
    RANKCHUNK32_ALIGN32="$RANKDELTA8_ALIGN32" RANKDELTA8_ALIGN32="$RANKDELTA8_ALIGN32" \
...
for mode in rankchunk32 rankdelta8; do
...
for mode in ('rankchunk32','rankdelta8'):
...
for m in ('wall_s','forward_high_s','reverse_high_s'):
 if q['rankchunk32'][m]!='NA' and q['rankdelta8'][m]!='NA': print(f'rankdelta8_{m}_speedup={float(q["rankchunk32"][m])/float(q["rankdelta8"][m]):.6f}x')
if all(q[x][m]!='NA' for x in ('rankchunk32','rankdelta8') for m in ('forward_high_s','reverse_high_s')):
 a=float(q['rankchunk32']['forward_high_s'])+float(q['rankchunk32']['reverse_high_s']); b=float(q['rankdelta8']['forward_high_s'])+float(q['rankdelta8']['reverse_high_s']); print(f'rankdelta8_total_high_speedup={a/b:.6f}x')
ld=pathlib.Path(logdir)
p=ld/'rankchunk32_r1.err'
if p.exists():
 ...
 if vals: print(f'rankchunk32_rankstream_bytes_total={sum(v[2]*2 for v in vals)}')
...
if p.exists():
 ...
 if vals:
  print(f'rankdelta8_stream_bytes_total={sum(v[3] for v in vals)}'); print(f'rankdelta8_total_table_bytes={sum(v[4] for v in vals)}'); print(f'rankdelta8_max_prefix_runtime={max(v[6] for v in vals)}'); print(f'rankdelta8_max_delta_runtime={max(v[7] for v in vals)}')
if run_ptxas=='1':
...
echo "b300-depthcode-rankdelta8-ab OK n=$N repeats=$REPEATS align32=$RANKDELTA8_ALIGN32 lut=$RANKSTREAM_LUT_LOAD fused16=$RANKCHUNK32_FUSED16 result=$RESULT" >&2
