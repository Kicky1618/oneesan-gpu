...
RECURRENCE_ILP="${RECURRENCE_ILP:-2}"
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_nextgen_latency_h${HIGH_DROP_CHUNK}_i${RECURRENCE_ILP}_c${RANDOM_CG}_d${DUALMASK}_b${CLOSURE_BATCH}_row${ROWS}}"
...
case "$RECURRENCE_ILP" in 2|4|8) ;; *) exit 2;; esac
...
GEN_BIN="$ONEESAN_BUILD_DIR/b300_nextgen_latency_source_n27"
GEN_OUT="$LOGDIR/source.build.out"
GEN_ERR="$LOGDIR/source.build.err"
...
SYNC_SRC="$(sed -nE 's/^  source_after_all=(.*)$/\1/p' "$GEN_OUT" | tail -n1)"
[[ -f "$SYNC_SRC" ]] || { echo 'latency sweep source missing' >&2; exit 3; }
PREF_SRC="$LOGDIR/prefetch.cu"
python3 "$ONEESAN_ROOT/scripts/build/gen-b300-mainrec-prefetch-l2.py" "$SYNC_SRC" "$PREF_SRC" >"$LOGDIR/prefetch.transform.out"
grep -Fq 'mainrec_prefetch_l2=1' "$LOGDIR/prefetch.transform.out"
...
for cap in 0 $REGCAP_LIST; do
  ...
  [[ -z "${seen[$cap]+x}" ]] || continue
  seen[$cap]=1
  for pre in 0 1; do
    if [[ "$pre" == 1 ]]; then cls=prefetch; src="$PREF_SRC"; else cls=sync; src="$SYNC_SRC"; fi
    profile="${cls}_r${cap}"
    bin="$ONEESAN_BUILD_DIR/b300_nextgen_${profile}_h${HIGH_DROP_CHUNK}_i${RECURRENCE_ILP}_c${RANDOM_CG}_d${DUALMASK}_b${CLOSURE_BATCH}_n27"
    err="$LOGDIR/${profile}.build.err"
    reg=(); ((cap>0)) && reg+=("-maxrregcount=$cap")
    echo "=== compile $profile ===" >&2
    set +e
    TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" -Xptxas=-v "${reg[@]}" "${DEFS[@]}" "$src" -o "$bin" >"$LOGDIR/${profile}.compile.out" 2>"$err"
    rc=$?
    set -e
...
# Unified recurrence ILP4/8 changes the body and launch geometry but keeps this
# production kernel name intentionally.
while IFS=$'\t' read -r profile pre cap bin err; do
...
printf 'profile\tprefetch_l2\tcap\tthreads\trepeat\tresidue\twall_s\thigh_rec_groups\thigh_rec_fallback_groups\tmc_avg_pct\tmc_max_pct\tmc_samples\n' >"$RESULT"
run_one(){
  local profile="$1" pre="$2" cap="$3" bin="$4" t="$5" r="$6"
  local so="$LOGDIR/${profile}_t${t}_rep${r}.out" se="$LOGDIR/${profile}_t${t}_rep${r}.err" mem="$LOGDIR/${profile}_t${t}_rep${r}.mem"
  set +e
  B300_ROW_LIMIT="$ROWS" GRIDFP_THREADS="$t" "$bin" 27 "$TARGET_MIB" "$MAX_WINDOW" 8 "$MOD" >"$so" 2>"$se" &
  local pid=$!
  sample_mem "$pid" "$mem" & local mpid=$!
  wait "$pid"; local rc=$?
  set -e
  wait "$mpid" 2>/dev/null || true
  ((rc==0)) || { echo "$profile t=$t failed rc=$rc" >&2; return "$rc"; }
  local line hg hf mcavg mcmax mcn
  ...
  read -r mcavg mcmax mcn < <(awk '{s+=$1;n++;if($1>m)m=$1}END{if(n)printf "%.3f %.3f %d\n",s/n,m,n;else print "nan nan 0"}' "$mem")
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$profile" "$pre" "$cap" "$t" "$r" "$(field residue "$line")" "$(field wall_s "$line")" "$hg" "$hf" "$mcavg" "$mcmax" "$mcn" >>"$RESULT"
}
...
for t in $THREADS_LIST; do
  ...
  while IFS=$'\t' read -r profile pre cap bin err; do
    ...
    for ((r=1; r<=REPEATS; ++r)); do
      echo "=== $profile threads=$t repeat=$r ===" >&2
      run_one "$profile" "$pre" "$cap" "$bin" "$t" "$r"
    done
  done <"$LOGDIR/binaries.tsv"
done
...
rr=list(csv.DictReader(open(sys.argv[2]),delimiter='\t'))
if not rows: raise SystemExit('no latency results')
res={r['residue'] for r in rows}
if len(res)!=1:
    raise SystemExit('FATAL latency partial residue mismatch '+repr({(r['profile'],r['threads']):r['residue'] for r in rows}))
resource={}
for r in rr:
    try:
        cur=(int(r['registers']),int(r['spill_store_bytes']),int(r['spill_load_bytes']))
    except (ValueError,KeyError):
        continue
    old=resource.get(r['profile'],(-1,-1,-1))
    resource[r['profile']]=tuple(max(a,b) for a,b in zip(old,cur))
by={}
for r in rows:
    by.setdefault((r['profile'],int(r['prefetch_l2']),int(r['cap']),int(r['threads'])),[]).append(r)
med=[]
for (p,pre,c,t),rs in by.items():
    ...
    regs,ss,sl=resource.get(p,(-1,-1,-1))
    med.append((wall,p,pre,c,t,mc,regs,ss,sl,samples))
for x in sorted(med):
    print(f'profile={x[1]} prefetch={x[2]} cap={x[3]} threads={x[4]} wall={x[0]:.9f} mc={x[5]:.3f} mc_samples={x[9]} regs={x[6]} spill_store={x[7]} spill_load={x[8]}',file=sys.stderr)
...
print(f'b300_nextgen_latency_best_mc_samples={best[9]}')
...
PY
cat "$RESOURCE"
echo "b300-nextgen-latency-regcap-sweep OK result=$RESULT resources=$RESOURCE sample_interval=$SAMPLE_INTERVAL" >&2
