...
grep -Fq 'packed32=1 shift2_i32=1 cross_i32=1' <<<"$out"
...
grep -Fq 'production_cross_abs_max=1029686463' <<<"$out"
grep -Fq 'production_cross_ll_min=-1029686463' <<<"$out"
grep -Fq 'production_cross_rr_max=821588872' <<<"$out"
grep -Fq 'table_bytes=20880' <<<"$out"
grep -Fq 'known_constant_bytes=41760' <<<"$out"
echo 'b300-closure-contrib-table-proof OK exact=1 packed32=1 shift2=1 cross=1 constant_budget=OK' >&2
