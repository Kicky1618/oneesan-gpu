...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_slotrow32_ab_n${N}_${RANKSTREAM_LUT_LOAD}_${TRANSPOSE_MODE}}"
...
build_one(){
  ...
  N="$N" OUT="$bin" HIGH_CTX=warpstriped_delta_direct_affine_rankformula_cross5 \
    ...
    RANKFORMULA_SLOTMETA=1 RANKFORMULA_SLOTROW32="$fused" \
...
run_one(){
  ...
  residue="$(field residue "$line")"; wall="$(field wall_s "$line")"; [[ "$residue" == "$EXPECT" ]] || { echo "$label residue mismatch got=$residue expected=$EXPECT" >&2; exit 4; }
...
for spec in 'slotmeta2load 0' 'slotrow32 1'; do
  read -r label fused <<<"$spec"
  if [[ "$RUN_SELFTEST" == 1 ]]; then
    if [[ "$fused" == 1 ]]; then runner=pattern10-depthcode-rankformula-slotrow32-selftest.sh; else runner=pattern10-depthcode-rankformula-slotmeta-selftest.sh; fi
    RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" PM_ACCUM="$PM_ACCUM" \
      bash "$ONEESAN_ROOT/scripts/bench/$runner" >"$LOGDIR/$label.selftest.out" 2>"$LOGDIR/$label.selftest.err"
  fi
  bin="$ONEESAN_BUILD_DIR/ab_rankformula_slotrow32_${label}_n${N}"
  build_one "$label" "$fused" "$bin"
...
for mode in ('slotmeta2load','slotrow32'):
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
    if q['slotmeta2load'][k] and q['slotrow32'][k]: print(f'rankformula_slotrow32_{k}_speedup={q["slotmeta2load"][k]/q["slotrow32"][k]:.6f}x')
ld=pathlib.Path(logdir)
p=ld/'slotrow32_r1.err'
if p.exists():
    rows32=[]
    for line in p.read_text(errors='replace').splitlines():
        if not line.startswith('p10dc_low_rankformula_slotrow32 fixed_owner='): continue
        d=dict(re.findall(r'([A-Za-z0-9_]+)=([^ ]+)',line)); rows32.append(d)
    if rows32:
        print(f'slotrow32_bytes_all_gpus={sum(int(x["bytes"]) for x in rows32)}')
        print(f'slotrow32_entries_all_gpus={sum(int(x["entries"]) for x in rows32)}')
        print(f'slotrow32_loads_per_lookup={max(int(x["loads_per_lookup"]) for x in rows32)}')
if run_ptxas=='1':
...
print('slotmeta2load_support_delta_loads=2')
print('slotrow32_support_delta_loads=1')
print('slotrow32_packed_bits=30')
PY

echo "b300-depthcode-rankformula-slotrow32-ab OK n=$N repeats=$REPEATS result=$RESULT" >&2
