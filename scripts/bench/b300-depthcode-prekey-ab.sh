...
REPEATS="${REPEATS:-3}"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_depthcode_prekey_ab_n${N}_${TRANSPOSE_MODE}_${DEPTHCODE_DECODE_LOAD}_pm${PM_ACCUM}_t${BUCKET_THREADS}_gx${BUCKET_GRID_X}_gy${BUCKET_GRID_Y}}"
...
for ctx in warpstriped_delta_direct_affine_cross5 warpstriped_delta_direct_affine_prekey_cross5; do
...
order=('warpstriped_delta_direct_affine_cross5','warpstriped_delta_direct_affine_prekey_cross5'); out=[]
for ctx in order:
 g=[r for r in rows if r['high_ctx']==ctx]; z={'high_ctx':ctx,'repeats':str(len(g))}
 for m in metrics:
...
for m in ('wall_s','forward_high_s','reverse_high_s'):
 if q[b][m]!='NA' and q[o][m]!='NA': print(f'prekey_{m}_speedup={float(q[b][m])/float(q[o][m]):.6f}x')
if all(q[x][m]!='NA' for x in (b,o) for m in ('forward_high_s','reverse_high_s')):
 base=float(q[b]['forward_high_s'])+float(q[b]['reverse_high_s']); opt=float(q[o]['forward_high_s'])+float(q[o]['reverse_high_s']); print(f'prekey_total_high_speedup={base/opt:.6f}x')
print('prekey_low_key_bytes_per_code=4'); print('prekey_cross_runtime_ternary_fold=0'); print(f'summary={dst}')
PY

echo "depthcode-prekey-ab OK n=$N repeats=$REPEATS decode_load=$DEPTHCODE_DECODE_LOAD transpose=$TRANSPOSE_MODE result=$RESULT" >&2
