...
RUN_OWNER_COMPACT="${RUN_OWNER_COMPACT:-1}"
...
RUN_PRIMITIVE_RANK_PACKED="${RUN_PRIMITIVE_RANK_PACKED:-1}"
RUN_PRIMITIVE1_TABLE="${RUN_PRIMITIVE1_TABLE:-1}"
RUN_PRIMITIVE_SYM_U32="${RUN_PRIMITIVE_SYM_U32:-1}"
RUN_CHOOSE_SYM_U32="${RUN_CHOOSE_SYM_U32:-1}"
...
for name in RUN_PREFIX_CARRY RUN_OWNER_LOCAL_CARRY RUN_OWNER_COMPACT RUN_OWNER_TREE RUN_ADJACENT_SUPPORT RUN_PLAN_SHARED RUN_MATERIALIZE_LAST_R RUN_MATERIALIZE_PACKED RUN_PRIMITIVE_RANK_PACKED RUN_PRIMITIVE1_TABLE RUN_PRIMITIVE_SYM_U32 RUN_CHOOSE_SYM_U32 RUN_SUPPORT13_TABLE RUN_TURN_LOCAL_CARRY RUN_TURN_TREE RUN_TURN_NONN; do
  value="${!name}"
...
if [[ "$RUN_PREFIX_CARRY" == 0 && "$RUN_OWNER_LOCAL_CARRY" == 0 &&
      ...
      "$RUN_PRIMITIVE_RANK_PACKED" == 0 && "$RUN_PRIMITIVE1_TABLE" == 0 &&
      "$RUN_PRIMITIVE_SYM_U32" == 0 && "$RUN_CHOOSE_SYM_U32" == 0 &&
...
if [[ "$RUN_OWNER_COMPACT" == 1 ]]; then
  run_case owner_local_sector_compact_table \
    gridfp-runtime-owner-local-sector-compact-table-microprobe.sh
fi
...
if [[ "$RUN_PLAN_SHARED" == 1 ]]; then
  run_case owner_plan_shared_cache \
    gridfp-runtime-owner-plan-shared-cache-microprobe.sh
fi
...
if [[ "$RUN_PRIMITIVE_RANK_PACKED" == 1 ]]; then
  run_case primitive_rank_packed \
    gridfp-runtime-primitive-rank-packed-microprobe.sh
fi
if [[ "$RUN_PRIMITIVE1_TABLE" == 1 ]]; then
  run_case primitive1_table_layout \
    gridfp-primitive1-u32-table-microprobe.sh
fi
if [[ "$RUN_PRIMITIVE_SYM_U32" == 1 ]]; then
  run_case primitive_sym_u32 \
    gridfp-primitive-sym-u32-table-microprobe.sh
fi
if [[ "$RUN_CHOOSE_SYM_U32" == 1 ]]; then
  run_case choose_sym_u32 \
    gridfp-choose-sym-u32-table-microprobe.sh
fi
