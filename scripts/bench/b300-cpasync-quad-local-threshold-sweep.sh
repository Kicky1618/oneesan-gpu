...
THREADS="${THREADS:-256}"; GX="${GX:-32}"; GY="${GY:-8}"; SPARSE64="${SPARSE64:-1}"; SORTED="${SORTED:-1}"
THRESHOLDS="${THRESHOLDS:-0 1 2 4 6 8}"; SAMPLE_INTERVAL="${SAMPLE_INTERVAL:-0.25}"
if [[ -z "${EXPECT+x}" ]]; then
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_quad_local_threshold_n${N}}"
LOGDIR="${LOGDIR:-${PREFIX}_logs}"; RESULT="${RESULT:-${PREFIX}.tsv}"; RESOURCE="${RESOURCE:-${PREFIX}_ptxas.tsv}"
mkdir -p "$LOGDIR" "$(dirname "$RESULT")"
printf 'threshold\trepeat\tresidue\twall_s\tforward_high_s\treverse_high_s\thigh_s\tavg_gpu_util_pct\tavg_memctrl_util_pct\tmax_memctrl_util_pct\n' >"$RESULT"
...
for th in $THRESHOLDS; do
  [[ "$th" =~ ^[0-9]+$ ]] && (( th <= 8 )) || { echo "bad threshold $th" >&2; exit 2; }
  mode="qld${th}"
  bin="$ONEESAN_BUILD_DIR/b300_quad_local_${mode}_n${N}"
  N="$N" ARCH="$ARCH" OUT="$bin" COL_ILP=4 PM_ACCUM=1 \
    DEPTHMAJOR=1 PAIR_MLP=1 QUAD_MLP=1 QUAD_LOCAL_DIRECT_MAX="$th" MLP_WINDOW4=1 \
    ...
    FORCE7=0 PREFETCH_NEXT=0 PRECTX_FORWARD=0 PRECTX_REVERSE=0 HIGH_PLAN_PROFILE=0 PTXAS_VERBOSE=1 \
...
PY
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
      "$th" "$r" "$residue" "$wall" "$fh" "$rh" "$high" "$ag" "$am" "$mm" >>"$RESULT"
  done
done
...
for r in rows: by.setdefault(int(r['threshold']),[]).append(r)
res={k:{x['residue'] for x in v} for k,v in by.items()}
if any(len(v)!=1 for v in res.values()) or len({next(iter(v)) for v in res.values()})!=1: raise SystemExit(f'RESIDUE MISMATCH {res}')
out=[]
for th,g in by.items():
    ...
    out.append((h,w,th,mc)); print(f'threshold={th}',f'high_s={h:.6f}',f'wall_s={w:.6f}',f'mc_avg_pct={mc:.3f}')
best=min(out)
print('BEST_THRESHOLD',best[2],f'high_s={best[0]:.6f}',f'wall_s={best[1]:.6f}',f'mc_avg_pct={best[3]:.3f}')
PY

echo "b300-cpasync-quad-local-threshold-sweep OK result=$RESULT resources=$RESOURCE" >&2
