...
PROFILE_IN="${PROFILE_IN:-$ONEESAN_ROOT/work/b300_hbm_profile_warpcoop21.env}"
PROFILE_OUT="${PROFILE_OUT:-$ONEESAN_ROOT/work/b300_hbm_profile_warpcoop_auto21.env}"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_hbm_profile_orbit_warpcoop_auto21}"
...
AUTO_RESULT="${AUTO_RESULT:-${PREFIX}_auto.tsv}"; AUTO_SUMMARY="${AUTO_SUMMARY:-${PREFIX}_auto_summary.tsv}"; SUMMARY="${SUMMARY:-${PREFIX}_summary.tsv}"
...
CURRENT_BIN="$ONEESAN_BUILD_DIR/b300_orbit_warpcoop_auto_current_n21"
N=21 ARCH="$ARCH" OUT="$CURRENT_BIN" PM_ACCUM="$PM_ACCUM" DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$CUR_SPARSE64" DIRECTGATHER_SORT_RANKS=0 \
  ...
  PRECTX_FORWARD="$CUR_PRE_F" PRECTX_REVERSE="$CUR_PRE_R" PRECTX_COMPACT="$CUR_PRE_C" PRECTX_FLAT_BID="$CUR_BID" PRECTX_FLAT_BID_FUSED="$CUR_BF" PRECTX_WARPCOOP="$CUR_WC" PTXAS_VERBOSE=1 \
...
# Always remove pool overrides here: this stage exists specifically to let the
# serial and warpcoop kernels derive their own forward/reverse resident pools.
env -u BUCKET_ORBITCTA_FLAT_BLOCKS -u BUCKET_ORBITCTA_FLAT_BLOCKS_PER_SM \
  N=21 ARCH="$ARCH" TARGET_MIB="$TARGET_MIB" MAX_WINDOW="$MAX_WINDOW" REPEATS="$REPEATS" \
  ...
  PREFIX="${PREFIX}_auto" RESULT="$AUTO_RESULT" SUMMARY="$AUTO_SUMMARY" \
  bash "$ONEESAN_ROOT/scripts/bench/b300-orbitcta-flat-prectx-warpcoop-ab.sh"

python3 - "$CURRENT_RESULT" "$AUTO_RESULT" "$PROFILE_IN" "$PROFILE_OUT" "$SUMMARY" "$Q_CHUNK" <<'PY'
import csv,statistics,sys,re
cur,ab,pin,pout,summary,qchunk=sys.argv[1:]
def med(path, selector=None):
 ...
 return {'wall_s':statistics.median(float(r['wall_s']) for r in rows),'high_s':statistics.median(float(r['high_s']) for r in rows)}
current=med(cur); serial=med(ab,'serial'); coop=med(ab,'warpcoop')
candidates=[('current',current),('quad_auto_serial',serial),('quad_auto_warpcoop',coop)]
...
if best_name!='current':
 kv.update({'ORBITCTA_FLAT':'1','ORBITCTA_FLAT_CHUNK':qchunk,'ORBITCTA_FLAT_BLOCKS_PER_SM':'0',
            ...
            'ORBIT_PRECTX_WARPCOOP':'1' if best_name=='quad_auto_warpcoop' else '0',
            'ORBIT_QUAD_MLP':'1','ORBIT_QUAD_OVERLAP_LOCAL':'1','ORBIT_QUAD_LOCAL_DIRECT_MAX':'0','ORBIT_QUAD_SPARSE_DESC_MLP':'0'})
kv['ORBIT_WARPCOOP_AUTO_PROFILE']=best_name
if 'ORBIT_WARPCOOP_AUTO_PROFILE' not in order: order.append('ORBIT_WARPCOOP_AUTO_PROFILE')
if best_name!='current':
 root=re.sub(r'_wca_.*$','',kv.get('ORBIT_PROFILE','orbit'))
 kv['ORBIT_PROFILE']=root+'_wca_'+best_name
with open(pout,'w') as f:
 f.write('# generated by b300-hbm-profile-refine-orbit-warpcoop-auto21.sh\n')
 for k in order:
  v=kv[k]
...
for name,z in sorted(candidates,key=lambda x:x[1]['wall_s']): print('WARPCOOP_AUTO',name,f'wall_s={z["wall_s"]:.6f}',f'high_s={z["high_s"]:.6f}')
print('BEST_ORBIT_WARPCOOP_AUTO='+best_name,f'wall_s={best["wall_s"]:.6f}',f'profile_file={pout}',f'summary={summary}')
PY
cat "$SUMMARY"
cat "$PROFILE_OUT"
echo "orbit warpcoop occupancy refine OK input=$PROFILE_IN output=$PROFILE_OUT quad_sidecar=$QUAD_WINNER_ENV summary=$SUMMARY" >&2
