...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 || BLOCKS < 1 || REPEATS < 1 || WARMUP < 0 )); then echo "reverse p1 main step A/B supports even W=8..10" >&2; exit 2; fi
if [[ "$MOD" != 4294967291 ]]; then echo "reverse p1 main step A/B requires MOD=4294967291" >&2; exit 2; fi
...
printf 'reverse_p1_main_step\trepeat\twall_ms\n' >"$RESULT"
...
for m in ('0','1'):
 xs=[float(r['wall_ms']) for r in rows if r['reverse_p1_main_step']==m]; out.append({'reverse_p1_main_step':m,'repeats':len(xs),'wall_ms_median':f'{statistics.median(xs):.9f}','wall_ms_min':f'{min(xs):.9f}','wall_ms_max':f'{max(xs):.9f}'})
with open(dst,'w',newline='') as f:
 w=csv.DictWriter(f,fieldnames=out[0].keys(),delimiter='\t'); w.writeheader(); w.writerows(out)
q={r['reverse_p1_main_step']:float(r['wall_ms_median']) for r in out}; print(f'runtime_reverse_p1_main_step_wall_speedup={q["0"]/q["1"]:.6f}x'); print('runtime_reverse_p1_main_step_include_result_dispatch=0'); print('runtime_reverse_p1_main_step_generic_project_reverse_calls=0'); print('runtime_reverse_p1_main_step_projection=direct_q2'); print('runtime_reverse_p1_main_step_extra_shared_bytes=0'); print(f'summary={dst}')
PY
if [[ "$PTXAS_VERBOSE" == 1 ]]; then for fast in 0 1; do echo "--- ptxas reverse_p1_main=$fast ---" >&2; grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/fast${fast}.build.err" >&2 || true; done; fi
echo "gridfp-reduced-runtime-reverse-p1-main-step-ab OK W=$W ngpu=$NGPU blocks=$BLOCKS repeats=$REPEATS result=$RESULT" >&2
