...
REPEATS="${REPEATS:-3}"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_depthcode_affine_row_ab_n${N}_${TRANSPOSE_MODE}_${DEPTHCODE_DECODE_LOAD}_pm${PM_ACCUM}_t${BUCKET_THREADS}_gx${BUCKET_GRID_X}_gy${BUCKET_GRID_Y}}"
...
for ctx in warpstriped_delta_direct_cross5 warpstriped_delta_direct_affine_cross5; do
...
order=('warpstriped_delta_direct_cross5','warpstriped_delta_direct_affine_cross5')
out=[]
for ctx in order:
...
for m in ('wall_s','forward_high_s','reverse_high_s'):
    if q[base][m]!='NA' and q[opt][m]!='NA':
        print(f'affine_row_{m}_speedup={float(q[base][m])/float(q[opt][m]):.6f}x')
