...
PREFETCH_VALUES="${PREFETCH_VALUES:-0 64 128 256}"
...
[[ "$ORBITCTA_FLAT" == 1 ]] && (( ORBITCTA_FLAT_CHUNK > 1 )) || { echo 'prefetch refine requires chunked flat orbit CTA' >&2; exit 2; }
[[ "$ORBITCTA_FLAT_BLOCKS_PER_SM" =~ ^[0-9]+$ ]] || { echo 'bad ORBITCTA_FLAT_BLOCKS_PER_SM' >&2; exit 2; }
[[ "$ORBIT_QUAD_LOCAL_DIRECT_MAX" == 0 ]] || { echo 'prefetch refine expects QOL direct local path' >&2; exit 2; }
[[ "$ORBIT_QUAD_SPARSE_DESC_MLP" == 0 || "$ORBIT_SPARSE64" == 1 ]] || { echo 'qsd1 requires sparse64' >&2; exit 2; }
if [[ "$ORBIT_PRECTX_WARPCOOP" == 1 ]]; then
  [[ "$ORBIT_PRECTX_COMPACT" == 1 && "$ORBIT_PRECTX_FORWARD" == 1 && "$ORBIT_PRECTX_REVERSE" == 1 ]] || { echo 'warpcoop prefetch refine requires compact forward+reverse prectx' >&2; exit 2; }
fi
for pf in $PREFETCH_VALUES; do case "$pf" in 0|64|128|256) ;; *) echo "bad PREFETCH_VALUES entry=$pf" >&2; exit 2;; esac; done
...
printf 'prefetch_bytes\trepeat\tresidue\twall_s\tforward_high_s\treverse_high_s\thigh_s\tavg_gpu_util_pct\tavg_memctrl_util_pct\tmax_memctrl_util_pct\n' >"$RESULT"
...
for pf in $PREFETCH_VALUES; do
  ...
  for ((r=1;r<=REPEATS;++r)); do
    ...
    (( rc == 0 )) || { echo "prefetch=$pf repeat=$r failed rc=$rc" >&2; exit "$rc"; }
    line="$(grep '^residue=' "$so" | tail -n1 || true)"; [[ -n "$line" ]] || { echo "prefetch=$pf missing residue" >&2; exit 3; }
    residue="$(field residue "$line")"; [[ "$residue" == "$EXPECT" ]] || { echo "prefetch=$pf residue=$residue expected=$EXPECT" >&2; exit 4; }
    ...
    [[ -n "$fh" && -n "$rh" ]] || { echo "prefetch=$pf missing HIGH timing" >&2; exit 7; }
...
PY
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$pf" "$r" "$residue" "$(field wall_s "$line")" "$fh" "$rh" "$high" "$ag" "$am" "$mm" >>"$RESULT"
  done
done
...
for r in rows: by.setdefault(int(r['prefetch_bytes']),[]).append(r)
if not by: raise SystemExit('no prefetch rows')
...
for k in sorted(stats): print(f'PREFETCH_{k}',f"wall_s={stats[k]['wall']:.6f}",f"high_s={stats[k]['high']:.6f}",f"mc_avg_pct={stats[k]['mc']:.3f}")
