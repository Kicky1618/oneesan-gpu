...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_pipe2_producer_quad_ab_n${N}_t${THREADS}_b${BATCH}}"
...
(( THREADS >= 64 && THREADS % 32 == 0 )) || { echo 'producer-quad A/B requires BUCKET_THREADS>=64 and multiple of 32' >&2; exit 2; }
...
for x in SPARSE64 PM_ACCUM CPASYNC_PAIR PRECTX_FORWARD PRECTX_REVERSE PRECTX_COMPACT; do
  v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0/1" >&2; exit 2; }
done
[[ "$CPASYNC_PAIR" == 1 ]] || { echo 'producer-quad A/B fixes CPASYNC_PAIR=1' >&2; exit 2; }
...
PIPE2_PRODUCER_PATCH_ONLY=1 QUAD_MLP=1 ORBITCTA_COL_ILP=4 \
  ORBITCTA_FLAT=1 ORBITCTA_FLAT_DYNAMIC=1 ORBITCTA_FLAT_DYNAMIC_PIPE2=1 \
  ORBITCTA_FLAT_CHUNK=1 ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP=0 \
  bash "$ONEESAN_ROOT/scripts/build/b300-directgather-orbitcta-pipe2-producer-warp.sh" >"$LOGDIR/producer-quad-patch.out"
grep -q 'quad_mlp=1 col_ilp=4' "$LOGDIR/producer-quad-patch.out" || { echo 'producer quad patch marker missing' >&2; exit 5; }
...
COMMON=(N="$N" ARCH="$ARCH" PM_ACCUM="$PM_ACCUM" DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$SPARSE64" DIRECTGATHER_SORT_RANKS=0
  ...
  QUAD_OVERLAP_LOCAL=0 QUAD_LOCAL_DIRECT_MAX=0 QUAD_SPARSE_DESC_MLP=0 QUAD_OVERLAP_BYPASS_LOCAL0=0
...
build_one(){
  local quad="$1" name="$2" bin="$ONEESAN_BUILD_DIR/b300_pipe2_producer_${name}_t${THREADS}_b${BATCH}_n${N}"
  env "${COMMON[@]}" QUAD_MLP="$quad" OUT="$bin" \
  ...
  grep -q "quad_mlp=$quad" "$LOGDIR/${name}.build.err" || { echo "$name quad marker mismatch" >&2; exit 6; }
  python3 "$PARSER" "$LOGDIR/${name}.build.err" --label "$name" >>"$RESOURCE" || true
  printf '%s' "$bin"
}

pair_bin="$(build_one 0 pair)"
quad_bin="$(build_one 1 quad)"
...
for ((r=1;r<=REPEATS;++r)); do run_one pair "$pair_bin" "$r"; done
for ((r=1;r<=REPEATS;++r)); do run_one quad "$quad_bin" "$r"; done
...
print(f'producer_quad_wall_speedup={m("pair","wall_s")/m("quad","wall_s"):.6f}x')
print(f'producer_quad_high_speedup={m("pair","high_s")/m("quad","high_s"):.6f}x')
print(f'producer_quad_memctrl_delta={m("quad","avg_memctrl_pct")-m("pair","avg_memctrl_pct"):.6f}pp')
PY

echo "pipe2 producer pair-vs-quad A/B OK result=$RESULT resources=$RESOURCE" >&2
