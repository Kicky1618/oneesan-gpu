...
run_one(){local m="$1" r="$2" out="$LOGDIR/${m}_${r}.out" err="$LOGDIR/${m}_${r}.err";B300_ROW_LIMIT="$ROWS" GRIDFP_THREADS="$GRIDFP_THREADS" GRIDFP_VRAM_RESERVE_MIB="${GRIDFP_VRAM_RESERVE_MIB:-8192}" "${bin[$m]}" "$N" "$MOD" "$TARGET_MIB" "$MAX_WINDOW" "$NGPU" >"$out" 2>"$err";local line;line="$(grep '^backend=gridfp-b300-hbm32-fullmate-dropN ' "$out"|tail -n1)";[[ -n "$line" ]]||{ tail -n100 "$err" >&2;exit 3;};f(){sed -nE "s/.* $1=([^[:space:]]+).*/\\1/p"<<<"$line";};printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$m" "$r" "$(f residue)" "$(f wall_s)" "$(f active_max_s)" "$(f active_sum_s)" "$(f prepare_s)" >>"$RESULT";}
