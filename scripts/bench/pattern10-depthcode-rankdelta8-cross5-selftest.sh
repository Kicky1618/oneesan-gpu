...
for x in PM_ACCUM RANKCHUNK32_FUSED16 RANKDELTA8_FUSED13 RANKDELTA8_ALIGN32; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0 or 1" >&2; exit 2; }; done
...
BIN="${BIN:-$ONEESAN_BUILD_DIR/pattern10_depthcode_rankdelta8_cross5_selftest_w${W}_pm${PM_ACCUM}_${DECODE_LOAD}_ranklut${RANKSTREAM_LUT_LOAD}_fused13${RANKDELTA8_FUSED13}_align${RANKDELTA8_ALIGN32}}"
mkdir -p "$(dirname "$BIN")"
...
if grep -Fq "OK W=$W" <<<"$out"; then
  ...
  grep -Fq "fused13=$RANKDELTA8_FUSED13" <<<"$out"
  grep -Fq 'cross_runtime_div=0 cross_runtime_mod=0' <<<"$out"
fi
echo "pattern10-depthcode-rankdelta8-cross5-selftest OK W=$W pm=$PM_ACCUM lut=$RANKSTREAM_LUT_LOAD fused13=$RANKDELTA8_FUSED13 align32=$RANKDELTA8_ALIGN32" >&2
