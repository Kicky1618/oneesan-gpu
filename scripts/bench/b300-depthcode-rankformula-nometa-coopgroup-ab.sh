...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_nometa_coopgroup_ab_n${N}_${TRANSPOSE_MODE}}"
...
build_one(){
  local coop="$1" label="$2" bin="$3"
  N="$N" ARCH="$ARCH" OUT="$bin" RANKFORMULA_NOMETA_BLOCK=8 RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP="$coop" \
...
for spec in 'warpshare 0' 'coopgroup 1'; do
  read -r label coop <<<"$spec"
  if [[ "$RUN_SELFTEST" == 1 ]]; then
    RANKFORMULA_NOMETA_BLOCK=8 RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP="$coop" \
  ...
  bin="$ONEESAN_BUILD_DIR/ab_rankformula_nometa_b8_${label}_n${N}"
  build_one "$coop" "$label" "$bin"
...
for mode in ('warpshare','coopgroup'):
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
    if q['warpshare'][k] and q['coopgroup'][k]: print(f'coopgroup_{k}_speedup={q["warpshare"][k]/q["coopgroup"][k]:.6f}x')
if run_ptxas=='1':
...
echo "b300-depthcode-rankformula-nometa-coopgroup-ab OK n=$N repeats=$REPEATS block=8 result=$RESULT" >&2
