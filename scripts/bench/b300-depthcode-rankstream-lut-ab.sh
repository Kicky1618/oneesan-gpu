...
REPEATS="${REPEATS:-5}"; RUN_SELFTEST="${RUN_SELFTEST:-1}"; RANK_BACKEND="${RANK_BACKEND:-rankstream}"

case "$RANK_BACKEND" in
  rankstream) CTX=warpstriped_delta_direct_affine_prekey_rankstream_cross5; SELFTEST="$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankstream-cross5-selftest.sh" ;;
  rankchunk32) CTX=warpstriped_delta_direct_affine_rankchunk32_cross5; SELFTEST="$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankchunk32-cross5-selftest.sh" ;;
...
if [[ "$RANK_BACKEND" == rankchunk32 ]]; then bash "$ONEESAN_ROOT/scripts/bench/rankchunk32-warpbase-proof.sh"; fi
...
build_one(){ local mode="$1" bin="$2"; N="$N" OUT="$bin" HIGH_CTX="$CTX" DEPTHCODE_DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" RANKSTREAM_LUT_LOAD="$mode" TRANSPOSE_MODE="$TRANSPOSE_MODE" PM_ACCUM="$PM_ACCUM" TERNARY_KEY4="$TERNARY_KEY4" bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-depthcode-graph-batch.sh" >"$LOGDIR/${mode}.build.out" 2>"$LOGDIR/${mode}.build.err"; }
...
for mode in "${MODES[@]}"; do
  ...
  echo "=== build $RANK_BACKEND LUT $mode ===" >&2; build_one "$mode" "$bin"
  for ((r=1;r<=REPEATS;++r)); do echo "=== run $RANK_BACKEND LUT $mode $r/$REPEATS ===" >&2; run_one "$mode" "$bin" "$r"; done
done
cat "$RESULT"
...
metrics=('wall_s','forward_high_s','reverse_high_s','forward_low_s','reverse_low_s','transpose_s'); modes=('constant','ldg','ldg256'); out=[]
for mode in modes:
    group=[r for r in rows if r['lut_load']==mode]; z={'lut_load':mode,'repeats':str(len(group))}
    for metric in metrics:
        xs=[float(r[metric]) for r in group if r[metric]!='NA']; z[metric]=f'{statistics.median(xs):.9f}' if xs else 'NA'
    out.append(z)
with open(dst,'w',newline='') as f:
    w=csv.DictWriter(f,fieldnames=('lut_load','repeats',*metrics),delimiter='\t'); w.writeheader(); w.writerows(out)
q={r['lut_load']:r for r in out}; prefix='rankstream_lut' if backend=='rankstream' else 'rankchunk32_lut'
def speedup(base,opt,metric,suffix):
    if q[base][metric]!='NA' and q[opt][metric]!='NA': print(f'{prefix}_{suffix}_{metric}_speedup={float(q[base][metric])/float(q[opt][metric]):.6f}x')
for metric in ('wall_s','forward_high_s','reverse_high_s'):
    speedup('constant','ldg',metric,'ldg'); speedup('ldg','ldg256',metric,'ldg256_vs_ldg'); speedup('constant','ldg256',metric,'ldg256_vs_constant')
for base,opt,suffix in (('constant','ldg','ldg'),('ldg','ldg256','ldg256_vs_ldg'),('constant','ldg256','ldg256_vs_constant')):
    if all(q[x][m]!='NA' for x in (base,opt) for m in ('forward_high_s','reverse_high_s')):
        b=float(q[base]['forward_high_s'])+float(q[base]['reverse_high_s']); o=float(q[opt]['forward_high_s'])+float(q[opt]['reverse_high_s']); print(f'{prefix}_{suffix}_total_high_speedup={b/o:.6f}x')
