...
for x in SPARSE64 PAIR_MLP WINDOW4 PM_ACCUM CPASYNC_PAIR; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0/1" >&2; exit 2; }; done
...
with open(sys.argv[2],'w') as f:
 ...
 f.write(f'PRECTX_FUSED_WALL_S={z[w]["wall_s"]:.9f}\nPRECTX_FUSED_HIGH_S={z[w]["high_s"]:.9f}\n')
PY
cat "$RESULT"
