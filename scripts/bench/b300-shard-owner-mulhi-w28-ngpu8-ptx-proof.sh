...
extract gridfp_b300_shard_owner_u32shift_mask_probe "$OUTDIR/u32shift_mask.body.ptx"

metric(){ local re="$1" file="$2"; grep -Ec "$re" "$file" || true; }
for mode in branchy mulhi_mul mulhi_table mulhi_mask u32limb_mask u32shift_mask; do
  ...
  if [[ "$mode" == u32limb_mask || "$mode" == u32shift_mask ]]; then
    if (( mulhi64 != 0 || mullo64 != 0 || mulwide64 != 0 || ldconst != 0 )); then
      echo "$mode unexpectedly uses u64 multiply or constant-table load" >&2
      grep -En '\bmul\.(hi|lo|wide)\.u64\b|\bld\.const\..*\.u64\b' "$body" >&2 || true
      exit 7
    fi
  fi
  if [[ "$mode" == u32limb_mask ]] && (( mulhi32 < 3 )); then
    echo "u32limb_mask missing expected mul.hi.u32 limbs" >&2
    exit 8
  fi
  if [[ "$mode" == u32shift_mask ]]; then
    if (( mulhi32 > 2 )); then
      echo "u32shift_mask failed to reduce mul.hi.u32 count to <=2" >&2
      exit 9
    fi
    if (( mullo32 > 1 )); then
      echo "u32shift_mask failed to reduce explicit low-u32 multiply count to <=1" >&2
      exit 10
    fi
  fi
  printf '%s_divrem_u64=%s\n%s_mulhi_u64=%s\n%s_mullo_u64=%s\n%s_mulwide_u64=%s\n%s_mulhi_u32=%s\n%s_mullo_u32=%s\n%s_mulwide_u32_total=%s\n%s_setp_u32=%s\n%s_setp_u64=%s\n%s_sub_u64=%s\n%s_bra=%s\n%s_selp=%s\n%s_ldconst_u64=%s\n' \
    "$mode" "$div" "$mode" "$mulhi64" "$mode" "$mullo64" "$mode" "$mulwide64" \
    "$mode" "$mulhi32" "$mode" "$mullo32" "$mode" "$mulwide32" "$mode" "$setp32" "$mode" "$setp64" \
    "$mode" "$sub64" "$mode" "$bra" "$mode" "$selp" "$mode" "$ldconst"
done

echo "gridfp-b300-shard-owner-mulhi-w28-ngpu8-ptx-proof OK arch=$ARCH main_magic=195888106327 main_high_shift=9 block_magic=139905900989 block_high_shift=7 masked_base_mul64=0 masked_base_table=0 u32limb_device_mul64=0 u32limb_umulhi32=3 u32shift_device_mul64=0 u32shift_umulhi32_max=2 u32shift_mullo32_max=1 exact_proof=1"