...
RANKCHUNK32_ONESHFL="${RANKCHUNK32_ONESHFL:-1}"; RANKCHUNK32_FUSED16="${RANKCHUNK32_FUSED16:-0}"; RANKCHUNK32_ALIGN32="${RANKCHUNK32_ALIGN32:-0}"; PM_ACCUM="${PM_ACCUM:-0}"; TERNARY_KEY4="${TERNARY_KEY4:-1}"
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_depthcode_rankchunk32_block64_ab_n${N}_${TRANSPOSE_MODE}_${DEPTHCODE_DECODE_LOAD}_${RANKSTREAM_LUT_LOAD}_align${RANKCHUNK32_ALIGN32}_fused${RANKCHUNK32_FUSED16}_pm${PM_ACCUM}_t${BUCKET_THREADS}_gx${BUCKET_GRID_X}_gy${BUCKET_GRID_Y}}"
...
for x in RANKCHUNK32_ONESHFL RANKCHUNK32_FUSED16 RANKCHUNK32_ALIGN32 PM_ACCUM TERNARY_KEY4 RUN_SELFTEST RUN_PTXAS; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0 or 1" >&2; exit 2; }; done
...
if [[ "$RUN_SELFTEST" == 1 ]]; then
  for b64 in 0 1; do
    RUN_LAYOUT_PROOF=0 RANKCHUNK32_BLOCK64="$b64" RANKCHUNK32_BYTEPACK=0 RANKCHUNK32_ALIGN32="$RANKCHUNK32_ALIGN32" \
...
for b64 in 0 1; do bin="$ONEESAN_BUILD_DIR/ab_depthcode_rankchunk32_block64${b64}_align${RANKCHUNK32_ALIGN32}_${RANKSTREAM_LUT_LOAD}_${DEPTHCODE_DECODE_LOAD}_${TRANSPOSE_MODE}_n${N}"; echo "=== build rankchunk32 block64=$b64 align=$RANKCHUNK32_ALIGN32 ===" >&2; build_one "$b64" "$bin"; if [[ "$RUN_PTXAS" == 1 ]]; then python3 "$PARSER" "$LOGDIR/block64${b64}.build.err" --label "block64${b64}" >>"$RESOURCE"; fi; for ((r=1;r<=REPEATS;++r)); do echo "=== run rankchunk32 block64=$b64 align=$RANKCHUNK32_ALIGN32 $r/$REPEATS ===" >&2; run_one "$b64" "$bin" "$r"; done; done
cat "$RESULT"
python3 - "$RESULT" "$SUMMARY" "$RESOURCE" "$RUN_PTXAS" "$LOGDIR" "$RANKCHUNK32_ALIGN32" <<'PY'
import csv,re,statistics,sys,pathlib
src,dst,resource,run_ptxas,logdir,aligned=sys.argv[1:]; rows=list(csv.DictReader(open(src),delimiter='\t')); metrics=('wall_s','forward_high_s','reverse_high_s','forward_low_s','reverse_low_s','transpose_s'); out=[]
for mode in ('0','1'):
...
for mode in ('0','1'):
 ...
 if p.exists():
  ...
  if vals:
   print(f'rankchunk32_block64{mode}_runtime_metadata_mib_total={sum(v[3] for v in vals)/(1<<20):.6f}'); print(f'rankchunk32_block64{mode}_blocks_total={sum(v[1] for v in vals)}'); print(f'rankchunk32_block64{mode}_padding_entries_total={sum(v[5] for v in vals)}')
print('rankchunk32_block32_max_prefix=217'); print('rankchunk32_block64_max_prefix=441'); print('rankchunk32_block64_prefix9_exact=1'); print('rankchunk32_block64_block_table_reduction_target=2x')
if aligned=='1':
 print('rankchunk32_block32_height_align=32'); print('rankchunk32_block64_height_align=64'); print('rankchunk32_block_base_loads_per_warp_max=1')
else:
 print('rankchunk32_block32_fullwarp_crossing_alignments=31/32'); print('rankchunk32_block64_fullwarp_crossing_alignments=31/64'); print('rankchunk32_block_base_loads_per_warp_max=2')
print(f'summary={dst}')
PY
echo "b300-depthcode-rankchunk32-block64-ab OK n=$N repeats=$REPEATS lut=$RANKSTREAM_LUT_LOAD decode_load=$DEPTHCODE_DECODE_LOAD align=$RANKCHUNK32_ALIGN32 fused16=$RANKCHUNK32_FUSED16 transpose=$TRANSPOSE_MODE result=$RESULT resource=$RESOURCE" >&2