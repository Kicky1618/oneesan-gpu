...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 || BLOCKS < 1 || REPEATS < 1 || WARMUP < 0 )); then echo "direct low turn compression A/B supports even W=8..10" >&2; exit 2; fi
if [[ "$MOD" != 4294967291 ]]; then echo "direct low turn compression A/B requires MOD=4294967291" >&2; exit 2; fi
...
printf 'turn_direct_low_compress_step\trepeat\twall_ms\n' >"$RESULT"
...
for m in ('0','1'):
 xs=[float(r['wall_ms']) for r in rows if r['turn_direct_low_compress_step']==m]; out.append({'turn_direct_low_compress_step':m,'repeats':len(xs),'wall_ms_median':f'{statistics.median(xs):.9f}','wall_ms_min':f'{min(xs):.9f}','wall_ms_max':f'{max(xs):.9f}'})
with open(dst,'w',newline='') as f:
 w=csv.DictWriter(f,fieldnames=out[0].keys(),delimiter='\t'); w.writeheader(); w.writerows(out)
q={r['turn_direct_low_compress_step']:float(r['wall_ms_median']) for r in out}; print(f'runtime_turn_direct_low_compress_step_wall_speedup={q["0"]/q["1"]:.6f}x'); print('runtime_turn_direct_low_compress_step_generic_include_calls=0'); print('runtime_turn_direct_low_compress_step_local=NN_NR_RN'); print('runtime_turn_direct_low_compress_step_rr=closure_match_right'); print('runtime_turn_direct_low_compress_step_extra_shared_bytes=0'); print(f'summary={dst}')
PY
if [[ "$PTXAS_VERBOSE" == 1 ]]; then for fast in 0 1; do echo "--- ptxas direct_low=$fast ---" >&2; grep -E 'Used .* registers|bytes smem|bytes cmem' "$LOGDIR/fast${fast}.build.err" >&2 || true; done; fi
echo "gridfp-reduced-runtime-turn-direct-low-compress-step-ab OK W=$W ngpu=$NGPU blocks=$BLOCKS repeats=$REPEATS result=$RESULT" >&2
