...
case "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WORKER_WEIGHT" in 0|1|2|3|4) ;; *) echo 'producer worker weight must be 0..4' >&2; exit 2;; esac
...
PY
cat "$PROFILE_OUT"
echo "dynamic producer prectx refine OK input=$PROFILE_IN output=$PROFILE_OUT result=$RESULT worker_weight=$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WORKER_WEIGHT cached_bid=$ORBIT_PRECTX_FLAT_BID pool_per_sm=$ORBITCTA_FLAT_BLOCKS_PER_SM" >&2
