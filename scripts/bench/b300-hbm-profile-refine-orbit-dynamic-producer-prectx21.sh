...
[[ "$ORBIT_PRECTX_FORWARD" == 1 && "$ORBIT_PRECTX_REVERSE" == 1 && "$ORBIT_PRECTX_COMPACT" == 1 && "$ORBIT_PRECTX_FLAT_BID" == 0 ]] || { echo 'producer prectx exact runner requires compact forward+reverse prectx and flat-bid=0' >&2; exit 2; }
...
PY
cat "$PROFILE_OUT"
echo "dynamic producer prectx refine OK input=$PROFILE_IN output=$PROFILE_OUT result=$RESULT" >&2
