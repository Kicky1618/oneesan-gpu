...
TARGET_MIB="${TARGET_MIB:-16384}"; MAX_WINDOW="${MAX_WINDOW:-14}"; TRANSPOSE_MODE="${TRANSPOSE_MODE:-pipeline}"; PM_ACCUM="${PM_ACCUM:-0}"; TERNARY_KEY4="${TERNARY_KEY4:-1}"; DEPTH4_HIGH_CTX="${DEPTH4_HIGH_CTX:-resolved}"
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_pattern10_depth4_depthcode_graph_ab_n${N}_${TRANSPOSE_MODE}_pm${PM_ACCUM}_key4${TERNARY_KEY4}}"
...
run_one(){
  ...
  meta="$(grep -E 'bucket_(forward|reverse)_pattern10_depth4|pattern10_depthcode direct_build=' "$se" | tr '\n' ';' || true)"
...
run_one depth4_lut scripts/build/b300-bucket-snake-pattern10-depth4-graph-batch.sh "$ONEESAN_BUILD_DIR/ab_depth4_lut_${DEPTH4_HIGH_CTX}_${TRANSPOSE_MODE}_n${N}" P10_DECODE=lut HIGH_CTX="$DEPTH4_HIGH_CTX"
run_one depthcode_payload scripts/build/b300-bucket-snake-pattern10-depthcode-graph-batch.sh "$ONEESAN_BUILD_DIR/ab_depthcode_payload_${TRANSPOSE_MODE}_n${N}"
cat "$RESULT"
w4="$(awk -F '\t' '$1=="depth4_lut"{print $3}' "$RESULT")"; wc="$(awk -F '\t' '$1=="depthcode_payload"{print $3}' "$RESULT")"
python3 - "$w4" "$wc" <<'PY'
import sys
w4,wc=map(float,sys.argv[1:])
print(f"depthcode_speedup_vs_depth4_lut={w4/wc:.6f}x")
print(f"depthcode_wall_delta_s={wc-w4:.6f}")
PY
echo "depth4-depthcode-graph-ab OK n=$N transpose=$TRANSPOSE_MODE pm_accum=$PM_ACCUM ternary_key4=$TERNARY_KEY4 result=$RESULT" >&2
