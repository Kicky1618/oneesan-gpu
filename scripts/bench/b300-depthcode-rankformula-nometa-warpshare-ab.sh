...
RANKFORMULA_NOMETA_BLOCK="${RANKFORMULA_NOMETA_BLOCK:-8}"
...
if [[ "$RANKFORMULA_NOMETA_BLOCK" != 8 ]]; then echo "warp-share A/B intentionally fixes RANKFORMULA_NOMETA_BLOCK=8" >&2; exit 2; fi
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_nometa_warpshare_ab_n${N}_${TRANSPOSE_MODE}}"
...
build_one(){
  local ws="$1" label="$2" bin="$3"
  N="$N" ARCH="$ARCH" OUT="$bin" RANKFORMULA_NOMETA_BLOCK=8 RANKFORMULA_NOMETA_WARPSHARE="$ws" \
...
for spec in 'scalar 0' 'warpshare 1'; do
  read -r label ws <<<"$spec"
  if [[ "$RUN_SELFTEST" == 1 ]]; then
    RANKFORMULA_NOMETA_BLOCK=8 RANKFORMULA_NOMETA_WARPSHARE="$ws" \
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
    if q['scalar'][k] and q['warpshare'][k]: print(f'warpshare_{k}_speedup={q["scalar"][k]/q["warpshare"][k]:.6f}x')
if run_ptxas=='1':
