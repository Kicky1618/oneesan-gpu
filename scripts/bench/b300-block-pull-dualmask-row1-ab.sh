...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_block_pull_dualmask_row${ROWS}_high${HIGH_DROP_CHUNK}}"
TMP="${TMP:-$ONEESAN_BUILD_DIR/b300_block_pull_dualmask_gen}"
BASE_BIN="$ONEESAN_BUILD_DIR/b300_block_pull_dualmask_base_n27_high${HIGH_DROP_CHUNK}"
CAND_BIN="$ONEESAN_BUILD_DIR/b300_block_pull_dualmask_n27_high${HIGH_DROP_CHUNK}"
...
echo "=== build baseline high_drop_chunk=$HIGH_DROP_CHUNK ===" >&2
...
S10="$TMP/10_ilp2.cu"; S11="$TMP/11_shard8.cu"; S12="$TMP/12_rowlimit.cu"; S13="$TMP/13_threads.cu"
...
grep -Fq 'block_pull_dualmask=1' <(python3 "$ONEESAN_ROOT/scripts/build/gen-b300-block-pull-dualmask.py" "$S8" "$TMP/verify_dualmask.cu")
grep -Fq 'b300_block_endpoint_masks(d)' "$BUILD_SRC"
if grep -Fq 'const MateValue v=mget(d,q);' "$BUILD_SRC"; then echo 'stale scan mget remains' >&2; exit 3; fi

echo "=== compile dualmask candidate ===" >&2
...
read -r BR BW BM BMAX BBUSY BSM BPWR <<<"$(run_one base "$BASE_BIN")"
read -r CR CW CM CMAX CBUSY CSM CPWR <<<"$(run_one cand "$CAND_BIN")"
[[ "$BR" == "$CR" ]] || { echo "FATAL dualmask residue mismatch base=$BR cand=$CR" >&2; exit 6; }

python3 - "$BW" "$CW" "$BM" "$CM" "$BBUSY" "$CBUSY" "$BSM" "$CSM" "$BPWR" "$CPWR" <<'PY'
import sys
bw,cw,bm,cm,bb,cb,bs,cs,bp,cp=map(float,sys.argv[1:])
print('b300_block_dualmask_exact_intermediate_match=1')
print(f'b300_block_dualmask_wall_baseline_s={bw:.9f}')
print(f'b300_block_dualmask_wall_candidate_s={cw:.9f}')
print(f'b300_block_dualmask_wall_speedup={bw/cw:.9f}x')
print(f'b300_block_dualmask_memctl_all_baseline_avg_pct={bm:.3f}')
print(f'b300_block_dualmask_memctl_all_candidate_avg_pct={cm:.3f}')
print(f'b300_block_dualmask_memctl_busy_baseline_avg_pct={bb:.3f}')
print(f'b300_block_dualmask_memctl_busy_candidate_avg_pct={cb:.3f}')
print(f'b300_block_dualmask_sm_baseline_avg_pct={bs:.3f}')
print(f'b300_block_dualmask_sm_candidate_avg_pct={cs:.3f}')
print(f'b300_block_dualmask_power_baseline_avg_w={bp:.3f}')
print(f'b300_block_dualmask_power_candidate_avg_w={cp:.3f}')
PY
printf 'b300_block_dualmask_rows=%s\n' "$ROWS"
printf 'b300_block_dualmask_threads=%s\n' "$THREADS"
printf 'b300_block_dualmask_high_drop_chunk=%s\n' "$HIGH_DROP_CHUNK"
printf 'b300_block_dualmask_residue=%s\n' "$BR"
printf 'b300_block_dualmask_note=R/L bitplanes replace per-endpoint mget; adopt only on exact match plus wall-time win\n'
