...
build_one() {
  local mode="$1" bin="$2"
  local cache fast poll packed fastdiv setbits broadword boundary
  if [[ "$mode" == baseline ]]; then
    cache=0; fast=0; poll=1; packed=0; fastdiv=0; setbits=0; broadword=0; boundary=0
  else
    cache=1; fast=1; poll=0; packed=1; fastdiv=1; setbits=1; broadword=1; boundary=1
  fi
  MODE=two-row-runtime-multigpu \
    ...
    RUNTIME_FAST_DIV64="$fastdiv" RUNTIME_PRIMITIVE_RANK_SETBITS="$setbits" \
...
print('runtime_fast_stack_baseline=edge_cache0,fast_mod0,error_poll1,packed_keys0,fast_div640,primitive_setbits0,broadword_support0,owner_boundaries0')
print('runtime_fast_stack_fast=edge_cache1,fast_mod1,error_poll0,packed_keys1,fast_div641,primitive_setbits1,broadword_support1,owner_boundaries1')
