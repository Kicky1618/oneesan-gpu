...
bash "$ONEESAN_ROOT/scripts/bench/gridfp-runtime-primitive-rank-setbits-proof.sh" >"$LOGDIR/primitive-setbits-proof.out" 2>"$LOGDIR/primitive-setbits-proof.err"
...
bash "$ONEESAN_ROOT/scripts/bench/gridfp-runtime-support-rank-setbits-proof.sh" >"$LOGDIR/support-setbits-proof.out" 2>"$LOGDIR/support-setbits-proof.err"

build_one() {
  local mode="$1" bin="$2"
  local cache fast poll packed fastdiv primitive broadword boundary support
  if [[ "$mode" == baseline ]]; then
    cache=0; fast=0; poll=1; packed=0; fastdiv=0; primitive=0; broadword=0; boundary=0; support=0
  else
    cache=1; fast=1; poll=0; packed=1; fastdiv=1; primitive=1; broadword=1; boundary=1; support=1
  fi
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for mode in "${order[@]}"; do [[ "$mode" == baseline ]] && bin="$BASE" || bin="$FAST"; echo "=== runtime stack=$mode run $r/$REPEATS ===" >&2; run_one "$mode" "$bin" "$r" run; done
done
cat "$RESULT"
...
print('runtime_fast_stack_baseline=edge_cache0,fast_mod0,error_poll1,packed_keys0,fast_div640,primitive_setbits0,broadword_support0,owner_boundaries0,support_setbits0')
print('runtime_fast_stack_fast=edge_cache1,fast_mod1,error_poll0,packed_keys1,fast_div641,primitive_setbits1,broadword_support1,owner_boundaries1,support_setbits1')
...
print('runtime_fast_stack_owner_lookup=owner_begin_boundaries')
print('runtime_fast_stack_support_rank_scan=ones')
print(f'summary={dst}')
PY
