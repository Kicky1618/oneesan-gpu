...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 ||
  ...
  echo "compact codec table runtime A/B supports even W=8..10" >&2
  exit 2
fi
if [[ "$MOD" != 4294967291 ]]; then
  echo "compact codec table runtime A/B requires MOD=4294967291" >&2
  exit 2
fi
...
OUT="$ONEESAN_BUILD_DIR/gridfp_codec_tables_nvcc_prepend_smoke_should_not_exist" \
...
build_one() {
  ...
  grep -Fq "nvcc_prepend=$inject)" "$LOGDIR/mode${mode}.build.out" || {
    echo "codec-table mode=$mode build did not report expected prepend flags" >&2
    cat "$LOGDIR/mode${mode}.build.out" >&2 || true
    exit 6
  }
  printf 'mode=%s choose=%s primitive=%s\n' "$mode" "$choose" "$primitive" \
...
for ((r=1; r<=WARMUP; ++r)); do
  for mode in 0 1 2 3; do run_one "$mode" "$r" warmup; done
done
...
for ((r=1; r<=REPEATS; ++r)); do
  ...
  for mode in "${order[@]}"; do
    echo "=== runtime compact codec tables mode=$mode run $r/$REPEATS ===" >&2
    run_one "$mode" "$r" run
  done
done
cat "$RESULT"
...
names={'0':'baseline','1':'choose','2':'primitive','3':'choose_primitive'}
out=[]
...
print('runtime_codec_tables_nvcc_prepend_smoke=1')
...
if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  for mode in 0 1 2 3; do
    echo "--- ptxas compact codec tables mode=$mode ---" >&2
