...
if (( W < 8 || W > 10 || W % 2 != 0 || NGPU < 2 || NGPU > 8 ||
  ...
  echo "turn discovery non-N runtime A/B supports even W=8..10" >&2
  exit 2
fi
if [[ "$MOD" != 4294967291 ]]; then
  echo "turn discovery non-N A/B requires MOD=4294967291" >&2
  exit 2
fi
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/gridfp_reduced_runtime_turn_discovery_nonn_scan_ab_w${W}_g${NGPU}_b${BLOCKS}}"
...
bash "$ONEESAN_ROOT/scripts/bench/gridfp-runtime-turn-discovery-nonn-scan-proof.sh" \
  >"$LOGDIR/turn-discovery-nonn-scan-proof.out" \
  2>"$LOGDIR/turn-discovery-nonn-scan-proof.err"
...
build_one() {
  ...
  RUNTIME_TURN_DISCOVERY_NONN_SCAN="$fast" \
  ...
  bash "$ONEESAN_ROOT/scripts/build/gridfp-reduced-component-probe.sh" \
    >"$LOGDIR/fast${fast}.build.out" \
    2>"$LOGDIR/fast${fast}.build.err"
}

BIN0="$ONEESAN_BUILD_DIR/gridfp_reduced_runtime_turn_discovery_nonn_scan0_w${W}_g${NGPU}_b${BLOCKS}"
BIN1="$ONEESAN_BUILD_DIR/gridfp_reduced_runtime_turn_discovery_nonn_scan1_w${W}_g${NGPU}_b${BLOCKS}"
build_one 0 "$BIN0"
build_one 1 "$BIN1"
...
run_one() {
  ...
  [[ -n "$line" ]] || {
    echo "turn_discovery_nonn_scan=$fast $phase$rep missing runtime result" >&2
    cat "$out" >&2 || true
    cat "$err" >&2 || true
    exit 3
  }
  grep -Fq ' exact=OK' <<<"$line" || {
    echo "turn_discovery_nonn_scan=$fast $phase$rep failed exactness" >&2
    echo "$line" >&2
    exit 4
  }
...
printf 'turn_discovery_nonn_scan\trepeat\twall_ms\n' >"$RESULT"
for ((r=1; r<=REPEATS; ++r)); do
  if ((r & 1)); then order=(0 1); else order=(1 0); fi
  for fast in "${order[@]}"; do
    [[ "$fast" == 0 ]] && bin="$BIN0" || bin="$BIN1"
    echo "=== runtime turn-discovery-nonn-scan=$fast run $r/$REPEATS ===" >&2
...
out = []
for mode in ('0', '1'):
    xs = [float(r['wall_ms']) for r in rows
          if r['turn_discovery_nonn_scan'] == mode]
    if not xs:
        raise SystemExit(f'missing turn_discovery_nonn_scan={mode}')
    out.append({
        'turn_discovery_nonn_scan': mode,
        'repeats': len(xs),
...
with open(dst, 'w', newline='') as f:
    w = csv.DictWriter(
        f,
        fieldnames=(
            'turn_discovery_nonn_scan', 'repeats',
            'wall_ms_median', 'wall_ms_min', 'wall_ms_max'),
        delimiter='\t')
    w.writeheader()
    w.writerows(out)
q = {r['turn_discovery_nonn_scan']: r for r in out}
old = float(q['0']['wall_ms_median'])
new = float(q['1']['wall_ms_median'])
print(f'runtime_turn_discovery_nonn_scan_wall_speedup={old/new:.6f}x')
print(f'runtime_turn_discovery_nonn_scan_wall_delta_pct={(new/old-1)*100:.4f}%')
print('runtime_turn_discovery_old=turn_nn_full_position_scan')
print('runtime_turn_discovery_new=turn_nn_non_n_setbit_scan')
print('runtime_turn_discovery_candidate_sequence=exact')
print('runtime_turn_discovery_extra_constant_bytes=0')
print('runtime_turn_discovery_extra_shared_bytes=0')
print(f'summary={dst}')
PY

if [[ "$PTXAS_VERBOSE" == 1 ]]; then
  echo '--- ptxas full-position turn discovery ---' >&2
  grep -E 'Used .* registers|bytes smem|bytes cmem' \
    "$LOGDIR/fast0.build.err" >&2 || true
  echo '--- ptxas non-N turn discovery ---' >&2
  grep -E 'Used .* registers|bytes smem|bytes cmem' \
    "$LOGDIR/fast1.build.err" >&2 || true
fi

echo "gridfp-reduced-runtime-turn-discovery-nonn-scan-ab OK W=$W ngpu=$NGPU blocks=$BLOCKS repeats=$REPEATS result=$RESULT" >&2
