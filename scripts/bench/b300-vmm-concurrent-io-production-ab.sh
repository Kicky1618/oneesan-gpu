...
MIN_IO_SPEEDUP="${MIN_IO_SPEEDUP:-1.01}";MIN_WORST_IO_SPEEDUP="${MIN_WORST_IO_SPEEDUP:-0.995}";MICRO_REPEATS="${MICRO_REPEATS:-9}"
...
read -r IO_SPEEDUP WORST_SPEEDUP < <(python3 - "$MICRO_RESULT" <<'PY'
import csv,statistics,sys
r=list(csv.DictReader(open(sys.argv[1]),delimiter='\t'))
combined=[float(x['combined_speedup']) for x in r]
directions=[float(x[k]) for x in r for k in ('read_speedup','write_speedup')]
print(f"{statistics.median(combined):.9f} {min(directions):.9f}")
PY
)
echo "b300_concurrent_io_micro_combined_median_speedup=${IO_SPEEDUP}x worst_direction_source_speedup=${WORST_SPEEDUP}x" >&2
set +e
python3 - "$IO_SPEEDUP" "$MIN_IO_SPEEDUP" "$WORST_SPEEDUP" "$MIN_WORST_IO_SPEEDUP" <<'PY'
import sys
med,med_min,worst,worst_min=map(float,sys.argv[1:])
raise SystemExit(0 if med>=med_min and worst>=worst_min else 9)
PY
gate_rc=$?;set -e
case "$gate_rc" in
  0);;
  9) echo "b300-vmm-concurrent-io-production-ab SKIP_FULL combined_median=${IO_SPEEDUP}x median_threshold=${MIN_IO_SPEEDUP}x worst_direction_source=${WORST_SPEEDUP}x worst_threshold=${MIN_WORST_IO_SPEEDUP}x";exit 0;;
  *)exit "$gate_rc";;
esac
...
cat "$RESULT"
python3 - "$RESULT" "$SUMMARY" "$IO_SPEEDUP" "$WORST_SPEEDUP" <<'PY'
import csv,statistics,sys
...
print(f'b300_concurrent_io_micro_combined_median_speedup={float(micro):.6f}x')
print(f'b300_concurrent_io_micro_worst_direction_source_speedup={float(worst):.6f}x')
...
print(f'summary={dst}')
PY
echo "b300-vmm-concurrent-io-production-ab OK runs=$RUNS median_gate=${IO_SPEEDUP}x median_threshold=${MIN_IO_SPEEDUP}x worst_gate=${WORST_SPEEDUP}x worst_threshold=${MIN_WORST_IO_SPEEDUP}x result=$RESULT" >&2
