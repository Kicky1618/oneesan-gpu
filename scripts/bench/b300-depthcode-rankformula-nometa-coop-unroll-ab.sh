...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_nometa_coop_unroll_ab_n${N}_${TRANSPOSE_MODE}}"
...
build_one(){
  local unroll="$1" label="$2" bin="$3"
  N="$N" ARCH="$ARCH" OUT="$bin" RANKFORMULA_NOMETA_BLOCK=16 \
    RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP=1 RANKFORMULA_NOMETA_COOP_UNROLL="$unroll" \
...
for spec in 'unrolled 1' 'rolled 0'; do
  read -r label unroll <<<"$spec"
  if [[ "$RUN_SELFTEST" == 1 ]]; then
    RANKFORMULA_NOMETA_BLOCK=16 RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP=1 RANKFORMULA_NOMETA_COOP_UNROLL="$unroll" \
  ...
  bin="$ONEESAN_BUILD_DIR/ab_rankformula_nometa_b16_${label}_n${N}"
  build_one "$unroll" "$label" "$bin"
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
    if q['unrolled'][k] and q['rolled'][k]: print(f'rolled_{k}_speedup={q["unrolled"][k]/q["rolled"][k]:.6f}x')
if run_ptxas=='1':
    ...
    for mode in ('unrolled','rolled'):
        ...
        stack=[int(r['stack_bytes']) for r in g if r['stack_bytes']!='NA']
        ...
        print(f'{mode}_high_stack_bytes={sum(stack) if stack else "NA"}')
        print(f'{mode}_high_spill_store_bytes={sum(ss) if ss else "NA"}')
        print(f'{mode}_high_spill_load_bytes={sum(sl) if sl else "NA"}')
print('block=16 coopgroup=1 warpshare=1')
print('table_loads_model=215509')
print('avg_early_ballots=2.251382')
print('semantic_delta=coop_loop_unroll_only')
PY

echo "b300-depthcode-rankformula-nometa-coop-unroll-ab OK n=$N repeats=$REPEATS block=16 result=$RESULT" >&2
