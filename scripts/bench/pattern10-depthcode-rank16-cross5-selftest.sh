...
if (( W > 12 || LOW_LUT_K <= 0 || HIGH_LUT_K <= 0 || LOW_LUT_K + HIGH_LUT_K + 1 != W )); then echo "rank16 CROSS5 selftest requires valid W<=12 split" >&2; exit 2; fi
...
SRC="$ONEESAN_ROOT/src/cuda/gridfp/probes/ramstream32_bucket_orbit_closure_pattern10_depthcode_rank16_cross5_selftest.cu"
BIN="${BIN:-$ONEESAN_BUILD_DIR/pattern10_depthcode_rank16_cross5_selftest_w${W}_pm${PM_ACCUM}_${DECODE_LOAD}}"
mkdir -p "$(dirname "$BIN")"
...
out="$($BIN)"
printf '%s\n' "$out"
grep -Eq "bucket-closure-pattern10-depthcode-rank16-cross5-selftest (OK W=$W|SKIP no CUDA device)" <<<"$out"
if grep -Fq "OK W=$W" <<<"$out"; then
  grep -Fq 'forward_exact=1 reverse_exact=1' <<<"$out"
  grep -Fq 'prekey_scope=fixed_owner prekey_table_exact=1 rank16_table_exact=1' <<<"$out"
...
echo "pattern10-depthcode-rank16-cross5-selftest OK W=$W pm_accum=$PM_ACCUM decode_load=$DECODE_LOAD rank16=1" >&2
