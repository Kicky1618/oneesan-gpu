...
MODES="${MODES:-ilp2 ilp3 ilp4 ilp4q ilp4warp ilp4qhot}"
...
build_one(){
  local mode="$1" i2=0 i3=0 i4=0 cq=0 cw=0 hot=0
  case "$mode" in
    ilp2) i2=1 ;;
    ilp3) i3=1 ;;
    ilp4) i4=1 ;;
  ...
  N=27 ARCH="$ARCH" OUT="$bin" FAST_SHARD_ADDRESS8=1 \
    ...
    RANK_STATE_ILP2="$i2" RANK_STATE_ILP3="$i3" RANK_STATE_ILP4="$i4" BLOCK_CLOSURE_QUAD="$cq" BLOCK_CLOSURE_WARP="$cw" \
...
for mode in $MODES; do
  ...
  [[ "$mode" == ilp4warp ]] && python3 "$PARSER" "$LOGDIR/$mode.build.err" --label "$mode" --contains closure_warp >>"$RESOURCE" || true
done
