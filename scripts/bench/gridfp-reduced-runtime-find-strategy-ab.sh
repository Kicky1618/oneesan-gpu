...
for ((r=1;r<=REPEATS;++r)); do case $((r%3)) in 1) order=(recent signature index) ;; 2) order=(index recent signature) ;; 0) order=(signature index recent) ;; esac; for mode in "${order[@]}"; do case "$mode" in recent) bin="$RECENT";; signature) bin="$SIGNATURE";; index) bin="$INDEX";; esac; echo "=== runtime find-strategy=$mode run $r/$REPEATS ===" >&2; run_one "$mode" "$bin" "$r" run; done; done
cat "$RESULT"
...
print(f'runtime_find_strategy_best={best}'); print(f'runtime_find_signature_vs_recent_speedup={q["recent"]/q["signature"]:.6f}x'); print(f'runtime_find_index_vs_recent_speedup={q["recent"]/q["index"]:.6f}x'); print(f'runtime_find_index_vs_signature_speedup={q["signature"]/q["index"]:.6f}x')
print('runtime_find_recent_shared_extra_bytes=0'); print('runtime_find_signature_shared_extra_bytes=0'); print('runtime_find_index_storage_bytes_per_set=64'); print('runtime_find_index_ways=2'); print('runtime_find_index_hash_buckets=32'); print('runtime_find_index_shared_extra_bytes=4096'); print('runtime_find_index_total_known_shared_bytes=20480'); print('runtime_find_index_net_vs_unpacked_baseline_shared_bytes=-3584'); print('runtime_find_global_defaults=recent0_signature0_index0_ways1_pending_gpu_ab'); print(f'summary={dst}')
PY
...
echo "gridfp-reduced-runtime-find-strategy-ab OK W=$W ngpu=$NGPU blocks=$BLOCKS index=s64w2 repeats=$REPEATS result=$RESULT" >&2
