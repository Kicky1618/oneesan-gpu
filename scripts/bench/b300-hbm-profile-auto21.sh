...
for x in RUN_PRECTX RUN_ORBIT_SCHEDULER RUN_ORBIT_ADVANCED RUN_ORBIT_DYNAMIC RUN_ORBIT_DYNAMIC_BID RUN_ORBIT_DYNAMIC_FUSE RUN_ORBIT_DYNAMIC_ADAPTIVE RUN_ORBIT_DYNAMIC_PIPE2 RUN_ORBIT_WARPCOOP RUN_ORBIT_WARPCOOP_AUTO RUN_ORBIT_QUAD_DESC RUN_ORBIT_QUAD_LOCAL0 RUN_ORBIT_QUAD_GROUP RUN_ORBIT_QUAD_PREFETCH; do
...
if [[ "$RUN_PRECTX" == 1 ]]; then
  ...
  PROFILE_IN="$BASE_PROFILE" PROFILE_OUT="$PRECTX_PROFILE" PREFIX="$PRECTX_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-compact-prectx21.sh"
else cp "$BASE_PROFILE" "$PRECTX_PROFILE"; fi
if [[ "$RUN_ORBIT_SCHEDULER" == 1 ]]; then
  echo "=== HBM tune21 orbit scheduler refinement ===" >&2
  PROFILE_IN="$PRECTX_PROFILE" PROFILE_OUT="$SCHED_PROFILE" PREFIX="$SCHED_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-scheduler21.sh"
else cp "$PRECTX_PROFILE" "$SCHED_PROFILE"; fi
if [[ "$RUN_ORBIT_ADVANCED" == 1 ]]; then
  echo "=== HBM tune21 advanced orbit branch selection ===" >&2
  PROFILE_IN="$SCHED_PROFILE" PROFILE_OUT="$ADV_PROFILE" PREFIX="$ADV_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-advanced21.sh"
else cp "$SCHED_PROFILE" "$ADV_PROFILE"; fi
if [[ "$RUN_ORBIT_DYNAMIC" == 1 && "$RUN_ORBIT_ADVANCED" == 1 ]]; then
  echo "=== HBM tune21 dynamic flat queue refinement ===" >&2
  PROFILE_BASE="$SCHED_PROFILE" PROFILE_ADV="$ADV_PROFILE" ADV_SUMMARY="${ADV_PREFIX}_summary.tsv" PROFILE_OUT="$DYNAMIC_PROFILE" PREFIX="$DYNAMIC_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-dynamic21.sh"
else cp "$ADV_PROFILE" "$DYNAMIC_PROFILE"; fi
...
[[ "$DYNAMIC_SELECTED" == 0 || "$DYNAMIC_SELECTED" == 1 ]] || exit 2
if [[ "$RUN_ORBIT_DYNAMIC_BID" == 1 && "$DYNAMIC_SELECTED" == 1 && "${ORBIT_PRECTX_FORWARD:-0}" == 1 && "${ORBIT_PRECTX_REVERSE:-0}" == 1 && "${ORBIT_PRECTX_COMPACT:-0}" == 1 ]]; then
  ...
  PROFILE_IN="$DYNAMIC_PROFILE" PROFILE_OUT="$DYNAMIC_BID_PROFILE" PREFIX="$DYNAMIC_BID_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-dynamic-bid21.sh"
else
  [[ "$DYNAMIC_SELECTED" == 0 || "$RUN_ORBIT_DYNAMIC_BID" == 0 ]] || echo "=== skip dynamic flat-bid: dynamic winner lacks compact forward+reverse prectx ===" >&2
...
if [[ "$RUN_ORBIT_DYNAMIC_FUSE" == 1 && "$DYNAMIC_SELECTED" == 1 ]]; then
  ...
  PROFILE_IN="$DYNAMIC_BID_PROFILE" PROFILE_OUT="$DYNAMIC_FUSE_PROFILE" PREFIX="$DYNAMIC_FUSE_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-dynamic-fuse21.sh"
else cp "$DYNAMIC_BID_PROFILE" "$DYNAMIC_FUSE_PROFILE"; fi
ORBITCTA_FLAT_DYNAMIC=0 ORBITCTA_FLAT_DYNAMIC_BATCH=1; source "$DYNAMIC_FUSE_PROFILE"
DYNAMIC_SELECTED="${ORBITCTA_FLAT_DYNAMIC:-0}"; DYNAMIC_BATCH_SELECTED="${ORBITCTA_FLAT_DYNAMIC_BATCH:-1}"
if [[ "$RUN_ORBIT_DYNAMIC_ADAPTIVE" == 1 && "$DYNAMIC_SELECTED" == 1 && "$DYNAMIC_BATCH_SELECTED" != 1 ]]; then
  ...
  PROFILE_IN="$DYNAMIC_FUSE_PROFILE" PROFILE_OUT="$DYNAMIC_ADAPTIVE_PROFILE" PREFIX="$DYNAMIC_ADAPTIVE_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-dynamic-adaptive21.sh"
else
...
if [[ "$RUN_ORBIT_DYNAMIC_PIPE2" == 1 && "$DYNAMIC_SELECTED" == 1 ]]; then
  echo "=== HBM tune21 dynamic PIPE2 refinement ===" >&2
  PROFILE_IN="$DYNAMIC_ADAPTIVE_PROFILE" PROFILE_OUT="$DYNAMIC_PIPE2_PROFILE" PREFIX="$DYNAMIC_PIPE2_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-dynamic-pipe2-21.sh"
else cp "$DYNAMIC_ADAPTIVE_PROFILE" "$DYNAMIC_PIPE2_PROFILE"; fi
...
if [[ "$RUN_ORBIT_WARPCOOP" == 1 && "$RUN_ORBIT_ADVANCED" == 1 && "$DYNAMIC_SELECTED" == 0 ]]; then
  ...
  PROFILE_IN="$DYNAMIC_PIPE2_PROFILE" PROFILE_OUT="$WARPCOOP_PROFILE" PREFIX="$WARPCOOP_PREFIX" QUAD_WINNER_ENV="${ADV_PREFIX}_quad_winner.env" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-warpcoop21.sh"
else
  [[ "$DYNAMIC_SELECTED" == 0 ]] || echo "=== skip warpcoop refine: dynamic queue selected ===" >&2
  cp "$DYNAMIC_PIPE2_PROFILE" "$WARPCOOP_PROFILE"
fi
if [[ "$RUN_ORBIT_WARPCOOP_AUTO" == 1 && "$RUN_ORBIT_ADVANCED" == 1 && "$DYNAMIC_SELECTED" == 0 ]]; then
  ...
  PROFILE_IN="$WARPCOOP_PROFILE" PROFILE_OUT="$WARPCOOP_AUTO_PROFILE" PREFIX="$WARPCOOP_AUTO_PREFIX" QUAD_WINNER_ENV="${ADV_PREFIX}_quad_winner.env" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-warpcoop-auto21.sh"
else cp "$WARPCOOP_PROFILE" "$WARPCOOP_AUTO_PROFILE"; fi
if [[ "$RUN_ORBIT_QUAD_DESC" == 1 ]]; then
  ORBIT_QUAD_MLP=0 ORBIT_QUAD_OVERLAP_LOCAL=0 ORBIT_SPARSE64=0; source "$WARPCOOP_AUTO_PROFILE"
  if [[ "${ORBIT_QUAD_MLP:-0}" == 1 && "${ORBIT_QUAD_OVERLAP_LOCAL:-0}" == 1 && "${ORBIT_SPARSE64:-0}" == 1 ]]; then
    ...
    PROFILE_IN="$WARPCOOP_AUTO_PROFILE" PROFILE_OUT="$DESC_PROFILE" PREFIX="$QUAD_DESC_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-quad-desc21.sh"
  else cp "$WARPCOOP_AUTO_PROFILE" "$DESC_PROFILE"; fi
else cp "$WARPCOOP_AUTO_PROFILE" "$DESC_PROFILE"; fi
if [[ "$RUN_ORBIT_QUAD_LOCAL0" == 1 ]]; then
  ORBIT_QUAD_MLP=0 ORBIT_QUAD_OVERLAP_LOCAL=0; source "$DESC_PROFILE"
  if [[ "${ORBIT_QUAD_MLP:-0}" == 1 && "${ORBIT_QUAD_OVERLAP_LOCAL:-0}" == 1 ]]; then
    echo "=== HBM tune21 QOL local_n=0 bypass refinement ===" >&2
    PROFILE_IN="$DESC_PROFILE" PROFILE_OUT="$LOCAL0_PROFILE" PREFIX="$QUAD_LOCAL0_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-quad-local0-21.sh"
  else cp "$DESC_PROFILE" "$LOCAL0_PROFILE"; fi
else cp "$DESC_PROFILE" "$LOCAL0_PROFILE"; fi
if [[ "$RUN_ORBIT_QUAD_GROUP" == 1 ]]; then
  ORBIT_QUAD_MLP=0 ORBIT_QUAD_OVERLAP_LOCAL=0; source "$LOCAL0_PROFILE"
  if [[ "${ORBIT_QUAD_MLP:-0}" == 1 && "${ORBIT_QUAD_OVERLAP_LOCAL:-0}" == 1 ]]; then
    echo "=== HBM tune21 QOL cp.async commit-group refinement ===" >&2
    PROFILE_IN="$LOCAL0_PROFILE" PROFILE_OUT="$GROUP_PROFILE" PREFIX="$QUAD_GROUP_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-quad-group21.sh"
  else cp "$LOCAL0_PROFILE" "$GROUP_PROFILE"; fi
else cp "$LOCAL0_PROFILE" "$GROUP_PROFILE"; fi
if [[ "$RUN_ORBIT_QUAD_PREFETCH" == 1 ]]; then
  ORBIT_QUAD_MLP=0 ORBIT_QUAD_OVERLAP_LOCAL=0; source "$GROUP_PROFILE"
  if [[ "${ORBIT_QUAD_MLP:-0}" == 1 && "${ORBIT_QUAD_OVERLAP_LOCAL:-0}" == 1 ]]; then
    echo "=== HBM tune21 QOL cp.async L2 prefetch refinement ===" >&2
    PROFILE_IN="$GROUP_PROFILE" PROFILE_OUT="$FINAL_PROFILE" PREFIX="$QUAD_PREFETCH_PREFIX" bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-quad-prefetch21.sh"
  else cp "$GROUP_PROFILE" "$FINAL_PROFILE"; fi
else cp "$GROUP_PROFILE" "$FINAL_PROFILE"; fi

echo "=== final HBM profile ===" >&2
cat "$FINAL_PROFILE"
echo "b300 HBM profile auto21 OK base_profile=$BASE_PROFILE prectx_profile=$PRECTX_PROFILE sched_profile=$SCHED_PROFILE advanced_profile=$ADV_PROFILE dynamic_profile=$DYNAMIC_PROFILE dynamic_bid_profile=$DYNAMIC_BID_PROFILE dynamic_fuse_profile=$DYNAMIC_FUSE_PROFILE dynamic_adaptive_profile=$DYNAMIC_ADAPTIVE_PROFILE dynamic_pipe2_profile=$DYNAMIC_PIPE2_PROFILE dynamic_selected=$DYNAMIC_SELECTED warpcoop_profile=$WARPCOOP_PROFILE warpcoop_auto_profile=$WARPCOOP_AUTO_PROFILE desc_profile=$DESC_PROFILE local0_profile=$LOCAL0_PROFILE group_profile=$GROUP_PROFILE final_profile=$FINAL_PROFILE" >&2
