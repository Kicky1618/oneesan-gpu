...
for x in RUN_PRECTX RUN_ORBIT_SCHEDULER RUN_ORBIT_QUAD; do
...
if [[ "$RUN_ORBIT_QUAD" == 1 ]]; then
  ...
  PROFILE_IN="$SCHED_PROFILE" PROFILE_OUT="$FINAL_PROFILE" PREFIX="$QUAD_PREFIX" \
    bash "$ONEESAN_ROOT/scripts/bench/b300-hbm-profile-refine-orbit-quad21.sh"
else
  cp "$SCHED_PROFILE" "$FINAL_PROFILE"
fi

echo "=== final HBM profile ===" >&2
cat "$FINAL_PROFILE"
echo "b300 HBM profile auto21 OK base_profile=$BASE_PROFILE prectx_profile=$PRECTX_PROFILE sched_profile=$SCHED_PROFILE final_profile=$FINAL_PROFILE" >&2
