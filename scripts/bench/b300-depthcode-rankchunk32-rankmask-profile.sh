...
for x in RANKCHUNK32_ONESHFL RANKCHUNK32_FUSED16 RANKCHUNK32_DIRECT3 RANKCHUNK32_BYTEPACK RANKCHUNK32_ALIGN32 RANKCHUNK32_BLOCK64 PM_ACCUM TERNARY_KEY4 RUN_SELFTEST; do
  v="${!x}"
...
if [[ "$RUN_SELFTEST" == 1 ]]; then
  echo "=== CUDA rankchunk32 profile compile/selftest ===" >&2
  W=10 ARCH="$ARCH" PM_ACCUM="$PM_ACCUM" DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" \
    RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" RANKCHUNK32_ONESHFL="$RANKCHUNK32_ONESHFL" \
    RANKCHUNK32_FUSED16="$RANKCHUNK32_FUSED16" RANKCHUNK32_DIRECT3="$RANKCHUNK32_DIRECT3" \
    RANKCHUNK32_RANKMASK_PROFILE=1 RANKCHUNK32_RANKMASK_PROFILE_LOG2="$PROFILE_LOG2" \
    RANKCHUNK32_BYTEPACK="$RANKCHUNK32_BYTEPACK" RANKCHUNK32_ALIGN32="$RANKCHUNK32_ALIGN32" \
    RANKCHUNK32_BLOCK64="$RANKCHUNK32_BLOCK64" RUN_LAYOUT_PROOF=0 \
