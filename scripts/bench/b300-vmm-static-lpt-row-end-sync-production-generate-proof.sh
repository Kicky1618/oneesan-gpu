...
grep -Fq 'record global window done' "$OUT";grep -Fq 'local main wait global window' "$OUT";grep -Fq 'local block wait global window' "$OUT";grep -Fq 'gpu0 row end sync' "$OUT"
for stale in 'gpu0 global window sync' 'window batch sync' 'scatter join sync';do grep -Fq "$stale" "$OUT"&&{ echo "row-end sync stale=$stale" >&2;exit 3;}||true;done
[[ "$(grep -Fc 'gpu0 row end sync' "$OUT")" == 1 ]]||exit 4
echo 'b300-vmm-static-lpt-row-end-sync-production-generate-proof OK cross_device_window_chain=1 host_gpu_syncs_per_row=1 expected_rows=28 expected_host_gpu_sync_calls=28 crosswindow_old_calls=56 per_group_host_syncs=0 scratch_realloc_per_group=0'
