...
BIN="${BIN:-$ONEESAN_BUILD_DIR/pattern10_depthcode_rankformula_nometa4_abstract_block${RANKFORMULA_NOMETA_BLOCK}_selftest_w${W}}"
mkdir -p "$(dirname "$BIN")"
...
grep -Fq "rankformula-nometa4-abstract-block-selftest compiled_block=$RANKFORMULA_NOMETA_BLOCK max_locator_steps_bound=$((RANKFORMULA_NOMETA_BLOCK - 1)) wrapper_ok=1" <<<"$out"
...
echo "pattern10-depthcode-rankformula-nometa4-abstract-block-selftest OK W=$W block=$RANKFORMULA_NOMETA_BLOCK" >&2
