...
grep -Fq 'max_owner_bytes=619052' <<<"$out"
grep -Fq 'max_delta=572' <<<"$out"
grep -Fq 'slow_delta=2620/2535272' <<<"$out"
grep -Fq 'original_rankstream_bytes=7441610' <<<"$out"
grep -Fq 'rankdelta8_bytes=4911578' <<<"$out"
grep -Fq 'max_prefix_packed=364' <<<"$out"
grep -Fq 'max_prefix_align32=364' <<<"$out"
echo 'rankdelta8-plan OK exact_w28=1 delta_fast8_escape16=1 prefix9_block32=1' >&2
