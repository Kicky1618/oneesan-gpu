...
for spec in '4 0 0' '8 0 0' '8 1 0' '8 1 1' '16 1 1'; do
...
if [[ "$RUN_BUILD_SMOKE" == 1 ]]; then
  echo '=== abstract nometa B300 production compile smoke block=16 warpshare=1 coopgroup=1 ===' >&2
  N="$N" ARCH="$ARCH" RANKFORMULA_NOMETA_BLOCK=16 RANKFORMULA_NOMETA_WARPSHARE=1 RANKFORMULA_NOMETA_COOPGROUP=1 \
    ...
    TRANSPOSE_MODE=pipeline PTXAS_VERBOSE=1 OUT="$ONEESAN_BUILD_DIR/rankformula_nometa_abstract_b16_coopgroup_preflight_n${N}" \
    bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-depthcode-rankformula-nometa4-abstract.sh"
fi

echo "rankformula-nometa4-abstract-preflight OK w=$W n=$N metadata_bytes_per_code=0 exact_modes=b4_scalar,b8_scalar,b8_warpshare,b8_coopgroup,b16_coopgroup packed_group64_bits=59 self_group_index=1 support_positions_runtime=0 b8_aux_bytes_all=857642 b16_aux_bytes_all=707406 b8_coopgroup_table_loads_model=361431 b16_coopgroup_table_loads_model=215509 b8_avg_early_ballots=1.921043 b16_avg_early_ballots=2.251382 abstract_lut_bytes=94206 ballot_runtime_loads=0 lazy_source_loads=1 build_smoke_b16_coopgroup=$RUN_BUILD_SMOKE" >&2
