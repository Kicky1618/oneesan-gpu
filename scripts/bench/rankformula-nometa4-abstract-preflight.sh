...
for block in 4 8; do
  echo "=== abstract nometa exact CUDA block=$block ===" >&2
  RANKFORMULA_NOMETA_BLOCK="$block" ARCH="$ARCH" W="$W" DECODE_LOAD="$DECODE_LOAD" \
...
if [[ "$RUN_BUILD_SMOKE" == 1 ]]; then
  echo '=== abstract nometa B300 production compile smoke block=8 ===' >&2
  N="$N" ARCH="$ARCH" RANKFORMULA_NOMETA_BLOCK=8 \
    DEPTHCODE_DECODE_LOAD="$DECODE_LOAD" RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" \
    TRANSPOSE_MODE=pipeline PTXAS_VERBOSE=1 OUT="$ONEESAN_BUILD_DIR/rankformula_nometa_abstract_block8_preflight_n${N}" \
    bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-depthcode-rankformula-nometa4-abstract.sh"
fi

echo "rankformula-nometa4-abstract-preflight OK w=$W n=$N metadata_bytes_per_code=0 blocks_exact=4,8 packed_group64=1 support_positions_runtime=0 b4_aux_bytes_all=1158104 b8_aux_bytes_all=857642 abstract_lut_bytes=94206 ballot_runtime_loads=0 lazy_source_loads=1 build_smoke_block8=$RUN_BUILD_SMOKE" >&2
