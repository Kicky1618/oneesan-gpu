...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_nextgen_hybrid8_mate_evict_row${ROWS}}"; LOGDIR="${LOGDIR:-${PREFIX}_logs}"; RESULT="${RESULT:-${PREFIX}.tsv}"; RESOURCE="${RESOURCE:-${PREFIX}_ptxas.tsv}"; WINNER_ENV="${WINNER_ENV:-${PREFIX}_winner.env}"
...
(( $(nvidia-smi --query-gpu=index --format=csv,noheader | wc -l) >= 1 )) || { echo 'need at least one visible GPU' >&2; exit 2; }
...
for th in $THREADS_LIST; do
  while IFS=$'\t' read -r ev bin err; do
    ...
    for ((r=1;r<=REPEATS;++r)); do
      echo "=== mate-evict=$ev threads=$th repeat=$r rows=$ROWS self=w${SW}d${SD}/$SE mate=w${MW}d${MD} ===" >&2
      run_one "$ev" "$bin" "$th" "$r"
    done
  done <"$LOGDIR/binaries.tsv"
done

python3 - "$RESULT" "$RESOURCE" "$LOGDIR/binaries.tsv" "$WINNER_ENV" "$ROWS" "$SW" "$SD" "$SE" "$MW" "$MD" "$BASE_ME" <<'PY'
import csv,math,statistics,sys,shlex
result,resource,bins_path,winner,rows_arg,sw,sd,se,mw,md,base_ev=sys.argv[1:]
...
for x in sorted(agg,key=rank): print(f'MATE_EVICT evict={x[1]} threads={x[2]} wall_s={x[0]:.9f} high_s={x[3]:.9f} regs={x[4]} spill_store={x[5]} spill_load={x[6]} spill_free={int(x[7])}',file=sys.stderr)
...
vals={
 'B300_STAGEK_ROWS':rows_arg,'B300_STAGEK_RESIDUE':next(iter(res)),'B300_STAGEK_SELF_WIDTH':int(sw),'B300_STAGEK_SELF_DISTANCE':int(sd),'B300_STAGEK_SELF_EVICT':se,
...
print('b300_stagek_exact_match=1'); print(f'b300_stagek_mate_evict={test[1]}'); print(f'b300_stagek_speedup={speed:.9f}x'); print(f'b300_stagek_best_enabled={enabled}')
PY
cat "$RESULT"; cat "$RESOURCE"
echo "b300-nextgen-hybrid8-mate-evict-sweep OK rows=$ROWS self=w${SW}d${SD}/$SE mate=w${MW}d${MD} baseline=$BASE_ME hints=${evicts[*]} winner_env=$WINNER_ENV" >&2
