...
for x in PM_ACCUM RANKCHUNK32_ONESHFL RANKCHUNK32_FUSED16 RANKCHUNK32_DIRECT3 RANKCHUNK32_DIRECTMASK RANKCHUNK32_RANKPLANE RANKCHUNK32_RANKMASK_PROFILE RANKCHUNK32_BYTEPACK RANKCHUNK32_ALIGN32 RANKCHUNK32_BLOCK64 RUN_LAYOUT_PROOF; do
...
if [[ "$RANKCHUNK32_RANKPLANE" == 1 && "$RANKCHUNK32_DIRECTMASK" != 1 ]]; then echo "RANKPLANE requires DIRECTMASK=1" >&2; exit 2; fi
...
if grep -Fq "OK W=$W" <<<"$out"; then
  ...
  grep -Fq "directmask=$RANKCHUNK32_DIRECTMASK rankplane=$RANKCHUNK32_RANKPLANE ilp=$RANKCHUNK32_ILP" <<<"$out"
  if [[ "$RANKCHUNK32_DIRECTMASK" == 1 ]]; then
    ...
    if [[ "$RANKCHUNK32_RANKPLANE" == 1 ]]; then
      grep -Fq 'directrankplane_exact=1 offset_runtime=0' <<<"$out"
    fi
  fi
