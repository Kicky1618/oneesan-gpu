...
for x in PM_ACCUM RANKCHUNK32_ONESHFL RANKCHUNK32_FUSED16 RANKCHUNK32_BYTEPACK RANKCHUNK32_ALIGN32 RUN_LAYOUT_PROOF; do
  v="${!x}"
...
if [[ "$RANKCHUNK32_BYTEPACK" == 1 ]]; then
  EXPECT_CHUNK_BITS=24; EXPECT_PREFIX_BITS=8; PACK_NAME=24bit_chunks_8bit_prefix
else
  EXPECT_CHUNK_BITS=23; EXPECT_PREFIX_BITS=9; PACK_NAME=23bit_chunks_9bit_prefix
fi
if [[ "$RANKCHUNK32_ALIGN32" == 1 ]]; then
  EXPECT_ALIGN=32; EXPECT_LOADS=1; ALIGN_NAME=align32
else
  EXPECT_ALIGN=1; EXPECT_LOADS=2; ALIGN_NAME=packed_heights
fi
LAYOUT_NAME="${PACK_NAME}_block32_${ALIGN_NAME}"
...
BIN="${BIN:-$ONEESAN_BUILD_DIR/pattern10_depthcode_rankchunk32_cross5_selftest_w${W}_pm${PM_ACCUM}_${DECODE_LOAD}_ranklut${RANKSTREAM_LUT_LOAD}_oneshfl${RANKCHUNK32_ONESHFL}_fused16${RANKCHUNK32_FUSED16}_bytepack${RANKCHUNK32_BYTEPACK}_align${RANKCHUNK32_ALIGN32}}"
mkdir -p "$(dirname "$BIN")"
...
if grep -Fq "OK W=$W" <<<"$out"; then
  ...
  grep -Fq "chunk_bits=$EXPECT_CHUNK_BITS prefix_bits=$EXPECT_PREFIX_BITS block=32 height_align=$EXPECT_ALIGN" <<<"$out"
