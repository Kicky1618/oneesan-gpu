...
MODES="${MODES:-cross local overlap pipe2}"
if [[ -z "${EXPECT+x}" ]]; then
...
for mode in cross local overlap pipe2; do
  has "$mode" || continue
  read -r local_cpa overlap_cpa pipe2 <<<"$(mode_flags "$mode")"
  ...
  N="$N" ARCH="$ARCH" OUT="$bin" COL_ILP="$COL_ILP" PM_ACCUM="$PM_ACCUM" \
    ...
    CPASYNC_OVERLAP_LOCAL_PIPE2="$pipe2" OVERLAP_LOCAL_CG=0 \
...
for mode in ('cross','local','overlap','pipe2'):
...
print('cross=cross_cpasync14 local=reuse_shared_cpasync14_plus_ldg2 overlap=cross_cpasync14_plus_local_ldg16 pipe2=waitA_reduceA_while_crossB_pending')
PY
