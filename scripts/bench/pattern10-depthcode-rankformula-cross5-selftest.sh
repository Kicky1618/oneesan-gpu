...
RANKFORMULA_RAWCODE="${RANKFORMULA_RAWCODE:-0}"; RANKFORMULA_INLINE_CROSS="${RANKFORMULA_INLINE_CROSS:-0}"
...
for x in PM_ACCUM RANKDELTA8_FUSED13 RANKFORMULA_SPARSE_BASE RANKFORMULA_RAWCODE RANKFORMULA_INLINE_CROSS RANKFORMULA_BASE_DELTA; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0 or 1" >&2; exit 2; }; done
...
BIN="${BIN:-$ONEESAN_BUILD_DIR/pattern10_depthcode_rankformula_cross5_selftest_w${W}_pm${PM_ACCUM}_${DECODE_LOAD}_ranklut${RANKSTREAM_LUT_LOAD}_fused${RANKDELTA8_FUSED13}_sparse${RANKFORMULA_SPARSE_BASE}_raw${RANKFORMULA_RAWCODE}_inline${RANKFORMULA_INLINE_CROSS}_basedelta${RANKFORMULA_BASE_DELTA}}"
mkdir -p "$(dirname "$BIN")"
...
if grep -Fq "OK W=$W" <<<"$out"; then
  ...
  grep -Fq "base_delta=$RANKFORMULA_BASE_DELTA" <<<"$out"
  expected_values=2; [[ "$RANKFORMULA_BASE_DELTA" == 1 ]] && expected_values=1
  grep -Fq "base_values_per_lookup=$expected_values" <<<"$out"
  grep -Fq "rawcode=$RANKFORMULA_RAWCODE" <<<"$out"
  grep -Fq "chunkinfo_elided=$RANKFORMULA_RAWCODE" <<<"$out"
  grep -Fq "inline_cross=$RANKFORMULA_INLINE_CROSS" <<<"$out"
  grep -Fq "cross_lut_elided=$RANKFORMULA_INLINE_CROSS" <<<"$out"
  grep -Fq 'cross_runtime_div=0 cross_runtime_mod=0' <<<"$out"
fi
echo "pattern10-depthcode-rankformula-cross5-selftest OK W=$W pm=$PM_ACCUM lut=$RANKSTREAM_LUT_LOAD fused13=$RANKDELTA8_FUSED13 sparse_base=$RANKFORMULA_SPARSE_BASE rawcode=$RANKFORMULA_RAWCODE inline_cross=$RANKFORMULA_INLINE_CROSS base_delta=$RANKFORMULA_BASE_DELTA" >&2
