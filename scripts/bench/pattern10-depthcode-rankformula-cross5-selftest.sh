...
for x in PM_ACCUM RANKDELTA8_FUSED13 RANKFORMULA_SPARSE_BASE RANKFORMULA_RAWCODE RANKFORMULA_INLINE_CROSS; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0 or 1" >&2; exit 2; }; done
...
BIN="${BIN:-$ONEESAN_BUILD_DIR/pattern10_depthcode_rankformula_cross5_selftest_w${W}_pm${PM_ACCUM}_${DECODE_LOAD}_ranklut${RANKSTREAM_LUT_LOAD}_fused${RANKDELTA8_FUSED13}_sparse${RANKFORMULA_SPARSE_BASE}_raw${RANKFORMULA_RAWCODE}_inline${RANKFORMULA_INLINE_CROSS}}"
mkdir -p "$(dirname "$BIN")"
...
echo "pattern10-depthcode-rankformula-cross5-selftest OK W=$W pm=$PM_ACCUM lut=$RANKSTREAM_LUT_LOAD fused13=$RANKDELTA8_FUSED13 sparse_base=$RANKFORMULA_SPARSE_BASE rawcode=$RANKFORMULA_RAWCODE inline_cross=$RANKFORMULA_INLINE_CROSS" >&2
