...
(( RUNS>=1 ))||exit 2;command -v "$NVCC" >/dev/null||exit 2;command -v nvidia-smi >/dev/null||exit 2
...
python3 - "$ARG_SPEEDUP" "$MIN_ARG_SPEEDUP" <<'PY'
import sys
s,t=map(float,sys.argv[1:])
if s<t: raise SystemExit(9)
PY
case $? in
  0) ;;
  9) echo "b300-vmm-base-source-production-ab SKIP_FULL micro_arg_speedup=${ARG_SPEEDUP}x threshold=${MIN_ARG_SPEEDUP}x"; exit 0 ;;
  *) exit $? ;;
esac
...
run_one(){
  local mode="$1" run="$2" bin; [[ "$mode" == symbol ]]&&bin="$BIN_SYMBOL"||bin="$BIN_ARG"
...
for((r=1;r<=RUNS;++r));do if((r&1));then order=(symbol arg);else order=(arg symbol);fi;for m in "${order[@]}";do run_one "$m" "$r";done;done
cat "$RESULT"
...
for mode in ('symbol','arg'):
 ...
 res[mode]=next(iter(rr))
 med=lambda k:statistics.median(float(x[k]) for x in r)
