...
SELF_BIN="$ONEESAN_BUILD_DIR/b300_stageh_self_w${W}_d${D}_t${T}_n27"; SELF_ERR="$LOGDIR/self.build.err"; SELF_OUT="$LOGDIR/self.build.out"
MATE_BIN="$ONEESAN_BUILD_DIR/b300_stageh_mate_w${W}_d${D}_t${T}_n27"; MATE_ERR="$LOGDIR/mate.build.err"; MATE_OUT="$LOGDIR/mate.build.out"
COMMON=(N=27 ARCH="$ARCH" HIGH_DROP_CHUNK="$H" HYBRID_THRESHOLD="$T" NEXTSELF_WIDTH="$W" NEXTSELF_DISTANCE="$D" RANDOM_CG="$CG" RANDOM_CG_L2_FETCH_BYTES="$CGL2" PREFETCH_L2="$PRE" DUALMASK="$DUAL" CLOSURE_BATCH="$BATCH" MAXRREGCOUNT="$CAP" PTXAS_VERBOSE=1)
env "${COMMON[@]}" OUT="$SELF_BIN" BUILD_ERR="$SELF_ERR" bash "$ONEESAN_ROOT/scripts/build/b300-forced-nextgen-hybrid8-nextself-distance.sh" >"$SELF_OUT" 2>"$LOGDIR/self.driver.err"
env "${COMMON[@]}" OUT="$MATE_BIN" BUILD_ERR="$MATE_ERR" bash "$ONEESAN_ROOT/scripts/build/b300-forced-nextgen-hybrid8-nextself-mate.sh" >"$MATE_OUT" 2>"$LOGDIR/mate.driver.err"
...
grep -Fq 'next_mate_prefetch=1' "$MATE_OUT" || exit 3
...
for th in $THREADS_LIST; do for ((r=1;r<=REPEATS;++r)); do echo "=== Stage H self t=$th r=$r rows=$ROWS w=$W d=$D ===" >&2; run_one self "$SELF_BIN" "$th" "$r"; done; for ((r=1;r<=REPEATS;++r)); do echo "=== Stage H mate t=$th r=$r rows=$ROWS w=$W d=$D ===" >&2; run_one mate "$MATE_BIN" "$th" "$r"; done; done

python3 - "$RESULT" "$RESOURCE" "$WINNER_ENV" "$SELF_BIN" "$MATE_BIN" "$ROWS" "$W" "$D" <<'PY'
import csv,math,statistics,sys,shlex
result,resource,winner,selfbin,matebin,rows_arg,w,d=sys.argv[1:]
...
for x in sorted(agg,key=rank): print(f'STAGE_H profile={x[1]} threads={x[2]} wall_s={x[0]:.9f} high_s={x[3]:.9f} regs={x[4]} spill_store={x[5]} spill_load={x[6]} spill_free={int(x[7])}',file=sys.stderr)
