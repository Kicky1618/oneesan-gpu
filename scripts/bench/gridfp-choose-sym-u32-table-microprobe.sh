...
BLOCKS="${BLOCKS:-4096}"; THREADS="${THREADS:-256}"; ITERS="${ITERS:-512}"; REPEATS="${REPEATS:-7}"; WARMUP="${WARMUP:-2}"; ARCH="${ARCH:-native}"; PTXAS_VERBOSE="${PTXAS_VERBOSE:-1}"
...
bash "$ONEESAN_ROOT/scripts/bench/gridfp-choose-sym-u32-table-proof.sh" >"$LOGDIR/proof.out" 2>"$LOGDIR/proof.err"

SRC="$ONEESAN_ROOT/src/cuda/gridfp/gridfp_choose_sym_u32_table_microprobe.cu"; PTXAS_FLAGS=(); [[ "$PTXAS_VERBOSE" == 1 ]] && PTXAS_FLAGS+=("-Xptxas=-v"); BINS=()
for mode in 0 1 2 3; do
  BINS[$mode]="$ONEESAN_BUILD_DIR/gridfp_choose_sym_u32_table_microprobe${mode}"
  TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" "${PTXAS_FLAGS[@]}" -DRP_CHOOSE_TABLE_MODE="$mode" "$SRC" -o "${BINS[$mode]}" >"$LOGDIR/mode${mode}.build.out" 2>"$LOGDIR/mode${mode}.build.err"
done
...
run_one() {
  ...
  local line="$(grep '^gridfp-choose-sym-u32-table-microprobe ' "$out" | tail -n1 || true)"; [[ -n "$line" ]] || { cat "$out" >&2 || true; cat "$err" >&2 || true; exit 3; }
...
for ((r=1;r<=REPEATS;++r)); do for pattern in 0 1; do case $(((r-1)%4)) in 0) order=(0 1 2 3);; 1) order=(1 2 3 0);; 2) order=(2 3 0 1);; *) order=(3 0 1 2);; esac; for mode in "${order[@]}"; do echo "=== choose table pattern=$pattern mode=$mode run $r/$REPEATS ===" >&2; run_one "$pattern" "$mode" "$r"; done; done; done
cat "$RESULT"
...
for pattern,name in (('0','middle'),('1','mirrored')):
 ...
 for mode,label in (('1','sym900'),('2','tri1740'),('3','full3364')):
  new=float(q[(pattern,mode)]['ns_per_call_median']); print(f'choose_{label}_{name}_speedup={old/new:.6f}x'); print(f'choose_{label}_{name}_delta_pct={(new/old-1)*100:.4f}%')
print('choose_table_mode0=full_2d_u64_6728B'); print('choose_table_mode1=symmetric_u32_900B'); print('choose_table_mode2=triangular_u32_1740B'); print('choose_table_mode3=full_2d_u32_3364B')
print('choose_sym_saved_bytes=5828'); print('choose_tri_saved_bytes=4988'); print('choose_full_u32_saved_bytes=3364'); print('choose_table_exact=1'); print(f'summary={dst}')
PY
