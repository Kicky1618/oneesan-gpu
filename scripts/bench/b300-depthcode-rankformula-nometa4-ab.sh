...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_nometa4_ab_n${N}}"
...
if [[ "$RUN_SELFTEST" == 1 ]]; then
  RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" PM_ACCUM="$PM_ACCUM" \
    bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankformula-slotrow32-selftest.sh" >"$LOGDIR/slotrow32.selftest.out" 2>"$LOGDIR/slotrow32.selftest.err"
  RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" PM_ACCUM="$PM_ACCUM" \
    bash "$ONEESAN_ROOT/scripts/bench/pattern10-depthcode-rankformula-nometa4-selftest.sh" >"$LOGDIR/nometa4.selftest.out" 2>"$LOGDIR/nometa4.selftest.err"
fi

slot_bin="$ONEESAN_BUILD_DIR/ab_rankformula_slotrow32_n${N}"
nometa_bin="$ONEESAN_BUILD_DIR/ab_rankformula_nometa4_n${N}"
N="$N" ARCH="$ARCH" OUT="$slot_bin" HIGH_CTX=warpstriped_delta_direct_affine_rankformula_cross5 \
  DEPTHCODE_DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" \
  RANKFORMULA_SLOTMETA=1 RANKFORMULA_SLOTROW32=1 TRANSPOSE_MODE="$TRANSPOSE_MODE" \
  PM_ACCUM="$PM_ACCUM" TERNARY_KEY4="$TERNARY_KEY4" PTXAS_VERBOSE="$RUN_PTXAS" \
  bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-depthcode-graph-batch.sh" >"$LOGDIR/slotrow32.build.out" 2>"$LOGDIR/slotrow32.build.err"
N="$N" ARCH="$ARCH" OUT="$nometa_bin" DEPTHCODE_DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" \
  RANKSTREAM_LUT_LOAD="$RANKSTREAM_LUT_LOAD" TRANSPOSE_MODE="$TRANSPOSE_MODE" \
  PM_ACCUM="$PM_ACCUM" TERNARY_KEY4="$TERNARY_KEY4" PTXAS_VERBOSE="$RUN_PTXAS" \
  bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-depthcode-rankformula-nometa4.sh" >"$LOGDIR/nometa4.build.out" 2>"$LOGDIR/nometa4.build.err"

field(){ local key="$1" line="$2"; sed -nE "s/(^|.*[[:space:]])${key}=([^[:space:]]+).*/\\2/p" <<<"$line" | tail -n1; }
printf 'mode\trepeat\tresidue\twall_s\tforward_high_s\treverse_high_s\n' >"$RESULT"
for mode in slotrow32 nometa4; do
  [[ "$mode" == slotrow32 ]] && bin="$slot_bin" || bin="$nometa_bin"
...
cat "$RESULT"
echo "b300-depthcode-rankformula-nometa4-ab OK n=$N repeats=$REPEATS result=$RESULT logs=$LOGDIR" >&2
