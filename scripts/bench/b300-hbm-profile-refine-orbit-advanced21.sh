...
for x in ORBIT_SPARSE64 ORBIT_SORTED ORBIT_CPASYNC_PAIR ORBIT_PRECTX_FORWARD ORBIT_PRECTX_REVERSE ORBIT_PRECTX_COMPACT ORBITCTA_FLAT ORBIT_QUAD_MLP ORBIT_QUAD_OVERLAP_LOCAL ORBIT_QUAD_SPARSE_DESC_MLP ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0 ORBIT_PRECTX_FLAT_BID ORBIT_PRECTX_FLAT_BID_FUSED ORBIT_PRECTX_WARPCOOP; do
  v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0/1" >&2; exit 2; }
done
case "$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES" in 0|64|128|256) ;; *) echo 'bad ORBIT_QUAD_CPASYNC_PREFETCH_BYTES' >&2; exit 2;; esac
...
# Every advanced candidate uses PAIR_MLP=1, so ILP1 is not a valid root.
case "$ORBIT_COL_ILP" in 2|4) ;; *) echo 'advanced orbit refine requires ORBIT_COL_ILP=2 or 4' >&2; exit 2;; esac
...
[[ "$ORBITCTA_FLAT_CHUNK" == 1 && "$ORBIT_QUAD_MLP" == 0 && "$ORBIT_QUAD_OVERLAP_LOCAL" == 0 && "$ORBIT_QUAD_LOCAL_DIRECT_MAX" == 0 && "$ORBIT_QUAD_SPARSE_DESC_MLP" == 0 && "$ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0" == 0 && "$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES" == 0 && "$ORBIT_QUAD_CPASYNC_GROUP_COLS" == 1 && "$ORBIT_PRECTX_FLAT_BID" == 0 && "$ORBIT_PRECTX_FLAT_BID_FUSED" == 0 && "$ORBIT_PRECTX_WARPCOOP" == 0 ]] || {
  echo 'PROFILE_IN must be the pre-advanced scheduler winner (chunk=1, quad/qol tuning=0, group=1, flat-bid/warpcoop=0)' >&2; exit 2;
}
...
N=21 ARCH="$ARCH" OUT="$CURRENT_BIN" PM_ACCUM="$PM_ACCUM" DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$ORBIT_SPARSE64" DIRECTGATHER_SORT_RANKS=0 \
  ...
  ORBITCTA_FLAT="$ORBITCTA_FLAT" ORBITCTA_FLAT_CHUNK=1 QUAD_MLP=0 QUAD_OVERLAP_LOCAL=0 QUAD_LOCAL_DIRECT_MAX=0 QUAD_SPARSE_DESC_MLP=0 \
  ...
  PRECTX_FLAT_BID=0 PRECTX_FLAT_BID_FUSED=0 PRECTX_WARPCOOP=0 PTXAS_VERBOSE=1 \
...
for line in open(pin):
 ...
 kv[k]=v.strip('"')
advanced_defaults={
 'ORBITCTA_FLAT_CHUNK':'1','ORBIT_QUAD_MLP':'0','ORBIT_QUAD_OVERLAP_LOCAL':'0','ORBIT_QUAD_LOCAL_DIRECT_MAX':'0','ORBIT_QUAD_SPARSE_DESC_MLP':'0',
...
baseline_qol={'ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0':'0','ORBIT_QUAD_CPASYNC_GROUP_COLS':'1','ORBIT_QUAD_CPASYNC_PREFETCH_BYTES':'0','ORBIT_PRECTX_WARPCOOP':'0'}
if best_name=='flat_compact_both':
 kv.update({'ORBITCTA_FLAT':'1','ORBITCTA_FLAT_CHUNK':'1','ORBITCTA_FLAT_BLOCKS_PER_SM':used_psm,
            ...
            'ORBIT_QUAD_MLP':'0','ORBIT_QUAD_OVERLAP_LOCAL':'0','ORBIT_QUAD_LOCAL_DIRECT_MAX':'0','ORBIT_QUAD_SPARSE_DESC_MLP':'0',**baseline_qol})
 ...
 kv.update({'ORBITCTA_FLAT':'1','ORBITCTA_FLAT_CHUNK':q['ORBITCTA_FLAT_CHUNK'],'ORBITCTA_FLAT_BLOCKS_PER_SM':q['ORBITCTA_FLAT_BLOCKS_PER_SM'],
            ...
            'ORBIT_PRECTX_FLAT_BID':'0','ORBIT_PRECTX_FLAT_BID_FUSED':'0','ORBIT_PRECTX_WARPCOOP':q.get('ORBIT_PRECTX_WARPCOOP','0'),
            'ORBIT_QUAD_MLP':q['ORBIT_QUAD_MLP'],'ORBIT_QUAD_OVERLAP_LOCAL':q['ORBIT_QUAD_OVERLAP_LOCAL'],'ORBIT_QUAD_LOCAL_DIRECT_MAX':q['ORBIT_QUAD_LOCAL_DIRECT_MAX'],'ORBIT_QUAD_SPARSE_DESC_MLP':'0',
            'ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0':q.get('ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0','0'),'ORBIT_QUAD_CPASYNC_GROUP_COLS':q.get('ORBIT_QUAD_CPASYNC_GROUP_COLS','1'),'ORBIT_QUAD_CPASYNC_PREFETCH_BYTES':q.get('ORBIT_QUAD_CPASYNC_PREFETCH_BYTES','0')})
...
cat "$SUMMARY"
cat "$PROFILE_OUT"
echo "advanced orbit refine OK input=$PROFILE_IN output=$PROFILE_OUT summary=$SUMMARY isolated_later_qol=1" >&2
