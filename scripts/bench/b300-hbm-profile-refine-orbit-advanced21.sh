...
for x in ORBIT_SPARSE64 ORBIT_SORTED ORBIT_CPASYNC_PAIR ORBIT_PRECTX_FORWARD ORBIT_PRECTX_REVERSE ORBIT_PRECTX_COMPACT ORBITCTA_FLAT ORBIT_QUAD_MLP ORBIT_QUAD_OVERLAP_LOCAL ORBIT_QUAD_SPARSE_DESC_MLP ORBIT_PRECTX_FLAT_BID ORBIT_PRECTX_FLAT_BID_FUSED; do
...
[[ "$ORBITCTA_FLAT_CHUNK" == 1 && "$ORBIT_QUAD_MLP" == 0 && "$ORBIT_QUAD_OVERLAP_LOCAL" == 0 && "$ORBIT_QUAD_LOCAL_DIRECT_MAX" == 0 && "$ORBIT_QUAD_SPARSE_DESC_MLP" == 0 && "$ORBIT_PRECTX_FLAT_BID" == 0 && "$ORBIT_PRECTX_FLAT_BID_FUSED" == 0 ]] || {
  echo 'PROFILE_IN must be the pre-advanced scheduler winner (chunk=1, quad=0, qsd=0, flat-bid=0)' >&2; exit 2;
}
