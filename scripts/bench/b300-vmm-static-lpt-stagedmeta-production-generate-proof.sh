...
for stale in 'std::atomic<int>next{0}' 'next.fetch_add(1,std::memory_order_relaxed)' 'for(auto&c:ctx)c.stage_group_meta(staged_group_meta);' 'copy_mode=H2D_once_then_D2D_per_group'; do
  if grep -Fq "$stale" "$OUT"; then
    echo "static LPT generated source still contains dynamic/replicated artifact: $stale" >&2
    exit 3
  fi
done

echo "b300-vmm-static-lpt-stagedmeta-production-generate-proof OK scheduler=static_lpt work_stealing=0 metadata_replication=0 staged_h2d_once_local=1 per_group_meta_copy=D2D_sync expected_default_groups=16384 expected_default_group_min=2044 expected_default_group_max=2052 expected_default_max_mib_per_gpu=27.271911621 expected_default_total_h2d_gib=0.212646484375 static_vs_replicated_h2d_reduction=8x stage_within_hbm_reserve=1"
