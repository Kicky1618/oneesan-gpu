...
for x in ORBIT_SPARSE64 ORBIT_CPASYNC_PAIR ORBIT_PRECTX_FORWARD ORBIT_PRECTX_REVERSE ORBIT_PRECTX_COMPACT ORBIT_PRECTX_FLAT_BID ORBIT_PRECTX_FLAT_BID_FUSED PM_ACCUM; do
  v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0/1" >&2; exit 2; }
done
[[ "$ORBIT_PRECTX_FLAT_BID_FUSED" == 0 || "$ORBIT_PRECTX_FLAT_BID" == 1 ]] || exit 2
...
printf 'fuse\trepeat\tresidue\twall_s\tforward_high_s\treverse_high_s\thigh_s\tavg_gpu_util_pct\tavg_memctrl_util_pct\tmax_memctrl_util_pct\n' >"$RESULT"
run_one(){
...
PY
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$fuse" "$rep" "$residue" "$(field wall_s "$line")" "$fh" "$rh" "$high" "$ag" "$am" "$mm" >>"$RESULT"
}
for fuse in 0 1; do for ((r=1;r<=REPEATS;++r)); do run_one "$fuse" "$r"; done; done
cat "$RESULT"
...
for f in ('0','1'):
 ...
 z[f]=(statistics.median(float(r['wall_s']) for r in g),statistics.median(float(r['high_s']) for r in g))
best=min(z,key=lambda f:z[f][0])
lines=open(pin).read().splitlines(); out=[]; seen=False
for line in lines:
 if line.startswith('ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP='):
  out.append('ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP='+best); seen=True
 elif line.startswith('ORBIT_PROFILE='):
  val=line.split('=',1)[1]
  val=re.sub(r'_df[01]$','',val)
  out.append('ORBIT_PROFILE='+val+('_df1' if best=='1' else ''))
 else: out.append(line)
if not seen: out.append('ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP='+best)
out += [f'ORBIT_DYNAMIC_FUSE_WALL_S={z[best][0]:.9f}',f'ORBIT_DYNAMIC_FUSE_HIGH_S={z[best][1]:.9f}']
open(pout,'w').write('\n'.join(out)+'\n')
for f in ('0','1'): print('DYNAMIC_FUSE',f,f'wall_s={z[f][0]:.6f}',f'high_s={z[f][1]:.6f}')
print('BEST_DYNAMIC_FUSE='+best,f'profile_file={pout}')
PY
cat "$PROFILE_OUT"
echo "dynamic fuse refinement OK input=$PROFILE_IN output=$PROFILE_OUT" >&2
