...
build_one() {
  ...
  N="$N" OUT="$bin" HIGH_CTX=warpstriped_delta_direct_affine_rankchunk32_cross5 \
    ...
    RANKCHUNK32_DIRECT3="$direct3" RANKCHUNK32_BYTEPACK="$RANKCHUNK32_BYTEPACK" \
    RANKCHUNK32_ALIGN32="$RANKCHUNK32_ALIGN32" RANKCHUNK32_BLOCK64="$RANKCHUNK32_BLOCK64" \
    ...
    >"$LOGDIR/direct3_${direct3}.build.out" 2>"$LOGDIR/direct3_${direct3}.build.err"
}

printf 'direct3\trepeat\tresidue\twall_s\tforward_high_s\treverse_high_s\tforward_low_s\treverse_low_s\ttranspose_s\n' >"$RESULT"

run_one() {
  local direct3="$1" bin="$2" rep="$3"
  local so="$LOGDIR/direct3_${direct3}_r${rep}.out" se="$LOGDIR/direct3_${direct3}_r${rep}.err"
  ...
  [[ -n "$line" ]] || { echo "direct3=$direct3 missing residue" >&2; exit 3; }
  residue="$(field residue "$line")"; wall="$(field wall_s "$line")"
  [[ "$residue" == "$EXPECT" ]] || {
    echo "direct3=$direct3 residue mismatch got=$residue expected=$EXPECT" >&2; exit 4
  }
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$direct3" "$rep" "$residue" "$wall" "${fh:-NA}" "${rh:-NA}" \
    "${fl:-NA}" "${rl:-NA}" "${ts:-NA}" >>"$RESULT"
}

for direct3 in 0 1; do
  bin="$ONEESAN_BUILD_DIR/ab_depthcode_rankchunk32_direct3_${direct3}_${RANKSTREAM_LUT_LOAD}_${DEPTHCODE_DECODE_LOAD}_${TRANSPOSE_MODE}_n${N}"
  echo "=== build rankchunk32 direct3=$direct3 ===" >&2
  build_one "$direct3" "$bin"
  for ((r = 1; r <= REPEATS; ++r)); do
    echo "=== run rankchunk32 direct3=$direct3 $r/$REPEATS ===" >&2
    run_one "$direct3" "$bin" "$r"
  done
done
...
out=[]
for mode in ('0','1'):
    group=[r for r in rows if r['direct3']==mode]
    z={'direct3':mode,'repeats':str(len(group))}
    for metric in metrics:
        xs=[float(r[metric]) for r in group if r[metric] != 'NA']
        z[metric]=f'{statistics.median(xs):.9f}' if xs else 'NA'
    out.append(z)
with open(dst,'w',newline='') as f:
    w=csv.DictWriter(f,fieldnames=('direct3','repeats',*metrics),delimiter='\t')
    w.writeheader(); w.writerows(out)
q={r['direct3']:r for r in out}
def speedup(metric):
    if q['0'][metric]=='NA' or q['1'][metric]=='NA': return
    print(f'rankchunk32_direct3_{metric}_speedup={float(q["0"][metric])/float(q["1"][metric]):.6f}x')
...
if all(q[x][m] != 'NA' for x in ('0','1') for m in ('forward_high_s','reverse_high_s')):
    ...
    print(f'rankchunk32_direct3_total_high_speedup={old/new:.6f}x')
print('rankmask_allowed=0,1,2,3,5,7')
print('rankmask_upper_bits_zero=1')
print('rankmask_max_popcount=3')
print('direct3_replaces=ffs_plus_bitclear_loop')
print(f'summary={dst}')
PY
