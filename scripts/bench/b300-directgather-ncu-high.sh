...
COL_ILP="${COL_ILP:-2}"; PAIR_MLP="${PAIR_MLP:-1}"; DEPTHMAJOR="${DEPTHMAJOR:-1}"
...
BIN="${BIN:-$ONEESAN_BUILD_DIR/b300_directgather_ncu_n${N}_c${COL_ILP}_p${PAIR_MLP}}"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_directgather_ncu_n${N}_c${COL_ILP}_p${PAIR_MLP}}"
...
N="$N" ARCH="$ARCH" OUT="$BIN" COL_ILP="$COL_ILP" DEPTHMAJOR="$DEPTHMAJOR" \
  ...
  PM_ACCUM="$PM_ACCUM" MAXRREGCOUNT="${MAXRREGCOUNT:-0}" PTXAS_VERBOSE=1 \
...
# utilization.memory is a useful trend signal, but these sections expose actual
...
echo "config: col_ilp=$COL_ILP pair_mlp=$PAIR_MLP window4=$WINDOW4 depthmajor=$DEPTHMAJOR pm_accum=$PM_ACCUM high_grid=${HIGH_GX}x${HIGH_GY}" >&2
