...
[[ "$(grep -c '^exec bash ' "$FIRST")" == 1 && "$(grep -c '^exec bash ' "$EXACT")" == 1 ]] || { echo 'latest entrypoints must tail-exec Stage P exactly once' >&2; exit 3; }
python3 - "$FIRST" <<'PY'
from pathlib import Path
import sys
s=Path(sys.argv[1]).read_text()
p=s.find('b300x8-grand-latest-preflight.sh')
e=s.find('exec bash "$ONEESAN_ROOT/scripts/run/b300x8-grand-firstpass-stagep.sh"')
if p<0 or e<0 or p>=e: raise SystemExit('environment preflight must run before Stage-P firstpass exec')
PY
echo 'b300-grand-latest-entrypoint-preflight OK latest_stage=P firstpass=1 exact_promotion=1 environment_preflight=1 preflight_default_on=1 tail_exec=1 gpu_work=0'
