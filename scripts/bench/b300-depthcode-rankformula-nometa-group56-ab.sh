...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_rankformula_nometa_group56_ab_n${N}_${TRANSPOSE_MODE}}"
...
for spec in 'group59 0' 'group56 1'; do
  read -r label group56 <<<"$spec"
...
for k in ('wall_s','forward_high_s','reverse_high_s','total_high_s'):
    if q['group59'][k] and q['group56'][k]:print(f'group56_{k}_speedup={q["group59"][k]/q["group56"][k]:.6f}x')
if run_ptxas=='1':
...
print('group59_pack_bits=59')
print('group59_self_index_bits=14')
print('group59_abstract_lut_bytes=86058')
print('group56_pack_bits=56')
print('group56_self_index_bits=0')
...
echo "b300-depthcode-rankformula-nometa-group56-ab OK n=$N repeats=$REPEATS block=16 result=$RESULT" >&2
