...
for g in $GUARD_LIST; do
  case "$g" in bb|pb|bp|pp) ;; *) echo "bad Stage-L guard profile=$g" >&2; exit 2;; esac
  seen=0; for old in "${profiles[@]}"; do [[ "$old" == "$g" ]] && seen=1; done; ((seen)) || profiles+=("$g")
done
has_bb=0; for g in "${profiles[@]}"; do [[ "$g" == bb ]] && has_bb=1; done
((has_bb)) || { echo 'Stage-L GUARD_LIST must include bb baseline' >&2; exit 2; }
GUARD_LIST="${profiles[*]}"

# Known exact residues from the earlier staged chain.
...
for k in B300_HYBRID8_NEXTSELF_STAGE_E_CORE_ROWS B300_HYBRID8_NEXTSELF_STAGE_E_CORE_RESIDUE \
  ...
  B300_HYBRID8_NEXTSELF_FINAL_STAGE_ROWS B300_HYBRID8_NEXTSELF_FINAL_STAGE_RESIDUE; do
  [[ -n "${!k+x}" ]] || { echo "Stage-F env missing $k" >&2; exit 3; }
done

# Resolve the exact J/K upstream identity and its last validated row.
# shellcheck disable=SC1090
source "$UPSTREAM_PREPARE_ENV"
UPSTREAM_KIND=""; SW=""; SD=""; SE=""; MW=""; MD=""; ME=""
if [[ "${B300_STAGEK_PREPARED:-0}" == 1 ]]; then
  ...
  echo 'Stage-L upstream prepare is not Stage J/K' >&2; exit 3
fi
...
run_stage(){
  local rows="$1" guards="$2" threads="$3" repeats="$4" tag="$5"
  local p="${PREFIX}.${tag}.r${rows}" log="${p}.log" envf="${p}_winner.env"
  STAGE_F_ENV="$STAGE_F_ENV" UPSTREAM_PREPARE_ENV="$UPSTREAM_PREPARE_ENV" \
    ARCH="$ARCH" MOD="$MOD" ROWS="$rows" TARGET_MIB="$TARGET_MIB" MAX_WINDOW="$MAX_WINDOW" NGPU="$NGPU" \
    GUARD_LIST="$guards" THREADS_LIST="$threads" REPEATS="$repeats" PREFIX="$p" WINNER_ENV="$envf" \
    bash "$SWEEP" | tee "$log" >&2
  grep -Fq 'b300_stagel_exact_match=1' "$log" || { echo "Stage-L exact gate missing rows=$rows" >&2; exit 4; }
  grep -Fq "b300_stagel_ngpu=$NGPU" "$log" || { echo "Stage-L NGPU marker missing rows=$rows" >&2; exit 4; }
  [[ -s "$envf" ]] || exit 4
  printf '%s\n' "$envf"
}
...
if [[ "$B300_STAGEL_BEST_ENABLED" == 1 && "$B300_STAGEL_BASE_SPILL_FREE" == 1 && "$B300_STAGEL_SPILL_FREE" == 1 && "$(passes "$B300_STAGEL_SPEEDUP")" == 1 ]]; then
  ...
  for rows in "${stage_validate_rows[@]}"; do
    ((stage+=1))
    CURRENT_ENV="$(run_stage "$rows" "bb $SELECTED" "$vt" "$VALIDATE_REPEATS" "validate${stage}")"
    # shellcheck disable=SC1090
    source "$CURRENT_ENV"
    check_residue "$rows" "$B300_STAGEL_RESIDUE"
    [[ "$B300_STAGEL_NGPU" == "$NGPU" && "$B300_STAGEL_UPSTREAM_KIND" == "$UPSTREAM_KIND" ]] || exit 4
    if [[ "$B300_STAGEL_PROFILE" != "$SELECTED" || "$B300_STAGEL_BEST_ENABLED" != 1 || "$B300_STAGEL_BASE_SPILL_FREE" != 1 || "$B300_STAGEL_SPILL_FREE" != 1 || "$(passes "$B300_STAGEL_SPEEDUP")" != 1 ]]; then
      VALIDATED=0; break
    fi
    bt="$B300_STAGEL_BASE_THREADS"; tt="$B300_STAGEL_THREADS"; vt="$bt"; [[ "$tt" == "$bt" ]] || vt+=" $tt"
  done
...
source "$CURRENT_ENV"
if [[ "$VALIDATED" == 1 ]]; then
  FINAL_PROFILE="$SELECTED"; FINAL_SG="$B300_STAGEL_SELF_GUARD"; FINAL_MG="$B300_STAGEL_MATE_GUARD"
  FINAL_BIN="$B300_STAGEL_BIN"; FINAL_THREADS="$B300_STAGEL_THREADS"; FINAL_WALL="$B300_STAGEL_WALL_S"; FINAL_HIGH="$B300_STAGEL_HIGH_S"; FINAL_SPEED="$B300_STAGEL_SPEEDUP"
else
  FINAL_PROFILE=bb; FINAL_SG=branch; FINAL_MG=branch
  FINAL_BIN="$B300_STAGEL_BASE_BIN"; FINAL_THREADS="$B300_STAGEL_BASE_THREADS"; FINAL_WALL="$B300_STAGEL_BASE_WALL_S"; FINAL_HIGH="$B300_STAGEL_BASE_HIGH_S"; FINAL_SPEED=1.000000000
fi
{
