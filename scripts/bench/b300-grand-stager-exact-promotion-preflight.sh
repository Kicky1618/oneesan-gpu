...
wrong_up="$tmp/wrong-up.env"; cp "$valid" "$wrong_up"; sed -i 's/B300_GRAND_SELECTED_STAGER_UPSTREAM_KIND=stageq/B300_GRAND_SELECTED_STAGER_UPSTREAM_KIND=stagep/' "$wrong_up"; run_bad "$wrong_up" wrong-up 'Stage R ignored maximal accepted upstream'
...
echo 'b300-grand-stager-exact-promotion-preflight OK legacy_schema3=1 stager_requires_stagen=1 maximal_upstream=Q,P,O,N low_tuple_change_required=1 stageq_high_tuple_preserved=1 summary_match=1 manifest_gate=1 rejection_before_promoter=1 gpu_work=0'
