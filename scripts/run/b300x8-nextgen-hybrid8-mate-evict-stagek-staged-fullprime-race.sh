...
if [[ "$RUN_STAGED" == 1 ]]; then
  ...
  STAGE_F_ENV="$STAGE_F_ENV" STAGEJ_WINNER_ENV="$STAGEJ_WINNER_ENV" STAGEJ_PREPARE_ENV="$STAGEJ_PREPARE_ENV" \
    ARCH="$ARCH" MOD="${MOD:-4294967291}" TARGET_MIB="$TARGET_MIB" MAX_WINDOW="$MAX_WINDOW" EVICT_LIST="$EVICT_LIST" MIN_SPEEDUP="$MIN_SPEEDUP" \
...
if [[ "$RUN_STAGED" == 1 ]]; then
  ...
  sha256sum "$WINNER_ENV" "$STAGE_F_ENV" "$STAGEJ_WINNER_ENV" "$STAGEJ_PREPARE_ENV" "$B300_STAGEK_FINAL_BIN" "$B300_STAGEK_CONTROL_BIN" >"$tmp"
  mv "$tmp" "$MANIFEST"
else
...
if [[ "$PREPARE_ONLY" == 1 ]]; then
  cat "$PREPARE_ENV"
  echo "STAGE K PREPARED mate_evict=${B300_STAGEK_FINAL_MATE_EVICT} baseline=${B300_STAGEK_BASE_MATE_EVICT} speedup=${B300_STAGEK_FINAL_SPEEDUP}x env=$PREPARE_ENV" >&2
  exit 0
fi
