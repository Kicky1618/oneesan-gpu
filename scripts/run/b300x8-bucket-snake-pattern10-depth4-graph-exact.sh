...
TRANSPOSE_MODE="${TRANSPOSE_MODE:-pipeline}"; PM_ACCUM="${PM_ACCUM:-0}"; P10_DECODE="${P10_DECODE:-lut}"; HIGH_CTX="${HIGH_CTX:-thread}"; TERNARY_KEY4="${TERNARY_KEY4:-1}"
...
WORK_TAG="pattern10_depth4_graph_${P10_DECODE}_${HIGH_CTX}_${TRANSPOSE_MODE}"; if [[ "$PM_ACCUM" == 1 ]]; then WORK_TAG="${WORK_TAG}_pm"; fi; if [[ "$TERNARY_KEY4" == 0 ]]; then WORK_TAG="${WORK_TAG}_keyscalar"; else WORK_TAG="${WORK_TAG}_key4"; fi
...
if [[ ! -x "$BIN" ]]; then
  echo "$BIN not found; building pattern10 depth4 graph batch n=$N decode=$P10_DECODE high_ctx=$HIGH_CTX transpose=$TRANSPOSE_MODE pm=$PM_ACCUM ternary_key4=$TERNARY_KEY4" >&2
  N="$N" OUT="$BIN" P10_DECODE="$P10_DECODE" HIGH_CTX="$HIGH_CTX" TRANSPOSE_MODE="$TRANSPOSE_MODE" PM_ACCUM="$PM_ACCUM" TERNARY_KEY4="$TERNARY_KEY4" bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-depth4-graph-batch.sh"
fi
