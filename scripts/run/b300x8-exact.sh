...
N="${1:-27}"
if (( $# > 0 )); then shift; fi
# Ask for a wide scratch/cache budget. The backend caps this to
# cudaMemGetInfo()-GRIDFP_VRAM_RESERVE_MIB after authoritative state is resident.
...
for name in MAIN_MATE_CACHE MAIN_PULL BLOCK_PULL BLOCK_MATE_CACHE REBUILD; do
  value="${!name}"
...
if [[ "$REBUILD" == 1 || ! -x "$BIN" ]]; then
  echo "building optimized full-pull batch binary for n=$N mainpull=$MAIN_PULL blockpull=$BLOCK_PULL blockmate=$BLOCK_MATE_CACHE" >&2
...
echo "exact batch n=$N gpus=$NGPU target_mib=$TARGET_MIB max_window=$MAX_WINDOW main_mate_cache=$MAIN_MATE_CACHE main_pull=$MAIN_PULL block_pull=$BLOCK_PULL block_mate_cache=$BLOCK_MATE_CACHE binary=$BIN" >&2
