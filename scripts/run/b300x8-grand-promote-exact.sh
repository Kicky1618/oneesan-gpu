...
# Remove stale tuning knobs from the caller before reconstructing exactly the
# runtime shape that won the single-pass race.
...
if [[ "$DRY_RUN" == 1 ]]; then
  printf 'B300 GRAND EXACT PROMOTION VALIDATED command=' >&2
  printf '%q ' "${cmd[@]}" >&2
  printf '\n' >&2
...
echo "=== B300 GRAND EXACT PROMOTION backend=$B300_GRAND_SELECTED_BACKEND profile=$B300_GRAND_SELECTED_PROFILE sha=${BIN_SHA:0:12} cached_smoke_prime=$B300_GRAND_SELECTED_SMOKE_PRIME ===" >&2
