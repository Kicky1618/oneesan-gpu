...
WORK_TAG="zero_graph_${ZERO_HIGH_PLAN}_${TRANSPOSE_MODE}"; if [[ "$PM_ACCUM" == 1 ]]; then WORK_TAG="${WORK_TAG}_pm"; fi
...
if [[ ! -x "$BIN" ]]; then
  echo "$BIN not found; building closure-zero graph batch binary n=$N high_plan=$ZERO_HIGH_PLAN transpose=$TRANSPOSE_MODE pm=$PM_ACCUM" >&2
  N="$N" OUT="$BIN" ZERO_HIGH_PLAN="$ZERO_HIGH_PLAN" TRANSPOSE_MODE="$TRANSPOSE_MODE" PM_ACCUM="$PM_ACCUM" bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-zero-graph-batch.sh"
fi
