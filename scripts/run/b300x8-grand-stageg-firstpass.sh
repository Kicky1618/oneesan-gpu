...
STAGEG_META="${STAGEG_META:-${PREFIX}.meta}"
...
[[ -x "$BEST_BIN" ]] || exit 4
BEST_SHA="$(sha256sum "$BEST_BIN" | awk '{print $1}')"; SHA12="${BEST_SHA:0:12}"; BEST_WORK="$WORK_ROOT/b300_exact_singlepass_${BEST}_${BEST_PROFILE}_${SHA12}_n27"; CHECKPOINT="$BEST_WORK/checkpoint.json"
[[ -s "$CHECKPOINT" ]] || exit 4
...
d=json.load(open(cp))
if int(d.get('n',-1)) != 27: raise SystemExit('Stage-G checkpoint n mismatch')
if d.get('solver_fingerprint') != {'schema':3,'binary_sha256':bsha,'profile_sha256':psha}:
    raise SystemExit('Stage-G checkpoint fingerprint mismatch')
r=d.get('residues',{}).get(str(int(prime)))
if not r or int(r.get('residue',-1)) != int(residue): raise SystemExit('Stage-G checkpoint smoke residue mismatch')
PY

{
  printf 'schema=1\n'
  printf 'stageg_validated=1\n'
  printf 'base_selected_env=%s\n' "$BASE_SELECTED_ENV"
  printf 'stageg_prepare_env=%s\n' "$STAGEG_PREPARE_ENV"
  printf 'stageg_width=%s\n' "$B300_STAGEG_PREPARED_WIDTH"
  printf 'stageg_distance=%s\n' "$B300_STAGEG_PREPARED_DISTANCE"
...
{
  # Standard grand contract: hardened promotion/verifier runners consume this
  # regardless of whether the winner came from the base grand race or legacy G.
  ...
  # Stage-G-specific provenance remains available to distinguish this branch.
  ...
  printf 'B300_GRAND_STAGEG_SELECTED_RACE_RESULT_SHA256=%q\n' "$FINAL_RESULT_SHA"
} >"$SELECTED_ENV"
cat "$SELECTED_ENV"
echo "b300x8-grand-stageg-firstpass OK backend=$BEST profile=$BEST_PROFILE wall_s=$BEST_WALL stageg_width=$B300_STAGEG_PREPARED_WIDTH stageg_distance=$B300_STAGEG_PREPARED_DISTANCE selected_env=$SELECTED_ENV normalized_contract=1" >&2
