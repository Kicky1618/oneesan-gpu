...
TRANSPOSE_MODE="${TRANSPOSE_MODE:-pipeline}"; PM_ACCUM="${PM_ACCUM:-0}"; HIGH_CTX="${HIGH_CTX:-thread}"
...
SUFFIX="_${HIGH_CTX}_${TRANSPOSE_MODE}"; if [[ "$PM_ACCUM" == 1 ]]; then SUFFIX="${SUFFIX}_pm"; fi
BIN="${BIN:-$ONEESAN_BUILD_DIR/oneesan_cuda_gridfp_b300_bucket_snake_onepass_pattern10_depth8_graph_batch${SUFFIX}_n${N}}"
WORK_TAG="pattern10_depth8_graph_${HIGH_CTX}_${TRANSPOSE_MODE}"; if [[ "$PM_ACCUM" == 1 ]]; then WORK_TAG="${WORK_TAG}_pm"; fi
...
if [[ ! -x "$BIN" ]]; then
  echo "$BIN not found; building pattern10 depth8 graph batch binary n=$N high_ctx=$HIGH_CTX transpose=$TRANSPOSE_MODE pm=$PM_ACCUM" >&2
  N="$N" OUT="$BIN" HIGH_CTX="$HIGH_CTX" TRANSPOSE_MODE="$TRANSPOSE_MODE" PM_ACCUM="$PM_ACCUM" bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-depth8-graph-batch.sh"
fi
