...
TRANSPOSE_MODE="${TRANSPOSE_MODE:-pipeline}"; HIGH_CTX="${HIGH_CTX:-thread}"; DEPTHCODE_DECODE_LOAD="${DEPTHCODE_DECODE_LOAD:-global}"; PM_ACCUM="${PM_ACCUM:-0}"; TERNARY_KEY4="${TERNARY_KEY4:-1}"; BUCKET_THREADS="${BUCKET_THREADS:-256}"
...
WORK_TAG="pattern10_depthcode_graph_payload_${HIGH_CTX}_${TRANSPOSE_MODE}"; [[ "$DEPTHCODE_DECODE_LOAD" == ldg ]] && WORK_TAG="${WORK_TAG}_ldg"; [[ "$PM_ACCUM" == 1 ]] && WORK_TAG="${WORK_TAG}_pm"; if [[ "$TERNARY_KEY4" == 0 ]]; then WORK_TAG="${WORK_TAG}_keyscalar"; else WORK_TAG="${WORK_TAG}_key4"; fi; WORK_TAG="${WORK_TAG}_t${BUCKET_THREADS}"
...
if [[ ! -x "$BIN" ]]; then
  echo "$BIN not found; building pattern10 depthcode Graph batch n=$N decode=payload high_ctx=$HIGH_CTX decode_load=$DEPTHCODE_DECODE_LOAD transpose=$TRANSPOSE_MODE pm=$PM_ACCUM ternary_key4=$TERNARY_KEY4 threads=$BUCKET_THREADS" >&2
...
export BUCKET_TRANSPOSE_CHUNK_MIB BUCKET_RESERVE_MIB BUCKET_THREADS
