...
TRANSPOSE_MODE="${TRANSPOSE_MODE:-pipeline}"; HIGH_CTX="${HIGH_CTX:-thread}"; DEPTHCODE_DECODE_LOAD="${DEPTHCODE_DECODE_LOAD:-global}"; DEPTHCODE_SOURCE_KEY="${DEPTHCODE_SOURCE_KEY:-scalar}"; PM_ACCUM="${PM_ACCUM:-0}"; TERNARY_KEY4="${TERNARY_KEY4:-1}"
...
SUFFIX="_payload_${HIGH_CTX}_${TRANSPOSE_MODE}"; [[ "$DEPTHCODE_DECODE_LOAD" == ldg ]] && SUFFIX="${SUFFIX}_ldg"; [[ "$DEPTHCODE_SOURCE_KEY" == key4 ]] && SUFFIX="${SUFFIX}_srckey4"; [[ "$PM_ACCUM" == 1 ]] && SUFFIX="${SUFFIX}_pm"; [[ "$TERNARY_KEY4" == 0 ]] && SUFFIX="${SUFFIX}_keyscalar"
BIN="${BIN:-$ONEESAN_BUILD_DIR/oneesan_cuda_gridfp_b300_bucket_snake_onepass_pattern10_depthcode_graph_batch${SUFFIX}_n${N}}"
WORK_TAG="pattern10_depthcode_graph_payload_${HIGH_CTX}_${TRANSPOSE_MODE}"; [[ "$DEPTHCODE_DECODE_LOAD" == ldg ]] && WORK_TAG="${WORK_TAG}_ldg"; [[ "$DEPTHCODE_SOURCE_KEY" == key4 ]] && WORK_TAG="${WORK_TAG}_srckey4"; [[ "$PM_ACCUM" == 1 ]] && WORK_TAG="${WORK_TAG}_pm"; if [[ "$TERNARY_KEY4" == 0 ]]; then WORK_TAG="${WORK_TAG}_keyscalar"; else WORK_TAG="${WORK_TAG}_key4"; fi; WORK_TAG="${WORK_TAG}_t${BUCKET_THREADS}_gx${BUCKET_GRID_X}_gy${BUCKET_GRID_Y}"
...
if [[ ! -x "$BIN" ]]; then
  echo "$BIN not found; building pattern10 depthcode Graph batch n=$N decode=payload high_ctx=$HIGH_CTX decode_load=$DEPTHCODE_DECODE_LOAD source_key=$DEPTHCODE_SOURCE_KEY transpose=$TRANSPOSE_MODE pm=$PM_ACCUM ternary_key4=$TERNARY_KEY4 threads=$BUCKET_THREADS gx=$BUCKET_GRID_X gy=$BUCKET_GRID_Y" >&2
  N="$N" OUT="$BIN" HIGH_CTX="$HIGH_CTX" DEPTHCODE_DECODE_LOAD="$DEPTHCODE_DECODE_LOAD" DEPTHCODE_SOURCE_KEY="$DEPTHCODE_SOURCE_KEY" TRANSPOSE_MODE="$TRANSPOSE_MODE" PM_ACCUM="$PM_ACCUM" TERNARY_KEY4="$TERNARY_KEY4" bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-depthcode-graph-batch.sh"
fi
