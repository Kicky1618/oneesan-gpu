...
for rel in \
  ...
  scripts/bench/b300-grand-stager-firstpass-preflight.sh; do
  [[ -s "$ONEESAN_ROOT/$rel" ]] || { echo "missing latest Stage-R artifact=$rel" >&2; exit 3; }
done
if [[ "$RUN_LATEST_PREFLIGHT" == 1 ]]; then
  bash "$ONEESAN_ROOT/scripts/run/b300x8-grand-latest-preflight.sh" "$N"
fi
exec bash "$ONEESAN_ROOT/scripts/run/b300x8-grand-firstpass-stager.sh" "$@"
