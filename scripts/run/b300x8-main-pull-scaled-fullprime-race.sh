...
[[ -f "$WINNER_ENV" && -f "$SWEEP_RESULT" && -f "$SWEEP_LOGDIR/binaries.tsv" ]] || {
  echo 'missing scaled main-pull sweep artifacts; use RUN_SWEEP=1' >&2; exit 3;
}
...
# If calibration artifacts are reused, require the current binaries to be the
# exact files recorded by the sweep.  This avoids silently mixing a stale env
# with a newly rebuilt executable after a launch-transform change.
