...
BIN="${BIN:-$ONEESAN_BUILD_DIR/oneesan_cuda_gridfp_b300_bucket_snake_onepass_pattern10_graph_batch${SUFFIX}_n${N}}"
WORK_TAG="pattern10_graph_${TRANSPOSE_MODE}"; if [[ "$PM_ACCUM" == 1 ]]; then WORK_TAG="${WORK_TAG}_pm"; fi; if [[ "$TERNARY_KEY4" == 0 ]]; then WORK_TAG="${WORK_TAG}_keyscalar"; fi
WORK_DIR="${WORK_DIR:-$ONEESAN_ROOT/work/b300_bucket_snake_${WORK_TAG}_exact_n${N}}"
if (( NGPU != 8 )); then echo "pattern10 graph snake backend currently requires NGPU=8" >&2; exit 2; fi
...
if [[ ! -x "$BIN" ]]; then
  echo "$BIN not found; building pattern10 graph batch binary n=$N transpose=$TRANSPOSE_MODE pm=$PM_ACCUM ternary_key4=$TERNARY_KEY4" >&2
  N="$N" OUT="$BIN" TRANSPOSE_MODE="$TRANSPOSE_MODE" PM_ACCUM="$PM_ACCUM" TERNARY_KEY4="$TERNARY_KEY4" bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-graph-batch.sh"
fi
