...
WORK_TAG="pattern10_graph_${TRANSPOSE_MODE}"; if [[ "$PM_ACCUM" == 1 ]]; then WORK_TAG="${WORK_TAG}_pm"; fi
...
if [[ ! -x "$BIN" ]]; then
  echo "$BIN not found; building pattern10 graph batch binary n=$N transpose=$TRANSPOSE_MODE pm=$PM_ACCUM" >&2
  N="$N" OUT="$BIN" TRANSPOSE_MODE="$TRANSPOSE_MODE" PM_ACCUM="$PM_ACCUM" bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-pattern10-graph-batch.sh"
fi
