...
required_files=(
  ...
  scripts/run/b300x8-grand-promote-exact-staget.sh
  ...
  scripts/bench/b300-grand-staget-firstpass-preflight.sh
  scripts/bench/b300-grand-staget-exact-promotion-preflight.sh
...
printf '%s\n' "$TOPO"
printf 'b300_latest_preflight=OK stage=T head=%s dirty=%s ngpu=%s target_mib=%s reserve_mib=%s min_gpu_free_mib=%s host_available_mib=%s driver=%s full_p2p=%s strict_b300_model=%s\n' \
