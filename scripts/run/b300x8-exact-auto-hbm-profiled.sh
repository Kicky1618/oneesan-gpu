...
if [[ "$ORBITCTA_FLAT_DYNAMIC" == 1 ]]; then
  ...
  if [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP" == 1 ]]; then [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2" == 1 ]] || { echo 'PIPE2 producer warp requires PIPE2=1' >&2; exit 2; }; fi
  ...
  [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP" == 0 && "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_PRECTX_WARPCOOP" == 0 ]] || { echo 'PIPE2 producer modes require dynamic queue' >&2; exit 2; }
fi
...
if has orbit_tuned && [[ "$REBUILD" == 1 || ! -x "$ORBIT_BIN" ]]; then
  ...
  if [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP" == 1 ]]; then
    grep -q "pipe2_producer_warp=1 pipe2_producer_prectx_warpcoop=$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_PRECTX_WARPCOOP" "$LOGDIR/orbit_tuned.build.err" || { echo 'orbit producer build marker mismatch' >&2; exit 5; }
  fi
fi
