...
if has orbit_tuned && [[ "$REBUILD" == 1 || ! -x "$ORBIT_BIN" ]]; then
  ...
  grep -q "flat_dynamic=$ORBITCTA_FLAT_DYNAMIC flat_dynamic_batch=$ORBITCTA_FLAT_DYNAMIC_BATCH flat_dynamic_fuse_lease_prep=$ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP" "$LOGDIR/orbit_tuned.build.err" || { echo 'orbit dynamic build marker mismatch' >&2; exit 5; }
