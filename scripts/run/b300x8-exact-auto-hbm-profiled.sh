...
# dynamic flat scheduling, flat-bid, warpcoop and chunked quad execution.
...
for x in PM_ACCUM REBUILD SELECT_ONLY WARP_PRECTX_COMPACT ORBIT_PRECTX_COMPACT ORBIT_PRECTX_FLAT_BID ORBIT_PRECTX_FLAT_BID_FUSED ORBIT_PRECTX_WARPCOOP ORBITCTA_FLAT ORBITCTA_FLAT_DYNAMIC ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP ORBIT_QUAD_MLP ORBIT_QUAD_OVERLAP_LOCAL ORBIT_QUAD_SPARSE_DESC_MLP ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0; do
...
if [[ "$ORBITCTA_FLAT_DYNAMIC" == 1 ]]; then
  ...
  if (( ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES != 0 )); then
    (( ORBITCTA_FLAT_DYNAMIC_BATCH > 1 )) || { echo 'adaptive dynamic waves require dynamic batch >1' >&2; exit 2; }
  fi
else
...
if [[ "$ORBIT_PRECTX_FLAT_BID" == 1 ]]; then
  ...
  [[ "$ORBIT_PRECTX_FLAT_BID_FUSED" == 0 ]] || { echo 'ORBIT_PRECTX_FLAT_BID_FUSED requires ORBIT_PRECTX_FLAT_BID=1' >&2; exit 2; }
fi
...
if [[ "$ORBIT_QUAD_MLP" == 1 ]]; then
  ...
  [[ "$ORBIT_QUAD_SPARSE_DESC_MLP" == 0 ]] || { echo 'ORBIT_QUAD_SPARSE_DESC_MLP requires ORBIT_QUAD_MLP=1' >&2; exit 2; }
  [[ "$ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0" == 0 ]] || { echo 'ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0 requires ORBIT_QUAD_MLP=1' >&2; exit 2; }
  [[ "$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES" == 0 ]] || { echo 'ORBIT_QUAD_CPASYNC_PREFETCH_BYTES requires ORBIT_QUAD_MLP=1' >&2; exit 2; }
  [[ "$ORBIT_QUAD_CPASYNC_GROUP_COLS" == 1 ]] || { echo 'ORBIT_QUAD_CPASYNC_GROUP_COLS requires ORBIT_QUAD_MLP=1' >&2; exit 2; }
fi
...
if has orbit_tuned && [[ "$REBUILD" == 1 || ! -x "$ORBIT_BIN" ]]; then
  N=27 ARCH="$ARCH" OUT="$ORBIT_BIN" DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$ORBIT_SPARSE64" DIRECTGATHER_SORT_RANKS=0 ORBITCTA_COL_ILP="$ORBIT_COL_ILP" ORBITCTA_FLAT="$ORBITCTA_FLAT" ORBITCTA_FLAT_CHUNK="$ORBITCTA_FLAT_CHUNK" ORBITCTA_FLAT_DYNAMIC="$ORBITCTA_FLAT_DYNAMIC" ORBITCTA_FLAT_DYNAMIC_BATCH="$ORBITCTA_FLAT_DYNAMIC_BATCH" ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP="$ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP" ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES="$ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES" \
    PAIR_MLP=1 CPASYNC_PAIR="$ORBIT_CPASYNC_PAIR" QUAD_MLP="$ORBIT_QUAD_MLP" QUAD_OVERLAP_LOCAL="$ORBIT_QUAD_OVERLAP_LOCAL" QUAD_LOCAL_DIRECT_MAX="$ORBIT_QUAD_LOCAL_DIRECT_MAX" QUAD_SPARSE_DESC_MLP="$ORBIT_QUAD_SPARSE_DESC_MLP" QUAD_OVERLAP_BYPASS_LOCAL0="$ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0" \
    QUAD_CPASYNC_PREFETCH_BYTES="$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES" QUAD_CPASYNC_GROUP_COLS="$ORBIT_QUAD_CPASYNC_GROUP_COLS" \
    PRECTX_FORWARD="$ORBIT_PRECTX_FORWARD" PRECTX_REVERSE="$ORBIT_PRECTX_REVERSE" PRECTX_COMPACT="$ORBIT_PRECTX_COMPACT" PRECTX_WARPCOOP="$ORBIT_PRECTX_WARPCOOP" PRECTX_FLAT_BID="$ORBIT_PRECTX_FLAT_BID" PRECTX_FLAT_BID_FUSED="$ORBIT_PRECTX_FLAT_BID_FUSED" \
  ...
  grep -q "flat_dynamic=$ORBITCTA_FLAT_DYNAMIC flat_dynamic_batch=$ORBITCTA_FLAT_DYNAMIC_BATCH flat_dynamic_fuse_lease_prep=$ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP flat_dynamic_adaptive_waves=$ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES" "$LOGDIR/orbit_tuned.build.err" || { echo 'orbit dynamic build marker mismatch' >&2; exit 5; }
  grep -q "quad_cpasync_prefetch_bytes=$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES" "$LOGDIR/orbit_tuned.build.err" || { echo 'orbit prefetch build marker mismatch' >&2; exit 5; }
  grep -q "quad_cpasync_group_cols=$ORBIT_QUAD_CPASYNC_GROUP_COLS" "$LOGDIR/orbit_tuned.build.err" || { echo 'orbit group build marker mismatch' >&2; exit 5; }
fi
