...
for x in PM_ACCUM REBUILD SELECT_ONLY WARP_PRECTX_COMPACT ORBIT_PRECTX_COMPACT ORBIT_PRECTX_FLAT_BID ORBIT_PRECTX_FLAT_BID_FUSED ORBIT_PRECTX_WARPCOOP ORBITCTA_FLAT ORBITCTA_FLAT_DYNAMIC ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP ORBITCTA_FLAT_DYNAMIC_PIPE2 ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_PRECTX_WARPCOOP ORBIT_QUAD_MLP ORBIT_QUAD_OVERLAP_LOCAL ORBIT_QUAD_SPARSE_DESC_MLP ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0; do
  v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0 or 1" >&2; exit 2; }
done
case "$ORBITCTA_FLAT_DYNAMIC_BATCH" in 1|2|4|8|16) ;; *) echo 'ORBITCTA_FLAT_DYNAMIC_BATCH must be 1,2,4,8,16' >&2; exit 2;; esac
case "$ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES" in 0|1|2|4) ;; *) echo 'ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES must be 0,1,2,4' >&2; exit 2;; esac
case "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WORKER_WEIGHT" in 0|1|2|3|4) ;; *) echo 'ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WORKER_WEIGHT must be 0..4' >&2; exit 2;; esac
case "$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES" in 0|64|128|256) ;; *) echo 'ORBIT_QUAD_CPASYNC_PREFETCH_BYTES must be 0,64,128,256' >&2; exit 2;; esac
case "$ORBIT_QUAD_CPASYNC_GROUP_COLS" in 1|2|4) ;; *) echo 'ORBIT_QUAD_CPASYNC_GROUP_COLS must be 1,2,4' >&2; exit 2;; esac
...
if [[ "$ORBITCTA_FLAT_DYNAMIC" == 1 ]]; then
  ...
  if [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP" == 1 ]]; then
    [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2" == 1 ]] || { echo 'PIPE2 producer warp requires PIPE2=1' >&2; exit 2; }
  else
    [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WORKER_WEIGHT" == 0 ]] || { echo 'producer worker weight requires producer warp' >&2; exit 2; }
  fi
  if [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_PRECTX_WARPCOOP" == 1 ]]; then
    [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP" == 1 ]] || { echo 'producer prectx warpcoop requires producer warp' >&2; exit 2; }
    [[ "${ORBIT_PRECTX_FORWARD:-0}" == 1 && "${ORBIT_PRECTX_REVERSE:-0}" == 1 && "$ORBIT_PRECTX_COMPACT" == 1 ]] || { echo 'producer prectx warpcoop requires compact forward+reverse prectx' >&2; exit 2; }
    [[ "$ORBIT_PRECTX_FLAT_BID_FUSED" == 0 ]] || { echo 'producer prectx warpcoop currently requires flat-bid-fused=0' >&2; exit 2; }
  fi
else
  [[ "$ORBITCTA_FLAT_DYNAMIC_BATCH" == 1 ]] || { echo 'dynamic batch >1 requires dynamic queue' >&2; exit 2; }
  [[ "$ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP" == 0 ]] || { echo 'dynamic lease fusion requires dynamic queue' >&2; exit 2; }
  [[ "$ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES" == 0 ]] || { echo 'adaptive dynamic waves require dynamic queue' >&2; exit 2; }
  [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2" == 0 ]] || { echo 'dynamic PIPE2 requires dynamic queue' >&2; exit 2; }
  [[ "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP" == 0 && "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WORKER_WEIGHT" == 0 && "$ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_PRECTX_WARPCOOP" == 0 ]] || { echo 'PIPE2 producer modes require dynamic queue' >&2; exit 2; }
fi
...
if [[ "$ORBIT_PRECTX_WARPCOOP" == 1 ]]; then
  ...
  [[ "$ORBIT_PRECTX_COMPACT" == 1 && "$ORBIT_PRECTX_FORWARD" == 1 && "$ORBIT_PRECTX_REVERSE" == 1 ]] || { echo 'ORBIT_PRECTX_WARPCOOP requires compact forward+reverse prectx' >&2; exit 2; }
...
if [[ "$ORBIT_QUAD_MLP" == 1 ]]; then
  ...
  [[ "$ORBIT_QUAD_SPARSE_DESC_MLP" == 0 && "$ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0" == 0 && "$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES" == 0 && "$ORBIT_QUAD_CPASYNC_GROUP_COLS" == 1 ]] || { echo 'quad suboptions require ORBIT_QUAD_MLP=1' >&2; exit 2; }
fi
if [[ "$ORBIT_QUAD_OVERLAP_LOCAL" == 1 ]]; then [[ "$ORBIT_CPASYNC_PAIR" == 1 ]] || exit 2; (( ORBIT_QUAD_LOCAL_DIRECT_MAX == 0 )) || exit 2; fi
if [[ "$ORBIT_QUAD_SPARSE_DESC_MLP" == 1 ]]; then [[ "$ORBIT_QUAD_OVERLAP_LOCAL" == 1 && "$ORBIT_SPARSE64" == 1 ]] || exit 2; fi
if [[ "$ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0" == 1 ]]; then [[ "$ORBIT_QUAD_OVERLAP_LOCAL" == 1 ]] || exit 2; fi
if (( ORBIT_QUAD_CPASYNC_PREFETCH_BYTES != 0 || ORBIT_QUAD_CPASYNC_GROUP_COLS != 1 )); then [[ "$ORBIT_QUAD_OVERLAP_LOCAL" == 1 && "$ORBIT_CPASYNC_PAIR" == 1 ]] || exit 2; fi

has(){ [[ " $CANDIDATES " == *" $1 "* ]]; }
FORCED_BIN="$ONEESAN_BUILD_DIR/b300_profiled_forced_n27"
WARP_BIN="$ONEESAN_BUILD_DIR/b300_profiled_warp_${WARP_PROFILE}_n27"
ORBIT_BIN="$ONEESAN_BUILD_DIR/b300_profiled_orbit_${ORBIT_PROFILE}_flat${ORBITCTA_FLAT}_chunk${ORBITCTA_FLAT_CHUNK}_dyn${ORBITCTA_FLAT_DYNAMIC}_db${ORBITCTA_FLAT_DYNAMIC_BATCH}_df${ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP}_daw${ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES}_dp${ORBITCTA_FLAT_DYNAMIC_PIPE2}_dpw${ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WARP}_pww${ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_WORKER_WEIGHT}_ppw${ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_PRECTX_WARPCOOP}_psm${ORBITCTA_FLAT_BLOCKS_PER_SM}_bid${ORBIT_PRECTX_FLAT_BID}_bf${ORBIT_PRECTX_FLAT_BID_FUSED}_wc${ORBIT_PRECTX_WARPCOOP}_quad${ORBIT_QUAD_MLP}_qol${ORBIT_QUAD_OVERLAP_LOCAL}_qld${ORBIT_QUAD_LOCAL_DIRECT_MAX}_qsd${ORBIT_QUAD_SPARSE_DESC_MLP}_ql0${ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0}_qgc${ORBIT_QUAD_CPASYNC_GROUP_COLS}_qpf${ORBIT_QUAD_CPASYNC_PREFETCH_BYTES}_n27"
...
if (( WARP_CPASYNC_PAIR || ORBIT_CPASYNC_PAIR )); then
  ...
  grep -q 'cp_async_remote_peer=OK exact=OK' "$LOGDIR/cpasync-peer.out" || exit 4
fi
...
plan_bucket(){ local mode="$1" bin="$2" pe="$LOGDIR/$mode.plan.err"; set +e; "$bin" 27 "$BUCKET_TARGET_MIB" "$MAX_WINDOW" 8 --plan-only >"$LOGDIR/$mode.plan.out" 2>"$pe"; local rc=$?; set -e; ((rc==0)) || return "$rc"; grep 'backend=gridfp-b300-bucket-snake-onepass-graph-batch-v0.1-plan' "$pe" | tail -n1; }
WARP_PLAN=''; ORBIT_PLAN=''; has warp_tuned && WARP_PLAN="$(plan_bucket warp_tuned "$WARP_BIN")" || true; has orbit_tuned && ORBIT_PLAN="$(plan_bucket orbit_tuned "$ORBIT_BIN")" || true
...
smoke(){
  local mode="$1" profile="$2" bin="$3" target="$4" family="$5" plan="$6" so="$LOGDIR/$mode.smoke.out" se="$LOGDIR/$mode.smoke.err" dm="$LOGDIR/$mode.smoke.dmon"
  ...
  elif [[ "$ORBITCTA_FLAT" == 1 && "$ORBITCTA_FLAT_BLOCKS_PER_SM" == 0 ]]; then env -u BUCKET_ORBITCTA_FLAT_BLOCKS -u BUCKET_ORBITCTA_FLAT_BLOCKS_PER_SM BUCKET_THREADS="$THREADS" BUCKET_ORBITCTA_GRID_Y="$ORBIT_GY" BUCKET_GRID_X="$LOW_GX" BUCKET_GRID_Y="$LOW_GY" BUCKET_LOW_GRID_X="$LOW_GX" BUCKET_LOW_GRID_Y="$LOW_GY" "$bin" 27 "$target" "$MAX_WINDOW" 8 "$PRIME" >"$so" 2>"$se"
  else env -u BUCKET_ORBITCTA_FLAT_BLOCKS BUCKET_THREADS="$THREADS" BUCKET_ORBITCTA_GRID_Y="$ORBIT_GY" BUCKET_ORBITCTA_FLAT_BLOCKS_PER_SM="$ORBITCTA_FLAT_BLOCKS_PER_SM" BUCKET_GRID_X="$LOW_GX" BUCKET_GRID_Y="$LOW_GY" BUCKET_LOW_GRID_X="$LOW_GX" BUCKET_LOW_GRID_Y="$LOW_GY" "$bin" 27 "$target" "$MAX_WINDOW" 8 "$PRIME" >"$so" 2>"$se"; fi
  local rc=$?; set -e; kill "$dp" 2>/dev/null || true; wait "$dp" 2>/dev/null || true
  if ((rc)); then printf '%s\t%s\t%s\tfailed:%s\tNA\tNA\tNA\tNA\t0\tNA\tNA\n' "$mode" "$profile" "$bin" "$rc" >>"$RESULT"; return; fi
  local line="$(grep '^residue=' "$so" | tail -n1 || true)"; [[ -n "$line" ]] || { printf '%s\t%s\t%s\tfailed:no_residue\tNA\tNA\tNA\tNA\t0\tNA\tNA\n' "$mode" "$profile" "$bin" >>"$RESULT"; return; }
  local avg mx ns extra=0 need=0; IFS=$'\t' read -r avg mx ns <<<"$(sample_summary "$dm")"; [[ -n "$plan" ]] && { extra="$(field extra_metadata_mib "$plan")"; need="$(field max_device_need_gib "$plan")"; }
...
for r in sorted(ok,key=lambda x:float(x['wall_s'])): print('CANDIDATE',r['backend'],'profile='+r['profile'],'wall_s='+r['wall_s'],'mc_avg='+r['mc_avg_pct'],file=sys.stderr)
...
echo "PROFILED SELECTED backend=$BEST profile=$BEST_PROFILE wall_s=$BEST_WALL residue=$BEST_RES" >&2; cat "$RESULT" >&2
SHA12="$(sha256sum "$BEST_BIN" | awk '{print substr($1,1,12)}')"; BEST_WORK="$WORK_ROOT/b300_exact_profiled_${BEST}_${BEST_PROFILE}_${SHA12}_n27"; mkdir -p "$BEST_WORK"
