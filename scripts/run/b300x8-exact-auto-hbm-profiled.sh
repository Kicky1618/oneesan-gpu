...
for x in PM_ACCUM REBUILD SELECT_ONLY WARP_PRECTX_COMPACT ORBIT_PRECTX_COMPACT ORBIT_PRECTX_FLAT_BID ORBIT_PRECTX_FLAT_BID_FUSED ORBITCTA_FLAT ORBIT_QUAD_MLP ORBIT_QUAD_OVERLAP_LOCAL ORBIT_QUAD_SPARSE_DESC_MLP; do
...
if has orbit_tuned && [[ "$REBUILD" == 1 || ! -x "$ORBIT_BIN" ]]; then
  N=27 ARCH="$ARCH" OUT="$ORBIT_BIN" DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$ORBIT_SPARSE64" DIRECTGATHER_SORT_RANKS=0 ORBITCTA_COL_ILP="$ORBIT_COL_ILP" ORBITCTA_FLAT="$ORBITCTA_FLAT" ORBITCTA_FLAT_CHUNK="$ORBITCTA_FLAT_CHUNK" \
    PAIR_MLP=1 CPASYNC_PAIR="$ORBIT_CPASYNC_PAIR" QUAD_MLP="$ORBIT_QUAD_MLP" QUAD_OVERLAP_LOCAL="$ORBIT_QUAD_OVERLAP_LOCAL" QUAD_LOCAL_DIRECT_MAX="$ORBIT_QUAD_LOCAL_DIRECT_MAX" QUAD_SPARSE_DESC_MLP="$ORBIT_QUAD_SPARSE_DESC_MLP" \
