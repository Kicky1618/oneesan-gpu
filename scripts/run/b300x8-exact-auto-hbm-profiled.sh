...
[[ -f "$PROFILE_FILE" ]] || { echo "missing profile file: $PROFILE_FILE" >&2; echo 'run: bash scripts/bench/b300-hbm-profile-tune21.sh' >&2; exit 2; }
# The profile file is generated by our tuner and contains only KEY=value pairs.
...
for x in PM_ACCUM REBUILD SELECT_ONLY; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0 or 1" >&2; exit 2; }; done
...
ORBIT_BIN="$ONEESAN_BUILD_DIR/b300_profiled_orbit_${ORBIT_PROFILE}_n27"
...
if has warp_tuned && [[ "$REBUILD" == 1 || ! -x "$WARP_BIN" ]]; then
  N=27 ARCH="$ARCH" OUT="$WARP_BIN" COL_ILP="$WARP_COL_ILP" PM_ACCUM="$PM_ACCUM" DEPTHMAJOR=1 PAIR_MLP=1 MLP_WINDOW4=1 \
    ...
    PRECTX_FORWARD="$WARP_PRECTX_FORWARD" PRECTX_REVERSE="$WARP_PRECTX_REVERSE" FORCE7=0 PREFETCH_NEXT=0 PTXAS_VERBOSE=1 TRANSPOSE_MODE=pipeline \
...
if has orbit_tuned && [[ "$REBUILD" == 1 || ! -x "$ORBIT_BIN" ]]; then
  N=27 ARCH="$ARCH" OUT="$ORBIT_BIN" DIRECTGATHER64=1 DIRECTGATHER_SPARSE64="$ORBIT_SPARSE64" ORBITCTA_COL_ILP="$ORBIT_COL_ILP" \
    PAIR_MLP=1 CPASYNC_PAIR="$ORBIT_CPASYNC_PAIR" PRECTX_FORWARD="$ORBIT_PRECTX_FORWARD" PRECTX_REVERSE="$ORBIT_PRECTX_REVERSE" \
...
smoke(){
  ...
  else BUCKET_THREADS="$THREADS" BUCKET_ORBITCTA_GRID_Y="$ORBIT_GY" BUCKET_GRID_X="$LOW_GX" BUCKET_GRID_Y="$LOW_GY" BUCKET_LOW_GRID_X="$LOW_GX" BUCKET_LOW_GRID_Y="$LOW_GY" "$bin" 27 "$target" "$MAX_WINDOW" 8 "$PRIME" >"$so" 2>"$se"; fi
  local rc=$?; set -e
...
else export BUCKET_THREADS="$THREADS" BUCKET_ORBITCTA_GRID_Y="$ORBIT_GY" BUCKET_GRID_X="$LOW_GX" BUCKET_GRID_Y="$LOW_GY" BUCKET_LOW_GRID_X="$LOW_GX" BUCKET_LOW_GRID_Y="$LOW_GY"; RUN_TARGET="$BUCKET_TARGET_MIB"; fi
