...
for name in FAST_SHARD_ADDRESS8 MAIN_MATE_CACHE MAIN_PULL BLOCK_PULL BLOCK_MATE_CACHE RANK_DELTA_CACHE REBUILD; do
  value="${!name}"
...
if [[ "$RANK_DELTA_CACHE" == 1 && ( "$MAIN_PULL" != 1 || "$BLOCK_PULL" != 1 || "$MAIN_MATE_CACHE" != 1 || "$BLOCK_MATE_CACHE" != 1 ) ]]; then
  echo "RANK_DELTA_CACHE=1 requires full-pull plus both MateID caches" >&2; exit 2
fi
...
if [[ "$REBUILD" == 1 || ! -x "$BIN" ]]; then
  echo "building specialized n=$N binary shardaddr8=$FAST_SHARD_ADDRESS8 matecache=$MAIN_MATE_CACHE mainpull=$MAIN_PULL blockpull=$BLOCK_PULL blockmate=$BLOCK_MATE_CACHE rankdelta=$RANK_DELTA_CACHE" >&2
  N="$N" FAST_SHARD_ADDRESS8="$FAST_SHARD_ADDRESS8" \
  MAIN_MATE_CACHE="$MAIN_MATE_CACHE" MAIN_PULL="$MAIN_PULL" BLOCK_PULL="$BLOCK_PULL" BLOCK_MATE_CACHE="$BLOCK_MATE_CACHE" RANK_DELTA_CACHE="$RANK_DELTA_CACHE" \
