...
# Experimental two-destination/thread kernel. It increases useful outstanding
# HBM requests without adding metadata traffic, but can lose to register pressure.
MAIN_PULL_ILP2="${MAIN_PULL_ILP2:-0}"
# Experimental arithmetic-for-HBM state streams. Keep separate until row-1 A/B.
...
for name in FAST_SHARD_ADDRESS8 MAIN_MATE_CACHE MAIN_PULL BLOCK_PULL BLOCK_MATE_CACHE MAIN_PULL_ILP2 HEIGHT_CACHE RANK_DELTA_CACHE REBUILD; do
  value="${!name}"
...
if [[ "$REBUILD" == 1 || ! -x "$BIN" ]]; then
  echo "building specialized n=$N binary shardaddr8=$FAST_SHARD_ADDRESS8 matecache=$MAIN_MATE_CACHE mainpull=$MAIN_PULL blockpull=$BLOCK_PULL blockmate=$BLOCK_MATE_CACHE ilp2=$MAIN_PULL_ILP2 height=$HEIGHT_CACHE rankdelta=$RANK_DELTA_CACHE" >&2
  N="$N" FAST_SHARD_ADDRESS8="$FAST_SHARD_ADDRESS8" \
  MAIN_MATE_CACHE="$MAIN_MATE_CACHE" MAIN_PULL="$MAIN_PULL" BLOCK_PULL="$BLOCK_PULL" BLOCK_MATE_CACHE="$BLOCK_MATE_CACHE" MAIN_PULL_ILP2="$MAIN_PULL_ILP2" HEIGHT_CACHE="$HEIGHT_CACHE" RANK_DELTA_CACHE="$RANK_DELTA_CACHE" \
