...
for name in FAST_SHARD_ADDRESS8 MAIN_MATE_CACHE MAIN_PULL BLOCK_PULL BLOCK_MATE_CACHE MAIN_PULL_ILP2 HEIGHT_CACHE RANK_DELTA_CACHE RANK_STATE_PACKED RANK_STATE_ILP2 RANK_STATE_ILP4 BLOCK_CLOSURE_QUAD HOT_DELTA_TABLE CONCURRENT_GROUP_IO REBUILD; do
  value="${!name}"
...
if [[ "$REBUILD" == 1 || ! -x "$BIN" ]]; then
  echo "building specialized n=$N binary shardaddr8=$FAST_SHARD_ADDRESS8 matecache=$MAIN_MATE_CACHE mainpull=$MAIN_PULL blockpull=$BLOCK_PULL blockmate=$BLOCK_MATE_CACHE ilp2=$MAIN_PULL_ILP2 height=$HEIGHT_CACHE rankdelta=$RANK_DELTA_CACHE rankstate=$RANK_STATE_PACKED rankstate_ilp2=$RANK_STATE_ILP2 rankstate_ilp4=$RANK_STATE_ILP4 closure_quad=$BLOCK_CLOSURE_QUAD hot_delta=$HOT_DELTA_TABLE concurrent_io=$CONCURRENT_GROUP_IO maxrregcount=$MAXRREGCOUNT" >&2
