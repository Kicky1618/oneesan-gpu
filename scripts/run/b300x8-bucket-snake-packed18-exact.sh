...
BIN="${BIN:-$ONEESAN_BUILD_DIR/oneesan_cuda_gridfp_b300_bucket_snake_onepass_packed18_batch${SUFFIX}_n${N}}"
WORK_TAG="packed18_${TRANSPOSE_MODE}"; if [[ "$PM_ACCUM" == 1 ]]; then WORK_TAG="${WORK_TAG}_pm"; fi
WORK_DIR="${WORK_DIR:-$ONEESAN_ROOT/work/b300_bucket_snake_${WORK_TAG}_exact_n${N}}"
if (( NGPU != 8 )); then echo "packed18 snake backend currently requires NGPU=8" >&2; exit 2; fi
...
if [[ ! -x "$BIN" ]]; then
  echo "$BIN not found; building packed18 snake batch binary n=$N transpose=$TRANSPOSE_MODE pm=$PM_ACCUM" >&2
  N="$N" OUT="$BIN" TRANSPOSE_MODE="$TRANSPOSE_MODE" PM_ACCUM="$PM_ACCUM" bash "$ONEESAN_ROOT/scripts/build/b300-bucket-snake-packed18-batch.sh"
fi
