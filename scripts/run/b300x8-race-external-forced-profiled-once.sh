...
FORCED_EXTRA2_BIN="${FORCED_EXTRA2_BIN:-}"; FORCED_EXTRA2_LABEL="${FORCED_EXTRA2_LABEL:-external_forced_extra2}"; FORCED_EXTRA2_THREADS="${FORCED_EXTRA2_THREADS:-256}"
FORCED_EXTRA3_BIN="${FORCED_EXTRA3_BIN:-}"; FORCED_EXTRA3_LABEL="${FORCED_EXTRA3_LABEL:-external_forced_extra3}"; FORCED_EXTRA3_THREADS="${FORCED_EXTRA3_THREADS:-256}"
[[ -n "$FORCED_OVERRIDE_BIN" && -x "$FORCED_OVERRIDE_BIN" ]] || { echo 'FORCED_OVERRIDE_BIN must be executable' >&2; exit 2; }
for v in FORCED_OVERRIDE_THREADS FORCED_BASE_THREADS FORCED_EXTRA_THREADS FORCED_EXTRA2_THREADS FORCED_EXTRA3_THREADS; do x="${!v}"; [[ "$x" =~ ^[0-9]+$ ]] && ((x>=32&&x<=1024&&x%32==0)) || { echo "$v must be warp multiple 32..1024" >&2; exit 2; }; done
HAS_FORCED_BASE=0
...
HAS_FORCED_EXTRA2=0
if [[ -n "$FORCED_EXTRA2_BIN" && "$FORCED_EXTRA2_BIN" != "$FORCED_OVERRIDE_BIN" && "$FORCED_EXTRA2_BIN" != "$FORCED_BASE_BIN" && "$FORCED_EXTRA2_BIN" != "$FORCED_EXTRA_BIN" ]]; then [[ -x "$FORCED_EXTRA2_BIN" ]] || { echo 'FORCED_EXTRA2_BIN must be executable' >&2; exit 2; }; HAS_FORCED_EXTRA2=1; fi
HAS_FORCED_EXTRA3=0
if [[ -n "$FORCED_EXTRA3_BIN" && "$FORCED_EXTRA3_BIN" != "$FORCED_OVERRIDE_BIN" && "$FORCED_EXTRA3_BIN" != "$FORCED_BASE_BIN" && "$FORCED_EXTRA3_BIN" != "$FORCED_EXTRA_BIN" && "$FORCED_EXTRA3_BIN" != "$FORCED_EXTRA2_BIN" ]]; then [[ -x "$FORCED_EXTRA3_BIN" ]] || { echo 'FORCED_EXTRA3_BIN must be executable' >&2; exit 2; }; HAS_FORCED_EXTRA3=1; fi
...
if [[ "$HAS_FORCED_EXTRA2" == 1 ]]; then smoke_forced forced_extra2 "$FORCED_EXTRA2_LABEL" "$FORCED_EXTRA2_BIN" "$FORCED_EXTRA2_THREADS"; fi
if [[ "$HAS_FORCED_EXTRA3" == 1 ]]; then smoke_forced forced_extra3 "$FORCED_EXTRA3_LABEL" "$FORCED_EXTRA3_BIN" "$FORCED_EXTRA3_THREADS"; fi
smoke_warp
smoke_orbit
...
elif [[ "$HAS_FORCED_EXTRA2" == 1 && "$BEST_BIN" == "$FORCED_EXTRA2_BIN" ]]; then export GRIDFP_THREADS="$FORCED_EXTRA2_THREADS"; RUN_TARGET="$FORCED_TARGET_MIB"
elif [[ "$HAS_FORCED_EXTRA3" == 1 && "$BEST_BIN" == "$FORCED_EXTRA3_BIN" ]]; then export GRIDFP_THREADS="$FORCED_EXTRA3_THREADS"; RUN_TARGET="$FORCED_TARGET_MIB"
