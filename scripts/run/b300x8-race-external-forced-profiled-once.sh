...
for v in FORCED_OVERRIDE_THREADS FORCED_BASE_THREADS FORCED_EXTRA_THREADS FORCED_EXTRA2_THREADS; do x="${!v}"; [[ "$x" =~ ^[0-9]+$ ]] && ((x>=32&&x<=1024&&x%32==0)) || { echo "$v must be warp multiple 32..1024" >&2; exit 2; }; done
HAS_FORCED_BASE=0
...
smoke_warp
smoke_orbit
EXPECTED_OK=$((3+HAS_FORCED_BASE+HAS_FORCED_EXTRA+HAS_FORCED_EXTRA2))
