...
ARCH="${ARCH:-native}"; TARGET_MIB="${TARGET_MIB:-65536}"; MAX_WINDOW="${MAX_WINDOW:-14}"; NGPU="${NGPU:-8}"
...
if [[ "$RUN_STAGED" == 1 ]]; then
  STAGE_F_ENV="$STAGE_F_ENV" UPSTREAM_PREPARE_ENV="$UPSTREAM_PREPARE_ENV" UPSTREAM_WINNER_ENV="$UPSTREAM_WINNER_ENV" \
    ARCH="$ARCH" MOD="${MOD:-4294967291}" TARGET_MIB="$TARGET_MIB" MAX_WINDOW="$MAX_WINDOW" NGPU="$NGPU" \
...
case "$B300_STAGEL_UPSTREAM_KIND" in stagej|stagek) ;; *) exit 3;; esac
...
# Recover the exact fixed geometry/eviction state from the prepared upstream.
# shellcheck disable=SC1090
source "$UPSTREAM_PREPARE_ENV"
if [[ "$B300_STAGEL_UPSTREAM_KIND" == stagek ]]; then
  [[ "${B300_STAGEK_PREPARED:-0}" == 1 ]] || exit 3
  ...
  [[ "${B300_STAGEJ_PREPARED:-0}" == 1 ]] || exit 3
...
if [[ "$PREPARE_ONLY" == 1 ]]; then
  ...
  cat "$PREPARE_ENV"
  echo "STAGE L PREPARED profile=$B300_STAGEL_FINAL_PROFILE guards=$B300_STAGEL_FINAL_SELF_GUARD/$B300_STAGEL_FINAL_MATE_GUARD upstream=$B300_STAGEL_UPSTREAM_KIND speedup=${B300_STAGEL_FINAL_SPEEDUP}x" >&2
  exit 0
fi
