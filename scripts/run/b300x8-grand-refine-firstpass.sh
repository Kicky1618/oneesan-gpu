...
[[ "$N" == 27 ]] || { echo 'grand refine first-pass targets n=27' >&2; exit 2; }
...
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_grand_refine_firstpass_n27}"
...
# First choose the best candidate from the existing grand family.  The second
# pass below keeps that winner if it is a forced backend; warp/orbit are already
# reintroduced by the common race runner.
...
for k in B300_GRAND_SELECTED_HEAD_SHA B300_GRAND_SELECTED_HEAD_DIRTY B300_GRAND_SELECTED_PROFILE_FILE B300_GRAND_SELECTED_PROFILE_SHA256 \
  B300_GRAND_SELECTED_BACKEND B300_GRAND_SELECTED_BINARY B300_GRAND_SELECTED_RUNTIME_KIND B300_GRAND_SELECTED_THREADS \
  B300_GRAND_SELECTED_SMOKE_PRIME B300_GRAND_SELECTED_MAX_WINDOW; do
  [[ -n "${!k+x}" ]] || { echo "base selected env missing $k" >&2; exit 3; }
done
...
BASE_PROFILE_FILE="$B300_GRAND_SELECTED_PROFILE_FILE"; BASE_PROFILE_SHA="$B300_GRAND_SELECTED_PROFILE_SHA256"; BASE_PRIME="$B300_GRAND_SELECTED_SMOKE_PRIME"; BASE_MAXW="$B300_GRAND_SELECTED_MAX_WINDOW"
...
if [[ ! -s "$STAGE_F_ENV" ]]; then
  ...
  printf 'B300_GRAND_REFINE_APPLICABLE=0\nB300_GRAND_REFINE_REASON=no_stage_f_env\n' >>"$SELECTED_ENV"
  exit 0
fi

prepare_one(){
  local kind="$1" script="$2" min="$3" prefix="$4" envf="$5" race="$6" prep="$7" rc
  set +e
  # PREPARE_ONLY runners intentionally print their prepared env to stdout.
  # Send that diagnostic stream to stderr so command substitution captures only
  # the single H=0/1 or I=0/1 status line below.
  PROFILE_FILE="$BASE_PROFILE_FILE" ARCH="$ARCH" TARGET_MIB="$FORCED_TARGET_MIB" MAX_WINDOW="$BASE_MAXW" MOD="$BASE_PRIME" \
    INPUT_ENV="$STAGE_F_ENV" RUN_STAGED=1 PREPARE_ONLY=1 MIN_SPEEDUP="$min" STAGED_PREFIX="$prefix" WINNER_ENV="$envf" \
    RACE_PREFIX="$race" PREPARE_ENV="$prep" bash "$script" 27 >&2
  rc=$?
  set -e
  if ((rc==0)); then printf '%s=1\n' "$kind"; elif ((rc==4)); then printf '%s=0\n' "$kind"; else return "$rc"; fi
}
H_OK=0; I_OK=0
x="$(prepare_one H "$ONEESAN_ROOT/scripts/run/b300x8-nextgen-hybrid8-nextmate-staged-fullprime-race.sh" "$STAGEH_MIN_SPEEDUP" "$H_PREFIX" "$H_ENV" "$H_RACE" "$H_PREP")" || exit $?
[[ "$x" == H=1 ]] && H_OK=1
x="$(prepare_one I "$ONEESAN_ROOT/scripts/run/b300x8-nextgen-hybrid8-nextself-evict-staged-fullprime-race.sh" "$STAGEI_MIN_SPEEDUP" "$I_PREFIX" "$I_ENV" "$I_RACE" "$I_PREP")" || exit $?
[[ "$x" == I=1 ]] && I_OK=1
...
if (( !H_OK && !I_OK )); then
  cp "$BASE_SELECTED_ENV" "$SELECTED_ENV"
  printf 'B300_GRAND_REFINE_APPLICABLE=1\nB300_GRAND_REFINE_H_OK=0\nB300_GRAND_REFINE_I_OK=0\nB300_GRAND_REFINE_REASON=both_refinements_rejected\n' >>"$SELECTED_ENV"
  exit 0
fi

P_BIN=""; P_LABEL=""; P_THREADS=256; B_BIN=""; B_LABEL=""; B_THREADS=256
E1_BIN=""; E1_LABEL=""; E1_THREADS=256; E2_BIN=""; E2_LABEL=""; E2_THREADS=256
if ((H_OK)); then
  ...
  P_BIN="$B300_STAGEI_PREPARED_BIN"; P_LABEL="$B300_STAGEI_PREPARED_LABEL"; P_THREADS="$B300_STAGEI_PREPARED_THREADS"
  B_BIN="$B300_STAGEI_PREPARED_CONTROL_BIN"; B_LABEL="$B300_STAGEI_PREPARED_CONTROL_LABEL"; B_THREADS="$B300_STAGEI_PREPARED_CONTROL_THREADS"
fi
if [[ "$BASE_RUNTIME" == forced && "$BASE_BIN" != "$P_BIN" && "$BASE_BIN" != "$B_BIN" && "$BASE_BIN" != "$E1_BIN" ]]; then
  E2_BIN="$BASE_BIN"; E2_LABEL="grand_previous_${BASE_BACKEND}"; E2_THREADS="$BASE_THREADS"
fi

echo "=== refinement full-prime race H=$H_OK I=$I_OK previous=$BASE_BACKEND ===" >&2
set +e
env PROFILE_FILE="$BASE_PROFILE_FILE" ARCH="$ARCH" SMOKE_PRIME="$BASE_PRIME" MAX_WINDOW="$BASE_MAXW" \
  ...
  SELECT_ONLY=1 REBUILD_BUCKETS="$REBUILD_BUCKETS" PREFIX="$FINAL_PREFIX" RESULT="$FINAL_RESULT" \
  "$ONEESAN_ROOT/scripts/run/b300x8-race-external-forced-profiled-once.sh" 27 "$@" 2>&1 | tee "$FINAL_LOG"
rc=${PIPESTATUS[0]}; set -e
((rc==0)) || exit "$rc"; [[ -s "$FINAL_RESULT" ]] || exit 4
...
BEST_WORK="$WORK_ROOT/b300_exact_singlepass_${BEST}_${BEST_PROFILE}_${SHA12}_n27"; CHECKPOINT="$BEST_WORK/checkpoint.json"
[[ -s "$CHECKPOINT" ]] || exit 4
RESULT_SHA="$(sha256sum "$FINAL_RESULT" | awk '{print $1}')"
...
else RUNTIME=orbit; RUN_TARGET="$BUCKET_TARGET_MIB"
fi
python3 - "$CHECKPOINT" "$BEST_SHA" "$BASE_PROFILE_SHA" "$BASE_PRIME" "$BEST_RES" <<'PY'
import json,sys
cp,bsha,psha,p,r=sys.argv[1:]
d=json.load(open(cp))
if int(d.get('n',-1))!=27 or d.get('solver_fingerprint')!={'schema':3,'binary_sha256':bsha,'profile_sha256':psha}:
    raise SystemExit('refine checkpoint fingerprint mismatch')
...
} >"$META"
{
  ...
  printf 'B300_GRAND_REFINE_H_PREP=%q\n' "$H_PREP"; printf 'B300_GRAND_REFINE_I_PREP=%q\n' "$I_PREP"
} >"$SELECTED_ENV"
cat "$SELECTED_ENV"
echo "b300x8-grand-refine-firstpass OK H=$H_OK I=$I_OK backend=$BEST profile=$BEST_PROFILE wall_s=$BEST_WALL selected_env=$SELECTED_ENV prepare_stdout_isolated=1" >&2
