...
N="${1:-27}"
if (( $# > 0 )); then shift; fi
[[ "$N" == 27 ]] || { echo "HBM5 selector currently targets n=27" >&2; exit 2; }
...
CANDIDATES="${CANDIDATES:-forced warp_dense warp_sparse orbit_dense orbit_sparse}"
PREFIX="${PREFIX:-$ONEESAN_ROOT/work/b300_exact_hbm5_n${N}}"
...
has(){ [[ " $CANDIDATES " == *" $1 "* ]]; }
FORCED_BIN="$ONEESAN_BUILD_DIR/b300_hbm5_forced_n27"
WARP_DENSE_BIN="$ONEESAN_BUILD_DIR/b300_hbm5_warp_dense_n27"
WARP_SPARSE_BIN="$ONEESAN_BUILD_DIR/b300_hbm5_warp_sparse_n27"
ORBIT_DENSE_BIN="$ONEESAN_BUILD_DIR/b300_hbm5_orbit_dense_n27"
ORBIT_SPARSE_BIN="$ONEESAN_BUILD_DIR/b300_hbm5_orbit_sparse_n27"

build_forced(){
  [[ "$REBUILD" == 1 || ! -x "$FORCED_BIN" ]] || return 0
...
# Resource table: compare register/spill cost before interpreting MC utilization.
printf 'backend\tkernel\tregisters\tstack_bytes\tspill_store_bytes\tspill_load_bytes\tsmem_bytes\tcmem0_bytes\n' >"$PTXAS"
PARSER="$ONEESAN_ROOT/scripts/bench/parse-ptxas-resources.py"
for mode in forced warp_dense warp_sparse orbit_dense orbit_sparse; do
  has "$mode" || continue
...
# Bucket plan-only is cheap and catches obvious n=27 HBM failures before smoke.
...
if not ok:raise SystemExit('no successful HBM5 candidate')
...
echo "HBM5 SELECTED backend=$BEST wall_s=$BEST_WALL residue=$BEST_RES" >&2
cat "$RESULT" >&2

BEST_WORK="$WORK_ROOT/b300_exact_hbm5_${BEST}_n27"
mkdir -p "$BEST_WORK"
