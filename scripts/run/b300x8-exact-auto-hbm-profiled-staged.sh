...
# selector will use. Defaults preserve profiles generated before flat/chunked
# orbit scheduling. psm=0 means occupancy-derived forward/reverse pools.
# shellcheck disable=SC1090
source "$PROFILE_FILE"
...
for x in ORBITCTA_FLAT ORBIT_QUAD_MLP ORBIT_QUAD_OVERLAP_LOCAL; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "bad $x in n21 profile" >&2; exit 3; }; done
...
if [[ "$ORBIT_QUAD_MLP" == 1 ]]; then
  ...
  [[ "$ORBIT_QUAD_OVERLAP_LOCAL" == 0 ]] && (( ORBIT_QUAD_LOCAL_DIRECT_MAX == 0 )) || { echo 'quad suboptions require ORBIT_QUAD_MLP=1' >&2; exit 3; }
fi
...
ORBIT_FIXED="$ONEESAN_BUILD_DIR/b300_profiled_orbit_${ORBIT_PROFILE}_flat${ORBITCTA_FLAT}_chunk${ORBITCTA_FLAT_CHUNK}_psm${ORBITCTA_FLAT_BLOCKS_PER_SM}_quad${ORBIT_QUAD_MLP}_qol${ORBIT_QUAD_OVERLAP_LOCAL}_qld${ORBIT_QUAD_LOCAL_DIRECT_MAX}_n27"
