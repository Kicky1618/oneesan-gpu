...
for x in ORBIT_PRECTX_COMPACT ORBIT_PRECTX_FLAT_BID ORBIT_PRECTX_FLAT_BID_FUSED ORBIT_PRECTX_WARPCOOP ORBITCTA_FLAT ORBITCTA_FLAT_DYNAMIC ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP ORBIT_QUAD_MLP ORBIT_QUAD_OVERLAP_LOCAL ORBIT_QUAD_SPARSE_DESC_MLP ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0 ORBIT_PRECTX_FORWARD ORBIT_PRECTX_REVERSE; do
  v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "bad $x in n21 profile" >&2; exit 3; }
done
case "$ORBITCTA_FLAT_DYNAMIC_BATCH" in 1|2|4|8|16) ;; *) echo 'bad ORBITCTA_FLAT_DYNAMIC_BATCH in n21 profile' >&2; exit 3;; esac
case "$ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES" in 0|1|2|4) ;; *) echo 'bad ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES in n21 profile' >&2; exit 3;; esac
...
case "$ORBIT_QUAD_CPASYNC_GROUP_COLS" in 1|2|4) ;; *) echo 'bad ORBIT_QUAD_CPASYNC_GROUP_COLS in n21 profile' >&2; exit 3;; esac
...
if [[ "$ORBITCTA_FLAT_DYNAMIC" == 1 ]]; then
  [[ "$ORBITCTA_FLAT" == 1 && "$ORBITCTA_FLAT_CHUNK" == 1 ]] || { echo 'dynamic n21 profile requires flat chunk=1' >&2; exit 3; }
  [[ "$ORBIT_QUAD_MLP" == 0 && "$ORBIT_PRECTX_WARPCOOP" == 0 ]] || { echo 'dynamic and quad/warpcoop are mutually exclusive' >&2; exit 3; }
  if (( ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES != 0 )); then
    (( ORBITCTA_FLAT_DYNAMIC_BATCH > 1 )) || { echo 'adaptive dynamic waves require dynamic batch >1' >&2; exit 3; }
  fi
else
  [[ "$ORBITCTA_FLAT_DYNAMIC_BATCH" == 1 ]] || { echo 'dynamic batch >1 requires dynamic queue' >&2; exit 3; }
  [[ "$ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP" == 0 ]] || { echo 'dynamic lease fusion requires dynamic queue' >&2; exit 3; }
  [[ "$ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES" == 0 ]] || { echo 'adaptive dynamic waves require dynamic queue' >&2; exit 3; }
fi
if [[ "$ORBIT_PRECTX_FLAT_BID" == 1 ]]; then
  ...
  [[ "$ORBIT_QUAD_MLP" == 0 && "$ORBIT_PRECTX_WARPCOOP" == 0 ]] || { echo 'flat-bid and quad/warpcoop n21 profile are mutually exclusive' >&2; exit 3; }
else
  [[ "$ORBIT_PRECTX_FLAT_BID_FUSED" == 0 ]] || { echo 'flat-bid-fused requires flat-bid' >&2; exit 3; }
fi
if [[ "$ORBIT_PRECTX_WARPCOOP" == 1 ]]; then
  [[ "$ORBITCTA_FLAT" == 1 ]] && (( ORBITCTA_FLAT_CHUNK > 1 )) || { echo 'warpcoop n21 profile requires chunked flat orbit CTA' >&2; exit 3; }
  [[ "$ORBIT_PRECTX_COMPACT" == 1 && "$ORBIT_PRECTX_FORWARD" == 1 && "$ORBIT_PRECTX_REVERSE" == 1 ]] || { echo 'warpcoop n21 profile requires compact forward+reverse prectx' >&2; exit 3; }
  [[ "$ORBIT_PRECTX_FLAT_BID" == 0 && "$ORBITCTA_FLAT_DYNAMIC" == 0 ]] || { echo 'warpcoop and flat-bid/dynamic are mutually exclusive' >&2; exit 3; }
fi
if [[ "$ORBIT_QUAD_MLP" == 1 ]]; then
  ...
  [[ "$ORBIT_PRECTX_FLAT_BID" == 0 && "$ORBIT_PRECTX_FLAT_BID_FUSED" == 0 && "$ORBITCTA_FLAT_DYNAMIC" == 0 ]] || { echo 'quad and flat-bid/dynamic are mutually exclusive' >&2; exit 3; }
else
  [[ "$ORBIT_QUAD_OVERLAP_LOCAL" == 0 ]] && (( ORBIT_QUAD_LOCAL_DIRECT_MAX == 0 )) && [[ "$ORBIT_QUAD_SPARSE_DESC_MLP" == 0 && "$ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0" == 0 ]] && [[ "$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES" == 0 && "$ORBIT_QUAD_CPASYNC_GROUP_COLS" == 1 ]] || { echo 'quad suboptions require ORBIT_QUAD_MLP=1' >&2; exit 3; }
fi
...
if (( ORBIT_QUAD_CPASYNC_PREFETCH_BYTES != 0 )) || (( ORBIT_QUAD_CPASYNC_GROUP_COLS != 1 )); then
  [[ "$ORBIT_QUAD_OVERLAP_LOCAL" == 1 ]] || { echo 'quad cp.async tuning requires overlap-local' >&2; exit 3; }
fi
...
echo "profiled staged forced=$FORCED_PROFILE bin=$FORCED_PROFILE_BIN warp=$WARP_PROFILE orbit=$ORBIT_PROFILE flat=$ORBITCTA_FLAT chunk=$ORBITCTA_FLAT_CHUNK dynamic=$ORBITCTA_FLAT_DYNAMIC dynamic_batch=$ORBITCTA_FLAT_DYNAMIC_BATCH dynamic_fuse_lease_prep=$ORBITCTA_FLAT_DYNAMIC_FUSE_LEASE_PREP dynamic_adaptive_waves=$ORBITCTA_FLAT_DYNAMIC_ADAPTIVE_WAVES flat_blocks_per_sm=$ORBITCTA_FLAT_BLOCKS_PER_SM pool_mode=$([[ "$ORBITCTA_FLAT_BLOCKS_PER_SM" == 0 ]] && echo occupancy || echo per_sm) bid=$ORBIT_PRECTX_FLAT_BID bid_fused=$ORBIT_PRECTX_FLAT_BID_FUSED warpcoop=$ORBIT_PRECTX_WARPCOOP quad=$ORBIT_QUAD_MLP quad_overlap_local=$ORBIT_QUAD_OVERLAP_LOCAL quad_local_direct_max=$ORBIT_QUAD_LOCAL_DIRECT_MAX quad_sparse_desc_mlp=$ORBIT_QUAD_SPARSE_DESC_MLP quad_local0=$ORBIT_QUAD_OVERLAP_BYPASS_LOCAL0 quad_group_cols=$ORBIT_QUAD_CPASYNC_GROUP_COLS quad_prefetch_bytes=$ORBIT_QUAD_CPASYNC_PREFETCH_BYTES rebuild_buckets=$REBUILD_BUCKETS" >&2
