...
if [[ "$REBUILD" == 1 || ! -x "$BIN" ]]; then
  ...
  HYBRID_SRC="$ISO/final_main_hybrid_ilp8.cu"
  ...
  [[ "$RANDOM_CG" == 0 ]] || grep -Fq 'b300_rankstate_random_load_cg(in+pj7)' "$FINAL_SRC"
  [[ "$WARP_SCAN" == 0 ]] || grep -Fq 'const int ql=p-2-int(lane)' "$FINAL_SRC"

  echo "=== compile B300 hybrid ILP4/ILP8 threshold=$ILP8_MIN_STATES random_cg=$RANDOM_CG warpscan=$WARP_SCAN ===" >&2
...
echo "B300 x8 saturation-main-hybrid-ilp8 n=$N rows=$ROWS threads=$THREADS target=${TARGET_MIB}MiB plan=${PLAN_MIB}MiB window=$MAX_WINDOW ilp8_min_states=$ILP8_MIN_STATES random_cg=$RANDOM_CG warpscan=$WARP_SCAN" >&2
echo 'features=main_rankstate_ilp4_ilp8_hybrid,per_launch_state_threshold,closure_warp,dualmask,closure_contrib_shift_cross_tables,random_cg_optional,closure_warpscan_optional,concurrent_io' >&2
echo "BIN=$BIN" >&2
