...
PRECTX_COMPACT="${PRECTX_COMPACT:-0}"; PRECTX_FLAT_BID="${PRECTX_FLAT_BID:-0}"
...
for x in WINDOW4 DIRECTGATHER64 DIRECTGATHER_SPARSE64 DIRECTGATHER_SORT_RANKS PAIR_MLP CPASYNC_PAIR QUAD_MLP QUAD_OVERLAP_LOCAL PRECTX_FORWARD PRECTX_REVERSE PRECTX_COMPACT PRECTX_FLAT_BID ORBITCTA_FLAT FORCE7 PM_ACCUM PTXAS_VERBOSE; do
...
echo "built $BIN backend=$BACKEND_TAG flat=$ORBITCTA_FLAT flat_chunk=$ORBITCTA_FLAT_CHUNK depthmajor=1 directgather64=$DIRECTGATHER64 sparse64=$DIRECTGATHER_SPARSE64 sort_ranks=$DIRECTGATHER_SORT_RANKS pair_mlp=$PAIR_MLP cpasync_pair=$CPASYNC_PAIR quad_mlp=$QUAD_MLP quad_overlap_local=$QUAD_OVERLAP_LOCAL quad_local_direct_max=$QUAD_LOCAL_DIRECT_MAX prectx_forward=$PRECTX_FORWARD prectx_reverse=$PRECTX_REVERSE prectx_compact=$PRECTX_COMPACT prectx_flat_bid=$PRECTX_FLAT_BID orbitcta_col_ilp=$ORBITCTA_COL_ILP pm_accum=$PM_ACCUM window4=$WINDOW4 force7=$FORCE7" >&2
if [[ "$ORBITCTA_FLAT" == 1 ]]; then
  echo "run: BUCKET_THREADS=${BUCKET_THREADS:-256} flat_pool=auto-occupancy [BUCKET_ORBITCTA_FLAT_BLOCKS_PER_SM=<tune>] flat_chunk=$ORBITCTA_FLAT_CHUNK prectx_flat_bid=$PRECTX_FLAT_BID BUCKET_LOW_GRID_X=${BUCKET_LOW_GRID_X:-16} BUCKET_LOW_GRID_Y=${BUCKET_LOW_GRID_Y:-8} $BIN $N <target_mib> <max_window> 8 <mod>" >&2
else
