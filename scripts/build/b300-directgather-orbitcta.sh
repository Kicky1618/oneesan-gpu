...
for x in WINDOW4 DIRECTGATHER64 DIRECTGATHER_SPARSE64 DIRECTGATHER_SORT_RANKS PAIR_MLP CPASYNC_PAIR QUAD_MLP QUAD_OVERLAP_LOCAL PRECTX_FORWARD PRECTX_REVERSE PRECTX_COMPACT ORBITCTA_FLAT FORCE7 PM_ACCUM PTXAS_VERBOSE; do
...
if [[ "$ORBITCTA_FLAT" == 1 ]]; then
  echo "run: BUCKET_THREADS=${BUCKET_THREADS:-256} flat_pool=auto-occupancy [BUCKET_ORBITCTA_FLAT_BLOCKS_PER_SM=<tune>] flat_chunk=$ORBITCTA_FLAT_CHUNK BUCKET_LOW_GRID_X=${BUCKET_LOW_GRID_X:-16} BUCKET_LOW_GRID_Y=${BUCKET_LOW_GRID_Y:-8} $BIN $N <target_mib> <max_window> 8 <mod>" >&2
else
