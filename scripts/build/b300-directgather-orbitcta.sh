...
if [[ "$ORBITCTA_FLAT" == 1 ]]; then
  echo "run: BUCKET_THREADS=${BUCKET_THREADS:-256} flat_pool=auto-occupancy [BUCKET_ORBITCTA_FLAT_BLOCKS_PER_SM=<tune>] flat_chunk=$ORBITCTA_FLAT_CHUNK prectx_flat_bid=$PRECTX_FLAT_BID fused=$PRECTX_FLAT_BID_FUSED prectx_warpcoop=$PRECTX_WARPCOOP BUCKET_LOW_GRID_X=${BUCKET_LOW_GRID_X:-16} BUCKET_LOW_GRID_Y=${BUCKET_LOW_GRID_Y:-8} $BIN $N <target_mib> <max_window> 8 <mod>" >&2
else
