...
SCHEDULE_NEW='''        PreparedWindow pw;pw.wp=wp;pw.groups.reserve(nj);
        for(int g=0;g<nj;++g)pw.groups.push_back(prepare_group(W,pw.wp,g));
        std::sort(pw.groups.begin(),pw.groups.end(),[](auto const&a,auto const&b){if(a.work!=b.work)return a.work>b.work;if(a.ms.size!=b.ms.size)return a.ms.size>b.ms.size;if(a.ds.size!=b.ds.size)return a.ds.size>b.ds.size;return a.g<b.g;});
...
STAGE_NEW='''    size_t staged_group_count=0;for(auto const&pw:schedule)staged_group_count+=pw.groups.size();
    ...
    if(W==28&&ng==8&&effective_target_mib==16384&&max_window==14&&(staged_group_count!=16384||static_lpt_group_min!=2044||static_lpt_group_max!=2052||staged_group_meta_total_bytes!=228327424ull||staged_group_meta_max_bytes!=28596672ull||static_lpt_work_sum!=1812909037294ull||static_lpt_work_min!=226613163686ull||static_lpt_work_max!=226614905940ull)){std::cerr<<"static LPT default-plan proof mismatch: groups="<<staged_group_count<<" group_min="<<static_lpt_group_min<<" group_max="<<static_lpt_group_max<<" total_bytes="<<staged_group_meta_total_bytes<<" max_bytes="<<staged_group_meta_max_bytes<<" work_sum="<<static_lpt_work_sum<<" work_min="<<static_lpt_work_min<<" work_max="<<static_lpt_work_max<<"\n";return 14;}
...
def main()->None:
    ...
    text=once(text,SCHEDULE_OLD,SCHEDULE_NEW,'window deterministic LPT assignment')
    ...
    for required in ('std::vector<std::vector<int>> by_gpu','pw.by_gpu.resize(ng)','pw.assigned_work.assign(ng,0)','if(a.work!=b.work)return a.work>b.work','if(a.ms.size!=b.ms.size)return a.ms.size>b.ms.size','return a.g<b.g','for(int q:pw.by_gpu[d])','ctx[d].stage_group_meta(staged_group_meta[d])','static_lpt_work_sum!=1812909037294ull','static_lpt_work_min!=226613163686ull','static_lpt_work_max!=226614905940ull','tie_break=work_main_block_group','scheduler=static_lpt','copy_mode=H2D_once_local_then_D2D_per_group','static LPT default-plan proof mismatch:'):
    ...
    print(f'lowered {a.out} scheduler=static_lpt work_stealing=0 deterministic_tie_break=work_main_block_group metadata_replication=0 per_gpu_local_meta_ids=1 staged_h2d_once_local=1 per_group_constant_copy=d2d_sync default_expected_group_min=2044 default_expected_group_max=2052 default_expected_max_mib_per_gpu=27.271911621 default_expected_total_h2d_gib=0.212646484375')
