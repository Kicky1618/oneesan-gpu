...
PAIR_MLP="${PAIR_MLP:-1}"
COL_ILP="${COL_ILP:-2}"
OUT="${OUT:-oneesan_cuda_gridfp_b300_directgather_pair${PAIR_MLP}_colilp${COL_ILP}_n${N}}"

N="$N" OUT="$OUT" ARCH="${ARCH:-native}" \
  LOW_LUT_K="${LOW_LUT_K:-$(((N + 1) / 2))}" \
  TRANSPOSE_MODE="${TRANSPOSE_MODE:-pipeline}" \
  COL_ILP="$COL_ILP" DEPTHMAJOR=1 FORCE7=0 MLP_WINDOW4=1 \
  PAIR_MLP="$PAIR_MLP" PREFETCH_NEXT="${PREFETCH_NEXT:-0}" PM_ACCUM="${PM_ACCUM:-1}" \
...
echo "preset=directgather-pair-fast n=$N pair_mlp=$PAIR_MLP col_ilp=$COL_ILP depthmajor=1 window4=1 pm_accum=${PM_ACCUM:-1}" >&2
echo "run example:" >&2
