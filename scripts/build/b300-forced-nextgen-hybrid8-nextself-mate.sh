...
TAG="mate_w${NEXTSELF_WIDTH}_d${NEXTSELF_DISTANCE}_t${HYBRID_THRESHOLD}_h${HIGH_DROP_CHUNK}_cg${RANDOM_CG}_l2${RANDOM_CG_L2_FETCH_BYTES}_pre${PREFETCH_L2}_dual${DUALMASK}_cb${CLOSURE_BATCH}_r${MAXRREGCOUNT}_$$"
...
N=27 ARCH="$ARCH" OUT="$SELF_BIN" HIGH_DROP_CHUNK="$HIGH_DROP_CHUNK" HYBRID_THRESHOLD="$HYBRID_THRESHOLD" NEXTSELF_WIDTH="$NEXTSELF_WIDTH" NEXTSELF_DISTANCE="$NEXTSELF_DISTANCE" \
...
NEXT="$ISO/self_plus_mate.cu"; python3 "$ONEESAN_ROOT/scripts/build/gen-b300-mainrec-hybrid8-next-mate-prefetch.py" "$SRC" "$NEXT" "$NEXTSELF_WIDTH" "$NEXTSELF_DISTANCE" >"$ISO/mate.transform.out"
...
[[ -x "$OUT" ]] || exit 5
echo "built $OUT"
echo "  nextgen_forced=1 recurrence_hybrid_ilp8=1 recurrence_hybrid_ilp8_min_states=$HYBRID_THRESHOLD recurrence_hybrid_ilp8_nextself=1 recurrence_hybrid_ilp8_nextself_width=$NEXTSELF_WIDTH recurrence_hybrid_ilp8_nextself_distance=$NEXTSELF_DISTANCE next_mate_prefetch=1"
