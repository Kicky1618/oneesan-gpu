...
lines.insert(i+3,'  -DP10DC_ORBITCTA_FLAT_DYNAMIC_PIPE2_PRODUCER_PRECTX_WARPCOOP="$PRODUCER_PRECTX_WARPCOOP" \\\n')
src=''.join(lines)
...
line=line[:q]+' pipe2_producer_warp=1 pipe2_producer_worker_weight=$PRODUCER_WORKER_WEIGHT pipe2_producer_prectx_warpcoop=$PRODUCER_PRECTX_WARPCOOP'+line[q:]
...
if [[ "$PATCH_ONLY" == 1 ]]; then
  ...
  echo "b300_pipe2_producer_warp_patch=OK quad_mlp=$QUAD_MLP col_ilp=$COL_ILP producer_worker_weight=$PRODUCER_WORKER_WEIGHT producer_prectx_warpcoop=$PRODUCER_PRECTX_WARPCOOP prectx_flat_bid=$PRECTX_FLAT_BID prectx_flat_bid_fused=$PRECTX_FLAT_BID_FUSED gpu_work=0"
  exit 0
fi
exec env ORBITCTA_FLAT=1 ORBITCTA_FLAT_CHUNK=1 ORBITCTA_FLAT_DYNAMIC=1 \
  ...
  PRODUCER_PRECTX_WARPCOOP="$PRODUCER_PRECTX_WARPCOOP" PRODUCER_WORKER_WEIGHT="$PRODUCER_WORKER_WEIGHT" \
