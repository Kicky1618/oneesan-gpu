...
TRANSPOSE_MODE="${TRANSPOSE_MODE:-pipeline}"; PM_ACCUM="${PM_ACCUM:-0}"; P10_DECODE="${P10_DECODE:-unrank}"; HIGH_CTX="${HIGH_CTX:-thread}"; TERNARY_KEY4="${TERNARY_KEY4:-1}"; PTXAS_VERBOSE="${PTXAS_VERBOSE:-0}"
...
NVCC_EXTRA=(); if [[ "$PTXAS_VERBOSE" == 1 ]]; then NVCC_EXTRA+=("-Xptxas=-v"); fi; if [[ "$HIGH_CTX" == shared ]]; then NVCC_EXTRA+=("-DP10D4_HIGH_CTX_SHARED=1"); else NVCC_EXTRA+=("-DP10D4_HIGH_CTX_SHARED=0"); fi
TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  ...
  "$SRC" -o "$OUT"
echo "built $OUT (closure=pattern10+depth4 ordinary_scan=0 cross_scan=0 depth_bytes_per_orbit=0.5_padded decode=$P10_DECODE high_ctx=$HIGH_CTX window=graph transpose=$TRANSPOSE_MODE pm_accum=$PM_ACCUM ternary_key4=$TERNARY_KEY4 ptxas_verbose=$PTXAS_VERBOSE)"
