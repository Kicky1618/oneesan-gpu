...
OUT="$(build_path "${OUT:-oneesan_cuda_gridfp_b300_rankformula_nometa_b${RANKFORMULA_NOMETA_BLOCK}_${TRANSPOSE_MODE}_n${N}}")"
...
echo "built $OUT (closure=pattern10-depthcode high_ctx=rankformula-nometa block=$RANKFORMULA_NOMETA_BLOCK per_code_metadata_bytes=0 ballot_unrank=1 transpose=$TRANSPOSE_MODE decode_load=$DEPTHCODE_DECODE_LOAD rankstream_lut_load=$RANKSTREAM_LUT_LOAD pm_accum=$PM_ACCUM ptxas_verbose=$PTXAS_VERBOSE)"
