...
def main()->None:
    ...
    for stale in ('__constant__ DeviceGroupMeta D_GROUP_META;','cudaMemcpyToSymbol(D_GROUP_META,c.dGroupMeta+pg.meta_id','cudaMemcpyDeviceToDevice),"staged group meta D2D"'):
        if stale in text: raise SystemExit(f'full metadata constant-copy artifact remains: {stale}')
    for required in ('__constant__ const DeviceGroupMeta* D_GROUP_META_PTR;','D_GROUP_META_PTR->main_dp','D_GROUP_META_PTR->block_dp','const DeviceGroupMeta*group_meta_ptr=c.dGroupMeta+pg.meta_id','cudaMemcpyToSymbol(D_GROUP_META_PTR,&group_meta_ptr,sizeof(group_meta_ptr))'):
    ...
    print(f'lowered {a.out} group_meta_source=staged_global_via_constant_pointer constant_payload_bytes_per_group=8 old_constant_payload_bytes_per_group=13936 payload_reduction=1742x per_group_symbol_copy_calls=1')
