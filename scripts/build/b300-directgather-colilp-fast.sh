...
PAIR_MLP="${PAIR_MLP:-0}"; QUAD_MLP="${QUAD_MLP:-0}"; QUAD_LOCAL_DIRECT_MAX="${QUAD_LOCAL_DIRECT_MAX:-0}"; QUAD_OVERLAP_LOCAL="${QUAD_OVERLAP_LOCAL:-0}"; QUAD_SPARSE_DESC_MLP="${QUAD_SPARSE_DESC_MLP:-0}"; QUAD_OVERLAP_BYPASS_LOCAL0="${QUAD_OVERLAP_BYPASS_LOCAL0:-0}"; QUAD_CPASYNC_PREFETCH_BYTES="${QUAD_CPASYNC_PREFETCH_BYTES:-0}"
...
if (( QUAD_CPASYNC_PREFETCH_BYTES != 0 )); then
  [[ "$QUAD_MLP" == 1 && "$QUAD_OVERLAP_LOCAL" == 1 && "$CPASYNC_PAIR" == 1 ]] || { echo "QUAD_CPASYNC_PREFETCH_BYTES requires quad overlap-local cp.async" >&2; exit 2; }
fi
