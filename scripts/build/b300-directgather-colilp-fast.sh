...
PAIR_MLP="${PAIR_MLP:-0}"; QUAD_MLP="${QUAD_MLP:-0}"; QUAD_LOCAL_DIRECT_MAX="${QUAD_LOCAL_DIRECT_MAX:-0}"; QUAD_OVERLAP_LOCAL="${QUAD_OVERLAP_LOCAL:-0}"; QUAD_SPARSE_DESC_MLP="${QUAD_SPARSE_DESC_MLP:-0}"; QUAD_OVERLAP_BYPASS_LOCAL0="${QUAD_OVERLAP_BYPASS_LOCAL0:-0}"; QUAD_CPASYNC_PREFETCH_BYTES="${QUAD_CPASYNC_PREFETCH_BYTES:-0}"; QUAD_CPASYNC_GROUP_COLS="${QUAD_CPASYNC_GROUP_COLS:-1}"
...
for x in PM_ACCUM DEPTHMAJOR FORCE7 MLP_WINDOW4 PAIR_MLP QUAD_MLP QUAD_OVERLAP_LOCAL QUAD_SPARSE_DESC_MLP QUAD_OVERLAP_BYPASS_LOCAL0 CPASYNC_PAIR CPASYNC_LOCAL_PAIR CPASYNC_OVERLAP_LOCAL_PAIR CPASYNC_OVERLAP_LOCAL_PIPE2 OVERLAP_LOCAL_CG SORTED PRECTX_FORWARD PRECTX_REVERSE PRECTX_COMPACT PREFETCH_NEXT DIRECTGATHER64 DIRECTGATHER_SPARSE64 HIGH_PLAN_PROFILE PTXAS_VERBOSE; do
...
case "$QUAD_CPASYNC_PREFETCH_BYTES" in 0|64|128|256) ;; *) echo "QUAD_CPASYNC_PREFETCH_BYTES must be 0,64,128,256" >&2; exit 2;; esac
case "$QUAD_CPASYNC_GROUP_COLS" in 1|2|4) ;; *) echo "QUAD_CPASYNC_GROUP_COLS must be 1,2,4" >&2; exit 2;; esac
...
if [[ "$QUAD_OVERLAP_BYPASS_LOCAL0" == 1 ]]; then
  [[ "$QUAD_MLP" == 1 && "$QUAD_OVERLAP_LOCAL" == 1 ]] || { echo "QUAD_OVERLAP_BYPASS_LOCAL0 requires QUAD_MLP=1 and QUAD_OVERLAP_LOCAL=1" >&2; exit 2; }
fi
if (( QUAD_CPASYNC_PREFETCH_BYTES != 0 )) || (( QUAD_CPASYNC_GROUP_COLS != 1 )); then
  [[ "$QUAD_MLP" == 1 && "$QUAD_OVERLAP_LOCAL" == 1 && "$CPASYNC_PAIR" == 1 ]] || { echo "QOL cp.async tuning requires quad overlap-local cp.async" >&2; exit 2; }
fi
...
TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  ...
  -DP10DC_RANKFORMULA_QUAD_OVERLAP_BYPASS_LOCAL0="$QUAD_OVERLAP_BYPASS_LOCAL0" \
  -DP10DC_RANKFORMULA_QUAD_CPASYNC_PREFETCH_BYTES="$QUAD_CPASYNC_PREFETCH_BYTES" \
  -DP10DC_RANKFORMULA_QUAD_CPASYNC_GROUP_COLS="$QUAD_CPASYNC_GROUP_COLS" \
