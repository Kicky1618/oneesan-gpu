...
PAIR_MLP="${PAIR_MLP:-0}"; PREFETCH_NEXT="${PREFETCH_NEXT:-0}"; DIRECTGATHER64="${DIRECTGATHER64:-0}"
MAXRREGCOUNT="${MAXRREGCOUNT:-0}"; PTXAS_VERBOSE="${PTXAS_VERBOSE:-1}"
OUT="${OUT:-oneesan_cuda_gridfp_b300_directgather_colilp${COL_ILP}_pair${PAIR_MLP}_dg64${DIRECTGATHER64}_n${N}}"
...
for x in PM_ACCUM DEPTHMAJOR FORCE7 MLP_WINDOW4 PAIR_MLP PREFETCH_NEXT DIRECTGATHER64 PTXAS_VERBOSE; do
...
echo "built $BIN (directmap=1 directgather=1 directgather64=$DIRECTGATHER64 depthmajor=$DEPTHMAJOR force7=$FORCE7 mlp_window4=$MLP_WINDOW4 pair_mlp=$PAIR_MLP prefetch_next=$PREFETCH_NEXT gather_mlp=1 group61=1 block=16 col_ilp=$COL_ILP pm_accum=$PM_ACCUM maxrregcount=$MAXRREGCOUNT transpose=$TRANSPOSE_MODE)" >&2
echo "run example:" >&2
