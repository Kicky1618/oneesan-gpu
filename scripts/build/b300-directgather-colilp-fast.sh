...
CPASYNC_PAIR="${CPASYNC_PAIR:-0}"; CPASYNC_LOCAL_PAIR="${CPASYNC_LOCAL_PAIR:-0}"; CPASYNC_OVERLAP_LOCAL_PAIR="${CPASYNC_OVERLAP_LOCAL_PAIR:-0}"; SORTED="${SORTED:-0}"
...
OUT="${OUT:-oneesan_cuda_gridfp_b300_directgather_colilp${COL_ILP}_pair${PAIR_MLP}_quad${QUAD_MLP}_cpa${CPASYNC_PAIR}_cpalocal${CPASYNC_LOCAL_PAIR}_overlap${CPASYNC_OVERLAP_LOCAL_PAIR}_sort${SORTED}_prectxf${PRECTX_FORWARD}r${PRECTX_REVERSE}_dg64${DIRECTGATHER64}_sp64${DIRECTGATHER_SPARSE64}_n${N}}"
...
for x in PM_ACCUM DEPTHMAJOR FORCE7 MLP_WINDOW4 PAIR_MLP QUAD_MLP CPASYNC_PAIR CPASYNC_LOCAL_PAIR CPASYNC_OVERLAP_LOCAL_PAIR SORTED PRECTX_FORWARD PRECTX_REVERSE PREFETCH_NEXT DIRECTGATHER64 DIRECTGATHER_SPARSE64 PTXAS_VERBOSE; do
