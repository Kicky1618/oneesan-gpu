...
PAIR_MLP="${PAIR_MLP:-0}"; QUAD_MLP="${QUAD_MLP:-0}"; QUAD_LOCAL_DIRECT_MAX="${QUAD_LOCAL_DIRECT_MAX:-0}"
...
OUT="${OUT:-oneesan_cuda_gridfp_b300_directgather_colilp${COL_ILP}_pair${PAIR_MLP}_quad${QUAD_MLP}_qld${QUAD_LOCAL_DIRECT_MAX}_cpa${CPASYNC_PAIR}_cpalocal${CPASYNC_LOCAL_PAIR}_overlap${CPASYNC_OVERLAP_LOCAL_PAIR}_pipe2${CPASYNC_OVERLAP_LOCAL_PIPE2}_cg${OVERLAP_LOCAL_CG}_sort${SORTED}_prectxf${PRECTX_FORWARD}r${PRECTX_REVERSE}_dg64${DIRECTGATHER64}_sp64${DIRECTGATHER_SPARSE64}_prof${HIGH_PLAN_PROFILE}_n${N}}"
...
for x in PM_ACCUM DEPTHMAJOR FORCE7 MLP_WINDOW4 PAIR_MLP QUAD_MLP CPASYNC_PAIR CPASYNC_LOCAL_PAIR CPASYNC_OVERLAP_LOCAL_PAIR CPASYNC_OVERLAP_LOCAL_PIPE2 OVERLAP_LOCAL_CG SORTED PRECTX_FORWARD PRECTX_REVERSE PREFETCH_NEXT DIRECTGATHER64 DIRECTGATHER_SPARSE64 HIGH_PLAN_PROFILE PTXAS_VERBOSE; do
