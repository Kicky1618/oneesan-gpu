...
__device__ __forceinline__ bool b300_high_main_state_active(){
    if constexpr(TARGET_W!=28)return false;
        ...
        // forced2window is exactly p=27..15 followed by p=14..1. The high
        // window therefore needs transition symbols 14..27; position 13 is
        // outside the transition pair and belongs to the fixed low prefix.
...
for required in ('b300_high_main_state_active','b300_high_state_drop_rank','b300_main_pair_rank_h','b300_pack_main_transition_cache','b300_high_state_advance(m1,p)','return b300_pack_low_window_main_mate(m);','p-14','14+3*c'):
...
print(f'generated {out} from {src}: high_main_recurrence=1 ilp=2 extra_state_bytes=0 p_range=27..15 trit_positions=14 trit_range=14..27 trit_bits=25 signed_delta_bits=35 height_bits=4 helper_before_gather=1 mate_hbm_store_per_state_step=8 high_drop_walk_or_table_loads_per_state_step=0 high_height_walk_per_state_step=0')
