...
# bounds register pressure. CPASYNC_PAIR, SORTED and PRECTX_FORWARD remain opt-in
# until their B300 exact A/B measurements show a win.
N="${N:-27}"
COL_ILP="${COL_ILP:-2}"
PAIR_MLP="${PAIR_MLP:-1}"
...
echo "b300-rankformula-hbm-max OK out=$OUT n=$N col_ilp=$COL_ILP depthmajor=${DEPTHMAJOR:-1} pair_mlp=$PAIR_MLP cpasync_pair=$CPASYNC_PAIR sorted=$SORTED prectx_forward=$PRECTX_FORWARD window4=$MLP_WINDOW4 prefetch_next=$PREFETCH_NEXT force7=$FORCE7 maxrregcount=${MAXRREGCOUNT:-0}" >&2
