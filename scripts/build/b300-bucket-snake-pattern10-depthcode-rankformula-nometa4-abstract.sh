...
for x in RANKFORMULA_NOMETA_WARPSHARE RANKFORMULA_NOMETA_COOPGROUP RANKFORMULA_NOMETA_COOP_UNROLL RANKFORMULA_ABSTRACT_SELECT8 PM_ACCUM TERNARY_KEY4 PTXAS_VERBOSE; do v="${!x}"; [[ "$v" == 0 || "$v" == 1 ]] || { echo "$x must be 0 or 1" >&2; exit 2; }; done
...
mode_suffix=""; [[ "$RANKFORMULA_NOMETA_WARPSHARE" == 1 ]] && mode_suffix="_warpshare"; [[ "$RANKFORMULA_NOMETA_COOPGROUP" == 1 ]] && mode_suffix="_coopgroup_u${RANKFORMULA_NOMETA_COOP_UNROLL}"; [[ "$RANKFORMULA_ABSTRACT_SELECT8" == 1 ]] && mode_suffix="${mode_suffix}_select8"
...
TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  ...
  "$SRC" -o "$OUT"
ABSTRACT_LUT_BYTES=94206; [[ "$RANKFORMULA_ABSTRACT_SELECT8" == 1 ]] && ABSTRACT_LUT_BYTES=171866
