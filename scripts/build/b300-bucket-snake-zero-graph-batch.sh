...
case "$TRANSPOSE_MODE" in
  sync) SRC_REL="src/cuda/b300/oneesan_cuda_gridfp_b300_bucket_snake_onepass_zero_graph_batch.cu" ;;
  events) SRC_REL="src/cuda/b300/oneesan_cuda_gridfp_b300_bucket_snake_onepass_zero_graph_batch_events.cu" ;;
  pipeline) SRC_REL="src/cuda/b300/oneesan_cuda_gridfp_b300_bucket_snake_onepass_zero_graph_batch_pipeline.cu" ;;
...
TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  ...
  "$SRC" -o "$OUT"
echo "built $OUT (forward=orbit reverse=split54 closure=zero window=graph transpose=$TRANSPOSE_MODE pm_accum=$PM_ACCUM)"
