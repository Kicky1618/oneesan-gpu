...
if len(sys.argv) != 5:
    raise SystemExit('usage: gen-b300-mainrec-ilp2-pair-block-load-policy.py INPUT.cu OUTPUT.cu PAIR_POLICY BLOCK_POLICY')
...
s = src.read_text()
if 'b300_mainrec_stageq_ilp2_pair_block_policy=' in s:
    raise SystemExit('source already contains Stage-Q ILP2 pair/block policy')
mn = re.search(r'// b300_mainrec_stagen_pair_block_policy=1 pair=(default|cg|cs) block=(default|cg|cs) cg_l2_bytes=(0|64|128|256)', s)
if not mn:
    raise SystemExit('Stage Q requires Stage-N pair/block policy marker')
...
def qexpr(kind: str, k: int, policy: str) -> str:
    ptr = f'in+pj{k}' if kind == 'pair' else f'in_block+bj{k}'
    direct = f'in[pj{k}]' if kind == 'pair' else f'in_block[bj{k}]'
    if policy == 'default':
        return direct
    if policy == 'cg':
        return f'b300_mainrec_stageq_ilp2_load_cg({ptr})'
    return f'b300_mainrec_stageq_ilp2_load_cs({ptr})'
...
if ids != [0, 1]:
    raise SystemExit(f'Stage Q expects exactly ILP2 lanes 0,1; got {ids}')
for k in ids:
    ...
    body = body[:pm.start()] + f'const Count pair{k}=hp{k}?{qexpr("pair", k, pair_policy)}:Count(0);' + body[pm.end():]
    bm = re.search(rf'const Count block{k}=hb{k}\?([^;]+):Count\(0\);', body)
    assert bm
    body = body[:bm.start()] + f'const Count block{k}=hb{k}?{qexpr("block", k, block_policy)}:Count(0);' + body[bm.end():]
for req in ('const Count self0=', 'const MateID m0='):
    if req not in body:
        raise SystemExit(f'Stage Q damaged ILP2 artifact: {req}')
s = s[:ilp2_start] + body + s[ilp2_end:]

insert_at = function_span(s, 'main_pull_kernel_ilp2')[0]
helper = ''
if 'cg' in (pair_policy, block_policy):
    helper += '''static_assert(sizeof(Count)==4,"Stage-Q ILP2 CG assumes 32-bit Count");
__device__ __forceinline__ Count b300_mainrec_stageq_ilp2_load_cg(const Count* p){
#if __CUDA_ARCH__
...
if 'cs' in (pair_policy, block_policy):
    helper += '''__device__ __forceinline__ Count b300_mainrec_stageq_ilp2_load_cs(const Count* p){
#if __CUDA_ARCH__
    return __ldcs(p);
#else
    return *p;
...
# The high-state kernel must remain byte-for-byte identical. Helpers are inserted
# before ILP2, so re-locate ILP8 after insertion before comparing.
new8_start, new8_end = function_span(s, 'main_pull_kernel_ilp8_hybrid')
if s[new8_start:new8_end] != ilp8_before:
    raise SystemExit('Stage Q changed ILP8 high-state kernel')
new2_start, new2_end = function_span(s, 'main_pull_kernel_ilp2')
final2 = s[new2_start:new2_end]
for k in ids:
    if final2.count(f'const Count pair{k}=hp{k}?{qexpr("pair", k, pair_policy)}:Count(0);') != 1:
        raise SystemExit(f'Stage Q final pair lane {k} mismatch')
    if final2.count(f'const Count block{k}=hb{k}?{qexpr("block", k, block_policy)}:Count(0);') != 1:
        raise SystemExit(f'Stage Q final block lane {k} mismatch')

s += (
    f'\n// b300_mainrec_stageq_ilp2_pair_block_policy=1 pair={pair_policy} block={block_policy} '
    f'high_pair={high_pair} high_block={high_block} high_pair_l2={high_pair_l2} high_block_l2={high_block_l2}\n'
)
out.parent.mkdir(parents=True, exist_ok=True)
out.write_text(s)
print(
    f'generated {out} from {src}: b300_mainrec_stageq_ilp2_pair_block_policy=1 '
    f'pair_policy={pair_policy} block_policy={block_policy} high_pair={high_pair} high_block={high_block} '
    f'high_pair_l2={high_pair_l2} high_block_l2={high_block_l2} lanes=2 ilp8_byte_identical=1 '
    'self_unchanged=1 mate_unchanged=1 semantics_unchanged=1'
)
