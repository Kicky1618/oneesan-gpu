...
if 'b300_mainrec_stageq_ilp2_pair_block_policy=' in s:
    raise SystemExit('source contains obsolete conflicting Stage-Q ILP2 marker; regenerate from pre-R upstream')
...
def rexpr(kind: str, k: int, policy: str) -> str:
    ...
    if policy == 'cg': return f'b300_mainrec_stager_ilp2_load_cg({ptr})'
...
if ids != [0, 1]: raise SystemExit(f'Stage R expects exactly ILP2 lanes 0,1; got {ids}')
for k in ids:
    pm = re.search(rf'const Count pair{k}=hp{k}\?([^;]+):Count\(0\);', body)
    bm = re.search(rf'const Count block{k}=hb{k}\?([^;]+):Count\(0\);', body)
    if not pm or not bm: raise SystemExit(f'ILP2 lane {k} pair/block anchor missing')
    body = body[:pm.start()] + f'const Count pair{k}=hp{k}?{rexpr("pair", k, pair_policy)}:Count(0);' + body[pm.end():]
    bm = re.search(rf'const Count block{k}=hb{k}\?([^;]+):Count\(0\);', body); assert bm
...
for req in ('const Count self0=', 'const MateID m0='):
    if req not in body: raise SystemExit(f'Stage R damaged ILP2 artifact: {req}')
...
if s[new8_start:new8_end] != ilp8_before: raise SystemExit('Stage R changed ILP8 high-state kernel')
new2_start, new2_end = function_span(s, 'main_pull_kernel_ilp2'); final2 = s[new2_start:new2_end]
for k in ids:
    if final2.count(f'const Count pair{k}=hp{k}?{rexpr("pair", k, pair_policy)}:Count(0);') != 1: raise SystemExit(f'Stage R final pair lane {k} mismatch')
    if final2.count(f'const Count block{k}=hb{k}?{rexpr("block", k, block_policy)}:Count(0);') != 1: raise SystemExit(f'Stage R final block lane {k} mismatch')
s += f'\n// b300_mainrec_stager_ilp2_pair_block_policy=1 pair={pair_policy} block={block_policy} high_pair={high_pair} high_block={high_block} high_pair_l2={high_pair_l2} high_block_l2={high_block_l2} stageq_preserved={1 if mq else 0}\n'
out.parent.mkdir(parents=True, exist_ok=True); out.write_text(s)
print(f'generated {out} from {src}: b300_mainrec_stager_ilp2_pair_block_policy=1 pair_policy={pair_policy} block_policy={block_policy} high_pair={high_pair} high_block={high_block} high_pair_l2={high_pair_l2} high_block_l2={high_block_l2} stageq_preserved={1 if mq else 0} lanes=2 ilp8_byte_identical=1 self_unchanged=1 mate_unchanged=1 semantics_unchanged=1')
