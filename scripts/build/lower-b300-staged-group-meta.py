...
def main()->None:
    ...
    for required in ('DeviceGroupMeta*dGroupMeta=nullptr','void stage_group_meta(const std::vector<DeviceGroupMeta>& h)','size_t meta_id=0','cudaMemcpyDeviceToDevice','B300_STAGED_META_MAX_MIB','staged group meta exceeds cap:','staged group meta exceeds HBM reserve:','reserve_mib=','cap_mib=','staged_group_meta.clear()'):
    ...
    print(f'lowered {a.out} staged_group_meta=1 per_group_meta_source=device staged_h2d_once=1 per_group_constant_copy=d2d_sync host_meta_rebuild_per_group=0 default_stage_cap_mib=512 stage_within_hbm_reserve=1')
