...
if [[ "$MODE" == two-row-runtime-multigpu && "$RUNTIME_CACHE_EDGES" == 0 ]]; then DEFAULT_OUT="${DEFAULT_OUT}_edgecache0"; fi
if [[ "$MODE" == two-row-runtime-multigpu && "$RUNTIME_FAST_P32M5_MOD" == 0 ]]; then DEFAULT_OUT="${DEFAULT_OUT}_slowmod"; fi
if [[ "$MODE" == two-row-runtime-multigpu && "$RUNTIME_POLL_GLOBAL_ERROR" == 1 ]]; then DEFAULT_OUT="${DEFAULT_OUT}_errorpoll"; fi
if [[ "$MODE" == two-row-runtime-multigpu && "$RUNTIME_PACK_SHARED_KEYS" == 0 ]]; then DEFAULT_OUT="${DEFAULT_OUT}_unpackedkeys"; fi
