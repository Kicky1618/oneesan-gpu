...
for name in RUNTIME_CACHE_EDGES RUNTIME_FAST_P32M5_MOD RUNTIME_POLL_GLOBAL_ERROR RUNTIME_PACK_SHARED_KEYS RUNTIME_FAST_DIV64 RUNTIME_PRIMITIVE_RANK_SETBITS RUNTIME_BROADWORD_SUPPORT RUNTIME_OWNER_FROM_BOUNDARIES RUNTIME_OWNER_RECIPROCAL RUNTIME_OWNER_FIXED54 RUNTIME_SUPPORT_RANK_SETBITS RUNTIME_SECTOR_OFFSET_TABLE RUNTIME_CACHE_SECTOR_ROW_BASE RUNTIME_OUTER_GROUP_TABLE RUNTIME_FAST_OUTSIDE_COMPACT RUNTIME_FAST_ERASE_TWO_BITS RUNTIME_FAST_DISCOVERY_VALIDITY RUNTIME_DISCOVERY_ENDPOINT_SCAN RUNTIME_FIND_RECENT_FIRST; do value="${!name}"; if [[ "$value" != 0 && "$value" != 1 ]]; then echo "$name must be 0 or 1" >&2; exit 2; fi; done
