...
# Remove the harmless marker declaration introduced solely to make the exact
# transform anchor unique and insert the interval-only materializer there.
...
once(
'size_t countBytes=size_t(2*ms.size+2*ds.size)*sizeof(Count),mateBytes=size_t(ms.size)*sizeof(MateID);bool useMate=(countBytes+mateBytes<=target);\n    c.ensure(ms.size,ds.size,useMate,pg.mi.size(),pg.di.size());',
...
for stale in ('gather_block_kernel<<<bd,threads>>>(c.dD,ds.size)',
...
print(f'generated {out} from {src}: b300_block_mate_cache=1 window_reuse=1 conditional_scratch=1 gather_fused_materialize=1 interval_materialize=1')
