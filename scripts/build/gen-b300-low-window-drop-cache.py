...
# bits 0..29 low 15 MateValue symbols; bits 30..59 main-base minus block-base;
# bits 60..63 height entering the low suffix. Still exactly 8 bytes/state.
...
#pragma unroll
    for(int pos=14;pos>=0;--pos){
        ...
        if(v==R){delta+=static_cast<long long>(D_FULL_DP[pos-1][h])-static_cast<long long>(D_FULL_DP[pos][h]);--h;}
        else if(v==L){
...
# Target the main gather explicitly. After block-mate transformation there is a
# second `if(mates)mates[i]=m` in gather_block_kernel, so a generic substring
# anchor is intentionally forbidden here.
once('out[i]=global_load_main(g);if(mates)mates[i]=m;',
     'out[i]=global_load_main(g);if(mates)mates[i]=b300_pack_low_window_main_mate(m);',
     'main gather packed mate')
...
# p=1 stays on the exact uncached source-centric boundary kernel because the
# packed low cache intentionally omits the high labels.
...
print(f'generated {out} from {src}: low_window_packed_main_cache=1 cache_bytes_per_main=8 high_prefix_walk=0 low_drop_max_steps=12 p1_uncached=1 w28_k13_only=1 explicit_main_gather_anchor=1')
