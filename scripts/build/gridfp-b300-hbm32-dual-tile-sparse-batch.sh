...
echo "  scratch-free full peer-kernel exact:"
echo "    ONEESAN_DUAL_SHUFFLE=peer-kernel ONEESAN_DUAL_THREADS=256 \\"
echo "      python3 scripts/solve/solve_b300_exact_batch.py $N --binary $OUT"
echo "  scratch-free static reachability pruning:"
echo "    ONEESAN_DUAL_SHUFFLE=peer-pruned ONEESAN_DUAL_PRUNE_LAUNCH_KIB=8192 ONEESAN_DUAL_THREADS=256 \\"
echo "      python3 scripts/solve/solve_b300_exact_batch.py $N --binary $OUT"
echo "  scratch-free exact dynamic zero-tile pruning:"
echo "    ONEESAN_DUAL_SHUFFLE=peer-dynamic ONEESAN_DUAL_SCAN_CHUNK_MIB=16 \\"
echo "      ONEESAN_DUAL_PRUNE_LAUNCH_KIB=8192 ONEESAN_DUAL_THREADS=256 \\"
echo "      python3 scripts/solve/solve_b300_exact_batch.py $N --binary $OUT"
echo "  ONEESAN_DUAL_PRUNE_LAUNCH_KIB=0 gives minimum bytes; larger values trade bytes for fewer GPU operations."
echo "  peer-dynamic reports scan time and actual per-modulus shuffled TiB; keep it only if scan_s is repaid by NVLink savings."
