...
helper=r'''
#ifndef B300_BLOCK_CLOSURE_BATCH
#define B300_BLOCK_CLOSURE_BATCH 4
#endif
static_assert(B300_BLOCK_CLOSURE_BATCH==2 || B300_BLOCK_CLOSURE_BATCH==4,
              "B300_BLOCK_CLOSURE_BATCH must be 2 or 4");
...
__device__ __forceinline__ void b300_block_closure_quad_flush(
    ...
    if constexpr(B300_BLOCK_CLOSURE_BATCH==4){
...
__device__ __forceinline__ void b300_block_closure_quad_emit(
    ...
    if(q.n==0)q.r0=r;else if(q.n==1)q.r1=r;
    else if constexpr(B300_BLOCK_CLOSURE_BATCH==4){if(q.n==2)q.r2=r;else q.r3=r;}
    ++q.n;if(q.n==B300_BLOCK_CLOSURE_BATCH)b300_block_closure_quad_flush(acc,in_main,q);
}
...
if hot.count('b300_block_closure_quad_emit(acc,in_main,b300_quad,')!=3:raise SystemExit('closure batch emit count mismatch')
if hot.count('b300_block_closure_quad_flush(acc,in_main,b300_quad);')<1:raise SystemExit('closure batch tail flush missing')
if 'block_pull_add_rank(acc,in_main,' not in hot:raise SystemExit('endpoint immediate source load unexpectedly removed')
qdecl=hot.find('B300BlockClosureQuad b300_quad{};');qflush=hot.find('b300_block_closure_quad_flush(acc,in_main,b300_quad);',qdecl);qout=hot.find('out_block[i]=acc;',qdecl)
if qdecl<0 or qflush<qdecl or qout<qflush:raise SystemExit('closure batch scope/order invalid')
for req in ('B300_BLOCK_CLOSURE_BATCH','B300BlockClosureQuad','const Count v0=','q.n==B300_BLOCK_CLOSURE_BATCH'):
    if req not in s:raise SystemExit(f'closure batch artifact missing: {req}')
out.parent.mkdir(parents=True,exist_ok=True);out.write_text(s)
print(f'generated {out} from {src}: b300_block_closure_batch=1 batch_macro=B300_BLOCK_CLOSURE_BATCH allowed=2,4 queued_candidate_sites=3 endpoint_load_immediate=1 closure_tail_flush=1 flush_scope=closure extra_state_bytes=0 production_default=off')
