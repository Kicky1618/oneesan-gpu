...
__global__ void b300_main_pull_rankstate_ilp4_kernel(
    ...
    for(Code base=tid;base<n;base+=4*grid){
        ...
        const bool hb0=nblock&&mget(m0,p)==N,hb1=v1&&nblock&&mget(m1,p)==N,hb2=v2&&nblock&&mget(m2,p)==N,hb3=v3&&nblock&&mget(m3,p)==N;
        ...
        if(v3){uint64_t a=uint64_t(self3)+pair3+block3;if(a>=mod)a-=mod;if(a>=mod)a-=mod;out_main[i3]=Count(a);const MateValue mv=mget(m3,p);rank_state[i3]=b300_pack_rank_state(rd3+b300_rank_delta_step(mv,p,h3),b300_rank_height_advance(h3,mv));}
    }
}
'''
...
for required in ('b300_main_pull_rankstate_ilp4_kernel','base+=4*grid','pair3=','block3=','rank_state[i3]=b300_pack_rank_state'):
    if required not in s: raise SystemExit(f'missing rank-state ILP4 artifact: {required}')
out.parent.mkdir(parents=True,exist_ok=True);out.write_text(s)
print(f'generated {out} from {src}: b300_rank_state_ilp4=1 destinations_per_thread=4 packed_rank_state=1 index_first=1 hbm_request_overlap=pair,block,four_destinations recurrence_exact=1 register_pressure_requires_ab=1')
