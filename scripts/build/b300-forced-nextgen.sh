...
[[ "$RECURRENCE_HYBRID_ILP8" == 0 || "$RECURRENCE_HYBRID_ILP8" == 1 ]] || { echo 'RECURRENCE_HYBRID_ILP8 must be 0/1' >&2; exit 2; }
...
TAG="hd${HIGH_DROP_CHUNK}_ilp${RECURRENCE_ILP}_hyb${RECURRENCE_HYBRID_ILP8}_hybt${RECURRENCE_HYBRID_ILP8_MIN_STATES}_cg${RANDOM_CG}_cgl2${RANDOM_CG_L2_FETCH_BYTES}_pre${PREFETCH_L2}_dual${DUALMASK}_cb${CLOSURE_BATCH}_r${MAXRREGCOUNT}_$$"
...
if [[ "$RECURRENCE_HYBRID_ILP8" == 1 ]]; then grep -Fq 'main_pull_kernel_ilp8_hybrid' "$CURRENT"; grep -Fq 'base+=Code(8)*grid' "$CURRENT"; grep -Fq "if(ms.size>=Code($RECURRENCE_HYBRID_ILP8_MIN_STATES))" "$CURRENT"; fi
if [[ "$RANDOM_CG" == 1 ]]; then
...
if [[ "$RECURRENCE_ILP" == 2 && "$RECURRENCE_HYBRID_ILP8" == 0 && "$RANDOM_CG" == 0 && "$RANDOM_CG_L2_FETCH_BYTES" == 0 && "$PREFETCH_L2" == 0 && "$DUALMASK" == 0 && "$CLOSURE_BATCH" == 0 && "$MAXRREGCOUNT" == 0 ]]; then
  cp "$BASE_BIN" "$OUT"; chmod +x "$OUT"
  echo "built $OUT"
  echo "  nextgen_forced=1 high_drop_chunk=$HIGH_DROP_CHUNK recurrence_ilp=2 recurrence_hybrid_ilp8=0 recurrence_hybrid_ilp8_min_states=$RECURRENCE_HYBRID_ILP8_MIN_STATES random_cg=0 prefetch_l2=0 random_cg_l2_fetch_bytes=0 dualmask=0 closure_batch=0 maxrregcount=0"
...
echo "built $OUT"
echo "  nextgen_forced=1 high_drop_chunk=$HIGH_DROP_CHUNK recurrence_ilp=$RECURRENCE_ILP recurrence_hybrid_ilp8=$RECURRENCE_HYBRID_ILP8 recurrence_hybrid_ilp8_min_states=$RECURRENCE_HYBRID_ILP8_MIN_STATES random_cg=$RANDOM_CG prefetch_l2=$PREFETCH_L2 random_cg_l2_fetch_bytes=$RANDOM_CG_L2_FETCH_BYTES dualmask=$DUALMASK closure_batch=$CLOSURE_BATCH maxrregcount=$MAXRREGCOUNT"
...
echo "  transform_order=production_recurrence,recurrence_ilp,recurrence_hybrid_ilp8,random_cg_l2_fetch,prefetch_l2,dualmask,closure_batch,register_cap"
echo "  ptxas_log=$FINAL_ERR"
