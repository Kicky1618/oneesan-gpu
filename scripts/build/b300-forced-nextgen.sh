...
HIGH_DROP_CHUNK="${HIGH_DROP_CHUNK:-0}"; RECURRENCE_ILP="${RECURRENCE_ILP:-2}"; RANDOM_CG="${RANDOM_CG:-0}"; DUALMASK="${DUALMASK:-0}"; CLOSURE_BATCH="${CLOSURE_BATCH:-0}"; MAXRREGCOUNT="${MAXRREGCOUNT:-0}"; PTXAS_VERBOSE="${PTXAS_VERBOSE:-1}"
...
TAG="hd${HIGH_DROP_CHUNK}_ilp${RECURRENCE_ILP}_cg${RANDOM_CG}_dual${DUALMASK}_cb${CLOSURE_BATCH}_r${MAXRREGCOUNT}_$$"
...
if [[ "$RECURRENCE_ILP" == 2 && "$RANDOM_CG" == 0 && "$DUALMASK" == 0 && "$CLOSURE_BATCH" == 0 && "$MAXRREGCOUNT" == 0 ]]; then
  cp "$BASE_BIN" "$OUT"; chmod +x "$OUT"
  echo "built $OUT"
  echo "  nextgen_forced=1 high_drop_chunk=$HIGH_DROP_CHUNK recurrence_ilp=2 random_cg=0 dualmask=0 closure_batch=0 maxrregcount=0"
...
echo "built $OUT"
echo "  nextgen_forced=1 high_drop_chunk=$HIGH_DROP_CHUNK recurrence_ilp=$RECURRENCE_ILP random_cg=$RANDOM_CG dualmask=$DUALMASK closure_batch=$CLOSURE_BATCH maxrregcount=$MAXRREGCOUNT"
echo "  source_after_all=$CURRENT"
echo "  production_chain_proof_gates=1 experimental_post_transforms=1 extra_state_bytes=0"
echo "  transform_order=production_recurrence,recurrence_ilp,random_cg,dualmask,closure_batch,register_cap"
echo "  ptxas_log=$FINAL_ERR"
