...
if (( N >= 27 )); then DEFAULT_SHARD8=1; DEFAULT_LOW_DROP_CACHE=1; DEFAULT_LOW_DROP_CHUNK=1; DEFAULT_LOW_BLOCK_CACHE=1; else DEFAULT_SHARD8=0; DEFAULT_LOW_DROP_CACHE=0; DEFAULT_LOW_DROP_CHUNK=0; DEFAULT_LOW_BLOCK_CACHE=0; fi
...
# Still A/B-only until an actual B300 compile/residue comparison is seen.
HIGH_DROP_CHUNK="${HIGH_DROP_CHUNK:-0}"
RUNTIME_THREADS="${RUNTIME_THREADS:-1}"
PTXAS_VERBOSE="${PTXAS_VERBOSE:-1}"

for name in MAIN_MATE_CACHE MAIN_PULL BLOCK_PULL BLOCK_MATE_CACHE FAST_SHARD_ADDRESS8 LOW_DROP_CACHE LOW_DROP_CHUNK LOW_BLOCK_CACHE HIGH_DROP_CHUNK RUNTIME_THREADS PTXAS_VERBOSE; do
