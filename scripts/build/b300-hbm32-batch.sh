...
for name in MAIN_MATE_CACHE MAIN_PULL BLOCK_PULL BLOCK_MATE_CACHE FAST_SHARD_ADDRESS8 LOW_DROP_CACHE LOW_DROP_CHUNK LOW_BLOCK_CACHE HIGH_DROP_CHUNK LOW_MAIN_RECURRENCE HIGH_MAIN_RECURRENCE MAIN_RECURRENCE RUNTIME_THREADS PTXAS_VERBOSE; do
...
if [[ "$MAIN_RECURRENCE" == 1 && ( "$MAIN_PULL_ILP" != 2 || "$LOW_DROP_CACHE" != 1 || "$LOW_DROP_CHUNK" != 1 || "$MAIN_MATE_CACHE" != 1 || "$MAIN_PULL" != 1 ) ]]; then echo "MAIN_RECURRENCE=1 requires MAIN_PULL_ILP=2 LOW_DROP_CACHE=LOW_DROP_CHUNK=MAIN_MATE_CACHE=MAIN_PULL=1" >&2; exit 2; fi
if (( LOW_MAIN_RECURRENCE + HIGH_MAIN_RECURRENCE + MAIN_RECURRENCE > 1 )); then echo "LOW_MAIN_RECURRENCE, HIGH_MAIN_RECURRENCE and MAIN_RECURRENCE are mutually exclusive A/B transforms" >&2; exit 2; fi
