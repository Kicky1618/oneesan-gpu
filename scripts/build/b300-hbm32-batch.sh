...
if (( N >= 27 )); then DEFAULT_SHARD8=1; DEFAULT_LOW_DROP_CACHE=1; DEFAULT_LOW_DROP_CHUNK=1; else DEFAULT_SHARD8=0; DEFAULT_LOW_DROP_CACHE=0; DEFAULT_LOW_DROP_CHUNK=0; fi
...
# Kept opt-in until B300 ptxas/local-memory and a short exact residue A/B are
# observed. It does not change authoritative state or scratch layout.
HIGH_DROP_CHUNK="${HIGH_DROP_CHUNK:-0}"
RUNTIME_THREADS="${RUNTIME_THREADS:-1}"
PTXAS_VERBOSE="${PTXAS_VERBOSE:-1}"

for name in MAIN_MATE_CACHE MAIN_PULL BLOCK_PULL BLOCK_MATE_CACHE FAST_SHARD_ADDRESS8 LOW_DROP_CACHE LOW_DROP_CHUNK HIGH_DROP_CHUNK RUNTIME_THREADS PTXAS_VERBOSE; do
  value="${!name}"
...
if [[ "$LOW_DROP_CHUNK" == 1 && "$LOW_DROP_CACHE" != 1 ]]; then
  echo "LOW_DROP_CHUNK=1 requires LOW_DROP_CACHE=1" >&2; exit 2
fi
if [[ "$HIGH_DROP_CHUNK" == 1 && ( "$LOW_DROP_CACHE" != 1 || "$BLOCK_PULL" != 1 ) ]]; then
  echo "HIGH_DROP_CHUNK=1 requires LOW_DROP_CACHE=1 BLOCK_PULL=1" >&2; exit 2
fi
...
if [[ "$LOW_DROP_CHUNK" == 1 ]]; then
  bash "$ONEESAN_ROOT/scripts/bench/b300-low-drop-chunk-proof.sh"
fi
...
if [[ "$LOW_DROP_CHUNK" == 1 ]]; then
  LOW_DROP_CHUNK_SRC="$ONEESAN_BUILD_DIR/b300_hbm32_batch_n${N}_low_drop_chunk.cu"
  python3 "$ONEESAN_ROOT/scripts/build/gen-b300-low-drop-chunk.py" "$BUILD_SRC" "$LOW_DROP_CHUNK_SRC"
  BUILD_SRC="$LOW_DROP_CHUNK_SRC"
fi
if [[ "$HIGH_DROP_CHUNK" == 1 ]]; then
  HIGH_DROP_CHUNK_SRC="$ONEESAN_BUILD_DIR/b300_hbm32_batch_n${N}_high_drop_chunk.cu"
  python3 "$ONEESAN_ROOT/scripts/build/gen-b300-high-drop-chunk.py" "$BUILD_SRC" "$HIGH_DROP_CHUNK_SRC"
  BUILD_SRC="$HIGH_DROP_CHUNK_SRC"
fi
if [[ "$FAST_SHARD_ADDRESS8" == 1 ]]; then
