...
TRANSPOSE_MODE="${TRANSPOSE_MODE:-sync}"; PM_ACCUM="${PM_ACCUM:-0}"; HIGH_CTX="${HIGH_CTX:-thread}"; P10_DECODE="${P10_DECODE:-unrank}"; TERNARY_KEY4="${TERNARY_KEY4:-1}"; PTXAS_VERBOSE="${PTXAS_VERBOSE:-0}"
...
[[ "$HIGH_CTX" == shared ]] && base="${base}_highctx"
...
TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  ...
  "$SRC" -o "$OUT"
echo "built $OUT (closure=pattern10+depth8 ordinary_scan=0 cross_scan=0 decode=$P10_DECODE high_ctx=$HIGH_CTX window=direct transpose=$TRANSPOSE_MODE pm_accum=$PM_ACCUM ternary_key4=$TERNARY_KEY4 ptxas_verbose=$PTXAS_VERBOSE)"
