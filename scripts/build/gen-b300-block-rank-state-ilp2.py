...
for req in (
    ...
    'block_pull_kernel<true,true>','b300_rankstate_ilp2_blocks'
):
...
__global__ void b300_block_pull_rankstate_ilp2_kernel(
    ...
    for(Code base=tid;base<n;base+=2*grid){
        ...
        const RankDelta nr0=rd0+b300_rank_delta_step(l0,p,h0),nr1=v1?rd1+b300_rank_delta_step(l1,p,h1):RankDelta(0);
        const Code j0=ep0?b300_sub_rank_delta(i0,rd0):Code(0),j1=ep1?b300_sub_rank_delta(i1,rd1):Code(0);
        ...
        const Count endpoint0=ep0?in_main[j0]:Count(0),endpoint1=ep1?in_main[j1]:Count(0);
        Count a0=endpoint0,a1=endpoint1;
...
new='''if(ds.size){if(useRankDelta)b300_block_pull_rankstate_ilp2_kernel<<<b300_rankstate_ilp2_blocks(ds.size,threads),threads,0,c.sBlock>>>(cur,c.dBlockMate,ds.size,dnext,p,c.dBlockRankDelta);'''
if s.count(old)!=1: raise SystemExit(f'block rank-state ILP2 launch anchor expected one match got {s.count(old)}')
s=s.replace(old,new,1)
for req in ('b300_rankstate_ilp2_blocks','__noinline__ Count b300_block_rankstate_closure','b300_block_pull_rankstate_ilp2_kernel','b300_rankstate_ilp2_blocks(ds.size,threads)','base+=2*grid','const Count endpoint0=','rank_state[i1]=b300_pack_rank_state'):
    if req not in s: raise SystemExit(f'missing block rank-state ILP2 artifact: {req}')
out.parent.mkdir(parents=True,exist_ok=True);out.write_text(s)
print(f'generated {out} from {src}: b300_block_rank_state_ilp2=1 destinations_per_thread=2 launch_cover_per_thread=2 launch_blocks=ceil_n_over_2threads_capped65535 endpoint_loads_issued_before_closure=1 closure_slow_path=noinline closure_register_isolation=1 packed_rank_state=1 rank_state_store_before_count_gather=1 register_live_range_reduced=1')
