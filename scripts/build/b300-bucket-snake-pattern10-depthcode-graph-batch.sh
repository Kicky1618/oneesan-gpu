...
case "$HIGH_CTX" in
  thread|resolved|resolved_delta|warp|warpstriped|warpstriped_delta|warpstriped_delta_cross5|warpstriped_delta_direct_cross5|warpstriped_delta_direct_affine_cross5) ;;
  *) echo "HIGH_CTX must be thread, resolved, resolved_delta, warp, warpstriped, warpstriped_delta, warpstriped_delta_cross5, warpstriped_delta_direct_cross5, or warpstriped_delta_direct_affine_cross5" >&2; exit 2;;
esac
...
echo "built $OUT (closure=pattern10-depthcode sidecar_bytes_per_orbit=0 temporary_depth_bytes=0 decode=payload-masks runtime_unrank=0 high_ctx=$HIGH_CTX decode_load=$DEPTHCODE_DECODE_LOAD window=graph transpose=$TRANSPOSE_MODE pm_accum=$PM_ACCUM ternary_key4=$TERNARY_KEY4 ptxas_verbose=$PTXAS_VERBOSE)"
