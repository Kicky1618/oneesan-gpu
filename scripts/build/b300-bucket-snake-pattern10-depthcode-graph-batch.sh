...
RANKCHUNK32_RANKPLANE="${RANKCHUNK32_RANKPLANE:-0}"
...
RANKFORMULA_BASE_DELTA="${RANKFORMULA_BASE_DELTA:-0}"
RANKFORMULA_SLOTMETA="${RANKFORMULA_SLOTMETA:-0}"
RANKFORMULA_SLOTROW32="${RANKFORMULA_SLOTROW32:-0}"
PM_ACCUM="${PM_ACCUM:-0}"
...
if [[ "$HIGH_CTX" == warpstriped_delta_direct_affine_rankformula_cross5 ]]; then
  ...
  RANKCHUNK32_BLOCK64=0
  if [[ "$RANKFORMULA_SLOTROW32" == 1 ]]; then RANKFORMULA_SLOTMETA=1; fi
  if [[ "$RANKFORMULA_SLOTMETA" == 1 ]]; then
    RANKFORMULA_SPARSE_BASE=1
    RANKFORMULA_RAWCODE=0
    RANKFORMULA_INLINE_CROSS=1
    RANKFORMULA_BASE_DELTA=1
  fi
fi
for x in RANKCHUNK32_ONESHFL RANKCHUNK32_FUSED16 RANKCHUNK32_DIRECT3 RANKCHUNK32_DIRECTMASK RANKCHUNK32_RANKPLANE RANKCHUNK32_RANKMASK_PROFILE RANKCHUNK32_BYTEPACK RANKCHUNK32_ALIGN32 RANKCHUNK32_BLOCK64 RANKDELTA8_ALIGN32 RANKDELTA8_FUSED13 RANKFORMULA_SPARSE_BASE RANKFORMULA_RAWCODE RANKFORMULA_INLINE_CROSS RANKFORMULA_BASE_DELTA RANKFORMULA_SLOTMETA RANKFORMULA_SLOTROW32 PM_ACCUM TERNARY_KEY4 PTXAS_VERBOSE; do
  v="${!x}"
  if [[ "$v" != 0 && "$v" != 1 ]]; then echo "$x must be 0 or 1" >&2; exit 2; fi
done
if ! [[ "$RANKCHUNK32_RANKMASK_PROFILE_LOG2" =~ ^[0-9]+$ ]] || (( RANKCHUNK32_RANKMASK_PROFILE_LOG2 > 16 )); then
  echo "RANKCHUNK32_RANKMASK_PROFILE_LOG2 must be an integer in [0,16]" >&2
  exit 2
fi
...
if [[ "$RANKCHUNK32_RANKPLANE" == 1 && "$RANKCHUNK32_DIRECTMASK" != 1 ]]; then
  echo "RANKCHUNK32_RANKPLANE=1 requires RANKCHUNK32_DIRECTMASK=1" >&2; exit 2
fi
...
if [[ "$RANKCHUNK32_RANKMASK_PROFILE" == 1 && "$HIGH_CTX" != warpstriped_delta_direct_affine_rankchunk32_cross5 ]]; then
  echo "RANKCHUNK32_RANKMASK_PROFILE=1 requires HIGH_CTX=warpstriped_delta_direct_affine_rankchunk32_cross5" >&2
  exit 2
fi
if [[ "$RANKCHUNK32_RANKMASK_PROFILE" == 0 && "$RANKCHUNK32_RANKMASK_PROFILE_LOG2" != 0 ]]; then
  echo "RANKCHUNK32_RANKMASK_PROFILE_LOG2 requires RANKCHUNK32_RANKMASK_PROFILE=1" >&2
  exit 2
fi
...
[[ "$RANKCHUNK32_RANKPLANE" == 1 ]] && SUFFIX="${SUFFIX}_rankplane"
...
if [[ "$HIGH_CTX" == warpstriped_delta_direct_affine_rankdelta8_cross5 ]]; then
  ...
  [[ "$RANKFORMULA_BASE_DELTA" == 1 ]] && SUFFIX="${SUFFIX}_rankformulabasedelta"
  [[ "$RANKFORMULA_SLOTMETA" == 1 ]] && SUFFIX="${SUFFIX}_rankformulaslotmeta"
  [[ "$RANKFORMULA_SLOTROW32" == 1 ]] && SUFFIX="${SUFFIX}_rankformulaslotrow32"
else
...
TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  ...
  -DP10DC_RANKCHUNK32_RANKPLANE="$RANKCHUNK32_RANKPLANE" \
