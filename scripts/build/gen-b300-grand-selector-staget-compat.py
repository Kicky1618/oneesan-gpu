...
if old in s:
    if s.count(old)!=1: raise SystemExit(f'Stage-T compat ambiguous old anchor count={s.count(old)}')
    s=s.replace(old,fixed,1)
elif fixed not in s:
    raise SystemExit('Stage-T compat found neither old nor fixed default-L2 anchor')
