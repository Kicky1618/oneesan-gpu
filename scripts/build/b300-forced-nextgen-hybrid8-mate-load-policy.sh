...
SELF_GUARD="${SELF_GUARD:-branch}"; MATE_GUARD="${MATE_GUARD:-branch}"
...
for name in SELF_GUARD MATE_GUARD; do case "${!name}" in branch|predicated) ;; *) echo "$name must be branch or predicated" >&2; exit 2;; esac; done
...
TAG="p${POLICY}_selfw${SELF_WIDTH}_selfd${SELF_DISTANCE}_matew${MATE_WIDTH}_mated${MATE_DISTANCE}_sev${SELF_EVICT}_mev${MATE_EVICT}_sg${SELF_GUARD}_mg${MATE_GUARD}_t${HYBRID_THRESHOLD}_$$"
...
# Reuse the fully proven geometry/eviction/guard builder to materialize the
# exact upstream source, then alter only the eight ILP8 mate loads.
...
grep -Fq "self_geometry width=$SELF_WIDTH distance=$SELF_DISTANCE evict=$SELF_EVICT guard=$SELF_GUARD" "$CONTROL_OUT" || { echo 'self geometry/guard drift in mate-load control' >&2; exit 3; }
grep -Fq "mate_geometry width=$MATE_WIDTH distance=$MATE_DISTANCE evict=$MATE_EVICT guard=$MATE_GUARD" "$CONTROL_OUT" || { echo 'mate geometry/guard drift in mate-load control' >&2; exit 3; }
...
echo "  geometry_builder_proof_gates_reused=1 mate_load_policy_transform=1 ilp2_unchanged=1 mate_writes_unchanged=1 guard_policy_preserved=1"
