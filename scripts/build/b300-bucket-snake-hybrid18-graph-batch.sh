...
case "$TRANSPOSE_MODE" in
  sync) SRC_REL="src/cuda/b300/oneesan_cuda_gridfp_b300_bucket_snake_onepass_hybrid18_graph_batch.cu" ;;
  events) SRC_REL="src/cuda/b300/oneesan_cuda_gridfp_b300_bucket_snake_onepass_hybrid18_graph_batch_events.cu" ;;
  pipeline) SRC_REL="src/cuda/b300/oneesan_cuda_gridfp_b300_bucket_snake_onepass_hybrid18_graph_batch_pipeline.cu" ;;
...
SRC="$(repo_path "$SRC_REL")"; OUT="$(build_path "${OUT:-oneesan_cuda_gridfp_b300_bucket_snake_onepass_hybrid18_graph_batch${SUFFIX}_n${N}}")"
TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  -DTARGET_W="$W" -DLOW_LUT_K="$LOW_LUT_K" -DHIGH_LUT_K="$HIGH_LUT_K" -DGPU_DIRECT_PM_ACCUM="$PM_ACCUM" \
  "$SRC" -o "$OUT"
echo "built $OUT (forward=packed18 reverse=split18 window=graph transpose=$TRANSPOSE_MODE pm_accum=$PM_ACCUM)"
