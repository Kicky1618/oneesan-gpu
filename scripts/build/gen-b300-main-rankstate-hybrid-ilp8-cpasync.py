...
if len(sys.argv)!=3:
    raise SystemExit('usage: gen-b300-main-rankstate-hybrid-ilp8-cpasync.py INPUT.cu OUTPUT.cu')
src=pathlib.Path(sys.argv[1]);out=pathlib.Path(sys.argv[2]);s=src.read_text()
for req in (
    'b300_main_rankstate_ilp8_blocks','base+=8*grid',
    'b300_main_pull_rankstate_ilp4_kernel','b300_main_pull_rankstate_ilp8_kernel',
    'const Code pj7=','const Code bj7=',
    'rank_state[i7]=b300_pack_rank_state','const Count pair7=','const Count block7=',
    'if(ms.size>=Code('
):
    if req not in s: raise SystemExit(f'hybrid ILP8 cp.async requires artifact: {req}')
if 'b300_cpasync_count_ca' in s:
    raise SystemExit('source already contains cp.async count helper')
...
__device__ __forceinline__ void b300_cpasync_count_ca(
    ...
    static_assert(sizeof(Count)==4,"hybrid ILP8 cp.async assumes 32-bit Count");
...
A('    }')
A('}')
s=replace_function(s,'b300_main_pull_rankstate_ilp8_kernel','\n'.join(L))

old='b300_main_pull_rankstate_ilp8_kernel<<<b300_main_rankstate_ilp8_blocks(ms.size,threads),threads,0,c.sMain>>>'
new='b300_main_pull_rankstate_ilp8_kernel<<<b300_main_rankstate_ilp8_blocks(ms.size,threads),threads,size_t(threads)*16u*sizeof(Count),c.sMain>>>'
if s.count(old)!=1: raise SystemExit(f'hybrid ILP8 launch anchor expected one match got {s.count(old)}')
...
for req in (
    ...
    if req not in s: raise SystemExit(f'missing hybrid ILP8 cp.async artifact: {req}')
...
for stale in ('const Count pair7=hp7?', 'const Count block7=(hb7'):
    if stale in s: raise SystemExit(f'stale synchronous hybrid ILP8 gather remains: {stale}')

out.parent.mkdir(parents=True,exist_ok=True);out.write_text(s)
print(f'generated {out} from {src}: b300_main_rankstate_hybrid_ilp8_cpasync=1 cp_bytes=4 cache_operator=ca async_random_copies_per_ilp8_thread=16 async_groups=2 dynamic_shared_bytes_per_ilp8_thread=64 ilp4_dynamic_shared_bytes=0 overlap=rank_state+self_load zero_fill_invalid=1 register_pressure_isolated_by_kernel=1')
