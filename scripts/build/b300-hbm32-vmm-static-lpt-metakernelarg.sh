...
GENSRC="${GENSRC:-$ONEESAN_BUILD_DIR/generated_b300_static_lpt_metakernelarg_n27.cu}"; OUT="$(build_path "${OUT:-oneesan_cuda_gridfp_b300_hbm32_vmm_static_lpt_metakernelarg_n27}")"
require_nvcc_version_at_least "$NVCC" 13 0 "B300 metadata kernel-arg build"
bash "$ONEESAN_ROOT/scripts/bench/b300-vmm-static-lpt-metakernelarg-production-generate-proof.sh"
python3 "$GEN" "$SRC" "$GENSRC"; python3 "$PRUNE" "$GENSRC" "$GENSRC"; python3 "$BASEARG" "$GENSRC" "$GENSRC"; python3 "$PACK" "$GENSRC" "$GENSRC"; python3 "$STAGE" "$GENSRC" "$GENSRC"; python3 "$STATIC" "$GENSRC" "$GENSRC"; python3 "$METAARG" "$GENSRC" "$GENSRC"; python3 "$ROWLIMIT" "$GENSRC" "$GENSRC"
...
echo "  group_meta_source=staged_global_kernelarg"
echo "  per_group_meta_copy_bytes=0 per_group_meta_symbol_copy_calls=0"
