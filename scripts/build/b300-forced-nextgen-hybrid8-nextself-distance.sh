...
NEXTSELF_WIDTH="${NEXTSELF_WIDTH:-8}"; NEXTSELF_DISTANCE="${NEXTSELF_DISTANCE:-1}"
...
TAG="hybns_w${NEXTSELF_WIDTH}_d${NEXTSELF_DISTANCE}_h${HIGH_DROP_CHUNK}_t${HYBRID_THRESHOLD}_cg${RANDOM_CG}_l2${RANDOM_CG_L2_FETCH_BYTES}_pre${PREFETCH_L2}_dual${DUALMASK}_cb${CLOSURE_BATCH}_r${MAXRREGCOUNT}_$$"
...
python3 "$ONEESAN_ROOT/scripts/build/gen-b300-mainrec-hybrid8-next-self-prefetch.py" "$SRC" "$NEXT" "$NEXTSELF_WIDTH" "$NEXTSELF_DISTANCE" >"$ISO/transform.out"
...
echo "built $OUT"
echo "  nextgen_forced=1 recurrence_hybrid_ilp8=1 recurrence_hybrid_ilp8_min_states=$HYBRID_THRESHOLD recurrence_hybrid_ilp8_nextself=1 recurrence_hybrid_ilp8_nextself_width=$NEXTSELF_WIDTH recurrence_hybrid_ilp8_nextself_distance=$NEXTSELF_DISTANCE"
...
echo "  canonical_nextgen_proof_gates_reused=1 experimental_distance_transform=1 extra_state_bytes=0"
