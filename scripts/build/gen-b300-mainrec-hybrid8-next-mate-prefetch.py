...
if len(sys.argv) not in (3, 4, 5):
    raise SystemExit('usage: gen-b300-mainrec-hybrid8-next-mate-prefetch.py INPUT.cu OUTPUT.cu [WIDTH] [DISTANCE]')
...
__device__ __forceinline__ void b300_mainrec_hybrid8_prefetch_next_mate_l2(const MateID* p,bool valid){
...
print(f'generated {out} from {src}: b300_mainrec_hybrid8_next_mate_prefetch=1 prefetch_width={width} prefetch_distance_iterations={distance} prefetch_advance={advance}grid cache=L2 prefetch_before_current_reduction=1 semantics_unchanged=1 extra_shared_bytes=0')
