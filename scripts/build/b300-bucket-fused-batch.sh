...
case "$TRANSPOSE_MODE" in
  events)
    SRC="$(repo_path "src/cuda/b300/oneesan_cuda_gridfp_b300_bucket_fused_batch_events.cu")"
    ;;
  sync)
    SRC="$(repo_path "src/cuda/b300/oneesan_cuda_gridfp_b300_bucket_fused_batch.cu")"
    ;;
  *)
    echo "TRANSPOSE_MODE must be events or sync" >&2
    exit 2
    ;;
esac
OUT="$(build_path "${OUT:-oneesan_cuda_gridfp_b300_bucket_fused_batch_n${N}}")"
