...
OUT="$(build_path "${OUT:-oneesan_cuda_gridfp_b300_bucket_snake_fused_batch${SUFFIX}_n${N}}")"

TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  -DTARGET_W="$W" -DLOW_LUT_K="$LOW_LUT_K" -DHIGH_LUT_K="$HIGH_LUT_K" \
  -DGPU_DIRECT_PM_ACCUM="$PM_ACCUM" \
  "$SRC" -o "$OUT"
echo "built $OUT (reverse=fused transpose=$TRANSPOSE_MODE pm_accum=$PM_ACCUM)"
