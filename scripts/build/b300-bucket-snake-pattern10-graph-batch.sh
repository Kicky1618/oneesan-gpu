...
TRANSPOSE_MODE="${TRANSPOSE_MODE:-pipeline}"; PM_ACCUM="${PM_ACCUM:-0}"; PTXAS_VERBOSE="${PTXAS_VERBOSE:-0}"
...
TMPDIR="$ONEESAN_TMP_DIR" nvcc -O3 -std=c++17 -lineinfo -arch="$ARCH" \
  ...
  "$SRC" -o "$OUT"
echo "built $OUT (closure_metadata=zero ordinary=pattern10 reverse=split54 window=graph transpose=$TRANSPOSE_MODE pm_accum=$PM_ACCUM ptxas_verbose=$PTXAS_VERBOSE)"
