...
static constexpr RankState B300_RANK_DELTA_MOD=RankState(1)<<56;
...
__device__ __forceinline__ RankDelta b300_unpack_rank_delta(RankState s){
    const RankState raw=s&B300_RANK_DELTA_MASK;
    return (raw&B300_RANK_DELTA_SIGN)?RankDelta(raw-B300_RANK_DELTA_MOD):RankDelta(raw);
}
...
once('int p,RankDelta*rank_delta){','int p,RankState*rank_state){','main rank-state pointer')
once(
...
'''        RankDelta rd=0;int rank_h=0;
        ...
        uint64_t acc=in[i];''','main rank-state load')
once('const Code j=main_pull_direct_pair_source_rank(i,m,p);','const Code j=main_pull_direct_pair_source_rank(i,m,p,rank_h);','main pair uses packed height')
once(
...
'''        if constexpr(CACHED_RANK_DELTA){RankDelta nd=rd+b300_rank_delta_step(mget(m,p),p,rank_h);rank_state[i]=b300_pack_rank_state(nd,b300_rank_height_advance(rank_h,mget(m,p)));}''','main rank-state store')
once('int p,RankDelta*rank_delta){','int p,RankState*rank_state){','block rank-state pointer')
once(
...
'''        RankDelta rd=0,next_rd=0;int rank_h=0;
        ...
        Count acc=0;''','block rank-state load')
once('const int H=height_before_rank_pos<TARGET_W>(d,p);','const int H=rank_h;','block closure uses packed height')
once('''        if constexpr(CACHED_RANK_DELTA)rank_delta[i]=next_rd;''','''        if constexpr(CACHED_RANK_DELTA)rank_state[i]=b300_pack_rank_state(next_rd,b300_rank_height_advance(rank_h,mget(b,p-1)));''','block rank-state store')

s=s.replace('RankDelta*dMainRankDelta=nullptr,*dBlockRankDelta=nullptr;','RankState*dMainRankDelta=nullptr,*dBlockRankDelta=nullptr;',1)
...
for required in ('B300_RANK_DELTA_MASK','B300_RANK_DELTA_SIGN','B300_RANK_DELTA_MOD','b300_pack_rank_state','RankState*dMainRankDelta','main_pull_direct_pair_source_rank(i,m,p,rank_h)','const int H=rank_h'):
    if required not in s:raise SystemExit(f'missing packed rank-state artifact: {required}')
out.parent.mkdir(parents=True,exist_ok=True);out.write_text(s)
print(f'generated {out} from {src}: b300_rank_state_packed=1 storage_bytes=8 delta_bits=56 height_bits=8 signed_decode=defined_subtract hbm_rw_per_state_step=16 prefix_rank_walk=0 prefix_height_popcount=0 width_max=28 full_state_bound=385719506620 fallback_required=0')
