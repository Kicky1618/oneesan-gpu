...
if ! command -v nvcc >/dev/null 2>&1 || ! command -v nvidia-smi >/dev/null 2>&1 || ! nvidia-smi -L >/dev/null 2>&1; then
  echo 'row6 pre-rank regression: SKIP (CUDA compiler/GPU unavailable)'
  exit 0
fi
...
run_case() {
  ...
  local bin="$ONEESAN_BUILD_DIR/row6_prerank_regression_n${n}"
  ...
  python3 "$ONEESAN_ROOT/scripts/solve/build_provenance.py" verify \
    "${bin}.provenance.json" --binary "$bin" \
    --expect-compile-arg="-DTARGET_W=$((n + 1))" >/dev/null
  local log="$ONEESAN_BUILD_DIR/row6_prerank_regression_n${n}_${mod}.log"
  GRIDFP_BOUNDED_PREFIX_K=6 "$bin" "$n" 4096 14 1 "$mod" >"$log" 2>&1
  grep -q 'high pre-rank owner=' "$log"
  grep -q 'factor LOW VMM ' "$log"
  grep -q 'factor HIGH VMM ' "$log"
  local got
  got="$(sed -n 's/.* residue=\([0-9][0-9]*\) modulus=.*/\1/p' "$log" | tail -n 1)"
  if [[ "$got" != "$expected" ]]; then
    echo "row6 pre-rank n=$n mod=$mod residue mismatch: got=$got expected=$expected" >&2
    tail -n 30 "$log" >&2
    exit 1
  fi
  echo "row6 pre-rank n=$n mod=$mod: PASS residue=$got"
}
...
echo 'row6 pre-rank regression: PASS'
