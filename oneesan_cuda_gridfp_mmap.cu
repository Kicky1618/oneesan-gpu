...
int main(int argc,char**argv){
    ...
    for(int row=0;row<W;++row){
        ...
        while(p_hi>=1){
            ...
            for(int g:order){
                ...
                gather(mainv.p,mi,hA);gather(blockv.p,di,hD);transferred+=double(ms.size+ds.size)*8.0;
                ...
                for(int p=wp.p_hi;p>=wp.p_lo;--p){
                    if(ms.size)ck(cudaMemcpy(nxt,cur,size_t(ms.size)*8,cudaMemcpyDeviceToDevice),"identity");
                ...
                if(ms.size)ck(cudaMemcpy(hB.data(),cur,size_t(ms.size)*8,cudaMemcpyDeviceToHost),"D2H main");
    ...
    std::cout<<"backend=gridfp-mmap n="<<n<<" residue="<<ans<<" modulus="<<mod<<" main_states="<<mainN<<" blocked_states="<<blockN<<" external_bytes="<<size_t(mainN+blockN)*8ULL<<" max_group_main="<<maxGM<<" max_group_blocked="<<maxGD<<" max_vram_bytes="<<size_t(2*maxGM+maxGD)*8ULL<<" max_intervals="<<maxIntervals<<" windows="<<total_windows<<" max_groups="<<max_groups<<" max_window_len="<<max_window_len<<" target_mib="<<target_mib<<" max_window_cfg="<<max_window<<" transfer_gib="<<transferred/(1ull<<30)<<" gpu_ms="<<ms<<"\n";
