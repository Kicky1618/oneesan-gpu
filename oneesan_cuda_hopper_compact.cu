...
__device__ static inline void hash_insert(
    ...
    uint32_t* table_index, unsigned int* table_state,
    ...
    for (uint32_t probe = 0; probe <= mask; ++probe, slot = (slot + 1) & mask) {
        ...
        for (;;) {
            if (s == ready) {
                ...
                if (idx != 0xffffffffu && dense_keys[idx] == key) {
                    #pragma unroll
...
__global__ void expand_hash_kernel(
    ...
    if (transition_decoded(base, m, 0, out))
        hash_insert(out, v, table_index, table_state, dense_keys, dense_vals, dense_n, dense_cap, overflow, mask, generation, mod_base, active_res);
...
int main(int argc, char** argv) {
    ...
    for (int ei=0; ei<(int)edges.size(); ++ei) {
        ...
        if (cap > allocated_cap) {
            if (table_index) { cudaFree(table_index); cudaFree(table_state); }
        ...
        for (int attempt = 0; attempt < 2; ++attempt) {
            ...
            expand_hash_kernel<<<blocks,threads>>>(
                ...
                table_index, table_state, spare_keys, spare_vals, dense_n,
        ...
        {
            const size_t table_bytes = allocated_cap * (sizeof(uint32_t) + sizeof(unsigned int));
    ...
    std::cout << "backend=hopper-compact-lowmem4-key192 n=" << n << " residues=";
    ...
    cudaFree(table_index); cudaFree(table_state); cudaFree(dense_n); cudaFree(overflow);
