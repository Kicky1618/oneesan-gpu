...
__global__ void expand_hash_kernel(
    ...
    if (transition_decoded(base, m, 0, out))
        hash_insert(out, v, table_keys, table_index, table_state, dense_keys, dense_vals, dense_n, dense_cap, overflow, mask, generation);
...
int main(int argc, char** argv) {
    ...
    for (int ei=0; ei<(int)edges.size(); ++ei) {
        ...
        for (int attempt = 0; attempt < 2; ++attempt) {
            ...
            expand_hash_kernel<<<blocks,threads>>>(
                ...
                static_cast<uint32_t>(cap-1), gen);
    ...
    std::cout << "backend=hopper-mem-key192 n=" << n << " residues=";
