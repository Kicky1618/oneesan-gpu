...
__global__ void blocked_group_kernel(const Count*in,const MateID*mates,Code n,Count*out_main,int p){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID sm=mates?mates[i]:unrank_group_t<TARGET_W-1>(i,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);MateID t=minsert(sm,p,N);Code j=rank_group_t<TARGET_W>(t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(out_main+j,c);}}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads){
    ...
    if(ms.size){if(pg.use_mi)interval_io_kernel<false,false><<<interval_blocks(pg.mi.size(),threads),threads>>>(c.dA,c.dIM,pg.mi.size());else gather_main_kernel<<<bm,threads>>>(c.dA,c.dMateM,ms.size);}
    if(ds.size){if(pg.use_di)interval_io_kernel<true,false><<<interval_blocks(pg.di.size(),threads),threads>>>(c.dD,c.dID,pg.di.size());else gather_block_kernel<<<bd,threads>>>(c.dD,c.dMateD,ds.size);}
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        ...
        if(ms.size)main_group_kernel<<<bm,threads,0,c.sMain>>>(cur,pg.use_mi?nullptr:c.dMateM,ms.size,nxt,dnext,p);
        if(ds.size)blocked_group_kernel<<<bd,threads,0,c.sBlock>>>(dcur,pg.use_di?nullptr:c.dMateD,ds.size,nxt,p);
