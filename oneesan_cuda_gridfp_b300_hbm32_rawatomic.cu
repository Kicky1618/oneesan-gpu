...
__device__ __forceinline__ void atomic_add_mod(Count*p,Count v){if(v) atomicAdd(p,v);}
__global__ void normalize_mod_kernel(Count* a,Code n){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    Count mod=D_MOD;
    for(;i<n;i+=stride){Count x=a[i];if(x>=mod)a[i]=x%mod;}
}
