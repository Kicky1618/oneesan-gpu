...
Both variants use `CPU_HIGH_MODE=direct`, the same worker counts, GPU target, overlap setting, affinity list, modulus, binary, and authoritative algorithm. The threshold variant clears `CPU_HIGH_GROUPS_FILE`; the policy variant sets `CPU_HIGH_MAX_MIB=0` and uses the supplied exact group file.
...
The TSV records wall time, H2D+D2H components, GPU kernel time, CPU HIGH/LOW wall time, PCIe volume removed, CPU HIGH group count, and the production selection hash. The metadata file records commit, host/GPU/driver, binary SHA-256, group-policy SHA-256, affinity, threshold, and all execution parameters.
...
A generated policy should only replace the simpler threshold policy after this A/B benchmark shows a repeatable wall-time improvement. If its predicted advantage disappears here, inspect the affine model residuals, NUMA placement, and overlap contention before adding more model complexity.
