...
int main(int argc,char**argv){
    ...
    for(int row=0;row<width;++row){
        for(int j=0;j<width-1;++j){
            ...
            std::swap(a,b);
            if (n==4 && row<3) {
                std::vector<Count> hm(main_n), hd(blocked_n);
                ck(cudaMemcpy(hm.data(),a,value_bytes,cudaMemcpyDeviceToHost),"dbg main");
                ck(cudaMemcpy(hd.data(),blocked,blocked_bytes,cudaMemcpyDeviceToHost),"dbg blocked");
                std::cerr<<"STEP "<<(row*(width-1)+j+1)<<" p="<<p<<"\nM";
                for(Code z=0;z<main_n;++z) if(hm[z]) std::cerr<<" "<<z<<":"<<hm[z];
                std::cerr<<"\nD"; for(Code z=0;z<blocked_n;++z) if(hd[z]) std::cerr<<" "<<z<<":"<<hd[z]; std::cerr<<"\n";
            }
        }
