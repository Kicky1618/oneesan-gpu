...
__device__ __forceinline__ Count add_mod_local(Count a,Count b){Count mod=D_MOD;return a>=mod-b?a-(mod-b):a+b;}

__global__ void gather_main_next_kernel(const Count*cur,Code n,const Count*blocked,Code dn,Count*nxt,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        MateID t=unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);
        Count acc=cur[i];MateValuePair w=mpair(t,p);MateID src;
        if(w==LR){src=msetpair(t,p,NN);acc=add_mod_local(acc,cur[rank_group_t<TARGET_W>(src,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP)]);}
        else if(w==NR){src=msetpair(t,p,RN);acc=add_mod_local(acc,cur[rank_group_t<TARGET_W>(src,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP)]);}
        else if(w==NL){src=msetpair(t,p,LN);acc=add_mod_local(acc,cur[rank_group_t<TARGET_W>(src,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP)]);}
        if(dn&&mget(t,p)==N){MateID sm=mshrink(t,p);Code j=rank_group_t<TARGET_W-1>(sm,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);if(j<dn)acc=add_mod_local(acc,blocked[j]);}
        nxt[i]=acc;
    }
}

__global__ void main_to_block_kernel(const Count*in,Code n,Count*out_block,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,p);MateID t;
        switch(w){
        case NR:case NL:t=mshrink(m,p);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);break;
        case LL:{t=msetpair(m,p,NN);int q=p-1,d=1;while(d){--q;auto v=mget(t,q);if(v==L)++d;else if(v==R)--d;}t=mset(t,q,L);t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);break;}
        case RR:{t=msetpair(m,p,NN);int q=p,d=1;while(d){++q;auto v=mget(t,q);if(v==L)--d;else if(v==R)++d;}t=mset(t,q,R);t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);break;}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(p>1){
            if(ms.size)gather_main_next_kernel<<<bm,threads>>>(cur,ms.size,c.dD,ds.size,nxt,p);
            if(ds.size)ck(cudaMemset(c.dD,0,size_t(ds.size)*sizeof(Count)),"clear D");
            if(ms.size&&ds.size)main_to_block_kernel<<<bm,threads>>>(cur,ms.size,c.dD,p);
        }else{
        ...
        ck(cudaGetLastError(),"transition");std::swap(cur,nxt);
    }
