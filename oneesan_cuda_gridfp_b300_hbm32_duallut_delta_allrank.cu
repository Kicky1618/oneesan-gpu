...
template<int WIDTH>
__device__ __forceinline__ Code rank_adjust(Code src,Code old_part,Code new_part){
    return new_part>=old_part?src+(new_part-old_part):src-(old_part-new_part);
}
template<int WIDTH>
__device__ __forceinline__ Code rank_insert_n_t(Code block_rank,MateID sm,int k,
    uint32_t mf,uint32_t mo,const Code mdp[MAXW+1][MAXW+2],
    uint32_t bf,uint32_t bo,const Code bdp[MAXW+1][MAXW+2]){
    MateID u=minsert(sm,k,N);
    Code oldp=rank_slice_t<WIDTH-1>(sm,WIDTH-2,k,1,bf,bo,bdp);
    Code newp=rank_slice_t<WIDTH>(u,WIDTH-1,k+1,1,mf,mo,mdp);
    return rank_adjust<WIDTH>(block_rank,oldp,newp);
}
template<int WIDTH>
__device__ __forceinline__ Code rank_remove_n_t(Code main_rank,MateID u,int k,
    uint32_t mf,uint32_t mo,const Code mdp[MAXW+1][MAXW+2],
    uint32_t bf,uint32_t bo,const Code bdp[MAXW+1][MAXW+2]){
    MateID sm=mshrink(u,k);
    Code oldp=rank_slice_t<WIDTH>(u,WIDTH-1,k+1,1,mf,mo,mdp);
    Code newp=rank_slice_t<WIDTH-1>(sm,WIDTH-2,k,1,bf,bo,bdp);
    return rank_adjust<WIDTH>(main_rank,oldp,newp);
}
...
__global__ void gather_main_kernel(Count*out,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride){Code g;unrank_group_global_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP,g);out[i]=global_load_main(g);}}
...
__global__ void blocked_group_kernel(const Count*in,Code n,Count*out_main,int p){
    ...
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID sm=unrank_group_t<TARGET_W-1>(i,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);Code j=rank_insert_n_t<TARGET_W>(i,sm,p,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_mod(out_main+j,c);}
}
__global__ void main_group_kernel(const Count*in,Code n,Count*out_main,Count*out_block,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m=unrank_group_t<TARGET_W>(i,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,p);
        switch(w){
        case NN:{MateID u=msetpair(m,p,LR);Code j=rank_same_t<TARGET_W>(i,m,u,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(out_main+j,c);break;}
        case NR:case NL:{if(p==1){MateID u=msetpair(m,p,w==NR?RN:LN);Code j=rank_same_t<TARGET_W>(i,m,u,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(out_main+j,c);}else{Code j=rank_remove_n_t<TARGET_W>(i,m,p,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_mod(out_block+j,c);}break;}
        case RN:{MateID u=msetpair(m,p,NR);Code j=rank_same_t<TARGET_W>(i,m,u,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(out_main+j,c);break;}
        case LN:{MateID u=msetpair(m,p,NL);Code j=rank_same_t<TARGET_W>(i,m,u,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add_mod(out_main+j,c);break;}
        case LL:{MateID u=msetpair(m,p,NN);int q=p-1,s=1;while(s){--q;auto v=mget(u,q);if(v==L)++s;else if(v==R)--s;}u=mset(u,q,L);Code ru=rank_same_t<TARGET_W>(i,m,u,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);if(p==1)atomic_add_mod(out_main+ru,c);else{Code j=rank_remove_n_t<TARGET_W>(ru,u,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_mod(out_block+j,c);}break;}
        case RR:{MateID u=msetpair(m,p,NN);int q=p,s=1;while(s){++q;auto v=mget(u,q);if(v==L)--s;else if(v==R)++s;}u=mset(u,q,R);Code ru=rank_same_t<TARGET_W>(i,m,u,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);if(p==1)atomic_add_mod(out_main+ru,c);else{Code j=rank_remove_n_t<TARGET_W>(ru,u,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_mod(out_block+j,c);}break;}
        case RL:{MateID u=msetpair(m,p,NN);Code ru=rank_same_t<TARGET_W>(i,m,u,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);if(p==1)atomic_add_mod(out_main+ru,c);else{Code j=rank_remove_n_t<TARGET_W>(ru,u,p-1,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add_mod(out_block+j,c);}break;}
        default:break;}
    }
}
...
struct DeviceCtx{int dev=-1;Count*dA=nullptr,*dB=nullptr,*dD=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;Code capM=0,capD=0;size_t capIM=0,capID=0,maxIntervals=0;double active=0;uint64_t groups=0;void init(int d,Count mod,Count**mp,Count**bp,Code mc,Code bc,int ng){dev=d;ck(cudaSetDevice(dev),"set init");ck(cudaMemcpyToSymbol(D_FULL_DP,H_DP,sizeof(H_DP)),"full dp");ck(cudaMemcpyToSymbol(D_MOD,&mod,sizeof(mod)),"mod");ck(cudaMemcpyToSymbol(D_MAIN_PTR,mp,sizeof(Count*)*MAXGPU),"main ptrs");ck(cudaMemcpyToSymbol(D_BLOCK_PTR,bp,sizeof(Count*)*MAXGPU),"block ptrs");ck(cudaMemcpyToSymbol(D_MAIN_CHUNK,&mc,sizeof(mc)),"main chunk");ck(cudaMemcpyToSymbol(D_BLOCK_CHUNK,&bc,sizeof(bc)),"block chunk");ck(cudaMemcpyToSymbol(D_NGPU,&ng,sizeof(ng)),"ngpu");}void ensure(Code m,Code b,size_t im,size_t id){ck(cudaSetDevice(dev),"set ensure");if(m>capM){if(dA){cudaFree(dA);cudaFree(dB);}capM=m;ck(cudaMalloc(&dA,size_t(m)*sizeof(Count)),"scratch A");ck(cudaMalloc(&dB,size_t(m)*sizeof(Count)),"scratch B");}if(b>capD){if(dD)cudaFree(dD);capD=b;ck(cudaMalloc(&dD,size_t(b)*sizeof(Count)),"scratch D");}if(im>capIM){if(dIM)cudaFree(dIM);capIM=im;ck(cudaMalloc(&dIM,capIM*sizeof(PeerInterval)),"interval main");}if(id>capID){if(dID)cudaFree(dID);capID=id;ck(cudaMalloc(&dID,capID*sizeof(PeerInterval)),"interval block");}maxIntervals=std::max({maxIntervals,im,id});}void destroy(){if(dev<0)return;cudaSetDevice(dev);if(dA){cudaFree(dA);cudaFree(dB);}if(dD)cudaFree(dD);if(dIM)cudaFree(dIM);if(dID)cudaFree(dID);}};
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads){
    ...
    auto const&ms=pg.ms;auto const&ds=pg.ds;
    ...
    if(ms.size){if(pg.use_mi)interval_io_kernel<false,false><<<interval_blocks(pg.mi.size(),threads),threads>>>(c.dA,c.dIM,pg.mi.size());else gather_main_kernel<<<bm,threads>>>(c.dA,ms.size);}
    ...
    if(ds.size){if(pg.use_di)interval_io_kernel<true,true><<<interval_blocks(pg.di.size(),threads),threads>>>(c.dD,c.dID,pg.di.size());else scatter_block_kernel<<<bd,threads>>>(c.dD,ds.size);}
    ck(cudaGetLastError(),"prepared scatter");ck(cudaDeviceSynchronize(),"group sync");c.groups++;c.active+=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
}
