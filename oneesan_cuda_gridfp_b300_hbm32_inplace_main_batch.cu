...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(p>1){
            ...
            if(ms.size)ck(cudaMemcpy(nxt,cur,size_t(ms.size)*sizeof(Count),cudaMemcpyDeviceToDevice),"identity p1");
            if(ds.size)ck(cudaMemset(dnext,0,size_t(ds.size)*sizeof(Count)),"clear p1 D");
            if(ms.size)main_group_kernel<<<bm,threads>>>(cur,useMate?c.dMate:nullptr,ms.size,nxt,dnext,p);
            if(ds.size)blocked_group_kernel<<<bd,threads>>>(dcur,ds.size,nxt,p);
            ck(cudaGetLastError(),"p1 transition");ck(cudaDeviceSynchronize(),"p1 sync");
