...
__global__ void main_group_kernel(const Count*in,const uint32_t*prefix,int cutoff,Code n,Count*out_main,Count*out_block,int p){
    ...
    for(;i<n;i+=stride){Count c=in[i];if(!c)continue;MateID m;MateValuePair w;
        if(prefix){m=MateID(prefix[i])<<(2*cutoff);w=mpair(m,p);
        ...
        switch(w){case NN:{MateID t=msetpair(m,p,LR);atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);break;}
        ...
        case RL:{MateID t=msetpair(m,p,NN);if(p==1)atomic_add_mod(out_main+rank_same_t<TARGET_W>(i,m,t,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP),c);else{t=mshrink(t,p-1);atomic_add_mod(out_block+rank_group_t<TARGET_W-1>(t,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP),c);}break;}default:break;}
    }
}
...
struct DeviceCtx{
    int dev=-1;uint8_t*arena=nullptr;size_t capArena=0;Count*dA=nullptr,*dB=nullptr,*dD=nullptr,*dE=nullptr;uint32_t*dPrefix=nullptr;PeerInterval*dIM=nullptr,*dID=nullptr;size_t capIM=0,capID=0,maxIntervals=0;double active=0;uint64_t groups=0;cudaStream_t sMain=nullptr,sBlock=nullptr;cudaEvent_t copyDone=nullptr,clearDone=nullptr,mainDone=nullptr,blockDone=nullptr;
    ...
    void ensure(Code m,Code b,bool usePrefix,size_t im,size_t id){ck(cudaSetDevice(dev),"set ensure");auto al=[](size_t x){return(x+255)&~size_t(255);};size_t ab=al(size_t(m)*sizeof(Count)),db=al(size_t(b)*sizeof(Count)),pb=usePrefix?al(size_t(m)*sizeof(uint32_t)):0,need=2*ab+2*db+pb;if(need>capArena){if(arena)cudaFree(arena);capArena=need;ck(cudaMalloc(&arena,capArena),"scratch arena");}size_t off=0;dA=(Count*)(arena+off);off+=ab;dB=(Count*)(arena+off);off+=ab;dD=(Count*)(arena+off);off+=db;dE=(Count*)(arena+off);off+=db;dPrefix=usePrefix?(uint32_t*)(arena+off):nullptr;if(im>capIM){if(dIM)cudaFree(dIM);capIM=im;ck(cudaMalloc(&dIM,capIM*sizeof(PeerInterval)),"interval main");}if(id>capID){if(dID)cudaFree(dID);capID=id;ck(cudaMalloc(&dID,capID*sizeof(PeerInterval)),"interval block");}maxIntervals=std::max({maxIntervals,im,id});}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads,size_t target){
    ...
    int cutoff=wp.p_lo-1;size_t countBytes=size_t(2*ms.size+2*ds.size)*sizeof(Count),prefixBytes=size_t(ms.size)*sizeof(uint32_t);bool usePrefix=(wp.p_hi==W-1)&&!pg.use_mi&&cutoff>=0&&(W-cutoff)<=16&&(countBytes+prefixBytes<=target);
