...
using Count32 = uint32_t;
struct Count4 { Count32 a,b,c,d; };
...
__constant__ Count32 D_MOD0,D_MOD1,D_MOD2,D_MOD3;
...
static WindowPlan plan_window(int W,int p_hi,int p_lo,size_t target_bytes,int max_group_bits=16){
    ...
    for(int k=0;k<=klim;++k){
        ...
        for(uint64_t g=0;g<ng;++g){
            ...
            size_t b=size_t(2*ms.size+ds.size)*sizeof(Count4);
...
__device__ __forceinline__ void atomic_add_mod32(Count32* p,Count32 v,Count32 mod){
    if(!v)return;
    Count32 old=atomicCAS(p,0u,0u);
    for(;;){
        Count32 neu=(old>=mod-v)?old-(mod-v):old+v;
        Count32 seen=atomicCAS(p,old,neu);
        if(seen==old)return;
        old=seen;
    }
}
__device__ __forceinline__ void atomic_add4(Count4* p,Count4 v){
    atomic_add_mod32(&p->a,v.a,D_MOD0);
    atomic_add_mod32(&p->b,v.b,D_MOD1);
    atomic_add_mod32(&p->c,v.c,D_MOD2);
    atomic_add_mod32(&p->d,v.d,D_MOD3);
}
__global__ void blocked_group_kernel(const Count4* in,Code n,Count4* out_main,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x, stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){Count4 v=in[i];if(!(v.a|v.b|v.c|v.d))continue;MateID sm=unrank_group(i,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);MateID t=minsert(sm,p,N);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add4(out_main+j,v);}
}
__global__ void main_group_kernel(const Count4* in,Code n,Count4* out_main,Count4* out_block,int p){
    Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x, stride=Code(gridDim.x)*blockDim.x;
    for(;i<n;i+=stride){
        Count4 c=in[i];if(!(c.a|c.b|c.c|c.d))continue;MateID m=unrank_group(i,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);MateValuePair w=mpair(m,p);
        switch(w){
        case NN:{MateID t=msetpair(m,p,LR);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add4(out_main+j,c);break;}
        case NR:case NL:{if(p==1){MateID t=msetpair(m,p,w==NR?RN:LN);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add4(out_main+j,c);}else{MateID t=mshrink(m,p);Code j=rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add4(out_block+j,c);}break;}
        case RN:{MateID t=msetpair(m,p,NR);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add4(out_main+j,c);break;}
        case LN:{MateID t=msetpair(m,p,NL);Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add4(out_main+j,c);break;}
        case LL:{MateID t=msetpair(m,p,NN);int q=p-1,s=1;while(s){--q;auto v=mget(t,q);if(v==L)++s;else if(v==R)--s;}t=mset(t,q,L);if(p==1){Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add4(out_main+j,c);}else{t=mshrink(t,p-1);Code j=rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add4(out_block+j,c);}break;}
        case RR:{MateID t=msetpair(m,p,NN);int q=p,s=1;while(s){++q;auto v=mget(t,q);if(v==L)--s;else if(v==R)++s;}t=mset(t,q,R);if(p==1){Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add4(out_main+j,c);}else{t=mshrink(t,p-1);Code j=rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add4(out_block+j,c);}break;}
        case RL:{MateID t=msetpair(m,p,NN);if(p==1){Code j=rank_group(t,D_MAIN_W,D_MAIN_FIXED,D_MAIN_OCC,D_MAIN_DP);atomic_add4(out_main+j,c);}else{t=mshrink(t,p-1);Code j=rank_group(t,D_BLOCK_W,D_BLOCK_FIXED,D_BLOCK_OCC,D_BLOCK_DP);atomic_add4(out_block+j,c);}break;}
        default:break;
        }
    }
}
...
struct MappedCounts {
    int fd=-1; Count4* p=nullptr; size_t n=0, bytes=0; std::string path;
    void open_file(const std::string& fn,size_t count){
        path=fn;n=count;bytes=n*sizeof(Count4);
    ...
    Count4& operator[](size_t i){return p[i];}
};

static void gather_raw(const Count4* global,const std::vector<Interval>& iv,std::vector<Count4>& local){
    for(auto const& x:iv)std::memcpy(local.data()+x.local,global+x.global,size_t(x.len)*sizeof(Count4));
}
static void scatter_raw(Count4* global,const std::vector<Interval>& iv,const std::vector<Count4>& local){
    for(auto const& x:iv)std::memcpy(global+x.global,local.data()+x.local,size_t(x.len)*sizeof(Count4));
}

struct DeviceCtx {
    int dev = -1;
    Count4 *dA=nullptr, *dB=nullptr, *dD=nullptr;
    Code capM=0, capD=0;
    std::vector<Count4> hM, hD;
    ...
    void init(int device, Count32 m0, Count32 m1, Count32 m2, Count32 m3) {
        dev=device;
        ck(cudaSetDevice(dev), "cudaSetDevice init");
        ck(cudaMemcpyToSymbol(D_FULL_DP,H_DP,sizeof(H_DP)),"full dp");
        ck(cudaMemcpyToSymbol(D_MOD0,&m0,sizeof(m0)),"m0");
        ck(cudaMemcpyToSymbol(D_MOD1,&m1,sizeof(m1)),"m1");
        ck(cudaMemcpyToSymbol(D_MOD2,&m2,sizeof(m2)),"m2");
        ck(cudaMemcpyToSymbol(D_MOD3,&m3,sizeof(m3)),"m3");
    }
    void ensure(Code main_cap, Code block_cap) {
        ...
        if(main_cap>capM){
            ...
            capM=main_cap;
            ck(cudaMalloc(&dA,size_t(capM)*sizeof(Count4)),"dA");
            ck(cudaMalloc(&dB,size_t(capM)*sizeof(Count4)),"dB");
            hM.resize(capM);
        }
        if(block_cap>capD){
            if(dD)cudaFree(dD);
            capD=block_cap;
            ck(cudaMalloc(&dD,size_t(capD)*sizeof(Count4)),"dD");
            hD.resize(capD);
        }
    }
    void destroy(){
...
static void process_group(DeviceCtx& c,
    ...
    c.transferred += double(ms.size+ds.size)*sizeof(Count4);
    if(ms.size)ck(cudaMemcpy(c.dA,c.hM.data(),size_t(ms.size)*sizeof(Count4),cudaMemcpyHostToDevice),"H2D main");
    if(ds.size)ck(cudaMemcpy(c.dD,c.hD.data(),size_t(ds.size)*sizeof(Count4),cudaMemcpyHostToDevice),"H2D block");
    ...
    Count4* cur=c.dA; Count4* nxt=c.dB;
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){
        if(ms.size)ck(cudaMemcpy(nxt,cur,size_t(ms.size)*sizeof(Count4),cudaMemcpyDeviceToDevice),"identity");
        if(ds.size)blocked_group_kernel<<<bd,threads>>>(c.dD,ds.size,nxt,p);
        if(ds.size)ck(cudaMemset(c.dD,0,size_t(ds.size)*sizeof(Count4)),"clear new block");
    ...
    if(ms.size)ck(cudaMemcpy(c.hM.data(),cur,size_t(ms.size)*sizeof(Count4),cudaMemcpyDeviceToHost),"D2H main");
    if(ds.size)ck(cudaMemcpy(c.hD.data(),c.dD,size_t(ds.size)*sizeof(Count4),cudaMemcpyDeviceToHost),"D2H block");
    c.transferred += double(ms.size+ds.size)*sizeof(Count4);
...
int main(int argc,char**argv){
    ...
    Count32 mod0=argc>2?std::strtoul(argv[2],nullptr,10):2147483647u;
    Count32 mod1=argc>3?std::strtoul(argv[3],nullptr,10):2147483629u;
    Count32 mod2=argc>4?std::strtoul(argv[4],nullptr,10):2147483587u;
    Count32 mod3=argc>5?std::strtoul(argv[5],nullptr,10):2147483579u;
    int target_mib=argc>6?std::atoi(argv[6]):256;
    int max_window=argc>7?std::atoi(argv[7]):(n+1-1);
    int requested_gpus=argc>8?std::atoi(argv[8]):0;   // 0 = all visible GPUs
    int W=n+1;
    ...
    for(int d=0;d<ngpu;++d)ctx[d].init(d,mod0,mod1,mod2,mod3);

    Code mainN=H_DP[W][1], blockN=H_DP[W-1][1];
    std::string store_dir=argc>9?argv[9]:".gridfp_b300_r4x32_store";
    std::filesystem::create_directories(store_dir);
    MappedCounts mainv,blockv;
    mainv.open_file(store_dir+"/main.bin",mainN);
    blockv.open_file(store_dir+"/blocked.bin",blockN);
    MateID init=MateID(R)<<(2*(W-1)); mainv[rank_full(init,W)]={1,1,1,1};
    int threads=256;
    ...
    Count4 ans=mainv[rank_full(MateID(R),W)];
    ...
    std::cout<<"backend=gridfp-multigpu-mmap n="<<n<<" residue0="<<ans.a<<" residue1="<<ans.b<<" residue2="<<ans.c<<" residue3="<<ans.d<<" modulus0="<<mod0<<" modulus1="<<mod1<<" modulus2="<<mod2<<" modulus3="<<mod3
             <<" gpus="<<ngpu<<" peer_links="<<peer_links<<" main_states="<<mainN<<" blocked_states="<<blockN
             <<" external_bytes="<<size_t(mainN+blockN)*sizeof(Count4)
             <<" max_group_main="<<maxGM<<" max_group_blocked="<<maxGD
             <<" max_vram_per_gpu_bytes="<<size_t(2*maxGM+maxGD)*sizeof(Count4)
