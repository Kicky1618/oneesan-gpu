...
__constant__ uint32_t* D_FAST_CORR;
__constant__ uint32_t D_FAST_SHIFT,D_FAST_MASK;
...
__device__ __forceinline__ Count reduce_small(Count x){uint32_t a=x>>D_FAST_SHIFT,b=x&D_FAST_MASK,y=b+D_FAST_CORR[a];Count mod=D_MOD;return y>=mod?y-mod:y;}
__device__ __forceinline__ void atomic_add_mod(Count*p,Count v){if(v)atomicAdd(p,reduce_small(v));}
__global__ void normalized_identity_kernel(Count*dst,const Count*src,Code n){Code i=Code(blockIdx.x)*blockDim.x+threadIdx.x,stride=Code(gridDim.x)*blockDim.x;for(;i<n;i+=stride)dst[i]=reduce_small(src[i]);}
...
static void process_group(DeviceCtx&c,int W,const WindowPlan&wp,const PreparedGroup&pg,int threads){
    ...
    for(int p=wp.p_hi;p>=wp.p_lo;--p){if(ms.size)normalized_identity_kernel<<<bm,threads>>>(nxt,cur,ms.size);if(ds.size)blocked_group_kernel<<<bd,threads>>>(c.dD,ds.size,nxt,p);if(ds.size)ck(cudaMemset(c.dD,0,size_t(ds.size)*sizeof(Count)),"clear D");if(ms.size)main_group_kernel<<<bm,threads>>>(cur,ms.size,nxt,c.dD,p);ck(cudaGetLastError(),"transition");std::swap(cur,nxt);}
...
int main(int argc,char**argv){int n=argc>1?std::atoi(argv[1]):16;unsigned long long raw_mod=argc>2?std::strtoull(argv[2],nullptr,10):4294967291ULL;if(raw_mod<2||raw_mod>0xffffffffULL){std::cerr<<"HBM32 modulus must be in [2, 4294967295], got "<<raw_mod<<"\n";return 1;}if(raw_mod>477218589ULL){std::cerr<<"dynamic fastmod requires modulus <= 477218589, got "<<raw_mod<<"\n";return 1;}uint64_t zm=raw_mod-1;int fast_shift=__builtin_ctzll(zm);uint64_t fast_k=zm>>fast_shift;if(fast_shift<19||(fast_k&1)==0){std::cerr<<"dynamic fastmod requires m=k*2^s+1 with odd k and s>=19, got m="<<raw_mod<<" s="<<fast_shift<<" k="<<fast_k<<"\n";return 1;}Count mod=(Count)raw_mod;int target_mib=argc>3?std::atoi(argv[3]):16384;int max_window=argc>4?std::atoi(argv[4]):14;int requested=argc>5?std::atoi(argv[5]):0;int W=n+1;if(n<2||W>MAXW){std::cerr<<"n=2..27\n";return 1;}if(W!=TARGET_W){std::cerr<<"specialized for width "<<TARGET_W<<" (n="<<(TARGET_W-1)<<")\n";return 1;}build_full_dp();int visible=0;ck(cudaGetDeviceCount(&visible),"count");int ng=requested<=0?visible:std::min(requested,visible);if(ng<1||ng>MAXGPU){std::cerr<<"need 1..8 GPUs\n";return 2;}
    ...
    uint32_t corr_n=1u<<(32-fast_shift),fast_mask=(1u<<fast_shift)-1u;std::vector<uint32_t> fast_corr(corr_n);uint64_t B=1ull<<fast_shift;for(uint32_t a=0;a<corr_n;++a){uint64_t q=a/fast_k,r=a-q*fast_k;int64_t v=int64_t(r*B)-int64_t(q);if(v<0)v+=raw_mod;fast_corr[a]=(uint32_t)v;}std::cerr<<"fastmod m="<<raw_mod<<" k="<<fast_k<<" shift="<<fast_shift<<" corr_entries="<<corr_n<<" corr_kib="<<double(corr_n*sizeof(uint32_t))/1024.0<<"\n";uint32_t* fastD[MAXGPU]{};for(int d=0;d<ng;++d){cudaSetDevice(d);ck(cudaMalloc(&fastD[d],fast_corr.size()*sizeof(uint32_t)),"fast corr");ck(cudaMemcpy(fastD[d],fast_corr.data(),fast_corr.size()*sizeof(uint32_t),cudaMemcpyHostToDevice),"fast corr copy");uint32_t fs=(uint32_t)fast_shift;ck(cudaMemcpyToSymbol(D_FAST_CORR,&fastD[d],sizeof(fastD[d])),"fast corr ptr");ck(cudaMemcpyToSymbol(D_FAST_SHIFT,&fs,sizeof(fs)),"fast shift");ck(cudaMemcpyToSymbol(D_FAST_MASK,&fast_mask,sizeof(fast_mask)),"fast mask");}
    ...
    double wall=std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0).count();Code fg=rank_full(MateID(R),W);int fo=int(fg/mc);Count ans=0;cudaSetDevice(fo);ck(cudaMemcpy(&ans,mp[fo]+(fg-Code(fo)*mc),sizeof(ans),cudaMemcpyDeviceToHost),"answer");ans%=mod;double mx=0,sum=0;size_t maxIntervals=0;for(auto&c:ctx){mx=std::max(mx,c.active);sum+=c.active;maxIntervals=std::max(maxIntervals,c.maxIntervals);std::cerr<<"gpu "<<c.dev<<" groups="<<c.groups<<" active_s="<<c.active<<" max_intervals="<<c.maxIntervals<<"\n";}std::cout<<"backend=gridfp-b300-hbm32-fastmod-dynamic n="<<n<<" residue="<<ans<<" modulus="<<mod<<" gpus="<<ng<<" peers="<<peers<<" main_states="<<mainN<<" blocked_states="<<blockN<<" auth_gib="<<double(mainN+blockN)*sizeof(Count)/(1ull<<30)<<" auth_per_gpu_gib="<<double(mainN+blockN)*sizeof(Count)/ng/(1ull<<30)<<" scratch_target_mib="<<effective_target_mib<<" scratch_requested_mib="<<target_mib<<" reserve_mib="<<reserve_mib<<" windows="<<total_windows<<" max_groups="<<maxgroups<<" max_intervals="<<maxIntervals<<" active_max_s="<<mx<<" active_sum_s="<<sum<<" prepare_s="<<prepare_s<<" wall_s="<<wall<<"\n";
    for(auto&c:ctx)c.destroy();for(int d=0;d<ng;++d){cudaSetDevice(d);if(mp[d])cudaFree(mp[d]);if(bp[d])cudaFree(bp[d]);if(lowE[d])cudaFree(lowE[d]);if(lowO[d])cudaFree(lowO[d]);if(lowR[d])cudaFree(lowR[d]);if(highEM[d])cudaFree(highEM[d]);if(highEB[d])cudaFree(highEB[d]);if(highOM[d])cudaFree(highOM[d]);if(highOB[d])cudaFree(highOB[d]);if(fastD[d])cudaFree(fastD[d]);}
}
